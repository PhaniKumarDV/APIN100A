...
-------------------------------------------------------------------------------
  ...
  $Header: //depot/asic/ipq8066/IPQ8064.ILQ.4.0_P/meta/main/latest/common/build/update_common_info.py#6 $
  $DateTime: 2016/10/12 04:47:59 $
...
if FIT_tool:
   ...
   os.rename("./ipq_p/nor-system-partition-v2.bin", "./ipq_p/nor-system-partition.bin")
   os.rename("./ipq_p_debug/nor-system-partition-v2.bin", "./ipq_p_debug/nor-system-partition.bin")
   os.rename("./ipq_p_gcc5/nor-system-partition-v2.bin", "./ipq_p_gcc5/nor-system-partition.bin")  

   if os.path.isfile("./ipq/ubinize"):
      shutil.copy("./ipq/ubinize", "./ipq_p/")
      shutil.copy("./ipq/ubinize", "./ipq_p_debug/")  
      shutil.copy("./ipq/ubinize", "./ipq_p_gcc5/")
   
   shutil.rmtree('./ipq')
   tools_dir = './ipq_p/tools'
   distutils.dir_util.copy_tree( exported_tools ,tools_dir)	

   tools_dir = './ipq_p_debug/tools'
   distutils.dir_util.copy_tree( exported_tools ,tools_dir)
   
   tools_dir = './ipq_p_gcc5/tools'
   distutils.dir_util.copy_tree( exported_tools ,tools_dir)
   
   os.rename("./ipq_p_gcc5", "./ipq")
   ipq_p_gcc5_dir = './ipq'
   lg.log("update_common_info: Generate ipq single image for gcc5")
   ret_nand_p_gcc = lg.log_exec(['python',pack_tool,'-t','nand','-B', '-F','boardconfig','-o', bin_dir +'/'+'nand-ipq806x-p-gcc5-single.img', '-M', 'build_its.sh',ipq_p_gcc5_dir],verbose=0)
   print "ret_nand_p_gcc %d." % ret_nand_p_gcc
   ret_nor_p_gcc = lg.log_exec(['python',pack_tool,'-t','nor','-B', '-F','boardconfig','-o', bin_dir +'/'+'nor-ipq806x-p-gcc5-single.img', '-M', 'build_its.sh',ipq_p_gcc5_dir],verbose=0)
   print "ret_nor_p_gcc %d." % ret_nor_p_gcc
   os.rename("./ipq", "./ipq_p_gcc5")

   os.rename("./ipq_p_debug", "./ipq")
   ipq_p_debug_dir = './ipq'   
   lg.log("update_common_info: Generate ipq single image for debug")   
   ret_nand_P_debug = lg.log_exec(['python',pack_tool,'-t','nand','-B', '-F','boardconfig','-o', bin_dir +'/'+'nand-ipq806x-p-debug-single.img', '-M', 'build_its.sh',ipq_p_debug_dir],verbose=0)
   print "ret_nand_P_debug %d." % ret_nand_P_debug
   ret_nor_P_debug = lg.log_exec(['python',pack_tool,'-t','nor','-B', '-F','boardconfig','-o', bin_dir +'/'+'nor-ipq806x-p-debug-single.img', '-M', 'build_its.sh',ipq_p_debug_dir],verbose=0)
   print "ret_nor_P_debug %d." % ret_nor_P_debug
   
   # Streamboost3 binary generation
   ret_streamboost_debug = 0
   if os.path.isfile("./ipq/openwrt-ipq806x-streamboost3-squashfs-rootfs.img"):
		...
		ret_streamboost_debug = lg.log_exec(['python',pack_tool,'-t','nand','-B', '-F','boardconfig','-o', bin_dir +'/'+'nand-ipq806x-debug-single-streamboost.img', '-M', 'build_its.sh',ipq_p_debug_dir],verbose=0)
		print "ret_streamboost_debug %d." % ret_streamboost_debug
   
   os.rename("./ipq", "./ipq_p_debug")

   os.rename("./ipq_p", "./ipq")
   ipq_dir = './ipq'
   lg.log("update_common_info: Generate ipq single image")    
   ret_nand_single = lg.log_exec(['python',pack_tool,'-t','nand','-B', '-F','boardconfig','-o', bin_dir +'/'+'nand-ipq806x-single.img', '-M', 'build_its.sh',ipq_dir],verbose=0)
   print "ret_nand_single %d." % ret_nand_single
   ret_nor_single = lg.log_exec(['python',pack_tool,'-t','nor','-B', '-F','boardconfig','-o', bin_dir +'/'+'nor-ipq806x-single.img', '-M', 'build_its.sh',ipq_dir],verbose=0)
   print "ret_nor_single %d." % ret_nor_single 
   ret_norplusnand_single = lg.log_exec(['python',pack_tool,'-t','norplusnand','-B', '-F','boardconfig','-o', bin_dir +'/'+'norplusnand-ipq806x-single.img', '-M', 'build_its.sh',ipq_dir],verbose=0)
   print "ret_norplusnand_single %d." % ret_norplusnand_single
   ret_nand_apps = lg.log_exec(['python',pack_tool,'-t','nand','-B', '-F','appsboardconfig', '-o',bin_dir +'/'+'ipq806x-nand-apps.img', '-M', 'build_its.sh',ipq_dir],verbose=0)
   print "ret_nand_apps %d." % ret_nand_apps
   ret_nor_apps = lg.log_exec(['python',pack_tool,'-t','nor','-B', '-F','appsboardconfig', '-o',bin_dir +'/'+'ipq806x-nor-apps.img', '-M', 'build_its.sh',ipq_dir],verbose=0)
   print "ret_nor_apps %d." % ret_nor_apps
   ret_norplusnand_apps = lg.log_exec(['python',pack_tool,'-t','norplusnand','-B', '-F','appsboardconfig', '-o', bin_dir +'/'+'ipq806x-norplusnand-apps.img', '-M', 'build_its.sh',ipq_dir],verbose=0)
   print "ret_norplusnand_apps %d." % ret_norplusnand_apps
   ret_emmc_single = lg.log_exec(['python',pack_tool,'-t','emmc','-B', '-F','boardconfig', '-o', bin_dir +'/'+'emmc-ipq806x-single.img', '-M', 'build_its.sh',ipq_dir],verbose=0)
   print "ret_emmc_single %d." % ret_emmc_single
   ret_norplusemmc_single = lg.log_exec(['python',pack_tool,'-t','norplusemmc','-B', '-F','boardconfig', '-o', bin_dir +'/'+'norplusemmc-ipq806x-single.img', '-M', 'build_its.sh',ipq_dir],verbose=0)
   print "ret_norplusemmc_single %d." % ret_norplusemmc_single
   ret_norplusemmc_apps = lg.log_exec(['python',pack_tool,'-t','norplusemmc','-B', '-F','appsboardconfig', '-o', bin_dir +'/'+'ipq806x-norplusemmc-apps.img', '-M', 'build_its.sh',ipq_dir],verbose=0)
   print "ret_norplusemmc_apps %d." % ret_norplusemmc_apps
   ret_emmc_apps = lg.log_exec(['python',pack_tool,'-t','emmc','-B', '-F','appsboardconfig', '-o', bin_dir +'/'+'ipq806x-emmc-apss.img', '-M', 'build_its.sh',ipq_dir],verbose=0)
   ...
   if os.path.isfile("./ipq/openwrt-ipq806x-streamboost3-squashfs-rootfs.img"):
		os.rename("./ipq/openwrt-ipq806x-streamboost3-squashfs-rootfs.img", "./ipq/openwrt-ipq806x-squashfs-root.img")
		os.rename("./ipq/openwrt-ipq806x-ipq40xx-ubi-root-streamboost3-enabled.img", "./ipq/openwrt-ipq806x-ipq40xx-ubi-root.img")
		ret_streamboost = lg.log_exec(['python',pack_tool,'-t','nand','-B', '-F','boardconfig','-o', bin_dir +'/'+'nand-ipq806x-single-streamboost.img', '-M', 'build_its.sh',ipq_dir],verbose=0)
		print "ret_streamboost %d." % ret_streamboost
	  
   if ret_streamboost != 0 or ret_emmc_apps !=0 or ret_norplusemmc_apps !=0 or ret_norplusemmc_single !=0 or ret_emmc_single !=0 or ret_norplusnand_apps !=0 or ret_nor_apps!=0 or ret_nand_apps !=0 or ret_norplusnand_single !=0 or ret_nor_P_debug !=0 or ret_nor_single !=0 or ret_nand_P_debug !=0 or ret_nand_single !=0 or ret_nand_p_gcc !=0 or ret_nor_p_gcc !=0 or ret_streamboost_debug !=0:

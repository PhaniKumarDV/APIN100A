...
-------------------------------------------------------------------------------
  ...
  $Header: //depot/asic/ipq8066/IPQ4019.ILQ.4.0_P/meta/main/latest/common/build/update_common_info_standard.py#2 $
  $DateTime: 2016/09/02 14:10:58 $
...
if FIT_tool:
   ...
   ipq_s_gcc5_dir = './ipq_s_gcc5'
   if not os.path.exists(ipq_s_gcc5_dir):
      os.makedirs(ipq_s_gcc5_dir,mode = 0755)
   ...
   for flash_file in mi.get_files(attr='flash_file'):
      shutil.copy(flash_file, ipq_s_gcc5_dir)
   ...
   for flash_file in mi.get_files(attr='flash_file_s_gcc'):
      shutil.copy(flash_file, ipq_s_gcc5_dir)
   ...
   for flash_standard_conf_debug_file in mi.get_files(attr='flash_standard_conf_debug_file'):
	  ...
	  # since this is standard build also copy all files has attribute flash_standard_conf_file to ipq folder
   for flash_standard_conf_file in mi.get_files(attr='flash_standard_conf_file'):
      shutil.copy(flash_standard_conf_file, ipq_dir)

   for flash_standard_conf_file in mi.get_files(attr='flash_standard_conf_file'):
      shutil.copy(flash_standard_conf_file, ipq_s_gcc5_dir)

	  # since this is standard build also copy all files has attribute flash_standard_conf_debug_file to ipq_s_debug_dir folder
   for flash_standard_conf_debug_file in mi.get_files(attr='flash_standard_conf_debug_file'):
      shutil.copy(flash_standard_conf_debug_file, ipq_s_debug_dir)
  ...
  # copy all files has attribute QPST_file to uppermost meta folder
   ...
   os.rename("./ipq_s_gcc5/nor-system-partition-ipq40xx-s.bin", "./ipq_s_gcc5/nor-system-partition-ipq40xx.bin")

   if os.path.isfile("./ipq_s_gcc5/openwrt-ipq806x-qcom-ipq8064-ap148-ubi-root.img"):
	  os.rename("./ipq_s_gcc5/openwrt-ipq806x-qcom-ipq8064-ap148-ubi-root.img", "./ipq_s_gcc5/ipq806x-ubi-root.img")
   ...
   lg.log_exec(['python',pack_tool,'-t','nor','-B', '-F','boardconfig_standard','-o', bin_dir +'/'+'nor-ipq40xx-standard-single.img',ipq_dir],verbose=0)
   lg.log_exec(['python',pack_tool,'-t','nor','-B', '-F','appsboardconfig_standard','-o', bin_dir +'/'+'nor-ipq40xx-standard-apps.img',ipq_dir],verbose=0)
   lg.log_exec(['python',pack_tool,'-t','nor','-B', '-F','boardconfig_standard','-o', bin_dir +'/'+'nor-ipq40xx-standard-s-gcc5-single.img',ipq_s_gcc5_dir],verbose=0)
   lg.log_exec(['python',pack_tool,'-t','nor','-B', '-F','appsboardconfig_standard','-o', bin_dir +'/'+'nor-ipq40xx-standard-s-gcc5-apps.img',ipq_s_gcc5_dir],verbose=0)

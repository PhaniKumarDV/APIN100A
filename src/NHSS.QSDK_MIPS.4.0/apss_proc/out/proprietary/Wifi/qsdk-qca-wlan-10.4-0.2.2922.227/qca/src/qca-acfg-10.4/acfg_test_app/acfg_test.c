        ...
        acfg_chan_start_t *chan)
{
    ...
    if(chan->reason == ACFG_CHAN_CHANGE_DFS){
        sprintf((char *)buf, "%s: DFS channel change\n", ifname);
    }else{
        sprintf((char *)buf, "%s: Normal channel change\n", ifname);
    }
    acfg_logger(buf);
    sprintf((char *)buf, "%s: New Chan %d\n", ifname, chan->freq);
    acfg_logger(buf);
        ...
        acfg_radar_t *radar)
{
    ...
    for (i = 0; i < radar->count; i++) {
        sprintf((char *)buf, "%s: RADAR. Chan %d unusable\n", ifname, radar->freqs[i]);
        acfg_logger(buf);
    }
    return QDF_STATUS_SUCCESS ;
}
...
cb_tx_overflow(uint8_t * ifname)
{
    ...
    sprintf((char *)buf, "%s: TX overflow! \n", ifname);
    acfg_logger(buf);
                   ...
                   acfg_gpio_t *gpio)
{
    ...
    sprintf((char *)buf, "%s: GPIO pin %d changed state\n", ifname, gpio->num);
    acfg_logger(buf);
                   ...
                   acfg_nf_dbr_dbm_t *nf_dbr_dbm)
{
    ...
    sprintf((char *)buf, "%s: nfdBr\tnfdBm", ifname);
    acfg_logger(buf);
    for(j=0; j<4; j++)
    {
        sprintf((char *)buf, "Freq = %d", nf_dbr_dbm->freqNum[j]);
        acfg_logger(buf);
        for(i=0;i<4;i++)
        {
            sprintf((char *)buf, "%d\t%d", nf_dbr_dbm->nfdbr[4*j+i], nf_dbr_dbm->nfdbm[4*j+i]);
            acfg_logger(buf);
        }
    }
                   ...
                   acfg_packet_power_t *power_info)
{
    uint8_t buf[128];
    uint32_t status = QDF_STATUS_SUCCESS;
    sprintf((char *)buf, "%s: packet power (dBm) = %f",ifname,power_info->packet_power/2.0);
    acfg_logger(buf);
    return status;
}
...
cb_wdt_event(uint8_t * ifname, acfg_wdt_event_t * wdt_event)
{
    ...
    if(wdt_event->reason==ACFG_WDT_TARGET_ASSERT){
        sprintf((char *)buf, "%s: Watchdog event: target assert!\n", ifname);
        acfg_logger(buf);
    }else if(wdt_event->reason==ACFG_WDT_REINIT_DONE){
        sprintf((char *)buf, "%s: Watchdog event: re-init done!\n", ifname);
        acfg_logger(buf);
    }
...
cb_cac_start_event(uint8_t * ifname)
{
    ...
    sprintf((char *)buf, "%s: DFS CAC timeout \n", ifname);
    acfg_logger(buf);
...
cb_up_after_cac_event(uint8_t * ifname)
{
    ...
    sprintf((char *)buf, "%s: VAP up after CAC\n", ifname);
    acfg_logger(buf);
        ...
        acfg_diag_event_t *diag)
{
    ...
    sprintf((char *)buf, "\n==================================================");
    acfg_logger(buf);
    sprintf((char *)buf, "%s:Diagnostics", ifname);
    acfg_logger(buf);

    if(diag->type & ACFG_DIAGNOSTICS_WARNING) {
        sprintf((char *)buf, "Type: WARNING");
        acfg_logger(buf);
    } else if(diag->type & ACFG_DIAGNOSTICS_ERROR) {
        sprintf((char *)buf, "Type: ERROR");
        acfg_logger(buf);
    } else if(diag->type & ACFG_DIAGNOSTICS_NORMAL) {
        sprintf((char *)buf, "Type: NORMAL");
        acfg_logger(buf);
    }

    if(diag->status & ACFG_DIAGNOSTICS_DOWN) {
	sprintf((char *)buf, "Status: Below threshold");
	acfg_logger(buf);
    } else if(diag->status & ACFG_DIAGNOSTICS_DOWN_15_SEC) {
	sprintf((char *)buf, "Status: Below threshold for more than 15secs");
	acfg_logger(buf);
    } else if(diag->status & ACFG_DIAGNOSTICS_DOWN_1_HR) {
	sprintf((char *)buf, "Status: Below threshold for more than 1hr");
	acfg_logger(buf);
    } else {
        sprintf((char *)buf, "Status: Above threshold");
	acfg_logger(buf);
    }

   if (diag->data_rate_threshold) {
        sprintf((char *)buf, "Configured datarate threshold: %d", diag->data_rate_threshold);
        acfg_logger(buf);
    }

    sprintf((char *)buf, "Timestamp: %s", diag->tstamp);
    acfg_logger(buf);

    sprintf((char *)buf, "Client mode: %s", diag->mode);
    acfg_logger(buf);

    sprintf((char *)buf,"Client MAC: %02x:%02x:%02x:%02x:%02x:%02x",
            diag->macaddr[0],  diag->macaddr[1],
            diag->macaddr[2],  diag->macaddr[3],
            diag->macaddr[4],  diag->macaddr[5]);
    acfg_logger(buf);

    sprintf((char *)buf, "Power level: %d", diag->power_level);
    acfg_logger(buf);

    sprintf((char *)buf, "Rssi: %d", diag->rssi);
    acfg_logger(buf);

    sprintf((char *)buf, "Channel index: %d", diag->channel_index);
    acfg_logger(buf);

    sprintf((char *)buf, "Current Tx data rate: %d", diag->tx_data_rate);
    acfg_logger(buf);

    if (diag->mcs_index != 0xFF) {
        sprintf((char *)buf, "Last Tx pkt MCS index: %d", diag->mcs_index);
        acfg_logger(buf);
    } else {
        sprintf((char *)buf, "Last Tx pkt MCS index: NA(Legacy datarate)");
        acfg_logger(buf);
    }

    sprintf((char *)buf, "Last Tx Channel width: %d", diag->chan_width);
    acfg_logger(buf);

    if(diag->sgi) {
        sprintf((char *)buf, "Last Tx Guard Interval: Short");
        acfg_logger(buf);
    } else {
        sprintf((char *)buf, "Last Tx Guard Interval: Long");
        acfg_logger(buf);
    }

    sprintf((char *)buf, "Last Tx pkt no of spatial streams: %d", diag->nss);
    acfg_logger(buf);

    if (diag->airtime != 0xFF) {
        sprintf((char *)buf, "Airtime: %d percentage", diag->airtime);
        acfg_logger(buf);
    }

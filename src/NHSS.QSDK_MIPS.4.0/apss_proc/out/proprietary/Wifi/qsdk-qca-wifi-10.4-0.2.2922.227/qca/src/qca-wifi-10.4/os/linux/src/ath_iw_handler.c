    ...
    for (i = 0; i < HAL_NUM_TX_QUEUES; i++) {
        ...
        TAILQ_FOREACH(ac, &txq->axq_acq, ac_qelem) {
            ...
            if (tid) {
                ...
                printk("   ac_q=%d, ac=0x%p, hw_q=%d, node=%p, an_flags=0x%x, sched=%d, hwqcnt=%d, ",
                    ac->qnum, ac, i, ni, tid->an->an_flags, ac->sched, ac->hwqcnt);
            } else {
                printk("   ac_q=%d, ac=0x%p, hw_q=%d, node=<null>, sched=%d, hwqcnt=%d, ",
    ...
    for (i = 0; i < HAL_NUM_TX_QUEUES; i++) {
        ...
        TAILQ_FOREACH(ac, &txq->axq_acq, ac_qelem) {
            TAILQ_FOREACH_SAFE(tid, &ac->tid_q, tid_qelem, next_tid) {
                ...
                printk("   vap=%d, tidno=%d, tid=0x%p, node=%p, an_flags=0x%x, ac_q=%d, ",
                    tvap->iv_unit, tid->tidno, tid,
                    ni, tid->an->an_flags, ac->qnum);
                printk("depth=%d, num_mcast_buf=%d, sched=%d, paused=%d, min_depth=%d\n",

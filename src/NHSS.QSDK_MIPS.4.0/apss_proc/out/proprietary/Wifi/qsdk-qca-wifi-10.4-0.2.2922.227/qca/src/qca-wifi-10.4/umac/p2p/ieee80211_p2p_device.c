...
wlan_p2p_create(wlan_dev_t dev_handle, wlan_p2p_params *p2p_params)
{
    ...
    if (p2p_params) {
        if (is_dev_type_cat_id_valid(p2p_params->pri_dev_type)) {
            ...
            ieee80211com_note(dev_handle, "%s: Primary Device Type: cat=%x %x, subcat=%x %x, oui=%x %x %x %x\n",
        ...
        if (is_dev_type_cat_id_valid(p2p_params->sec_dev_type[0])) {
            ...
            if (sec_dev_type_list == NULL) {
                ieee80211com_note(dev_handle, "%s p2p secondary device type list allocation failed. size=%d\n",
                                  __func__, (IEEE80211_P2P_DEVICE_TYPE_LEN * IEEE80211_P2P_SEC_DEVICE_TYPES));
            ...
            while (is_dev_type_cat_id_valid(p2p_params->sec_dev_type[p2p_handle->num_sec_dev_types]))
            {
                ...
                ieee80211com_note(dev_handle, "%s: Secondary Device Type[%d]: cat=%x %x, subcat=%x %x, oui=%x %x %x %x\n",
...
wlan_p2p_set_primary_dev_type(wlan_p2p_t p2p_handle, u_int8_t *pri_dev_type)
{
    ...
    if (is_dev_type_cat_id_valid(pri_dev_type)) {
        ...
        ieee80211com_note(dev_handle, "%s: ERROR: invalid primary Device Type: cat=%x %x, subcat=%x %x, oui=%x %x %x %x\n",
...
wlan_p2p_set_secondary_dev_type(wlan_p2p_t p2p_handle, int num_dev_types, u_int8_t *sec_dev_type)
{
    ...
    if (is_dev_type_cat_id_valid(sec_dev_type)) {
        ...
        ieee80211com_note(dev_handle, "%s: ERROR: invalid first secondary Device Type: cat=%x %x, subcat=%x %x, oui=%x %x %x %x\n",

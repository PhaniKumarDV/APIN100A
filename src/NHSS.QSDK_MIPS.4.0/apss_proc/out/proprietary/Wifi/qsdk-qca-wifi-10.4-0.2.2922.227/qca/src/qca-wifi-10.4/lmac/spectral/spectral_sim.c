                              ...
                              bool is_80_80)
{
    ...
    switch(width)
    {
        case IEEE80211_CWM_WIDTH20:
            ...
            if (report->data == NULL) {
                printk("Spectral simulation: Could not allocate memory for "
                       "report data\n");
                goto bad;
            }
        ...
        case IEEE80211_CWM_WIDTH160:
            ...
            break;
        default:
            printk("Unhandled width. Please correct. Asserting\n");
            qdf_assert_always(0);
    }
                              ...
                              bool is_80_80)
{
    ...
    switch(width)
    {
        ...
        case IEEE80211_CWM_WIDTH160:
            ...
            break;
        default:
            printk("Unhandled width. Please rectify.\n");
            qdf_assert_always(0);
    };
...
static int populate_reportset_fromfile(ath_spectralsim_reportset *reportset,
                              ...
                              bool is_80_80)
{
    printk("%s: To be implemented if required\n", __func__);
...
int spectral_sim_attach(struct ath_spectral *spectral)
{
    ...
    if (simctx == NULL) {
        printk("Spectral simulation: Could not allocate memory for context\n");
        return -1;
    }
    ...
    if (populate_simdata(simctx) != 0) {
        ...
        printk("Spectral simulation attach failed\n");
        return -1;
    }
    ...
    printk("Spectral simulation attached\n");
...
void spectral_sim_detach(struct ath_spectral *spectral)
{
    ...
    printk("Spectral simulation detached\n");
}
...
u_int32_t spectral_sim_start_spectral_scan(void* arg)
{
    ...
    if (simctx->curr_reportset == NULL) {
        printk("Spectral simulation: No current report set configured  - unable "
               "to start simulated Spectral scan\n");
        return 0;
    }

    if (simctx->curr_reportset->curr_report == NULL) {
         printk("Spectral simulation: No report data instances populated - "
...
u_int32_t spectral_sim_configure_params(void* arg, HAL_SPECTRAL_PARAM* params)
{
...
#ifdef SPECTRAL_SIM_DUMP_PARAM_DATA
    {
        ...
        printk("Spectral simulation: Param data dump:\n"
               "ss_fft_period=%hu\n"
               "ss_period=%hu\n"
               "ss_count=%hu\n"
               "ss_short_report=%hu\n"
        ...
        {
            printk("ss_nf_cal[%d]=%hhd\n",i,params->ss_nf_cal[i]);
        }

        for (i = 0; i < AH_MAX_CHAINS * 2; i++)
        {
            printk("ss_nf_pwr[%d]=%hhd\n",i,params->ss_nf_pwr[i]);
        }
    ...
    if (vap == NULL) {
        printk("Spectral simulation: No VAPs found - not proceeding with param "
               "config.\n");
        return 0;
    }
    ...
    switch(bw)
    {
        ...
        case IEEE80211_CWM_WIDTH160:
            ...
            if (phymode == IEEE80211_MODE_11AC_VHT160) {
                ...
                printk("Spectral simulation: Unexpected PHY mode %u found for "
         ...
         case IEEE80211_CWM_WIDTHINVALID:
            printk("Spectral simulation: Invalid width configured - not "
                   "proceeding with param config.\n");
            is_invalid_width = true;
         default:
            printk("Spectral simulation: Unknown width %u...asserting\n", bw);
            qdf_assert_always(0);
            break;
    }
    ...
    if (des_headreportset == NULL) {
        printk("Spectral simulation: No simulation data present for configured "
    ...
    if (simctx->curr_reportset == NULL) {
        printk("Spectral simulation: No simulation data present for desired "
...
u_int32_t spectral_sim_get_params(void* arg, HAL_SPECTRAL_PARAM* params)
{
    ...
    if (simctx->curr_reportset == NULL) {
        printk("Spectral simulation: No configured reportset found.\n");
        return 0;
    }

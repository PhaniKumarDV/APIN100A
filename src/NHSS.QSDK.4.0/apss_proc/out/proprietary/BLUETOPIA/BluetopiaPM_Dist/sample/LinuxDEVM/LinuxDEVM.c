...
static Event_t             RoleSwitchModeChangeEvent;
...
static int RequestRoleSwitch(ParameterList_t *TempParam);
...
static void UserInterface(void)
{
   ...
   AddCommand("REQUESTROLESWITCH", RequestRoleSwitch);
...
static int RequestRoleSwitch(ParameterList_t *TempParam)
{
   ...
   int                             ret_val;
   BD_ADDR_t                       BD_ADDR;
   DEVM_Remote_Device_Properties_t RemoteDeviceProperites;
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 2) && (TempParam->Params[0].strParam))
      {
         ...
         /* Check if we need to come out of sniff.                      */
         if((TempParam->NumberofParameters >= 3) && (TempParam->Params[2].intParam))
         {
            /* Check if we are already active.                          */
            if(!(ret_val = DEVM_QueryRemoteDeviceProperties(BD_ADDR, 0, &RemoteDeviceProperites)))
            {
               if(RemoteDeviceProperites.RemoteDeviceFlags & DEVM_REMOTE_DEVICE_FLAGS_DEVICE_LINK_CURRENTLY_SNIFF_MODE)
               {
                  /* We need to bring the device out of sniff. Create an*/
                  /* event to wait on for the mode change event.        */
                  RoleSwitchModeChangeEvent = BTPS_CreateEvent(FALSE);

                  if(RoleSwitchModeChangeEvent)
                  {
                     ret_val = DEVM_SetRemoteDeviceLinkActive(BD_ADDR);

                     if(!ret_val)
                     {
                        /* Wait for the device to come out of sniff.    */
                        printf("Waiting for the Mode Change event.\n");

                        if(!BTPS_WaitEvent(RoleSwitchModeChangeEvent, 5000))
                        {
                           printf("Timed out waiting for Mode Change\n");
                        ...
                        printf("DEVM_SetRemoteDeviceLinkActive() Error: %d, %s\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));

                     BTPS_CloseEvent(RoleSwitchModeChangeEvent);
                     RoleSwitchModeChangeEvent = NULL;
                  }
                  else
                  {
                  ...
                  printf("Device not in sniff mode.\n");
            }
            else
               printf("DEVM_QueryRemoteDeviceProperties() Failure: %d, %s\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
         }
         ...
         if(!ret_val)
         {
            printf("Attempting to switch to %s for Remote Device: %s.\r\n", TempParam->Params[1].intParam?"Slave":"Master", TempParam->Params[0].strParam);

            if((Result = DEVM_RoleSwitch(BD_ADDR, TempParam->Params[1].intParam?drSlave:drMaster)) >= 0)
            {
               printf("DEVM_RoleSwitch() Success: %d.\r\n", Result);
            ...
            else
            {
               /* Error setting link for Remote Device, inform the user and*/
               /* flag an error.                                           */
               printf("DEVM_RoleSwitch() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: RequestRoleSwitch [BD_ADDR] [Role (0 - Master, 1 - Slave)] [Set Link Active (1=True) (Optional)].\r\n");
   ...
   if(RemoteDeviceProperties)
   {
      ...
      if((!UpdateMask) || (UpdateMask & DEVM_REMOTE_DEVICE_PROPERTIES_CHANGED_SNIFF_STATE))
      {
         ...
         else
         {
            ...
            if(RoleSwitchModeChangeEvent)
               BTPS_SetEvent(RoleSwitchModeChangeEvent);
         }
      }

...
#include "SPPMMSG.h"             /* BTPM SPP Manager Message Formats.         */
   ...
   /* initialization information when the Serial Port Manager Module is */
   /* initialized.                                                      */
   /* * NOTE * This structure is used with the SPPM_Initialize()        */
   /*          function to denote the initialization parameters required*/
   /*          for the module.                                          */
typedef struct _tagSPPM_Initialization_Data_t
{
   unsigned long  ConfigurationFlags;
   void          *ACPInitializationData;
} SPPM_Initialization_Data_t;
   ...
   /* SPPM_Initialization_Data_t structure.                             */
#define SPPM_INITIALIZATION_CONFIGURATION_FLAGS_SUPPORT_MFI    0x00000001
...
#define SPPM_SERVICE_RECORD_INFORMATION_SIZE                   (sizeof(SPPM_Service_Record_Information_t))
   ...
   /* The following structure is used to report details of remote       */
   /* SPP-based services and is used with the SPPM_QueryRemoteServices  */
   /* function.                                                         */
typedef struct _tagSPPM_Remote_Service_Information_t
{
   unsigned int      ServiceRecordIndex;
   unsigned int      NumberServiceClassUUIDs;
   SDP_UUID_Entry_t *ServiceClassUUIDs;
   unsigned int      PortNumber;
   char             *ServiceName;
   char             *ServiceDescription;
} SPPM_Remote_Service_Information_t;

#define SPPM_REMOTE_SERVICE_INFORMATION_SIZE                   (sizeof(SPPM_Remote_Service_Information_t))
...
#define SPPM_CLOSE_DATA_FLUSH_TIMEOUT_INFINITE                 0xFFFFFFFF
   ...
   /*          timeout is reached).                                     */
   /* * NOTE * This function will return zero if a timeout occurs and no*/
...
#define SPPM_BUFFER_SIZE_CURRENT                               0
   ...
   /*          disconnection.                                           */
BTPSAPI_DECLARATION int BTPSAPI SPPM_WaitForPortDisconnection(unsigned int ServerPort, Boolean_t IsServer, BD_ADDR_t BD_ADDR, unsigned int Timeout);

#ifdef INCLUDE_BLUETOOTH_API_PROTOTYPES
   typedef int (BTPSAPI *PFN_SPPM_WaitForPortDisconnection_t)(unsigned int ServerPort, Boolean_t IsServer, BD_ADDR_t BD_ADDR, unsigned int Timeout);
#endif

   /* The following function is provided to allow a mechanism to query  */
   /* the published service records of a remote device for all SPP-based*/
   /* services.  This function accepts as input the Bluetooth address   */
   /* of the remote device, (optionally) the size of the specified      */
   /* buffer in bytes and a buffer in which to store the remote service */
   /* information structures, and (optionally) a buffer in which to     */
   /* store the total amount of bytes necessary to store information    */
   /* for all known records.  The return value from this function will  */
   /* be the number of service information structures that that were    */
   /* copied into the specified buffer (if specified) -- zero signifies */
   /* success, but no structures copied.  A negative value represents a */
   /* return error code if there was an error.                          */
BTPSAPI_DECLARATION int BTPSAPI SPPM_QueryRemoteServices(BD_ADDR_t RemoteDevice, unsigned int BufferSize, SPPM_Remote_Service_Information_t *ServiceInfoListBuffer, unsigned int *TotalBufferSize);

#ifdef INCLUDE_BLUETOOTH_API_PROTOTYPES
   typedef int (BTPSAPI *PFN_SPPM_QueryRemoteServices_t)(BD_ADDR_t RemoteDevice, unsigned int BufferSize, SPPM_Remote_Service_Information_t *ServiceInfoListBuffer, unsigned int *TotalBufferSize);
#endif

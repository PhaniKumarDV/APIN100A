...
BTPSAPI_DECLARATION int BTPSAPI GLPM_Start_Procedure_Request(unsigned int GLPMCollectorCallbackID, BD_ADDR_t RemoteDeviceAddress, GLPM_Procedure_Data_t *ProcedureData);
   ...
   /* The following function is provided to allow a mechanism of        */
   /* stopping a previously started Glucose Procedure to a remote       */
   /* Glucose Device. This function accepts as input the GLP Manager    */
   /* Data Event Callback ID (return value from                         */
   /* GLPM_Register_Collector_Event_Callback() function), the BD_ADDR   */
   /* of the remote Glucose Device and the Procedure ID that was        */
   /* returned via a successfull call to GLPM_Start_Procedure_Request().*/
   /* This function returns zero on success or a negative error code.   */
   /* * NOTE * The getGLPMProcedureStopped event will be generated when */
   /*          the remote Glucose Device responds to the Stop Procedure */
   /*          Request.                                                 */

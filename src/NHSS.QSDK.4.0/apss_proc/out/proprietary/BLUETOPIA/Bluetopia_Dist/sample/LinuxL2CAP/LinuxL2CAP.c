...
#define UNABLE_TO_REGISTER_PSM                     (-9)  /* Denotes that an   */
                                                         /* error occurred    */
                                                         /* when trying to    */
                                                         /* register a PSM    */
                                                         /* with L2CAP.       */
...
#define DEFAULT_DEBUG_LOG_FILE_NAME  "LinuxL2CAP_ASC.log"
#define DEFAULT_DEBUG_FTS_FILE_NAME  "LinuxL2CAP_FTS.log"
...
static int                 PSMID;                   /* Variable which contains the     */
                                                    /* PSM ID of the L2CAP Server      */
                                                    /* that was opened.                */

static unsigned int        LocalCID;                /* Variable which contains the     */
                                                    /* Local Connection Identifier     */
                                                    /* (CID) of an active L2CAP        */
                                                    /* Channel.                        */
...
static int Ping(ParameterList_t *TempParam);
...
static void BTPSAPI L2CA_Event_Callback(unsigned int BluetoothStackID, L2CA_Event_Data_t *L2CA_Event_Data, unsigned long CallbackParameter);
...
static void UserInterface_Client(void)
{
   ...
   while(Result != EXIT_CODE)
   {
            ...
            if(CommandParser(&TempCommand, UserInput) >= 0)
            {
               ...
               switch(Result)
               {
                  ...
                  case EXIT_CODE:
                     ...
                     /* go ahead an close any Remote L2CAP Server       */
                     /* Channel that we have open.                      */
                     if(LocalCID)
      ...
      {
         /* Close any Remote L2CAP Server Channel that we have open.    */
         if(LocalCID)
            CloseRemoteServer(NULL);
...
static void UserInterface_Server(void)
{
   ...
   AddCommand("GETREMOTENAME", GetRemoteName);
   AddCommand("SERVICEDISCOVERY", ServiceDiscovery);
   AddCommand("REGISTERPSM", OpenServer);
   AddCommand("UNREGISTERPSM", CloseServer);
   AddCommand("WRITE", Write);
...
static int DeleteLinkKey(BD_ADDR_t BD_ADDR)
{
   ...
   /* Command Options for either L2CAP Client or L2CAP Server.  The     */
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   else
   {
      ...
      printf("*                  GetLocalAddress, GetLocalName, SetLocalName,  *\r\n");
      printf("*                  GetClassOfDevice, SetClassOfDevice,           *\r\n");
      printf("*                  GetRemoteName, ServiceDiscovery, RegisterPSM, *\r\n");
      printf("*                  UnRegisterPSM, Write, Disconnect, Ping,       *\r\n");
      printf("*                  EnableDebug, Help, Quit.                      *\r\n");
      printf("******************************************************************\r\n");
   }
...
static int ServiceDiscovery(ParameterList_t *TempParam)
{
   ...
   /* The following function is responsible for opening an L2CAP Server */
   /* on the default PSM Channel.  This function returns zero if        */
...
static int OpenServer(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      /* Make sure that there is not already an L2CAP PSM Registered.   */
      if(!PSMID)
      {
         ...
         {
            /* Simply attempt to open an L2CAP Server, on the default   */
            /* PSM value that is defined else where in this file.       */
            ret_val = L2CA_Register_PSM(BluetoothStackID, TempParam->Params[0].intParam, L2CA_Event_Callback, (unsigned long)0);

            /* If the Open was successful, then note the L2CAP Server   */
            /* PSM ID.                                                  */
            if(ret_val > 0)
            {
               /* Note the L2CAP Server PSM ID of the opened L2CAP      */
               /* Server.                                               */
               PSMID = ret_val;

               /* Simply flag to the user that everything initialized   */
               /* correctly.                                            */
               printf("L2CAP Server PSM registered successfully (PSM 0x%04X).\r\n", TempParam->Params[0].intParam);
            ...
            {
               printf("Unable to register L2CAP Server PSM 0x%04X, Error = %d.\r\n", TempParam->Params[0].intParam, ret_val);

               ret_val = UNABLE_TO_REGISTER_PSM;
            }
         }
         else
         {
            printf("Usage: RegisterPSM [PSM].\r\n");
      ...
      {
         /* A PSM is already registered, this program only supports one */
         /* Server or Client at a time.                                 */
         printf("L2CAP PSM already registered.\r\n");
   ...
   /* The following function is responsible for closing a Registered    */
   /* L2CAP Server that was previously opened via a successful call to  */
   /* the OpenServer() function.  This function returns zero if         */
...
static int CloseServer(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      /* If an L2CAP PSM is already registered, then simply close it.   */
      if(PSMID)
      {
         /* Finally close the L2CAP Server PSM.                         */
         ret_val = L2CA_Un_Register_PSM(BluetoothStackID, PSMID);

         if(ret_val < 0)
         {
            printf("L2CA_Un_Register_PSM(): Error %d.\r\n", ret_val);
         ...
         /* Flag that there is no L2CAP Server PSM currently registered.*/
         PSMID = 0;

         printf("L2CAP Server PSM un-registered.\r\n");
      }
      else
      {
         printf("There is NO L2CAP Server PSM currently registered.\r\n");
   ...
   /* with on the Local L2CAP Server.  This function returns zero if    */
...
static int Disconnect(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      /* Next, make sure that we are operating in Server Mode.          */
      if(!IsClient)
      {
         /* Next, let's make sure that a L2CAP connection is indeed     */
         /* established.                                                */
         if(LocalCID)
         {
            printf("L2CAP Channel closed successfully.\r\n");

            /* Simply close the open L2CAP Channel.                     */
            L2CA_Disconnect_Request(BluetoothStackID, (Word_t)LocalCID);

            /* Flag that there is no longer an L2CAP Channel            */
            /* established.                                             */
            LocalCID = 0;
         ...
         {
            /* Display an error to the user informing them that no L2CAP*/
            /* Channel is currently established.                        */
            printf("There is currently no L2CAP Channel connected.\r\n");
   ...
   /* with a Remote L2CAP Server.  This function returns zero if        */
   /* successful and a negative value if an error occurred.             */
static int OpenRemoteServer(ParameterList_t *TempParam)
{
   int       ret_val;
   ...
   if(BluetoothStackID)
   {
      ...
      if(IsClient)
      {
         /* Next, let's make sure that there is not an L2CAP Client     */
         /* already open.                                               */
         if(!LocalCID)
         {
            ...
            {
               /* Now let's attempt to open the Remote L2CAP Channel.   */
               Result = L2CA_Connect_Request(BluetoothStackID, InquiryResultList[(TempParam->Params[0].intParam - 1)], TempParam->Params[1].intParam, L2CA_Event_Callback, (unsigned long)0);

               if(Result > 0)
               {
                  /* Inform the user that the call to open the Remote   */
                  /* L2CAP Server was successful.                       */
                  printf("L2CA_Connect_Request() was successful.\r\n");
               ...
               else
               {
                  /* Inform the user that the call to Open the Remote   */
                  /* L2CAP Server failed.                               */
                  printf("Error calling L2CA_Connect_Request(): %d.\r\n", Result);
            ...
            {
               printf("Usage: Open [Inquiry Index] [Remote PSM].\r\n");
         ...
         {
            printf("Unable to open remote L2CAP Server.\r\nL2CAP Channel is already established.\r\n");
...
static int CloseRemoteServer(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(IsClient)
      {
         /* Next, let's make sure that a L2CAP Server connection is     */
         /* indeed established.                                         */
         if(LocalCID)
         {
            printf("L2CAP Channel closed successfully.\r\n");
         ...
         else
         {
            /* Display an error to the user informing them that no L2CAP*/
            /* Channel is currently established.                        */
            printf("You must first establish an L2CAP Channel before you can close an L2CAP Channel.\r\n");
   ...
   /* L2CAP Channel.  The string that is written is defined by the      */
   /* constant TEST_DATA (at the top of this file).  This function      */
   /* requires that a valid Bluetooth Stack ID and an L2CAP Channel     */
   /* exist before running.  This function returns zero is successful or*/
...
static int Write(ParameterList_t *TempParam)
{
   ...
   if((BluetoothStackID) && (LocalCID))
   {
      /* Simply write out the default string value (including the       */
      /* NULL terminator).                                              */
      Result = L2CA_Data_Write(BluetoothStackID, (Word_t)LocalCID, (Word_t)sizeof(TEST_DATA), (Byte_t *)TEST_DATA);
      ...
      if(Result >= 0)
      {
         ...
         printf("L2CA_Data_Write() Success: %d.\r\n", Result);
      ...
      {
         /* There was an error writing the Data to the L2CAP Channel.   */
         printf("L2CA_Data_Write() Failure: %d.\r\n", Result);
   ...
   /* The following function is responsible for attempting to Ping (via */
   /* L2CAP) a Remote Device's L2CAP Implementation.  This function     */
   /* returns zero is successful or a negative return value if there was*/
   /* an error.                                                         */
static int Ping(ParameterList_t *TempParam)
{
   int       ret_val;
   ...
   if(BluetoothStackID)
   {
      ...
      /* Bluetooth Device to Ping.                                      */
      if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].intParam) && (NumberofValidResponses) && (TempParam->Params[0].intParam <= NumberofValidResponses) && (!COMPARE_BD_ADDR(InquiryResultList[(TempParam->Params[0].intParam - 1)], NullADDR)))
      {
         /* Valid Remote Device has been specified, now let's attempt to*/
         /* Ping the Remote Device.                                     */
         Result = L2CA_Ping(BluetoothStackID, InquiryResultList[(TempParam->Params[0].intParam - 1)], (Word_t)strlen(TEST_DATA), (Byte_t *)TEST_DATA, L2CA_Event_Callback, (unsigned long)0);
         if(Result >= 0)
         {
            /* The Ping Request was submitted successfully.             */
            printf("L2CA_Ping() Success: %d.\r\n", Result);
         ...
         {
            /* There was an error attempting to submit the Ping Request.*/
            printf("L2CA_Ping() Failure: %d.\r\n", Result);
      ...
      {
         printf("Usage: Ping [Inquiry Index].\r\n");
...
static void BTPSAPI SDP_Event_Callback(unsigned int BluetoothStackID, unsigned int SDPRequestID, SDP_Response_Data_t *SDP_Response_Data, unsigned long CallbackParameter)
{
   ...
   /* The following function is for the L2CAP Event Receive Data        */
   /* Callback.  This function will be called whenever a defined L2CAP  */
   /* Action occurs within the Bluetooth Protocol Stack that is         */
   /* specified with the specified Bluetooth Stack ID.  This function   */
   /* passes to the caller the Bluetooth Stack ID, the L2CAP Event Data */
   /* associated with the L2CAP Event that occurred, and the L2CAP      */
...
static void BTPSAPI L2CA_Event_Callback(unsigned int BluetoothStackID, L2CA_Event_Data_t *L2CA_Event_Data, unsigned long CallbackParameter)
{
   ...
   /* **** SEE L2CAPAPI.H for a list of all possible event types. ***** */

   /* First, check to see if the required parameters appear to be       */
   /* semi-valid.                                                       */
   if((BluetoothStackID) && (L2CA_Event_Data))
   {
      switch(L2CA_Event_Data->L2CA_Event_Type)
      {
         case etConnect_Indication:
            /* Someone is trying to Connect to us, so let's accept the  */
            /* Request by issuing a Connect Response.                   */
            BD_ADDR    = L2CA_Event_Data->Event_Data.L2CA_Connect_Indication->BD_ADDR;
            CID        = L2CA_Event_Data->Event_Data.L2CA_Connect_Indication->LCID;
            Identifier = L2CA_Event_Data->Event_Data.L2CA_Connect_Indication->Identifier;
            ...
            if(Index == NumberofValidResponses)
            {
               if(NumberofValidResponses == MAX_INQUIRY_RESULTS)
                  InquiryResultList[NumberofValidResponses - 1] = BD_ADDR;
               else
                  InquiryResultList[NumberofValidResponses++] = BD_ADDR;
            }

            /* We will only accept the connection if we do not already  */
            /* have a client installed.                                 */
            if(!LocalCID)
            {
               Result = L2CA_Connect_Response(BluetoothStackID, BD_ADDR, Identifier, CID, L2CAP_CONNECT_RESPONSE_RESPONSE_SUCCESSFUL, 0);
               if(!Result)
               {
                  /* Connect Response was issued successfully, so let's */
                  /* inform the user of what has taken place so far.    */

                  /* Let's note the Local CID of the Connection.        */
                  LocalCID = CID;

                  printf("     Connection Accepted for CID: 0x%04X.\r\n", CID);

                  printf("     Sending Config Request.\r\n");

                  /* Next, let's start the L2CAP Configuration Process  */
                  /* by issuing an L2CAP Configuration Request.  Note   */
                  /* that we are simply accepting the Default parameters*/
                  /* for an L2CAP Connection (i.e.  NO Negotiation is   */
                  /* being performed).                                  */
                  ConfigRequest.Option_Flags = 0;

                  Result = L2CA_Config_Request(BluetoothStackID, CID, L2CAP_LINK_TIMEOUT_MAXIMUM_VALUE, &ConfigRequest);
                  if(Result)
                  {
                     /* Config Request Error, so let's issue an error to*/
                     /* the user and flag that there is no longer a     */
                     /* connection present.                             */
                     printf("     Config Request: Function Error %d.\r\n", Result);

                     /* Flag that there is no longer a valid connection.*/
                     LocalCID = 0;
                  }
               }
               else
               {
                  /* Connect Response Error, so let's inform the user.  */
                  printf("     Connect Response: Function Error %d.\r\n", Result);
               }
            }
            else
            {
               /* Inform the user that a Connection Request has occurred*/
               /* and that we are going to reject it because there is   */
               /* already an L2CAP Connection (this program will only   */
               /* support one L2CAP Connection at a time).              */
               printf("Attempting to Reject the Connection Request because this program only supports one simulataneous L2CAP Connection.\r\n");
               Result = L2CA_Connect_Response(BluetoothStackID, BD_ADDR, Identifier, CID, L2CAP_CONNECT_RESPONSE_RESPONSE_REFUSED_NO_RESOURCES, L2CAP_CONNECT_RESPONSE_STATUS_NO_FURTHER_INFORMATION);
               if(!Result)
               {
                  printf("     Connection Rejected for CID: 0x%04X.\r\n", CID);
               }
               else
               {
                  /* Connect Response Error, so let's inform the user.  */
         ...
         case etConnect_Confirmation:
            /* We have received a Positive or Negative Connection       */
            /* Response from a Connection Request that we issued        */
            /* earlier.                                                 */
            CID = L2CA_Event_Data->Event_Data.L2CA_Connect_Confirmation->LCID;

            printf("\r\n");
            printf("\r\nConnection Confirmation Received.\r\n");

            /* Let's determine if the Connection was accepted by the    */
            /* Remote side.                                             */
            if(L2CA_Event_Data->Event_Data.L2CA_Connect_Confirmation->Result == L2CAP_CONNECT_RESULT_CONNECTION_SUCCESSFUL)
            {
               /* Connection accepted, so inform the user.              */
               printf("     Connection Confirmed for CID: 0x%04X.\r\n", CID);

               printf("     Sending Config Request.\r\n");

               /* For demonstration purposes, let's issue a Config      */
               /* Request (required at this point) with a Configuration */
               /* Option (not required).                                */
               ConfigRequest.Option_Flags = L2CA_CONFIG_OPTION_FLAG_FLUSH_TIMEOUT;
               ConfigRequest.OutFlushTO   = 1000;

               Result = L2CA_Config_Request(BluetoothStackID, CID, L2CAP_LINK_TIMEOUT_MAXIMUM_VALUE, &ConfigRequest);
               if(Result)
               {
                  /* Error issuing Config Request, so let's inform the  */
                  /* user and flag that there is no longer a connection */
                  /* present.                                           */
                  printf("     Config Request: Function Error %d.\r\n", Result);
            ...
            {
               /* Check to see if the Remote side has requested that the*/
               /* Connect Response be pended (until some future point in*/
               /* time).                                                */
               if(L2CA_Event_Data->Event_Data.L2CA_Connect_Confirmation->Result == L2CAP_CONNECT_RESULT_CONNECTION_PENDING)
               {
                  printf("     Connect Confirmation Pending (CID: 0x%04X, Status: 0x%04X).\r\n", CID,
                                                                                                 L2CA_Event_Data->Event_Data.L2CA_Connect_Confirmation->Status);
               }
               else
               {
                  /* Remote side Rejected the Connect Request for some  */
                  /* reason, so let's inform the user and flag that     */
                  /* there is no longer a connection present.           */
                  printf("     Connect Confirmation Neg (CID: 0x%04X, Result: 0x%04X).\r\n", CID,
                                                                                             L2CA_Event_Data->Event_Data.L2CA_Connect_Confirmation->Result);
                  /* Flag that there is no longer a valid connection.   */
                  LocalCID = 0;
               }
            }
         ...
         case etConfig_Indication:
            /* Remote Side has issued an L2CAP Connect Request and we   */
            /* are being notified of it.                                */
            CID = L2CA_Event_Data->Event_Data.L2CA_Config_Indication->LCID;

            printf("\r\n");
            printf("\r\nConfig Indication Received.\r\n");

            /* Let's simply accept whatever Configuration option they   */
            /* have sent us and issue a Configuration Response.         */
            printf("     Sending Config Response.\r\n");

            ConfigResponse.Option_Flags = 0;
            Response                    = L2CAP_CONFIGURE_RESPONSE_RESULT_SUCCESS;

            /* Check to see if there is a request for Non-Basic Mode.   */
            if(L2CA_Event_Data->Event_Data.L2CA_Config_Indication->Option_Flags & L2CA_CONFIG_OPTION_FLAG_MODE_INFO)
            {
               /* Echo back the Mode Option Information.                */
               ConfigResponse.Option_Flags  = L2CA_CONFIG_OPTION_FLAG_MODE_INFO;
               ConfigResponse.ModeInfo.Mode = L2CAP_MODE_INFO_MODE_BASIC;
               if(L2CA_Event_Data->Event_Data.L2CA_Config_Indication->ModeInfo.Mode != L2CAP_MODE_INFO_MODE_BASIC)
               {
                  /* Unacceptable Configuration Parameter suggesting    */
                  /* Basic Mode.                                        */
                  printf("     Config Response: Unacceptable Paramerer.\r\n");
                  Response = L2CAP_CONFIGURE_RESPONSE_RESULT_FAILURE_UNACCEPTABLE_PARAMETERS;
               }
            }

            Result = L2CA_Config_Response(BluetoothStackID, CID, Response, &ConfigResponse);
            if(Result)
            {
               /* Error Issuing Configuration Response so inform the    */
               /* user and flag that the connection is no longer valid. */
               printf("     Config Response: Function Error %d.\r\n", Result);
         ...
         case etConfig_Confirmation:
            /* We have received a Configuration Confirmation from the   */
            /* remote side (in response to a Configuration Request that */
            /* we issued).                                              */
            CID = L2CA_Event_Data->Event_Data.L2CA_Config_Confirmation->LCID;

            printf("\r\n");
            printf("\r\nConfig Confirmation Received.\r\n");

            /* Inform the user of what has happened.                    */
            printf("     L2CA_Config_Confirmation (CID = 0x%04X, Result = 0x%04X).\r\n", CID, L2CA_Event_Data->Event_Data.L2CA_Config_Confirmation->Result);

            /* If the Remote side has not accepted the Connect Request  */
            /* we have sent, then let's accept whatever they have sent  */
            /* us (for demonstration purposes).  There could be more    */
            /* intense negotiation here if needed.                      */
            if(L2CA_Event_Data->Event_Data.L2CA_Config_Confirmation->Result != 0)
            {
               ConfigRequest.Option_Flags = L2CA_Event_Data->Event_Data.L2CA_Config_Confirmation->Option_Flags;
               ConfigRequest.InMTU        = L2CA_Event_Data->Event_Data.L2CA_Config_Confirmation->InMTU;
               ConfigRequest.OutFlushTO   = L2CA_Event_Data->Event_Data.L2CA_Config_Confirmation->OutFlushTO;

               Result = L2CA_Config_Request(BluetoothStackID, CID, L2CAP_LINK_TIMEOUT_MAXIMUM_VALUE, &ConfigRequest);
               if(Result)
               {
                  /* Error Issuing the Configuration Response so inform */
                  /* the user and flag that the connection is no longer */
                  /* valid.                                             */
                  printf("     Config Request: Function Error %d.\r\n", Result);
         ...
         case etData_Indication:
            /* Remote Side has sent us Data so inform the user of the   */
            /* Data.                                                    */
            CID = L2CA_Event_Data->Event_Data.L2CA_Data_Indication->CID;

            printf("\r\n");
            printf("\r\nData Indication Received.\r\n");

            printf("L2CA Data Received :(Bytes Read = %d, CID = 0x%04X).\r\n", L2CA_Event_Data->Event_Data.L2CA_Data_Indication->Data_Length, CID);

            printf("     Data: %s\r\n", L2CA_Event_Data->Event_Data.L2CA_Data_Indication->Data_Length?((char *)(L2CA_Event_Data->Event_Data.L2CA_Data_Indication->Variable_Data)):((char *)""));
         ...
         case etData_Error_Indication:
            /* L2CAP Layer has determined there was a problem sending   */
            /* or receiving data, so the L2CAP Layer is notifying us.   */
            /* Simply inform the user of what has happened.             */
            CID = L2CA_Event_Data->Event_Data.L2CA_Data_Error_Indication->CID;

            printf("\r\n");
            printf("\r\nData Error Indication Received.\r\n");

            printf("     L2CA_Data_Error_Indication: CID 0x%04X.\r\n", CID);
            ...
            if(IsClient)
               ...
               printf("Server>");
            break;
         case etEcho_Confirmation:
            /* The L2CAP Layer is notifying us of the result of a Prior */
            /* L2CAP Echo Request that we have issued.                  */
            printf("\r\n");
            printf("\r\nEcho Confirmation Received.\r\n");

            if(!L2CA_Event_Data->Event_Data.L2CA_Echo_Confirmation->Result)
            {
               /* Remote L2CAP Layer has issued L2CAP Echo Response so  */
               /* we need to inform the user.                           */
               if(L2CA_Event_Data->Event_Data.L2CA_Echo_Confirmation->Echo_Data_Length)
               {
                  L2CA_Event_Data->Event_Data.L2CA_Echo_Confirmation->Variable_Data[L2CA_Event_Data->Event_Data.L2CA_Echo_Confirmation->Echo_Data_Length] = 0;

                  printf("     L2CA_Echo_Confirmation:\r\n");

                  printf("       Data: %s\r\n", L2CA_Event_Data->Event_Data.L2CA_Echo_Confirmation->Variable_Data);
               }
               else
               {
                  printf("     L2CA_Echo_Confirmation : No Data.\r\n");
               }
            }
            else
            {
               /* L2CAP Echo Response Negative, so inform the user.     */
               printf("     L2CA_Echo_Confirmation : Result = 0x%04X.\r\n", L2CA_Event_Data->Event_Data.L2CA_Echo_Confirmation->Result);
            }
         ...
         case etTimeout_Indication:
            CID = L2CA_Event_Data->Event_Data.L2CA_Timeout_Indication->LCID;

            printf("\r\n");
            printf("\r\nTimeout Indication Received.\r\n");

            printf("     L2CA_Timeout_Indication (CID = 0x%04X).\r\n", CID);
         ...
         case etDisconnect_Confirmation:
            /* Either the remote or local side has requested termination*/
            /* of the L2CAP Link, so we need to inform the user, flag   */
            /* that the connection has been ended, and Respond to the   */
            /* Disconnect Event (really only need to respond if there   */
            /* was a Disconnect Indication).                            */
            printf("\r\n");

            if(L2CA_Event_Data->L2CA_Event_Type == etDisconnect_Indication)
            {
               CID = L2CA_Event_Data->Event_Data.L2CA_Disconnect_Indication->LCID;

               printf("\r\nDisconnect Indication Received.\r\n");
            }
            else
            {
               CID = L2CA_Event_Data->Event_Data.L2CA_Disconnect_Confirmation->LCID;

               printf("\r\nDisconnect Confirmation Received.\r\n");
            }
            ...
            printf("     Disconnected from: CID 0x%04X.\r\n", CID);

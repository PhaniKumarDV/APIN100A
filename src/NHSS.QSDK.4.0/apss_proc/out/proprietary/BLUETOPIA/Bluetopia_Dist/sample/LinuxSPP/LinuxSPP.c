...
#define DEFAULT_DEBUG_LOG_FILE_NAME  "LinuxSPP_ASC.log"
#define DEFAULT_DEBUG_FTS_FILE_NAME  "LinuxSPP_FTS.log"
...
static void UserInterface_Client(void)
{
   ...
   while(Result != EXIT_CODE)
   {
            ...
            if(CommandParser(&TempCommand, UserInput) >= 0)
            {
               ...
               switch(Result)
               {
                  ...
                  case EXIT_CODE:
                     ...
                     /* go ahead an close any Remote Serial Port Server */
                     /* that we have open.                              */
                     if(SerialPortID)
      ...
      {
         /* Close any Remote Serial Port Server that we have open.      */
         if(SerialPortID)
            CloseRemoteServer(NULL);
...
static int OpenServer(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(!SerialPortID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters >= 1) && (TempParam->Params[0].intParam))
         {
            ...
            if(ret_val > 0)
            {
               ...
               sprintf(ServiceName, "Serial Port Server Port %u", TempParam->Params[0].intParam);
               ...
               if(ret_val < 0)
               {
                  printf("Unable to Register Serial Port Server SDP Record, Error = %d.\r\n", ret_val);
               ...
               else
               {
                  ...
                  /* correctly.                                         */
                  printf("Serial Port Server Opened (Channel = %u).\r\n", TempParam->Params[0].intParam);
            ...
            {
               printf("Unable to Open Serial Port Server on port: %d, Error = %d.\r\n", TempParam->Params[0].intParam, ret_val);
      ...
      else
      {
         ...
         printf("Serial Port Server already open.\r\n");
...
static int CloseServer(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(SerialPortID)
      {
         ...
         if(ret_val < 0)
         {
            printf("SPP_Close_Server_Port(): Error %d\r\n", ret_val);
         ...
         printf("Serial Port Server Closed.\r\n");
      }
      else
      {
         printf("There is NO Serial Port Server currently open.\r\n");
...
static int OpenRemoteServer(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(IsClient)
      {
         ...
         if(!SerialPortID)
         {
            ...
            if((TempParam) && (TempParam->NumberofParameters > 1) && (TempParam->Params[0].intParam) && (NumberofValidResponses) && (TempParam->Params[0].intParam <= NumberofValidResponses) && (!COMPARE_BD_ADDR(InquiryResultList[(TempParam->Params[0].intParam - 1)], NullADDR)) && (TempParam->Params[1].intParam))
            {
               ...
               if(Result > 0)
               {
                  ...
                  printf("SPP_Open_Remote_Port() was successful.\r\n");
               ...
               else
               {
                  ...
                  printf("Error calling SPP_Open_Remote_Port(): %d.\r\n", Result);
         ...
         {
            printf("Unable to open Serial Port Client.\r\nSerial Port Client is already open.\r\n");
...
static int CloseRemoteServer(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(IsClient)
      {
         ...
         if(SerialPortID)
         {
            printf("Serial Port Client closed successfully.\r\n");
         ...
         else
         {
            ...
            printf("You must open a Serial Port Client before you can close an Serial Port Client.\r\n");
...
static int Read(ParameterList_t *TempParam)
{
   ...
   if((BluetoothStackID) && (SerialPortID))
   {
      ...
      if(Result >= 0)
      {
         /* Data was read successfully, the result indicates the number */
         /* of bytes that were successfully Read.                       */
         printf("SPP_Data_Read() Success: %d\r\n", Result);

         /* Check to see if there was Data available to Read.           */
         if(Result)
         {
            /* Data was available to read, display the Data.            */
            Buffer[Result] = '\0';
         ...
         {
            /* No Data was Read from the SPP Port.                      */
      ...
      else
      {
         ...
         printf("SPP_Data_Read() Failure: %d.\r\n", Result);
...
static int Write(ParameterList_t *TempParam)
{
   ...
   if((BluetoothStackID) && (SerialPortID))
   {
      ...
      Result = SPP_Data_Write(BluetoothStackID, SerialPortID, (Word_t)strlen(TEST_DATA), (Byte_t *)TEST_DATA);
      ...
      if(Result >= 0)
      {
         ...
         printf("SPP_Data_Write() Success: %d.\r\n", Result);
      ...
      else
      {
         ...
         printf("SPP_Data_Write() Failure: %d.\r\n", Result);
...
static void BTPSAPI SDP_Event_Callback(unsigned int BluetoothStackID, unsigned int SDPRequestID, SDP_Response_Data_t *SDP_Response_Data, unsigned long CallbackParameter)
{
   ...
   /* specified installed callback (i.e. this function DOES NOT have to */
   /* be reentrant). It needs to be noted however, that if the same     */
   /* Callback is installed more than once, then the callbacks will be  */
   /* called serially.  Because of this, the processing in this function*/
   /* should be as efficient as possible.  It should also be noted that */
   /* this function is called in the Thread Context of a Thread that the*/
...
static void BTPSAPI SPP_Event_Callback(unsigned int BluetoothStackID, SPP_Event_Data_t *SPP_Event_Data, unsigned long CallbackParameter)
{
   ...
   /* First, check to see if the required parameters appear to be       */
   /* semi-valid.                                                       */
   if((SPP_Event_Data != NULL) && (BluetoothStackID))
   {
      /* The parameters appear to be semi-valid, now check to see what  */
      /* type the incoming event is.                                    */
      switch(SPP_Event_Data->Event_Data_Type)
      {
         ...
         case etPort_Status_Indication:
            ...
            printf("\r\n");
            printf("SPP Port Status Indication, ID: 0x%04X, Status: 0x%04X, Break Status: 0x%04X, Length: 0x%04X.\r\n", SPP_Event_Data->Event_Data.SPP_Port_Status_Indication_Data->SerialPortID,
   ...
   {
      /* There was an error with one or more of the input parameters.   */
      printf("SPP callback data: Event_Data = NULL.\r\n");
   }

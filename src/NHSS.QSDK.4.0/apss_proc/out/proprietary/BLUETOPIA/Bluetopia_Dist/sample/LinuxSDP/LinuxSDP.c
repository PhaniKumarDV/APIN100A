...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXSDP - Simple Linux application using SDP Protocol.                   */
/*                                                                            */
/*  Author:  Rory Sledge                                                      */
/*                                                                            */
...
#define SAMPLE_RFCOMM_CHANNEL_ID                      1  /* Denotes the       */
                                                         /* Channel ID to use */
                                                         /* as part of the    */
                                                         /* Sample SDP Record.*/
                                                         /* Note: no valid    */
...
#define SAMPLE_SERVICE_NAME      "Sample SPP SDP Record" /* Denotes the       */
                                                         /* Service Name to   */
                                                         /* be used as part   */
                                                         /* of the Sample SDP */
                                                         /* Record.           */
...
#define UNABLE_TO_REGISTER_SDP_RECORD              (-9)  /* Denotes that an   */
                                                         /* error occurred    */
                                                         /* when trying to    */
                                                         /* register an SDP   */
                                                         /* Record with the   */
...
#define DEFAULT_DEBUG_LOG_FILE_NAME  "LinuxSDP_ASC.log"
#define DEFAULT_DEBUG_FTS_FILE_NAME  "LinuxSDP_FTS.log"
...
static DWord_t             SDPServiceRecordHandle;  /* Variable which holds the SDP    */
                                                    /* Service Record Handle of the    */
                                                    /* Sample SDP Record that this     */
                                                    /* program can add and remove      */
                                                    /* to/from the SDP Database.       */

static int                 DisconnectModeManual;    /* Variable which holds the current*/
                                                    /* SDP Disconnect Mode.  If this   */
                                                    /* variable is non-zero (TRUE) then*/
                                                    /* the current SDP Disconnect Mode */
                                                    /* is Manual, otherwise it is      */
                                                    /* in the Automatic Mode (default).*/
...
static int AddSDPServiceRecord(ParameterList_t *TempParam);
static int DeleteSDPServiceRecord(ParameterList_t *TempParam);
static int ServiceSearchRequest(ParameterList_t *TempParam);
static int ServiceAttributeRequest(ParameterList_t *TempParam);
static int ServiceSearchAttributeRequest(ParameterList_t *TempParam);
static int SetDisconnectModeManual(ParameterList_t *TempParam);
static int SetDisconnectModeAutomatic(ParameterList_t *TempParam);
...
static void UserInterface(void)
{
   ...
   /* Next let's make sure that the user knows what Disconnect Mode     */
   /* the SDP Client Functions are operating in.                        */
   SetDisconnectModeAutomatic(NULL);
   ...
   AddCommand("SEARCHREQUEST", ServiceSearchRequest);
   AddCommand("ATTRIBUTEREQUEST", ServiceAttributeRequest);
   AddCommand("SEARCHATTRIBUTEREQUEST", ServiceSearchAttributeRequest);
   AddCommand("SETDISCONNECTMODEAUTOMATIC", SetDisconnectModeAutomatic);
   AddCommand("SETDISCONNECTMODEMANUAL", SetDisconnectModeManual);
   AddCommand("ADDSDPRECORD", AddSDPServiceRecord);
   AddCommand("DELETESDPRECORD", DeleteSDPServiceRecord);
   ...
   if(!BluetoothStackID)
   {
      ...
      if(HCI_DriverInformation)
      {
         ...
         if(Result > 0)
         {
            ...
            /* Next flag that there are no SDP Service Records presently*/
            /* occupying the SDP Database.                              */
            ...
            /* Fake that the Disconnect Mode is Manual (we do this      */
            /* because on Program Startup we will change the            */
            /* Disconnect Mode to Automatic).  Since we cannot change   */
            /* the Mode if there is no change, go ahead and make it     */
            /* the opposite so we can change it.                        */
            DisconnectModeManual   = 1;
...
static int DeleteLinkKey(BD_ADDR_t BD_ADDR)
{
   ...
   /* Command Options for this SDP Sample Application.  The input       */
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("*                  GetRemoteName, SearchRequest,                 *\r\n");
   printf("*                  AttributeRequest, SearchAttributeRequest,     *\r\n");
   printf("*                  SetDisconnectModeAutomatic,                   *\r\n");
   printf("*                  SetDisconnectModeManual, AddSDPRecord,        *\r\n");
   printf("*                  DeleteSDPRecord, Disconnect,                  *\r\n");
...
static int GetRemoteName(ParameterList_t *TempParam)
{
   ...
   /* The following function is responsible for adding an SDP Record to */
   /* the Local SDP Database.  This function returns zero if successful */
   /* and a negative value if an error occurred.                        */
   /* * NOTE * This function ONLY adds an SDP Record to the Database it */
   /*          does NOT actually Add the Service that the SDP Record    */
   /*          Identifies to the Local System.                          */
static int AddSDPServiceRecord(ParameterList_t *TempParam)
{
   int                 Result;
   int                 ret_val;
   ...
   if(BluetoothStackID)
   {
      /* Next, let's check to see if the SDP Service Record has already */
      /* been added because this program will only support one Record   */
      /* being added at a time (although the Stack will support more).  */
      if(!SDPServiceRecordHandle)
      {
         /* Sample SDP Service Record has NOT been added to the SDP     */
         /* Database so let's add a generic SPP SDP Service Record to   */
         /* the SDP Database.                                           */
         /* * NOTE * We are not installing an SPP Server, only an       */
         /*          example SPP SDP Service Record.                    */
         sprintf(ServiceName, SAMPLE_SERVICE_NAME);

         /* Initialize the Default SPP UUID Entry Information.          */
         NumberUUIDEntries = 1;
         SDPUUIDEntries    = &SDPUUIDEntries_Default;

         /* Initialize the Serial Port Profile UUID.                    */
         SDPUUIDEntries_Default.SDP_Data_Element_Type = deUUID_128;
         SDP_ASSIGN_SERIAL_PORT_PROFILE_UUID_128(SDPUUIDEntries_Default.UUID_Value.UUID_128);

         /* Now let's add the actual Record itself.  If the SDP Record  */
         /* is added, then we need to add SDP Attributes to the Record. */
         if((Result = SDP_Create_Service_Record(BluetoothStackID, NumberUUIDEntries, SDPUUIDEntries)) > 0)
         {
            /* Note the Service Record Handle.                          */
            SDPServiceRecordHandle = (DWord_t)Result;

            /* Now allocate enough space to hold the SDP Data Element   */
            /* Sequence List.                                           */
            if((SDP_Data_Element = (SDP_Data_Element_t *)malloc(2*SDP_DATA_ELEMENT_SIZE)) != NULL)
            {
               ...
               SDP_Data_Element_L2CAP.SDP_Data_Element_Type                   = deUUID_128;
               SDP_Data_Element_L2CAP.SDP_Data_Element_Length                 = sizeof(SDP_Data_Element_L2CAP.SDP_Data_Element.UUID_128);
               SDP_ASSIGN_L2CAP_UUID_128(SDP_Data_Element_L2CAP.SDP_Data_Element.UUID_128);
               ...
               SDP_Data_Element_RFCOMM[0].SDP_Data_Element_Type                 = deUUID_128;
               SDP_Data_Element_RFCOMM[0].SDP_Data_Element_Length               = UUID_128_SIZE;
               SDP_ASSIGN_RFCOMM_UUID_128(SDP_Data_Element_RFCOMM[0].SDP_Data_Element.UUID_128);
               ...
               SDP_Data_Element_RFCOMM[1].SDP_Data_Element.UnsignedInteger1Byte = SAMPLE_RFCOMM_CHANNEL_ID;

               /* Format the Main SDP Data Element Sequence.            */
               SDP_Data_Element_Main.SDP_Data_Element_Type                      = deSequence;
               SDP_Data_Element_Main.SDP_Data_Element_Length                    = 2;
               SDP_Data_Element_Main.SDP_Data_Element.SDP_Data_Element_Sequence = SDP_Data_Element;

               /* Now let's add all necessary Attributes.               */
               Result = SDP_Add_Attribute(BluetoothStackID, SDPServiceRecordHandle, SDP_ATTRIBUTE_ID_PROTOCOL_DESCRIPTOR_LIST, &SDP_Data_Element_Main);

               /* Now we need to add the name.                          */
               if(!Result)
               {
                  /* Add a default Language Attribute ID List Entry for */
                  /* UTF-8 English.                                     */
                  ...
                  ret_val = SDP_Add_Attribute(BluetoothStackID, SDPServiceRecordHandle, SDP_ATTRIBUTE_ID_LANGUAGE_BASE_ATTRIBUTE_ID_LIST, SDP_Data_Element_Language);

                  /* Finally Add the Service Name to the SDP Database.  */
                  if(!Result)
                  {
                     SDP_Data_Element_Main.SDP_Data_Element_Type       = deTextString;
                     SDP_Data_Element_Main.SDP_Data_Element_Length     = strlen(ServiceName);
                     SDP_Data_Element_Main.SDP_Data_Element.TextString = (Byte_t *)ServiceName;

                     Result = SDP_Add_Attribute(BluetoothStackID, SDPServiceRecordHandle, (SDP_DEFAULT_LANGUAGE_BASE_ATTRIBUTE_ID + SDP_ATTRIBUTE_OFFSET_ID_SERVICE_NAME), &SDP_Data_Element_Main);

                     if(!Result)
                     {
                        /* The SDP Service Record as successfully added.*/
                        printf("SDP Add Sample Record: Record Added Successfully.\r\n");
                     ...
                     {
                        /* An error occurred while attempting to Add an */
                        /* SDP Attribute to the SDP Record we added so  */
                        /* flag an error to the user.                   */
                        printf("SDP_Add_Attribute() Failure: %d.\r\n", Result);
                  ...
                  {
                     /* An error occurred while attempting to Add an SDP*/
                     /* Attribute to the SDP Record we added so flag an */
                     /* error to the user.                              */
                     printf("SDP_Add_Attribute() Failure: %d.\r\n", Result);
               ...
               /* Free the memory that we allocated earlier.            */
               free(SDP_Data_Element);
            }
            else
            {
               /* Unable to Allocate Memory so flag an error to the     */
               /* user.                                                 */
               printf("Error adding SDP Sample Record: Unable To Allocate Memory.\r\n");
            ...
            if(ret_val < 0)
               SDP_Delete_Service_Record(BluetoothStackID, SDPServiceRecordHandle);
         }
         else
         {
            /* An error occurred while adding the actual SDP Record to  */
            /* the SDP Database so flag an error to the user.           */
            printf("SDP_Create_Service_Record() Failure: %d.\r\n", Result);
      ...
      {
         /* The Sample SDP Record has already been added to the SDP     */
         /* Database so flag an error to the user.                      */
         printf("Unable to add Sample SDP Record to the SDP Database.\r\nSample SDP Record already added.\r\n");
   ...
   /* from the Local SDP Database (that was previously added via a call */
   /* to the AddSDPServiceRecord() function).  This function returns    */
...
static int DeleteSDPServiceRecord(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      /* Next, let's check to see if the SDP Service Record has been    */
      /* added.  If it has not been added then we need to flag an error */
      /* to the user.                                                   */
      if(SDPServiceRecordHandle)
      {
         /* Simply Delete the SDP Service Record from the SDP Database. */
         Result = SDP_Delete_Service_Record(BluetoothStackID, SDPServiceRecordHandle);

         if(!Result)
         {
            /* The Sample SDP Service Record was deleted successfully   */
            /* from the SDP Database so inform the user.                */
            printf("SDP_Delete_Service_Record() Success.\r\n");

            /* Flag that there is no longer a Sample SDP Database       */
            /* Record present in the SDP Database.                      */
            SDPServiceRecordHandle = 0;
         ...
         {
            /* Error Deleting the SDP Service Record from the SDP       */
            /* Database so flag an error to the user.                   */
            printf("SDP_Delete_Service_Record() Failure: %d.\r\n", Result);
      ...
      {
         /* The Sample SDP Record has no been added to the SDP Database */
         /* so flag an error to the user.                               */
         printf("Unable to delete Sample SDP Record from the SDP Database.\r\nSample SDP Record has not been added.\r\n");
   ...
   /* The following function is responsible for issuing a Service       */
   /* Search Request to a Remote SDP Server.  This function returns     */
   /* zero if successful and a negative value if an error occurred.     */
static int ServiceSearchRequest(ParameterList_t *TempParam)
{
   int              Result;
   ...
   int              NumUUIDs;
   BD_ADDR_t        NullADDR;
   SDP_UUID_Entry_t SDPUUIDEntry[3];
   ...
   if(BluetoothStackID)
   {
      ...
      {
         /* OK, parameters appear to be semi-valid, now let's attempt   */
         /* to issue the SDP Service Search Request.                    */

         /* First let's build th UUID 32 value(s).                      */
         SDPUUIDEntry[0].SDP_Data_Element_Type = deUUID_32;

         ASSIGN_SDP_UUID_32(SDPUUIDEntry[0].UUID_Value.UUID_32, ((TempParam->Params[1].intParam & 0xFF000000) >> 24), ((TempParam->Params[1].intParam & 0x00FF0000) >> 16), ((TempParam->Params[1].intParam & 0x0000FF00) >> 8), (TempParam->Params[1].intParam & 0x000000FF));

         NumUUIDs = 1;

         if((TempParam->NumberofParameters > 2) && (TempParam->Params[2].intParam))
         {
            SDPUUIDEntry[1].SDP_Data_Element_Type = deUUID_32;

            ASSIGN_SDP_UUID_32(SDPUUIDEntry[1].UUID_Value.UUID_32, ((TempParam->Params[2].intParam & 0xFF000000) >> 24), ((TempParam->Params[2].intParam & 0x00FF0000) >> 16), ((TempParam->Params[2].intParam & 0x0000FF00) >> 8), (TempParam->Params[2].intParam & 0x000000FF));

            NumUUIDs++;

            if((TempParam->NumberofParameters > 3) && (TempParam->Params[3].intParam))
            {
               SDPUUIDEntry[2].SDP_Data_Element_Type = deUUID_32;

               ASSIGN_SDP_UUID_32(SDPUUIDEntry[2].UUID_Value.UUID_32, ((TempParam->Params[3].intParam & 0xFF000000) >> 24), ((TempParam->Params[3].intParam & 0x00FF0000) >> 16), ((TempParam->Params[3].intParam & 0x0000FF00) >> 8), (TempParam->Params[3].intParam & 0x000000FF));
         ...
         Result = SDP_Service_Search_Request(BluetoothStackID, InquiryResultList[(TempParam->Params[0].intParam - 1)], 255, NumUUIDs, SDPUUIDEntry, SDP_Event_Callback, (unsigned long)0);

         if(Result > 0)
         {
            /* The SDP Request was submitted successfully.              */
            printf("SDP_Service_Search_Request() Success.\r\n");
         ...
         else
         {
            ...
            printf("SDP_Service_Search_Request() Failure: %d.\r\n", Result);
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: SearchRequest [Inquiry Index] [16/32 Bit UUID] [16/32 Bit UUID] [16/32 Bit UUID].\r\n");
...
static int ServiceAttributeRequest(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters > 2) && (TempParam->Params[0].intParam) && (NumberofValidResponses) && (TempParam->Params[0].intParam <= NumberofValidResponses) && (!COMPARE_BD_ADDR(InquiryResultList[(TempParam->Params[0].intParam - 1)], NullADDR)))
      {
         ...
         AttributeID.Start_Attribute_ID = TempParam->Params[2].intParam;
         AttributeID.Attribute_Range    = 0;

         /* Next check to see if an Attribute Range was specified.      */
         if(TempParam->NumberofParameters > 3)
         {
            AttributeID.End_Attribute_ID = TempParam->Params[3].intParam;
            AttributeID.Attribute_Range  = 1;
         }

         /* Finally submit the SDP Request.                             */
         Result = SDP_Service_Attribute_Request(BluetoothStackID, InquiryResultList[(TempParam->Params[0].intParam - 1)], TempParam->Params[1].intParam, 1, &AttributeID, SDP_Event_Callback, (unsigned long)0);

         if(Result > 0)
         {
            /* The SDP Request was submitted successfully.              */
            printf("SDP_Service_Attribute_Request() Success.\r\n");
         ...
         {
            /* There was an error submitting the SDP Request.           */
            printf("SDP_Service_Attribute_Request() Failure: %d.\r\n", Result);
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: AttributeRequest [Inquiry Index] [SDP Record Handle] [Start Attribute] [End Attribute].\r\n");
...
static int ServiceSearchAttributeRequest(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters > 2) && (TempParam->Params[0].intParam) && (NumberofValidResponses) && (TempParam->Params[0].intParam <= NumberofValidResponses) && (!COMPARE_BD_ADDR(InquiryResultList[(TempParam->Params[0].intParam - 1)], NullADDR)))
      {
         ...
         ASSIGN_SDP_UUID_32(SDPUUIDEntry.UUID_Value.UUID_32, ((TempParam->Params[1].intParam & 0xFF000000) >> 24), ((TempParam->Params[1].intParam & 0x00FF0000) >> 16), ((TempParam->Params[1].intParam & 0x0000FF00) >> 8), (TempParam->Params[1].intParam & 0x000000FF));
         ...
         {
            /* The SDP Request was submitted successfully.              */
            printf("SDP_Service_Search_Attribute_Request() Success.\r\n");
         ...
         {
            /* There was an error submitting the SDP Request.           */
            printf("SDP_Service_Search_Attribute_Request() Failure: %d.\r\n", Result);
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: SearchAttributeRequest [Inquiry Index] [16/32 Bit UUID] [Start Attribute] [End Attribute].\r\n");
   ...
   /* The following function is responsible for changing the SDP        */
   /* Disconnect Mode to dmManual for the Local SDP Client Module.  This*/
...
static int SetDisconnectModeManual(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      /* Next, let's check to see if the Disconnect Mode is already     */
      /* set to Manual.                                                 */
      if(!DisconnectModeManual)
      {
         /* Simply attempt to set the current Disconnect Mode to        */
         /* Manual.                                                     */
         Result = SDP_Set_Disconnect_Mode(BluetoothStackID, dmManual);

         if(!Result)
         {
            /* SDP Disconnect Mode was set successfully so inform the   */
            /* user of the change.                                      */
            printf("SDP_Set_Disconnect_Mode() Success: %d.\r\n", Result);

            /* Note the new SDP Disconnect Mode.                        */
            DisconnectModeManual = 1;
         ...
         {
            /* There as an error issuing the SDP_Set_Disconnect_Mode()  */
            /* function so flag an error to the user.                   */
            printf("SDP_Set_Disconnect_Mode() Failure: %d.\r\n", Result);
      ...
      {
         /* The current Disconnect Mode is already set to Manual so     */
         /* flag an error to the user.                                  */
         printf("The current SDP Disconnect Mode is already set to Manual.\r\n");
   ...
   /* The following function is responsible for changing the SDP        */
   /* Disconnect Mode to dmAutomatic for the Local SDP Client Module.   */
...
static int SetDisconnectModeAutomatic(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(DisconnectModeManual)
      {
         /* Simply attempt to set the current Disconnect Mode to        */
         /* Automatic.                                                  */
         Result = SDP_Set_Disconnect_Mode(BluetoothStackID, dmAutomatic);
      ...
      {
         /* The current Disconnect Mode is already set to Automatic so  */
         /* flag an error to the user.                                  */
         printf("The current SDP Disconnect Mode is already set to Automatic.\r\n");
   ...
   /* with a Remote SDP Server.  This function returns zero if          */
...
static int Disconnect(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      /* Next, let's make sure that the parameters passed to this       */
      /* function are valid.                                            */
      if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].intParam) && (NumberofValidResponses) && (TempParam->Params[0].intParam <= NumberofValidResponses) && (!COMPARE_BD_ADDR(InquiryResultList[(TempParam->Params[0].intParam - 1)], NullADDR)))
      {
         /* Now let's simply issue the SDP_Disconnect_Server() function.*/
         Result = SDP_Disconnect_Server(BluetoothStackID, InquiryResultList[(TempParam->Params[0].intParam - 1)]);

         if(!Result)
         {
            /* The SDP_Disconnect_Server() function was issued          */
            /* successfully so inform the user.                         */
            printf("SDP_Disconnect_Server() Success: %d.\r\n", Result);
         ...
         {
            /* There as an error issuing the SDP_Disconnect_Server()    */
            /* function so flag an error to the user.                   */
            printf("SDP_Disconnect_Server() Failure: %d.\r\n", Result);

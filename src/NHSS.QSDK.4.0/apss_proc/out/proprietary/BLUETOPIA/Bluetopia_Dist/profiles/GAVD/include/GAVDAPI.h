...
typedef enum
{
   ...
   etGAVD_Get_All_Capabilities_Confirmation,
   etGAVD_Initial_Delay_Report_Request,
   etGAVD_Delay_Report_Indication,
   etGAVD_Delay_Report_Confirmation
} GAVD_Event_Type_t;
...
#define GAVD_FEC_BLOCK_SIZE                             (sizeof(GAVD_FEC_Block_t))
   ...
   /* GAVD_Register_SDP_Record_Version() function to specify a specific */
   /* GAVD Version to be supported.  This allows a device to only       */
   ...
   /*          GAVD_Register_SDP_Record(), merely registers an SDP      */
...
#define GAVD_DISCOVER_CONFIRMATION_DATA_SIZE            (sizeof(GAVD_Discover_Confirmation_Data_t))
   ...
   /* Capabilities" OR a "Stream Get All Capabilities" procedure        */
   /* initiated by the GAVD/AVDTP Client.  The GAVDID member is the ID  */
   /* of the local GAVD server or client.  The RSEID member is the      */
   /* Remote Stream Endpoint ID (from the Client's point of view) that  */
   /* this Confirmation Event (response) holds the capabilities of.  The*/
   /* ErrorCode member contains any errors reported.  The               */
   /* NumberServiceCapabilities member specifies how many service       */
   /* capabilities elements are contained in the array pointed to by the*/
   /* next parameter.  Finally, the ServiceCapabilities member is a     */
   /* pointer to an array of Service Capabilities.                      */
...
#define GAVD_ABORT_CONFIRMATION_DATA_SIZE               (sizeof(GAVD_Abort_Confirmation_Data_t))
   ...
   /* This event is sent by a GAVD/AVDTP entity to the local application*/
   /* when a Local Stream End Point requires that a Delay Report be sent*/
   /* to the Remote GAVD/AVDTP entity.  The LSEID member specifies the  */
   /* ID of the Local Stream End Point.                                 */
   /* * NOTE * This event is ONLY dispatched:                           */
   /*             - To Local SNK device with Delay Reporting configured */
   /*             - During stream establishement                        */
   /* * NOTE * If the above two things are true, the application *MUST* */
   /*          send a Delay Report via the GAVD_Delay_Report_Request()  */
   /*          function or the Endpoint will NEVER enter the open       */
   /*          state !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! */
typedef struct _tagGAVD_Initial_Delay_Report_Request_Data_t
{
   unsigned int LSEID;
} GAVD_Initial_Delay_Report_Request_Data_t;

#define GAVD_INITIAL_DELAY_REPORT_REQUEST_DATA_SIZE      (sizeof(GAVD_Initial_Delay_Report_Request_Data_t))

   /* This event is sent by a GAVD/AVDTP entity to the local application*/
   /* when a Local Stream End Point receives a "Delay Report" request   */
   /* from a Remote GAVD/AVDTP entity.  The LSEID member specifies the  */
   /* ID of the Local Stream End Point and the Delay member specifies   */
   /* the Delay (in 1/10 milli-seconds increments).                     */
typedef struct _tagGAVD_Delay_Report_Indication_Data_t
{
   unsigned int LSEID;
   Word_t       Delay;
} GAVD_Delay_Report_Indication_Data_t;

#define GAVD_DELAY_REPORT_INDICATION_DATA_SIZE           (sizeof(GAVD_Delay_Report_Indication_Data_t))

   /* This event is sent by a GAVD/AVDTP entity to the local application*/
   /* in response to a Delay Report Request that the local application  */
   /* had sent to the Remote GAVD/AVDTP entity.  The LSEID member       */
   /* specifies the ID of the Local Stream Endpoint.  The ErrorCode     */
   /* member contains the status of the Stream Start attempt (0 if      */
   /* successful).                                                      */
typedef struct _tagGAVD_Delay_Report_Confirmation_Data_t
{
   unsigned int LSEID;
   unsigned int ErrorCode;
} GAVD_Delay_Report_Confirmation_Data_t;

#define GAVD_DELAY_REPORT_CONFIRMATION_DATA_SIZE         (sizeof(GAVD_Delay_Report_Confirmation_Data_t))
...
typedef struct _tagGAVD_Event_Data_t
{
   ...
   union
   {
      ...
      GAVD_Initial_Delay_Report_Request_Data_t                 *GAVD_Initial_Delay_Report_Request_Data;
      GAVD_Delay_Report_Indication_Data_t                      *GAVD_Delay_Report_Indication_Data;
      GAVD_Delay_Report_Confirmation_Data_t                    *GAVD_Delay_Report_Confirmation_Data;
...
BTPSAPI_DECLARATION int BTPSAPI GAVD_Register_SDP_Record(unsigned int BluetoothStackID, GAVD_SDP_Service_Record_t *SDPServiceRecord, char *ServiceName, DWord_t *SDPServiceRecordHandle);
   ...
   /* additional SDP Information to add to the record.  The third       */
   /* parameter specifies the Service Name to associate with the SDP    */
   /* Record.  The fourth parameter specifies the GAVD version to       */
   /* publish.  The final parameter is a pointer to a DWord_t which     */
   /* receives the SDP Service Record Handle if this function           */
   /* successfully creates an SDP Service Record.  If this function     */
   /* returns zero, then the SDPServiceRecordHandle entry will contain  */
   /* the Service Record Handle of the added SDP Service Record.  If    */
   ...
   /*            that specifies that 0x0100 is UTF-8 Encoded, English   */
   /*            Language.                                              */
BTPSAPI_DECLARATION int BTPSAPI GAVD_Register_SDP_Record_Version(unsigned int BluetoothStackID, GAVD_SDP_Service_Record_t *SDPServiceRecord, char *ServiceName, GAVD_Version_t GAVDVersion, DWord_t *SDPServiceRecordHandle);

#ifdef INCLUDE_BLUETOOTH_API_PROTOTYPES
   typedef int (BTPSAPI *PFN_GAVD_Register_SDP_Record_Version_t)(unsigned int BluetoothStackID, GAVD_SDP_Service_Record_t *SDPServiceRecord, char *ServiceName, GAVD_Version_t GAVDVersion, DWord_t *SDPServiceRecordHandle);
#endif
...
BTPSAPI_DECLARATION int BTPSAPI GAVD_Get_End_Point_Capabilities(unsigned int BluetoothStackID, unsigned int GAVDID, unsigned int RSEID);
   ...
   /* The following function is responsible for sending a "Get All      */
   /* Capabilities Request" to a remote Stream End Point Manager.  This */
   /* function accepts as input, the Bluetooth Stack ID of the local    */
   /* Bluetooth Protocol Stack, the GAVDID of the local GAVD/AVDTP      */
   /* Client, and the Remote Stream Endpoint ID (RSEID) of the Stream   */
   ...
   /* Points.  This function returns zero if successful, or a negative  */
   /* return error code if an error occurred (see BTERRORS.H).          */
BTPSAPI_DECLARATION int BTPSAPI GAVD_Get_All_End_Point_Capabilities(unsigned int BluetoothStackID, unsigned int GAVDID, unsigned int RSEID);

#ifdef INCLUDE_BLUETOOTH_API_PROTOTYPES
   typedef int (BTPSAPI *PFN_GAVD_Get_All_End_Point_Capabilities_t)(unsigned int BluetoothStackID, unsigned int GAVDID, unsigned int RSEID);
#endif
...
BTPSAPI_DECLARATION int BTPSAPI GAVD_Un_Register_End_Point(unsigned int BluetoothStackID, unsigned int LSEID);
   ...
   /* The following function is responsible for creating an End Point   */
   ...
   /* within GAVD (with it's own LSEID).  This function accepts as it's */
   /* parameters, the Bluetooth Stack ID of the Bluetooth Stack to      */
   /* create the group on.  The next parameter specifies the Group LSEID*/
   /* (master of the group).  The remaining parameters specify the      */
   /* remaining LSEID's in the form of a list (that are part of the     */
   /* group).  This function returns zero if successful or a negative   */
   /* return error code if there was an error.                          */
   /* * NOTE * All LSEID's passed to this function *MUST* have been     */
...
BTPSAPI_DECLARATION int BTPSAPI GAVD_Create_End_Point_Group(unsigned int BluetoothStackID, unsigned int GroupLSEID, unsigned int NumberLSEIDs, unsigned int *LSEIDList);
   ...
   /* This function is responsible for deleting a previously created End*/
   /* Point Group (created via a successful call to the                 */
   /* GAVD_Create_End_Point_Group() function).  This function accepts   */
   /* the Bluetooth Stack ID of the Bluetooth Stack that created the End*/
   /* Point Group, followed by the Group LSEID (master LSEID that       */
   /* created the group).  This function returns zero if successful or a*/
...
BTPSAPI_DECLARATION int BTPSAPI GAVD_Delete_End_Point_Group(unsigned int BluetoothStackID, unsigned int GroupLSEID);
   ...
   /* LSEID, is the Local Stream End Point of the End Point that is be  */
   /* associated with the connection.  The third parameter, BD_ADDR, is */
   /* the Bluetooth address of the remote device.  The fourth parameter,*/
   /* RSEID, is the ID of the Remote Stream End Point that the          */
   /* application wishes to open.  The next two parameters,             */
   ...
   /* * NOTE * If this function is successful, then the return value    */
   /*          from this function will be the same as the LSEID that was*/
   /*          passed into this function.                               */
   /* * NOTE * This function differs from the                           */
...
BTPSAPI_DECLARATION int BTPSAPI GAVD_Security_Control_Response(unsigned int BluetoothStackID, unsigned int LSEID, unsigned int ErrorCode, unsigned int ContentProtectionDataLength, Byte_t *ContentProtectionData);
   ...
   /* The following function is provided to allow a mechanism to request*/
   /* to abort a stream on the remote GAVD/AVDTP device.  This function */
   /* accepts as input, the Bluetooth Stack ID of the Bluetooth Protocol*/
   /* Stack, and the ID of the Local Stream End Point (LSEID) that is   */
   /* connected to the stream.  This function returns zero if           */
   /* successful, or a negative return value if there was an error.     */
BTPSAPI_DECLARATION int BTPSAPI GAVD_Abort_Stream_Request(unsigned int BluetoothStackID, unsigned int LSEID);
   ...
   /* The following function is provided to allow a mechanism to request*/
   /* to send a delay report on the specified stream on the remote      */
   /* GAVD/AVDTP device.  This function accepts as input, the Bluetooth */
   /* Stack ID of the Bluetooth Protocol Stack, and the ID of the Local */
   /* Stream End Point (LSEID) that is connected to the stream, followed*/
   /* by the actual delay value for the report.  This function returns  */
   /* zero if successful, or a negative return value if there was an    */
   /* error.                                                            */
   /* * NOTE * This function is only applicable:                        */
   /*             - Local SNK Endpoints                                 */
   /*             - Endpoint state is at least configured               */
   /* * NOTE * The delay value is specified in 1/10 milli-seconds       */
   /*          increments.                                              */
BTPSAPI_DECLARATION int BTPSAPI GAVD_Delay_Report_Request(unsigned int BluetoothStackID, unsigned int LSEID, Word_t Delay);

#ifdef INCLUDE_BLUETOOTH_API_PROTOTYPES
   typedef int (BTPSAPI *PFN_GAVD_Delay_Report_Request_t)(unsigned int BluetoothStackID, unsigned int LSEID, Word_t Delay);
#endif

   /* The following function is provided to allow a mechanism to respond*/
   /* to a request from a remote GAVD/AVDTP entity that has reported    */
   /* delay reporting.  This function accepts as input, the Bluetooth   */
   /* Stack ID of the Bluetooth Protocol Stack, the LSEID of the Local  */
   /* Stream End Point responding to the Delay Report Request, and an   */
   /* error code identifying the reason for the failure (0 if no        */
   /* errors).  This function returns zero if successful, or a negative */
   /* return value if there was an error.                               */
BTPSAPI_DECLARATION int BTPSAPI GAVD_Delay_Report_Response(unsigned int BluetoothStackID, unsigned int LSEID, unsigned int ErrorCode);

#ifdef INCLUDE_BLUETOOTH_API_PROTOTYPES
   typedef int (BTPSAPI *PFN_GAVD_Delay_Report_Response_t)(unsigned int BluetoothStackID, unsigned int LSEID, unsigned int ErrorCode);
#endif

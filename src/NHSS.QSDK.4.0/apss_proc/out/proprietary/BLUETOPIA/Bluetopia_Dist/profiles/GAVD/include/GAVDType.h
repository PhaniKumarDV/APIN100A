...
#define AVDTP_PROTOCOL_VERSION                                            (0x0103)

   /* The following constants define the previous Profile Versions of   */
   /* the Audio/Video Data Transfer Protocol that are supported by this */
   /* implementation.  These values can be used in the SDP Records to   */
   /* specify a specific AVDTP Version.                                 */
#define AVDTP_PROTOCOL_VERSION_1_0                                        (0x0100)
#define AVDTP_PROTOCOL_VERSION_1_2                                        (0x0102)
#define AVDTP_PROTOCOL_VERSION_1_3                                        (0x0103)
...
#define AVDTP_MAXIMUM_LSEID                                               (0x3E)
   ...
   /* * NOTE * The following are the Basic Service Capabilities (as     */
   /*          defined by AVDTP 1.2 specification and below).           */
...
#define AVDTP_SERVICE_CATEGORY_MEDIA_CODEC                                (0x07)
   ...
   /* Service Categories for stream endpoints, non basic as defined by  */
   /* AVDTP 1.3 specification and above).                               */
#define AVDTP_SERVICE_CATEGORY_DELAY_REPORTING                            (0x08)
...
#define MAX_TRANSPORT_ID_VALUE                                            (0x1E)
   ...
   /* * NOTE * The following masks apply to the VPXCC member of the RTP */
   /*          Header.                                                  */
...
#define RTP_EXTENSION_SHIFT                                                  (4)
#define RTP_CSRC_COUNT_SHIFT                                                 (0)

   /* * NOTE * The following masks apply to the MPT member of the RTP   */
   /*          Header.                                                  */
...
#define RTP_MARKER_SHIFT                                                     (7)
#define RTP_PAYLOAD_TYPE_SHIFT                                               (0)

   /* * NOTE * The following are miscellaneous constants define by RFC. */
...
#define AVDTP_DISCOVER_RESPONSE_END_POINT_DATA_SIZE     (sizeof(AVDTP_Discover_Response_End_Point_Data_t))
   ...
   /* This structure defines the AVDTP Get Capabilities and Get All     */
   /* Capabilities Reject Response.                                     */
typedef __PACKED_STRUCT_BEGIN__ struct _tagAVDTP_Get_Capabilities_Reject_Response_t
{
   Byte_t ErrorCode;
} __PACKED_STRUCT_END__ AVDTP_Get_Capabilities_Reject_Response_t;

#define AVDTP_GET_CAPABILITIES_REJECT_RESPONSE_SIZE      (sizeof(AVDTP_Get_Capabilities_Reject_Response_t))
...
#define AVDTP_SECURITY_CONTROL_COMMAND_SIZE(_x)         (sizeof(AVDTP_Security_Control_Command_t) - sizeof(Byte_t) + (unsigned int)(_x))
   ...
   /* This structure defines the AVDTP General Reject Response.         */
typedef __PACKED_STRUCT_BEGIN__ struct _tagAVDTP_General_Reject_Response_t
{
   Byte_t InvalidSignalIdentifier;
} __PACKED_STRUCT_END__ AVDTP_General_Reject_Response_t;

#define AVDTP_GENERAL_REJECT_RESPONSE_SIZE               (sizeof(AVDTP_General_Reject_Response_t))

   /* This structure defines the AVDTP Delay Report Command.            */
typedef __PACKED_STRUCT_BEGIN__ struct _tagAVDTP_Delay_Report_Command_t
{
   Byte_t           RSEID;
   NonAlignedWord_t Delay;
} __PACKED_STRUCT_END__ AVDTP_Delay_Report_Command_t;

#define AVDTP_DELAY_REPORT_COMMAND_SIZE                 (sizeof(AVDTP_Delay_Report_Command_t))

   /* This structure defines the AVDTP Delay Report Reject Response.    */
typedef __PACKED_STRUCT_BEGIN__ struct _tagAVDTP_Delay_Report_Reject_Response_t
{
   Byte_t ErrorCode;
} __PACKED_STRUCT_END__ AVDTP_Delay_Report_Reject_Response_t;

#define AVDTP_DELAY_REPORT_REJECT_RESPONSE_SIZE          (sizeof(AVDTP_Delay_Report_Reject_Response_t))
...
#define AVDTP_MULTIPLEXING_CAPABILITY_DATA_SIZE         (sizeof(AVDTP_Multiplexing_Capability_Data_t))
   ...
   /* This structure defines the AVDTP Delay Reporting Capability Data. */
   /* * NOTE * There is currently no defined data for this capability.  */
#define AVDTP_DELAY_REPORTING_CAPABILITY_DATA_SIZE      (0)

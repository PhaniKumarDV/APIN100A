...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXFAX - Simple Linux application using the FAX Profile.                */
/*                                                                            */
/*  Author:  Rory Sledge                                                      */
/*                                                                            */
...
#include "LinuxFAX.h"      /* Main Application Prototypes and Constants.      */

#include "SS1BTPS.h"       /* Includes the SS1 Bluetooth Protocol Stack.      */
#include "SS1BTFAX.h"      /* Includes the SS1 FAX Profile.                   */
#include "SS1BTDBG.h"      /* Includes/Constants for Bluetooth Debugging.     */

#define LOCAL_NAME_ROOT                      "LinuxFAX"  /* Root of the local */
                                                         /* name              */
...
#define DEFAULT_DEBUG_LOG_FILE_NAME  "LinuxFAX_ASC.log"
#define DEFAULT_DEBUG_FTS_FILE_NAME  "LinuxFAX_FTS.log"
...
static unsigned int        FAXServerID;             /* Variable which contains the     */
                                                    /* handle of the most recently     */
                                                    /* opened FAX Server.              */

static DWord_t             FAXServerSDPHandle;      /* Variable which holds the handle */
                                                    /* to the FAX SDP Record.          */

static Boolean_t           ServerConnected;         /* Variable which contains the     */
                                                    /* current connection state of the */
                                                    /* FAX Server.                     */

static unsigned int        FAXClientID;             /* Variable which contains the     */
                                                    /* handle of the most recently     */
                                                    /* opened FAX Client.              */
...
static void BTPSAPI FAX_Event_Callback(unsigned int BluetoothStackID, FAX_Event_Data_t *FAX_Event_Data, unsigned long CallbackParameter);
...
static int OpenStack(HCI_DriverInformation_t *HCI_DriverInformation)
{
   ...
   if(!BluetoothStackID)
   {
      ...
      else
      {
         ...
         printf("Either the specified Comm Port Number or Baud Rate are Invalid.\r\n");
...
static int DeleteLinkKey(BD_ADDR_t BD_ADDR)
{
   ...
   /* Command Options for this FAX Sample Application.  The input       */
...
static int Pair(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if((!FAXClientID) && (!ServerConnected))
      {
...
static int ServiceDiscovery(ParameterList_t *TempParam)
{
   ...
   /* The following function is responsible for opening a FAX Server and*/
   /* registering a SDP record to be associated with the opened Server. */
   /* This function returns zero on successful execution and a negative */
   /* value on all errors.                                              */
static int OpenServer(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      /* Check to see if there is already FAX Server Port Open.         */
      if(!FAXServerID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters > 1) && (TempParam->Params[0].intParam))
         {
            ...
            /* Submit the request to Open a FAX Server Port.            */
            ret_val = FAX_Open_Server_Port(BluetoothStackID, TempParam->Params[0].intParam, TempParam->Params[1].intParam, BaudRate, FALSE, FAX_Event_Callback, 0);

            /* Next, check to see if the FAX_Open_Server_Port() function*/
            /* was successful.                                          */
            if(ret_val > 0)
            {
               /* Inform the user that the Server was successfully      */
               /* opened and save the Server Port ID for later use.     */
               printf("Return Value is %d FAX_Open_Server_Port().\r\n", ret_val);
               FAXServerID = ret_val;
               ...
               sprintf(ServiceName, "FAX Serial Port %u", TempParam->Params[0].intParam);

               /* Now that a Service Name has been created try and      */
               /* Register the SDP Record.                              */
               ret_val = FAX_Register_SDP_Record(BluetoothStackID, FAXServerID, FAX_CLASS_SUPPORT_ALL_VALUE, ServiceName, &FAXServerSDPHandle);
               ...
               if(!ret_val)
               {
                  ...
                  printf("Return Value is %d FAX_Register_SDP_Record().\r\n", ret_val);
               }
               else
               {
                  /* Display an Error Message and make sure the Current */
                  /* Server SDP Handle is invalid, and close the FAX    */
                  /* Server Port we just opened because we weren't able */
                  /* to register a SDP Record.                          */
                  printf("Return Value is %d FAX_Register_SDP_Record() FAILURE.\r\n", ret_val);
                  FAXServerSDPHandle = 0;

                  /* Now try and close the Opened Server Port.          */
                  ret_val = FAX_Close_Server_Port(BluetoothStackID, FAXServerID);
                  FAXServerID = 0;
                  ...
                  if(!ret_val)
                  {
                     ...
                     /* successfully closed.                            */
                     printf("Return Value is %d FAX_Close_Server_Port().\r\n", ret_val);
                  }
                  else
                  {
                     /* An error occurred while attempting to close the */
                     /* Port.                                           */
                     printf("Return Value is %d FAX_Close_Server_Port() FAILURE.\r\n", ret_val);
                  }
               }
            }
            else
            {
               ...
               printf("Return Value is %d FAX_Open_Server_Port() FAILURE.\r\n", ret_val);
            }
         }
         else
         {
      ...
      {
         /* This application supports only one FAX Server at a time,    */
         /* inform the user that an FAX Server already exists.          */
         printf("FAX Server Already Exists.\r\n");
   ...
   /* The following function is responsible for closing an opened FAX   */
   /* Server and unregistering the SDP record associated with the open  */
...
static int CloseServer(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      /* valid.                                                         */
      if(FAXServerSDPHandle)
      {
         /* The SDP Record Handle is valid so remove the record from the*/
         /* SDP Database.                                               */
         ret_val = FAX_Un_Register_SDP_Record(BluetoothStackID, CurrentPortID, FAXServerSDPHandle);
         ...
         if(!ret_val)
         {
            ...
            printf("Return Value is %d FAX_Un_Register_SDP_Record().\r\n", ret_val);
            FAXServerSDPHandle = 0;
         }
         else
         {
            ...
            /* Record.                                                  */
            printf("Return Value is %d FAX_Un_Register_SDP_Record() FAILURE.\r\n", ret_val);
         }
      }
      else
      {
      ...
      /* Check to see if there is an open FAX Server Port.              */
      if(FAXServerID)
      {
         /* Submit the request to Close a FAX Server Port.              */
         ret_val = FAX_Close_Server_Port(BluetoothStackID, FAXServerID);

         /* Next, check to see if the FAX_Close_Server_Port() function  */
         /* was successful.                                             */
         if(!ret_val)
         {
            printf("Return Value is %d FAX_Close_Server_Port().\r\n", ret_val);
   ...
   /* a remote FAX Server.  This function returns zero on successful    */
   /* execution and a negative value on all errors.                     */
static int OpenClient(ParameterList_t *TempParam)
{
   int       ret_val;
   ...
   if(BluetoothStackID)
   {
      /* Check to see if there is already FAX Client Port Open and that */
      ...
      if((!FAXClientID) && (!ServerConnected))
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters > 2) && (TempParam->Params[0].intParam) && (NumberofValidResponses) && (TempParam->Params[0].intParam <= NumberofValidResponses) && (!COMPARE_BD_ADDR(InquiryResultList[(TempParam->Params[0].intParam-1)], NullADDR)) && (TempParam->Params[1].intParam))
         {
            ...
            printf("FAX_Open_Remote_Port(BD_ADDR = %s).\r\n", BoardStr);

            /* Submit the request to Open a FAX Client Port.            */
            ret_val = FAX_Open_Remote_Port(BluetoothStackID, InquiryResultList[(TempParam->Params[0].intParam-1)], TempParam->Params[1].intParam, TempParam->Params[2].intParam, FAX_Event_Callback, 0);

            /* Next, check to see if the FAX_Open_Remote_Port() function*/
            /* was successful.                                          */
            if(ret_val > 0)
            {
               printf("Return Value is %d FAX_Open_Remote_Port().\r\n", ret_val);
            ...
            {
               printf("Return Value is %d FAX_Open_Remote_Port() FAILURE.\r\n", ret_val);
            }
         }
         else
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: OpenClient [RFCOMM Port] [Inquiry Index] [Local Device To Open].\r\n");
      ...
      {
         /* This application supports only one FAX Connection at a time,*/
         /* inform the user that an FAX Connection already exists.      */
         printf("FAX Connection Already Exists.\r\n");
   ...
   /* remote FAX Port.  This function returns zero on successful        */
   /* execution and a negative value on all errors.                     */
static int CloseClient(ParameterList_t *TempParam)
{
   int ret_val;
   ...
   if(BluetoothStackID)
   {
      /* Check to see if there is an open FAX Client Port or the Server */
      /* is currently connected.                                        */
      if(((ServerConnected)?FAXServerID:FAXClientID))
      {
         /* Submit the request to Close a FAX Client Port.              */
         ret_val = FAX_Close_Port(BluetoothStackID, ((ServerConnected)?FAXServerID:FAXClientID));

         /* Next, check to see if the FAX_Close_Port() function was     */
         /* successful.                                                 */
         if(!ret_val)
         {
            printf("Return Value is %d FAX_Close_Port().\r\n", ret_val);
            FAXClientID     = 0;
            ServerConnected = FALSE;
         }
         else
         {
            printf("Return Value is %d FAX_Close_Port() FAILURE.\r\n", ret_val);
         }
      }
      else
      {
   ...
   /* of an SDP Service Attribute Response.                             */
static void DisplaySDPAttributeResponse(SDP_Service_Attribute_Response_Data_t *SDPServiceAttributeResponse, unsigned int InitLevel)
{
   int Index;
...
static void BTPSAPI SDP_Event_Callback(unsigned int BluetoothStackID, unsigned int SDPRequestID, SDP_Response_Data_t *SDP_Response_Data, unsigned long CallbackParameter)
{
   ...
   /* The following function is for the FAX Event Callback.  This       */
   /* function will be called whenever a Callback has been registered   */
   /* for the specified FAX Action that is associated with the Bluetooth*/
   /* Stack.  This function passes to the caller the FAX Event Data of  */
   /* the specified Event and the FAX Event Callback Parameter that was */
   /* specified when this Callback was installed.  The caller is free to*/
   /* use the contents of the FAX Event Data ONLY in the context of this*/
   /* callback.  If the caller requires the Data for a longer period of */
   /* time, then the callback function MUST copy the data into another  */
   /* Data Buffer.  This function is guaranteed NOT to be invoked more  */
   /* than once simultaneously for the specified installed callback     */
   ...
   /* the Thread Context of a Thread that the User does NOT own.        */
   /* Therefore, processing in this function should be as efficient as  */
   /* possible (this argument holds anyway because other FAX Events will*/
   /* not be processed while this function call is outstanding).        */
   /* * NOTE * This function MUST NOT Block and wait for events that    */
   /*          can only be satisfied by Receiving other FAX Events.  A  */
   /*          Deadlock WILL occur because NO FAX Event Callbacks will  */
   /*          be issued while this function is currently outstanding.  */
static void BTPSAPI FAX_Event_Callback(unsigned int BluetoothStackID, FAX_Event_Data_t *FAX_Event_Data, unsigned long CallbackParameter)
{
   char BoardStr[13];
   ...
   /* First, check to see if the required parameters appear to be       */
   /* semi-valid.                                                       */
   if((FAX_Event_Data) && (BluetoothStackID))
   {
      /* The parameters appear to be semi-valid, now check to see what  */
      /* type the incoming event is.                                    */
      switch(FAX_Event_Data->Event_Data_Type)
      {
         case etFAX_Open_Port_Indication:
            /* A Client has connected to the Server, display the BD_ADDR*/
            /* of the connecting device.                                */
            printf("\r\n");
            BD_ADDRToStr(FAX_Event_Data->Event_Data.FAX_Open_Port_Indication_Data->BD_ADDR, BoardStr);
            printf("FAX Open Port Indication, ID: 0x%04X, Board: %s.\r\n", FAX_Event_Data->Event_Data.FAX_Open_Port_Indication_Data->FAXPortID, BoardStr);

            ServerConnected = TRUE;
            break;
         case etFAX_Open_Port_Confirmation:
            /* The Client received a Connect Confirmation, display      */
            /* relevant information.                                    */
            printf("\r\n");
            printf("FAX Open Port Confirmation, ID: 0x%04X, Status: 0x%04X.\r\n", FAX_Event_Data->Event_Data.FAX_Open_Port_Confirmation_Data->FAXPortID,
                                                                                  FAX_Event_Data->Event_Data.FAX_Open_Port_Confirmation_Data->PortOpenStatus);

            /* Check to see if the Client Port was opened successfully. */
            if(FAX_Event_Data->Event_Data.FAX_Open_Port_Confirmation_Data->PortOpenStatus)
            {
            ...
            break;
         case etFAX_Close_Port_Indication:
            /* A Close Port Indication was received, display all        */
            /* relevant information.                                    */
            printf("\r\n");
            printf("FAX Close Port Indication, ID: 0x%04X.\r\n", FAX_Event_Data->Event_Data.FAX_Close_Port_Indication_Data->FAXPortID);

            /* Check to see if the closing port is the Client Port.     */
            if(FAX_Event_Data->Event_Data.FAX_Close_Port_Indication_Data->FAXPortID == FAXClientID)
            {
            ...
            break;
         default:
            /* We are not interested in any other FAX Events.           */
            break;
      }
   }
   else
   {
      /* There was an error with one or more of the input parameters.   */
      printf("FAX callback data: Event_Data = NULL.\r\n");
   }

...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXHDS - Simple Linux application using Headset Profile.                */
/*                                                                            */
/*  Author:  Damon Lange                                                      */
/*                                                                            */
...
#include "LinuxHDS.h"      /* Main Application Prototypes and Constants.      */

#include "SS1BTPS.h"       /* Includes for the SS1 Bluetooth Protocol Stack.  */
#include "SS1BTHDS.h"      /* Includes for the SS1 Headset Profile.           */
#include "SS1BTDBG.h"      /* Includes/Constants for Bluetooth Debugging.     */

#define LOCAL_NAME_ROOT                      "LinuxHDS"  /* Root of the local */
                                                         /* name              */
...
#define DEFAULT_DEBUG_LOG_FILE_NAME  "LinuxHDS_ASC.log"
#define DEFAULT_DEBUG_FTS_FILE_NAME  "LinuxHDS_FTS.log"
...
typedef struct _tagUUIDInfo_t
{
  ...
  /* The following type definition represents the Connection Status of  */
  /* an Active Connection (Headset or Audio Gateway).                   */
typedef enum
{
   csConnected,
   csAudioConnected,
...
static int                 IsHeadset;               /* Variable used to indicate if the*/
                                                    /* program is to be run in Headset */
                                                    /* Mode or Audio Gateway Mode.     */
...
static void UserInterface_Headset(void);
static void UserInterface_AudioGateway(void);
...
static void UserInterface_Headset(void)
{
   ...
   AddCommand("CHANGEVOLUME", ChangeVolume);
   ...
   while(Result != EXIT_CODE)
   {
      ...
      {
         /* Close any Remote Audio Gateway Server that we have open.    */
         if(HDSClientID)
            CloseRemoteServer(NULL);
...
static void UserInterface_AudioGateway(void)
{
   ...
   AddCommand("SETUPAUDIO", SetupAudio);
   AddCommand("CHANGEVOLUME", ChangeVolume);
   AddCommand("CHANGEMICROPHONEGAIN", ChangeMicrophoneGain);
   AddCommand("SETSCOTRANSPORT", SetSCOTransport);
   AddCommand("SETSCODATAFORMAT", SetSCODataFormat);
   ...
   while(Result != EXIT_CODE)
   {
         ...
         if(strlen(UserInput))
         {
            ...
            if(CommandParser(&TempCommand, UserInput) >= 0)
            {
               ...
               switch(Result)
               {
                  ...
                  case EXIT_CODE:
                     /* If the user has request to exit we might as well*/
                     /* go ahead an close any Remote Headset Server that*/
                     /* we have open.                                   */
                     if(HDSClientID)
                        CloseRemoteServer(NULL);

                     /* Close the Local Audio Gateway Server that we    */
                     /* have open.                                      */
                     CloseServer();
                     break;
               }
      ...
      {
         /* Close any Remote Headset Server that we have open.          */
         if(HDSClientID)
            CloseRemoteServer(NULL);

         /* Close the Local Audio Gateway Server that we have open.     */
         CloseServer();
   ...
   if(!BluetoothStackID)
   {
      ...
      if(HCI_DriverInformation)
      {
         ...
         if(Result > 0)
         {
            ...
            printf("Current SCO Parameters: CODEC, %u bit, %s Test Mode active.\r\n", AudioData8BitFormat?8:16, (AudioTestMode == AUDIO_DATA_TEST_NONE)?"No":((AudioTestMode == AUDIO_DATA_TEST_1KHZ_TONE)?"1KHz Tone":"Loopback"));
   ...
   if(BluetoothStackID)
   {
      ...
      if(IsHeadset)
      {
         ...
         if(ret_val > 0)
         {
            ...
            if(ret_val < 0)
            {
               printf("Unable to Register Headset Server SDP Record, Error = %d.\r\n", ret_val);
            ...
            else
            {
               ...
               /* correctly.                                            */
               printf("Headset Server Opened (Channel = %u).\r\n", LOCAL_SERVER_CHANNEL_ID);
         ...
         {
            printf("Unable to Open Headset Server, Error = %d.\r\n", ret_val);
      ...
      else
      {
         ...
         if(ret_val > 0)
         {
            ...
            if(ret_val < 0)
            {
               printf("Unable to Register Audio Gateway Server SDP Record, Error = %d.\r\n", ret_val);
            ...
            else
            {
               /* Simply flag to the user that everything initialized   */
               /* correctly.                                            */
               printf("Audio Gateway Server Opened (Channel = %u).\r\n", LOCAL_SERVER_CHANNEL_ID);
         ...
         {
            printf("Unable to Open Audio Gateway Server, Error = %d.\r\n", ret_val);
   ...
   if(BluetoothStackID)
   {
      ...
      if(HDSServerID)
      {
         ...
         else
         {
            ...
            printf("Audio Gateway Server Closed.\r\n");
         }
      ...
      {
         if(IsHeadset)
            printf("There is NO Headset Server currently open.\r\n");
         else
            printf("There is NO Audio Gateway Server currently open.\r\n");
...
static int DeleteLinkKey(BD_ADDR_t BD_ADDR)
{
   ...
   /* Command Options for either Headset or Audio Gateway Client and    */
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   if(IsHeadset)
   {
      ...
      printf("*                  GetRemoteName, ServiceDiscovery, Open,        *\r\n");
      printf("*                  Close, PressButton, ChangeVolume,             *\r\n");
      printf("*                  ChangeMicrophoneGain, SetSCOTransport,        *\r\n");
      printf("*                  SetSCODataFormatSetSCOTestMode, EnableDebug,  *\r\n");
      printf("*                  Help, Quit.                                   *\r\n");
      printf("******************************************************************\r\n");
   }
   else
   {
      ...
      printf("*                  RingIndication, SetupAudio, ChangeVolume,     *\r\n");
      printf("*                  ChangeMicrophoneGain, SetSCOTransport,        *\r\n");
      printf("*                  SetSCODataFormatSetSCOTestMode, EnableDebug,  *\r\n");
      printf("*                  Help, Quit.                                   *\r\n");
      printf("******************************************************************\r\n");
...
static int OpenRemoteServer(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
         ...
         if((TempParam) && (TempParam->NumberofParameters > 1) && (TempParam->Params[0].intParam) && (NumberofValidResponses) && (TempParam->Params[0].intParam <= NumberofValidResponses) && (!COMPARE_BD_ADDR(InquiryResultList[(TempParam->Params[0].intParam - 1)], NullADDR)) && (TempParam->Params[1].intParam))
         {
            ...
            if(IsHeadset)
            {
               ...
               if(Result > 0)
               {
                  ...
                  printf("HDSET_Open_Remote_Audio_Gateway_Port() was successful.\r\n");
               ...
               else
               {
                  ...
                  printf("Error calling HDSET_Open_Remote_Audio_Gateway_Port(): %d.\r\n", Result);
            ...
            else
            {
               ...
               if(Result > 0)
               {
                  ...
                  printf("HDSET_Open_Remote_Headset_Port() was successful.\r\n");
               ...
               else
               {
                  ...
                  printf("Error calling HDSET_Open_Remote_Headset_Port(): %d.\r\n", Result);
      ...
      {
         if(IsHeadset)
            printf("Unable to open Headset Client.\r\nHeadset Client is already open.\r\n");
         else
            printf("Unable to open Audio Gateway Client.\r\nAudio Gateway Client is already open.\r\n");
...
static int CloseRemoteServer(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(HDSClientID)
      {
         ...
         if(IsHeadset)
         {
            printf("Headset Client closed successfully.\r\n");
         ...
         {
            printf("Audtio Gateway Client closed successfully.\r\n");
      ...
      else
      {
         ...
         if(IsHeadset)
            printf("You must open a Headset Client before you can close a Headset Client.\r\n");
         else
            printf("You must open an Audio Gateway Client before you can close an Audio Gateway Client.\r\n");
...
static int PressButton(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(IsHeadset)
      {
         ...
         if(HDSClientID)
         {
            ...
            Result = HDSET_Send_Button_Press(BluetoothStackID, HDSClientID);

            if(Result >= 0)
            {
               /* Send Button Press was issued successfully so flag     */
               /* success to the user.                                  */
               printf("HDSET_Send_Button_Press() Success.\r\n");
            ...
            else
            {
               ...
               /* error to the user.                                    */
               printf("HDSET_Send_Button_Press() Failure: %d.\r\n", Result);
         ...
         else
         {
            ...
            printf("You must have an active connection to use this command.\r\n");
   ...
   /* The following function is responsible for sending a Volume Change */
   /* Command to the Remote Connection.  This function returns zero if  */
...
static int ChangeVolume(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(HDSClientID)
      {
        ...
        {
            Result = HDSET_Set_Speaker_Gain(BluetoothStackID, HDSClientID, TempParam->Params[0].intParam);

            if(Result >= 0)
            {
               /* Speaker Gain was issued successfully so flag success  */
               /* to the user.                                          */
               printf("HDSET_Set_Speaker_Gain() Success.\r\n");
            ...
            else
            {
               ...
               /* error to the user.                                    */
               printf("HDSET_Set_Speaker_Gain() Failure: %d.\r\n", Result);
         ...
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: ChangeVolume [Volume Value (0-15)].\r\n");
...
static int ChangeMicrophoneGain(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(HDSClientID)
      {
        ...
        {
            Result = HDSET_Set_Microphone_Gain(BluetoothStackID, HDSClientID, TempParam->Params[0].intParam);

            if(Result >= 0)
            {
               /* Microphone Gain was issued successfully so flag       */
               /* success to the user.                                  */
               printf("HDSET_Set_Microphone_Gain() Success.\r\n");
            ...
            else
            {
               ...
               /* error to the user.                                    */
               printf("HDSET_Set_Microphone_Gain() Failure: %d.\r\n", Result);
         ...
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: ChangeMicrophoneGain [Gain Value (0-15)].\r\n");
...
static int RingIndication(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(!IsHeadset)
      {
         ...
         if(HDSClientID)
         {
            Result = HDSET_Ring_Indication(BluetoothStackID, HDSClientID);

            if(Result >= 0)
            {
               /* Ring Indication was issued successfully so flag       */
               /* success to the user.                                  */
               printf("HDSET_Ring_Indication() Success.\r\n");
            ...
            else
            {
               ...
               /* error to the user.                                    */
               printf("HDSET_Ring_Indication() Failure: %d.\r\n", Result);
   ...
   /* The following function is responsible for setting up or           */
   /* disconnecting an SCO connection to the Remote connected Headset.  */
...
static int SetupAudio(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(!IsHeadset)
      {
         ...
         if(HDSClientID)
         {
            /* Verify that a parameter was specified.                   */
            if((TempParam->NumberofParameters) && ((TempParam->Params[0].intParam == 0) || (TempParam->Params[0].intParam == 1)))
            {
               /* Check to see if we need to setup audio.               */
               if(TempParam->Params[0].intParam == 1)
               {
                  /* Verify that there is not already an audio          */
                  /* connection.                                        */
                  if(CurrentConnectionState != csAudioConnected)
                  {
                     Result = HDSET_Setup_Audio_Connection(BluetoothStackID, HDSClientID, FALSE);
                     if(Result >= 0)
                     {
                        /* Setup Audio was issued successfully so flag  */
                        /* success to the user.                         */
                        printf("HDSET_Setup_Audio_Connection() Success.\r\n");
                     ...
                     {
                        /* Setup Audio was issued unsuccessfully so flag*/
                        /* an error to the user.                        */
                        printf("HDSET_Setup_Audio_Connection() Failure: %d.\r\n", Result);
                  ...
                  {
                     printf("Audio already connected.\r\n");
               ...
               {
                  /* Verify that there is already an audio connection.  */
                  if(CurrentConnectionState == csAudioConnected)
                  {
                     Result = HDSET_Release_Audio_Connection(BluetoothStackID, HDSClientID);
                     if(Result >= 0)
                     {
                        /* Release Audio was issued successfully so flag*/
                        /* success to the user.                         */
                        printf("HDSET_Release_Audio_Connection() Success.\r\n");
                     ...
                     {
                        /* Release Audio was issued unsuccessfully so   */
                        /* flag an error to the user.                   */
                        printf("HDSET_Release_Audio_Connection() Failure: %d.\r\n", Result);
            ...
            else
            {
               ...
               /* user.                                                 */
               printf("Usage: SetupAudio [Disconnect = 0, Connect = 1)].\r\n");
   ...
   if((BluetoothStackID) && (HDSET_Event_Data))
   {
      switch(HDSET_Event_Data->Event_Data_Type)
      {
         case etHDSET_Open_Port_Indication:
            ...
            /* Note the Headset/Audio Gateway Server ID so that we can  */
            /* use Client functions on it.                              */
            HDSClientID            = HDSServerID;
         ...
         case etHDSET_Audio_Data_Indication:
            ...
            {
               printf("\r\n");
               printf("HDSET Audio Data Indication, ID: 0x%04X, Length 0x%04X: %02X%02X%02X%02X.\r\n", HDSET_Event_Data->Event_Data.HDSET_Audio_Data_Indication_Data->HDSETPortID, HDSET_Event_Data->Event_Data.HDSET_Audio_Data_Indication_Data->AudioDataLength, HDSET_Event_Data->Event_Data.HDSET_Audio_Data_Indication_Data->AudioData[0], HDSET_Event_Data->Event_Data.HDSET_Audio_Data_Indication_Data->AudioData[1], HDSET_Event_Data->Event_Data.HDSET_Audio_Data_Indication_Data->AudioData[2], HDSET_Event_Data->Event_Data.HDSET_Audio_Data_Indication_Data->AudioData[3]);
            ...
            if(AudioTestMode == AUDIO_DATA_TEST_LOOPBACK)
            {
               ...
               HDSET_Send_Audio_Data(BluetoothStackID, HDSET_Event_Data->Event_Data.HDSET_Audio_Data_Indication_Data->HDSETPortID, HDSET_Event_Data->Event_Data.HDSET_Audio_Data_Indication_Data->AudioDataLength, HDSET_Event_Data->Event_Data.HDSET_Audio_Data_Indication_Data->AudioData);
            }
            else
            {
               if(AudioTestMode == AUDIO_DATA_TEST_1KHZ_TONE)
               {
                  ...
                  /* Audio Buffer.                                      */
                  for(Index=0;Index<(unsigned int)HDSET_Event_Data->Event_Data.HDSET_Audio_Data_Indication_Data->AudioDataLength;Index++)
                  {
                  ...
                  /* data.                                              */
                  HDSET_Send_Audio_Data(BluetoothStackID, HDSET_Event_Data->Event_Data.HDSET_Audio_Data_Indication_Data->HDSETPortID, HDSET_Event_Data->Event_Data.HDSET_Audio_Data_Indication_Data->AudioDataLength, AudioData);
               }
            }
            break;
         default:
...
int main(int argc, char* argv[])
{
   ...
   if(argc >= NUM_EXPECTED_PARAMETERS_USB)
   {
      ...
      if(HCI_DriverInformationPtr)
      {
         ...
         IsHeadset = !strtol(argv[2], &endptr, 10);
            ...
            if(!Result)
            {
               ...
               if(!Result)
               {
                  ...
                  if(!Result)
                  {
                     ...
                     /* Pairable so open up either a Headset or an Audio*/
                     /* Gateway Server.                                 */
                     if(!OpenServer())
                     {
                        ...
                        if(IsHeadset)
                           UserInterface_Headset();
                        else
                           UserInterface_AudioGateway();
                     }
                  }
               }
            }
      ...
      else
      {
         ...
         /* invalid.                                                    */
         printf("Parameter Error (Expecting = [USB = 0, UART = 1, BCSP = 2 Flag] [Headset/Audio Gateway Flag (0 = Headset)] [IF !USB [Comm Port or Device File] [Baud Rate]])\r\n");
      }
   }
   else
   {

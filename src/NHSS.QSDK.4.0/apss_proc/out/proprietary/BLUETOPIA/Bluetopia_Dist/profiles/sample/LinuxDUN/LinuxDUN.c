...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXDUN - Simple Linux application using the Dial-up Networking Profile. */
/*                                                                            */
/*  Author:  Rory Sledge                                                      */
/*                                                                            */
...
/*   --------  -----------    ------------------------------------------------*/
/*   08/20/02  R. Sledge      Initial creation.                               */
/******************************************************************************/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>

#include "LinuxDUN.h"      /* Main Application Prototypes and Constants.      */

#include "SS1BTPS.h"       /* Includes the SS1 Bluetooth Protocol Stack.      */
#include "SS1BTDUN.h"      /* Includes the SS1 Dial-up Networking Profile.    */
#include "SS1BTDBG.h"      /* Includes/Constants for Bluetooth Debugging.     */

#define LOCAL_NAME_ROOT                      "LinuxDUN"  /* Root of the local */
                                                         /* name              */
...
#define DEFAULT_DEBUG_LOG_FILE_NAME  "LinuxDUN_ASC.log"
#define DEFAULT_DEBUG_FTS_FILE_NAME  "LinuxDUN_FTS.log"
...
static unsigned int        DUNServerID;             /* Variable which contains the     */
                                                    /* handle of the most recently     */
                                                    /* opened Dial-up Networking       */
                                                    /* Server.                         */

static DWord_t             DUNServerSDPHandle;      /* Variable which holds the handle */
                                                    /* to the Dial-up Networking SDP   */
                                                    /* Record.                         */
...
static unsigned int        DUNClientID;             /* Variable which contains the     */
                                                    /* handle of the most recently     */
                                                    /* opened Dial-up Networking       */
                                                    /* Client.                         */
...
static void BTPSAPI DUN_Event_Callback(unsigned int BluetoothStackID, DUN_Event_Data_t *DUN_Event_Data, unsigned long CallbackParameter);
...
static int DeleteLinkKey(BD_ADDR_t BD_ADDR)
{
   ...
   /* Command Options for this Dial-up Networking Sample Application.   */
...
static int Pair(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if((!DUNClientID) && (!ServerConnected))
      {
...
static int ServiceDiscovery(ParameterList_t *TempParam)
{
   ...
   /* Networking Server and registering a SDP record to be associated   */
...
static int OpenServer(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      /* Check to see if there is already DUN Server Port Open.         */
      if(!DUNServerID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters > 1) && (TempParam->Params[0].intParam))
         {
            ...
            /* Submit the request to Open a DUN Server Port.            */
            ret_val = DUN_Open_Server_Port(BluetoothStackID, TempParam->Params[0].intParam, TempParam->Params[1].intParam, BaudRate, FALSE, DUN_Event_Callback, 0);

            /* Next, check to see if the DUN_Open_Server_Port() function*/
            /* was successful.                                          */
            if(ret_val > 0)
            {
               /* Inform the user that the Server was successfully      */
               /* opened and save the Server Port ID for later use.     */
               printf("Return Value is %d DUN_Open_Server_Port().\r\n", ret_val);
               DUNServerID = ret_val;
               ...
               {
                  SET_MAJOR_DEVICE_CLASS(ClassOfDevice, HCI_LMP_CLASS_OF_DEVICE_MAJOR_DEVICE_CLASS_PHONE);
               ...
               sprintf(ServiceName, "Dial-up Networking Serial Port %u", TempParam->Params[0].intParam);

               /* Now that a Service Name has been created try and      */
               /* Register the SDP Record.                              */
               ret_val = DUN_Register_SDP_Record(BluetoothStackID, DUNServerID, ServiceName, &DUNServerSDPHandle);
               ...
               if(!ret_val)
               {
                  ...
                  printf("Return Value is %d DUN_Register_SDP_Record().\r\n", ret_val);
               }
               else
               {
                  /* Display an Error Message and make sure the Current */
                  /* Server SDP Handle is invalid, and close the DUN    */
                  /* Server Port we just opened because we weren't able */
                  /* to register a SDP Record.                          */
                  printf("Return Value is %d DUN_Register_SDP_Record() FAILURE.\r\n", ret_val);
                  DUNServerSDPHandle = 0;

                  /* Now try and close the Opened Server Port.          */
                  ret_val = DUN_Close_Server_Port(BluetoothStackID, DUNServerID);
                  DUNServerID = 0;
                  ...
                  if(!ret_val)
                  {
                     ...
                     /* successfully closed.                            */
                     printf("Return Value is %d DUN_Close_Server_Port().\r\n", ret_val);
                  }
                  else
                  {
                     /* An error occurred while attempting to close the */
                     /* Port.                                           */
                     printf("Return Value is %d DUN_Close_Server_Port() FAILURE.\r\n", ret_val);
                  }
               }
            }
            else
            {
               ...
               printf("Return Value is %d DUN_Open_Server_Port() FAILURE.\r\n", ret_val);
            }
         }
         else
         {
      ...
      {
         /* This application supports only one DUN Server at a time,    */
         /* inform the user that an DUN Server already exists.          */
         printf("DUN Server Already Exists.\r\n");
   ...
   /* Dial-up Networking Server and unregistering the SDP record        */
...
static int CloseServer(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      /* valid.                                                         */
      if(DUNServerSDPHandle)
      {
         /* The SDP Record Handle is valid so remove the record from the*/
         /* SDP Database.                                               */
         ret_val = DUN_Un_Register_SDP_Record(BluetoothStackID, CurrentPortID, DUNServerSDPHandle);
         ...
         if(!ret_val)
         {
            ...
            printf("Return Value is %d DUN_Un_Register_SDP_Record().\r\n", ret_val);
            DUNServerSDPHandle = 0;
         }
         else
         {
            ...
            /* Record.                                                  */
            printf("Return Value is %d DUN_Un_Register_SDP_Record() FAILURE.\r\n", ret_val);
         }
      }
      else
      {
      ...
      /* Check to see if there is an open DUN Server Port.              */
      if(DUNServerID)
      {
         /* Submit the request to Close a DUN Server Port.              */
         ret_val = DUN_Close_Server_Port(BluetoothStackID, DUNServerID);

         /* Next, check to see if the DUN_Close_Server_Port() function  */
         /* was successful.                                             */
         if(!ret_val)
         {
            printf("Return Value is %d DUN_Close_Server_Port().\r\n", ret_val);
   ...
   /* a remote Dial-up Networking Server.  This function returns zero on*/
   /* successful execution and a negative value on all errors.          */
static int OpenClient(ParameterList_t *TempParam)
{
   int       ret_val;
   ...
   if(BluetoothStackID)
   {
      /* Check to see if there is already DUN Client Port Open and that */
      ...
      if((!DUNClientID) && (!ServerConnected))
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters > 2) && (TempParam->Params[0].intParam) && (NumberofValidResponses) && (TempParam->Params[0].intParam <= NumberofValidResponses) && (!COMPARE_BD_ADDR(InquiryResultList[(TempParam->Params[0].intParam-1)], NullADDR)) && (TempParam->Params[1].intParam))
         {
            ...
            printf("DUN_Open_Remote_Port(BD_ADDR = %s).\r\n", BoardStr);

            /* Submit the request to Open a DUN Client Port.            */
            ret_val = DUN_Open_Remote_Port(BluetoothStackID, InquiryResultList[(TempParam->Params[0].intParam-1)], TempParam->Params[1].intParam, TempParam->Params[2].intParam, DUN_Event_Callback, 0);

            /* Next, check to see if the DUN_Open_Remote_Port() function*/
            /* was successful.                                          */
            if(ret_val > 0)
            {
               printf("Return Value is %d DUN_Open_Remote_Port().\r\n", ret_val);
            ...
            {
               printf("Return Value is %d DUN_Open_Remote_Port() FAILURE.\r\n", ret_val);
            }
         }
         else
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: OpenClient [Inquiry Index] [RFCOMM Port] [Local Device To Open].\r\n");
      ...
      {
         /* This application supports only one DUN Connection at a time,*/
         /* inform the user that an DUN Connection already exists.      */
         printf("DUN Connection Already Exists.\r\n");
   ...
   /* remote Dial-up Networking Port.  This function returns zero on    */
   /* successful execution and a negative value on all errors.          */
static int CloseClient(ParameterList_t *TempParam)
{
   int ret_val;
   ...
   if(BluetoothStackID)
   {
      /* Check to see if there is an open DUN Client Port or the Server */
      /* is currently connected.                                        */
      if(((ServerConnected)?DUNServerID:DUNClientID))
      {
         /* Submit the request to Close a DUN Client Port.              */
         ret_val = DUN_Close_Port(BluetoothStackID, ((ServerConnected)?DUNServerID:DUNClientID));

         /* Next, check to see if the DUN_Close_Port() function was     */
         /* successful.                                                 */
         if(!ret_val)
         {
            printf("Return Value is %d DUN_Close_Port().\r\n", ret_val);
         ...
         {
            printf("Return Value is %d DUN_Close_Port() FAILURE.\r\n", ret_val);
         }
      }
      else
      {
...
static void BTPSAPI SDP_Event_Callback(unsigned int BluetoothStackID, unsigned int SDPRequestID, SDP_Response_Data_t *SDP_Response_Data, unsigned long CallbackParameter)
{
   ...
   /* The following function is for the DUN Event Callback.  This       */
   /* function will be called whenever a Callback has been registered   */
   /* for the specified DUN Action that is associated with the Bluetooth*/
   /* Stack.  This function passes to the caller the DUN Event Data of  */
   /* the specified Event and the DUN Event Callback Parameter that was */
   /* specified when this Callback was installed.  The caller is free to*/
   /* use the contents of the DUN Event Data ONLY in the context of this*/
   /* callback.  If the caller requires the Data for a longer period of */
   /* time, then the callback function MUST copy the data into another  */
   /* Data Buffer.  This function is guaranteed NOT to be invoked more  */
   /* than once simultaneously for the specified installed callback     */
   ...
   /* the Thread Context of a Thread that the User does NOT own.        */
   /* Therefore, processing in this function should be as efficient as  */
   /* possible (this argument holds anyway because other DUN Events will*/
   /* not be processed while this function call is outstanding).        */
   /* * NOTE * This function MUST NOT Block and wait for events that    */
   /*          can only be satisfied by Receiving other DUN Events.  A  */
   /*          Deadlock WILL occur because NO DUN Event Callbacks will  */
   /*          be issued while this function is currently outstanding.  */
static void BTPSAPI DUN_Event_Callback(unsigned int BluetoothStackID, DUN_Event_Data_t *DUN_Event_Data, unsigned long CallbackParameter)
{
   char BoardStr[13];
   ...
   /* First, check to see if the required parameters appear to be       */
   /* semi-valid.                                                       */
   if((DUN_Event_Data) && (BluetoothStackID))
   {
      /* The parameters appear to be semi-valid, now check to see what  */
      /* type the incoming event is.                                    */
      switch(DUN_Event_Data->Event_Data_Type)
      {
         case etDUN_Open_Port_Indication:
            /* A Client has connected to the Server, display the BD_ADDR*/
            /* of the connecting device.                                */
            printf("\r\n");
            BD_ADDRToStr(DUN_Event_Data->Event_Data.DUN_Open_Port_Indication_Data->BD_ADDR, BoardStr);
            printf("DUN Open Port Indication, ID: 0x%04X, Board: %s.\r\n", DUN_Event_Data->Event_Data.DUN_Open_Port_Indication_Data->DUNPortID, BoardStr);

            ServerConnected = TRUE;
            break;
         case etDUN_Open_Port_Confirmation:
            /* The Client received a Connect Confirmation, display      */
            /* relevant information.                                    */
            printf("\r\n");
            printf("DUN Open Port Confirmation, ID: 0x%04X, Status: 0x%04X.\r\n", DUN_Event_Data->Event_Data.DUN_Open_Port_Confirmation_Data->DUNPortID,
                                                                                  DUN_Event_Data->Event_Data.DUN_Open_Port_Confirmation_Data->PortOpenStatus);

            /* Check to see if the Client Port was opened successfully. */
            if(DUN_Event_Data->Event_Data.DUN_Open_Port_Confirmation_Data->PortOpenStatus)
            {
            ...
            break;
         case etDUN_Close_Port_Indication:
            /* A Close Port Indication was received, display all        */
            /* relevant information.                                    */
            printf("\r\n");
            printf("DUN Close Port Indication, ID: 0x%04X.\r\n", DUN_Event_Data->Event_Data.DUN_Close_Port_Indication_Data->DUNPortID);

            /* Check to see if the closing port is the Client Port.     */
            if(DUN_Event_Data->Event_Data.DUN_Close_Port_Indication_Data->DUNPortID == DUNClientID)
            {
            ...
            break;
         default:
            /* We are not interested in any other DUN Events.           */
            break;
      }
   }
   else
   {
      /* There was an error with one or more of the input parameters.   */
      printf("DUN callback data: Event_Data = NULL.\r\n");
   }

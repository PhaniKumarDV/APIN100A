...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXHID - Simple Linux application using HID Profile.                    */
/*                                                                            */
/*  Author:  Rory Sledge                                                      */
/*                                                                            */
...
#include "LinuxHID.h"      /* Main Application Prototypes and Constants.      */
...
#define LOCAL_NAME_ROOT                      "LinuxHID"  /* Root of the local */
                                                         /* name              */
...
#define DEFAULT_DEBUG_LOG_FILE_NAME  "LinuxHID_ASC.log"
#define DEFAULT_DEBUG_FTS_FILE_NAME  "LinuxHID_FTS.log"
...
static int                 IsHost;                  /* Variable used to indicate if the*/
...
static void DisplayHIDHostMenu(void)
{
   ...
   printf("*  ConnectRemoteHIDDevice [Inquiry Index]                           *\r\n");
   printf("*  CloseConnection                                                  *\r\n");
   printf("*  ControlRequest [Control Operation]                               *\r\n");
   printf("*  GetReportRequest [Size] [ReportType] [ReportID] [BufferSize]     *\r\n");
   printf("*  SetReportRequest [ReportType]                                    *\r\n");
   printf("*  GetProtocolRequest                                               *\r\n");
   printf("*  SetProtocolRequest [Protocol]                                    *\r\n");
   printf("*  GetIdleRequest                                                   *\r\n");
   printf("*  SetIdleRequest [IdleRate]                                        *\r\n");
   printf("*  DataWrite [ReportType]                                           *\r\n");
...
static void DisplayHIDDeviceMenu(void)
{
   ...
   printf("*  GetRemoteName [Inquiry Index]                                    *\r\n");
   printf("*  ServiceDiscovery [Inquiry Index] [Profile Index] [UUID (Manual)] *\r\n");
   printf("*  ConnectRemoteHIDHost [Inquiry Index]                             *\r\n");
   printf("*  CloseConnection                                                  *\r\n");
   printf("*  ControlRequest                                                   *\r\n");
   printf("*  GetReportResponse [ResultType] [ReportType]                      *\r\n");
   printf("*  SetReportResponse [ResultType]                                   *\r\n");
   printf("*  GetProtocolResponse [ResultType] [Protocol]                      *\r\n");
   printf("*  SetProtocolResponse [ResultType]                                 *\r\n");
   printf("*  GetIdleResponse [ResultType] [IdleRate]                          *\r\n");
   printf("*  SetIdleResponse [ResultType]                                     *\r\n");
   printf("*  DataWrite [ReportType]                                           *\r\n");
   printf("*  EnableDebug [Enable/Disable] [Log Type] [Log File Name]          *\r\n");
   printf("*  Help                                                             *\r\n");
   printf("*  Quit                                                             *\r\n");
...
static void UserInterface_Main(void)
{
   ...
   while(Result != EXIT_CODE)
   {
            ...
            if(CommandParser(&TempCommand, UserInput) >= 0)
            {
               ...
               switch(Result)
               {
                  ...
                  case EXIT_CODE:
                     ...
                     /* go ahead an close any Remote HID Connection to a*/
                     /* Server that we have open.                       */
                     if(HIDID)
                        HIDCloseConnection(NULL);
                     break;
               }
            }
            else
      ...
      {
         /* Close any Remote HID Connection to a Server that we have    */
         /* open.                                                       */
         if(HIDID)
            HIDCloseConnection(NULL);
...
static int InitializeHIDHost(void)
{
   ...
   /* to be a HID Host Server.  First, attempt to set the device to be  */
   /* connectable.                                                      */
   if(!SetConnect())
   {
...
static int InitializeHIDDevice(void)
{
   ...
   /* The program is currently running in Device mode.  Ready the device*/
   /* to be a HID Device Server.  First, attempt to set the device to be*/
   /* connectable.                                                      */
   if(!SetConnect())
   {
      /* Now that the device is connectable attempt to make it          */
...
static int HIDRegisterDeviceServer(void)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(!Result)
      {
         /* The server was successfully registered, set the Server      */
         /* Registered flag to true.                                    */
         printf("HID_Register_Device_Server: Function Successful.\r\n");
         ...
         if(!Result)
         {
            ...
            printf("HID_Register_Device_SDP_Record: Function Successful.\r\n");
         }
         else
         {
            ...
            /* Record.                                                  */
            printf("HID_Register_Device_SDP_Record: Function Failure.\r\n");
            ...
            if(!Result)
            {
               ...
               printf("HID_Un_Register_Server: Function Successful.\r\n");
            }
            else
            {
               /* An error occurred while attempting to unregister the  */
               /* server.                                               */
               printf("HID_Un_Register_Server: Function Failure: %d.\r\n", Result);
            }
         }
      }
      else
      {
         /* An error occurred while attempting to register the server.  */
         /* Display an error message indicating the error.              */
         printf("HID_Register_Device_Server: Function Failure: %d.\r\n", Result);
      }
   }
   else
   {
...
static int HIDConnectRemoteDevice(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(!HIDID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params->intParam) && (NumberofValidResponses) && (TempParam->Params->intParam<=NumberofValidResponses) && (!COMPARE_BD_ADDR(InquiryResultList[(TempParam->Params->intParam-1)], NullADDR)))
         {
            ...
            if(ret_val > 0)
            {
               ...
               printf("HID_Connect_Remote_Device: Function Successful (ID = %04X).\r\n", ret_val);
               HIDID   = ret_val;
               ret_val = 0;
            }
            else
            {
               ...
               printf("HID_Connect_Remote_Device: Function Failure: %d.\r\n", ret_val);
            }
         }
         else
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: ConnectRemoteHIDDevice [Inquiry Index].\r\n");
            ret_val = INVALID_PARAMETERS_ERROR;
         }
      }
      else
      {
         ...
         printf("Ongoing connection already exists.\r\n");
         ret_val = FUNCTION_ERROR;
      }
   }
   else
   ...
   if(BluetoothStackID)
   {
      ...
      if(!HIDID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params->intParam) && (NumberofValidResponses) && (TempParam->Params->intParam<=NumberofValidResponses) && (!COMPARE_BD_ADDR(InquiryResultList[(TempParam->Params->intParam-1)], NullADDR)))
         {
            ...
            /* Host.                                                    */
            BD_ADDRToStr(InquiryResultList[(TempParam->Params->intParam-1)], BoardStr);
            printf("Open Remote HID Host(BD_ADDR = %s)\r\n", BoardStr);
            ...
            {
               /* The Connect Request was successfully submitted.       */
               printf("HID_Connect_Remote_Host: Function Successful (ID = %04X).\r\n", ret_val);
            ...
            {
               /* There was an error submitting the connection request. */
               printf("HID_Connect_Remote_Host: Function Failure: %d.\r\n", ret_val);
            }
         }
         else
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: ConnectRemoteHIDHost [Inquiry Index].\r\n");
            ret_val = INVALID_PARAMETERS_ERROR;
         }
      }
      else
   ...
   if(BluetoothStackID)
   {
      ...
      if(HIDID)
      {
         ...
         if(!ret_val)
         {
            ...
            printf("HID_Close_Connection: Function Successful.\r\n");
            HIDID = 0;
         }
         else
         {
            ...
            /* Connection.                                              */
            printf("HID_Close_Connection: Function Failure: %d.\r\n", ret_val);
         }
      }
      else
      {
         /* The HID ID is invalid.                                      */
         printf("Close HID Connection: Invalid HIDID.\r\n");
         ret_val = FUNCTION_ERROR;
      }
   }
   else
...
static int HIDControlRequest(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(HIDID)
      {
         ...
         /* make sure that the parameters required for this function    */
         /* appear to be valid.                                         */
         if((!IsHost) || ((IsHost) && (TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params->intParam >= hcNop) && (TempParam->Params->intParam <= hcVirtualCableUnplug)))
         {
            /* The parameter appears to be at least semi-valid now      */
            /* attempt to submit the HID Control Request.               */
            ret_val = HID_Control_Request(BluetoothStackID, HIDID, (HID_Control_Operation_Type_t)((!IsHost)?(hcVirtualCableUnplug):(TempParam->Params->intParam)));
            ...
            if(!ret_val)
            {
               ...
               /* successfully submitted.                               */
               printf("HID_Control_Request: Function Successful.\r\n");
            }
            else
            {
               /* An error occurred while attempting to submit the HID  */
               /* Control Request command.                              */
               printf("HID_Control_Request: Function Failure: %d.\r\n", ret_val);
            }
         }
         else
         {
            ...
            if(IsHost)
               printf("Usage: ControlRequest [Control Operation].\r\n");
            else
               printf("Usage: ControlRequest.\r\n");
      ...
      {
         /* The HID ID is invalid.                                      */
         printf("HID Control Request: Invalid HIDID.\r\n");
         ret_val = FUNCTION_ERROR;
      }
   }
   else
...
static int HIDGetReportRequest(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(HIDID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters > 2) && (TempParam->Params[0].intParam >= grSizeOfReport) && (TempParam->Params[0].intParam <= grUseBufferSize) && (TempParam->Params[1].intParam >= rtInput) && (TempParam->Params[1].intParam <= rtFeature))
         {
            ...
            if(!ret_val)
            {
               ...
               /* successfully submitted.                               */
               printf("HID_Get_Report_Request: Function Successful.\r\n");
            }
            else
            {
               /* An error occurred while attempting to submit the HID  */
               /* Get Report Request command.                           */
               printf("HID_Get_Report_Request: Function Failure: %d.\r\n", ret_val);
            }
         }
         else
         {
            /* The required parameter is invalid.                       */
            printf("Usage: GetReportRequest [Size] [ReportType] [ReportID] [BufferSize].\r\n");
            ret_val = INVALID_PARAMETERS_ERROR;
         }
      }
      else
      {
         /* The HID ID is invalid.                                      */
         printf("HID Get Report Request: Invalid HIDID.\r\n");
         ret_val = FUNCTION_ERROR;
      }
   }
   else
...
static int HIDGetReportResponse(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(HIDID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters > 1) && (TempParam->Params[0].intParam >= rtSuccessful) && (TempParam->Params[0].intParam <= rtData) && (TempParam->Params[1].intParam >= rtInput) && (TempParam->Params[1].intParam <= rtFeature))
         {
            ...
            if(!ret_val)
            {
               ...
               /* successfully submitted.                               */
               printf("HID_Get_Report_Response: Function Successful.\r\n");
            }
            else
            {
               /* An error occurred while attempting to submit the HID  */
               /* Get Report Response command.                          */
               printf("HID_Get_Report_Response: Function Failure: %d.\r\n", ret_val);
            }
         }
         else
         {
            /* The required parameter is invalid.                       */
            printf("Usage: GetReportResponse [ResultType] [ReportType].\r\n");
            ret_val = INVALID_PARAMETERS_ERROR;
         }
      }
      else
      {
         /* The HID ID is invalid.                                      */
         printf("HID Get Report Response: Invalid HIDID.\r\n");
         ret_val = FUNCTION_ERROR;
      }
   }
   else
...
static int HIDSetReportRequest(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(HIDID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].intParam >= rtInput) && (TempParam->Params[0].intParam <= rtFeature))
         {
            ...
            if(!ret_val)
            {
               ...
               /* successfully submitted.                               */
               printf("HID_Set_Report_Request: Function Successful.\r\n");
            }
            else
            {
               /* An error occurred while attempting to submit the HID  */
               /* Set Report Request command.                           */
               printf("HID_Set_Report_Request: Function Failure: %d.\r\n", ret_val);
            }
         }
         else
         {
            /* The required parameter is invalid.                       */
            printf("Usage: SetReportRequest [ReportType].\r\n");
            ret_val = INVALID_PARAMETERS_ERROR;
         }
      }
      else
      {
         /* The HID ID is invalid.                                      */
         printf("HID Set Report Request: Invalid HIDID.\r\n");
         ret_val = FUNCTION_ERROR;
      }
   }
   else
...
static int HIDSetReportResponse(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(HIDID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].intParam >= rtSuccessful) && (TempParam->Params[0].intParam <= rtData))
         {
            ...
            if(!ret_val)
            {
               ...
               /* successfully submitted.                               */
               printf("HID_Set_Report_Response: Function Successful.\r\n");
            }
            else
            {
               /* An error occurred while attempting to submit the HID  */
               /* Set Report Response command.                          */
               printf("HID_Set_Report_Response: Function Failure: %d.\r\n", ret_val);
            }
         }
         else
         {
            /* The required parameter is invalid.                       */
            printf("Usage: SetReportResponse [ResultType].\r\n");
            ret_val = INVALID_PARAMETERS_ERROR;
         }
      }
      else
      {
         /* The HID ID is invalid.                                      */
         printf("HID Set Report Response: Invalid HIDID.\r\n");
         ret_val = FUNCTION_ERROR;
      }
   }
   else
   ...
   if(BluetoothStackID)
   {
      ...
      if(HIDID)
      {
         ...
         if(!ret_val)
         {
            ...
            /* successfully submitted.                                  */
            printf("HID_Get_Protocol_Request: Function Successful.\r\n");
         }
         else
         {
            /* An error occurred while attempting to submit the HID Get */
            /* Protocol Request command.                                */
            printf("HID_Get_Protocol_Request: Function Failure: %d.\r\n", ret_val);
         }
      }
      else
      {
         /* The HID ID is invalid.                                      */
         printf("HID Get Protocol Request: Invalid HIDID.\r\n");
         ret_val = FUNCTION_ERROR;
      }
   }
   else
...
static int HIDGetProtocolResponse(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(HIDID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters > 1) && (TempParam->Params[0].intParam >= rtSuccessful) && (TempParam->Params[0].intParam <= rtData) && (TempParam->Params[1].intParam >= ptBoot) && (TempParam->Params[1].intParam <= ptReport))
         {
            ...
            if(!ret_val)
            {
               ...
               /* successfully submitted.                               */
               printf("HID_Get_Protocol_Response: Function Successful.\r\n");
            }
            else
            {
               /* An error occurred while attempting to submit the HID  */
               /* Get Protocol Response command.                        */
               printf("HID_Get_Protocol_Response: Function Failure: %d.\r\n", ret_val);
            }
         }
         else
         {
            /* The required parameter is invalid.                       */
            printf("Usage: GetProtocolResponse [ResultType] [Protocol].\r\n");
            ret_val = INVALID_PARAMETERS_ERROR;
         }
      }
      else
      {
         /* The HID ID is invalid.                                      */
         printf("HID Get Protocol Response: Invalid HIDID.\r\n");
         ret_val = FUNCTION_ERROR;
      }
   }
   else
...
static int HIDSetProtocolRequest(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(HIDID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].intParam >= ptBoot) && (TempParam->Params[0].intParam <= ptReport))
         {
            ...
            if(!ret_val)
            {
               ...
               /* successfully submitted.                               */
               printf("HID_Set_Protocol_Request: Function Successful.\r\n");
            }
            else
            {
               /* An error occurred while attempting to submit the HID  */
               /* Set Protocol Request command.                         */
               printf("HID_Set_Protocol_Request: Function Failure: %d.\r\n", ret_val);
            }
         }
         else
         {
            /* The required parameter is invalid.                       */
            printf("Usage: SetProtocolRequest [Protocol].\r\n");
            ret_val = INVALID_PARAMETERS_ERROR;
         }
      }
      else
      {
         /* The HID ID is invalid.                                      */
         printf("HID Set Protocol Request: Invalid HIDID.\r\n");
         ret_val = FUNCTION_ERROR;
      }
   }
   else
...
static int HIDSetProtocolResponse(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(HIDID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].intParam >= rtSuccessful) && (TempParam->Params[0].intParam <= rtData))
         {
            ...
            if(!ret_val)
            {
               ...
               /* successfully submitted.                               */
               printf("HID_Set_Protocol_Response: Function Successful.\r\n");
            }
            else
            {
               /* An error occurred while attempting to submit the HID  */
               /* Set Protocol Response command.                        */
               printf("HID_Set_Protocol_Response: Function Failure: %d.\r\n", ret_val);
            }
         }
         else
         {
            /* The required parameter is invalid.                       */
            printf("Usage: SetProtocolResponse [ResultType].\r\n");
            ret_val = INVALID_PARAMETERS_ERROR;
         }
      }
      else
      {
         /* The HID ID is invalid.                                      */
         printf("HID Set Protocol Response: Invalid HIDID.\r\n");
         ret_val = FUNCTION_ERROR;
      }
   }
   else
   ...
   if(BluetoothStackID)
   {
      ...
      if(HIDID)
      {
         ...
         if(!ret_val)
         {
            ...
            /* successfully submitted.                                  */
            printf("HID_Get_Idle_Request: Function Successful.\r\n");
         }
         else
         {
            /* An error occurred while attempting to submit the HID Get */
            /* Idle Request command.                                    */
            printf("HID_Get_Idle_Request: Function Failure: %d.\r\n", ret_val);
         }
      }
      else
      {
         /* The HID ID is invalid.                                      */
         printf("HID Get Idle Request: Invalid HIDID.\r\n");
         ret_val = FUNCTION_ERROR;
      }
   }
   else
...
static int HIDGetIdleResponse(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(HIDID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters > 1) && (TempParam->Params[0].intParam >= rtSuccessful) && (TempParam->Params[0].intParam <= rtData))
         {
            ...
            if(!ret_val)
            {
               ...
               /* successfully submitted.                               */
               printf("HID_Get_Idle_Response: Function Successful.\r\n");
            }
            else
            {
               /* An error occurred while attempting to submit the HID  */
               /* Get Idle Response command.                            */
               printf("HID_Get_Idle_Response: Function Failure: %d.\r\n", ret_val);
            }
         }
         else
         {
            /* The required parameter is invalid.                       */
            printf("Usage: GetIdleResponse [ResultType] [IdleRate].\r\n");
            ret_val = INVALID_PARAMETERS_ERROR;
         }
      }
      else
      {
         /* The HID ID is invalid.                                      */
         printf("HID Get Idle Response: Invalid HIDID.\r\n");
         ret_val = FUNCTION_ERROR;
      }
   }
   else
...
static int HIDSetIdleRequest(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(HIDID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters > 0))
         {
            ...
            if(!ret_val)
            {
               ...
               /* successfully submitted.                               */
               printf("HID_Set_Idle_Request: Function Successful.\r\n");
            }
            else
            {
               /* An error occurred while attempting to submit the HID  */
               /* Set Idle Request command.                             */
               printf("HID_Set_Idle_Request: Function Failure: %d.\r\n", ret_val);
            }
         }
         else
         {
            /* The required parameter is invalid.                       */
            printf("Usage: SetIdleRequest [IdleRate].\r\n");
            ret_val = INVALID_PARAMETERS_ERROR;
         }
      }
      else
      {
         /* The HID ID is invalid.                                      */
         printf("HID Set Idle Request: Invalid HIDID.\r\n");
         ret_val = FUNCTION_ERROR;
      }
   }
   else
...
static int HIDSetIdleResponse(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(HIDID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].intParam >= rtSuccessful) && (TempParam->Params[0].intParam <= rtData))
         {
            ...
            if(!ret_val)
            {
               ...
               /* successfully submitted.                               */
               printf("HID_Set_Idle_Response: Function Successful.\r\n");
            }
            else
            {
               /* An error occurred while attempting to submit the HID  */
               /* Set Idle Response command.                            */
               printf("HID_Set_Idle_Response: Function Failure: %d.\r\n", ret_val);
            }
         }
         else
         {
            /* The required parameter is invalid.                       */
            printf("Usage: SetIdleResponse [ResultType].\r\n");
            ret_val = INVALID_PARAMETERS_ERROR;
         }
      }
      else
      {
         /* The HID ID is invalid.                                      */
         printf("HID Set Idle Response: Invalid HIDID.\r\n");
         ret_val = FUNCTION_ERROR;
      }
   }
   else
      ...
      if(HIDID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].intParam >= rtInput) && (TempParam->Params[0].intParam <= rtFeature))
         {
            ...
            if(!ret_val)
            {
               ...
               /* successfully submitted.                               */
               printf("HID_Data_Write: Function Successful.\r\n");
            }
            else
            {
               /* An error occurred while attempting to submit the HID  */
               /* Data Write command.                                   */
               printf("HID_Data_Write: Function Failure: %d.\r\n", ret_val);
            }
         }
         else
         {
            /* The required parameter is invalid.                       */
            printf("Usage: DataWrite [ReportType].\r\n");
            ret_val = INVALID_PARAMETERS_ERROR;
         }
      }
      else
   ...
   if((BluetoothStackID) && (HIDEventData != NULL))
   {
      ...
      switch(HIDEventData->Event_Data_Type)
      {
         ...
         case etHID_Close_Indication:
            ...
            /* information.                                             */
            printf("HID Close Indication, ID: 0x%04X\r\n", HIDEventData->Event_Data.HID_Close_Indication_Data->HIDID);

            /* Reset the HIDID if this is the last HID connection.      */
            /* * NOTE * Only one HID connection will be allowed.  The   */
            /*          extra attempts will be automatically closed     */
            /*          without resetting the HIDID for the current     */
            /*          connection.                                     */
            if(HIDID == HIDEventData->Event_Data.HID_Close_Indication_Data->HIDID)
            {
               HIDID = 0;
            }
            break;
         case etHID_Control_Indication:
            ...
            printf("HID Control Indication, ID: 0x%04X, Control Operation: ", HIDEventData->Event_Data.HID_Control_Indication_Data->HIDID);
            ...
            switch(HIDEventData->Event_Data.HID_Control_Indication_Data->ControlOperation)
            {
               ...
               case hcSuspend:
                  ...
                  break;
               case hcExitSuspend:
                  printf("hcExitSuspend\r\n");
                  break;
               case hcVirtualCableUnplug:
                  printf("hcVirtualCableUnplug\r\n");
                  break;
               default:
                  printf("Unknown Control Operation Type\r\n");
                  break;
            }
            break;
         case etHID_Get_Report_Indication:
            ...
            /* relevant information.                                    */
            printf("HID Get Report Indication, ID: 0x%04X, ", HIDEventData->Event_Data.HID_Get_Report_Indication_Data->HIDID);

            switch(HIDEventData->Event_Data.HID_Get_Report_Indication_Data->ReportType)
            {
               case rtOther:
                  printf("ReportType: rtOther, ");
                  break;
               case rtInput:
                  printf("ReportType: rtInput, ");
                  break;
               case rtOutput:
                  printf("ReportType: rtOutput, ");
                  break;
               case rtFeature:
                  printf("ReportType: rtFeature, ");
                  break;
               default:
                  printf("ReportType: Unknown Report Type, ");
                  break;
            }

            printf("ReportID: %u, ", HIDEventData->Event_Data.HID_Get_Report_Indication_Data->ReportID);

            switch(HIDEventData->Event_Data.HID_Get_Report_Indication_Data->Size)
            {
               case grSizeOfReport:
                  printf("Size: grSizeOfReport, ");
                  break;
               case grUseBufferSize:
                  printf("Size: grUseBufferSize, ");
                  break;
               default:
                  printf("Size: Unknown Size Type, ");
                  break;
            }
         ...
         case etHID_Get_Report_Confirmation:
            ...
            /* relevant information.                                    */
            printf("HID Get Report Confirmation, ID: 0x%04X, ", HIDEventData->Event_Data.HID_Get_Report_Confirmation_Data->HIDID);

            switch(HIDEventData->Event_Data.HID_Get_Report_Confirmation_Data->Status)
            {
               case rtSuccessful:
                  printf("Status: rtSuccessful, ");
                  break;
               case rtNotReady:
                  printf("Status: rtNotReady, ");
                  break;
               case rtErrInvalidReportID:
                  printf("Status: rtErrInvalidReportID, ");
                  break;
               case rtErrUnsupportedRequest:
                  printf("Status: rtErrUnsupportedRequest, ");
                  break;
               case rtErrInvalidParameter:
                  printf("Status: rtErrInvalidParameter, ");
                  break;
               case rtErrUnknown:
                  printf("Status: rtErrUnknown, ");
                  break;
               case rtErrFatal:
                  printf("Status: rtErrFatal, ");
                  break;
               case rtData:
                  printf("Status: rtData, ");
                  break;
               default:
                  printf("Status: Unknown Status Type, ");
                  break;
            }
         ...
         case etHID_Set_Report_Indication:
            ...
            /* relevant information.                                    */
            printf("HID Set Report Indication, ID: 0x%04X, ", HIDEventData->Event_Data.HID_Set_Report_Indication_Data->HIDID);
            ...
            {
               printf("0x%02X ", HIDEventData->Event_Data.HID_Set_Report_Indication_Data->ReportDataPayload[Index]);
            }
         ...
         case etHID_Set_Report_Confirmation:
            ...
            /* relevant information.                                    */
            printf("HID Set Report Confirmation, ID: 0x%04X, ", HIDEventData->Event_Data.HID_Set_Report_Confirmation_Data->HIDID);
         ...
         case etHID_Get_Protocol_Indication:
            ...
            /* relevant information.                                    */
            printf("HID Get Protocol Indication, ID: 0x%04X\r\n", HIDEventData->Event_Data.HID_Get_Protocol_Indication_Data->HIDID);
            break;
         case etHID_Get_Protocol_Confirmation:
            /* A HID Get Protocol Confirmation was received, display all*/
            /* relevant information.                                    */
            printf("HID Get Protocol Confirmation, ID: 0x%04X, ", HIDEventData->Event_Data.HID_Get_Protocol_Confirmation_Data->HIDID);
         ...
         case etHID_Set_Protocol_Confirmation:
            ...
            /* relevant information.                                    */
            printf("HID Set Protocol Confirmation, ID: 0x%04X, ", HIDEventData->Event_Data.HID_Set_Protocol_Confirmation_Data->HIDID);
         ...
         case etHID_Get_Idle_Indication:
            ...
            /* relevant information.                                    */
            printf("HID Get Idle Indication, ID: 0x%04X\r\n", HIDEventData->Event_Data.HID_Get_Idle_Indication_Data->HIDID);
            break;
         case etHID_Get_Idle_Confirmation:
            /* A HID Get Idle Confirmation was received, display all    */
            /* relevant information.                                    */
            printf("HID Get Idle Confirmation, ID: 0x%04X, ", HIDEventData->Event_Data.HID_Get_Idle_Confirmation_Data->HIDID);
         ...
         case etHID_Set_Idle_Confirmation:
            ...
            /* relevant information.                                    */
            printf("HID Set Idle Confirmation, ID: 0x%04X, ", HIDEventData->Event_Data.HID_Set_Idle_Confirmation_Data->HIDID);
         ...
         case etHID_Data_Indication:
            ...
            /* information.                                             */
            printf("HID Data Indication, ID: 0x%04X, ", HIDEventData->Event_Data.HID_Data_Indication_Data->HIDID);
            ...
            {
               printf("0x%02X ", HIDEventData->Event_Data.HID_Data_Indication_Data->ReportDataPayload[Index]);
            }
...
int main(int argc, char* argv[])
{
   ...
   if(argc >= NUM_EXPECTED_PARAMETERS_USB)
   {
      ...
      if(HCI_DriverInformationPtr)
      {
         ...
         IsHost = strtol(argv[2], &endptr, 10);
         ...
         if(!OpenStack(HCI_DriverInformationPtr))
         {
            ...
            if(IsHost)
               ret_val = InitializeHIDHost();
            else
               ret_val = InitializeHIDDevice();

            /* The Host or Deivce has been initialized, now show the    */
      ...
      else
      {
         ...
         /* invalid.                                                    */
         printf("Parameter Error (Expecting = [USB = 0, UART = 1, BCSP = 2 Flag] [Device/Host Flag (0 = Device)] [IF !USB [Comm Port or Device File] [Baud Rate]])\r\n");
      }
   }
   else
   {

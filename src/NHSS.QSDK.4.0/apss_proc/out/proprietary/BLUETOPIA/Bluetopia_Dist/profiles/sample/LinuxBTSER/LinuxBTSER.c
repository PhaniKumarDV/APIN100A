...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXBTSER - Simple Linux application using the Bluetooth Serial Profile. */
/*                                                                            */
/*  Author:  Rory Sledge                                                      */
/*                                                                            */
...
/*   --------  -----------    ------------------------------------------------*/
/*   07/19/01  R. Sledge      Initial creation.                               */
/******************************************************************************/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>

#include "LinuxBTSER.h"    /* Main Application Prototypes and Constants.      */

#include "SS1BTPS.h"       /* Includes the SS1 Bluetooth Protocol Stack.      */
#include "SS1BTSER.h"      /* Includes the SS1 Bluetooth Serial Profile.      */
#include "SS1BTDBG.h"      /* Includes/Constants for Bluetooth Debugging.     */

#define LOCAL_NAME_ROOT                    "LinuxBTSER"  /* Root of the local */
                                                         /* name              */
...
#define DEFAULT_DEBUG_LOG_FILE_NAME  "LinuxBTSER_ASC.log"
#define DEFAULT_DEBUG_FTS_FILE_NAME  "LinuxBTSER_FTS.log"
...
static unsigned int        BTSERServerID;           /* Variable which contains the     */
                                                    /* handle of the most recently     */
                                                    /* opened Bluetooth Serial Port    */
                                                    /* Server.                         */

static DWord_t             BTSERServerSDPHandle;    /* Variable which holds the handle */
                                                    /* to the Bluetooth Serial Port    */
                                                    /* SDP Record.                     */

static Boolean_t           ServerConnected;         /* Variable which contains the     */
                                                    /* current connection state of the */
                                                    /* Bluetooth Serial Port Server.   */

static unsigned int        BTSERClientID;           /* Variable which contains the     */
                                                    /* handle of the most recently     */
                                                    /* opened Bluetooth Serial Port    */
                                                    /* Client.                         */
...
static int OpenClient(ParameterList_t *TempParam);
...
static void BTPSAPI BTSER_Event_Callback(unsigned int BluetoothStackID, BTSER_Event_Data_t *BTSER_Event_Data, unsigned long CallbackParameter);
...
static void UserInterface(void)
{
   ...
   /* First display the available commands for each mode.               */
   DisplayHelp(NULL);

   /* Now let's add the commands to the command list.                   */
   ClearCommands();
   ...
   AddCommand("OPENCLIENT", OpenClient);
   AddCommand("CLOSECLIENT", CloseClient);
   AddCommand("ENABLEDEBUG", EnableDebug);
   AddCommand("HELP", DisplayHelp);

   /* This is the Main Loop of the program.  It gets user input from the*/
   /* Command Window, makes a call to the Command Parser, and executes  */
   /* the Command.  After the function has been executed it then checks */
   /* the return value and displays an Error Message (if appropriate).  */
   /* If the result returned is ever EXIT_CODE constant the loop will   */
   /* exit, leading to the exit of the program.                         */
   while(Result != EXIT_CODE)
   {
      /* Initialize the value of the variable used to store the users   */
      /* input.  Output the string "Input: " to the command window to   */
      /* inform the user that another command may be entered.           */
      UserInput[0] = '\0';
...
static int DeleteLinkKey(BD_ADDR_t BD_ADDR)
{
   ...
   /* Command Options for this Bluetooth Serial Port Sample Application.*/
   /* The input parameter to this function is completely ignored, and   */
   /* only needs to be passed in because all Commands that can be       */
   /* entered at the Prompt pass in the parsed information.  This       */
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("*                  GetRemoteName, ServiceDiscovery, OpenServer,  *\r\n");
   printf("*                  CloseServer, OpenClient, CloseClient,         *\r\n");
...
static int Pair(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if((!BTSERClientID) && (!ServerConnected))
      {
...
static int ServiceDiscovery(ParameterList_t *TempParam)
{
   ...
   /* The following function is responsible for opening a Bluetooth     */
   /* Serial Port Server and registering a SDP record to be associated  */
   /* with the opened server.  This function returns zero on successful */
   /* execution and a negative value on all errors.                     */
static int OpenServer(ParameterList_t *TempParam)
{
   int          ret_val;
   ...
   if(BluetoothStackID)
   {
      /* Check to see if there is already BTSER Server Port Open.       */
      if(!BTSERServerID)
      {
         ...
         {
            /* Check to see if a second parameter was specified, if so  */
            /* set the Baud Rate Equal to that value.                   */
            BaudRate = 0;
            if((TempParam->NumberofParameters > 2) && (TempParam->Params[2].intParam))
               BaudRate = TempParam->Params[2].intParam;

            /* Submit the request to Open a BTSER Server Port.          */
            ret_val = BTSER_Open_Server_Port(BluetoothStackID, TempParam->Params[0].intParam, TempParam->Params[1].intParam, BaudRate, FALSE, BTSER_Event_Callback, 0);

            /* Next, check to see if the BTSER_Open_Server_Port()       */
            /* function was successful.                                 */
            if(ret_val > 0)
            {
               /* Inform the user that the Server was successfully      */
               /* opened and save the Server Port ID for later use.     */
               printf("Return Value is %d BTSER_Open_Server_Port().\r\n", ret_val);
               BTSERServerID = ret_val;

               /* Nex register a SDP record indicating that a server    */
               /* exists.  Do this by first creating a Service Name.    */
               sprintf(ServiceName, "Bluetooth Serial Port %u", TempParam->Params[0].intParam);

               /* Now that a Service Name has been created try and      */
               /* Register the SDP Record.                              */
               ret_val = BTSER_Register_SDP_Record(BluetoothStackID, BTSERServerID, NULL, ServiceName, &BTSERServerSDPHandle);
               ...
               if(!ret_val)
               {
                  ...
                  /* for the Server was registered successfully.        */
                  printf("Return Value is %d BTSER_Register_SDP_Record().\r\n", ret_val);
               }
               else
               {
                  /* Display an Error Message and make sure the Current */
                  /* Server SDP Handle is invalid, and close the BTSER  */
                  /* Server Port we just opened because we weren't able */
                  /* to register a SDP Record.                          */
                  printf("Return Value is %d BTSER_Register_SDP_Record() FAILURE.\r\n", ret_val);
                  BTSERServerSDPHandle = 0;

                  /* Now try and close the Opened Server Port.          */
                  ret_val = BTSER_Close_Server_Port(BluetoothStackID, BTSERServerID);
                  BTSERServerID = 0;
                  ...
                  if(!ret_val)
                  {
                     ...
                     /* successfully closed.                            */
                     printf("Return Value is %d BTSER_Close_Server_Port().\r\n", ret_val);
                  }
                  else
                  {
                     /* An error occurred while attempting to close the */
                     /* Port.                                           */
                     printf("Return Value is %d BTSER_Close_Server_Port() FAILURE.\r\n", ret_val);
                  }
               }
            }
            else
            {
               /* An error occurred while attempting to open server port*/
               /* inform the user of the errror.                        */
               printf("Return Value is %d BTSER_Open_Server_Port() FAILURE.\r\n", ret_val);
            }
         }
         else
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: OpenServer [RFCOMM Port] [Local Device To Open] [Baud Rate (Optional)].\r\n");
      ...
      {
         /* This application supports only one BTSER Server at a time,  */
         /* inform the user that an BTSER Server already exists.        */
         printf("BTSER Server Already Exists.\r\n");
   ...
   /* Bluetooth Serial Port Server and unregistering the SDP record     */
   /* associated with the open server.  This function returns zero on   */
   /* successful execution and a negative value on all errors.          */
static int CloseServer(ParameterList_t *TempParam)
{
   int ret_val;
   ...
   if(BluetoothStackID)
   {
      ...
      /* valid.                                                         */
      if(BTSERServerSDPHandle)
      {
         /* The SDP Record Handle is valid so remove the record from the*/
         /* SDP Database.                                               */
         ret_val = BTSER_Un_Register_SDP_Record(BluetoothStackID, CurrentPortID, BTSERServerSDPHandle);
         ...
         if(!ret_val)
         {
            ...
            printf("Return Value is %d BTSER_Un_Register_SDP_Record().\r\n", ret_val);
            BTSERServerSDPHandle = 0;
         }
         else
         {
            ...
            /* Record.                                                  */
            printf("Return Value is %d BTSER_Un_Register_SDP_Record() FAILURE.\r\n", ret_val);
         }
      }
      else
      {
         /* The Current Server SDP Handle is invalid so the SDP Record  */
         /* must already have been removed or never installed.          */
         printf("Invalid SDP Record Handle: CloseServer().\r\n");
      }

      /* Check to see if there is an open BTSER Server Port.            */
      if(BTSERServerID)
      {
         /* Submit the request to Close a BTSER Server Port.            */
         ret_val = BTSER_Close_Server_Port(BluetoothStackID, BTSERServerID);

         /* Next, check to see if the BTSER_Close_Server_Port() function*/
         /* was successful.                                             */
         if(!ret_val)
         {
            printf("Return Value is %d BTSER_Close_Server_Port().\r\n", ret_val);
      ...
      {
         /* No Server exists to close.                                  */
         printf("No Server Exists to Close.\r\n");
         ret_val = FUNCTION_ERROR;
      }
   }
   else
   ...
   /* a remote Bluetooth Serial Port.  This function returns zero on    */
   /* successful execution and a negative value on all errors.          */
static int OpenClient(ParameterList_t *TempParam)
{
   int          ret_val;
   ...
   if(BluetoothStackID)
   {
      /* Check to see if there is already BTSER Client Port Open and    */
      ...
      if((!BTSERClientID) && (!ServerConnected))
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters > 2) && (TempParam->Params[0].intParam) && (NumberofValidResponses) && (TempParam->Params[0].intParam<=NumberofValidResponses) && (!COMPARE_BD_ADDR(InquiryResultList[(TempParam->Params[0].intParam-1)], NullADDR)) && (TempParam->Params[1].intParam))
         {
            ...
            if((TempParam->NumberofParameters > 3) && (TempParam->Params[3].intParam))
               BaudRate = TempParam->Params[3].intParam;

            /* Print the BD_ADDR that we are attempting to connect with.*/
            BD_ADDRToStr(InquiryResultList[(TempParam->Params[0].intParam-1)], BoardStr);
            printf("BTSER_Open_Remote_Port(BD_ADDR = %s).\r\n", BoardStr);

            /* Submit the request to Open a BTSER Client Port.          */
            ret_val = BTSER_Open_Remote_Port(BluetoothStackID, InquiryResultList[(TempParam->Params[0].intParam-1)], TempParam->Params[1].intParam, TempParam->Params[2].intParam, BaudRate, FALSE, BTSER_Event_Callback, 0);

            /* Next, check to see if the BTSER_Open_Remote_Port()       */
            /* function was successful.                                 */
            if(ret_val > 0)
            {
               printf("Return Value is %d BTSER_Open_Remote_Port().\r\n", ret_val);

               BTSERClientID = ret_val;
               ret_val       = 0;
            }
            else
            {
               printf("Return Value is %d BTSER_Open_Remote_Port() FAILURE.\r\n", ret_val);
            }
         }
         else
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: OpenClient [Inquiry Index] [RFCOMM Port] [Local Device To Open] [Baud Rate (Optional)].\r\n");
      ...
      {
         /* This application supports only one BTSER Connection at a    */
         /* time, inform the user that an BTSER Connection already      */
         /* exists.                                                     */
         printf("BTSER Connection Already Exists.\r\n");
   ...
   /* This function is responsible for closing a connection with a      */
   /* remote Bluetooth Serial Port.  This function returns zero on      */
   /* successful execution and a negative value on all errors.          */
static int CloseClient(ParameterList_t *TempParam)
{
   int ret_val;
   ...
   if(BluetoothStackID)
   {
      /* Check to see if there is an open Client Port or the Server is  */
      /* currently connected.                                           */
      if(((ServerConnected)?BTSERServerID:BTSERClientID))
      {
         /* Submit the request to Close a BTSER Client Port.            */
         ret_val = BTSER_Close_Port(BluetoothStackID, ((ServerConnected)?BTSERServerID:BTSERClientID));

         /* Next, check to see if the BTSER_Close_Port() function was   */
         /* successful.                                                 */
         if(!ret_val)
         {
            printf("Return Value is %d BTSER_Close_Port().\r\n", ret_val);
            BTSERClientID   = 0;
            ServerConnected = FALSE;
         }
         else
         {
            printf("Return Value is %d BTSER_Close_Port() FAILURE.\r\n", ret_val);
         }
      }
      else
      {
         /* No Server exists to close.                                  */
         printf("No Connection Exists to Close.\r\n");
         ret_val = FUNCTION_ERROR;
      }
   }
   else
...
static void DisplayDataElement(SDP_Data_Element_t *SDPDataElement, unsigned int Level)
{
   ...
   /* The following function is for the GAP Event Callback.  This       */
   /* function will be called whenever a Callback has been registered   */
   /* for the specified GAP Action that is associated with the Bluetooth*/
   /* Stack.  This function passes to the caller the GAP Event Data of  */
   /* the specified Event and the GAP Event Callback Parameter that was */
   /* specified when this Callback was installed.  The caller is free to*/
   /* use the contents of the GAP Event Data ONLY in the context of this*/
   ...
   /* possible (this argument holds anyway because other GAP Events will*/
...
static void BTPSAPI SDP_Event_Callback(unsigned int BluetoothStackID, unsigned int SDPRequestID, SDP_Response_Data_t *SDP_Response_Data, unsigned long CallbackParameter)
{
   ...
   /* The following function is for the BTSER Event Callback.  This     */
   /* function will be called whenever a Callback has been registered   */
   /* for the specified BTSER Action that is associated with the        */
   /* Bluetooth Stack.  This function passes to the caller the BTSER    */
   /* Event Data of the specified Event and the BTSER Event Callback    */
   /* Parameter that was specified when this Callback was installed.    */
   /* The caller is free to use the contents of the BTSER Event Data    */
   ...
   /* User does NOT own.  Therefore, processing in this function should */
   /* be as efficient as possible (this argument holds anyway because   */
   /* other BTSER Events will not be processed while this function call */
   /* is outstanding).                                                  */
   /* * NOTE * This function MUST NOT Block and wait for events that    */
   /*          can only be satisfied by Receiving other BTSER Events.  A*/
   /*          Deadlock WILL occur because NO BTSER Event Callbacks will*/
   /*          be issued while this function is currently outstanding.  */
static void BTPSAPI BTSER_Event_Callback(unsigned int BluetoothStackID, BTSER_Event_Data_t *BTSER_Event_Data, unsigned long CallbackParameter)
{
   char BoardStr[13];
   ...
   /* First, check to see if the required parameters appear to be       */
   /* semi-valid.                                                       */
   if((BTSER_Event_Data) && (BluetoothStackID))
   {
      /* The parameters appear to be semi-valid, now check to see what  */
      /* type the incoming event is.                                    */
      switch(BTSER_Event_Data->Event_Data_Type)
      {
         case etBTSER_Open_Port_Indication:
            /* A Client has connected to the Server, display the BD_ADDR*/
            /* of the connecting device.                                */
            printf("\r\n");
            BD_ADDRToStr(BTSER_Event_Data->Event_Data.BTSER_Open_Port_Indication_Data->BD_ADDR, BoardStr);
            printf("BTSER Open Port Indication, ID: 0x%04X, Board: %s.\r\n", BTSER_Event_Data->Event_Data.BTSER_Open_Port_Indication_Data->BTSERID, BoardStr);

            ServerConnected = TRUE;
            break;
         case etBTSER_Open_Port_Confirmation:
            /* The Client received a Connect Confirmation, display      */
            /* relevant information.                                    */
            printf("\r\n");
            printf("BTSER Open Port Confirmation, ID: 0x%04X, Status: 0x%04X.\r\n", BTSER_Event_Data->Event_Data.BTSER_Open_Port_Confirmation_Data->BTSERID,
                                                                                    BTSER_Event_Data->Event_Data.BTSER_Open_Port_Confirmation_Data->PortOpenStatus);

            /* Check to see if the Client Port was opened successfully. */
            if(BTSER_Event_Data->Event_Data.BTSER_Open_Port_Confirmation_Data->PortOpenStatus)
            {
            ...
            break;
         case etBTSER_Close_Port_Indication:
            /* A Close Port Indication was received, display all        */
            /* relevant information.                                    */
            printf("\r\n");
            printf("BTSER Close Port Indication, ID: 0x%04X.\r\n", BTSER_Event_Data->Event_Data.BTSER_Close_Port_Indication_Data->BTSERID);

            /* Check to see if the closing port is the Client Port.     */
            if(BTSER_Event_Data->Event_Data.BTSER_Close_Port_Indication_Data->BTSERID == BTSERClientID)
            {
               /* The closing port is the client so invalidate the      */
               /* Client Port ID.                                       */
               BTSERClientID = 0;
            }
            else
            ...
            break;
         default:
            /* We do not need to process any other Bluetooth Serial     */
            /* Events.                                                  */
            break;
      }
   }
   ...
   {
      /* There was an error with one or more of the input parameters.   */
      printf("BTSER callback data: Event_Data = NULL.\r\n");
   }
...
int main(int argc, char* argv[])
{
   ...
   if(argc >= NUM_EXPECTED_PARAMETERS_USB)
   {
      ...
      if(HCI_DriverInformationPtr)
      {
         ...
         {
            /* Attempt to set the Device to be Connectable.             */
            if(!SetConnect())
            {
               ...
               if(!SetDisc())
               {
                  ...
                  if(!SetPairable())
                  {
                     /* The Device is now Connectable and Discoverable, */

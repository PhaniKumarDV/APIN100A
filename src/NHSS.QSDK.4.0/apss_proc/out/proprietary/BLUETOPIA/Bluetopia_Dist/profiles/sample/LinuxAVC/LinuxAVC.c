...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXAVC - Simple Linux application using Audio/Video Control Transport   */
/*             Protocol (AVCTP).                                              */
/*                                                                            */
/*  Author:  Damon Lange                                                      */
/*                                                                            */
...
#include "LinuxAVC.h"      /* Main Application Prototypes and Constants.      */
...
#define LOCAL_NAME_ROOT                      "LinuxAVC"  /* Root of the local */
                                                         /* name              */
...
#define MAXIMUM_NUMBER_SUPPORTED_PROFILES           (3)  /* Denotes the number*/
                                                         /* of individual     */
                                                         /* profile instances */
                                                         /* that can be       */
                                                         /* supported         */
...
#define DEFAULT_DEBUG_LOG_FILE_NAME  "LinuxAVC_ASC.log"
#define DEFAULT_DEBUG_FTS_FILE_NAME  "LinuxAVC_FTS.log"
...
typedef struct _tagAVCTP_Profile_Info_t
{
   ...
   AVCTP_Event_Callback_t  CallbackFunction;
...
static AVCTP_Profile_Info_t ProfileInfo[MAXIMUM_NUMBER_SUPPORTED_PROFILES];
                                                    /* Variable which holds the        */
                                                    /* current profile state of all    */
                                                    /* supported AVCTP profile         */
                                                    /* instances supported by this     */
                                                    /* application.                    */
...
static void PrintProfileInformation(unsigned int ProfileInstanceIndex);
...
static void UnRegisterAllProfiles(void)
{
   ...
   if(BluetoothStackID)
   {
      /* Simply loop through the Profile List and clean up any currently*/
      /* active Profile.                                                */
      for(Index=0;Index<MAXIMUM_NUMBER_SUPPORTED_PROFILES;Index++)
      {
         /* If the Profile is Registered, go ahead and Un-register it.  */
         if(ProfileInfo[Index].ProfileID)
            AVCTP_UnRegister_Profile(BluetoothStackID, ProfileInfo[Index].ProfileID);

         /* If an SDP Record is present, go ahead and delete it from the*/
         /* SDP Database.                                               */
         if(ProfileInfo[Index].SDPRecordHandle)
         {
            AVCTP_UnRegister_Profile_SDP_Record(BluetoothStackID, ProfileInfo[Index].SDPRecordHandle);
         }

         /* Free the Device List.                                       */
         FreeDeviceList(&(ProfileInfo[Index].DeviceList));

         /* Make sure the Profile state reflects that the profile is now*/
         /* un-registered.                                              */
         ProfileInfo[Index].ProfileID       = 0;
         ProfileInfo[Index].Connected       = FALSE;
         ProfileInfo[Index].SDPRecordHandle = 0;
      }
   }
}
...
static void InitializeProfileInformation(void)
{
   /* Hardcode the profile Information for this sample.                 */

   /* Profile Instance 0.                                               */
   ASSIGN_SDP_UUID_16(ProfileInfo[0].ProfileUUID, 0x10, 0x00);

   strcpy(ProfileInfo[0].ServiceName,  "Service 0");
   strcpy(ProfileInfo[0].ProviderName, "Stonestreet One");

   ProfileInfo[0].SDPRecordHandle   = 0;
   ProfileInfo[0].Connected         = FALSE;
   ProfileInfo[0].DeviceList        = NULL;
   ProfileInfo[0].ProfileID         = 0;
   ProfileInfo[0].CallbackFunction  = AVCTP_Event_Callback;
   ProfileInfo[0].CallbackParameter = 0;

   /* Profile Instance 1.                                               */
   ASSIGN_SDP_UUID_16(ProfileInfo[1].ProfileUUID, 0x11, 0x0E);

   strcpy(ProfileInfo[1].ServiceName,  "Service 1");
   strcpy(ProfileInfo[1].ProviderName, "Stonestreet One");

   ProfileInfo[1].SDPRecordHandle   = 0;
   ProfileInfo[1].Connected         = FALSE;
   ProfileInfo[1].DeviceList        = NULL;
   ProfileInfo[1].ProfileID         = 0;
   ProfileInfo[1].CallbackFunction  = AVCTP_Event_Callback;
   ProfileInfo[1].CallbackParameter = 1;

   /* Profile Instance 2.                                               */
   ASSIGN_SDP_UUID_16(ProfileInfo[2].ProfileUUID, 0x52, 0x52);

   strcpy(ProfileInfo[2].ServiceName,  "Service 2");
   strcpy(ProfileInfo[2].ProviderName, "Stonestreet One");

   ProfileInfo[2].SDPRecordHandle   = 0;
   ProfileInfo[2].Connected         = FALSE;
   ProfileInfo[2].DeviceList        = NULL;
   ProfileInfo[2].ProfileID         = 0;
   ProfileInfo[2].CallbackFunction  = AVCTP_Event_Callback;
   ProfileInfo[2].CallbackParameter = 2;
}

   /* The following function is responsible for printing out Profile    */
   /* Instance Information for the specified Profile Instance (Profile  */
   /* Instance Index).                                                  */
static void PrintProfileInformation(unsigned int ProfileInstanceIndex)
{
   char          BoardStr[13];
   ...
   if(ProfileInstanceIndex < MAXIMUM_NUMBER_SUPPORTED_PROFILES)
   {
      printf("Profile ID: %d\r\n", ProfileInfo[ProfileInstanceIndex].ProfileID);

      printf("Connected: %s\r\n", ProfileInfo[ProfileInstanceIndex].Connected?"TRUE":"FALSE");

      DeviceListEntry = ProfileInfo[ProfileInstanceIndex].DeviceList;
      ...
      printf("UUID: 0x%02X%02X\r\n", ProfileInfo[ProfileInstanceIndex].ProfileUUID.UUID_Byte1, ProfileInfo[ProfileInstanceIndex].ProfileUUID.UUID_Byte0);

      printf("SDP Handle: %lu\r\n", ProfileInfo[ProfileInstanceIndex].SDPRecordHandle);

      printf("ServiceName: %s\r\n", ProfileInfo[ProfileInstanceIndex].ServiceName);
   }
   else
      printf("Invalid Profile Instance Index specified.\r\n");
}
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("*                  RegisterProfile, UnRegisterProfile, Connect,  *\r\n");
   printf("*                  Disconnect, AddBrowsingChannel,               *\r\n");
   printf("*                  CloseBrowsingChannelProfileState,             *\r\n");
   printf("*                  SendMessage, SendBrowsingMessage,             *\r\n");
...
static int Pair(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      for(Index=0;Index<MAXIMUM_NUMBER_SUPPORTED_PROFILES;Index++)
      {
         if((ProfileInfo[Index].ProfileID) && (ProfileInfo[Index].Connected))
            break;
      }

      if(Index == MAXIMUM_NUMBER_SUPPORTED_PROFILES)
      {
         /* There are no currently active connections, make sure that   */
...
static int RegisterProfile(ParameterList_t *TempParam)
{
   ...
   SDP_Data_Element_t         SDP_Data_Element_SupportedFeatures;
   SDP_Data_Element_t         ProfileListInformation[4];
   AVCTP_SDP_Service_Record_t SDPRecordInformation;
   ...
   if(BluetoothStackID)
   {
      ...
      /* function is valid.                                             */
      if((TempParam) && (TempParam->NumberofParameters >= 2) && (TempParam->Params[0].intParam < MAXIMUM_NUMBER_SUPPORTED_PROFILES))
      {
         /* Now check to make sure that a Profile doesn't already exist */
         /* at this instance.                                           */
         if(!ProfileInfo[TempParam->Params[0].intParam].ProfileID)
         {
            /* Try to register the Profile.                             */
            Result = AVCTP_Register_Profile(BluetoothStackID, ProfileInfo[TempParam->Params[0].intParam].ProfileUUID, ProfileInfo[TempParam->Params[0].intParam].CallbackFunction, ProfileInfo[TempParam->Params[0].intParam].CallbackParameter);
            ...
            if(Result > 0)
            {
               ...
               ProfileInfo[TempParam->Params[0].intParam].ProfileID                 = Result;

               /* Register a SDP Service Record.                        */
               SDPRecordInformation.NumberServiceClassUUID                          = 1;
               SDPRecordInformation.SDPUUIDEntries                                  = &UUIDEntry;
               ...
               {
                  /* Controller.                                        */
                  ASSIGN_SDP_UUID_16(UUIDEntry.UUID_Value.UUID_16, 0x11, 0x0E);
               }
               else
               {
                  /* Target.                                            */
                  ASSIGN_SDP_UUID_16(UUIDEntry.UUID_Value.UUID_16, 0x11, 0x0C);
               }

               /* Next flag that there are no additional protocols that */
               /* need to be added.                                     */
               SDPRecordInformation.ProtocolList                                    = NULL;

               /* Next add the Profile List Information to the record.  */
               SDPRecordInformation.ProfileList                                     = ProfileListInformation;
               ...
               ASSIGN_SDP_UUID_16(ProfileListInformation[2].SDP_Data_Element.UUID_16, 0x11, 0x0E);
               ...
               ProfileListInformation[3].SDP_Data_Element.UnsignedInteger2Bytes     = 0x0100;

               /* Now attempt to add the AVCTP SDP Record to the SDP    */
               /* Database.                                             */
               if((ret_val = AVCTP_Register_Profile_SDP_Record(BluetoothStackID, &SDPRecordInformation, ProfileInfo[TempParam->Params[0].intParam].ServiceName, ProfileInfo[TempParam->Params[0].intParam].ProviderName, &(ProfileInfo[TempParam->Params[0].intParam].SDPRecordHandle))) == 0)
               {
                  printf("AVCTP_Register_Profile_SDP_Record: Function Success.\r\n");

                  /* Note the Supported Features.                       */
                  SDP_Data_Element_SupportedFeatures.SDP_Data_Element_Type                  = deUnsignedInteger2Bytes;
                  SDP_Data_Element_SupportedFeatures.SDP_Data_Element_Length                = sizeof(SDP_Data_Element_SupportedFeatures.SDP_Data_Element.UnsignedInteger2Bytes);
                  SDP_Data_Element_SupportedFeatures.SDP_Data_Element.UnsignedInteger2Bytes = 0x03;

                  SDP_Add_Attribute(BluetoothStackID, ProfileInfo[TempParam->Params[0].intParam].SDPRecordHandle, SDP_ATTRIBUTE_ID_SUPPORTED_FEATURES, &SDP_Data_Element_SupportedFeatures);
               ...
               else
               {
                  ...
                  /* ahead unregister the Profile and notify the user.  */
                  AVCTP_UnRegister_Profile(BluetoothStackID, Result);

                  ProfileInfo[TempParam->Params[0].intParam].ProfileID       = 0;
                  ProfileInfo[TempParam->Params[0].intParam].SDPRecordHandle = 0;

                  printf("Unable to register SDP Record.  Profile not registered.\r\n");
         ...
         else
         {
            ...
            printf("Profile Instance already present at Instance Index: %d.\r\n", TempParam->Params[0].intParam);
      ...
      {
         printf("Usage: RegisterProfile [Instance Index] [Controller/Target (Controller = 0, Target = 1)].\r\n");
...
static int UnRegisterProfile(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      /* Now check to make sure that the Profile Instance passed to this*/
      /* function is valid.                                             */
      if((TempParam) && (TempParam->NumberofParameters >= 1) && (TempParam->Params[0].intParam < MAXIMUM_NUMBER_SUPPORTED_PROFILES))
      {
         ...
         if(ProfileInfo[TempParam->Params[0].intParam].ProfileID)
         {
            ...
            Result = AVCTP_UnRegister_Profile(BluetoothStackID, ProfileInfo[TempParam->Params[0].intParam].ProfileID);
            ...
            if(!Result)
            {
               ...
               /* longer active.                                        */
               ProfileInfo[TempParam->Params[0].intParam].ProfileID = 0;
               ProfileInfo[TempParam->Params[0].intParam].Connected = FALSE;

               if(ProfileInfo[TempParam->Params[0].intParam].SDPRecordHandle)
               {
                  AVCTP_UnRegister_Profile_SDP_Record(BluetoothStackID, ProfileInfo[TempParam->Params[0].intParam].SDPRecordHandle);
                  ProfileInfo[TempParam->Params[0].intParam].SDPRecordHandle = 0;
               }

               FreeDeviceList(&(ProfileInfo[TempParam->Params[0].intParam].DeviceList));
            ...
            {
               /* There was an error attempting to Un-register the      */
               /* Profile.                                              */
      ...
      {
         printf("Usage: UnRegisterProfile [Instance Index].\r\n");
...
static int Connect(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      /* Now check to make sure that the Profile Instance passed to this*/
      /* function is valid.                                             */
      if((TempParam) && (TempParam->NumberofParameters >= 2) && (TempParam->Params[0].intParam < MAXIMUM_NUMBER_SUPPORTED_PROFILES) && (TempParam->Params[1].intParam <= NumberofValidResponses) && (!COMPARE_BD_ADDR(InquiryResultList[(TempParam->Params[1].intParam - 1)], NullADDR)))
      {
         /* Now check to make sure that a Profile exists at this        */
         /* instance AND it is NOT connected.                           */
         if((ProfileInfo[TempParam->Params[0].intParam].ProfileID) && (!ProfileInfo[TempParam->Params[0].intParam].Connected))
         {
            /* Try to Connect the Profile.                              */
            Result = AVCTP_Connect_Device(BluetoothStackID, ProfileInfo[TempParam->Params[0].intParam].ProfileID, InquiryResultList[TempParam->Params[1].intParam - 1]);
            ...
            {
               /* There was an error attempting to connect the Profile. */
         ...
         {
            /* Instance is either not present or currently connected.   */
            printf("No Profile Instance present at Instance Index: %d.\r\n", TempParam->Params[0].intParam);
      ...
      {
         printf("Usage: Connect [Instance Index] [Inquiry Index].\r\n");
   ...
   if(BluetoothStackID)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 2) && (TempParam->Params[0].intParam < MAXIMUM_NUMBER_SUPPORTED_PROFILES) && (TempParam->Params[1].intParam <= NumberofValidResponses) && (!COMPARE_BD_ADDR(InquiryResultList[(TempParam->Params[1].intParam - 1)], NullADDR)))
      {
         ...
         if((ProfileInfo[TempParam->Params[0].intParam].ProfileID) && (ProfileInfo[TempParam->Params[0].intParam].Connected))
         {
            ...
            Result = AVCTP_Close_Connection(BluetoothStackID, ProfileInfo[TempParam->Params[0].intParam].ProfileID, InquiryResultList[TempParam->Params[1].intParam - 1]);
            ...
            if(!Result)
            {
               ...
               if((DeviceListEntry = SearchDeviceListEntryByBDADDR(&(ProfileInfo[TempParam->Params[0].intParam].DeviceList), InquiryResultList[TempParam->Params[1].intParam - 1])) != NULL)
               {
                  if(DeleteAVCTPConnectionInfoEntry(&(ProfileInfo[TempParam->Params[0].intParam].DeviceList), DeviceListEntry) != NULL)
                     FreeDeviceListEntryMemory(DeviceListEntry);

                  if(!ProfileInfo[TempParam->Params[0].intParam].DeviceList)
                     ProfileInfo[TempParam->Params[0].intParam].Connected = FALSE;
               }
      ...
      {
         printf("Usage: Disconnect [Instance Index] [Inquiry Index].\r\n");
   ...
   if(BluetoothStackID)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 2) && (TempParam->Params[0].intParam < MAXIMUM_NUMBER_SUPPORTED_PROFILES) && (TempParam->Params[1].intParam <= NumberofValidResponses) && (!COMPARE_BD_ADDR(InquiryResultList[(TempParam->Params[1].intParam - 1)], NullADDR)))
      {
         ...
         if((ProfileInfo[TempParam->Params[0].intParam].ProfileID) && (ProfileInfo[TempParam->Params[0].intParam].Connected))
         {
            ...
            Result = AVCTP_Connect_Browsing_Channel(BluetoothStackID, ProfileInfo[TempParam->Params[0].intParam].ProfileID, InquiryResultList[TempParam->Params[1].intParam - 1], L2CAP_MAXIMUM_SUPPORTED_STACK_MTU);
      ...
      {
         printf("Usage: AddBrowsingChannel [Instance Index] [Inquiry Index].\r\n");
   ...
   if(BluetoothStackID)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 2) && (TempParam->Params[0].intParam < MAXIMUM_NUMBER_SUPPORTED_PROFILES) && (TempParam->Params[1].intParam <= NumberofValidResponses) && (!COMPARE_BD_ADDR(InquiryResultList[(TempParam->Params[1].intParam - 1)], NullADDR)))
      {
         ...
         if((ProfileInfo[TempParam->Params[0].intParam].ProfileID) && (ProfileInfo[TempParam->Params[0].intParam].Connected))
         {
            ...
            Result = AVCTP_Close_Browsing_Channel(BluetoothStackID, ProfileInfo[TempParam->Params[0].intParam].ProfileID, InquiryResultList[TempParam->Params[1].intParam - 1]);
      ...
      {
         printf("Usage: CloseBrowsingChannel [Instance Index] [Inquiry Index].\r\n");
...
static int ProfileState(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 1) && (TempParam->Params[0].intParam < MAXIMUM_NUMBER_SUPPORTED_PROFILES))
      {
         ...
         PrintProfileInformation(TempParam->NumberofParameters);

         ret_val = 0;
      }
      else
      {
         printf("Usage: ProfileState [Instance Index].\r\n");
...
static int SendMessage(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      /* Now check to make sure that the Profile Instance passed to this*/
      /* function is valid.                                             */
      if((TempParam) && (TempParam->NumberofParameters >= 5) && (TempParam->Params[0].intParam < MAXIMUM_NUMBER_SUPPORTED_PROFILES) && (TempParam->Params[1].intParam <= NumberofValidResponses) && (!COMPARE_BD_ADDR(InquiryResultList[(TempParam->Params[1].intParam - 1)], NullADDR)))
      {
         ...
         {
            /* Allocate memory.                                         */
            if((Data = (Byte_t *)malloc(TempParam->Params[4].intParam)) != NULL)
            {
               /* Format up some sample data.                           */
               for(Index=0;Index < TempParam->Params[4].intParam;Index++)
                  Data[Index] = (Byte_t)(Index & 0xFF);

               /* Try to Send the Message.                              */
               if((Result = AVCTP_Send_Message(BluetoothStackID, ProfileInfo[TempParam->Params[0].intParam].ProfileID, InquiryResultList[TempParam->Params[1].intParam - 1], (Byte_t)(TempParam->Params[2].intParam & 0x0F), (Boolean_t)TempParam->Params[3].intParam, TempParam->Params[4].intParam, Data)) != 0)
               {
            ...
            {
               printf("Unable to allocate data memory of length: %u\r\n", TempParam->Params[4].intParam);
      ...
      {
         printf("Usage: SendMessage [Instance Index] [Inquiry Index] [Transaction ID (0 <= 15)] [Command/Response (Command = 0)] [Data Length].\r\n");
...
static int SendBrowsingMessage(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 5) && (TempParam->Params[0].intParam < MAXIMUM_NUMBER_SUPPORTED_PROFILES) && (TempParam->Params[1].intParam <= NumberofValidResponses) && (!COMPARE_BD_ADDR(InquiryResultList[(TempParam->Params[1].intParam - 1)], NullADDR)))
      {
         ...
         if(ProfileInfo[TempParam->Params[0].intParam].ProfileID)
         {
            ...
            if((Data = (Byte_t *)malloc(TempParam->Params[4].intParam)) != NULL)
            {
               ...
               /* Try to Send the Message.                              */
               if((Result = AVCTP_Send_Browsing_Channel_Message(BluetoothStackID, ProfileInfo[TempParam->Params[0].intParam].ProfileID, InquiryResultList[TempParam->Params[1].intParam - 1], (Byte_t)(TempParam->Params[2].intParam & 0x0F), (Boolean_t)TempParam->Params[3].intParam, TempParam->Params[4].intParam, Data)) != 0)
               {
                  /* There was an error attempting to Send the Browsing */
                  /* Message.                                           */
                  printf("AVCTP_Send_Browsing_Channel_Message: Function Failure: %d.\r\n", Result);

                  ret_val = Result;
               }
               else
               {
                  /* The Send Browsing Message Request was successfully */
                  /* submitted.                                         */
                  printf("AVCTP_Send_Browsing_Channel_Message: Function Successful.\r\n");
      ...
      {
         printf("Usage: SendBrowsingMessage [Instance Index] [Inquiry Index] [Transaction ID (0 <= 15)] [Command/Response (Command = 0)] [Data Length].\r\n");
   ...
   if((BluetoothStackID) && (AVCTPEventData))
   {
      ...
      switch(AVCTPEventData->Event_Data_Type)
      {
         case etAVCTP_Connect_Indication:
            ProfileInfo[CallbackParameter].Connected = TRUE;
            ...
            AddDeviceListEntry(&(ProfileInfo[CallbackParameter].DeviceList), &DeviceListEntry);

            BD_ADDRToStr(DeviceListEntry.BD_ADDR, BoardStr);

            printf("Connect Indication Received for %d, %s\r\n", ProfileInfo[CallbackParameter].ProfileID, BoardStr);
            break;
         case etAVCTP_Browsing_Channel_Connect_Indication:
            ...
            printf("Browsing Channel Connect Indication Received for %d, %s, MTU: %d\r\n", ProfileInfo[CallbackParameter].ProfileID, BoardStr, AVCTPEventData->Event_Data.AVCTP_Browsing_Channel_Connect_Indication_Data->MTU);
            break;
         case etAVCTP_Connect_Confirmation:
            ...
            printf("Connect Confirmation Received for %d, %s: Status: %d\r\n", ProfileInfo[CallbackParameter].ProfileID, BoardStr, AVCTPEventData->Event_Data.AVCTP_Connect_Confirmation_Data->Status);
            break;
         case etAVCTP_Browsing_Channel_Connect_Confirmation:
            ...
            printf("Browsing Channel Connect Confirmation Received for %d, %s: Status: %d, MTU: %d\r\n", ProfileInfo[CallbackParameter].ProfileID, BoardStr, AVCTPEventData->Event_Data.AVCTP_Browsing_Channel_Connect_Confirmation_Data->Status, AVCTPEventData->Event_Data.AVCTP_Browsing_Channel_Connect_Confirmation_Data->MTU);
            break;
         case etAVCTP_Disconnect_Indication:
            /* Delete the entry from the list.                          */
            DeviceListEntryPtr = SearchDeviceListEntryByBDADDR(&(ProfileInfo[CallbackParameter].DeviceList), AVCTPEventData->Event_Data.AVCTP_Disconnect_Indication_Data->BD_ADDR);

            if(DeleteAVCTPConnectionInfoEntry(&(ProfileInfo[CallbackParameter].DeviceList),DeviceListEntryPtr) != NULL)
               FreeDeviceListEntryMemory(DeviceListEntryPtr);

            if(!ProfileInfo[CallbackParameter].DeviceList)
               ProfileInfo[CallbackParameter].Connected = FALSE;
         ...
         case etAVCTP_Message_Indication:
            ...
            printf("Message Received from %d, %s. Response: %d.\r\n", ProfileInfo[CallbackParameter].ProfileID, BoardStr, AVCTPEventData->Event_Data.AVCTP_Message_Indication_Data->MessageType);

            printf("TrID: %d IPID: %d Length: %d\r\n", AVCTPEventData->Event_Data.AVCTP_Message_Indication_Data->TransactionID, AVCTPEventData->Event_Data.AVCTP_Message_Indication_Data->InvalidProfileID, AVCTPEventData->Event_Data.AVCTP_Message_Indication_Data->DataLength);

            DataBuffer = AVCTPEventData->Event_Data.AVCTP_Message_Indication_Data->DataBuffer;

            for(Index=0;Index<AVCTPEventData->Event_Data.AVCTP_Message_Indication_Data->DataLength;Index++)
               printf("0x%02X%s", *(DataBuffer+Index), (((Index+1)%8)?" ":"\r\n"));
         ...
         case etAVCTP_Browsing_Channel_Message_Indication:
            ...
            printf("Browsing Message Received from %d, %s. Response: %d.\r\n", ProfileInfo[CallbackParameter].ProfileID, BoardStr, AVCTPEventData->Event_Data.AVCTP_Browsing_Channel_Message_Indication_Data->MessageType);
            ...
            for(Index=0;Index<AVCTPEventData->Event_Data.AVCTP_Browsing_Channel_Message_Indication_Data->DataLength;Index++)
               printf("0x%02X%s", *(DataBuffer+Index), (((Index+1)%8)?" ":"\r\n"));

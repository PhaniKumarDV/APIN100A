...
#include <unistd.h>        /* Included for getcwd() prototype.                */

#include "LinuxFTP.h"      /* Main Application Prototypes and Constants.      */

#include "SS1BTPS.h"       /* Includes for the SS1 Bluetooth Protocol Stack.  */
#include "SS1BTFTP.h"      /* Includes for the SS1 File Transfer Profile.     */
#include "SS1BTDBG.h"      /* Includes/Constants for Bluetooth Debugging.     */

#define LOCAL_NAME_ROOT                      "LinuxFTP"  /* Root of the local */
                                                         /* name              */
...
#define DEFAULT_DEBUG_LOG_FILE_NAME  "LinuxFTP_ASC.log"
#define DEFAULT_DEBUG_FTS_FILE_NAME  "LinuxFTP_FTS.log"
...
static unsigned int        FTPServerID;             /* Variable used to hold the       */
...
static unsigned int        FTPClientID;             /* Variable used to hold the       */
...
static void BTPSAPI FTP_Server_Event_Callback(unsigned int BluetoothStackID, FTP_Server_Event_Data_t *FTP_Event_Data, unsigned long CallbackParameter);
static void BTPSAPI FTP_Client_Event_Callback(unsigned int BluetoothStackID, FTP_Client_Event_Data_t *FTP_Event_Data, unsigned long CallbackParameter);
...
static void UserInterface_Client(void)
{
   ...
   while(Result != EXIT_CODE)
   {
            ...
            if(CommandParser(&TempCommand, UserInput) >= 0)
            {
               ...
               switch(Result)
               {
                  ...
                  case EXIT_CODE:
                     ...
                     /* go ahead an close any Remote FTP Server that we */
                     /* have open.                                      */
                     if(FTPClientID)
      ...
      {
         /* Close any Remote FTP Server that we have open.              */
         if(FTPClientID)
            CloseRemoteServer(NULL);
...
static int DeleteLinkKey(BD_ADDR_t BD_ADDR)
{
   ...
   /* Command Options for either FTP Client for FTP Server.  The input  */
   ...
   /* Prompt pass in the parsed information.  This function displays    */
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   if(IsClient)
   {
      ...
      printf("*                  GetRemoteName, Open, Close, Dir, CD,          *\r\n");
      printf("*                  GetClassOfDevice, GetRemoteName, Open,        *\r\n");
      printf("*                  MKDir, RMDir, Del, Put, Get, MKFile, Abort,   *\r\n");
      printf("*                  LDIR, EnableDebug, Help, Quit.                *\r\n");
      printf("******************************************************************\r\n");
   }
   else
   {
...
static int Pair(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      /* Next, determine if we are currently connected or not.          */
      if(!Connected)
      {
         ...
         if(!FTPServerID)
         {
            ...
            if((TempParam->NumberofParameters > 0) && (TempParam->Params[0].intParam) && ((TempParam->Params[0].intParam >= SPP_PORT_NUMBER_MINIMUM) && (TempParam->Params[0].intParam <= SPP_PORT_NUMBER_MAXIMUM)))
            {
               ...
               if((TempParam->NumberofParameters > 1) && (TempParam->Params[1].strParam) && (strlen(TempParam->Params[1].strParam)))
                  strcpy(RootPath, TempParam->Params[1].strParam);
               else
               {
                  /* Let's figure out the current working directory of  */
                  /* the program so we can use that for our Root        */
                  /* Directory.                                         */
               ...
               /* Port specified, with the correct directory, with all  */
               /* the permissions.                                      */
               ret_val = FTP_Open_Server(BluetoothStackID, TempParam->Params[0].intParam, RootPath, (FTP_SERVER_PERMISSION_MASK_READ | FTP_SERVER_PERMISSION_MASK_WRITE | FTP_SERVER_PERMISSION_MASK_DELETE), FTP_Server_Event_Callback, (unsigned long)0);

               /* If the Open was successful, then note the FTP Server  */
               /* ID.                                                   */
               if(ret_val > 0)
               {
                  ...
                  /* SDP Record indicating that an FTP Server exists.   */
                  ...
                  ret_val = FTP_Register_Server_SDP_Record(BluetoothStackID, FTPServerID, ServiceName, &FTPServerSDPHandle);

                  /* If there was an error creating the FTP Server's SDP*/
                  /* Service Record then go ahead an close down the     */
                  /* server an flag an error.                           */
                  if(ret_val < 0)
                  {
                     printf("Unable to Register FTP Server SDP Record, Error = %d.\r\n", ret_val);

                     FTP_Close_Server(BluetoothStackID, FTPServerID);
            ...
            {
               printf("Usage: Open [RFCOMM Server Port] [Root Directory Path (optional)].\r\n");
...
static int CloseServer(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(FTPServerID)
      {
         /* If there is an SDP Service Record associated with the FTP   */
         /* Server then we need to remove it from the SDP Database.     */
         if(FTPServerSDPHandle)
         {
            FTP_Un_Register_SDP_Record(BluetoothStackID, FTPServerID, FTPServerSDPHandle);

            /* Flag that there is no longer an SDP FTP Server Record.   */
            FTPServerSDPHandle = 0;
         }

         /* Finally close the FTP Server.                               */
         ret_val = FTP_Close_Server(BluetoothStackID, FTPServerID);

         if(ret_val < 0)
         {
            printf("FTP_Close_Server(): Error %d\r\n", ret_val);
...
static int OpenRemoteServer(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(IsClient)
      {
         /* Next, let's make sure that there is not an FTP Client       */
         /* already open.                                               */
         if(!FTPClientID)
         {
            ...
            if((TempParam) && (TempParam->NumberofParameters > 1) && (TempParam->Params[0].intParam) && (NumberofValidResponses) && (TempParam->Params[0].intParam <= NumberofValidResponses) && (!COMPARE_BD_ADDR(InquiryResultList[(TempParam->Params[0].intParam - 1)], NullADDR)) && (TempParam->Params[1].intParam))
            {
               ...
               Result = FTP_Open_Remote_File_Server(BluetoothStackID, InquiryResultList[(TempParam->Params[0].intParam - 1)], TempParam->Params[1].intParam, FTP_Client_Event_Callback, 0);

               if(Result > 0)
               {
                  /* Inform the user that the call to open the Remote   */
                  /* FTP Server was successful.                         */
                  printf("FTP_Open_Remote_File_Server() was successful.\r\n");
               ...
               else
               {
                  /* Inform the user that the call to Open the Remote   */
                  /* File Server failed.                                */
                  printf("Error calling FTP_Open_Remote_File_Server(): %d.\r\n", Result);
            ...
            {
               printf("Usage: Open [Inquiry Index] [RFCOMM Server Port].\r\n");
...
static int CloseRemoteServer(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(IsClient)
      {
         /* Next, let's make sure that a Remote FTP Server is indeed    */
         /* Open.                                                       */
         if(FTPClientID)
         {
            ...
            FTP_Close_Client(BluetoothStackID, FTPClientID);
   ...
   if(BluetoothStackID)
   {
      ...
      if(IsClient)
      {
         ...
         if(FTPClientID)
         {
            ...
            Result = FTP_Get_Directory(BluetoothStackID, FTPClientID, NULL);

            if(Result >= 0)
            {
               /* Inform the user that the Get Directory Request was    */
               /* issued successfully.                                  */
               printf("FTP_Get_Directory() issued successfully.\r\n");
            ...
            else
            {
               ...
               printf("Error calling FTP_Get_Directory().\r\n");
         ...
         {
            /* Inform the user that the command cannot be completed     */
            /* because there is no FTP Client currently open.           */
   ...
   if(BluetoothStackID)
   {
      ...
      if(IsClient)
      {
         ...
         if(FTPClientID)
         {
            ...
            /* change into a specific directory, or has requested to    */
            /* go up a directory.                                       */
            if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].strParam) && (strlen(TempParam->Params[0].strParam)))
            {
               if(!strcmp(TempParam->Params[0].strParam, ".."))
               {
                  ...
                  if(!InRootDirectory)
                  {
                     /* We are not already in the Root Directory, so    */
                     /* issue the Change Directory Request.             */
                     Result = FTP_Set_Directory(BluetoothStackID, FTPClientID, NULL);

                     /* Inform the user of the result of the Set        */
                     /* Directory Command that was issued.              */
                     if(Result >= 0)
                     {
                        printf("FTP_Set_Directory() successful.\r\n");

                        ret_val = 0;
                     }
                     else
                     {
                        printf("FTP_Set_Directory() unsuccessful: %d.\r\n", Result);
                  ...
                  {
                     /* Inform the user that we are already in the      */
                     /* Root Directory.                                 */
               ...
               else
               {
                  ...
                  /* Simply issue the Change Directory Command with     */
                  /* this Directory.                                    */
                  Result = FTP_Set_Directory(BluetoothStackID, FTPClientID, TempParam->Params[0].strParam);
         ...
         {
            /* Inform the user that the command cannot be completed     */
            /* because there is FTP Client currently open.              */
            printf("You must open an FTP Client before issuing this command.\r\n");
...
static int MakeDirectory(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(IsClient)
      {
         ...
         if(FTPClientID)
         {
            /* Next, let's make sure that a directory was specified as  */
            /* a parameter to this command.                             */
            if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].strParam) && (strlen(TempParam->Params[0].strParam)))
            {
               /* The user has requested to create a directory.  Simply */
               /* issue the Create Directory Command with the Directory */
               /* Name.                                                 */
               Result = FTP_Create_Directory(BluetoothStackID, FTPClientID, TempParam->Params[0].strParam);

               /* Inform the user of the result of the Create Directory */
               /* Command that was issued.                              */
               if(Result >= 0)
               {
                  printf("FTP_Create_Directory() successful.\r\n");

                  ret_val = 0;
               }
               else
               {
                  printf("FTP_Create_Directory() unsuccessful: %d.\r\n", Result);
...
static int DeleteDirectory(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(IsClient)
      {
         ...
         if(FTPClientID)
         {
            /* Next, let's make sure that a remote directory was        */
            ...
            if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].strParam) && (strlen(TempParam->Params[0].strParam)))
            {
               ...
               /* Name.                                                 */
               Result = FTP_Delete_Directory(BluetoothStackID, FTPClientID, TempParam->Params[0].strParam);

               /* Inform the user of the result of the Delete Directory */
               /* Command that was issued.                              */
               if(Result >= 0)
               {
                  printf("FTP_Delete_Directory() successful.\r\n");

                  ret_val = 0;
               }
               else
               {
                  printf("FTP_Delete_Directory() unsuccessful: %d.\r\n", Result);
...
static int DeleteFile(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(IsClient)
      {
         ...
         if(FTPClientID)
         {
            /* Next, let's make sure that a remote file was specified as*/
            /* a parameter to this command.                             */
            if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].strParam) && (strlen(TempParam->Params[0].strParam)))
            {
               /* The user has requested to delete a file.  Simply issue*/
               /* the Delete File Command with the File Name.           */
               Result = FTP_Delete_File(BluetoothStackID, FTPClientID, TempParam->Params[0].strParam);

               /* Inform the user of the result of the Delete File      */
               /* Command that was issued.                              */
               if(Result >= 0)
               {
                  printf("FTP_Delete_File() successful.\r\n");

                  ret_val = 0;
               }
               else
               {
                  printf("FTP_Delete_File() unsuccessful: %d.\r\n", Result);
...
static int PutFile(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(IsClient)
      {
         ...
         if(FTPClientID)
         {
            /* Next, let's make sure that a local file was specified as */
            /* a parameter to this command.                             */
            if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].strParam) && (strlen(TempParam->Params[0].strParam)))
            {
               /* Next, let's check to see if the user has requested to */
               /* put the File to the Remote Device with a different    */
               /* name.                                                 */
               if(TempParam->NumberofParameters > 1)
               {
                  /* More than one parameter was specified, so let's    */
                  /* make sure that it is valid.                        */
                  if((TempParam->Params[1].strParam) && (strlen(TempParam->Params[1].strParam)))
                  {
                     /* Simply put the Local File to the Remote Device  */
                     /* with the specified Remote Device File Name.     */
                     Result = FTP_Put_File(BluetoothStackID, FTPClientID, NULL, TempParam->Params[0].strParam, TempParam->Params[1].strParam);

                     /* Inform the user of the result of the Put File   */
                     /* Command that was issued.                        */
                     if(Result >= 0)
                     {
                        printf("FTP_Put_File() successful.\r\n");

                        ret_val = 0;
                     }
                     else
                     {
                        printf("FTP_Put_File() unsuccessful: %d.\r\n", Result);
               ...
               else
               {
                  ...
                  /* Remote File, we will have to use the File name of  */
                  /* the Local File, but we have to make sure we strip  */
                  /* out all of the path information (if present).      */
                  ...
                  /* make sure that Index is located at the first       */
                  /* location of the File Name.                         */
                  Index++;

                  Result = FTP_Put_File(BluetoothStackID, FTPClientID, NULL, TempParam->Params[0].strParam, &(TempParam->Params[0].strParam[Index]));
   ...
   if(BluetoothStackID)
   {
      ...
      if(IsClient)
      {
         ...
         if(FTPClientID)
         {
            ...
            {
               /* Next, let's check to see if the user has requested to */
               /* get the File to the Local Device with a different     */
               /* name.                                                 */
               if(TempParam->NumberofParameters > 1)
               {
                  /* More than one parameter was specified, so let's    */
                  ...
                  if((TempParam->Params[1].strParam) && (strlen(TempParam->Params[1].strParam)))
                  {
                     ...
                     /* * NOTE * We are passing in 0 for the length     */
                     /*          of the file.  This has no meaning      */
                     /*          other than not displaying the file     */
                     /*          size while the transfer is occurring.  */
                     Result = FTP_Get_File(BluetoothStackID, FTPClientID, TempParam->Params[0].strParam, 0, NULL, TempParam->Params[1].strParam);

                     /* Inform the user of the result of the Get File   */
                     /* Command that was issued.                        */
                     if(Result >= 0)
                     {
                        printf("FTP_Get_File() successful.\r\n");

                        ret_val = 0;
                     }
                     else
                     {
                        printf("FTP_Get_File() unsuccessful: %d.\r\n", Result);
               ...
               else
               {
                  ...
                  Result = FTP_Get_File(BluetoothStackID, FTPClientID, TempParam->Params[0].strParam, 0, NULL, TempParam->Params[0].strParam);
      ...
      if(IsClient)
      {
         ...
         if(FTPClientID)
         {
            ...
            if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].strParam) && (strlen(TempParam->Params[0].strParam)))
            {
               ...
               Result = FTP_Create_File(BluetoothStackID, FTPClientID, TempParam->Params[0].strParam);

               /* Inform the user of the result of the Create File      */
               /* Command that was issued.                              */
               if(Result >= 0)
               {
                  printf("FTP_Create_File() successful.\r\n");

                  ret_val = 0;
               }
               else
               {
                  printf("FTP_Create_File() unsuccessful: %d.\r\n", Result);
   ...
   if(BluetoothStackID)
   {
      ...
      if(IsClient)
      {
         ...
         if(FTPClientID)
         {
            ...
            Result = FTP_Abort(BluetoothStackID, FTPClientID);

            if(Result >= 0)
            {
               /* Inform the user that the Abort was issued             */
               /* successfully.                                         */
               printf("FTP_Abort() issued successfully.\r\n");

               ret_val = 0;
            }
            else
            {
               /* Inform the user that there was an error submitting    */
               /* the Abort Request.                                    */
               printf("Error calling FTP_Abort().\r\n");
   ...
   if(BluetoothStackID)
   {
      ...
      if(IsClient)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].strParam) && (strlen(TempParam->Params[0].strParam)))
         {
            ...
            /* Note that if we do not have enough space for the         */
            /* requested Directory Listing then we will simply list all */
...
static void BTPSAPI SDP_Event_Callback(unsigned int BluetoothStackID, unsigned int SDPRequestID, SDP_Response_Data_t *SDP_Response_Data, unsigned long CallbackParameter)
{
   ...
   /* The following function is for an FTP Server Event Callback.  This */
   /* function will be called whenever a FTP Server Event occurs that is*/
   /* associated with the Bluetooth Stack.  This function passes to the */
   /* caller the FTP Event Data that occurred and the FTP Event Callback*/
   /* Parameter that was specified when this Callback was installed.    */
   /* The caller is free to use the contents of the FTP FTP Event Data  */
   ...
   /* User does NOT own.  Therefore, processing in this function should */
   /* be as efficient as possible (this argument holds anyway because   */
   /* another FTP Event will not be processed while this function call  */
   /* is outstanding).                                                  */
   /* * NOTE * This function MUST NOT Block and wait for Events that    */
   /*          can only be satisfied by Receiving FTP Event Packets.  A */
   /*          Deadlock WILL occur because NO FTP Event Callbacks will  */
   /*          be issued while this function is currently outstanding.  */
static void BTPSAPI FTP_Server_Event_Callback(unsigned int BluetoothStackID, FTP_Server_Event_Data_t *FTP_Event_Data, unsigned long CallbackParameter)
{
   char BoardStr[13];

   if((BluetoothStackID) && (FTP_Event_Data))
   {
      switch(FTP_Event_Data->Event_Data_Type)
      {
         case etFTP_Server_Connect_Indication:
            /* A Client has connected to the Server, display the BD_ADDR*/
            /* of the connecting Device.                                */
            printf("\r\n");
            printf("etFTP_Server_Connect_Indication, FTPID: 0x%04X\r\n", FTP_Event_Data->Event_Data.FTP_Server_Connect_Indication_Data->FTPID);
            BD_ADDRToStr(FTP_Event_Data->Event_Data.FTP_Server_Connect_Indication_Data->BD_ADDR, BoardStr);
            ...
            Connected = TRUE;
            break;
         case etFTP_Server_Disconnect_Indication:
            /* A Disconnect Indication was received, this means that the*/
            /* Client has disconnected from the Server.  Display the    */
            /* appropriate message.                                     */
            printf("\r\n");
            printf("etFTP_Server_Disconnect_Indication, FTPID: 0x%04X.", FTP_Event_Data->Event_Data.FTP_Server_Disconnect_Indication_Data->FTPID);
            printf("\r\nServer>");

            /* Flag that we are no longer connected.                    */
            Connected = FALSE;
            break;
         case etFTP_Server_Directory_Request_Indication:
            ...
            /* Client.                                                  */
            printf("\r\n");
            printf("etFTP_Server_Directory_Request_Indication, FTPID: 0x%04X\r\n", FTP_Event_Data->Event_Data.FTP_Server_Directory_Request_Indication_Data->FTPID);
            printf("   Directory: %s\r\n", FTP_Event_Data->Event_Data.FTP_Server_Directory_Request_Indication_Data->DirectoryName);
            printf("Server>");
            break;
         case etFTP_Server_Change_Directory_Indication:
            ...
            printf("\r\n");
            printf("etFTP_Server_Change_Directory_Indication, FTPID: 0x%04X\r\n", FTP_Event_Data->Event_Data.FTP_Server_Change_Directory_Indication_Data->FTPID);
            printf("   Directory: %s\r\n", FTP_Event_Data->Event_Data.FTP_Server_Change_Directory_Indication_Data->DirectoryName);
            printf("Server>");
            break;
         case etFTP_Server_Directory_Delete_Indication:
            /* The Client requested to Delete a Directory.  Display a   */
            /* message indicating this, the Directory to be Deleted, and*/
            /* the Current Directory.                                   */
            printf("\r\n");
            printf("etFTP_Server_Directory_Delete_Indication, FTPID: 0x%04X\r\n", FTP_Event_Data->Event_Data.FTP_Server_Directory_Delete_Indication_Data->FTPID);
            printf("   Deleted Directory: %s\r\n", FTP_Event_Data->Event_Data.FTP_Server_Directory_Delete_Indication_Data->DeletedDirectoryName);
            printf("   Current Directory:  %s\r\n", FTP_Event_Data->Event_Data.FTP_Server_Directory_Delete_Indication_Data->DirectoryName);
            printf("Server>");
            break;
         case etFTP_Server_Directory_Create_Indication:
            /* The Client requested to Create a Directory.  Display a   */
            /* message indicating this, the Directory to be created, and*/
            /* the Current Directory.                                   */
            printf("\r\n");
            printf("etFTP_Server_Directory_Create_Indication, FTPID: 0x%04X\r\n", FTP_Event_Data->Event_Data.FTP_Server_Directory_Create_Indication_Data->FTPID);
            printf("   Created Directory: %s\r\n", FTP_Event_Data->Event_Data.FTP_Server_Directory_Create_Indication_Data->CreatedDirectoryName);
            printf("   Current Directory: %s\r\n", FTP_Event_Data->Event_Data.FTP_Server_Directory_Create_Indication_Data->DirectoryName);
            printf("Server>");
            break;
         case etFTP_Server_File_Delete_Indication:
            /* The Client requested to delete a file.  Display a message*/
            /* indicating this, the name of the file being deleted, and */
            /* the current directory.                                   */
            printf("\r\n");
            printf("etFTP_Server_File_Delete_Indication, FTPID: 0x%04X\r\n", FTP_Event_Data->Event_Data.FTP_Server_File_Delete_Indication_Data->FTPID);
            printf("   Deleted File: %s\r\n", FTP_Event_Data->Event_Data.FTP_Server_File_Delete_Indication_Data->DeletedFileName);
            printf("   Current Directory: %s\r\n", FTP_Event_Data->Event_Data.FTP_Server_File_Delete_Indication_Data->DirectoryName);
            printf("Server>");
            break;
         case etFTP_Server_File_Create_Indication:
            /* The Client requested to Create a File.  Display a message*/
            /* indicating this, the Create File to be created, and the  */
            /* Current Directory.                                       */
            printf("\r\n");
            printf("etFTP_Server_File_Create_Indication, FTPID: 0x%04X\r\n", FTP_Event_Data->Event_Data.FTP_Server_File_Create_Indication_Data->FTPID);
            printf("   Created File: %s\r\n", FTP_Event_Data->Event_Data.FTP_Server_File_Create_Indication_Data->CreatedFileName);
            printf("   Current Directory: %s\r\n", FTP_Event_Data->Event_Data.FTP_Server_File_Create_Indication_Data->DirectoryName);
            printf("Server>");
            break;
         case etFTP_Server_File_Put_Indication:
            /* The Client requested the transfer of a File to the       */
            /* Server.  Display all relevant information.               */
            printf("\r\n");
            printf("etFTP_Server_File_Put_Indication, FTPID: 0x%04X\r\n", FTP_Event_Data->Event_Data.FTP_Server_File_Put_Indication_Data->FTPID);
            printf("   Transfer Complete: %s, Total Length: 0x%08X, Transferred Length: 0x%08X\r\n", (FTP_Event_Data->Event_Data.FTP_Server_File_Put_Indication_Data->TransferComplete?"TRUE":"FALSE"),
                                                                                                     FTP_Event_Data->Event_Data.FTP_Server_File_Put_Indication_Data->TotalLength,
                                                                                                     FTP_Event_Data->Event_Data.FTP_Server_File_Put_Indication_Data->TransferredLength);
            printf("   File: %s\r\n", FTP_Event_Data->Event_Data.FTP_Server_File_Put_Indication_Data->FileName);
            printf("   Current Directory: %s\r\n", FTP_Event_Data->Event_Data.FTP_Server_File_Put_Indication_Data->DirectoryName);
            printf("Server>");
            break;
         case etFTP_Server_File_Get_Indication:
            /* The Client requested the transfer of a File from the     */
            /* Server.  Display all relevant information.               */
            printf("\r\n");
            printf("etFTP_Server_File_Get_Indication, FTPID: 0x%08X\r\n", FTP_Event_Data->Event_Data.FTP_Server_File_Get_Indication_Data->FTPID);
            printf("   Transfer Complete: %s, Total Length: 0x%08X, Transferred Length: 0x%08X\r\n", (FTP_Event_Data->Event_Data.FTP_Server_File_Get_Indication_Data->TransferComplete?"TRUE":"FALSE"),
                                                                                                     FTP_Event_Data->Event_Data.FTP_Server_File_Get_Indication_Data->TotalLength,
                                                                                                     FTP_Event_Data->Event_Data.FTP_Server_File_Get_Indication_Data->TransferredLength);
            printf("   File: %s\r\n", FTP_Event_Data->Event_Data.FTP_Server_File_Get_Indication_Data->FileName);
            printf("   Current Directory: %s\r\n", FTP_Event_Data->Event_Data.FTP_Server_File_Get_Indication_Data->DirectoryName);
            printf("Server>");
            break;
         default:
            /* Unknown FTP Server Event Received.                       */
            printf("\r\n");
            printf("Unknown FTP Server Event Received.");
            printf("\r\nServer>");
            break;
      }
   ...
   /* The following function is for an FTP Client Event Callback.  This */
   /* function will be called whenever a FTP Client Event occurs that is*/
   /* associated with the Bluetooth Stack.  This function passes to the */
   /* caller the FTP Event Data that occurred and the FTP Event Callback*/
   /* Parameter that was specified when this Callback was installed.    */
   /* The caller is free to use the contents of the FTP FTP Event Data  */
...
static void BTPSAPI FTP_Client_Event_Callback(unsigned int BluetoothStackID, FTP_Client_Event_Data_t *FTP_Event_Data, unsigned long CallbackParameter)
{
   ...
   char                   temp[512];
   char                   size[132];
   FTP_Directory_Entry_t *Entry;

   if((BluetoothStackID) && (FTP_Event_Data))
   {
      switch(FTP_Event_Data->Event_Data_Type)
      {
         case etFTP_Client_Connect_Confirmation:
            /* The Client received a Connect Confirmation, display      */
            /* relevant information.                                    */
            printf("\r\n");
            printf("etFTP_Client_Connect_Confirmation, FTPID: 0x%04X, Status 0x%04X\r\n", FTP_Event_Data->Event_Data.FTP_Client_Connect_Confirmation_Data->FTPID,
                                                                                          FTP_Event_Data->Event_Data.FTP_Client_Connect_Confirmation_Data->FTPConnectStatus);
            BD_ADDRToStr(FTP_Event_Data->Event_Data.FTP_Client_Connect_Confirmation_Data->BD_ADDR, BoardStr);
            printf("BD_ADDR: %s\r\n", BoardStr);

            /* Check to see if the Client Port was opened successfully. */
            if(FTP_Event_Data->Event_Data.FTP_Client_Connect_Confirmation_Data->FTPConnectStatus)
            {
            ...
            printf("\r\nClient>");
            break;
         case etFTP_Client_Disconnect_Indication:
            /* The Client was disconnected, display the appropriate     */
            /* information and clean up the information relevant to this*/
            /* connection.                                              */
            printf("\r\n");
            printf("etFTP_Client_Disconnect_Indication, FTPID: 0x%04X\r\n", FTP_Event_Data->Event_Data.FTP_Client_Disconnect_Indication_Data->FTPID);
            ...
            printf("\r\nClient>");
            break;
         case etFTP_Client_Abort_Confirmation:
            /* An Abort was executed.  Display any relevant information.*/
            printf("\r\n");
            printf("etFTP_Client_Abort_Confirmation, FTPID: 0x%04X\r\n", FTP_Event_Data->Event_Data.FTP_Client_Abort_Confirmation_Data->FTPID);
            printf("\r\nClient>");
            break;
         case etFTP_Client_Directory_Request_Confirmation:
            /* A Directory Request Confirmation was received, display   */
            /* any relevant information and update the Directory.       */
            printf("\r\n");
            printf("etFTP_Client_Directory_Request_Confirmation, FTPID: 0x%04X, Request Complete: %s, Parent Directory: %s\r\n", FTP_Event_Data->Event_Data.FTP_Client_Directory_Request_Confirmation_Data->FTPID,
                                                                                                                                 (FTP_Event_Data->Event_Data.FTP_Client_Directory_Request_Confirmation_Data->RequestCompleted?"TRUE":"FALSE"),
                                                                                                                                 (FTP_Event_Data->Event_Data.FTP_Client_Directory_Request_Confirmation_Data->ParentDirectory?"TRUE":"FALSE"));

            /* Set variables, Entry and i, to point into the structure  */
            /* to increase readability.                                 */
            Entry = FTP_Event_Data->Event_Data.FTP_Client_Directory_Request_Confirmation_Data->FTPDirectoryEntries;
            i     = FTP_Event_Data->Event_Data.FTP_Client_Directory_Request_Confirmation_Data->NumberDirectoryEntries;
            ...
            while(i--)
            {
               ...
               if((Entry->FieldMask & (FTP_OBJECT_INFO_MASK_NAME | FTP_OBJECT_INFO_MASK_SIZE | FTP_OBJECT_INFO_MASK_MODIFIED)))
               {
            ...
            printf("\r\nClient>");
            break;
         case etFTP_Client_Change_Directory_Confirmation:
            /* A Change Directory Confirmation was received, display the*/
            /* appropriate information.                                 */
            printf("\r\n");
            printf("etFTP_Client_Change_Directory_Confirmation, FTPID: 0x%04X, Success: %s\r\n", FTP_Event_Data->Event_Data.FTP_Client_Change_Directory_Confirmation_Data->FTPID,
                                                                                                 (FTP_Event_Data->Event_Data.FTP_Client_Change_Directory_Confirmation_Data->Success?"TRUE":"FALSE"));

            /* Check to see if the command was successfully executed.   */
            if(FTP_Event_Data->Event_Data.FTP_Client_Change_Directory_Confirmation_Data->Success)
            {
               ...
               if(FTP_Event_Data->Event_Data.FTP_Client_Change_Directory_Confirmation_Data->ChangedDirectoryName)
               {
                  ...
                  strcat(CurrentPath, FTP_Event_Data->Event_Data.FTP_Client_Change_Directory_Confirmation_Data->ChangedDirectoryName);
            ...
            printf("\r\nClient>");
            break;
         case etFTP_Client_Directory_Delete_Confirmation:
            /* A Directory Delete Confirmation was received, display any*/
            /* relevant information.                                    */
            printf("\r\n");
            printf("etFTP_Client_Directory_Delete_Confirmation, FTPID: 0x%04X, Success: %s, Directory Not Empty: %s\r\n", FTP_Event_Data->Event_Data.FTP_Client_Directory_Delete_Confirmation_Data->FTPID,
                                                                                                                          (FTP_Event_Data->Event_Data.FTP_Client_Directory_Delete_Confirmation_Data->Success?"TRUE":"FALSE"),
                                                                                                                          (FTP_Event_Data->Event_Data.FTP_Client_Directory_Delete_Confirmation_Data->DirectoryNotEmpty?"TRUE":"FALSE"));
            printf("   Deleted Directory: %s\r\n", FTP_Event_Data->Event_Data.FTP_Client_Directory_Delete_Confirmation_Data->DeletedDirectoryName);
            printf("\r\nClient>");
            break;
         case etFTP_Client_Directory_Create_Confirmation:
            /* A Directory Create Confirmation was received, display any*/
            /* relevant information.                                    */
            printf("\r\n");
            printf("etFTP_Client_Directory_Create_Confirmation, FTPID: 0x%04X, Success: %s\r\n", FTP_Event_Data->Event_Data.FTP_Client_Directory_Create_Confirmation_Data->FTPID,
                                                                                                 (FTP_Event_Data->Event_Data.FTP_Client_Directory_Create_Confirmation_Data->Success?"TRUE":"FALSE"));
            printf("   Created Directory: %s\r\n", FTP_Event_Data->Event_Data.FTP_Client_Directory_Create_Confirmation_Data->CreatedDirectoryName);

            /* Check to see if the command was issued successfully.     */
            if(FTP_Event_Data->Event_Data.FTP_Client_Directory_Create_Confirmation_Data->Success)
            {
               ...
               strcat(CurrentPath, FTP_Event_Data->Event_Data.FTP_Client_Directory_Create_Confirmation_Data->CreatedDirectoryName);
            ...
            printf("\r\nClient>");
            break;
         case etFTP_Client_File_Delete_Confirmation:
            /* A File Delete Confirmation was received, display any     */
            /* relevant information.                                    */
            printf("\r\n");
            printf("etFTP_Client_File_Delete_Confirmation, FTPID: 0x%04X, Success: %s\r\n", FTP_Event_Data->Event_Data.FTP_Client_File_Delete_Confirmation_Data->FTPID,
                                                                                            (FTP_Event_Data->Event_Data.FTP_Client_File_Delete_Confirmation_Data->Success?"TRUE":"FALSE"));
            printf("   Deleted File: %s\r\n", FTP_Event_Data->Event_Data.FTP_Client_File_Delete_Confirmation_Data->DeletedFileName);

            printf("\r\nClient>");
            break;
         case etFTP_Client_File_Create_Confirmation:
            /* A File Create Confirmation was received, display any     */
            /* relevant information.                                    */
            printf("\r\n");
            printf("etFTP_Client_File_Create_Confirmation, FTPID: 0x%04X, Success: %s\r\n", FTP_Event_Data->Event_Data.FTP_Client_File_Create_Confirmation_Data->FTPID,
                                                                                            (FTP_Event_Data->Event_Data.FTP_Client_File_Create_Confirmation_Data->Success?"TRUE":"FALSE"));
            printf("   Created File: %s\r\n", FTP_Event_Data->Event_Data.FTP_Client_File_Create_Confirmation_Data->CreatedFileName);

            printf("\r\nClient>");
            break;
         case etFTP_Client_File_Put_Confirmation:
            /* A Put File Confirmation was received, display any        */
            /* relevant information.                                    */
            printf("\r\n");
            printf("etFTP_Client_File_Put_Confirmation, FTPID: 0x%04X, Success: %s\r\n", FTP_Event_Data->Event_Data.FTP_Client_File_Put_Confirmation_Data->FTPID,
                                                                                         (FTP_Event_Data->Event_Data.FTP_Client_File_Put_Confirmation_Data->Success?"TRUE":"FALSE"));
            printf("   Transfer Complete: %s, Total Length: 0x%08X, Transferred Length: 0x%08X\r\n", (FTP_Event_Data->Event_Data.FTP_Client_File_Put_Confirmation_Data->TransferComplete?"TRUE":"FALSE"),
                                                                                                     FTP_Event_Data->Event_Data.FTP_Client_File_Put_Confirmation_Data->TotalLength,
                                                                                                     FTP_Event_Data->Event_Data.FTP_Client_File_Put_Confirmation_Data->TransferredLength);
            printf("   File: %s\r\n", FTP_Event_Data->Event_Data.FTP_Client_File_Put_Confirmation_Data->FileName);

            printf("\r\nClient>");
            break;
         case etFTP_Client_File_Get_Confirmation:
            /* A Get File Confirmation was received, display any        */
            /* relevant information.                                    */
            printf("\r\n");
            printf("etFTP_Client_File_Get_Confirmation, FTPID: 0x%08X, Success: %s\r\n", FTP_Event_Data->Event_Data.FTP_Client_File_Get_Confirmation_Data->FTPID, (FTP_Event_Data->Event_Data.FTP_Client_File_Get_Confirmation_Data->Success?"TRUE":"FALSE"));
            ...
            printf("   Transfer Complete: %s, Transferred Length: 0x%08X\r\n", (FTP_Event_Data->Event_Data.FTP_Client_File_Get_Confirmation_Data->TransferComplete?"TRUE":"FALSE"),
                                                                               FTP_Event_Data->Event_Data.FTP_Client_File_Get_Confirmation_Data->TransferredLength);
            printf("   File: %s\r\n", FTP_Event_Data->Event_Data.FTP_Client_File_Get_Confirmation_Data->FileName);
            ...
            if(!Result)
            {
               ...
               if(!Result)
               {
                  ...
                  if(!Result)
                  {
                     ...
                     {
                        /* The program is currently running in Server   */
                        /* Mode.  Start the User Interface.             */

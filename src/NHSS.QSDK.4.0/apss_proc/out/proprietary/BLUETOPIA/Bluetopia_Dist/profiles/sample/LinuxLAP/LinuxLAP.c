...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXLAP - Simple Linux application using the LAN Access Profile.         */
/*                                                                            */
/*  Author:  Rory Sledge                                                      */
/*                                                                            */
...
/*   --------  -----------    ------------------------------------------------*/
/*   08/21/02  R. Sledge      Initial creation.                               */
/******************************************************************************/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>

#include "LinuxLAP.h"      /* Main Application Prototypes and Constants.      */

#include "SS1BTPS.h"       /* Includes the SS1 Bluetooth Protocol Stack.      */
#include "SS1BTLAP.h"      /* Includes the SS1 LAN Access Profile.            */
#include "SS1BTDBG.h"      /* Includes/Constants for Bluetooth Debugging.     */

#define LOCAL_NAME_ROOT                      "LinuxLAP"  /* Root of the local */
                                                         /* name              */
...
#define DEFAULT_DEBUG_LOG_FILE_NAME  "LinuxLAP_ASC.log"
#define DEFAULT_DEBUG_FTS_FILE_NAME  "LinuxLAP_FTS.log"
...
static unsigned int        LAPServerID;             /* Variable which contains the     */
                                                    /* handle of the most recently     */
                                                    /* opened LAN Access Server.       */

static DWord_t             LAPServerSDPHandle;      /* Variable which holds the handle */
                                                    /* to the LAN Access SDP Record.   */

static Boolean_t           ServerConnected;         /* Variable which contains the     */
                                                    /* current connection state of the */
                                                    /* LAN Access Server.              */

static unsigned int        LAPClientID;             /* Variable which contains the     */
                                                    /* handle of the most recently     */
                                                    /* opened LAN Access Client.       */
...
static void BTPSAPI LAP_Event_Callback(unsigned int BluetoothStackID, LAP_Event_Data_t *LAP_Event_Data, unsigned long CallbackParameter);
...
static int DeleteLinkKey(BD_ADDR_t BD_ADDR)
{
   ...
   /* Command Options for this LAN Access Sample Application.  The input*/
...
static int Pair(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if((!LAPClientID) && (!ServerConnected))
      {
...
static int ServiceDiscovery(ParameterList_t *TempParam)
{
   ...
   /* Server and registering a SDP record to be associated with the     */
   /* opened Server.  This function returns zero on successful execution*/
...
static int OpenServer(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      /* Check to see if there is already LAN Access Server Port Open.  */
      if(!LAPServerID)
      {
         ...
         {
            /* Submit the request to Open a LAN Access Server Port.     */
            ret_val = LAP_Open_Server_Port(BluetoothStackID, TempParam->Params[0].intParam, TempParam->Params[1].intParam, LAP_Event_Callback, 0);

            /* Next, check to see if the LAP_Open_Server_Port() function*/
            /* was successful.                                          */
            if(ret_val > 0)
            {
               /* Inform the user that the Server was successfully      */
               /* opened and save the Server Port ID for later use.     */
               printf("Return Value is %d LAP_Open_Server_Port().\r\n", ret_val);
               LAPServerID = ret_val;
               ...
               {
                  SET_MAJOR_DEVICE_CLASS(ClassOfDevice, HCI_LMP_CLASS_OF_DEVICE_MAJOR_DEVICE_CLASS_LAN_ACCESS_POINT);
               ...
               sprintf(ServiceName, "LAN Access Serial Port %u", TempParam->Params[0].intParam);

               /* Now that a Service Name has been created try and      */
               /* Register the SDP Record.                              */
               ret_val = LAP_Register_SDP_Record(BluetoothStackID, LAPServerID, ServiceName, &LAPServerSDPHandle);
               ...
               if(!ret_val)
               {
                  ...
                  printf("Return Value is %d LAP_Register_SDP_Record().\r\n", ret_val);
               }
               else
               {
                  /* Display an Error Message and make sure the Current */
                  /* Server SDP Handle is invalid, and close the LAN    */
                  /* Access Server Port we just opened because we       */
                  /* weren't able to register a SDP Record.             */
                  printf("Return Value is %d LAP_Register_SDP_Record() FAILURE.\r\n", ret_val);
                  LAPServerSDPHandle = 0;

                  /* Now try and close the Opened Server Port.          */
                  ret_val = LAP_Close_Server_Port(BluetoothStackID, LAPServerID);
                  LAPServerID = 0;
                  ...
                  if(!ret_val)
                  {
                     ...
                     /* successfully closed.                            */
                     printf("Return Value is %d LAP_Close_Server_Port().\r\n", ret_val);
                  }
                  else
                  {
                     /* An error occurred while attempting to close the */
                     /* Port.                                           */
                     printf("Return Value is %d LAP_Close_Server_Port() FAILURE.\r\n", ret_val);
                  }
               }
            }
            else
            {
               ...
               printf("Return Value is %d LAP_Open_Server_Port() FAILURE.\r\n", ret_val);
            }
         }
         else
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: OpenServer [RFCOMM Port] [Local Device To Open].\r\n");
      ...
      {
         /* This application supports only one LAN Access Server at a   */
         /* time, inform the user that an LAN Access Server already     */
         /* exists.                                                     */
         printf("LAN Server Already Exists.\r\n");
   ...
   /* The following function is responsible for closing an opened LAN   */
   /* Access Server and unregistering the SDP record associated with the*/
   /* open server.  This function returns zero on successful execution  */
   /* and a negative value on all errors.                               */
static int CloseServer(ParameterList_t *TempParam)
{
   int ret_val;
   ...
   if(BluetoothStackID)
   {
      ...
      /* valid.                                                         */
      if(LAPServerSDPHandle)
      {
         /* The SDP Record Handle is valid so remove the record from the*/
         /* SDP Database.                                               */
         ret_val = LAP_Un_Register_SDP_Record(BluetoothStackID, CurrentPortID, LAPServerSDPHandle);
         ...
         if(!ret_val)
         {
            ...
            printf("Return Value is %d LAP_Un_Register_SDP_Record().\r\n", ret_val);
            LAPServerSDPHandle = 0;
         }
         else
         {
            ...
            /* Record.                                                  */
            printf("Return Value is %d LAP_Un_Register_SDP_Record() FAILURE.\r\n", ret_val);
         }
      }
      else
      {
      ...
      /* Check to see if there is an open LAN Access Server Port.       */
      if(LAPServerID)
      {
         /* Submit the request to Close a LAN Access Server Port.       */
         ret_val = LAP_Close_Server_Port(BluetoothStackID, LAPServerID);

         /* Next, check to see if the LAP_Close_Server_Port() function  */
         /* was successful.                                             */
         if(!ret_val)
         {
            printf("Return Value is %d LAP_Close_Server_Port().\r\n", ret_val);
   ...
   /* a remote LAP Server.  This function returns zero on successful    */
   /* execution and a negative value on all errors.                     */
static int OpenClient(ParameterList_t *TempParam)
{
   int       ret_val;
   ...
   if(BluetoothStackID)
   {
      /* Check to see if there is already LAN Access Client Port Open   */
      /* and that the server is not currently connected.                */
      if((!LAPClientID) && (!ServerConnected))
      {
         /* Now check the Temp Parameter structure to make sure that it */
         /* contains valid Parameter Information.                       */
         if((TempParam) && (TempParam->NumberofParameters > 2) && (TempParam->Params[0].intParam) && (NumberofValidResponses) && (TempParam->Params[0].intParam <= NumberofValidResponses) && (!COMPARE_BD_ADDR(InquiryResultList[(TempParam->Params[0].intParam-1)], NullADDR)) && (TempParam->Params[1].intParam))
         {
            ...
            printf("LAP_Open_Remote_Port(BD_ADDR = %s).\r\n", BoardStr);

            /* Submit the request to Open a LAN Access Client Port.     */
            ret_val = LAP_Open_Remote_Port(BluetoothStackID, InquiryResultList[(TempParam->Params[0].intParam-1)], TempParam->Params[1].intParam, TempParam->Params[2].intParam, LAP_Event_Callback, 0);

            /* Next, check to see if the LAP_Open_Remote_Port() function*/
            /* was successful.                                          */
            if(ret_val > 0)
            {
               printf("Return Value is %d LAP_Open_Remote_Port().\r\n", ret_val);
            ...
            {
               printf("Return Value is %d LAP_Open_Remote_Port() FAILURE.\r\n", ret_val);
            }
         }
         else
         {
      ...
      {
         /* This application supports only one LAN Access Connection at */
         /* a time, inform the user that an LAN Access Connection       */
         /* already exists.                                             */
         printf("LAN Access Connection Already Exists.\r\n");
   ...
   /* remote LAN Access Port.  This function returns zero on successful */
   /* execution and a negative value on all errors.                     */
static int CloseClient(ParameterList_t *TempParam)
{
   int ret_val;
   ...
   if(BluetoothStackID)
   {
      /* Check to see if there is an open LAN Access Client Port or the */
      /* Server is currently connected.                                 */
      if(((ServerConnected)?LAPServerID:LAPClientID))
      {
         /* Submit the request to Close a LAN Access Client Port.       */
         ret_val = LAP_Close_Port(BluetoothStackID, ((ServerConnected)?LAPServerID:LAPClientID));

         /* Next, check to see if the LAP_Close_Port() function was     */
         /* successful.                                                 */
         if(!ret_val)
         {
            printf("Return Value is %d LAP_Close_Port().\r\n", ret_val);
            LAPClientID     = 0;
            ServerConnected = FALSE;
         }
         else
         {
            printf("Return Value is %d LAP_Close_Port() FAILURE.\r\n", ret_val);
         }
      }
      else
      {
...
static void BTPSAPI SDP_Event_Callback(unsigned int BluetoothStackID, unsigned int SDPRequestID, SDP_Response_Data_t *SDP_Response_Data, unsigned long CallbackParameter)
{
   ...
   /* The following function is for the LAP Event Callback.  This       */
   /* function will be called whenever a Callback has been registered   */
   /* for the specified LAP Action that is associated with the Bluetooth*/
   /* Stack.  This function passes to the caller the LAP Event Data of  */
   /* the specified Event and the LAP Event Callback Parameter that was */
   /* specified when this Callback was installed.  The caller is free to*/
   /* use the contents of the LAP Event Data ONLY in the context of this*/
   /* callback.  If the caller requires the Data for a longer period of */
   /* time, then the callback function MUST copy the data into another  */
   /* Data Buffer.  This function is guaranteed NOT to be invoked more  */
   /* than once simultaneously for the specified installed callback     */
   ...
   /* the Thread Context of a Thread that the User does NOT own.        */
   /* Therefore, processing in this function should be as efficient as  */
   /* possible (this argument holds anyway because other LAP Events will*/
   /* not be processed while this function call is outstanding).        */
   /* * NOTE * This function MUST NOT Block and wait for events that    */
   /*          can only be satisfied by Receiving other LAP Events.  A  */
   /*          Deadlock WILL occur because NO LAP Event Callbacks will  */
   /*          be issued while this function is currently outstanding.  */
static void BTPSAPI LAP_Event_Callback(unsigned int BluetoothStackID, LAP_Event_Data_t *LAP_Event_Data, unsigned long CallbackParameter)
{
   char BoardStr[13];
   ...
   /* First, check to see if the required parameters appear to be       */
   /* semi-valid.                                                       */
   if((LAP_Event_Data) && (BluetoothStackID))
   {
      /* The parameters appear to be semi-valid, now check to see what  */
      /* type the incoming event is.                                    */
      switch(LAP_Event_Data->Event_Data_Type)
      {
         case etLAP_Open_Port_Indication:
            /* A Client has connected to the Server, display the BD_ADDR*/
            /* of the connecting device.                                */
            printf("\r\n");
            BD_ADDRToStr(LAP_Event_Data->Event_Data.LAP_Open_Port_Indication_Data->BD_ADDR, BoardStr);
            printf("LAP Open Port Indication, ID: 0x%04X, Board: %s.\r\n", LAP_Event_Data->Event_Data.LAP_Open_Port_Indication_Data->LAPPortID, BoardStr);

            ServerConnected = TRUE;
            break;
         case etLAP_Open_Port_Confirmation:
            /* The Client received a Connect Confirmation, display      */
            /* relevant information.                                    */
            printf("\r\n");
            printf("LAP Open Port Confirmation, ID: 0x%04X, Status: 0x%04X.\r\n", LAP_Event_Data->Event_Data.LAP_Open_Port_Confirmation_Data->LAPPortID,
                                                                                  LAP_Event_Data->Event_Data.LAP_Open_Port_Confirmation_Data->PortOpenStatus);

            /* Check to see if the Client Port was opened successfully. */
            if(LAP_Event_Data->Event_Data.LAP_Open_Port_Confirmation_Data->PortOpenStatus)
            {
            ...
            break;
         case etLAP_Close_Port_Indication:
            /* A Close Port Indication was received, display all        */
            /* relevant information.                                    */
            printf("\r\n");
            printf("LAP Close Port Indication, ID: 0x%04X.\r\n", LAP_Event_Data->Event_Data.LAP_Close_Port_Indication_Data->LAPPortID);

            /* Check to see if the closing port is the Client Port.     */
            if(LAP_Event_Data->Event_Data.LAP_Close_Port_Indication_Data->LAPPortID == LAPClientID)
            {
            ...
            break;
         default:
            /* We are not interested in any other LAP Events.           */
            break;
      }
   }
   else
   {
      /* There was an error with one or more of the input parameters.   */
      printf("LAP callback data: Event_Data = NULL.\r\n");
   }

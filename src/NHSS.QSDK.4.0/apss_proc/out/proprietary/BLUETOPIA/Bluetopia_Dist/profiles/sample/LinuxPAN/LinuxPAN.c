...
#include "LinuxPAN.h"    /* Main Application Prototypes and Constants.        */

#include "SS1BTPS.h"     /* Includes the SS1 Bluetooth Protocol Stack.        */
#include "SS1BTPAN.h"    /* Includes the SS1 Personal Area Networking Profile.*/

#include "SS1BTDBG.h"    /* Includes/Constants for Bluetooth Debugging.       */

#define LOCAL_NAME_ROOT                      "LinuxPAN"  /* Root of the local */
                                                         /* name              */
...
#define DEFAULT_VNET_INDEX                          (0)  /* Denotes the VNET  */
                                                         /* Device Index to   */
                                                         /* use when          */
                                                         /* utilizing the PAN */
                                                         /* Profile.          */
...
#define DEFAULT_DEBUG_LOG_FILE_NAME  "LinuxPAN_ASC.log"
#define DEFAULT_DEBUG_FTS_FILE_NAME  "LinuxPAN_FTS.log"
...
static Boolean_t           PANServerOpen;           /* Variable which contains the     */
                                                    /* current server registration     */
                                                    /* state of the Personal Area      */
                                                    /* Networking Server.              */

static DWord_t             PANUSDPServiceRecordHandle; /* Variable which holds the     */
                                                    /* handle to the Personal Area     */
                                                    /* Networking PAN USER SDP Record. */

static DWord_t             NAPSDPServiceRecordHandle; /* Variable which holds the      */
                                                    /* handle to the Personal Area     */
                                                    /* Networking PAN NAP SDP Record.  */

static DWord_t             GNSDPServiceRecordHandle;  /* Variable which holds the      */
                                                    /* handle to the Personal Area     */
                                                    /* Networking PAN Group Ad-hoc SDP */
                                                    /* Record.                         */

static Boolean_t           ServerConnected;         /* Variable which contains the     */
                                                    /* current connection state of the */
                                                    /* Personal Area Networking Server.*/

static unsigned int        PANClientID;             /* Variable which contains the     */
                                                    /* handle of the most recently     */
                                                    /* opened Personal Area Networking */
                                                    /* Client.                         */
   ...
   if(!BluetoothStackID)
   {
      ...
      if(HCI_DriverInformation)
      {
         ...
         if(Result > 0)
         {
            ...
            PANUSDPServiceRecordHandle = 0;
            NAPSDPServiceRecordHandle  = 0;
            GNSDPServiceRecordHandle   = 0;

            /* Flag that there are no clients or servers active.        */
            PANClientID                = 0;
            PANServerOpen              = FALSE;
            ...
            /* Go ahead and initialize the PAN Profile since the stack  */
            /* initialization was successful.                           */
            if((ret_val = PAN_Initialize(BluetoothStackID, DEFAULT_VNET_INDEX)) == 0)
               printf("PAN_Initialize: Success.\r\n");
            else
            {
               printf("PAN_Initialize: Failure: %d\r\n", ret_val);
...
static int CloseStack(void)
{
   ...
   if(BluetoothStackID)
   {
      /* First, close down the PAN Profile.                             */
      PAN_Cleanup(BluetoothStackID);
...
static int DeleteLinkKey(BD_ADDR_t BD_ADDR)
{
   ...
   /* Command Options for this Personal Area Networking Sample          */
   /* Application.  The input parameter to this function is completely  */
...
static int Pair(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if((!PANClientID) && (!ServerConnected))
      {
...
static int ServiceDiscovery(ParameterList_t *TempParam)
{
   ...
   /* The following function is responsible for opening a Personal Area */
...
static int OpenServer(ParameterList_t *TempParam)
{
   ...
   Word_t                     NetworkPacketTypeList[2];
   ...
   if(BluetoothStackID)
   {
      /* Check to see if there is already Personal Area Networking      */
      /* Server Open.                                                   */
      if(!PANServerOpen)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters > 0))
         {
            /* Set the Services Mask to use for the connection.         */
            ServicesMask = TempParam->Params[0].intParam;

            /* Submit the request to Open a Personal Area Networking    */
            /* Server.                                                  */
            ret_val = PAN_Open_Server(BluetoothStackID, ServicesMask, PAN_Event_Callback, 0);

            /* Next, check to see if the PAN_Open_Server() function was */
            /* successful.                                              */
            if(!ret_val)
            {
               /* Inform the user that the Server was successfully      */
               /* opened and save the Personal Area Networking ID for   */
               /* later use.                                            */
               printf("Return Value is %d PAN_Open_Server().\r\n", ret_val);

               PANServerOpen = TRUE;

               /* Initialize the Network Packet Type List.              */
               NetworkPacketTypeList[0] = 0x0800;
               NetworkPacketTypeList[1] = 0x0806;

               /* The Server was opened successfully, now registered a  */
               /* SDP Record indicating that Server of the specified    */
               /* Service Type exists.  Determine if the Personal Area  */
               /* Network User Service Type was specified.              */
               if((ServicesMask & PAN_PERSONAL_AREA_NETWORK_USER_SERVICE) == PAN_PERSONAL_AREA_NETWORK_USER_SERVICE)
               {
                  /* The Personal Area Network User Service Type was    */
                  /* specified.  Initialized the required variables.    */
                  strcpy(ServiceName, "Personal Ad-hoc User Service");
                  strcpy(ServiceDescription, "Personal Ad-hoc User Service");

                  ret_val = PAN_Register_Personal_Area_Network_User_SDP_Record(BluetoothStackID, sizeof(NetworkPacketTypeList)/sizeof(Word_t), NetworkPacketTypeList, ServiceName, ServiceDescription, PAN_SECURITY_DESCRIPTION_NONE, &PANUSDPServiceRecordHandle);
               }

               /* Determine if the Network Access Point Service Type was*/
               /* specified and no error has occurred up until this     */
               /* point.                                                */
               if((!ret_val) && ((ServicesMask & PAN_NETWORK_ACCESS_POINT_SERVICE) == PAN_NETWORK_ACCESS_POINT_SERVICE))
               {
                  /* The Network Access Point Service Type was          */
                  /* specified.  Initialized the required variables.    */
                  strcpy(ServiceName, "Network Access Point Service");
                  strcpy(ServiceDescription, "Personal Ad-hoc Network Service which provides access to a network");

                  ret_val = PAN_Register_Network_Access_Point_SDP_Record(BluetoothStackID, sizeof(NetworkPacketTypeList)/sizeof(Word_t), NetworkPacketTypeList, ServiceName, ServiceDescription, PAN_SECURITY_DESCRIPTION_NONE, PAN_NETWORK_ACCESS_TYPE_10_MB_ETHERNET, 1000000, &NAPSDPServiceRecordHandle);
               }

               /* Determine if the Group Ad-hoc Network Service Type was*/
               /* specified, and no error has occurred up until this    */
               /* point.                                                */
               if((!ret_val) && ((ServicesMask & PAN_GROUP_ADHOC_NETWORK_SERVICE) == PAN_GROUP_ADHOC_NETWORK_SERVICE))
               {
                  /* The Group Ad-hoc Network Service Type was          */
                  /* specified.  Initialize the required variables.     */
                  strcpy(ServiceName, "Group Ad-hoc Network Service");
                  strcpy(ServiceDescription, "Personal Group Ad-hoc Network Service");

                  ret_val = PAN_Register_Group_Adhoc_Network_SDP_Record(BluetoothStackID, sizeof(NetworkPacketTypeList)/sizeof(Word_t), NetworkPacketTypeList, ServiceName, ServiceDescription, PAN_SECURITY_DESCRIPTION_NONE, &GNSDPServiceRecordHandle);
               }

               /* Check the result of the the call to add the SDP Record*/
               /* to the Service Discovery Database for success.        */
               if(!ret_val)
               {
                  /* Display a message indicating that the SDP records  */
                  /* were registered successfully.                      */
                  printf("PAN_Register_Server_SDP_Record: Function Successful.\r\n");

                  /* Since we are a PAN Server, we should go ahead and  */
                  /* try to become the Master to allow multiple PAN     */
               ...
               {
                  /* An error occurred while registering the SDP records*/
                  /* clean up any of the records which were successfully*/
                  /* registered.                                        */
                  PAN_Un_Register_SDP_Record(BluetoothStackID, PANUSDPServiceRecordHandle);
                  PAN_Un_Register_SDP_Record(BluetoothStackID, NAPSDPServiceRecordHandle);
                  PAN_Un_Register_SDP_Record(BluetoothStackID, GNSDPServiceRecordHandle);

                  PANUSDPServiceRecordHandle = 0;
                  NAPSDPServiceRecordHandle  = 0;
                  GNSDPServiceRecordHandle   = 0;

                  /* Next display an error message.                     */
                  printf("Return Value is %d PAN_Register_SDP_Record() FAILURE.\r\n", ret_val);

                  /* Now try and close the Opened Server.               */
                  ret_val       = PAN_Close_Server(BluetoothStackID);
                  PANServerOpen = FALSE;
                  ...
                  if(!ret_val)
                  {
                     ...
                     /* successfully closed.                            */
                     printf("Return Value is %d PAN_Close_Server().\r\n", ret_val);
                  }
                  else
                  {
                     /* An error occurred while attempting to close the */
                     /* Server.                                         */
                     printf("Return Value is %d PAN_Close_Server() FAILURE.\r\n", ret_val);
                  }
               }
            }
            else
            {
               /* An error occurred while attempting to open server.    */
               /* Inform the user of the errror.                        */
               printf("Return Value is %d PAN_Open_Server() FAILURE.\r\n", ret_val);
            }
         }
         else
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: OpenServer [Services Bit Mask (User = 0x01, NAP = 0x02, Group Ad-Hoc = 0x04].\r\n");
      ...
      {
         /* This application supports only one PAN Server at a time,    */
         /* inform the user that an PAN Server already exists.          */
         printf("PAN Server Already Exists.\r\n");
   ...
   /* Personal Area Networking Server and unregistering the SDP record  */
...
static int CloseServer(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      /* Delete the SDP Record associated with the PAN Server.          */
      if(PANUSDPServiceRecordHandle)
         PAN_Un_Register_SDP_Record(BluetoothStackID, PANUSDPServiceRecordHandle);

      if(NAPSDPServiceRecordHandle)
         PAN_Un_Register_SDP_Record(BluetoothStackID, NAPSDPServiceRecordHandle);

      if(GNSDPServiceRecordHandle)
         PAN_Un_Register_SDP_Record(BluetoothStackID, GNSDPServiceRecordHandle);

      /* Flag that there are no SDP Service Records Registered.         */
      PANUSDPServiceRecordHandle = 0;
      NAPSDPServiceRecordHandle  = 0;
      GNSDPServiceRecordHandle   = 0;

      /* Check to see if there is an open PAN Server.                   */
      if(PANServerOpen)
      {
         /* Submit the request to Close a PAN Server.                   */
         ret_val = PAN_Close_Server(BluetoothStackID);

         /* Next, check to see if the PAN_Close_Server() function was   */
         /* successful.                                                 */
         if(!ret_val)
         {
            printf("Return Value is %d PAN_Close_Server().\r\n", ret_val);
   ...
   /* a remote Personal Areay Networking Server.  This function returns */
   /* zero on successful execution and a negative value on all errors.  */
static int OpenClient(ParameterList_t *TempParam)
{
   int       ret_val;
   ...
   if(BluetoothStackID)
   {
      /* Check to see if there is already PAN Client Open and that the  */
      /* server is not currently connected.                             */
      if((!PANClientID) && (!ServerConnected))
      {
         /* Now check the Temp Parameter structure to make sure that it */
         /* contains valid Parameter Information.                       */
         if((TempParam) && (TempParam->NumberofParameters >= 3) && (TempParam->Params[0].intParam) && (NumberofValidResponses) && (TempParam->Params[0].intParam <= NumberofValidResponses) && (!COMPARE_BD_ADDR(InquiryResultList[(TempParam->Params[0].intParam-1)], NullADDR)) && (TempParam->Params[1].intParam) && (TempParam->Params[2].intParam))
         {
            ...
            printf("PAN_Open_Remote_Server(BD_ADDR = %s).\r\n", BoardStr);

            /* Submit the request to Open a PAN Client.                 */
            ret_val = PAN_Open_Remote_Server(BluetoothStackID, InquiryResultList[(TempParam->Params[0].intParam-1)], (PAN_Service_Type_t)(TempParam->Params[1].intParam - 1), (PAN_Service_Type_t)(TempParam->Params[2].intParam - 1), PAN_Event_Callback, 0);

            /* Next, check to see if the PAN_Open_Remote_Server()       */
            /* function was successful.                                 */
            if(ret_val > 0)
            {
               printf("Return Value is %d PAN_Open_Remote_Server().\r\n", ret_val);
            ...
            {
               printf("Return Value is %d PAN_Open_Remote_Server() FAILURE.\r\n", ret_val);
            }
         }
         else
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: OpenClient [Inquiry Index], [Local Service Type (User = 0x01, NAP = 0x02, Group Ad-hoc = 0x03)] [Remote Service Type (User = 0x01, NAP = 0x02, Group Ad-hoc = 0x03)].\r\n");
      ...
      {
         /* This application supports only one PAN Connection at a time.*/
         /* Inform the user that an PAN Connection already exists.      */
         printf("PAN Connection Already Exists.\r\n");
   ...
   /* remote Personal Area Networking connection.  This function returns*/
   /* zero on successful execution and a negative value on all errors.  */
static int CloseClient(ParameterList_t *TempParam)
{
   int ret_val;
   ...
   if(BluetoothStackID)
   {
      /* Check to see if there is an open PAN Client.                   */
      if(PANClientID)
      {
         /* Submit the request to Close a PAN Client.                   */
         ret_val = PAN_Close(BluetoothStackID, PANClientID);

         /* Next, check to see if the PAN_Close() function was          */
         /* successful.                                                 */
         if(!ret_val)
         {
            printf("Return Value is %d PAN_Close().\r\n", ret_val);

            PANClientID     = 0;
         }
         else
         {
            printf("Return Value is %d PAN_Close() FAILURE.\r\n", ret_val);
         }
      }
      else
      {
...
static void BTPSAPI SDP_Event_Callback(unsigned int BluetoothStackID, unsigned int SDPRequestID, SDP_Response_Data_t *SDP_Response_Data, unsigned long CallbackParameter)
{
   ...
   /* The following function is for the PAN Event Callback.  This       */
   /* function will be called whenever a Callback has been registered   */
   /* for the specified PAN Action that is associated with the Bluetooth*/
   /* Stack.  This function passes to the caller the PAN Event Data of  */
   /* the specified Event and the PAN Event Callback Parameter that was */
   /* specified when this Callback was installed.  The caller is free to*/
   /* use the contents of the PAN Event Data ONLY in the context of this*/
   /* callback.  If the caller requires the Data for a longer period of */
   /* time, then the callback function MUST copy the data into another  */
   /* Data Buffer.  This function is guaranteed NOT to be invoked more  */
   /* than once simultaneously for the specified installed callback     */
   ...
   /* the Thread Context of a Thread that the User does NOT own.        */
   /* Therefore, processing in this function should be as efficient as  */
   /* possible (this argument holds anyway because other PAN Events will*/
   /* not be processed while this function call is outstanding).        */
   /* * NOTE * This function MUST NOT Block and wait for events that    */
   /*          can only be satisfied by Receiving other PAN Events.  A  */
   /*          Deadlock WILL occur because NO PAN Event Callbacks will  */
   /*          be issued while this function is currently outstanding.  */
static void BTPSAPI PAN_Event_Callback(unsigned int BluetoothStackID, PAN_Event_Data_t *PAN_Event_Data, unsigned long CallbackParameter)
{
   char BoardStr[13];
   char ServiceString[64];

   /* First, check to see if the required parameters appear to be       */
   /* semi-valid.                                                       */
   if((PAN_Event_Data) && (BluetoothStackID))
   {
      /* The parameters appear to be semi-valid, now check to see what  */
      /* type the incoming event is.                                    */
      switch(PAN_Event_Data->Event_Data_Type)
      {
         case etPAN_Open_Indication:
            ...
            BD_ADDRToStr(PAN_Event_Data->Event_Data.PAN_Open_Indication_Data->BD_ADDR, (char *)BoardStr);

            switch(PAN_Event_Data->Event_Data.PAN_Open_Indication_Data->ServiceType)
            {
               case pstPersonalAreaNetworkUser:
                  strcpy(ServiceString, "Personal Area Network User");
                  break;
               case pstNetworkAccessPoint:
                  strcpy(ServiceString, "Network Access Point");
                  break;
               case pstGroupAdhocNetwork:
                  strcpy(ServiceString, "Group Ad-Hoc Network");
                  break;
               default:
                  strcpy(ServiceString, "Unknown");
                  break;
            }

            printf("PAN Open Indication, ID: 0x%04X, Board: %s, Service Type: %s\r\n", PAN_Event_Data->Event_Data.PAN_Open_Indication_Data->PANID, BoardStr, ServiceString);

            /* Flag that the server is now connected.                   */
            ServerConnected = TRUE;
            break;
         case etPAN_Open_Confirmation:
            /* The Client received a Open Confirmation, display relevant*/
            /* information.                                             */
            printf("PAN Open Confirmation, ID: 0x%04X, Status: 0x%04X\r\n", PAN_Event_Data->Event_Data.PAN_Open_Confirmation_Data->PANID, PAN_Event_Data->Event_Data.PAN_Open_Confirmation_Data->OpenStatus);

            /* Check to see if the Client was opened successfully.      */
            if(PAN_Event_Data->Event_Data.PAN_Open_Confirmation_Data->OpenStatus)
            {
         ...
         case etPAN_Close_Indication:
            ...
            /* information.                                             */
            printf("PAN Close Indication, ID: 0x%04X\r\n", PAN_Event_Data->Event_Data.PAN_Close_Indication_Data->PANID);
            ...
            break;
         default:
            /* Unknown PAN Profile Event.                               */
            printf("Unknown PAN Event %d\r\n", PAN_Event_Data->Event_Data_Type);
            break;
      }
   }
   else
   {
      /* There was an error with one or more of the input parameters.   */
      printf("PAN callback data: Event_Data = NULL.\r\n");
   }

...
#include "LinuxGAV.h"      /* Main Application Prototypes and Constants.      */
...
#define LOCAL_NAME_ROOT                      "LinuxGAV"  /* Root of the local */
                                                         /* name              */
...
#define DEFAULT_DEBUG_LOG_FILE_NAME  "LinuxGAV_ASC.log"
#define DEFAULT_DEBUG_FTS_FILE_NAME  "LinuxGAV_FTS.log"
...
static int                 IsClient;                /* Variable used to indicate if the*/
                                                    /* program is to be run in GAVD    */
                                                    /* Client Mode (TRUE) or GAVD      */
                                                    /* Server Mode (FALSE).            */
...
static void DisplayGAVDClientMenu(void);
static void DisplayGAVDServerMenu(void);
static void PopulateGAVDClientCommandTable(void);
static void PopulateGAVDServerCommandTable(void);
...
static int InitializeGAVDClient(void);
static int InitializeGAVDServer(void);
...
static void BTPSAPI GAVD_Event_Callback(unsigned int BluetoothStackID, GAVD_Event_Data_t *GAVDEventData, unsigned long CallbackParameter);
   ...
   /* The following function displays the GAVD Client Command Menu.     */
static void DisplayGAVDClientMenu(void)
{
   ...
   printf("*  ConnectRemoteServer [Inquiry Index]                              *\r\n");
   printf("*  DiscoverEndpoints                                                *\r\n");
   printf("*  GetCapabilities [RSEID]                                          *\r\n");
   printf("*  GetConfiguration [RSEID]                                         *\r\n");
   printf("*  DisconnectRemoteServer                                           *\r\n");
   printf("*  OpenRemoteEndpoint [Inquiry Index] [RSEID]                       *\r\n");
   printf("*  AbortEndpoint                                                    *\r\n");
   printf("*  CloseEndpoint                                                    *\r\n");
   printf("*  StartEndpoint                                                    *\r\n");
   printf("*  SuspendEndpoint                                                  *\r\n");
   printf("*  DataWrite                                                        *\r\n");
   printf("*  SecurityDataWrite                                                *\r\n");
   ...
   /* The following function displays the GAVD Server Command Menu.     */
static void DisplayGAVDServerMenu(void)
{
   /* First display the upper command bar.                              */
   printf("************************** Command Options **************************\r\n");
   ...
   printf("*  GetRemoteName [Inquiry Index]                                    *\r\n");
   printf("*  ServiceDiscovery [Inquiry Index] [Profile Index] [UUID (Manual)] *\r\n");
   printf("*  RegisterEndpoint [Source/Sink Flag, Source = 0, Sink = 1]        *\r\n");
   printf("*  UnRegisterEndpoint                                               *\r\n");
   printf("*  CloseEndpoint                                                    *\r\n");
   printf("*  StartEndpoint                                                    *\r\n");
   printf("*  SuspendEndpoint                                                  *\r\n");
   printf("*  DataWrite                                                        *\r\n");
   ...
   /* available for the GAVD Client.                                    */
static void PopulateGAVDClientCommandTable(void)
{
   /* First Clear all of the commands in the Command Table.             */
   ClearCommands();

   /* Now add all of the commands that are associated with the GAVD     */
...
static void UserInterface_Main(void)
{
   ...
   /* Determine if we are currently running in GAVD Server or GAVD      */
   /* Client Mode.                                                      */
   if(IsClient)
   {
      /* We are currently running in GAVD Client Mode, add the          */
      /* appropriate commands to the command table and display the menu */
      /* options.                                                       */
      PopulateGAVDClientCommandTable();

      DisplayGAVDClientMenu();
   }
   else
   {
      /* We are not currently running in GAVD Client Mode therefore we  */
      /* must be running in GAVD Server Mode.  Add the appropriate      */
      /* commands to teh command table and display the menu options.    */
      PopulateGAVDServerCommandTable();

      DisplayGAVDServerMenu();
   }
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   /* First check to if Currently in GAVD Client or GAVD Server Mode.   */
   if(IsClient)
   {
      /* Currently in GAVD Client Mode, display the GAVD Client Menu.   */
      DisplayGAVDClientMenu();
   }
   else
   {
      /* Currently in GAVD Server Mode, display the GAVD Server Menu.   */
      DisplayGAVDServerMenu();
   }
...
static int EnableDebug(ParameterList_t *TempParam)
{
   ...
   /* state of the Main Application to be a GAVD Client.  This function */
...
static int InitializeGAVDClient(void)
{
   ...
   /* The program is currently running in GAVD Client Mode.  Ready the  */
   ...
   /* The following function is responsible for setting the initial     */
   /* state of the Main Application to be a GAVD Server.  This function */
   /* returns zero on successful execution and a negative value on all  */
   /* errors.                                                           */
static int InitializeGAVDServer(void)
{
   ...
   /* The program is currently running in GAVD Server Mode.  Ready the  */
   /* application to support a GAVD Server.  First, attempt to set the  */
   ...
   if(!BluetoothStackID)
   {
      ...
      if(HCI_DriverInformation)
      {
         ...
         else
         {
            ...
            /* Flag an error to the caller (and flag that the Bluetooth */
...
static int CloseStack(void)
{
   ...
   if(BluetoothStackID)
   {
      /* First, close down the GAVD Profile.                            */
...
static int ConnectRemoteServer(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      /* connection to a remote Stream Endpoint Manager (and that we are*/
      ...
      else
      {
         ...
         if(GAVDID)
            ...
            printf("GAVD_Connect: Not in GAVD Client Mode.\r\n");
   ...
   if(BluetoothStackID)
   {
      ...
      else
      {
         ...
         if(!GAVDID)
            ...
            printf("GAVD_Discover_End_Points: Not in GAVD Client Mode.\r\n");
   ...
   if(BluetoothStackID)
   {
      ...
      else
      {
         ...
         if(!GAVDID)
            ...
            printf("GAVD_Get_End_Point_Capabilities: Not in GAVD Client Mode.\r\n");
   ...
   if(BluetoothStackID)
   {
      ...
      else
      {
         ...
         if(!GAVDID)
            ...
            printf("GAVD_Disconnect: Not in GAVD Client Mode.\r\n");
...
static int OpenRemoteEndpoint(ParameterList_t *TempParam)
{
   int                              ret_val;
   int                              Result;
   Byte_t                           SpecInfo[4] = { 0x21, 0x15, 0x0A, 0x23 };
   BD_ADDR_t                        NullADDR;
   ...
   if(BluetoothStackID)
   {
      ...
      /* Stream Endpoint connection at a time) and that we are operating*/
      /* in Client Mode.                                                */
      if((!LSEID) && (IsClient))
      {
         /* Make sure that all of the parameters required for this      */
         ...
         if((TempParam) && (TempParam->NumberofParameters > 1) && (TempParam->Params[0].intParam) && (NumberofValidResponses) && (TempParam->Params[0].intParam <= NumberofValidResponses) && (!COMPARE_BD_ADDR(InquiryResultList[(TempParam->Params[0].intParam - 1)], NullADDR)) && (TempParam->Params[1].intParam))
         {
            ...
            /* * NOTE * For demonstration purposes we will simply set   */
            /*          the configuration to our supported Capabilities.*/
            Result = GAVD_Open_Remote_End_Point(BluetoothStackID, InquiryResultList[(TempParam->Params[0].intParam - 1)], TempParam->Params[1].intParam, &EndPointInfo, 2, Capability, GAVD_Event_Callback, 0);
         ...
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: OpenRemoteEndpoint [Inquiry Index] [RSEID].\r\n");
   ...
   if(BluetoothStackID)
   {
      ...
      else
      {
         ...
         if(!LSEID)
            ...
            printf("GAVD_Abort_Stream_Request: Not in GAVD Client Mode.\r\n");
...
static int RegisterEndpoint(ParameterList_t *TempParam)
{
   ...
   {
      /* Next, check to make sure that there is not already an active   */
      /* Local Stream Endpoint (and that we are operating in GAVD Server*/
      /* Mode).                                                         */
      if((!LSEID) && (!IsClient))
      {
         ...
         /* * NOTE * The Capabilities that we are creating are for the  */
         /*          A2DP Profile.  We are hardcoding the Media Codec   */
         ...
         if(Result > 0)
         {
            ...
            {
               /* The end point was successfully registered and we were */
               /* able to add a SDP Record to the SDP Database.  Note   */
               /* the LSEID and the SDP Record Handle.                  */
               LSEID             = Result;
               EndpointSDPHandle = RecordHandle;

               /* The end point was successfully registered.  The return*/
               /* value of the call is the LSEID and is required for    */
               /* future GAVD calls to this end point.                  */
         ...
         {
            /* There was an error attempting to register the Endpoint.  */
      ...
      else
      {
         ...
         if(LSEID)
            ...
            printf("GAVD_Register_End_Point: Not in GAVD Server Mode.\r\n");
...
static int UnRegisterEndpoint(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      /* Endpoint (and that we are operating in GAVD Server Mode).      */
      if((LSEID) && (!IsClient))
      {
         /* Try to un-register the end point.                           */
         Result = GAVD_Un_Register_End_Point(BluetoothStackID, LSEID);
      ...
      else
      {
         ...
         if(!LSEID)
            ...
            printf("GAVD_Un_Register_End_Point: Not in GAVD Server Mode.\r\n");
   ...
   if((BluetoothStackID) && (GAVDEventData != NULL))
   {
      ...
      switch(GAVDEventData->Event_Data_Type)
      {
         ...
         case etGAVD_Disconnect_Indication:
            ...
            /* Flag that the connection to the remote Stream Endpoint   */
            /* Manager is no longer valid if we are operating in Client */
            /* Mode.                                                    */
            if(IsClient)
               GAVDID = 0;
            break;
         ...
         case etGAVD_Open_End_Point_Confirmation:
            ...
            /* user, and remove the LSEID from the LSEID List if this is*/
            /* a Client LSEID AND the End Point was not successfully    */
            /* opened.                                                  */
         ...
         case etGAVD_Close_End_Point_Indication:
            ...
            /* Flag that the LSEID is no longer valid if we are         */
            /* operating in Client Mode.                                */
            if(IsClient)
               LSEID = 0;
            break;
         ...
         case etGAVD_Data_Indication:
            ...
            printf("GAVD Data Indication: ");

            printf("LSEID: %d, Seq. #: %d\r\n", GAVDEventData->Event_Data.GAVD_Data_Indication_Data->LSEID, GAVDEventData->Event_Data.GAVD_Data_Indication_Data->SequenceNumber);

            for(Index=0;Index<GAVDEventData->Event_Data.GAVD_Data_Indication_Data->DataLength;Index++)
               printf("0x%02X ", GAVDEventData->Event_Data.GAVD_Data_Indication_Data->DataBuffer[Index]);
   ...
   if(argc >= NUM_EXPECTED_PARAMETERS_USB)
   {
      ...
      if(HCI_DriverInformationPtr)
      {
         ...
         if(!OpenStack(HCI_DriverInformationPtr))
         {
            ...
            if(IsClient)
               ret_val = InitializeGAVDClient();
            else
               ret_val = InitializeGAVDServer();

            /* The GAVD Server or GAVD Client has been initialized, now */

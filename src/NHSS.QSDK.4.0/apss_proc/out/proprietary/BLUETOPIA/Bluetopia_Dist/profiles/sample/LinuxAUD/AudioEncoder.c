...
static int CmpTimespec(const struct timespec *TS1, const struct timespec *TS2)
{
   ...
   /* error.                                                            */
int InitializeAudioEncoder(unsigned int BluetoothStackID, BD_ADDR_t BD_ADDR, unsigned int SamplesPerSecond, unsigned int NumberOfChannels)
{
   int                                           Result;
   int                                           ret_val = -1;
   unsigned int                                  MTU;
   ...
   if((Result = AUD_Query_Stream_Configuration(BluetoothStackID, BD_ADDR, astSRC, &StreamConfig)) == 0)
   {
...
int SendAudioData(BD_ADDR_t BD_ADDR, void *Buffer, unsigned int Length)
{
   int                          Result;
   ...
   if((Buffer != NULL) && (Length >= AUDIO_ENCODER_MIN_BUFFER_SIZE) && ((Length % 4) == 0))
   {
      ...
      while((RemainingAudioFrames > 0) && (Result >= 0))
      {
         ...
         while((PlaybackContext.SBCFrameCount >= PlaybackContext.SBCFramesPerPacket) || ((BufferRemaining == 0) && (PlaybackContext.SBCFrameCount > 0)))
         {
            ...
            /* Set the AVDTP Header Information structure.  Since we're */
            /* not fragmenting BC frames, all we care about is the      */
            /* number of SBC frames in the buffer.                      */
            ...
            Result = AUD_Send_Encoded_Audio_Data(PlaybackContext.BluetoothStackID, BD_ADDR, ((PlaybackContext.SBCFrameCount * PlaybackContext.SBCFrameSize) + 1), &(PlaybackContext.SBCFrameBuffer[1]));
            ...
            if(Result == 0)
            {
               ...
               if(PlaybackContext.ExtraTimeCount >= PlaybackContext.UnderrunTime)
               {
                  ...
                  /* threshold, so the next packet needs to be sent     */
            ...
            else
            {
               ...
               printf("Playback Thread aborting for error sending packet: %d", Result);

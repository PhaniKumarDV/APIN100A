...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXOBP - Simple Linux application using OBEX Object Push Profile.       */
/*                                                                            */
/*  Author:  Damon Lange                                                      */
/*                                                                            */
...
#include "LinuxOBP.h"      /* Main Application Prototypes and Constants.      */

#include "SS1BTPS.h"       /* Includes for the SS1 Bluetooth Protocol Stack.  */
#include "SS1BTOBP.h"      /* Includes for the SS1 Object Push Profile.       */
#include "SS1BTDBG.h"      /* Includes/Constants for Bluetooth Debugging.     */

#define LOCAL_NAME_ROOT                      "LinuxOBP"  /* Root of the local */
                                                         /* name              */
...
const char *ObjectTypeName[] =                           /* Holds the human   */
{                                                        /* readable Object   */
   "vCard",                                              /* Type names of all */
   "vCalendar",                                          /* of the Objects    */
   "iCalendar",                                          /* that are supported*/
   "vNote",                                              /* by this program.  */
   "vMessage",
   "UnknownObject"
} ;
...
#define DEFAULT_DEBUG_LOG_FILE_NAME  "LinuxOBP_ASC.log"
#define DEFAULT_DEBUG_FTS_FILE_NAME  "LinuxOBP_FTS.log"
...
static unsigned int        OBJPServerID;            /* Variable used to hold the       */
                                                    /* Object Push Server ID of the    */
                                                    /* currently opened Object Push    */
                                                    /* Server.                         */

static DWord_t             OBJPServerSDPHandle;     /* Variable used to hold the Object*/
                                                    /* Push Service Record of the      */
                                                    /* Object Push Server SDP Service  */
                                                    /* Record.                         */

static unsigned int        OBJPClientID;            /* Variable used to hold the Object*/
                                                    /* Push Client ID of the currently */
                                                    /* opened Remote Object Push Server*/
                                                    /* (or Local Object Push  Client). */
...
static int GetDefaultObject(ParameterList_t *TempParam);
static int PutvCARDObject(ParameterList_t *TempParam);
...
static void BTPSAPI OBJP_Server_Event_Callback(unsigned int BluetoothStackID, OBJP_Server_Event_Data_t *OBJP_Event_Data, unsigned long CallbackParameter);
static void BTPSAPI OBJP_Client_Event_Callback(unsigned int BluetoothStackID, OBJP_Client_Event_Data_t *OBJP_Event_Data, unsigned long CallbackParameter);
...
static void UserInterface_Client(void)
{
   ...
   AddCommand("GETOBJECT", GetDefaultObject);
   AddCommand("PUTVCARD", PutvCARDObject);
   ...
   while(Result != EXIT_CODE)
   {
            ...
            if(CommandParser(&TempCommand, UserInput) >= 0)
            {
               ...
               switch(Result)
               {
                  ...
                  case EXIT_CODE:
                     ...
                     /* go ahead an close any Remote Object Push Server */
                     /* that we have open.                              */
                     if(OBJPClientID)
      ...
      {
         /* Close any Remote Object Push Server that we have open.      */
         if(OBJPClientID)
            CloseRemoteServer(NULL);
...
static int DeleteLinkKey(BD_ADDR_t BD_ADDR)
{
   ...
   /* Command Options for either Object Push Client for Object Push     */
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   if(IsClient)
   {
      ...
      printf("*                  GetObject, PutvCARD, Abort,                   *\r\n");
...
static int ServiceDiscovery(ParameterList_t *TempParam)
{
   ...
   /* Server on the Local Device.  This function opens an Object Push   */
   /* Server in either the specified directory or the Current Directory */
   /* of the Local Machine.  This function accepts the RFCOMM Server    */
   /* Channel to open the server on, as well as the Default Object.     */
...
static int OpenServer(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(!IsClient)
      {
         /* Next, let's make sure that there is not an Object Push      */
         /* Server already open.                                        */
         if(!OBJPServerID)
         {
            /* Let's check to see if a Remote RFCOMM Port has been      */
            /* specified.                                               */
            if((TempParam->NumberofParameters > 1) && (TempParam->Params[0].intParam) && ((TempParam->Params[0].intParam >= SPP_PORT_NUMBER_MINIMUM) && (TempParam->Params[0].intParam <= SPP_PORT_NUMBER_MAXIMUM)) && (TempParam->Params[1].strParam) && (strlen(TempParam->Params[1].strParam)))
            {
               /* RFCOMM Port appears to be valid, so now let's check to*/
               /* see if an optional Inbox directory was specified.     */
               if((TempParam->NumberofParameters > 2) && (TempParam->Params[2].strParam) && (strlen(TempParam->Params[2].strParam)))
                  strcpy(InboxPath, TempParam->Params[2].strParam);
               else
               {
                  /* Let's figure out the current working directory of  */
                  /* the program so we can use that for our Inbox       */
                  /* Directory.                                         */
                  if(!getcwd(InboxPath, sizeof(InboxPath)))
                  {
                     /* There was an error fetching the Directory, so   */
                     /* let's just default it to the Inbox Directory.   */
                     InboxPath[0] = '/';
                  }
               }

               /* Simply attempt to open an Object Push Server, on      */
               /* RFCOMM Server Port 1, in the current directory, with  */
               /* all the permissions.                                  */
               ret_val = OBJP_Open_Server(BluetoothStackID, TempParam->Params[0].intParam, InboxPath, TempParam->Params[1].strParam, OBJP_ALL_OBJECTS_SUPPORTED_BIT, OBJP_ACCEPT_ALL_REQUESTS_BIT_MASK, OBJP_Server_Event_Callback, (unsigned long)0);

               /* If the Open was successful, then note the Object Push */
               /* Server ID.                                            */
               if(ret_val > 0)
               {
                  /* Note the Object Push Server ID of the opened Object*/
                  /* Push Server.                                       */
                  OBJPServerID = ret_val;

                  /* The Server was opened successfully, now register a */
                  /* SDP Record indicating that an Object Push Server   */
                  /* exists.  Do this by first creating a Service Name. */
                  sprintf(ServiceName, "Object Push Server Port %u", TempParam->Params[0].intParam);

                  /* Now that a Service Name has been created try to    */
                  /* Register the SDP Record.                           */
                  ret_val = OBJP_Register_Server_SDP_Record(BluetoothStackID, OBJPServerID, ServiceName, &OBJPServerSDPHandle);

                  /* If there was an error creating the Object Push     */
                  /* Server's SDP Service Record then go ahead an close */
                  /* down the server an flag an error.                  */
                  if(ret_val < 0)
                  {
                     printf("Unable to Register Object Push Server SDP Record, Error = %d.\r\n", ret_val);

                     OBJP_Close_Server(BluetoothStackID, OBJPServerID);

                     /* Flag that there is no longer an Object Push     */
                     /* Server Open.                                    */
                     OBJPServerID = 0;
                  ...
                  else
                  {
                     ...
                     printf("Object push Server Opened (Channel = %u, Inbox Directory = %s, Default Object = %s).\r\n", TempParam->Params[0].intParam, InboxPath, TempParam->Params[1].strParam);
               ...
               {
                  printf("Unable to Open Object Push Server, Error = %d.\r\n", ret_val);
            ...
            {
               printf("Usage: Open [RFCOMM Server Port] [Default Object Filename] [Inbox Directory Path (optional)].\r\n");
         ...
         {
            printf("Unable to open Object Push Server.\r\nObject Push Server is already open.\r\n");
   ...
   /* The following function is responsible for closing an Object Push  */
...
static int CloseServer(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      /* If an Object Push Server is already opened, then simply close  */
      /* it.                                                            */
      if(OBJPServerID)
      {
         /* If there is an SDP Service Record associated with the Object*/
         /* Push Server then we need to remove it from the SDP Database.*/
         if(OBJPServerSDPHandle)
         {
            OBJP_Un_Register_SDP_Record(BluetoothStackID, OBJPServerID, OBJPServerSDPHandle);

            /* Flag that there is no longer an SDP Object Push Server   */
            /* Record.                                                  */
            OBJPServerSDPHandle = 0;
         }

         /* Finally close the Object Push Server.                       */
         ret_val = OBJP_Close_Server(BluetoothStackID, OBJPServerID);

         if(ret_val < 0)
         {
            printf("OBJP_Close_Server(): Error %d\r\n", ret_val);
         ...
         /* Flag that there is no Object Push Server currently open.    */
         OBJPServerID = 0;

         /* Flag that we are not connected.                             */
         Connected    = FALSE;

         printf("Object Push Server Closed.\r\n");
      }
      else
      {
         printf("There is NO Object Push Server currently open.\r\n");
   ...
   if(BluetoothStackID)
   {
      ...
      if(IsClient)
      {
         ...
         if(!OBJPClientID)
         {
            ...
            {
               /* Now let's attempt to open the Remote Object Push      */
               /* Server.                                               */
               Result = OBJP_Open_Remote_Object_Server(BluetoothStackID, InquiryResultList[(TempParam->Params[0].intParam - 1)], TempParam->Params[1].intParam, OBJP_Client_Event_Callback, 0);

               if(Result > 0)
               {
                  /* Inform the user that the call to open the Remote   */
                  /* Object Push Server was successful.                 */
                  printf("OBJP_Open_Remote_Object_Server() was successful.\r\n");

                  /* Note the Object Push Client ID.                    */
                  OBJPClientID = Result;
               ...
               {
                  /* Inform the user that the call to Open the Remote   */
                  /* Object Push Server failed.                         */
                  printf("Error calling OBJP_Open_Remote_Object_Server(): %d.\r\n", Result);
         ...
         {
            printf("Unable to open Object Push Client.\r\nObject Push Client is already open.\r\n");
...
static int CloseRemoteServer(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(IsClient)
      {
         /* Next, let's make sure that a Remote Object Push Server is   */
         /* indeed Open.                                                */
         if(OBJPClientID)
         {
            printf("Object Push Client closed successfully.\r\n");

            /* Simply close the Object Push Client.                     */
            OBJP_Close_Client(BluetoothStackID, OBJPClientID);

            /* Flag that there is no longer an Object Push Client       */
            /* connected.                                               */
            OBJPClientID = 0;
         ...
         else
         {
            ...
            /* Object Push Client is open.                              */
            printf("You must open an Object Push Client before you can close an Object Push Client.\r\n");
   ...
   /* Object from a Remote Object Push Server.  This function returns   */
   /* zero if successful and a negative value if an error occurred.     */
static int GetDefaultObject(ParameterList_t *TempParam)
{
   int ret_val;
   ...
   if(BluetoothStackID)
   {
      ...
      if(IsClient)
      {
         ...
         if(OBJPClientID)
         {
            /* Next, let's make sure that a local file name was         */
            ...
            {
               Result = OBJP_Get_Default_Object(BluetoothStackID, OBJPClientID, NULL, TempParam->Params[0].strParam);

               if(Result >= 0)
               {
                  /* Inform the user that the Abort was issued          */
                  /* successfully.                                      */
                  printf("OBJP_Get_Default_Object() issued successfully.\r\n");
               ...
               else
               {
                  ...
                  /* the Abort Request.                                 */
                  printf("Error calling OBJP_Get_Default_Object(): %d.\r\n", Result);
            ...
            else
            {
               ...
               printf("Usage: GetObject [Local File].\r\n");
         ...
         else
         {
            /* Display an error to the user informing them that no      */
            /* Object Push Client is open.                              */
            printf("You must open an Object Push Client before you can retrieve the Default Object.\r\n");
   ...
   /* The following function is responsible for putting a vCARD Object  */
   /* on the Remote Object Push Server.  This function returns zero if  */
   /* successful and a negative value if an error occurred.             */
static int PutvCARDObject(ParameterList_t *TempParam)
{
   int ret_val;
      ...
      if(IsClient)
      {
         ...
         if(OBJPClientID)
         {
            ...
            if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].strParam) && (strlen(TempParam->Params[0].strParam)))
            {
               ...
               /* put the Object to the Remote Device with a different  */
               /* name.                                                 */
               if(TempParam->NumberofParameters > 1)
               {
                  ...
                  if((TempParam->Params[1].strParam) && (strlen(TempParam->Params[1].strParam)))
                  {
                     ...
                     /* with the specified Remote Name.                 */
                     Result = OBJP_Put_Object(BluetoothStackID, OBJPClientID, obtvCard, NULL, TempParam->Params[0].strParam, TempParam->Params[1].strParam);

                     /* Inform the user of the result of the Put Object */
                     /* Command that was issued.                        */
                     if(Result >= 0)
                     {
                        printf("OBJP_Put_Object() successful.\r\n");

                        ret_val = 0;
                     }
                     else
                     {
                        printf("OBJP_Put_Object() unsuccessful: %d.\r\n", Result);
                  ...
                  {
                     /* Invalid parameters, so inform the user.         */
                     printf("Usage: PutvCARD [Local File] [Remote Name].\r\n");
               ...
               {
                  /* The user has requested to put an Object.  Simply   */
                  /* issue the Put Object Command with the File Name.   */

                  /* Since the user did not specify a Name for the      */
                  ...
                  Result = OBJP_Put_Object(BluetoothStackID, OBJPClientID, obtvCard, NULL, TempParam->Params[0].strParam, &(TempParam->Params[0].strParam[Index]));
   ...
   /* Object Push Action on a Remote Object Push Server.  This function */
...
static int Abort(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(IsClient)
      {
         ...
         if(OBJPClientID)
         {
            /* Simply issue the Object Push Abort Request Command.      */
            Result = OBJP_Abort(BluetoothStackID, OBJPClientID);

            if(Result >= 0)
            {
               /* Inform the user that the Abort was issued             */
               /* successfully.                                         */
               printf("OBJP_Abort() issued successfully.\r\n");
            ...
            else
            {
               /* Inform the user that there was an error submitting    */
               /* the Abort Request.                                    */
               printf("Error calling OBJP_Abort().\r\n");
         ...
         else
         {
            ...
            /* because there is no Object Push Client currently open.   */
            printf("You must open an Object Push Client before issuing this command.\r\n");
...
static void BTPSAPI SDP_Event_Callback(unsigned int BluetoothStackID, unsigned int SDPRequestID, SDP_Response_Data_t *SDP_Response_Data, unsigned long CallbackParameter)
{
   ...
   /* The following function is for an Object Push Server Event         */
   /* Callback.  This function will be called whenever an Object Push   */
   /* Server Event occurs that is associated with the Bluetooth Stack.  */
   /* This function passes to the caller the Object Push Event Data that*/
   /* occurred and the Object Push Event Callback Parameter that was    */
   /* specified when this Callback was installed.  The caller is free to*/
   /* use the contents of the Object Push Object Push Event Data ONLY in*/
   ...
   /* User does NOT own.  Therefore, processing in this function should */
   /* be as efficient as possible (this argument holds anyway because   */
   /* another Object Push Event will not be processed while this        */
   /* function call is outstanding).                                    */
   /* * NOTE * This function MUST NOT Block and wait for Events that    */
   /*          can only be satisfied by Receiving Object Push Event     */
   /*          Packets.  A Deadlock WILL occur because NO Object Push   */
...
static void BTPSAPI OBJP_Server_Event_Callback(unsigned int BluetoothStackID, OBJP_Server_Event_Data_t *OBJP_Event_Data, unsigned long CallbackParameter)
{
   ...
   if((BluetoothStackID) && (OBJP_Event_Data))
   {
      switch(OBJP_Event_Data->Event_Data_Type)
      {
         case etOBJP_Server_Connect_Indication:
            /* A Client has connected to the Server, display the BD_ADDR*/
            /* of the Connecting Device.                                */
            printf("\r\n");
            printf("etOBJP_Server_Connect_Indication, OBJPID: 0x%04X\r\n", OBJP_Event_Data->Event_Data.OBJP_Server_Connect_Indication_Data->OBJPID);
            BD_ADDRToStr(OBJP_Event_Data->Event_Data.OBJP_Server_Connect_Indication_Data->BD_ADDR, BoardStr);
            ...
            Connected = TRUE;
            break;
         case etOBJP_Server_Disconnect_Indication:
            /* A Disconnect Indication was received.  Display the       */
            /* appropriate message.                                     */
            printf("\r\n");
            printf("etOBJP_Server_Disconnect_Indication, OBJPID: 0x%04X.", OBJP_Event_Data->Event_Data.OBJP_Server_Disconnect_Indication_Data->OBJPID);
            printf("\r\nServer>");

            /* Flag that we are no longer connected.                    */
            Connected = FALSE;
            break;
         case etOBJP_Server_Object_Put_Indication:
            /* The Client requested the transfer of an Object to the    */
            /* Server.  Display all relevant information.               */
            printf("\r\n");
            printf("etOBJP_Server_Object_Put_Indication, OBJPID: 0x%04X\r\n", OBJP_Event_Data->Event_Data.OBJP_Server_Object_Put_Indication_Data->OBJPID);
            printf("   Transfer Complete: %s, Total Length: 0x%08X, Transferred Length: 0x%08X\r\n", (OBJP_Event_Data->Event_Data.OBJP_Server_Object_Put_Indication_Data->TransferComplete?"TRUE":"FALSE"),
                                                                                                     OBJP_Event_Data->Event_Data.OBJP_Server_Object_Put_Indication_Data->TotalLength,
                                                                                                     OBJP_Event_Data->Event_Data.OBJP_Server_Object_Put_Indication_Data->TransferredLength);
            printf("   Object File Name: %s\r\n", OBJP_Event_Data->Event_Data.OBJP_Server_Object_Put_Indication_Data->ObjectFileName);
            printf("   Object Type: %s\r\n", ObjectTypeName[OBJP_Event_Data->Event_Data.OBJP_Server_Object_Put_Indication_Data->ObjectType]);
            printf("Server>");
            break;
         case etOBJP_Server_Object_Get_Indication:
            /* The Client requested the transfer of an Object from the  */
            /* Server.  Display all relevant information.               */
            printf("\r\n");
            printf("etOBJP_Server_File_Get_Indication, OBJPID: 0x%04X\r\n", OBJP_Event_Data->Event_Data.OBJP_Server_Object_Get_Indication_Data->OBJPID);
            printf("   Transfer Complete: %s, Total Length: 0x%08X, Transferred Length: 0x%08X\r\n", (OBJP_Event_Data->Event_Data.OBJP_Server_Object_Get_Indication_Data->TransferComplete?"TRUE":"FALSE"),
                                                                                                     OBJP_Event_Data->Event_Data.OBJP_Server_Object_Get_Indication_Data->TotalLength,
                                                                                                     OBJP_Event_Data->Event_Data.OBJP_Server_Object_Get_Indication_Data->TransferredLength);
            printf("   Object File Name: %s\r\n", OBJP_Event_Data->Event_Data.OBJP_Server_Object_Get_Indication_Data->ObjectFileName);
            printf("   Object Type: %s\r\n", ObjectTypeName[OBJP_Event_Data->Event_Data.OBJP_Server_Object_Get_Indication_Data->ObjectType]);
            printf("Server>");
            break;
         default:
            /* Unknown Object Push Server Event Received.               */
            printf("\r\n");
            printf("Unknown Object Push Server Event Received.");
            printf("\r\nServer>");
            break;
      }
   ...
   /* The following function is for an Object Push Client Event         */
   /* Callback.  This function will be called whenever a Object Push    */
   /* Client Event occurs that is associated with the Bluetooth Stack.  */
   /* This function passes to the caller the Object Push Event Data that*/
   /* occurred and the Object Push Event Callback Parameter that was    */
   /* specified when this Callback was installed.  The caller is free to*/
   /* use the contents of the Object Push Object Push Event Data ONLY in*/
...
static void BTPSAPI OBJP_Client_Event_Callback(unsigned int BluetoothStackID, OBJP_Client_Event_Data_t *OBJP_Event_Data, unsigned long CallbackParameter)
{
   ...
   if((BluetoothStackID) && (OBJP_Event_Data))
   {
      switch(OBJP_Event_Data->Event_Data_Type)
      {
         case etOBJP_Client_Connect_Confirmation:
            /* The Client received a Connect Confirmation, display      */
            /* relevant information.                                    */
            printf("\r\n");
            printf("etOBJP_Client_Connect_Confirmation, OBJPID: 0x%04X, Status 0x%04X\r\n", OBJP_Event_Data->Event_Data.OBJP_Client_Connect_Confirmation_Data->OBJPID,
                                                                                            OBJP_Event_Data->Event_Data.OBJP_Client_Connect_Confirmation_Data->OBJPConnectStatus);
            BD_ADDRToStr(OBJP_Event_Data->Event_Data.OBJP_Client_Connect_Confirmation_Data->BD_ADDR, BoardStr);
            printf("BD_ADDR: %s", BoardStr);

            /* Check to see if the Client Port was opened successfully. */
            if(OBJP_Event_Data->Event_Data.OBJP_Client_Connect_Confirmation_Data->OBJPConnectStatus)
            {
               /* There was an error while trying to open a Client Port.*/
               /* invalidate the OBJP Client ID set during the call to  */
               /* the OpenRemoteServer() function.                      */
               OBJPClientID = 0;
            ...
            printf("\r\nClient>");
            break;
         case etOBJP_Client_Disconnect_Indication:
            /* The Client was disconnected, display the appropriate     */
            /* information and clean up the information relevant to this*/
            /* Connection.                                              */
            printf("\r\n");
            printf("etOBJP_Client_Disconnect_Indication, OBJPID: 0x%04X.", OBJP_Event_Data->Event_Data.OBJP_Client_Disconnect_Indication_Data->OBJPID);
            OBJPClientID = 0;
            printf("\r\nClient>");
            ...
            Connected = FALSE;
            break;
         case etOBJP_Client_Abort_Confirmation:
            /* An Abort was executed.  Display any relevant information.*/
            printf("\r\n");
            printf("etOBJP_Client_Abort_Confirmation, OBJPID: 0x%04X.", OBJP_Event_Data->Event_Data.OBJP_Client_Abort_Confirmation_Data->OBJPID);
            printf("\r\nClient>");
            break;
         case etOBJP_Client_Object_Put_Confirmation:
            /* A Put Object Confirmation was received, display any      */
            /* relevant information.                                    */
            printf("\r\n");
            printf("etOBJP_Client_Object_Put_Confirmation, OBJPID: 0x%04X, Success: %s", OBJP_Event_Data->Event_Data.OBJP_Client_Object_Put_Confirmation_Data->OBJPID,
                                                                                         OBJP_Event_Data->Event_Data.OBJP_Client_Object_Put_Confirmation_Data->Success?"TRUE":"FALSE");
            printf("   Transfer Complete: %s, Total Length: 0x%08X, Transferred Length: 0x%08X\r\n", (OBJP_Event_Data->Event_Data.OBJP_Client_Object_Put_Confirmation_Data->TransferComplete?"TRUE":"FALSE"),
                                                                                                     OBJP_Event_Data->Event_Data.OBJP_Client_Object_Put_Confirmation_Data->TotalLength,
                                                                                                     OBJP_Event_Data->Event_Data.OBJP_Client_Object_Put_Confirmation_Data->TransferredLength);
            printf("   Object File Name: %s\r\n", OBJP_Event_Data->Event_Data.OBJP_Client_Object_Put_Confirmation_Data->ObjectFileName);
            printf("   Object Type: %s\r\n", ObjectTypeName[OBJP_Event_Data->Event_Data.OBJP_Client_Object_Put_Confirmation_Data->ObjectType]);
            printf("Client>");
            break;
         case etOBJP_Client_Object_Get_Confirmation:
            /* A Get Object Confirmation was received, display any      */
            /* relevant information.                                    */
            printf("\r\n");
            printf("etOBJP_Client_Object_Get_Confirmation, OBJPID: 0x%04X, Success: %s", OBJP_Event_Data->Event_Data.OBJP_Client_Object_Get_Confirmation_Data->OBJPID,
                                                                                         OBJP_Event_Data->Event_Data.OBJP_Client_Object_Get_Confirmation_Data->Success?"TRUE":"FALSE");
            printf("   Transfer Complete: %s, Transferred Length: 0x%08X\r\n", (OBJP_Event_Data->Event_Data.OBJP_Client_Object_Get_Confirmation_Data->TransferComplete?"TRUE":"FALSE"),
                                                                               OBJP_Event_Data->Event_Data.OBJP_Client_Object_Get_Confirmation_Data->TransferredLength);
            printf("   Object File Name: %s\r\n", OBJP_Event_Data->Event_Data.OBJP_Client_Object_Get_Confirmation_Data->ObjectFileName);
            printf("   Object Type: %s\r\n", ObjectTypeName[OBJP_Event_Data->Event_Data.OBJP_Client_Object_Get_Confirmation_Data->ObjectType]);
            printf("Client>");
            break;
         default:
            /* Unknown Object Push Client Event Received.               */
            printf("\r\n");
            printf("Unknown Object Push Client Event Received.");
            printf("\r\nClient>");
            break;
      }

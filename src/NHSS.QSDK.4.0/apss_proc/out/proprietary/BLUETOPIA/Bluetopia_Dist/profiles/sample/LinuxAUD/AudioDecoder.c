...
static void *PlaybackThread(void *ThreadParameter)
{
   ...
   /* parameter is a valid Bluetooth Stack ID followed by optional      */
   /* configuration flags. This function will return zero on success and*/
   /* negative on error.                                                */
int InitializeAudioDecoder(unsigned int BluetoothStackID, unsigned int ConfigFlags, BD_ADDR_t BD_ADDR, const char *WaveFileName)
{
   int Result = -1;
   ...
   if(!Initialized)
   {
      ...
      {
         if((Result = AUD_Query_Stream_Configuration(BluetoothStackID, BD_ADDR, astSNK, &PlaybackContext.StreamConfig)) == 0)
         {

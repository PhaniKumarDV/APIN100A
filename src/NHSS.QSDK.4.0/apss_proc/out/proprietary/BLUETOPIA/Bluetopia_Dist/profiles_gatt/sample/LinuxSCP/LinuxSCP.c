...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXSCP - Linux Bluetooth Scan Parameters Profile using GATT (LE)        */
/*             application.                                                   */
/*                                                                            */
/*  Author:  Cliff Xu                                                         */
/*                                                                            */
...
#include "LinuxSCP.h"            /* Application Header.                       */
...
#include "SS1BTSCP.h"            /* Main SS1 SCPS Service Header.             */
...
#define DEFAULT_DEBUG_LOG_FILE_NAME                "LinuxSCP_ASC.log"
#define DEFAULT_DEBUG_FTS_FILE_NAME                "LinuxSCP_FTS.log"

   /* Determine the Name we will use for this compilation.              */
#define LE_DEMO_DEVICE_NAME                        "LinuxSCP"
...
typedef struct _tagDeviceInfo_t
{
   ...
   SCPS_Client_Information_t  ClientInfo;
   SCPS_Server_Information_t  ServerInfo;
...
static unsigned int        SCPSInstanceID;          /* The following holds the SCPS    */
                                                    /* Instance ID that is returned    */
                                                    /* from SCPS_Initialize_Service(). */
...
static int SetGATTMTU(ParameterList_t *TempParam);
   ...
   /* Scan Parameters Profile Function Commands                         */
static int RegisterSCPS(ParameterList_t *TempParam);
static int UnregisterSCPS(ParameterList_t *TempParam);
static int DiscoverSCPS(ParameterList_t *TempParam);
static int ConfigureRemoteSCPS(ParameterList_t *TempParam);
static int SetScanIntervalWindow(ParameterList_t *TempParam);
static int NotifyScanRefresh(ParameterList_t *TempParam);

   /* Scan Parameters Helper Functions                                  */
static void SCPSPopulateHandles(DeviceInfo_t *DeviceInfo, GATT_Service_Discovery_Indication_Data_t *ServiceDiscoveryData);
...
static void BTPSAPI SCPS_EventCallback(unsigned int BluetoothStackID, SCPS_Event_Data_t *SCPS_Event_Data, unsigned long CallbackParameter);
static void BTPSAPI GATT_ClientEventCallback_SCPP(unsigned int BluetoothStackID, GATT_Client_Event_Data_t *GATT_Client_Event_Data, unsigned long CallbackParameter);
...
static void UserInterface(void)
{
   ...
   AddCommand("REGISTERSCPS",   RegisterSCPS);
   AddCommand("UNREGISTERSCPS", UnregisterSCPS);
   AddCommand("DISCOVERSCPS", DiscoverSCPS);
   AddCommand("CONFIGUREREMOTESCPS",ConfigureRemoteSCPS);
   AddCommand("SETSCANINTERVALWINDOW",SetScanIntervalWindow);
   AddCommand("NOTIFYSCANREFRESH",NotifyScanRefresh);
...
static int CloseStack(void)
{
   ...
   if(BluetoothStackID)
   {
      ...
      /* Cleanup SCPP Service.                                          */
      if(SCPSInstanceID)
      {
         SCPS_Cleanup_Service(BluetoothStackID, SCPSInstanceID);
...
static void SCPSPopulateHandles(DeviceInfo_t *DeviceInfo, GATT_Service_Discovery_Indication_Data_t *ServiceDiscoveryData)
{
   ...
   /* Verify that the input parameters are semi-valid.                  */
   if((DeviceInfo) && (ServiceDiscoveryData) && (ServiceDiscoveryData->ServiceInformation.UUID.UUID_Type == guUUID_16) && (SCPS_COMPARE_SCPS_SERVICE_UUID_TO_UUID_16(ServiceDiscoveryData->ServiceInformation.UUID.UUID.UUID_16)))
   {
      /* Loop through all characteristics discovered in the service     */
      /* and populate the correct entry.                                */
      ...
      if(CurrentCharacteristic)
      {
         ...
         {
            if(SCPS_COMPARE_SCAN_INTERVAL_WINDOW_UUID_TO_UUID_16(CurrentCharacteristic->Characteristic_UUID.UUID.UUID_16))
            {
               /* All SCPS UUIDs are defined to be 16 bit UUIDs.        */
               if(CurrentCharacteristic->Characteristic_UUID.UUID_Type == guUUID_16)
               {
                  DeviceInfo->ClientInfo.Scan_Interval_Window = CurrentCharacteristic->Characteristic_Handle;
                  ...
                  if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_WRITE_WITHOUT_RESPONSE))
                     printf("Warning - Mandatory write without response property of Scan Interval Window characteristic not supported!\r\n");
               }
            }
            else
            {
               if(SCPS_COMPARE_SCAN_REFRESH_UUID_TO_UUID_16(CurrentCharacteristic->Characteristic_UUID.UUID.UUID_16))
               {
                  DeviceInfo->ClientInfo.Scan_Refresh = CurrentCharacteristic->Characteristic_Handle;

                  /* Verify that Notify is supported.                   */
                  if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_NOTIFY))
                     printf("Warning - Mandatory notify property of Scan Refresh characteristic not supported!\r\n");
                  ...
                  for(Index2 = 0; Index2 < CurrentCharacteristic->NumberOfDescriptors; Index2++)
                  {
                     if(CurrentDescriptor->Characteristic_Descriptor_UUID.UUID_Type == guUUID_16)
                     {
                        ...
                        {
                           DeviceInfo->ClientInfo.Scan_Refresh_Client_Configuration = CurrentCharacteristic->DescriptorList[Index2].Characteristic_Descriptor_Handle;
                        }
                     }
                  }
               }
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("* Command Options SCPS:    RegisterSCPS, UnregisterSCPS,         *\r\n");
   printf("*                          DiscoverSCPS,                         *\r\n");
   printf("*                          ConfigureRemoteSCPS,                  *\r\n");
   printf("*                          SetScanIntervalWindow,                *\r\n");
   printf("*                          NotifyScanRefresh,                    *\r\n");
   printf("******************************************************************\r\n");
   ...
   if((BluetoothStackID) && (!ConnectionID))
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].intParam >= 0) && (TempParam->Params[0].intParam <= 1))
      {
         ...
         else
         {
            ...
            if(SCPSInstanceID)
            {
               /* Advertise the SCPS Server (1 byte type & 2 bytes UUID)*/
               ...
               SCPS_ASSIGN_SCPS_SERVICE_UUID_16(&(Advertisement_Data_Buffer.AdvertisingData.Advertising_Data[5]));
            }
...
static int EnableDebug(ParameterList_t *TempParam)
{
   ...
   /* The following function is responsible for registering a SCPP      */
...
static int RegisterSCPS(ParameterList_t *TempParam)
{
   ...
   if(!ConnectionID)
   {
      ...
      if(!SCPSInstanceID)
      {
         /* Register the SCPP Service with GATT.                        */
         ret_val = SCPS_Initialize_Service(BluetoothStackID, SCPS_EventCallback, 0, &SCPSInstanceID);
         if((ret_val > 0) && (SCPSInstanceID > 0))
         {
            /* Display success message.                                 */
            printf("Successfully registered SCPP Service.\r\n");

            /* Save the ServiceID of the registered service.            */
            SCPSInstanceID = (unsigned int)ret_val;
      ...
      {
         printf("SCPP Service already registered.\r\n");
   ...
   /* The following function is responsible for unregistering a SCPP    */
   /* Service.  This function will return zero on successful execution  */
   /* and a negative value on errors.                                   */
static int UnregisterSCPS(ParameterList_t *TempParam)
{
   ...
   if(SCPSInstanceID)
   {
      ...
      /* Unregister the SCPP Service with GATT.                         */
      ret_val = SCPS_Cleanup_Service(BluetoothStackID, SCPSInstanceID);
      if(ret_val == 0)
      {
         /* Display success message.                                    */
         printf("Successfully unregistered SCPP Service.\r\n");

         /* Clear the ServiceID of the registered service.              */
         SCPSInstanceID = 0;
      }
      else
         DisplayFunctionError("SCPS_Cleanup_Service", ret_val);
   }
   else
      printf("SCPP Service not registered.\r\n");

   return(ret_val);
}

   /* The following function is responsible for performing a SCPP       */
   /* Service Discovery Operation.  This function will return zero on   */
   /* successful execution and a negative value on errors.              */
static int DiscoverSCPS(ParameterList_t *TempParam)
{
   ...
   if(!SCPSInstanceID)
   {
      ...
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            if(!(DeviceInfo->Flags & DEVICE_INFO_FLAGS_SERVICE_DISCOVERY_OUTSTANDING))
            {
               /* Configure the filter so that only the SCPP Service is */
               /* discovered.                                           */
               UUID[0].UUID_Type = guUUID_16;
               SCPS_ASSIGN_SCPS_SERVICE_UUID_16(&(UUID[0].UUID.UUID_16));

               /* Start the service discovery process.                  */
               ret_val = GATT_Start_Service_Discovery(BluetoothStackID, ConnectionID, (sizeof(UUID)/sizeof(GATT_UUID_t)), UUID, GATT_Service_Discovery_Event_Callback, sdSCPS);
               if(!ret_val)
               {
                  /* Display success message.                           */
                  printf("GDIS_Service_Discovery_Start success.\r\n");
      ...
      printf("Cannot discover SCPP Services when registered as a service.\r\n");

   return(ret_val);
}

   /* The following function is responsible for configure a SCPP Service*/
...
static int ConfigureRemoteSCPS(ParameterList_t *TempParam)
{
   ...
   if(TempParam && (TempParam->NumberofParameters > 0))
   {
      ...
      if(!SCPSInstanceID)
      {
         ...
         if(ConnectionID)
         {
            ...
            if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
            {
               ...
               if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_SERVICE_DISCOVERY_COMPLETE)
               {
                  ...
                  if(DeviceInfo->ClientInfo.Scan_Refresh)
                  {
                     if(DeviceInfo->ClientInfo.Scan_Refresh_Client_Configuration)
                     {
                        ret_val = EnableDisableNotificationsIndications(DeviceInfo->ClientInfo.Scan_Refresh_Client_Configuration, (TempParam->Params[0].intParam ? GATT_CLIENT_CONFIGURATION_CHARACTERISTIC_NOTIFY_ENABLE : 0), GATT_ClientEventCallback_SCPP);
                     }
                     else
                        printf("   Error - Scan Refresh CC not found on this device.\r\n");
                  }
                  else
                  {
                     printf("   Error - Scan Refresh Characteristics not supported.\r\n");
                  }
         ...
         printf("Cannot configure remote SCPP Services when registered as a service.\r\n");
   }
   else
   {
      printf("Usage: ConfigureRemoteSCPS [Scan Refresh Notification (0 = disable, 1 = enable)]\r\n");
   }
...
static int SetScanIntervalWindow(ParameterList_t *TempParam)
{
   ...
   {
      ScanIntervalWindow.LE_Scan_Interval = (Word_t)TempParam->Params[0].intParam;
      ScanIntervalWindow.LE_Scan_Window   = (Word_t)TempParam->Params[1].intParam;
      ...
      if(!SCPSInstanceID)
      {
         ...
         if(ConnectionID)
         {
            ...
            if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
            {
               ...
               if(DeviceInfo->ClientInfo.Scan_Interval_Window != 0)
               {
                  /* Format the command                                 */
                  if((ret_val = SCPS_Format_Scan_Interval_Window(&ScanIntervalWindow, SCPS_SCAN_INTERVAL_WINDOW_SIZE, Buffer)) == 0)
                  {
                     /* Finally, submit a write without response request*/
                     /* to the server.                                  */
                     if((ret_val = GATT_Write_Without_Response_Request(BluetoothStackID, ConnectionID, DeviceInfo->ClientInfo.Scan_Interval_Window, SCPS_SCAN_INTERVAL_WINDOW_SIZE, ((void *)Buffer))) > 0)
                     {
                  ...
                  {
                     DisplayFunctionError("SCPS_Format_Scan_Interval_Window", ret_val);

                     ret_val = FUNCTION_ERROR;
                  }
               }
               else
                  printf("Error - Scan Interval Window not supported on remote service!\r\n");
            }
            else
         ...
         printf("Cannot Set Scan Interval Window when registered as a service.\r\n");
   }
   else
      DisplayUsage("SetScanIntervalWindow [LE_Scan_Interval uint16] [LE_Scan_Window uint16]");
   ...
   if(BluetoothStackID)
   {
      ...
      if(SCPSInstanceID)
      {
         if((ConnectionID) && (!COMPARE_NULL_BD_ADDR(ConnectionBD_ADDR)))
         {
            if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
            {
               ...
               /* notifications.                                        */
               if(DeviceInfo->ServerInfo.Scan_Refresh_Client_Configuration & GATT_CLIENT_CONFIGURATION_CHARACTERISTIC_NOTIFY_ENABLE)
               {
                  /* Initialize Scan Refresh Value                      */
                  ScanRefreshValue = SCPS_SCAN_REFRESH_VALUE_SERVER_REQUIRES_REFRESH;

                  /* Go ahead and send the Scan Refresh Value data.     */
                  if((ret_val = SCPS_Notify_Scan_Refresh(BluetoothStackID, SCPSInstanceID, ConnectionID, ScanRefreshValue))== 0)
                  {
                     printf("Scan Refresh Notification success.\r\n");
                  }
                  else
                     DisplayFunctionError("SCPS_Notify_Scan_Refresh", ret_val);
               }
               else
               {
                  printf("Client has not registered for Scan Refresh notifications.\r\n");
      ...
      else
      {
         if(ConnectionID)
            ...
            printf("Error - SCPS server not registered\r\n");
      }
   }
   else
   {
...
static void BTPSAPI GAP_LE_Event_Callback(unsigned int BluetoothStackID, GAP_LE_Event_Data_t *GAP_LE_Event_Data, unsigned long CallbackParameter)
{
   ...
   /* The following is a SCPP Server Event Callback.  This function will*/
   /* be called whenever an SCPP Server Profile Event occurs that is    */
...
static void BTPSAPI SCPS_EventCallback(unsigned int BluetoothStackID, SCPS_Event_Data_t *SCPS_Event_Data, unsigned long CallbackParameter)
{
   ...
   if((BluetoothStackID) && (SCPS_Event_Data))
   {
      ...
      {
         case etSCPS_Server_Read_Client_Configuration_Request:
            printf("etSCPS_Server_Read_Client_Configuration_Request with size %u.\r\n", SCPS_Event_Data->Event_Data_Size);
            ...
            if(SCPS_Event_Data->Event_Data.SCPS_Read_Client_Configuration_Data)
            {
               ...
               printf("   Connection Type:  %s.\r\n", ((SCPS_Event_Data->Event_Data.SCPS_Read_Client_Configuration_Data->ConnectionType == gctLE)?"LE":"BR/EDR"));
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, SCPS_Event_Data->Event_Data.SCPS_Read_Client_Configuration_Data->RemoteDevice)) != NULL)
               {
                  if(SCPSInstanceID == SCPS_Event_Data->Event_Data.SCPS_Read_Client_Configuration_Data->InstanceID)
                  {
                     ...
                     {
                        case ctScanRefresh:
                           printf("Config Type:      ctScanRefresh.\r\n");

                           Result = SCPS_Read_Client_Configuration_Response(BluetoothStackID, SCPS_Event_Data->Event_Data.SCPS_Read_Client_Configuration_Data->InstanceID, SCPS_Event_Data->Event_Data.SCPS_Read_Client_Configuration_Data->TransactionID, DeviceInfo->ServerInfo.Scan_Refresh_Client_Configuration);
                           break;
                     ...
                     if(Result)
                        DisplayFunctionError("SCPS_Read_Client_Configuration_Response", Result);
                  }
                  else
                  {
                     printf("\r\nInvalid Event data.\r\n");
         ...
         case etSCPS_Server_Update_Client_Configuration_Request:
            printf("etSCPS_Server_Update_Client_Configuration_Request with size %u.\r\n", SCPS_Event_Data->Event_Data_Size);
            ...
            if(SCPS_Event_Data->Event_Data.SCPS_Client_Configuration_Update_Data)
            {
               ...
               printf("   Connection Type:  %s.\r\n", ((SCPS_Event_Data->Event_Data.SCPS_Client_Configuration_Update_Data->ConnectionType == gctLE)?"LE":"BR/EDR"));
               printf("   Remote Device:    %s.\r\n", BoardStr);
                  ...
                  if(SCPS_Event_Data->Event_Data.SCPS_Client_Configuration_Update_Data->InstanceID == SCPSInstanceID)
                  {
                     switch(SCPS_Event_Data->Event_Data.SCPS_Client_Configuration_Update_Data->ClientConfigurationType)
                     {
                        case ctScanRefresh:
                           ...
                           DeviceInfo->ServerInfo.Scan_Refresh_Client_Configuration = SCPS_Event_Data->Event_Data.SCPS_Client_Configuration_Update_Data->ClientConfiguration;
                           break;
                        default:
                           printf("   Config Type:      Unknown.\r\n");
                           break;
                     ...
                     printf("Value:            0x%04X.\r\n", (Word_t)SCPS_Event_Data->Event_Data.SCPS_Client_Configuration_Update_Data->ClientConfiguration);
                  }
                  else
                  {
                     printf("\r\nInvalid Event data.\r\n");
         ...
         case etSCPS_Server_Write_Scan_Interval_Window_Request:
            printf("etSCPS_Server_Write_Scan_Interval_Window_Request with size %u.\r\n", SCPS_Event_Data->Event_Data_Size);
   ...
   else
   {
      ...
      printf("SCPS Callback Data: Event_Data = NULL.\r\n");
   }
      ...
      switch(GATT_Client_Event_Data->Event_Data_Type)
      {
         ...
         case etGATT_Client_Read_Response:
            if(GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data)
            {
               ...
               printf("   BD_ADDR:         %s.\r\n", BoardStr);
               printf("   AttributeValue:  %u.\r\n", ((Word_t)(*(GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data->AttributeValue))));
         ...
         case etGATT_Client_Write_Response:
            if(GATT_Client_Event_Data->Event_Data.GATT_Write_Response_Data)
            {
               ...
               {
                  if(CallbackParameter == DeviceInfo->ClientInfo.Scan_Refresh_Client_Configuration)
                  {
      ...
      switch(GATT_Connection_Event_Data->Event_Data_Type)
      {
            ...
            if(GATT_Connection_Event_Data->Event_Data.GATT_Device_Connection_Data)
            {
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, GATT_Connection_Event_Data->Event_Data.GATT_Device_Connection_Data->RemoteDevice)) != NULL)
               {
                  if(LocalDeviceIsMaster)
                  {
                     ...
                     /* supported.                                      */
                     GATT_Exchange_MTU_Request(BluetoothStackID, ConnectionID, BTPS_CONFIGURATION_GATT_DEFAULT_MAXIMUM_SUPPORTED_MTU_SIZE, GATT_ClientEventCallback_SCPP, 0);
                  }
               }
            }
            else
         ...
         case etGATT_Connection_Server_Notification:
            if(GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data)
            {
               ...
               {
                  if(DeviceInfo->ClientInfo.Scan_Refresh == GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data->AttributeHandle)
                  {
         ...
         switch(GATT_Service_Discovery_Event_Data->Event_Data_Type)
         {
            case etGATT_Service_Discovery_Indication:
               ...
               if(GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data)
               {
                  ...
                  /* Check the service discovery type for SCPS          */
                  if(((Service_Discovery_Type_t)CallbackParameter) == sdSCPS)
                  {
                     /* Attempt to populate the handles for the SCPP    */
                     /* Service.                                        */
            ...
            case etGATT_Service_Discovery_Complete:
               ...
               if(GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Complete_Data)
               {
                  ...
                  if(((Service_Discovery_Type_t)CallbackParameter) == sdSCPS)
                  {
                     ...
                     printf("\r\nSCPS Service Discovery Summary\r\n");
                     printf("   Scan Interval Window: %s\r\n", (DeviceInfo->ClientInfo.Scan_Interval_Window              ? "Supported" : "Not Supported"));
                     printf("   Scan Refresh:         %s\r\n", (DeviceInfo->ClientInfo.Scan_Refresh                      ? "Supported" : "Not Supported"));
                     printf("   Scan Refresh CC:      %s\r\n", (DeviceInfo->ClientInfo.Scan_Refresh_Client_Configuration ? "Supported" : "Not Supported"));
                  }
               }
               break;
         }

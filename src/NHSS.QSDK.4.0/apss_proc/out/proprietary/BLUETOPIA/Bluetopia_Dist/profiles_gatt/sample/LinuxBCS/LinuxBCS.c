...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXBCS - Linux Bluetooth Body Composition Service using GATT (LE/BREDR) */
/*             sample application.                                            */
/*                                                                            */
/*  Author:  Ryan McCord                                                      */
/*                                                                            */
...
/*   --------  -----------    ------------------------------------------------*/
/*   02/11/16  R. McCord      Initial creation.                               */
/******************************************************************************/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>

#include "LinuxBCS.h"            /* Application Header.                       */
...
#include "SS1BTBCS.h"            /* Main SS1 BCS Service Header.              */
...
#define BCS_SA_BC_MEASUREMENT_LIST_SIZE            (5)   /* Denotes the size  */
                                                         /* of the measurement*/
                                                         /* list.             */

#define BCS_SA_DEFAULT_BCS_SUPPORTED_FEATURES      (BCS_BC_FEATURE_FLAG_TIME_STAMP_SUPPORTED         |  \
                                                    BCS_BC_FEATURE_FLAG_MULTIPLE_USERS_SUPPORTED     |  \
                                                    BCS_BC_FEATURE_FLAG_BASAL_METABOLISM_SUPPORTED   |  \
                                                    BCS_BC_FEATURE_FLAG_MUSCLE_PERCENTAGE_SUPPORTED  |  \
                                                    BCS_BC_FEATURE_FLAG_MUSCLE_MASS_SUPPORTED        |  \
                                                    BCS_BC_FEATURE_FLAG_FAT_FREE_MASS_SUPPORTED      |  \
                                                    BCS_BC_FEATURE_FLAG_SOFT_LEAN_MASS_SUPPORTED     |  \
                                                    BCS_BC_FEATURE_FLAG_BODY_WATER_MASS_SUPPORTED    |  \
                                                    BCS_BC_FEATURE_FLAG_IMPEDANCE_SUPPORTED          |  \
                                                    BCS_BC_FEATURE_FLAG_WEIGHT_SUPPORTED             |  \
                                                    BCS_BC_FEATURE_FLAG_HEIGHT_SUPPORTED             |  \
                                                    BCS_BC_FEATURE_FLAG_MM_RESOLUTION_NOT_SPECIFIED  |  \
                                                    BCS_BC_FEATURE_FLAG_HM_RESOLUTION_NOT_SPECIFIED)
...
#define DEFAULT_DEBUG_LOG_FILE_NAME                "LinuxBCS_ASC.log"
#define DEFAULT_DEBUG_FTS_FILE_NAME                "LinuxBCS_FTS.log"

   /* Determine the Name we will use for this compilation.              */
#define LE_DEMO_DEVICE_NAME                        "LinuxBCS"
...
typedef struct _tagBCMeasurementData_t
{
   Boolean_t                  Valid;
   unsigned int               ID;
   Boolean_t                  Indicated;
   BCS_BC_Measurement_Data_t  Measurement;
} BCMeasurementData_t;

#define BC_MEASUREMENT_DATA_SIZE  (sizeof(BCMeasurementData_t))
...
typedef struct _tagDeviceInfo_t
{
   ...
   BCS_Client_Information_t  BCSClientInfo;
   BCS_Server_Information_t  BCSServerInfo;
...
#define DEVICE_INFO_FLAGS_BCS_SERVICE_DISCOVERY_COMPLETE    0x0080
...
static unsigned int        BCSInstanceID;
                                                    /* The following holds the BCS    */
                                                    /* Instance IDs that are returned  */
                                                    /* from BCS_Initialize_XXX().     */
...
static BCMeasurementData_t  BCMeasurementList[BCS_SA_BC_MEASUREMENT_LIST_SIZE];
                                                    /* The BC Measurement list.        */
...
static Boolean_t AppearanceIndexToAppearance(unsigned int Index, Word_t *Appearance);
   ...
   /* Body Composition Service Functions and Commands.                  */
static void BCSPopulateHandles(BCS_Client_Information_t *ClientInfo, GATT_Service_Discovery_Indication_Data_t *ServiceDiscoveryData);

static void DisplayBCSSupportedFeatures(DWord_t SupportedFeaturesd);
static void DecodeDisplayBodyCompositionMeasurement(unsigned int ValueLength, Byte_t *Value);
static void DisplayBodyCompositionMeasurement(BCS_BC_Measurement_Data_t *BodyCompositionMeasurement);

static int RegisterBCSCommand(ParameterList_t *TempParam);
static int UnRegisterBCSCommand(ParameterList_t *TempParam);
static int DiscoverBCSCommand(ParameterList_t *TempParam);
static int ReadFeaturesCommand(ParameterList_t *TempParam);
static int ReadCCCDCommand(ParameterList_t *TempParam);
static int WriteCCCDCommand(ParameterList_t *TempParam);
static int IndicateMeasurementCommand(ParameterList_t *TempParam);
static int CreateBCMeasurementCommand(ParameterList_t *TempParam);
static int PrintBCMeasurementsCommand(ParameterList_t *TempParam);
...
static void BTPSAPI BCS_EventCallback(unsigned int BluetoothStackID, BCS_Event_Data_t *BCS_Event_Data, unsigned long CallbackParameter);
...
static void BTPSAPI GATT_ClientEventCallback_BCS(unsigned int BluetoothStackID, GATT_Client_Event_Data_t *GATT_Client_Event_Data, unsigned long CallbackParameter);
...
static DeviceInfo_t *DeleteDeviceInfoEntry(DeviceInfo_t **ListHead, BD_ADDR_t BD_ADDR)
{
   ...
   /*          applications for use with BCS/BCS.                     */
static void FreeDeviceInfoEntryMemory(DeviceInfo_t *EntryToFree)
{
...
static void UserInterface(void)
{
   ...
   AddCommand("REGISTERBCS", RegisterBCSCommand);
   AddCommand("UNREGISTERBCS", UnRegisterBCSCommand);
   AddCommand("DISCOVERBCS", DiscoverBCSCommand);
   AddCommand("READFEATURES", ReadFeaturesCommand);
   AddCommand("READCCCD", ReadCCCDCommand);
   AddCommand("WRITECCCD", WriteCCCDCommand);
   AddCommand("INDICATE", IndicateMeasurementCommand);
   AddCommand("CREATE", CreateBCMeasurementCommand);
   AddCommand("PRINT",  PrintBCMeasurementsCommand);
      ...
      for(Index = 0; Index < Advertising_Data->Number_Data_Entries; Index++)
      {
         ...
         switch(Advertising_Data->Data_Entries[Index].AD_Type)
         {
            ...
            case HCI_LE_ADVERTISING_REPORT_DATA_TYPE_16_BIT_SERVICE_UUID_COMPLETE:
               ...
               /* We will check for the BCS Service data.  Make sure   */
               ...
               if(Advertising_Data_Entry->AD_Data_Length >= NON_ALIGNED_WORD_SIZE)
               {
                  ...
                  /* Check if this is the BCS Service data.            */
                  if(BCS_COMPARE_BCS_SERVICE_UUID_TO_UUID_16(*((UUID_16_t *)&(Advertising_Data_Entry->AD_Data_Buffer[0]))))
                  {
                     /* Decode the UUID.                                */
...
static int CloseStack(void)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(BCSInstanceID)
      {
         BCS_Cleanup_Service(BluetoothStackID, BCSInstanceID);
      }
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("* Command Options BCS:                                           *\r\n");
   printf("*                Server:   RegisterBCS                           *\r\n");
   printf("*                          UnRegisterBCS,                        *\r\n");
   printf("*                          Indicate,                             *\r\n");
   printf("*                          Create,                               *\r\n");
   printf("*                          Print,                                *\r\n");
   printf("*                Client:   DiscoverBCS,                          *\r\n");
   printf("*                          ReadCCCD,                             *\r\n");
   printf("*                          WriteCCCD,                            *\r\n");
   ...
   if((BluetoothStackID) && (!ConnectionID))
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].intParam >= 0) && (TempParam->Params[0].intParam <= 1))
      {
         ...
         else
         {
            ...
            /* Configure the flags field bcsed on the Discoverability  */
            /* Mode.                                                    */
            ...
            if(BCSInstanceID)
            {
               ...
               BCS_ASSIGN_BCS_SERVICE_UUID_16(&(Advertisement_Data_Buffer.AdvertisingData.Advertising_Data[5]));
            }
...
static int ReadRemoteAppearance(ParameterList_t *TempParam)
{
   ...
   /* The following function is a utility function that provides a      */
   /* mechanism of populating a BCS Client Information structure with   */
   /* the information discovered from a BCS Discovery operation.        */
static void BCSPopulateHandles(BCS_Client_Information_t *ClientInfo, GATT_Service_Discovery_Indication_Data_t *ServiceDiscoveryData)
{
   unsigned int                                  Index1;
   unsigned int                                  Index2;
   ...
   /* Verify that the input parameters are semi-valid.                  */
   if((ClientInfo) && (ServiceDiscoveryData) && (ServiceDiscoveryData->ServiceInformation.UUID.UUID_Type == guUUID_16) && (BCS_COMPARE_BCS_SERVICE_UUID_TO_UUID_16(ServiceDiscoveryData->ServiceInformation.UUID.UUID.UUID_16)))
   {
      ...
      if(CurrentCharacteristic)
      {
         ...
         {
            /* Check for BC Feature UUID.                               */
            if(BCS_COMPARE_BC_FEATURE_UUID_TO_UUID_16(CurrentCharacteristic->Characteristic_UUID.UUID.UUID_16))
            {
               /* All BCS UUIDs are defined to be 16 bit UUIDs.         */
               if(CurrentCharacteristic->Characteristic_UUID.UUID_Type == guUUID_16)
               {
                  ClientInfo->BC_Feature_Handle = CurrentCharacteristic->Characteristic_Handle;

                  /* Verify that read is supported.                     */
                  if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_READ))
                    printf("Warning - Mandatory read property of BCS Body Composition Feature characteristic not supported!\r\n");
               }
            }
            else
            {
               /* Check for BC Measurement UUID.                        */
               if(BCS_COMPARE_BC_MEASUREMENT_UUID_TO_UUID_16(CurrentCharacteristic->Characteristic_UUID.UUID.UUID_16))
               {
                  /* All BCS UUIDs are defined to be 16 bit UUIDs.      */
                  if(CurrentCharacteristic->Characteristic_UUID.UUID_Type == guUUID_16)
                  {
                     ClientInfo->BC_Measurement_Handle = CurrentCharacteristic->Characteristic_Handle;
                     ...
                     if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_INDICATE))
                        printf("Warning - Mandatory indicate property of BCS Body Composition Measurement characteristic not supported!\r\n");
                     ...
                     for(Index2 = 0; Index2 < CurrentCharacteristic->NumberOfDescriptors; Index2++)
                     {
                        if(CurrentDescriptor->Characteristic_Descriptor_UUID.UUID_Type == guUUID_16)
                        {
                           ...
                           {
                              ClientInfo->BC_Measurement_CCCD_Handle = CurrentCharacteristic->DescriptorList[Index2].Characteristic_Descriptor_Handle;
                           }
                        }
                     }
                  }
...
static void DisplayBCSSupportedFeatures(DWord_t SupportedFeatures)
{
   printf("\r\nBCS Body Composition Feature:\r\n");
   printf("   Value:             0x%06X.\r\n", SupportedFeatures);

   /* Display the supported features to the application.                */
   if(SupportedFeatures & BCS_BC_FEATURE_FLAG_TIME_STAMP_SUPPORTED)
      printf("   Time Stamp:        Supported.\r\n");
   else
      printf("   Time Stamp:        Not Supported.\r\n");

   if(SupportedFeatures & BCS_BC_FEATURE_FLAG_MULTIPLE_USERS_SUPPORTED)
      printf("   Multiple Users:    Supported.\r\n");
   else
      printf("   Multiple Users:    Not Supported.\r\n");

   if(SupportedFeatures & BCS_BC_FEATURE_FLAG_BASAL_METABOLISM_SUPPORTED)
      printf("   Basal Metabolism:  Supported.\r\n");
   else
      printf("   Basal Metabolism:  Not Supported.\r\n");

   if(SupportedFeatures & BCS_BC_FEATURE_FLAG_MUSCLE_PERCENTAGE_SUPPORTED)
      printf("   Muscle Percentage: Supported.\r\n");
   else
      printf("   Muscle Percentage: Not Supported.\r\n");

   if(SupportedFeatures & BCS_BC_FEATURE_FLAG_MUSCLE_MASS_SUPPORTED)
      printf("   Muscle Mass:       Supported.\r\n");
   else
      printf("   Muscle Mass:       Not Supported.\r\n");

   if(SupportedFeatures & BCS_BC_FEATURE_FLAG_FAT_FREE_MASS_SUPPORTED)
      printf("   Fat Free Mass:     Supported.\r\n");
   else
      printf("   Fat Free Mass:     Not Supported.\r\n");

   if(SupportedFeatures & BCS_BC_FEATURE_FLAG_SOFT_LEAN_MASS_SUPPORTED)
      printf("   Soft Lean Mass:    Supported.\r\n");
   else
      printf("   Soft Lean Mass:    Not Supported.\r\n");

   if(SupportedFeatures & BCS_BC_FEATURE_FLAG_BODY_WATER_MASS_SUPPORTED)
      printf("   Body Water Mass:   Supported.\r\n");
   else
      printf("   Body Water Mass:   Not Supported.\r\n");

   if(SupportedFeatures & BCS_BC_FEATURE_FLAG_IMPEDANCE_SUPPORTED)
      printf("   Impedance:         Supported.\r\n");
   else
      printf("   Impedance:         Not Supported.\r\n");

   if(SupportedFeatures & BCS_BC_FEATURE_FLAG_WEIGHT_SUPPORTED)
      printf("   Weight:            Supported.\r\n");
   else
      printf("   Weight:            Not Supported.\r\n");

   if(SupportedFeatures & BCS_BC_FEATURE_FLAG_HEIGHT_SUPPORTED)
      printf("   Height:            Supported.\r\n");
   else
      printf("   Height:            Not Supported.\r\n\r\n");

   /* Display the supported Weight resolution.                          */
   if(SupportedFeatures & BCS_BC_FEATURE_FLAG_MM_RESOLUTION_POINT_5_KG_OR_1_LB)
      printf("   Mass Measurement Resolution:     0.5 kg or 0.01 lb.\r\n");
   else
   {
      if(SupportedFeatures & BCS_BC_FEATURE_FLAG_MM_RESOLUTION_POINT_2_KG_OR_POINT_5_LB)
         printf("   Mass Measurement Resolution:     0.2 kg 0.02 lb.\r\n");
      else
      {
         if(SupportedFeatures & BCS_BC_FEATURE_FLAG_MM_RESOLUTION_POINT_1_KG_OR_POINT_2_LB)
         {
            printf("   Mass Measurement Resolution:     0.1 kg or 0.05 lb.\r\n");
         }
         else
         {
            if(SupportedFeatures & BCS_BC_FEATURE_FLAG_MM_RESOLUTION_POINT_05_KG_OR_POINT_1_LB)
               printf("   Mass Measurement Resolution:     0.05 kg or 0.1 lb.\r\n");
            else
            {
               if(SupportedFeatures & BCS_BC_FEATURE_FLAG_MM_RESOLUTION_POINT_02_KG_OR_POINT_05_LB)
                  printf("   Mass Measurement Resolution:     0.02 kg 0.2 lb.\r\n");
               else
               {
                  if(SupportedFeatures & BCS_BC_FEATURE_FLAG_MM_RESOLUTION_POINT_01_KG_OR_POINT_02_LB)
                     printf("   Mass Measurement Resolution:     0.01 kg or 0.5 lb.\r\n");
                  else
                  {
                     if(SupportedFeatures & BCS_BC_FEATURE_FLAG_MM_RESOLUTION_POINT_005_KG_OR_POINT_01_LB)
                        printf("   Mass Measurement Resolution:     0.001 kg or 0.01 lb.\r\n");
                     else
                        printf("   Mass Measurement Resolution:     Not specified.\r\n");
                  }
               }
            }
         }
   ...
   if(SupportedFeatures & BCS_BC_FEATURE_FLAG_HM_RESOLUTION_POINT_01_M_OR_1_INCH)
      printf("   Height Measurement Resolution:   0.01 m or 0.1 inch.\r\n");
   else
   {
      if(SupportedFeatures & BCS_BC_FEATURE_FLAG_HM_RESOLUTION_POINT_005_M_OR_POINT_5_INCH)
         printf("   Height Measurement Resolution:   0.005 m or 0.5 inch.\r\n");
      else
      {
         if(SupportedFeatures & BCS_BC_FEATURE_FLAG_HM_RESOLUTION_POINT_0001_M_OR_POINT_1_INCH)
   ...
   /* a Body Composition Measurement provided.                          */
static void DecodeDisplayBodyCompositionMeasurement(unsigned int ValueLength, Byte_t *Value)
{
   int                       Result;
   BCS_BC_Measurement_Data_t BodyCompositionMeasurement;
   ...
   if((ValueLength) && (Value))
   {
      ...
      /* Decode the Body Composition Measurement.                       */
      if((Result = BCS_Decode_Body_Composition_Measurement(ValueLength, Value, &BodyCompositionMeasurement)) == 0)
         DisplayBodyCompositionMeasurement(&BodyCompositionMeasurement);
      else
         DisplayFunctionError("BCS_Decode_Body_Composition_Measurement", Result);
   }
   else
      printf("Invalid Body Composition Measurement Response buffer.\r\n");
}

   /* The following function is a helper function to display the BC     */
   /* Measurement.                                                      */
static void DisplayBodyCompositionMeasurement(BCS_BC_Measurement_Data_t *BodyCompositionMeasurement)
{
   /* Display the flags.                                             */
   printf("   Flags:                0x%04X\r\n", BodyCompositionMeasurement->Flags);

   /* Display the Multiple Packet Measurement status.                */
   if(BodyCompositionMeasurement->Flags & BCS_BC_MEASUREMENT_FLAG_MULTIPLE_PACKET_MEASUREMENT)
      printf("   Multiple Packet:      Yes\r\n");
   else
      printf("   Multiple Packet:      No\r\n");

   /* Display the Body Fat Percentage.                               */
   printf("   Body Fat Percentage:  %u\r\n", BodyCompositionMeasurement->BodyFatPercentage);

   /* Display the Unit type.                                         */
   if(BodyCompositionMeasurement->Flags & BCS_BC_MEASUREMENT_FLAG_MEASUREMENT_UNITS_IMPERIAL)
      printf("   Units:                Imperial\r\n");
   else
      printf("   Units:                SI\r\n");

   /* Display the Time Stamp if included in the measurement.         */
   if(BodyCompositionMeasurement->Flags & BCS_BC_MEASUREMENT_FLAG_TIME_STAMP_PRESENT)
   {
      printf("   Time Stamp:           %02u/%02u/%02u %02u:%02u:%02u\r\n",    BodyCompositionMeasurement->TimeStamp.Month,
                                                                     BodyCompositionMeasurement->TimeStamp.Day,
                                                                     BodyCompositionMeasurement->TimeStamp.Year,
                                                                     BodyCompositionMeasurement->TimeStamp.Hours,
                                                                     BodyCompositionMeasurement->TimeStamp.Minutes,
                                                                     BodyCompositionMeasurement->TimeStamp.Seconds);
   }

   /* Display the User ID if included in the measurement.            */
   if(BodyCompositionMeasurement->Flags & BCS_BC_MEASUREMENT_FLAG_USER_ID_PRESENT)
   {
       printf("   User ID:              %u\r\n", BodyCompositionMeasurement->UserID);
   }

   /* Display the Basal Metabolism if included in the measurement.   */
   if(BodyCompositionMeasurement->Flags & BCS_BC_MEASUREMENT_FLAG_BASAL_METABOLISM_PRESENT)
   {
       printf("   Basal Metabolism:     %u\r\n", BodyCompositionMeasurement->BasalMetabolism);
   }

   /* Display the Muscle Percentage if included in the measurement.  */
   if(BodyCompositionMeasurement->Flags & BCS_BC_MEASUREMENT_FLAG_MUSCLE_PERCENTAGE_PRESENT)
   {
       printf("   Muscle Percentage:    %u\r\n", BodyCompositionMeasurement->MusclePercentage);
   }

   /* Display the Muscle Mass if included in the measurement.        */
   if(BodyCompositionMeasurement->Flags & BCS_BC_MEASUREMENT_FLAG_MUSCLE_MASS_PRESENT)
   {
       printf("   Muscle Mass:          %u\r\n", BodyCompositionMeasurement->MuscleMass);
   }

   /* Display the Fat Free Mass if included in the measurement.      */
   if(BodyCompositionMeasurement->Flags & BCS_BC_MEASUREMENT_FLAG_FAT_FREE_MASS_PRESENT)
   {
       printf("   Fat Free Mass:        %u\r\n", BodyCompositionMeasurement->FatFreeMass);
   }

   /* Display the Soft Lean Mass if included in the measurement.     */
   if(BodyCompositionMeasurement->Flags & BCS_BC_MEASUREMENT_FLAG_SOFT_LEAN_MASS_PRESENT)
   {
       printf("   Soft Lean Mass:       %u\r\n", BodyCompositionMeasurement->SoftLeanMass);
   }

   /* Display the Body Water Mass if included in the measurement.    */
   if(BodyCompositionMeasurement->Flags & BCS_BC_MEASUREMENT_FLAG_BODY_WATER_MASS_PRESENT)
   {
       printf("   Body Water Mass:      %u\r\n", BodyCompositionMeasurement->BodyWaterMass);
   }

   /* Display the Impedance if included in the measurement.          */
   if(BodyCompositionMeasurement->Flags & BCS_BC_MEASUREMENT_FLAG_IMPEDANCE_PRESENT)
   {
       printf("   Impedance:            %u\r\n", BodyCompositionMeasurement->Impedance);
   }

   /* Display the Weight if included in the measurement.             */
   if(BodyCompositionMeasurement->Flags & BCS_BC_MEASUREMENT_FLAG_WEIGHT_PRESENT)
   {
       printf("   Weight:               %u\r\n", BodyCompositionMeasurement->Weight);
   }

   /* Display the Height if included in the measurement.             */
   if(BodyCompositionMeasurement->Flags & BCS_BC_MEASUREMENT_FLAG_HEIGHT_PRESENT)
   {
       printf("   Height:               %u\r\n", BodyCompositionMeasurement->Height);
   }
}

   /* The following function is responsible for registering a BCS       */
...
static int RegisterBCSCommand(ParameterList_t *TempParam)
{
   ...
   if(!ConnectionID)
   {
      ...
      if(!BCSInstanceID)
      {
         /* Register the BCS Service with GATT.                         */
         ret_val = BCS_Initialize_Service(BluetoothStackID, BCS_SERVICE_FLAGS_DUAL_MODE, BCS_EventCallback, 0, &BCSInstanceID);
         if((ret_val > 0) && (BCSInstanceID > 0))
         {
            /* Display success message.                                 */
            printf("\r\nSuccessfully registered BCS Service.\r\n");

            /* Save the ServiceID of the registered service.            */
            BCSInstanceID  = (unsigned int)ret_val;
            ret_val        = 0;

            /* Initialize the BC Measurement list.  This will mark all  */
            /* measurements as invalid.                                 */
            BTPS_MemInitialize(BCMeasurementList, 0, (BC_MEASUREMENT_DATA_SIZE * BCS_SA_BC_MEASUREMENT_LIST_SIZE));

            /* Set the default features.                                */
            if((ret_val = BCS_Set_BC_Feature(BluetoothStackID, BCSInstanceID, (DWord_t)BCS_SA_DEFAULT_BCS_SUPPORTED_FEATURES)) != 0)
               DisplayFunctionError("BCS_Set_BC_Features", ret_val);
         }
      }
      else
      {
         printf("\r\nBCS Service already registered.\r\n");
   ...
   /* The following function is responsible for unregistering a BCS     */
   /* Service.  This function will return zero on successful execution  */
   /* and a negative value on errors.                                   */
static int UnRegisterBCSCommand(ParameterList_t *TempParam)
{
   ...
   if(BCSInstanceID)
   {
      ...
      /* Unregister the BCS Service with GATT.                          */
      ret_val = BCS_Cleanup_Service(BluetoothStackID, BCSInstanceID);
      if(ret_val == 0)
      {
         /* Display success message.                                    */
         printf("\r\nSuccessfully unregistered BCS Service.\r\n");

         /* Save the ServiceID of the registered service.               */
         BCSInstanceID = 0;
      }
      else
         DisplayFunctionError("BCS_Cleanup_Service", ret_val);
   }
   else
      printf("\r\nBCS Service not registered.\r\n");

   return(ret_val);
}

   /* The following function is responsible for performing a BCS Service*/
   ...
   if(!BCSInstanceID)
   {
      ...
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            if(!(DeviceInfo->Flags & DEVICE_INFO_FLAGS_SERVICE_DISCOVERY_OUTSTANDING))
            {
               /* Configure the filter so that only the BCS Service is  */
               /* discovered.                                           */
               UUID[0].UUID_Type = guUUID_16;
               BCS_ASSIGN_BCS_SERVICE_UUID_16(&(UUID[0].UUID.UUID_16));

               /* Start the service discovery process.                  */
               ret_val = GATT_Start_Service_Discovery(BluetoothStackID, ConnectionID, (sizeof(UUID)/sizeof(GATT_UUID_t)), UUID, GATT_Service_Discovery_Event_Callback, sdBCS);
               if(!ret_val)
               {
                  /* Display success message.                           */
                  printf("GATT_Service_Discovery_Start success.\r\n");
      ...
      printf("Cannot discover BCS Services when registered as a service.\r\n");

   return(ret_val);
}

   /* The following function is responsible for reading a BCS           */
   /* Measurement CCCD on a BCS Server.  This function will return zero */
...
static int ReadFeaturesCommand(ParameterList_t *TempParam)
{
   ...
   if(BCSInstanceID)
   {
      ...
      if((ret_val = BCS_Query_BC_Feature(BluetoothStackID, BCSInstanceID, &Features)) != 0)
         DisplayFunctionError("BCS_Query_BC_Feature", ret_val);

      /* Display the features.                                          */
      DisplayBCSSupportedFeatures(Features);
   }
   else
   {
      /* Verify that there is a connection that is established.      */
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            /* this device                                           */
            if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_BCS_SERVICE_DISCOVERY_COMPLETE)
            {
               /* Determine if BC Feature is supported (mandatory).     */
               if(DeviceInfo->BCSClientInfo.BC_Feature_Handle)
               {
                  /* Store the handle.                                  */
                  AttributeHandle = DeviceInfo->BCSClientInfo.BC_Feature_Handle;

                  /* Send the GATT read request.                  */
                  /* * NOTE * We will not save the transactionID  */
                  /*          returned by this function, which we */
                  /*          could use to cancel the request.    */
                  if((ret_val = GATT_Read_Value_Request(BluetoothStackID, ConnectionID, AttributeHandle, GATT_ClientEventCallback_BCS, AttributeHandle)) > 0)
                  {
                     printf("\r\nGATT Read Value Request sent:\r\n");
                     printf("   TransactionID:     %d\r\n",     ret_val);
                     printf("   Attribute Handle:  0x%04X\r\n", AttributeHandle);
               ...
               printf("\r\nBCS Service discovery has not been performed on this device.\r\n");
         }
         else
            printf("\r\nNo Device Info.\r\n");
      }
   ...
   if(!BCSInstanceID)
   {
      ...
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_BCS_SERVICE_DISCOVERY_COMPLETE)
            {
               /* Determine if BC Measurement CCCD is supported         */
               /* (mandatory).                                          */
               if(DeviceInfo->BCSClientInfo.BC_Measurement_CCCD_Handle)
               {
                  /* Store the handle.                                  */
                  AttributeHandle = DeviceInfo->BCSClientInfo.BC_Measurement_CCCD_Handle;
      ...
      printf("\r\nOnly the BCS Client may read the BC Measurement CCCD.\r\n");

   return(ret_val);
}

   /* The following function is responsible for configuring a BCS       */
      ...
      if(!BCSInstanceID)
      {
         ...
         if(ConnectionID)
         {
            ...
            if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
            {
               ...
               if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_BCS_SERVICE_DISCOVERY_COMPLETE)
               {
                  printf("\r\nAttempting to configure BCS Body Composition Measurement CCCD...\r\n");
                  ...
                  else
                  {
                     if(TempParam->Params[0].intParam == 1)
                     {
                        ASSIGN_HOST_WORD_TO_LITTLE_ENDIAN_UNALIGNED_WORD(&Client_Configuration, (Word_t)BCS_CLIENT_CHARACTERISTIC_CONFIGURATION_INDICATE_ENABLE);
                     }
                     else
                     {
                  ...
                  if((ret_val = GATT_Write_Request(BluetoothStackID, ConnectionID, AttributeHandle, NON_ALIGNED_WORD_SIZE, (Byte_t *)&Client_Configuration, GATT_ClientEventCallback_BCS, AttributeHandle)) > 0)
                  {
         ...
         printf("\r\nOnly the BCS Client can configure the BC Measurement CCCD.\r\n");
   }
   else
      printf("\r\nUsage: WriteCCCD [Configuration (0 = disable, 1 = indicate, 2 = notify)]\r\n");
...
static int IndicateMeasurementCommand(ParameterList_t *TempParam)
{
   ...
   unsigned int               Index;
   unsigned int               Index2;
   BCS_BC_Measurement_Data_t *BC_Measurement = NULL;
   unsigned int               ID;
   ...
   if(BluetoothStackID)
   {
      ...
      if(ConnectionID)
      {
         ...
         if(BCSInstanceID)
         {
            ...
            /*          list.                                           */
            for(Index = 0, Index2 = 0; (Index < BCS_SA_BC_MEASUREMENT_LIST_SIZE) && (Index2 < MeasurementIDCtr); Index2++)
            {
               /* If we find an ID that matches the counter is is       */
               /* already the oldest measurement.                       */
               if(BCMeasurementList[Index].ID == Index2)
               {
                  /* If we find a measurement that hasn't been          */
                  /* indicated.  Make sure the measurement is valid     */
                  /* first.                                             */
                  if((BCMeasurementList[Index].Valid) && (BCMeasurementList[Index].Indicated == FALSE))
                  {
                     /* Flag that we indicated the measurement.         */
                     BCMeasurementList[Index].Indicated = TRUE;

                     /* Set a pointer to it.                            */
                     BC_Measurement = &(BCMeasurementList[Index].Measurement);

                     /* Store the ID.                                   */
                     ID = BCMeasurementList[Index].ID;
               ...
               if(Index == (BCS_SA_BC_MEASUREMENT_LIST_SIZE-1))
                  Index = 0;
               else
                  Index++;
            }
            ...
            if(BC_Measurement)
            {
               ...
               /* * NOTE * The API will handle sending the indication in*/
               /*          multiple parts if necessary.                 */
               if((ret_val = BCS_Indicate_Body_Composition_Measurement(BluetoothStackID, BCSInstanceID, ConnectionID, BC_Measurement)) > 0)
               {
                  printf("\r\nBC Measurement indicated.\r\n");
                  printf("   Measurement ID:  %u\r\n", ID);
                  printf("   TransactionID:   %d\r\n", ret_val);
               }
               else
                  DisplayFunctionError("BCS_Indicate_Body_Composition_Measurement", ret_val);
            ...
            printf("\r\nOnly the BCS Server can send indications.\r\n");
      }
      else
         printf("\r\nNot connected to a remote BCS Client.\r\n");
   }
   else
      printf("\r\nInvalid BluetoothStackID.\r\n");
   ...
   /* The following function is responsible for creating a new BC       */
   /* Measurement.                                                      */
static int CreateBCMeasurementCommand(ParameterList_t *TempParam)
{
   int                        ret_val = 0;
   unsigned int               Index;
   unsigned int               Index2;
   BCS_BC_Measurement_Data_t  BC_Measurement;
   time_t                     CurrentTime;
   struct tm                 *TimePtr;
   ...
   if(BCSInstanceID)
   {
      ...
      BTPS_MemInitialize(&(BC_Measurement), 0, BCS_BC_MEASUREMENT_DATA_SIZE);

      /* Set the flags.                                                    */
      /* * NOTE * Multiple Packet measurement will be flagged if the       */
      /*          measurement cannot fit in a single packet.               */
      BC_Measurement.Flags |= BCS_BC_MEASUREMENT_FLAG_MEASUREMENT_UNITS_IMPERIAL;
      BC_Measurement.Flags |= BCS_BC_MEASUREMENT_FLAG_TIME_STAMP_PRESENT;
      BC_Measurement.Flags |= BCS_BC_MEASUREMENT_FLAG_USER_ID_PRESENT;
      BC_Measurement.Flags |= BCS_BC_MEASUREMENT_FLAG_BASAL_METABOLISM_PRESENT;
      BC_Measurement.Flags |= BCS_BC_MEASUREMENT_FLAG_MUSCLE_PERCENTAGE_PRESENT;
      BC_Measurement.Flags |= BCS_BC_MEASUREMENT_FLAG_MUSCLE_MASS_PRESENT;
      BC_Measurement.Flags |= BCS_BC_MEASUREMENT_FLAG_FAT_FREE_MASS_PRESENT;
      BC_Measurement.Flags |= BCS_BC_MEASUREMENT_FLAG_SOFT_LEAN_MASS_PRESENT;
      BC_Measurement.Flags |= BCS_BC_MEASUREMENT_FLAG_BODY_WATER_MASS_PRESENT;
      BC_Measurement.Flags |= BCS_BC_MEASUREMENT_FLAG_IMPEDANCE_PRESENT;
      BC_Measurement.Flags |= BCS_BC_MEASUREMENT_FLAG_WEIGHT_PRESENT;
      BC_Measurement.Flags |= BCS_BC_MEASUREMENT_FLAG_HEIGHT_PRESENT;

      /* Assign Measurement data.                                          */
      BC_Measurement.UserID            = 1;
      BC_Measurement.BodyFatPercentage = rand() % 40 + 20;
      ...
      BC_Measurement.TimeStamp.Year    = TimePtr->tm_year + 1900;
      BC_Measurement.TimeStamp.Month   = TimePtr->tm_mon + 1;
      BC_Measurement.TimeStamp.Day     = TimePtr->tm_mday;
      BC_Measurement.TimeStamp.Hours   = TimePtr->tm_hour;
      BC_Measurement.TimeStamp.Minutes = TimePtr->tm_min;
      BC_Measurement.TimeStamp.Seconds = TimePtr->tm_sec;

      BC_Measurement.BasalMetabolism   = rand() % 100 + 20;
      BC_Measurement.MusclePercentage  = rand() % 40 + 20;
      BC_Measurement.MuscleMass        = rand() % 100 + 20;
      BC_Measurement.FatFreeMass       = rand() % 100 + 20;
      BC_Measurement.SoftLeanMass      = rand() % 100 + 20;
      BC_Measurement.BodyWaterMass     = rand() % 100 + 20;
      BC_Measurement.Impedance         = rand() % 100 + 20;
      BC_Measurement.Weight            = rand() % 100 + 140;
      BC_Measurement.Height            = rand() % 10 + 55;

      /* Loop through the BC Measurement list and replace the oldest       */
      /* measurement if necessary.                                         */
      for(Index = 0, Index2 = 0; Index < BCS_SA_BC_MEASUREMENT_LIST_SIZE; Index2++)
      {
         /* If we haven't used all the Measurement entries in the list yet */
         /* we are guranteed to find an entry to store the measurement.    */
         if(MeasurementIDCtr < BCS_SA_BC_MEASUREMENT_LIST_SIZE)
         {
            /* Check if this entry has been used before.  The ID will be   */
            /* zero.                                                       */
            if(BCMeasurementList[Index].Valid == FALSE)
            {
               /* Set the BC Measurement data fields for this index.       */
               BCMeasurementList[Index].Measurement = BC_Measurement;
               BCMeasurementList[Index].Valid       = TRUE;
               BCMeasurementList[Index].ID          = MeasurementIDCtr++;
               BCMeasurementList[Index].Indicated   = FALSE;
      ...
      printf("\r\nOnly the BCS Server can create measurements.\r\n");
...
static int PrintBCMeasurementsCommand(ParameterList_t *TempParam)
{
   ...
   /* Make sure we are the BCS Server.                                  */
   if(BCSInstanceID)
   {
      /* Loop through the BC Measurement list and replace the oldest       */
      /* measurement if necessary.                                         */
      for(Index = 0; Index <= (unsigned int)BCS_SA_BC_MEASUREMENT_LIST_SIZE; Index++)
      {
         /* Only print valid measurements.                              */
         if(BCMeasurementList[Index].Valid)
         {
            printf("\r\nBody Composition Measurement Data:\r\n");
            printf("   ID:                   %u\r\n", BCMeasurementList[Index].ID);
            printf("   Indicated:            %s\r\n", (BCMeasurementList[Index].Indicated) ? "Yes" : "No");
            DisplayBodyCompositionMeasurement(&BCMeasurementList[Index].Measurement);
         }

      }
   }
   else
      printf("\r\nOnly the BCS Server can print measurements.\r\n");
...
static void BTPSAPI GAP_LE_Event_Callback(unsigned int BluetoothStackID, GAP_LE_Event_Data_t *GAP_LE_Event_Data, unsigned long CallbackParameter)
{
   ...
   /* The following is a BCS Server Event Callback.  This function will*/
   /* be called whenever an BCS Server Profile Event occurs that is    */
   /* associated with the specified Bluetooth Stack ID.  This function  */
   /* passes to the caller the Bluetooth Stack ID, the BCS Event Data  */
   /* that occurred and the BCS Event Callback Parameter that was      */
   /* specified when this Callback was The caller is free to installed. */
   /* use the contents of the BCS Event Data ONLY in the context If the*/
   ...
   /* possible (this argument holds anyway because another BCS Event   */
   /* will not be processed while this function call is outstanding).   */
   /* ** NOTE ** This function MUST NOT Block and wait for events that  */
   /*            can only be satisfied by Receiving BCS Event Packets. */
   /*            A Deadlock WILL occur because NO BCS Event Callbacks  */
...
static void BTPSAPI BCS_EventCallback(unsigned int BluetoothStackID, BCS_Event_Data_t *BCS_Event_Data, unsigned long CallbackParameter)
{
   ...
   /* Verify that all parameters to this callback are Semi-Valid.       */
   if((BluetoothStackID) && (BCS_Event_Data))
   {
      /* Switch through the event type.                                 */
      switch(BCS_Event_Data->Event_Data_Type)
      {
         case etBCS_Server_Read_CCCD_Request:
            printf("\r\netBCS_Server_Read_CCCD_Request with size %u.\r\n", BCS_Event_Data->Event_Data_Size);
            if(BCS_Event_Data->Event_Data.BCS_Read_CCCD_Request_Data)
            {
               /* Store event information.                              */
               InstanceID         = BCS_Event_Data->Event_Data.BCS_Read_CCCD_Request_Data->InstanceID;
               ConnectionID       = BCS_Event_Data->Event_Data.BCS_Read_CCCD_Request_Data->ConnectionID;
               TransactionID      = BCS_Event_Data->Event_Data.BCS_Read_CCCD_Request_Data->TransactionID;
               ConnectionType     = BCS_Event_Data->Event_Data.BCS_Read_CCCD_Request_Data->ConnectionType;
               RemoteDevice       = BCS_Event_Data->Event_Data.BCS_Read_CCCD_Request_Data->RemoteDevice;
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, RemoteDevice)) != NULL)
               {
                  ...
                  if((Result = BCS_Read_CCCD_Request_Response(BluetoothStackID, InstanceID, TransactionID, BCS_ERROR_CODE_SUCCESS, DeviceInfo->BCSServerInfo.BC_Measurement_CCCD)) != 0)
                     DisplayFunctionError("BCS_Read_CCCD_Request_Response", Result);
               }
               else
               {
                  printf("\r\nUnknown BCS Client.\r\n");
                  ...
                  if((Result = BCS_Read_CCCD_Request_Response(BluetoothStackID, InstanceID, TransactionID, ATT_PROTOCOL_ERROR_CODE_UNLIKELY_ERROR, 0)) != 0)
                     DisplayFunctionError("BCS_Read_CCCD_Request_Response", Result);
               }
            }
            break;
         case etBCS_Server_Write_CCCD_Request:
            printf("\r\netBCS_Server_Write_CCCD_Request with size %u.\r\n", BCS_Event_Data->Event_Data_Size);
            if(BCS_Event_Data->Event_Data.BCS_Write_CCCD_Request_Data)
            {
               /* Store event information.                              */
               InstanceID          = BCS_Event_Data->Event_Data.BCS_Write_CCCD_Request_Data->InstanceID;
               ConnectionID        = BCS_Event_Data->Event_Data.BCS_Write_CCCD_Request_Data->ConnectionID;
               TransactionID       = BCS_Event_Data->Event_Data.BCS_Write_CCCD_Request_Data->TransactionID;
               ConnectionType      = BCS_Event_Data->Event_Data.BCS_Write_CCCD_Request_Data->ConnectionType;
               RemoteDevice        = BCS_Event_Data->Event_Data.BCS_Write_CCCD_Request_Data->RemoteDevice;
               ClientConfiguration = BCS_Event_Data->Event_Data.BCS_Write_CCCD_Request_Data->ClientConfiguration;
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, RemoteDevice)) != NULL)
               {
                  /* Make sure the BCS Client has disabled or enabled   */
                  /* indications.                                       */
                  if((!ClientConfiguration) || (ClientConfiguration == BCS_CLIENT_CHARACTERISTIC_CONFIGURATION_INDICATE_ENABLE))
                  {
                     /* Store the new configuration.                    */
                     DeviceInfo->BCSServerInfo.BC_Measurement_CCCD = ClientConfiguration;

                     /* Send the write response.                        */
                     if((Result = BCS_Write_CCCD_Request_Response(BluetoothStackID, InstanceID, TransactionID, BCS_ERROR_CODE_SUCCESS)) != 0)
                        DisplayFunctionError("BCS_Write_CCCD_Request_Response", Result);
                  }
                  else
                  {
                     /* Send the error response.                        */
                     if((Result = BCS_Write_CCCD_Request_Response(BluetoothStackID, InstanceID, TransactionID, ATT_PROTOCOL_ERROR_CODE_CCCD_IMPROPERLY_CONFIGURED)) != 0)
                        DisplayFunctionError("BCS_Write_CCCD_Request_Response", Result);
                  }
               }
               else
            ...
            break;
         case etBCS_Confirmation_Data:
            printf("\r\netBCS_Confirmation_Data with size %u.\r\n", BCS_Event_Data->Event_Data_Size);
            if(BCS_Event_Data->Event_Data.BCS_Confirmation_Data)
            {
               /* Print event information.                              */
               InstanceID     = BCS_Event_Data->Event_Data.BCS_Confirmation_Data->InstanceID;
               ConnectionID   = BCS_Event_Data->Event_Data.BCS_Confirmation_Data->ConnectionID;
               TransactionID  = BCS_Event_Data->Event_Data.BCS_Confirmation_Data->TransactionID;
               ConnectionType = BCS_Event_Data->Event_Data.BCS_Confirmation_Data->ConnectionType;
               RemoteDevice   = BCS_Event_Data->Event_Data.BCS_Confirmation_Data->RemoteDevice;
               Status         = BCS_Event_Data->Event_Data.BCS_Confirmation_Data->Status;
               BytesWritten   = BCS_Event_Data->Event_Data.BCS_Confirmation_Data->BytesWritten;
   ...
   {
      /* There was an error with one or more of the input parameters.   */
      printf("\r\nBCS Callback Data: Event_Data = NULL.\r\n");
   }
...
static void BTPSAPI GATT_ClientEventCallback_BCS(unsigned int BluetoothStackID, GATT_Client_Event_Data_t *GATT_Client_Event_Data, unsigned long CallbackParameter)
{
   ...
   /* GATT BCS Client Event Callback data types.                       */
   unsigned int               ConnectionID;
   unsigned int               TransactionID;
   GATT_Connection_Type_t     ConnectionType;
   BD_ADDR_t                  RemoteDevice;
         ...
         case etGATT_Client_Read_Response:
            ...
            if(GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data)
            {
               ...
               if((Value) && (ValueLength != 0) && (CallbackParameter != 0))
               {
                  ...
                  if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, RemoteDevice)) != NULL)
                  {
                     /* Check for the BCS Feature.                      */
                     if(CallbackParameter == DeviceInfo->BCSClientInfo.BC_Feature_Handle)
                     {
                        /* Verify the length.                           */
                        if(ValueLength >= NON_ALIGNED_DWORD_SIZE)
                        {
                           ...
                           DisplayBCSSupportedFeatures(SupportedFeatures);
                        }
                        else
                        {
                     ...
                     {
                        /* Check for the BCS Measurement CCCD.          */
                        if(CallbackParameter == DeviceInfo->BCSClientInfo.BC_Measurement_CCCD_Handle)
                        {
                           /* Verify the length.                        */
               ...
               else
               {
                  ...
                  if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
                  {
                     if(LocalDeviceIsMaster)
                     {
                        ...
                        /* supported.                                   */
                        GATT_Exchange_MTU_Request(BluetoothStackID, ConnectionID, BTPS_CONFIGURATION_GATT_DEFAULT_MAXIMUM_SUPPORTED_MTU_SIZE, GATT_ClientEventCallback_BCS, 0);
                     }
                  }
               }
            }
         ...
         case etGATT_Connection_Server_Indication:
            ...
            if(GATT_Connection_Event_Data->Event_Data.GATT_Server_Indication_Data)
            {
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, RemoteBDADDR)) != NULL)
               {
                  ...
                  if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_BCS_SERVICE_DISCOVERY_COMPLETE)
                  {
                     /* Check if this is an indication for a BCS        */
                     /* Measurement.                                    */
                     if(DeviceInfo->BCSClientInfo.BC_Measurement_Handle == AttributeHandle)
                     {
                        /* Simply call the internal function to decode  */
                        /* the BC Measurement.                          */
                        DecodeDisplayBodyCompositionMeasurement(ValueLength, Value);
                     }
                     else
                        printf("\r\nError - Unknown BCS Attribute handle.\r\n");
                  }
                  else
               ...
               if(GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data)
               {
                  ...
                  switch((Service_Discovery_Type_t)CallbackParameter)
                  {
                     ...
                     case sdGAPS:
                        ...
                        break;
                     case sdBCS:
                        /* Attempt to populate the handles for the BCS */
                        /* Service.                                     */
                        BCSPopulateHandles(&(DeviceInfo->BCSClientInfo), GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data);
                        break;
                     default:
               ...
               if(GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Complete_Data)
               {
                  ...
                  switch((Service_Discovery_Type_t)CallbackParameter)
                  {
                     ...
                     case sdBCS:
                        /* Flag that service discovery has been         */
                        /* performed on for this connection.            */
                        DeviceInfo->Flags |= DEVICE_INFO_FLAGS_BCS_SERVICE_DISCOVERY_COMPLETE;

                        printf("\r\nBCS Service Discovery Summary\r\n\r\n");
                        printf("   BCS Feature             : %s\r\n", (DeviceInfo->BCSClientInfo.BC_Feature_Handle           ? "Supported" : "Not Supported"));
                        printf("   BCS BC Measurement      : %s\r\n", (DeviceInfo->BCSClientInfo.BC_Measurement_Handle       ? "Supported" : "Not Supported"));
                        printf("   BCS BC Measurement CCCD : %s\r\n", (DeviceInfo->BCSClientInfo.BC_Measurement_CCCD_Handle  ? "Supported" : "Not Supported"));
                        break;
                     default:
                        printf("\r\nWarning - Unknown service discovery type.\r\n");
                        break;

...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  HRSAPI - Stonestreet One Bluetooth Heart Rate Service (GATT based) API    */
...
#define HRS_HEART_RATE_CONTROL_POINT_RESET_ENERGY_EXPENDED_SUPPORTED    0x01
   ...
   /* array.                                                            */
   /* * NOTE * The Flags member is made up of bitmasks of the form      */
   /*          HRS_HEART_RATE_MEASUREMENT_FLAGS_XXX.                    */
   /* * NOTE * The Heart_Rate member, that is a word in the structure,  */
   ...
   /* * NOTE * The Energy_Expended member of this structure is ignored  */
   /*          UNLESS the following flag is specified in the Flags      */
   /*          member:                                                  */
   /*          HRS_HEART_RATE_MEASUREMENT_FLAGS_ENERGY_EXPENDED_PRESENT */
   /* * NOTE * The Number_Of_RR_Intervals and RR_Intervals members are  */
...
typedef enum
{
   etHRS_Server_Read_Client_Configuration_Request,
   etHRS_Server_Client_Configuration_Update,
   etHRS_Server_Heart_Rate_Control_Point_Command
} HRS_Event_Type_t;
...
#define HRS_CLIENT_CONFIGURATION_UPDATE_DATA_SIZE       (sizeof(HRS_Client_Configuration_Update_Data_t))
   ...
   /* ConnectionID, ConnectionType, and RemoteDevice specifiy the Client*/
   /* that is making the update.  The final parameter specifies the     */
...
#define HRS_EVENT_DATA_SIZE                             (sizeof(HRS_Event_Data_t))
   ...
   /* the caller the Bluetooth Stack ID, the HRS Event Data that        */
   /* occurred and the HRS Profile Event Callback Parameter that was    */
...
BTPSAPI_DECLARATION unsigned long BTPSAPI HRS_Suspend(unsigned int BluetoothStackID, unsigned long BufferSize, void *Buffer);

#ifdef INCLUDE_BLUETOOTH_API_PROTOTYPES
   typedef unsigned long (BTPSAPI *PFN_HRS_Suspend_t)(unsigned int BluetoothStackID, unsigned long BufferSize, void *Buffer);
#endif
   ...
   /* HRS_Suspend()).  This function accepts as input the Bluetooth     */
   /* Stack ID of the Bluetooth Stack that the Device is associated     */
   /* with.  The final two parameters are the buffer size and buffer    */
   /* that contains the memory that was used to collapse Bluetopia      */
   /* context into with a successfull call to HRS_Suspend().  This      */
   /* function returns ZERO on success or a negative error code.        */
BTPSAPI_DECLARATION int BTPSAPI HRS_Resume(unsigned int BluetoothStackID, unsigned long BufferSize, void *Buffer);

#ifdef INCLUDE_BLUETOOTH_API_PROTOTYPES
   typedef int (BTPSAPI *PFN_HRS_Resume_t)(unsigned int BluetoothStackID, unsigned long BufferSize, void *Buffer);
#endif
...
BTPSAPI_DECLARATION int BTPSAPI HRS_Read_Client_Configuration_Response(unsigned int BluetoothStackID, unsigned int InstanceID, unsigned int TransactionID, Word_t Client_Configuration);
   ...
   /*          parameter.  If this function returns success then on     */

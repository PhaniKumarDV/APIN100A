...
#define GAP_PERIPHERAL_PREFERRED_CONNECTION_PARAMETERS_CHARACTERISTIC_BLUETOOTH_UUID_CONSTANT { 0x04, 0x2A }
   ...
   /* specified UUID_16_t variable.  This MACRO accepts one parameter   */
   /* which is the UUID_16_t variable that is to receive the GAP Central*/
   /* Address Resolution UUID Constant value.                           */
   /* * NOTE * The UUID will be assigned into the UUID_16_t variable in */
   /*          Little-Endian format.                                    */
#define GAP_ASSIGN_CENTRAL_ADDRESS_RESOLUTION_UUID16(_x) ASSIGN_BLUETOOTH_UUID_16((_x), 0x2A, 0xA6)

   /* The following MACRO is a utility MACRO that exist to compare a    */
   /* UUID 16 to the defined GAP Central Address Resolution UUID in     */
   /* UUID16 form.  This MACRO only returns whether the UUID_16_t       */
   /* variable is equal to the GAP Central Address Resolution UUID      */
   /* (MACRO returns boolean result) NOT less than/greater than.  The   */
   /* first parameter is the UUID_16_t variable to compare to the GAP   */
   /* Central Address Resolution UUID.                                  */
#define GAP_COMPARE_GAP_CENTRAL_ADDRESS_RESOLUTION_UUID_TO_UUID_16(_x) COMPARE_BLUETOOTH_UUID_16_TO_CONSTANT((_x), 0x2A, 0xA6)

   /* The following defines the GAP Central Address Resolution UUID that*/
   /* is used when building the GAP Service Table.                      */
#define GAP_CENTRAL_ADDRESS_RESOLUTION_CHARACTERISTIC_BLUETOOTH_UUID_CONSTANT { 0xA6, 0x2A }
...
#define GAP_CENTRAL_ADDRESS_RESOLUTION_VALUE_ADDRESS_RESOLUTION_NOT_SUPPORTED    0x00
#define GAP_CENTRAL_ADDRESS_RESOLUTION_VALUE_ADDRESS_RESOLUTION_SUPPORTED        0x01

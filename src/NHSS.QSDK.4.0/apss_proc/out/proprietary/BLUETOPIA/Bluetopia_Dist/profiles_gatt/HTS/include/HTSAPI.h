...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  HTSAPI - Stonestreet One Bluetooth Health Thermometer Service (GATT       */
...
typedef enum
{
   etHTS_Server_Read_Client_Configuration_Request,
   etHTS_Server_Client_Configuration_Update,
   etHTS_Measurement_Interval_Update,
   etHTS_Confirmation_Response
} HTS_Event_Type_t;
...
#define HTS_EVENT_DATA_SIZE                             (sizeof(HTS_Event_Data_t))
   ...
   /* the caller the Bluetooth Stack ID, the HTS Event Data that        */
   /* occurred and the HTS Profile Event Callback Parameter that was    */
...
BTPSAPI_DECLARATION unsigned long BTPSAPI HTS_Suspend(unsigned int BluetoothStackID, unsigned long BufferSize, void *Buffer);

#ifdef INCLUDE_BLUETOOTH_API_PROTOTYPES
   typedef unsigned long (BTPSAPI *PFN_HTS_Suspend_t)(unsigned int BluetoothStackID, unsigned long BufferSize, void *Buffer);
#endif
   ...
   /* HTS_Suspend()).  This function accepts as input the Bluetooth     */
   /* Stack ID of the Bluetooth Stack that the Device is associated     */
   /* with.  The final two parameters are the buffer size and buffer    */
   /* that contains the memory that was used to collapse Bluetopia      */
   /* context into with a successfull call to HTS_Suspend().  This      */
   /* function returns ZERO on success or a negative error code.        */
BTPSAPI_DECLARATION int BTPSAPI HTS_Resume(unsigned int BluetoothStackID, unsigned long BufferSize, void *Buffer);

#ifdef INCLUDE_BLUETOOTH_API_PROTOTYPES
   typedef int (BTPSAPI *PFN_HTS_Resume_t)(unsigned int BluetoothStackID, unsigned long BufferSize, void *Buffer);
#endif

...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXBLP - Linux Bluetooth Blood Pressure Profile using GATT (LE)         */
/*             application.                                                   */
/*                                                                            */
/*  Author:  Cliff Xu                                                         */
/*                                                                            */
...
#include "LinuxBLP.h"            /* Application Header.                       */
...
#include "SS1BTBLS.h"            /* Main SS1 BLS Service Header.              */
...
#define MAX_COMMAND_LENGTH                        (100)  /* Denotes the max   */
                                                         /* buffer size used  */
                                                         /* for user commands */
                                                         /* input via the     */
                                                         /* User Interface.   */
...
#define DEFAULT_DEBUG_LOG_FILE_NAME                "LinuxBLP_ASC.log"
#define DEFAULT_DEBUG_FTS_FILE_NAME                "LinuxBLP_FTS.log"

   /* Determine the Name we will use for this compilation.              */
#define LE_DEMO_DEVICE_NAME                        "LinuxBLP"
...
typedef struct _tagDeviceInfo_t
{
   ...
   BLS_Client_Information_t ClientInfo;
   BLS_Server_Information_t ServerInfo;
...
static unsigned int        BLSInstanceID;           /* The following holds the BLS     */
                                                    /* Instance ID that is returned    */
                                                    /* from BLS_Initialize_Service().  */
...
static BTPSCONST char *IOCapabilitiesStrings[] =
{
   ...
   /* The following string table is used to map the Blood Pressure      */
   /* Feature values to an easily displayable string.                   */
static BTPSCONST char *BLSFeatureCommand[] =
{
...
#define NUM_BLS_FEATURE_COMMANDS                 ((sizeof(BLSFeatureCommand)/sizeof(char *)) - 1)

   /* The following string table is used to map the Blood Pressure      */
   /* Measurement Flag values to an easily displayable string.          */
static BTPSCONST char *BPMeasurementFlags[] =
{
   "Blood Pressure Units in KPA",
   "Time Stamp Present",
   "Pulse Rate Present",
   "UserID Present",
...
#define NUM_BP_MEASUREMENT_FLAGS                 ((sizeof(BPMeasurementFlags)/sizeof(char *)) - 1)

   /* The following correspond to the current Reserved for Future Use   */
   /* bits in the Measurement Flag field.                               */
#define BLS_MEASUREMENT_FLAGS_RFU_BIT_5                               0x20
#define BLS_MEASUREMENT_FLAGS_RFU_BIT_6                               0x40
#define BLS_MEASUREMENT_FLAGS_RFU_BIT_7                               0x80
...
static int SetGATTMTU(ParameterList_t *TempParam);
   ...
   /* Link BLS Service Profile Function Commands.                       */
static int RegisterBLS(ParameterList_t *TempParam);
static int UnregisterBLS(ParameterList_t *TempParam);
static int DiscoverBLS(ParameterList_t *TempParam);
static int SetBloodPressureFeature(ParameterList_t *TempParam);
static int GetBloodPressureFeature(ParameterList_t *TempParam);
static int IndicateBloodPressureMeasurement(ParameterList_t *TempParam);
static int NotifyIntermediateCuffPressure(ParameterList_t *TempParam);
static int ConfigureRemoteBLS(ParameterList_t *TempParam);

   /* BLS Service Helper Functions                                      */
static void BLSPopulateHandles(DeviceInfo_t *DeviceInfo, GATT_Service_Discovery_Indication_Data_t *ServiceDiscoveryData);
static void DisplayBloodPressureFeatureCommandUsage(void);
static void DisplayBloodPressureFeature(Word_t SupportedFeature);

static void DisplayBloodPressureMeasurement(BLS_Blood_Pressure_Measurement_Data_t *BloodPressureMeasurement);
static void DisplayBloodPressureMeasurementUsage(char* CharacteristicName);
static void DecodeDisplayBloodPressureMeasurement(Word_t BufferLength, Byte_t* Buffer);
static int EnableDisableNotificationsIndications(Word_t ClientConfigurationHandle, Word_t ClientConfigurationValue, GATT_Client_Event_Callback_t ClientEventCallback);
static void DisplayDISCharacteristicValue(char* Value);

   /* Helper functions added to display PulseRate in float format       */
static float Power(Word_t X, SWord_t Y);
static float Word_To_Float(Word_t value);

   /* BTPS BLS Callback function prototypes.                            */
...
static void BTPSAPI BLS_EventCallback(unsigned int BluetoothStackID, BLS_Event_Data_t *BLS_Event_Data, unsigned long CallbackParameter);
static void BTPSAPI GATT_ClientEventCallback_BLS(unsigned int BluetoothStackID, GATT_Client_Event_Data_t *GATT_Client_Event_Data, unsigned long CallbackParameter);
...
static void UserInterface(void)
{
   ...
   AddCommand("REGISTERBLS",   RegisterBLS);
   AddCommand("UNREGISTERBLS", UnregisterBLS);
   AddCommand("DISCOVERBLS", DiscoverBLS);
   AddCommand("CONFIGUREREMOTEBLS", ConfigureRemoteBLS);
   AddCommand("SETBLOODPRESSUREFEATURE", SetBloodPressureFeature);
   AddCommand("GETBLOODPRESSUREFEATURE", GetBloodPressureFeature);
   AddCommand("INDICATEBLOODPRESSUREMEASUREMENT", IndicateBloodPressureMeasurement);
   AddCommand("NOTIFYINTERMEDIATECUFFPRESSURE", NotifyIntermediateCuffPressure);
...
static int CloseStack(void)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(BLSInstanceID)
      {
        BLS_Cleanup_Service(BluetoothStackID, BLSInstanceID);
...
static void GAPSPopulateHandles(GAPS_Client_Info_t *ClientInfo, GATT_Service_Discovery_Indication_Data_t *ServiceInfo)
{
   ...
   /* The following function is a utility function that provides a      */
   /* mechanism of populating an BLS Client Information structure with  */
...
static void BLSPopulateHandles(DeviceInfo_t *DeviceInfo, GATT_Service_Discovery_Indication_Data_t *ServiceDiscoveryData)
{
   ...
   /* Verify that the input parameters are semi-valid.                  */
   if((DeviceInfo) && (ServiceDiscoveryData) && (ServiceDiscoveryData->ServiceInformation.UUID.UUID_Type == guUUID_16) && (BLS_COMPARE_BLS_SERVICE_UUID_TO_UUID_16(ServiceDiscoveryData->ServiceInformation.UUID.UUID.UUID_16)))
   {
      /* Loop through all characteristics discovered in the service     */
      /* and populate the correct entry.                                */
      ...
      if(CurrentCharacteristic)
      {
         ...
         {
            /* All BLS UUIDs are defined to be 16 bit UUIDs.            */
            if(CurrentCharacteristic->Characteristic_UUID.UUID_Type == guUUID_16)
            {
               /* verify Blood Pressure Feature .                       */

               if(BLS_COMPARE_BLOOD_PRESSURE_FEATURE_UUID_TO_UUID_16(CurrentCharacteristic->Characteristic_UUID.UUID.UUID_16))
               {
                  DeviceInfo->ClientInfo.Blood_Pressure_Feature = CurrentCharacteristic->Characteristic_Handle;
                  ...
                  if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_READ))
                     printf("Warning - Mandatory Read property of Blood Pressure Feature characteristic not supported!\r\n");
               }
               else
               {
                  /* verify Blood Pressure Measurement.                 */
                  if(BLS_COMPARE_BLOOD_PRESSURE_MEASUREMENT_UUID_TO_UUID_16(CurrentCharacteristic->Characteristic_UUID.UUID.UUID_16))
                  {
                     DeviceInfo->ClientInfo.Blood_Pressure_Measurement = CurrentCharacteristic->Characteristic_Handle;

                     /* Verify that write without response is supported.*/
                     if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_INDICATE))
                        printf("Warning - Mandatory Indicate property of Blood Pressure Measurement characteristic not supported!\r\n");

                     CurrentDescriptor = CurrentCharacteristic->DescriptorList;
                     /* Get Blood Pressure Measurment Descriptor        */
                     for(Index2 = 0; Index2 < CurrentCharacteristic->NumberOfDescriptors; Index2++)
                     {
                        if(CurrentDescriptor->Characteristic_Descriptor_UUID.UUID_Type == guUUID_16)
                        {
                           ...
                           {
                              DeviceInfo->ClientInfo.Blood_Pressure_Measurement_Client_Configuration = CurrentCharacteristic->DescriptorList[Index2].Characteristic_Descriptor_Handle;
                           }
                        }
                     }
                  }
                  ...
                  {
                        /* verify Blood Pressure Measurement.           */
                     if(BLS_COMPARE_INTERMEDIATE_CUFF_PRESSURE_UUID_TO_UUID_16(CurrentCharacteristic->Characteristic_UUID.UUID.UUID_16))
                     {
                        DeviceInfo->ClientInfo.Intermediate_Cuff_Pressure = CurrentCharacteristic->Characteristic_Handle;

                        /* Verify write without response is supported.  */
                        if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_NOTIFY))
                           printf("Warning - Mandatory Notify property of Intermediate Cuff Pressure characteristic not supported!\r\n");

                         CurrentDescriptor = CurrentCharacteristic->DescriptorList;
                        /* Get Intermediate Cuff Pressure  Descriptor   */
                        for(Index2 = 0; Index2 < CurrentCharacteristic->NumberOfDescriptors; Index2++)
                        {
                           if(CurrentDescriptor->Characteristic_Descriptor_UUID.UUID_Type == guUUID_16)
                           {
                              if(GATT_COMPARE_CLIENT_CHARACTERISTIC_CONFIGURATION_ATTRIBUTE_TYPE_TO_BLUETOOTH_UUID_16(CurrentCharacteristic->DescriptorList[Index2].Characteristic_Descriptor_UUID.UUID.UUID_16))
                              {
                                 DeviceInfo->ClientInfo.Intermediate_Cuff_Pressure_Client_Configuration = CurrentCharacteristic->DescriptorList[Index2].Characteristic_Descriptor_Handle;
                              }
                           }
                        }
                     }
...
static int EncryptionInformationRequestResponse(BD_ADDR_t BD_ADDR, Byte_t KeySize, GAP_LE_Authentication_Response_Information_t *GAP_LE_Authentication_Response_Information)
{
   ...
   /* information for a command line Blood Pressure Feature bitmask     */
   /* input field.                                                      */
static void DisplayBloodPressureFeatureCommandUsage(void)
{
   unsigned int Index;

   printf("   Where setting a bit BitMask enables the Blood Pressure Feature.\r\n");
   printf("\r\n   Bit  Blood Pressure Feature.\r\n   ---  ------------\r\n");

   for(Index = 0; Index <= NUM_BLS_FEATURE_COMMANDS; Index++)
      printf("     %d  %s\r\n", Index, BLSFeatureCommand[Index]);

   printf("\r\n");
}

   /* The following function is used to Display Blood Pressure          */
   /* Feature data. The first parameter provides the BLS Status data to */
   /* be displayed.                                                     */
static void DisplayBloodPressureFeature(Word_t SupportedFeature)
{
   ...
   printf("\r\nSupported Blood Pressure Feature Commands\r\n");
   printf("----------------------------------------------\r\n");

   for(CommandIndex = 0; CommandIndex <= NUM_BLS_FEATURE_COMMANDS; CommandIndex++)
   {
      printf("Commands   %s: ", BLSFeatureCommand[CommandIndex]);

      for(Index = 0; Index < (BTPS_StringLength(BLSFeatureCommand[4]) - BTPS_StringLength(BLSFeatureCommand[CommandIndex])); Index++)
         printf(" ");

      printf("%sSupported\r\n", ((SupportedFeature & (0x1 << CommandIndex))? "" : "Not "));
   }
   ...
   /* Helper function to calculate X to the power Y                     */
   /* This function is used to calculate exponent part of PulseRate     */
static float Power(Word_t X, SWord_t Y)
{
   SWord_t  sign   = 1;
   float    Result = 1;
   ...
   /* Helper function to covert Word_t to Float                         */
   /* This function is used to convert 16 bit IEEE Float value i.e.     */
   /* Pulse Rate value to compiler specific float value                 */
float Word_To_Float(Word_t value)
{
   float FloatValue = 0;

   /* Read 12 bit Mantissa                                              */
   SWord_t m = value & 0x0FFF;

   /* Read 4 bit exponent                                               */
   SWord_t e = (value & 0xF000)>>12;

   /* signed bit is 1                                                   */
   if(m & 0x800)
   {
      /* invert all the 12 bits                                         */
      m = (~m) & 0x0FFF;
   }
   ...
   if(e & 0x8)
   {
      /* e is in 2's complement form                                    */
      /* convert e to 1's complement form                               */

      /* Step 1. invert all the 4 bits                                  */
      e = (~e) & 0x000F;
      /* Step 2. add 1                                                  */
      e = e+1;
      /* Step 3. multiply -1                                            */
...
static void DisplayBloodPressureMeasurement(BLS_Blood_Pressure_Measurement_Data_t *BloodPressureMeasurement)
{
   printf("   Flags:                  0x%02X\r\n", BloodPressureMeasurement->Flags);
   printf("   CompoundValue\r\n");

   if(BloodPressureMeasurement->Flags & BLS_MEASUREMENT_FLAGS_BLOOD_PRESSURE_UNITS_IN_KPA)
      printf("    (kPA)");
   else
      printf("   (mmHg)");

   if(BloodPressureMeasurement->CompoundValue.Systolic != BLS_SFLOAT_NOT_A_NUMBER)
   {
      printf("Systolic: %.1f     ", Word_To_Float(BloodPressureMeasurement->CompoundValue.Systolic));
   }
   else
   {
      printf("Systolic: NaN     ");
   ...
   if(BloodPressureMeasurement->CompoundValue.Diastolic != BLS_SFLOAT_NOT_A_NUMBER)
   {
      printf("Diastolic: %.1f      ", Word_To_Float(BloodPressureMeasurement->CompoundValue.Diastolic));
   }
   else
   {
      printf("Diastolic: NaN      ");
   }

   if(BloodPressureMeasurement->CompoundValue.Mean_Arterial_Pressure != BLS_SFLOAT_NOT_A_NUMBER)
   {
      printf("MAP: %.1f\r\n", Word_To_Float(BloodPressureMeasurement->CompoundValue.Mean_Arterial_Pressure));
   }
   else
   {
      printf("MAP: NaN\r\n");
   ...
   if(BloodPressureMeasurement->Flags & BLS_MEASUREMENT_FLAGS_TIME_STAMP_PRESENT)
   {
       printf("   TimeStamp:              %u/%02u/%02u %02u:%02u:%02u\r\n", BloodPressureMeasurement->TimeStamp.Year,
                                                                              BloodPressureMeasurement->TimeStamp.Month,
                                                                              BloodPressureMeasurement->TimeStamp.Day,
                                                                              BloodPressureMeasurement->TimeStamp.Hours,
                                                                              BloodPressureMeasurement->TimeStamp.Minutes,
                                                                              BloodPressureMeasurement->TimeStamp.Seconds);
   }
   else
      printf("   TimeStamp:              Not present\r\n");

   if(BloodPressureMeasurement->Flags & BLS_MEASUREMENT_FLAGS_PULSE_RATE_PRESENT)
   {
      switch(BloodPressureMeasurement->PulseRate)
      {
         case BLS_SFLOAT_NOT_A_NUMBER:
            printf("   PulseRate:              NaN\r\n");
            break;
         case BLS_SFLOAT_NOT_AT_THIS_RESOLUTION:
            printf("   PulseRate:              NRes\r\n");
            break;
         case BLS_SFLOAT_POSITIVE_INFINITY:
            printf("   PulseRate:              +INFINITY\r\n");
            break;
         case BLS_SFLOAT_NEGATIVE_INFINITY:
            printf("   PulseRate:              -INFINITY\r\n");
            break;
         default:
            printf("   PulseRate:              %.1f\r\n", Word_To_Float(BloodPressureMeasurement->PulseRate));
            break;
      }
   }
   else
      printf("   PulseRate:              Not Present\r\n");

   if(BloodPressureMeasurement->Flags & BLS_MEASUREMENT_FLAGS_USER_ID_PRESENT)
      printf("   UserID:                 %u\r\n",BloodPressureMeasurement->UserID);
   else
      printf("   UserID:                 Not present\r\n");

   if(BloodPressureMeasurement->Flags & BLS_MEASUREMENT_FLAGS_MEASUREMENT_STATUS_PRESENT)
      printf("   MeasurementStatus:      0x%04X\r\n",BloodPressureMeasurement->MeasurementStatus);
   else
      printf("   MeasurementStatus:      Not present\r\b");

   if((BloodPressureMeasurement->Flags & BLS_MEASUREMENT_FLAGS_RFU_BIT_5) || (BloodPressureMeasurement->Flags & BLS_MEASUREMENT_FLAGS_RFU_BIT_6) || (BloodPressureMeasurement->Flags & BLS_MEASUREMENT_FLAGS_RFU_BIT_7))
   {
      printf("   (Optional)   Additional Octets:      0x%02X\r\n",BloodPressureMeasurement->Reserved[0]);
      printf("   (Optional)   Additional Octets:      0x%02X\r\n",BloodPressureMeasurement->Reserved[1]);
   }
}
...
static void DisplayBloodPressureMeasurementUsage(char* CharacteristicName)
{
   ...
   printf("Usage: %s\r\n", CharacteristicName);
   printf("   [CompoundValue.Diastolic]\r\n");
   printf("   [CompoundValue.Mean_Arterial_Pressure]\r\n");
   printf("   [CompoundValue.Systolic]\r\n");
   printf("   [Flags (BitMask)], [MeasurementStatus], [PulseRate],\r\n");
   printf("   [Day], [Month], [Year], [Hours], [Minutes], [Seconds],\r\n");
   printf("   [UserID].\r\n");
   printf("   [(Optional) Octet1].\r\n");
   printf("   [(Optional) Octet2].\r\n\r\n");

   printf("   Where setting a bit in BitMask enables the Measurement Flags.\r\n");
   printf("\r\n   Bit  Measurement Flags\r\n   ---  ----------------\r\n");

   for(Index = 0; Index <= NUM_BP_MEASUREMENT_FLAGS; Index++)
      printf("     %d  %s\r\n", Index, BPMeasurementFlags[Index]);
...
static void DecodeDisplayBloodPressureMeasurement(Word_t BufferLength, Byte_t* Buffer)
{
   ...
   {
      /* Decode the Blood Pressure Measurement.                         */
      Result = BLS_Decode_Blood_Pressure_Measurement(BufferLength, Buffer, &BloodPressureMeasurement);
      if(Result == 0)
      {
         DisplayBloodPressureMeasurement(&BloodPressureMeasurement);
      }
      else
         DisplayFunctionError("BLS_Decode_Blood_Pressure_Measurement", Result);
   }
   else
   {
      printf("Invalid Blood Pressure Measurement Response buffer.\r\n");
   }
}
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("* Command Options BLS:     RegisterBLS, UnRegisterBLS,           *\r\n");
   printf("*                          DiscoverBLS,                          *\r\n");
   printf("*                          ConfigureRemoteBLS,                   *\r\n");
   printf("*                          SetBloodPressureFeature,              *\r\n");
   printf("*                          GetBloodPressureFeature,              *\r\n");
   printf("*                          IndicateBloodPressureMeasurement,     *\r\n");
   printf("*                          NotifyIntermediateCuffPressure        *\r\n");
   printf("******************************************************************\r\n");
   ...
   if((BluetoothStackID) && (!ConnectionID))
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].intParam >= 0) && (TempParam->Params[0].intParam <= 1))
      {
         ...
         else
         {
            ...
            if(BLSInstanceID)
            {
               /* Advertise the BLS Server (1 byte type & 2 bytes UUID).*/
               ...
               BLS_ASSIGN_BLS_SERVICE_UUID_16(&(Advertisement_Data_Buffer.AdvertisingData.Advertising_Data[5]));
            }
...
static int EnableDebug(ParameterList_t *TempParam)
{
   ...
   /* The following function is responsible for registering a BLS       */
...
static int RegisterBLS(ParameterList_t *TempParam)
{
   ...
   if(!ConnectionID)
   {
      ...
      if(!BLSInstanceID)
      {
         /* Register the BLS Service with GATT.                         */
         ret_val = BLS_Initialize_Service(BluetoothStackID,BLS_EventCallback,0,&BLSInstanceID);
         if((ret_val > 0) && (BLSInstanceID > 0))
         {
            /* Display success message.                                 */
            printf("Successfully registered BLS Service.\r\n");

            /* Save the ServiceID of the registered service.            */
            BLSInstanceID  = (unsigned int)ret_val;
            ret_val        = 0;
         }
      }
      else
   ...
   /* The following function is responsible for unregistering a BLS     */
   /* Service.  This function will return zero on successful execution  */
   /* and a negative value on errors.                                   */
static int UnregisterBLS(ParameterList_t *TempParam)
{
   ...
   if(BLSInstanceID)
   {
      ...
      /* Unregister the BLS Service with GATT.                          */
      ret_val = BLS_Cleanup_Service(BluetoothStackID, BLSInstanceID);
      if(ret_val == 0)
      {
         /* Display success message.                                    */
         printf("Successfully unregistered BLS Service.\r\n");

         /* Save the ServiceID of the registered service.               */
         BLSInstanceID = 0;
      }
      else
         DisplayFunctionError("BLS_Cleanup_Service", ret_val);
   }
   else
      printf("BLS Service not registered.\r\n");

   return(ret_val);
}

   /* The following function is responsible for performing a BLS        */
   /* Service Discovery Operation.  This function will return zero on   */
   /* successful execution and a negative value on errors.              */
static int DiscoverBLS(ParameterList_t *TempParam)
{
   ...
   if(!BLSInstanceID)
   {
      ...
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            if(!(DeviceInfo->Flags & DEVICE_INFO_FLAGS_SERVICE_DISCOVERY_OUTSTANDING))
            {
               /* Configure the filter so that only the BLS Service is  */
               /* discovered.                                           */
               UUID[0].UUID_Type = guUUID_16;
               BLS_ASSIGN_BLS_SERVICE_UUID_16(&(UUID[0].UUID.UUID_16));

               /* Start the service discovery process.                  */
               ret_val = GATT_Start_Service_Discovery(BluetoothStackID, ConnectionID, (sizeof(UUID)/sizeof(GATT_UUID_t)), UUID,  GATT_Service_Discovery_Event_Callback, sdBLS);
               if(!ret_val)
               {
                  /* Display success message.                           */
                  printf("GATT_Service_Discovery_Start success.\r\n");
   ...
   /* The following function is responsible for configure a BLS Service */
   /* on a remote device.  This function will return zero on successful */
   /* execution and a negative value on errors.                         */
static int ConfigureRemoteBLS(ParameterList_t *TempParam)
{
   DeviceInfo_t *DeviceInfo;
   int           measurement_ret_val = FUNCTION_ERROR;
   int           cuff_ret_val        = FUNCTION_ERROR;
   int           ret_val             = FUNCTION_ERROR;
      ...
      if(!BLSInstanceID)
      {
         ...
         if(ConnectionID)
         {
            ...
            if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
            {
               ...
               {
                  ret_val             = 0;
                  measurement_ret_val = 0;
                  cuff_ret_val        = 0;

                  printf("Attempting to configure CCCDs...\r\n");

                  /* Determine if Blood Pressure Measurement CC is      */
                  /* supported (mandatory).                             */
                  if(DeviceInfo->ClientInfo.Blood_Pressure_Measurement_Client_Configuration)
                  {
                     measurement_ret_val = EnableDisableNotificationsIndications(DeviceInfo->ClientInfo.Blood_Pressure_Measurement_Client_Configuration, (TempParam->Params[0].intParam ? GATT_CLIENT_CONFIGURATION_CHARACTERISTIC_INDICATE_ENABLE : 0), GATT_ClientEventCallback_BLS);
                  }

                  if(measurement_ret_val > 0)
                  {
                     printf("BLS Blood Pressure Measurement Configuration Success.\r\n");
                  ...
                  else
                  {
                     ...
                     if(measurement_ret_val < 0)
                     {
                        printf("EnableDisableNotificationsIndications %d", measurement_ret_val);
                     }

                     measurement_ret_val = FUNCTION_ERROR;
                  }

                  /* Determine if  Cuff Pressure is supported           */
                  /* (mandatory).                                       */
                  if(DeviceInfo->ClientInfo.Intermediate_Cuff_Pressure_Client_Configuration)
                  {
                     cuff_ret_val =  EnableDisableNotificationsIndications(DeviceInfo->ClientInfo.Intermediate_Cuff_Pressure_Client_Configuration, (TempParam->Params[1].intParam ? GATT_CLIENT_CONFIGURATION_CHARACTERISTIC_NOTIFY_ENABLE : 0), GATT_ClientEventCallback_BLS);
                  }

                  if(cuff_ret_val > 0)
                  {
                     printf("BLS Intermediate Cuff Pressure Configuration Success.\r\n");
                     cuff_ret_val = 0;
                  }
                  else
                  {
                     ...
                     if(cuff_ret_val < 0)
                     {
                        DisplayFunctionError("EnableDisableNotificationsIndications", cuff_ret_val);
                     }

                     cuff_ret_val = FUNCTION_ERROR;
                  }
               }
               else
         ...
         printf("Cannot configure remote BLS Services when registered as a service.\r\n");
   }
   else
   {
      printf("Usage: ConfigureRemoteBLS [Indicate Blood Pressure Measurement (0 = disable, 1 = enable)]\r\n");
      printf("                          [Notify Intermediate Cuff Pressure   (0 = disable, 1 = enable)]\r\n");
   }

   if((measurement_ret_val == 0) && (cuff_ret_val == 0))
   {
      ret_val = 0;
   }
   else
   ...
   /* The following function is responsible for writing the Blood       */
   /* Pressure Feature. It can be executed only by a server. This       */
   /* function will return zero on successful execution and a negative  */
   /* value on errors                                                   */
static int SetBloodPressureFeature(ParameterList_t *TempParam)
{
   Word_t SupportedFeature;
   int    ret_val = FUNCTION_ERROR;

   /* Verify that the input parameters are semi-valid.                  */
   if((TempParam) && (TempParam->NumberofParameters >= 1))
   {
      /* Check for a registered BLS Server                              */
      if(BLSInstanceID)
      {
         SupportedFeature = (Word_t)TempParam->Params[0].intParam;
         ret_val = BLS_Set_Blood_Pressure_Feature(BluetoothStackID, BLSInstanceID, SupportedFeature);

         if (ret_val == 0)
         {
            printf("BLS Blood Pressure Feature successfully set\r\n");
         }
         else
            printf("BLS_Blood_Pressure_Feature_Data %d", ret_val);
      }
      else
      {
         if(ConnectionID)
            printf("Cannot write BLS Feature data as a client.\r\n");
         else
            printf("BLS server not registered.\r\n");
      }
   }
   else
   {
      printf("SetBloodPressureFeature    [BitMask (0xXXXX)].\r\n");
      DisplayBloodPressureFeatureCommandUsage();
   }
   ...
   /* The following function is responsible for reading the Blood       */
   /* Pressure Feature. It can be executed by a server or as a client   */
...
static int GetBloodPressureFeature(ParameterList_t *TempParam)
{
   ...
   /* First check for a registered BLS Server                           */
   if(BLSInstanceID)
   {
      ret_val = BLS_Query_Blood_Pressure_Feature(BluetoothStackID, BLSInstanceID, &SupportedFeature);
      if(ret_val == 0)
      {
         DisplayBloodPressureFeature(SupportedFeature);
      }
      else
         printf("BLS_Query_Blood_Pressure_Feature_Data %d", ret_val);
   }
   else
   {
      ...
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR))!= NULL)
         {
            /* Verify that the client has received a valid BLS Blood    */
            /* Pressure Feature Information Attribute Handle.           */
            if(DeviceInfo->ClientInfo.Blood_Pressure_Feature >= 0)
            {
               /* Finally, submit a read request to the server          */
               if((ret_val = GATT_Read_Value_Request(BluetoothStackID, ConnectionID, DeviceInfo->ClientInfo.Blood_Pressure_Feature, GATT_ClientEventCallback_BLS, DeviceInfo->ClientInfo.Blood_Pressure_Feature)) > 0)
               {
                  printf("Get BLS Blood Pressure feature Information Request sent, Transaction ID = %u", ret_val);
               ...
               printf("Error - BLS Blood Pressure Feature attribute handle is invalid!\r\n");
         }
         else
            printf("No Device Info.\r\n");
      }
      else
         printf("Either a BLS server must be registered or a BLS client must be connected.\r\n");
   }
   ...
   /* The following function is responsible for performing a BLS        */
   /* Measurement data Indication to a connected remote device.         */
   /* This function will return zero on successful execution            */
   /* and a negative value on errors.                                   */
static int IndicateBloodPressureMeasurement(ParameterList_t *TempParam)
{
   BLS_Blood_Pressure_Measurement_Data_t  Measurement;
   DeviceInfo_t                          *DeviceInfo;
   int                                    ret_val = FUNCTION_ERROR;

   if((TempParam) && (TempParam->NumberofParameters > 12))
   {
      /* First, check that valid Bluetooth Stack ID exists.             */
      if(BluetoothStackID)
      {
         ...
         if(BLSInstanceID)
         {
            if((ConnectionID) && (!COMPARE_NULL_BD_ADDR(ConnectionBD_ADDR)))
            {
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
               {
                  ...
                  /* indication.                                        */
                  if(DeviceInfo->ServerInfo.Blood_Pressure_Measurement_Client_Configuration & GATT_CLIENT_CONFIGURATION_CHARACTERISTIC_INDICATE_ENABLE)
                  {
                     /* Initialize Global BLS Measurement Object        */
                     BTPS_MemInitialize(&Measurement, 0, BLS_BLOOD_PRESSURE_MEASUREMENT_DATA_SIZE);

                     Measurement.CompoundValue.Diastolic               = (Word_t)TempParam->Params[0].intParam;
                     Measurement.CompoundValue.Mean_Arterial_Pressure  = (Word_t)TempParam->Params[1].intParam;
                     Measurement.CompoundValue.Systolic                = (Word_t)TempParam->Params[2].intParam;
                     Measurement.Flags                                 = (Byte_t)TempParam->Params[3].intParam;
                     Measurement.MeasurementStatus                     = (Word_t)TempParam->Params[4].intParam;
                     Measurement.PulseRate                             = (Word_t)TempParam->Params[5].intParam;
                     Measurement.TimeStamp.Day                         = (Byte_t)TempParam->Params[6].intParam;
                     Measurement.TimeStamp.Month                       = (Byte_t)TempParam->Params[7].intParam;
                     Measurement.TimeStamp.Year                        = (Word_t)TempParam->Params[8].intParam;
                     Measurement.TimeStamp.Hours                       = (Byte_t)TempParam->Params[9].intParam;
                     Measurement.TimeStamp.Minutes                     = (Byte_t)TempParam->Params[10].intParam;
                     Measurement.TimeStamp.Seconds                     = (Byte_t)TempParam->Params[11].intParam;
                     Measurement.UserID                                = (Byte_t)TempParam->Params[12].intParam;

                     if(TempParam->NumberofParameters > 13)
                     {
                        Measurement.Reserved[0]                        = (Byte_t)TempParam->Params[13].intParam;
                     }
                     if(TempParam->NumberofParameters > 14)
                     {
                        Measurement.Reserved[1]                        = (Byte_t)TempParam->Params[14].intParam;
                     }

                     /* Go ahead and send the BLS Measurement data.     */
                     if((ret_val = BLS_Indicate_Blood_Pressure_Measurement(BluetoothStackID, BLSInstanceID, ConnectionID, &Measurement)) == 0)
                     {
                        printf("BLS Blood Pressure Measurement Indication success.\r\n");
                        ret_val = 0;
                     }
                     else
                        DisplayFunctionError("BLS_Indicate_Blood_Pressure_Measurement", ret_val);
                  }
                  else
                  {
                     printf("Client has not registered for BLS Blood Pressure Measurement indication.\r\n");
                     ret_val = 0;
                  }
               }
               else
         ...
         else
         {
            if(ConnectionID)
               ...
               printf("Error - BLS server not registered\r\n");
         }
      }
      else
      {
   ...
   {
      DisplayBloodPressureMeasurementUsage("IndicateBloodPressureMeasurement");
   }
   ...
   /* The following function is responsible for performing a BLS        */
   /* Intermediate Cuff Pressure Notification to a connected remote     */
   /* device. This function will return zero on successful execution    */
   /* and a negative value on errors.                                   */
static int NotifyIntermediateCuffPressure(ParameterList_t *TempParam)
{
   BLS_Blood_Pressure_Measurement_Data_t  CuffPressureData;
   DeviceInfo_t                          *DeviceInfo;
   int                                    ret_val = FUNCTION_ERROR;
         ...
         if(BLSInstanceID)
         {
            if((ConnectionID) && (!COMPARE_NULL_BD_ADDR(ConnectionBD_ADDR)))
            {
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
               {
                  /* Verify  that  the  client  has  registered  for    */
                  /* notifications.                                     */
                  if((!BTPS_CONFIGURATION_BLS_SUPPORT_INTERMEDIATE_CUFF_PRESSURE) ||(DeviceInfo->ServerInfo.Intermediate_Cuff_Pressure_Client_Configuration & GATT_CLIENT_CONFIGURATION_CHARACTERISTIC_NOTIFY_ENABLE))
                  {
                     /* Initialize Global BLS Cuff Pressure Data        */
                     BTPS_MemInitialize(&CuffPressureData, 0, BLS_BLOOD_PRESSURE_MEASUREMENT_DATA_SIZE);

                     CuffPressureData.CompoundValue.Diastolic               = (Word_t) TempParam->Params[0].intParam;
                     CuffPressureData.CompoundValue.Mean_Arterial_Pressure  = (Word_t) TempParam->Params[1].intParam;
                     CuffPressureData.CompoundValue.Systolic                = (Word_t) TempParam->Params[2].intParam;
                     CuffPressureData.Flags                                 = (Byte_t) TempParam->Params[3].intParam;
                     CuffPressureData.MeasurementStatus                     = (Word_t) TempParam->Params[4].intParam;
                     CuffPressureData.PulseRate                             = (Word_t) TempParam->Params[5].intParam;
                     CuffPressureData.TimeStamp.Day                         = (Byte_t) TempParam->Params[6].intParam;
                     CuffPressureData.TimeStamp.Month                       = (Byte_t) TempParam->Params[7].intParam;
                     CuffPressureData.TimeStamp.Year                        = (Word_t) TempParam->Params[8].intParam;
                     CuffPressureData.TimeStamp.Hours                       = (Byte_t) TempParam->Params[9].intParam;
                     CuffPressureData.TimeStamp.Minutes                     = (Byte_t) TempParam->Params[10].intParam;
                     CuffPressureData.TimeStamp.Seconds                     = (Byte_t) TempParam->Params[11].intParam;
                     CuffPressureData.UserID                                = (Byte_t) TempParam->Params[12].intParam;

                     /* Go ahead and send the BLS Intermediate Cuff     */
                     /* Pressure data.                                  */
                     if((ret_val = BLS_Notify_Intermediate_Cuff_Pressure(BluetoothStackID, BLSInstanceID, ConnectionID, &CuffPressureData)) == 0)
                     {
                        printf("BLS Intermediate Cuff Pressure Notification success.\r\n");
                        ret_val = 0;
                     }
                     else
                     {
                        DisplayFunctionError("BLS_Notify_Intermediate_Cuff_Pressure", ret_val);
                  ...
                  {
                     printf("Client has not registered for BLS Intermediate Cuff Pressure notifications.\r\n");
                     ret_val = 0;
                  }
               }
               else
   ...
   {
      DisplayBloodPressureMeasurementUsage("NotifyIntermediateCuffPressure");
   }
...
static void BTPSAPI GAP_LE_Event_Callback(unsigned int BluetoothStackID, GAP_LE_Event_Data_t *GAP_LE_Event_Data, unsigned long CallbackParameter)
{
         ...
         case etLE_Disconnection_Complete:
            ...
            if(GAP_LE_Event_Data->Event_Data.GAP_LE_Disconnection_Complete_Event_Data)
            {
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
               {
                  ...
                  DeviceInfo->ServerInfo.Blood_Pressure_Measurement_Client_Configuration = 0;
                  DeviceInfo->ServerInfo.Intermediate_Cuff_Pressure_Client_Configuration = 0;
   ...
   /* The following is a BLS Server Event Callback. This function will  */
   /* be called whenever an BLS Server Profile Event occurs that is     */
   ...
   /* that occurred and the BLS Event Callback Parameter that was       */
   /* specified when this Callback was installed. The caller is free to */
   /* use the contents of the BLS Event Data ONLY in the context of this*/
   ...
   /* possible (this argument holds anyway because another BLS Event    */
   /* will not be processed while this function call is outstanding).   */
   /* ** NOTE ** This function MUST NOT Block and wait for events that  */
   /*            can only be satisfied by Receiving BLS Event Packets.  */
   /*            A Deadlock WILL occur because NO BLS Event Callbacks   */
...
static void BTPSAPI BLS_EventCallback(unsigned int BluetoothStackID, BLS_Event_Data_t *BLS_Event_Data, unsigned long CallbackParameter)
{
   ...
   /* Verify that all parameters to this callback are Semi-Valid.       */
   if((BluetoothStackID) && (BLS_Event_Data))
   {
      switch(BLS_Event_Data->Event_Data_Type)
      {
         case etBLS_Read_Client_Configuration_Request:
            printf("etBLS_Read_Client_Configuration_Request with size %u.\r\n", BLS_Event_Data->Event_Data_Size);

            if(BLS_Event_Data->Event_Data.BLS_Read_Client_Configuration_Data)
            {
               BD_ADDRToStr(BLS_Event_Data->Event_Data.BLS_Read_Client_Configuration_Data->RemoteDevice, BoardStr);
               printf("   Instance ID:      %u.\r\n", BLS_Event_Data->Event_Data.BLS_Read_Client_Configuration_Data->InstanceID);
               printf("   Connection ID:    %u.\r\n", BLS_Event_Data->Event_Data.BLS_Read_Client_Configuration_Data->ConnectionID);
               printf("   Transaction ID:   %u.\r\n", BLS_Event_Data->Event_Data.BLS_Read_Client_Configuration_Data->TransactionID);
               printf("   Connection Type:  %s.\r\n", ((BLS_Event_Data->Event_Data.BLS_Read_Client_Configuration_Data->ConnectionType == gctLE)?"LE":"BR/EDR"));
               printf("   Remote Device:    %s.\r\n", BoardStr);

               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, BLS_Event_Data->Event_Data.BLS_Read_Client_Configuration_Data->RemoteDevice)) != NULL)
               {
                  /* Validate event parameters                          */
                  if(BLS_Event_Data->Event_Data.BLS_Read_Client_Configuration_Data->InstanceID == BLSInstanceID)
                  {
                     Result = 0;

                     switch(BLS_Event_Data->Event_Data.BLS_Read_Client_Configuration_Data->ClientConfigurationType)
                     {
                        case ctBloodPressureMeasurement:
                           printf("   Config Type:      ctBloodPressureMeasurement.\r\n");
                           Result = BLS_Read_Client_Configuration_Response(BluetoothStackID, BLSInstanceID, BLS_Event_Data->Event_Data.BLS_Read_Client_Configuration_Data->TransactionID, DeviceInfo->ServerInfo.Blood_Pressure_Measurement_Client_Configuration);
                           break;
                        case ctIntermediateCuffPressure:
                           printf("   Config Type:      ctIntermediateCuffPressure.\r\n");
                           Result = BLS_Read_Client_Configuration_Response(BluetoothStackID, BLSInstanceID, BLS_Event_Data->Event_Data.BLS_Read_Client_Configuration_Data->TransactionID, DeviceInfo->ServerInfo.Intermediate_Cuff_Pressure_Client_Configuration);
                           break;
                        default:
                     ...
                     if(Result)
                        DisplayFunctionError("BLS_Read_Client_Configuration_Response", Result);
                  }
                  else
                     printf("\r\nInvalid Event data.\r\n");
               }
            ...
            break;
         case etBLS_Client_Configuration_Update:
            printf("etBLS_Client_Configuration_Update with size %u.\r\n", BLS_Event_Data->Event_Data_Size);

            if(BLS_Event_Data->Event_Data.BLS_Client_Configuration_Update_Data)
            {
               BD_ADDRToStr(BLS_Event_Data->Event_Data.BLS_Client_Configuration_Update_Data->RemoteDevice, BoardStr);
               printf("   Instance ID:      %u.\r\n", BLS_Event_Data->Event_Data.BLS_Client_Configuration_Update_Data->InstanceID);
               printf("   Connection ID:    %u.\r\n", BLS_Event_Data->Event_Data.BLS_Client_Configuration_Update_Data->ConnectionID);
               printf("   Connection Type:  %s.\r\n", ((BLS_Event_Data->Event_Data.BLS_Client_Configuration_Update_Data->ConnectionType == gctLE)?"LE":"BR/EDR"));
               printf("   Remote Device:    %s.\r\n", BoardStr);

               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, BLS_Event_Data->Event_Data.BLS_Client_Configuration_Update_Data->RemoteDevice)) != NULL)
               {
                  /* Validate event parameters                          */
                  if(BLS_Event_Data->Event_Data.BLS_Client_Configuration_Update_Data->InstanceID == BLSInstanceID)
                  {
                     switch(BLS_Event_Data->Event_Data.BLS_Client_Configuration_Update_Data->ClientConfigurationType)
                     {
                        case ctBloodPressureMeasurement:
                           printf("   Config Type:      ctBloodPressureMeasurement.\r\n");
                           DeviceInfo->ServerInfo.Blood_Pressure_Measurement_Client_Configuration = BLS_Event_Data->Event_Data.BLS_Client_Configuration_Update_Data->ClientConfiguration;
                           break;
                        case ctIntermediateCuffPressure:
                           printf("   Config Type:      ctIntermediateCuffPressure.\r\n");
                           DeviceInfo->ServerInfo.Intermediate_Cuff_Pressure_Client_Configuration = BLS_Event_Data->Event_Data.BLS_Client_Configuration_Update_Data->ClientConfiguration;
                           break;
                        default:
                           printf("   Config Type:      Unknown.\r\n");
                           break;
                     ...
                     printf("   Value:            0x%04X.\r\n", (unsigned int)BLS_Event_Data->Event_Data.BLS_Client_Configuration_Update_Data->ClientConfiguration);
                  }
                  else
                     printf("\r\nInvalid Event data.\r\n");
               }
            ...
            break;
         case etBLS_Confirmation_Data:
            printf("etBLS_Confirmation_Data with size %u.\r\n", BLS_Event_Data->Event_Data_Size);
            if(BLS_Event_Data->Event_Data.BLS_Confirmation_Data)
            {
               BD_ADDRToStr(BLS_Event_Data->Event_Data.BLS_Confirmation_Data->RemoteDevice, BoardStr);
               printf("   Instance ID:      %u.\r\n", BLS_Event_Data->Event_Data.BLS_Confirmation_Data->InstanceID);
               printf("   Connection ID:    %u.\r\n", BLS_Event_Data->Event_Data.BLS_Confirmation_Data->ConnectionID);
               printf("   Connection Type:  %s.\r\n", ((BLS_Event_Data->Event_Data.BLS_Confirmation_Data->ConnectionType == gctLE)?"LE":"BR/EDR"));
               printf("   Remote Device:    %s.\r\n", BoardStr);
               printf("   Status:           %u.\r\n", BLS_Event_Data->Event_Data.BLS_Confirmation_Data->Status);
            }
            break;
         default:
            printf("Unknown BLS Event\r\n");
            break;
      }
   }
   else
   {
      ...
      printf("\r\nBLS Callback Data: Event_Data = NULL.\r\n");
   }
      ...
      switch(GATT_Client_Event_Data->Event_Data_Type)
      {
         ...
         case etGATT_Client_Read_Response:
            if(GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data)
            {
               ...
               if(ValueLength != 0)
               {
                  /* Always display the data so that PTS test cases can */
                  /* pass.                                              */
                  printf("   Data:            { ");
                  ...
                  {
                     if(CallbackParameter == DeviceInfo->ClientInfo.Blood_Pressure_Feature)
                     {
                        if(ValueLength == 2)
                        {
                           DisplayBloodPressureFeature(*Value);
                        }
                     }
                  }
               }
         ...
         case etGATT_Client_Write_Response:
            if(GATT_Client_Event_Data->Event_Data.GATT_Write_Response_Data)
            {
               ...
               if(((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data->RemoteDevice)) != NULL) && (CallbackParameter != 0))
               {
                  if(CallbackParameter == DeviceInfo->ClientInfo.Blood_Pressure_Measurement_Client_Configuration)
                  {
                     printf("\r\nWrite Blood Pressure Measurement CC Compete.\r\n");
                  }
                  else
                  {
                     if(CallbackParameter == DeviceInfo->ClientInfo.Intermediate_Cuff_Pressure_Client_Configuration)
                     {
                        printf("\r\nWrite Intermediate Cuff Pressure CC Compete.\r\n");
                     }
                  }
               }
            }
      ...
      switch(GATT_Connection_Event_Data->Event_Data_Type)
      {
            ...
            if(GATT_Connection_Event_Data->Event_Data.GATT_Device_Connection_Data)
            {
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, GATT_Connection_Event_Data->Event_Data.GATT_Device_Connection_Data->RemoteDevice)) != NULL)
               {
                  if(LocalDeviceIsMaster)
                  {
                     ...
                     /* supported.                                      */
                     GATT_Exchange_MTU_Request(BluetoothStackID, ConnectionID, BTPS_CONFIGURATION_GATT_DEFAULT_MAXIMUM_SUPPORTED_MTU_SIZE, GATT_ClientEventCallback_BLS, 0);
                  }
               }
            }
            else
         ...
         case etGATT_Connection_Server_Indication:
            if(GATT_Connection_Event_Data->Event_Data.GATT_Server_Indication_Data)
            {
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, GATT_Connection_Event_Data->Event_Data.GATT_Server_Indication_Data->RemoteDevice)) != NULL)
               {
                  ...
                  if(DeviceInfo->ClientInfo.Blood_Pressure_Measurement == GATT_Connection_Event_Data->Event_Data.GATT_Server_Indication_Data->AttributeHandle)
                  {
                     /* Decode and display the data                     */
                     printf("\r\n   Blood Pressure Measurement Data:\r\n");
                     DecodeDisplayBloodPressureMeasurement(GATT_Connection_Event_Data->Event_Data.GATT_Server_Indication_Data->AttributeValueLength, GATT_Connection_Event_Data->Event_Data.GATT_Server_Indication_Data->AttributeValue);
                  }
               }
               else
         ...
         case etGATT_Connection_Server_Notification:
            if(GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data)
            {
               ...
               {
                  if(DeviceInfo->ClientInfo.Intermediate_Cuff_Pressure == GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data->AttributeHandle)
                  {
                     /* Decode and display the data.                    */
                     printf("\r\nIntermediate Cuff Pressure:\r\n");

                     DecodeDisplayBloodPressureMeasurement(GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data->AttributeValueLength, GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data->AttributeValue);
                  }
         ...
         switch(GATT_Service_Discovery_Event_Data->Event_Data_Type)
         {
            case etGATT_Service_Discovery_Indication:
               ...
               if(GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data)
               {
                  ...
                  switch(CallbackParameter)
                  {
                     ...
                     case sdBLS:
                        /* Populate the handles for BLS Service.        */
                        BLSPopulateHandles(DeviceInfo, GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data);
                        break;
            ...
            case etGATT_Service_Discovery_Complete:
               ...
               if(GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Complete_Data)
               {
                  ...
                  /* Check the service discovery type for BLS           */
                  if(((Service_Discovery_Type_t)CallbackParameter) == sdBLS)
                  {
                     ...
                     printf("\r\nBLS Service Discovery Summary\r\n");
                     printf("     Blood Pressure Feature    : %s\r\n", (DeviceInfo->ClientInfo.Blood_Pressure_Feature ? "Supported" : "Not Supported"));
                     printf("     Blood Pressure Measurement: %s\r\n", (DeviceInfo->ClientInfo.Blood_Pressure_Measurement ? "Supported" : "Not Supported"));
                     printf("     Intermediate Cuff Pressure: %s\r\n", (DeviceInfo->ClientInfo.Intermediate_Cuff_Pressure ? "Supported" : "Not Supported"));
                  }
                  else
                  {

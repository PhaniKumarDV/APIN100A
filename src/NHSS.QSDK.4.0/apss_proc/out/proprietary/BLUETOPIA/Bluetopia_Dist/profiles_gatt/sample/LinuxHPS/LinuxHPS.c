...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXHPS - Linux Bluetooth HTTP Proxy Service using GATT (LE/BREDR)       */
/*             sample application.                                            */
/*                                                                            */
/*  Author:  Ryan McCord                                                      */
/*                                                                            */
...
/*   --------  -----------    ------------------------------------------------*/
/*   02/18/16  R. McCord      Initial creation.                               */
/******************************************************************************/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>

#include "LinuxHPS.h"            /* Application Header.                       */
...
#include "SS1BTHPS.h"            /* Main SS1 HPS Service Header.              */
...
#define HPSP_PARAMETER_VALUE                        (2)  /* Denotes the value */
                                                         /* passed in on the  */
                                                         /* command line for  */
                                                         /* running with the  */
                                                         /* transport set to  */
...
#define HPS_SA_MAX_BUFFER_SIZE                     (BTPS_CONFIGURATION_HPS_MAXIMUM_SUPPORTED_STRING_LENGTH)
                                                         /* Denotes the size  */
                                                         /* of the buffers.   */
...
#define DEFAULT_DEBUG_LOG_FILE_NAME                "LinuxHPS_ASC.log"
#define DEFAULT_DEBUG_FTS_FILE_NAME                "LinuxHPS_FTS.log"

   /* Determine the Name we will use for this compilation.              */
#define LE_DEMO_DEVICE_NAME                        "LinuxHPS"
...
#define DEVICE_INFO_FLAGS_HPS_SERVICE_DISCOVERY_COMPLETE    0x0080

#define DEVICE_INFO_FLAGS_HPS_HTTP_REQUEST_OUTSTANDING      0x0100
#define DEVICE_INFO_FLAGS_HPS_VALIDATE_HTTP_SERVER          0x0200
#define DEVICE_INFO_FLAGS_HPS_HTTP_STATUS_CODE_READY        0x0400
#define DEVICE_INFO_FLAGS_HPS_URI_READY                     0x0800
#define DEVICE_INFO_FLAGS_HPS_HTTP_HEADERS_READY            0x1000
#define DEVICE_INFO_FLAGS_HPS_HTTP_ENTITY_BODY_READY        0x2000

#define DEVICE_INFO_FLAGS_HPS_HTTP_REQUEST_READY           (DEVICE_INFO_FLAGS_HPS_URI_READY          | \
                                                            DEVICE_INFO_FLAGS_HPS_HTTP_HEADERS_READY | \
                                                            DEVICE_INFO_FLAGS_HPS_HTTP_ENTITY_BODY_READY)
...
typedef struct _tagGAPS_Device_Appearance_Mapping_t
{
   ...
   /* This has been included to make the HPS Client Event Callback      */
   /* easier to read.                                                   */
typedef enum
{
   ahtURI,
   ahtHTTP_Headers,
   ahtHTTP_Entity_Body,
   ahtHTTP_Control_Point,
   ahtHTTP_Status_Code,
   ahtHTTP_Status_Code_CCCD,
   ahtHTTPS_Security
} HPS_Attribute_Handle_Type_t;

   /* The following structure holds the data that is needed for each    */
   /* GATT request/response.                                            */
   /* * NOTE * We will add one to the maximum size of the buffer for the*/
   /*          NULL terminator.                                         */
typedef struct _tagHPS_Request_Data_t
{
   /* HPS Client fields.                                                */
   HPS_Attribute_Handle_Type_t            AttributeHandleType;
   Word_t                                 AttributeHandle;
   Word_t                                 BufferOffset;
   Word_t                                 BufferLength;
   Byte_t                                 Buffer[HPS_SA_MAX_BUFFER_SIZE+1];
   Word_t                                 ClientConfiguration;
   HPS_HTTP_Control_Point_Request_Type_t  RequestType;

   /* HPS Server fields.                                                */
   HPS_HTTP_Status_Code_Data_t            StatusCodeData;
} HPS_Request_Data_t;

   /* The following structure holds all the information the HPS Server  */
   /* will need to store.                                               */
typedef struct _tagHPS_Server_Data_t
{
   Boolean_t  HTTP_Server_Validated;
} HPS_Server_Data_t;
...
typedef struct _tagDeviceInfo_t
{
   ...
   HPS_Client_Information_t  HPSClientInfo;
   HPS_Server_Information_t  HPSServerInfo;
   HPS_Request_Data_t        HPSRequestData;
...
static unsigned int        HPSInstanceID;
                                                    /* The following holds the HPS     */
                                                    /* Instance IDs that are returned  */
                                                    /* from HPS_Initialize_XXX().      */
...
static HPS_Server_Data_t   HPSServerInfo;           /* The HPS Server data.            */

static Boolean_t           HTTPServerConnected;     /* A boolean flag to indicate that */
                                                    /* HPS Server is connected to the  */
                                                    /* Network.                        */
...
static Boolean_t AppearanceIndexToAppearance(unsigned int Index, Word_t *Appearance);
   ...
   /* HTTP Proxy Service Functions and Commands.                        */
static void HPSPopulateHandles(HPS_Client_Information_t *ClientInfo, GATT_Service_Discovery_Indication_Data_t *ServiceDiscoveryData);

static int RegisterHPS(void);
static int UnregisterHPS(void);
static int ReadHPSCharacteristic(HPS_Attribute_Handle_Type_t AttributeHandleType, Boolean_t ReadLong);
static int WriteHPSCharacteristic(HPS_Request_Data_t *RequestData);
static int SetHTTPStatusCode(Word_t Status_Code, Byte_t Data_Status);
static int NotifyHTTPStatusCode(void);

static void DisplayHPSCharacteristicType(HPS_Characteristic_Type_t Type);
static void DisplayHTTPControlPointRequestType(HPS_HTTP_Control_Point_Request_Type_t RequestType);
static void DisplayHTTPRequestInformation(HPS_Server_Information_t *Info);

static int RegisterHPSCommand(ParameterList_t *TempParam);
static int UnregisterHPSCommand(ParameterList_t *TempParam);
static int DiscoverHPSCommand(ParameterList_t *TempParam);
static int ReadCharacteristicCommand(ParameterList_t *TempParam);
static int WriteCharacteristicCommand(ParameterList_t *TempParam);
static int SetHTTPStatusCodeCommand(ParameterList_t *TempParam);
static int NotifyHTTPStatusCodeCommand(ParameterList_t *TempParam);
static int ToggleHTTPConnectionCommand(ParameterList_t *TempParam);

static void DisplayReadCharacteristicCommandUsage(void);
static void DisplayWriteCharacteristicCommandUsage(void);
static void DisplaySetHTTPStatusCodeCommandUsage(void);
...
static void BTPSAPI HPS_EventCallback(unsigned int BluetoothStackID, HPS_Event_Data_t *HPS_Event_Data, unsigned long CallbackParameter);
...
static void BTPSAPI GATT_ClientEventCallback_HPS(unsigned int BluetoothStackID, GATT_Client_Event_Data_t *GATT_Client_Event_Data, unsigned long CallbackParameter);
...
static DeviceInfo_t *DeleteDeviceInfoEntry(DeviceInfo_t **ListHead, BD_ADDR_t BD_ADDR)
{
   ...
   /*          applications for use with HPS/HPS.                       */
static void FreeDeviceInfoEntryMemory(DeviceInfo_t *EntryToFree)
{
...
static void UserInterface(void)
{
   ...
   AddCommand("REGISTERHPS", RegisterHPSCommand);
   AddCommand("UNREGISTERHPS", UnregisterHPSCommand);
   AddCommand("DISCOVERHPS", DiscoverHPSCommand);
   AddCommand("READ", ReadCharacteristicCommand);
   AddCommand("WRITE", WriteCharacteristicCommand);
   AddCommand("SETSTATUSCODE", SetHTTPStatusCodeCommand);
   AddCommand("NOTIFY", NotifyHTTPStatusCodeCommand);
   AddCommand("TOGGLECONNECTION", ToggleHTTPConnectionCommand);
      ...
      for(Index = 0; Index < Advertising_Data->Number_Data_Entries; Index++)
      {
         ...
         switch(Advertising_Data->Data_Entries[Index].AD_Type)
         {
            ...
            case HCI_LE_ADVERTISING_REPORT_DATA_TYPE_16_BIT_SERVICE_UUID_COMPLETE:
               ...
               /* We will check for the HPS Service data.  Make sure the*/
               ...
               if(Advertising_Data_Entry->AD_Data_Length >= NON_ALIGNED_WORD_SIZE)
               {
                  ...
                  /* Check if this is the HPS Service data.             */
                  if(HPS_COMPARE_HPS_SERVICE_UUID_TO_UUID_16(*((UUID_16_t *)&(Advertising_Data_Entry->AD_Data_Buffer[0]))))
                  {
                     /* Decode the UUID.                                */
...
static int CloseStack(void)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(HPSInstanceID)
      {
         HPS_Cleanup_Service(BluetoothStackID, HPSInstanceID);
      }
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("* Command Options HPS:                                           *\r\n");
   printf("*                Server:   RegisterHPS                           *\r\n");
   printf("*                          UnRegisterHPS,                        *\r\n");
   printf("*                          SetStatusCode,                        *\r\n");
   printf("*                          Notify,                               *\r\n");
   printf("*                          ToggleConnection,                     *\r\n");
   printf("*                Client:   DiscoverHPS                           *\r\n");
   printf("*                Both:     Read,                                 *\r\n");
   printf("*                          Write                                 *\r\n");
   printf("******************************************************************\r\n");
   ...
   if((BluetoothStackID) && (!ConnectionID))
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].intParam >= 0) && (TempParam->Params[0].intParam <= 1))
      {
         ...
         else
         {
            ...
            /* Configure the flags field hpsed on the Discoverability   */
            /* Mode.                                                    */
            ...
            if(HPSInstanceID)
            {
               ...
               HPS_ASSIGN_HPS_SERVICE_UUID_16(&(Advertisement_Data_Buffer.AdvertisingData.Advertising_Data[5]));
            }
...
static Boolean_t AppearanceIndexToAppearance(unsigned int Index, Word_t *Appearance)
{
   ...
   /* The following function is a utility function that provides a      */
   /* mechanism of populating a HPS Client Information structure with   */
   /* the information discovered from a HPS Discovery operation.        */
static void HPSPopulateHandles(HPS_Client_Information_t *ClientInfo, GATT_Service_Discovery_Indication_Data_t *ServiceDiscoveryData)
{
   unsigned int                                  Index;
   unsigned int                                  Index2;
   ...
   /* Verify that the input parameters are semi-valid.                  */
   if((ClientInfo) && (ServiceDiscoveryData) && (ServiceDiscoveryData->ServiceInformation.UUID.UUID_Type == guUUID_16) && (HPS_COMPARE_HPS_SERVICE_UUID_TO_UUID_16(ServiceDiscoveryData->ServiceInformation.UUID.UUID.UUID_16)))
   {
      ...
      for(Index = 0; Index < ServiceDiscoveryData->NumberOfCharacteristics; Index++)
      {
         ...
         /* All HPS UUIDs are defined to be 16 bit UUIDs.               */
         if(CharacteristicInfoPtr->Characteristic_UUID.UUID_Type == guUUID_16)
         {
            /* Check for the URI UUID.                                  */
            if(HPS_COMPARE_URI_UUID_TO_UUID_16(CharacteristicInfoPtr->Characteristic_UUID.UUID.UUID_16))
            {
               /* Store the handle.                                     */
               ClientInfo->HPS_URI = CharacteristicInfoPtr->Characteristic_Handle;

               /* Verify that write is supported.                       */
               if(!(CharacteristicInfoPtr->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_WRITE))
                 printf("\r\nWarning - Mandatory write property of HPS URI Characteristic not supported!\r\n");

               /* Get the next Characteristic.                          */
               continue;
            }

            /* Check for the HTTP Headers UUID.                         */
            if(HPS_COMPARE_HTTP_HEADERS_UUID_TO_UUID_16(CharacteristicInfoPtr->Characteristic_UUID.UUID.UUID_16))
            {
               /* Store the handle.                                     */
               ClientInfo->HPS_HTTP_Headers = CharacteristicInfoPtr->Characteristic_Handle;

               /* Verify that read is supported.                        */
               if(!(CharacteristicInfoPtr->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_READ))
                 printf("\r\nWarning - Mandatory read property of HPS HTTP Headers Characteristic not supported!\r\n");

               /* Verify that write is supported.                       */
               if(!(CharacteristicInfoPtr->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_WRITE))
                 printf("\r\nWarning - Mandatory write property of HPS HTTP Headers Characteristic not supported!\r\n");

               /* Get the next Characteristic.                          */
               continue;
            }

            /* Check for the HTTP Entity Body UUID.                     */
            if(HPS_COMPARE_HTTP_ENTITY_BODY_UUID_TO_UUID_16(CharacteristicInfoPtr->Characteristic_UUID.UUID.UUID_16))
            {
               /* Store the handle.                                     */
               ClientInfo->HPS_HTTP_Entity_Body = CharacteristicInfoPtr->Characteristic_Handle;

               /* Verify that read is supported.                        */
               if(!(CharacteristicInfoPtr->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_READ))
                 printf("\r\nWarning - Mandatory read property of HPS HTTP Entity Body Characteristic not supported!\r\n");

               /* Verify that write is supported.                       */
               if(!(CharacteristicInfoPtr->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_WRITE))
                 printf("\r\nWarning - Mandatory write property of HPS HTTP Entity Body Characteristic not supported!\r\n");

               /* Get the next Characteristic.                          */
               continue;
            }

            /* Check for the HTTP Control Point UUID.                   */
            if(HPS_COMPARE_HTTP_CONTROL_POINT_UUID_TO_UUID_16(CharacteristicInfoPtr->Characteristic_UUID.UUID.UUID_16))
            {
               /* Store the handle.                                     */
               ClientInfo->HPS_HTTP_Control_Point = CharacteristicInfoPtr->Characteristic_Handle;

               /* Verify that write is supported.                       */
               if(!(CharacteristicInfoPtr->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_WRITE))
                 printf("\r\nWarning - Mandatory write property of HPS HTTP Control Point Characteristic not supported!\r\n");

               /* Get the next Characteristic.                          */
               continue;
            }

            /* Check for the HTTP Status Code Point UUID.               */
            if(HPS_COMPARE_HTTP_STATUS_CODE_UUID_TO_UUID_16(CharacteristicInfoPtr->Characteristic_UUID.UUID.UUID_16))
            {
               /* Store the handle.                                     */
               ClientInfo->HPS_HTTP_Status_Code = CharacteristicInfoPtr->Characteristic_Handle;

               /* Verify that notify is supported.                      */
               if(!(CharacteristicInfoPtr->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_NOTIFY))
                 printf("\r\nWarning - Mandatory notify property of HPS HTTP Status Code Characteristic not supported!\r\n");

               /* Loop through the Descriptor list since we are         */
               /* expecting to find the HPS HTTP Status Code CCCD.      */
               for(Index2 = 0; Index2 < CharacteristicInfoPtr->NumberOfDescriptors; Index2++)
               {
                  /* Store a pointer to the Characteristic information. */
                  DescriptorInfoPtr = &(CharacteristicInfoPtr->DescriptorList[Index2]);
                  ...
                  {
                     /* Check for the Status Code CCCD.                 */
                     ...
                     {
                        ClientInfo->HPS_HTTP_Status_Code_CCCD = DescriptorInfoPtr->Characteristic_Descriptor_Handle;
            ...
            /* Check for the HTTPS Security UUID.                       */
            if(HPS_COMPARE_HTTPS_SECURITY_UUID_TO_UUID_16(CharacteristicInfoPtr->Characteristic_UUID.UUID.UUID_16))
            {
               /* Store the handle.                                     */
               ClientInfo->HPS_HTTPS_Security = CharacteristicInfoPtr->Characteristic_Handle;

               /* Verify that read is supported.                        */
               if(!(CharacteristicInfoPtr->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_READ))
                 printf("\r\nWarning - Mandatory read property of HPS HTTPS Security not supported!\r\n");
   ...
   /* The following function is responsible for registering HPS.        */
static int RegisterHPS(void)
{
   int ret_val = 0;
   ...
   if(!ConnectionID)
   {
      ...
      if(!HPSInstanceID)
      {
         ...
         ret_val = HPS_Initialize_Service(BluetoothStackID, (unsigned int)HPS_SERVICE_FLAGS_DUAL_MODE, HPS_EventCallback, 0, &HPSInstanceID);
         if((ret_val > 0) && (HPSInstanceID > 0))
         {
            /* Display succhps message.                                 */
            printf("Successfully registered HPS Service, HPSInstanceID = %u.\r\n", ret_val);

            /* Save the ServiceID of the registered service.            */
            HPSInstanceID = (unsigned int)ret_val;

            /* The HTTP Server connection will always be initially      */
            /* connected.                                               */
            HTTPServerConnected = TRUE;

            /* Simply return success to the caller.                     */
            ret_val = 0;
         }
         else
            DisplayFunctionError("HPS_Initialize_Service", ret_val);
      }
      else
      {
         printf("\r\nHPS is already registered.\r\n");
      }
   }
   else
   {
   ...
   /* The following function is responsible for un-registering HPS.     */
static int UnregisterHPS(void)
{
   int           ret_val = 0;
   DeviceInfo_t *DeviceInfo;
   ...
   if(HPSInstanceID)
   {
      ...
      /* Unregister the HPS Service with GATT.                          */
      ret_val = HPS_Cleanup_Service(BluetoothStackID, HPSInstanceID);
      if(ret_val == 0)
      {
         /* Display succhps mhpsage.                                    */
         printf("Successfully unregistered HPS Service InstanceID %u.\r\n", HPSInstanceID);

         /* Clear the InstanceID.                                       */
         HPSInstanceID = 0;
      }
      else
         DisplayFunctionError("HPS_Cleanup_Service", ret_val);
   }
   else
      printf("\r\nHPS Service not registered.\r\n");

   return(ret_val);
}

   /* The following function is responsible for sending a read request  */
   /* for an HPS Characteristic to an HPS Server.                       */
static int ReadHPSCharacteristic(HPS_Attribute_Handle_Type_t AttributeHandleType, Boolean_t ReadLong)
{
   int           ret_val = 0;
   DeviceInfo_t *DeviceInfo;
   ...
   if(ConnectionID)
   {
      ...
      if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
      {
         /* If we are the HPS Server.                                   */
         if(HPSInstanceID)
         {
            /* Detemermine the HPS request.                             */
            switch(AttributeHandleType)
            {
               case ahtURI:
                  /* Read the HPS Characteristic.                       */
                  BufferLength = DeviceInfo->HPSServerInfo.URI_Data.Buffer_Length;
                  Buffer       = DeviceInfo->HPSServerInfo.URI_Data.Buffer;
                  ...
                  printf("   Value:  \"%s\"\r\n", Buffer);
                  break;
               case ahtHTTP_Headers:
                  /* Read the HPS Characteristic.                       */
                  BufferLength = DeviceInfo->HPSServerInfo.HTTP_Headers_Data.Buffer_Length;
                  Buffer       = DeviceInfo->HPSServerInfo.HTTP_Headers_Data.Buffer;
                  ...
                  printf("   Value:  \"%s\"\r\n", Buffer);
                  break;
               case ahtHTTP_Entity_Body:
                  /* Read the HPS Characteristic.                       */
                  BufferLength = DeviceInfo->HPSServerInfo.HTTP_Entity_Body_Data.Buffer_Length;
                  Buffer       = DeviceInfo->HPSServerInfo.HTTP_Entity_Body_Data.Buffer;
                  ...
                  break;
               default:
                  printf("\r\nOnly the URI, HTTP Headers, or HTTP Entity Body may be read by the HPS Server.\r\n");
                  break;
            }
         }
         else
         {
            ...
            if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_HPS_SERVICE_DISCOVERY_COMPLETE)
            {
               /* Detemermine the HPS request.                          */
               switch(AttributeHandleType)
               {
                  case ahtHTTP_Headers:
                     AttributeHandle = DeviceInfo->HPSClientInfo.HPS_HTTP_Headers;
                     break;
                  case ahtHTTP_Entity_Body:
                     AttributeHandle = DeviceInfo->HPSClientInfo.HPS_HTTP_Entity_Body;
                     break;
                  case ahtHTTP_Status_Code_CCCD:
                     AttributeHandle = DeviceInfo->HPSClientInfo.HPS_HTTP_Status_Code_CCCD;
                     break;
                  case ahtHTTPS_Security:
                     AttributeHandle = DeviceInfo->HPSClientInfo.HPS_HTTPS_Security;
                     break;
                  default:
                     break;
               }
               ...
               if(AttributeHandle)
               {
                  ...
                  DeviceInfo->HPSRequestData.AttributeHandleType = AttributeHandleType;
                  DeviceInfo->HPSRequestData.AttributeHandle     = AttributeHandle;

                  /* Determine if this a GATT Read or GATT Read Long    */
                  /* request.                                           */
                  /* * NOTE * A GATT read request MUST be peformed      */
                  /*          before a subsequent GATT Read Long may be */
                  /*          performed.                                */
                  if(!ReadLong)
                  {
                     /* We will always reset the response buffer so that*/
                     /* the buffer is initially empty for the URI, HTTP */
                     /* Headers, and HTTP Entity Body.                  */
                     /* * NOTE * The GATT Read Long will use this as the*/
                     /*          offset for the next read request.      */
                     DeviceInfo->HPSRequestData.BufferLength = 0;

                     /* Send the GATT read request.                     */
                     /* * NOTE * We will not save the transactionID     */
                     /*          returned by this function, which we    */
                     /*          could use to cancel the request.       */
                     if((ret_val = GATT_Read_Value_Request(BluetoothStackID, ConnectionID, AttributeHandle, GATT_ClientEventCallback_HPS, AttributeHandle)) > 0)
                     {
                        printf("\r\nGATT Read Value Request sent:\r\n");
                        printf("   TransactionID:     %d\r\n",     ret_val);
                        printf("   Attribute Handle:  0x%04X\r\n", AttributeHandle);
                  ...
                  {
                     /* Only the URI, HTTP Headers, or HTTP Entity Body */
                     /* may be requested with a GATT Read Long.         */
                     if((AttributeHandleType >= ahtURI) && (AttributeHandleType <= ahtHTTP_Entity_Body))
                     {
                        /* Make sure a GATT Read request has previously */
                        /* been performed.  This will also be the next  */
                        /* offset to read.                              */
                        if(DeviceInfo->HPSRequestData.BufferLength)
                        {
                           /* Send the GATT read request.               */
                           ...
                           /*          cancel the request.              */
                           if((ret_val = GATT_Read_Long_Value_Request(BluetoothStackID, ConnectionID, AttributeHandle, DeviceInfo->HPSRequestData.BufferLength, GATT_ClientEventCallback_HPS, AttributeHandle)) > 0)
                           {
                        ...
                        printf("\r\nInvalid attribute handle type for a GATT Read Long request.\r\n");
                  }
               }
               else
   ...
   /* The following function is responsible for sending a write request */
   /* for an HPS Characteristic to an HPS Server.                       */
static int WriteHPSCharacteristic(HPS_Request_Data_t *RequestData)
{
   int               ret_val = 0;
   ...
   NonAlignedByte_t  HTTP_Control_Point_Command;

   /* Make sure the parameters are semi-valid.                          */
   if(RequestData)
   {
      /* If we are the HPS Client.                                      */
      if(!HPSInstanceID)
      {
         /* We are the HPS Client so we need to make sure that a        */
         /* connection exists to the HPS Server.                        */
         if(ConnectionID)
         {
            /* Get the device info for the connection device.           */
            ...
            if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
            {
               ...
               if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_HPS_SERVICE_DISCOVERY_COMPLETE)
               {
                  /* Detemermine the attribute handle based on the      */
                  /* attribute handle type.                             */
                  switch(RequestData->AttributeHandleType)
                  {
                     case ahtURI:
                        AttributeHandle = DeviceInfo->HPSClientInfo.HPS_URI;
                        break;
                     case ahtHTTP_Headers:
                        AttributeHandle = DeviceInfo->HPSClientInfo.HPS_HTTP_Headers;
                        break;
                     case ahtHTTP_Entity_Body:
                        AttributeHandle = DeviceInfo->HPSClientInfo.HPS_HTTP_Entity_Body;
                        break;
                     case ahtHTTP_Control_Point:
                        AttributeHandle = DeviceInfo->HPSClientInfo.HPS_HTTP_Control_Point;
                        break;
                     case ahtHTTP_Status_Code_CCCD:
                        AttributeHandle = DeviceInfo->HPSClientInfo.HPS_HTTP_Status_Code_CCCD;
                        break;
                     default:
                        printf("\r\nInvalid write request type.\r\n");
                        break;
                  }
                  ...
                  if(AttributeHandle)
                  {
                     /* Store the attribute handle so we can handle     */
                     /* responses for GATT Prepare Writes.              */
                     RequestData->AttributeHandle = AttributeHandle;

                     /* Dispatch the write request based on the         */
                     /* attribute handle type.                          */
                     switch(RequestData->AttributeHandleType)
                     {
                        case ahtURI:
                        ...
                        case ahtHTTP_Entity_Body:
                           ...
                           if(RequestData->BufferLength <= (Word_t)HPS_SA_MAX_BUFFER_SIZE)
                           {
                              /* Query the GATT Connection MTU to       */
                              /* determine how we should handle the     */
                              /* write request.                         */
                              if((ret_val = GATT_Query_Connection_MTU(BluetoothStackID, ConnectionID, &MTU)) == 0)
                              {
                                 /* Check whether we need to send a GATT*/
                                 /* Write request or a GATT Prepare     */
                                 /* Write request.                      */
                                 if(RequestData->BufferLength <= (MTU-3))
                                 {
                                    ...
                                    /*          the request.            */
                                    if((ret_val = GATT_Write_Request(BluetoothStackID, ConnectionID, AttributeHandle, RequestData->BufferLength, (Byte_t *)(RequestData->Buffer), GATT_ClientEventCallback_HPS, AttributeHandle)) > 0)
                                    {
                                 ...
                                 {
                                    /* This is the GATT Prepare Write   */
                                    /* request so we need to store the  */
                                    /* request data to handle the       */
                                    /* response.                        */
                                    DeviceInfo->HPSRequestData = *RequestData;
                                    ...
                                    /*          we could use to cancel  */
                                    /*          the request.            */
                                    if((ret_val = GATT_Prepare_Write_Request(BluetoothStackID, ConnectionID, AttributeHandle, RequestData->BufferLength, 0, (Byte_t *)(RequestData->Buffer), GATT_ClientEventCallback_HPS, AttributeHandle)) > 0)
                                    {
                           ...
                           break;
                        case ahtHTTP_Control_Point:
                           /* Format the HTTP Control Point request.    */
                           ASSIGN_HOST_BYTE_TO_LITTLE_ENDIAN_UNALIGNED_BYTE(&HTTP_Control_Point_Command, (Byte_t)RequestData->RequestType);
                           ...
                           /*          function, which we could use to  */
                           /*          cancel the request.              */
                           if((ret_val = GATT_Write_Request(BluetoothStackID, ConnectionID, AttributeHandle, NON_ALIGNED_BYTE_SIZE, (Byte_t *)&HTTP_Control_Point_Command, GATT_ClientEventCallback_HPS, AttributeHandle)) > 0)
                           {
                              printf("\r\nGATT Write Request sent:\r\n");
                              printf("   TransactionID:     %d\r\n", ret_val);
                              printf("   Attribute Handle:  0x%04X\r\n", AttributeHandle);
                        ...
                        case ahtHTTP_Status_Code_CCCD:
                           ...
                           ASSIGN_HOST_WORD_TO_LITTLE_ENDIAN_UNALIGNED_WORD(&Client_Configuration, RequestData->ClientConfiguration);
                           ...
                           /*          function, which we could use to  */
                           /*          cancel the request.              */
                           if((ret_val = GATT_Write_Request(BluetoothStackID, ConnectionID, AttributeHandle, NON_ALIGNED_WORD_SIZE, (Byte_t *)&Client_Configuration, GATT_ClientEventCallback_HPS, AttributeHandle)) > 0)
                           {
                              printf("\r\nGATT Write Request sent:\r\n");
                              printf("   TransactionID:     %d\r\n", ret_val);
                              printf("   Attribute Handle:  0x%04X\r\n", AttributeHandle);
                     ...
                     printf("\r\nAttribute not discovered during HPS service discovery.\r\n");
               }
               else
                  printf("\r\nService discovery has not been performed.\r\n");
            }
            else
               printf("\r\nNo device information for the HPS Server.\r\n");
         }
         else
            printf("\r\nNo connection to the remote HPS Server.\r\n");
      }
      else
         printf("\r\nOnly the HPS Client may use this function.\r\n");
   }
   else
      printf("\r\nRequestData is invalid.\r\n");

   return ret_val;
}

   /* The following function is responsible for setting the HTTP Status */
   /* Code.  This function will basically be used to set up the         */
   /* notification so that the HPS Client can be notified the HTTP      */
   /* Status Code to let it know the HTTP request has completed.        */
static int SetHTTPStatusCode(Word_t Status_Code, Byte_t Data_Status)
{
   int           ret_val = 0;
   DeviceInfo_t *DeviceInfo;
   char          Response[HPS_SA_MAX_BUFFER_SIZE] = "Response Received";
   char          ResponseTruncated[HPS_SA_MAX_BUFFER_SIZE] = "Response Truncated";
   Word_t        BufferLength;
   Byte_t       *Buffer;

   /* Make sure we are the HPS Server.                                  */
   if(HPSInstanceID)
   {
      /* Make sure we are connected.                                    */
      if(ConnectionID)
      {
         /* Get the device information for the HPS Client.              */
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            /* Make sure an HTTP request is in progress.                */
            if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_HPS_HTTP_REQUEST_OUTSTANDING)
            {
               /* Check if we need to validate the HTTP Server          */
               /* certification.                                        */
               if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_HPS_VALIDATE_HTTP_SERVER)
               {
                  /* Simply clear the flag to indicate that we have     */
                  /* validated the HTTP Server certificate.             */
                  DeviceInfo->Flags &= ~DEVICE_INFO_FLAGS_HPS_VALIDATE_HTTP_SERVER;

                  /* Set the boolean so the HPS Client can read that the*/
                  /* HTTP Server certificate has been validated.  We    */
                  /* will always set the the certificate is valid for   */
                  /* this sample application.                           */
                  /* * NOTE * We won't send the notification of the HTTP*/
                  /*          Status Code in this function so the HPS   */
                  /*          Client may read this value.               */
                  HPSServerInfo.HTTP_Server_Validated = TRUE;

                  /* Let the application know that we have validated the*/
                  /* HTTP Server certificate.                           */
                  printf("\r\nHTTP Server certificate validated.\r\n");
               }
               else
               {
                  /* Simply set that the HTTP Server certificate was not*/
                  /* validated since this wasn't a request to validate  */
                  /* it.                                                */
                  HPSServerInfo.HTTP_Server_Validated = FALSE;
               }

               /* Let the application know we have receive the HTTP     */
               /* response.                                             */
               printf("\r\nHTTP response received.\r\n");

               /* Determine how to store the HTTP Headers based on the  */
               /* Data Status.                                          */
               if(Data_Status & HPS_DATA_STATUS_HEADER_RECEIVED)
               {
                  /* Set the HTTP Headers with the response information.*/
                  Buffer       = DeviceInfo->HPSServerInfo.HTTP_Headers_Data.Buffer;
                  BufferLength = BTPS_StringLength(Response);

                  /* Simply copy the data and update the length.        */
                  BTPS_MemCopy(Buffer, (Byte_t *)Response, BufferLength);
                  DeviceInfo->HPSServerInfo.HTTP_Headers_Data.Buffer_Length = BufferLength;
               }
               else
               {
                  /* Set the HTTP Headers with the response information.*/
                  Buffer       = DeviceInfo->HPSServerInfo.HTTP_Headers_Data.Buffer;
                  BufferLength = BTPS_StringLength(ResponseTruncated);

                  /* Simply copy the data and update the length.        */
                  /* * NOTE * We will always update the length to the   */
                  /*          maximum so that we can simulate the buffer*/
                  /*          being truncated.                          */
                  BTPS_MemCopy(Buffer, (Byte_t *)ResponseTruncated, BufferLength);
                  DeviceInfo->HPSServerInfo.HTTP_Headers_Data.Buffer_Length = (Word_t)HPS_SA_MAX_BUFFER_SIZE;
               }

               /* Determine how to store the HTTP Entity Body based on  */
               /* the Data Status.                                      */
               if(Data_Status & HPS_DATA_STATUS_BODY_RECEIVED)
               {
                  /* Set the HTTP Headers with the response information.*/
                  Buffer       = DeviceInfo->HPSServerInfo.HTTP_Entity_Body_Data.Buffer;
                  BufferLength = BTPS_StringLength(Response);
                  ...
                  /* Simply copy the data and update the length.        */
                  BTPS_MemCopy(Buffer, (Byte_t *)Response, BufferLength);
                  DeviceInfo->HPSServerInfo.HTTP_Entity_Body_Data.Buffer_Length = BufferLength;
               }
               else
               {
                  /* Set the HTTP Headers with the response information.*/
                  ...
                  /*          being truncated.                          */
                  BTPS_MemCopy(Buffer, (Byte_t *)ResponseTruncated, BufferLength);
                  DeviceInfo->HPSServerInfo.HTTP_Entity_Body_Data.Buffer_Length = (Word_t)HPS_SA_MAX_BUFFER_SIZE;
               }

               /* Store the Status Code so we can notify it.            */
               DeviceInfo->HPSRequestData.StatusCodeData.Status_Code = Status_Code;
               DeviceInfo->HPSRequestData.StatusCodeData.Data_Status = Data_Status;

               /* Simply flag that the HTTP Status Code is ready to     */
               /* notify.                                               */
               DeviceInfo->Flags |= DEVICE_INFO_FLAGS_HPS_HTTP_STATUS_CODE_READY;

               /* Let the application know the HTTP request is complete.*/
               /* * NOTE * We will not flag the HTTP request as         */
               /*          completed until the notification has been    */
               /*          sent.                                        */
               printf("\r\nHTTP request complete.\r\n");
               printf("\r\nUse the command: 'Notify', to notify the HTTP Status Code to the HPS Client.\r\n");

               /* Simply return success to the caller.                  */
               ret_val = 0;
            }
            else
               printf("\r\nHTTP request not in progress.\r\n");
         }
         else
            printf("\r\nUnknown HPS Client.\r\n");
      }
      else
         printf("\r\nNot connected to an HPS Client.\r\n");
   }
   else
      printf("\r\nOnly the HPS Server can set the HTTP Status Code.\r\n");

   return(ret_val);
}

   /* The following function function is responsible for notifying the  */
   /* HTTP Status Code to let the HPS Client know the HTTP request has  */
   /* completed.                                                        */
static int NotifyHTTPStatusCode(void)
{
   int           ret_val = 0;
   ...
   if(HPSInstanceID)
   {
      ...
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_HPS_HTTP_REQUEST_OUTSTANDING)
            {
               /* Make sure the HTTP Status Code has been formatted.    */
               if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_HPS_HTTP_STATUS_CODE_READY)
               {
                  /* Send the notification for the HTTP Status Code.    */
                  if((ret_val = HPS_Notify_HTTP_Status_Code(BluetoothStackID, HPSInstanceID, ConnectionID, &(DeviceInfo->HPSRequestData.StatusCodeData))) > 0)
                  {
                     printf("\r\nHTTP Status Code notification sent:\r\n");
                     printf("   Length: %d\r\n", ret_val);

                     /* Simply clear the flag that HTTP Status Code has */
                     /* been sent.                                      */
                     DeviceInfo->Flags &= ~DEVICE_INFO_FLAGS_HPS_HTTP_STATUS_CODE_READY;

                     /* Simply clear the flag that HTTP request has     */
                     /* completed.                                      */
                     DeviceInfo->Flags &= ~DEVICE_INFO_FLAGS_HPS_HTTP_REQUEST_OUTSTANDING;

                     /* Clear the flags so that the HPS Client MUST     */
                     /* re-write the URI, HTTP Headers, and HTTP Entity */
                     /* Body before issuing another HTTP request.       */
                     DeviceInfo->Flags &= ~DEVICE_INFO_FLAGS_HPS_HTTP_REQUEST_READY;
                  }
                  else
                     DisplayFunctionError("HPS_Notify_HTTP_Status_Code", ret_val);
   ...
   /* The following function displays the HPS Characteristic type.      */
static void DisplayHPSCharacteristicType(HPS_Characteristic_Type_t Type)
{
   ...
   {
      case hctURI:
         printf("   Characteristic Type:  URI.\r\n");
         break;
      case hctHTTP_Headers:
         printf("   Characteristic Type:  HTTP Headers.\r\n");
         break;
      case hctHTTP_Entity_Body:
         printf("   Characteristic Type:  HTTP Entity Body.\r\n");
         break;
      default:
         printf("   Characteristic Type:  Invalid.\r\n");
         break;
   }
}

   /* The following function displays the HTTP Control Point request    */
   /* type.                                                             */
static void DisplayHTTPControlPointRequestType(HPS_HTTP_Control_Point_Request_Type_t RequestType)
{
   switch(RequestType)
   {
      case httpGetRequest:
         printf("   CP Request type:      HTTP Get request.\r\n");
         break;
      case httpHeadRequest:
         printf("   CP Request type:      HTTP Head request.\r\n");
         break;
      case httpPostRequest:
         printf("   CP Request type:      HTTP Post request.\r\n");
         break;
      case httpPutRequest:
         printf("   CP Request type:      HTTP Put request.\r\n");
         break;
      case httpDeleteRequest:
         printf("   CP Request type:      HTTP Delete request.\r\n");
         break;
      case httpSecureGetRequest:
         printf("   CP Request type:      HTTPS Get request.\r\n");
         break;
      case httpSecureHeadRequest:
         printf("   CP Request type:      HTTPS Head request.\r\n");
         break;
      case httpSecurePostRequest:
         printf("   CP Request type:      HTTPS Post request.\r\n");
         break;
      case httpSecurePutRequest:
         printf("   CP Request type:      HTTPS Put request.\r\n");
         break;
      case httpSecureDeleteRequest:
         printf("   CP Request type:      HTTPS Delete request.\r\n");
         break;
      case httpCancelRequest:
         printf("   CP Request type:      HTTP Cancel request.\r\n");
         break;
      default:
         printf("   CP Request type:      Invalid.\r\n");
         break;
   }
}

   /* The following function will display the HTTP request information  */
   /* for the URI, HTTP Headers, and HTTP Entity Body.                  */
static void DisplayHTTPRequestInformation(HPS_Server_Information_t *Info)
{
   Word_t  BufferLength;
   Byte_t *Buffer;
   ...
   {
      /* Read the URI.                                                  */
      BufferLength = Info->URI_Data.Buffer_Length;
      Buffer       = Info->URI_Data.Buffer;

      /* Make sure the NULL terminator is set so we don't display past  */
      ...
      BufferLength = Info->HTTP_Headers_Data.Buffer_Length;
      Buffer       = Info->HTTP_Headers_Data.Buffer;
      ...
      /* Read the HTTP Entity Body.                                     */
      BufferLength = Info->HTTP_Entity_Body_Data.Buffer_Length;
      Buffer       = Info->HTTP_Entity_Body_Data.Buffer;
      ...
      /* Display the string.                                            */
      printf("   HTTP Entity Body:  \"%s\"\r\n", Buffer);
   }
   else
      printf("\r\nThe HPS Server information is invalid.\r\n");
}

   /* The following command is responsible for registering HPS.         */
static int RegisterHPSCommand(ParameterList_t *TempParam)
{
   ...
   /* The following command is responsible for un-registering HPS.      */
static int UnregisterHPSCommand(ParameterList_t *TempParam)
{
   /* Simply call and return the internal function.                     */
   return(UnregisterHPS());
}

   /* The following command is responsible for discovering attribute    */
   /* handles for HPS on a remote HPS Server.                           */
static int DiscoverHPSCommand(ParameterList_t *TempParam)
{
   int           ret_val = 0;
   DeviceInfo_t *DeviceInfo;
   ...
   /* Verify that we are not configured as the HPS Server               */
   if(!HPSInstanceID)
   {
      /* Verify that there is a connection that is established.         */
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            if(!(DeviceInfo->Flags & DEVICE_INFO_FLAGS_SERVICE_DISCOVERY_OUTSTANDING))
            {
               /* Configure the filter so that only the HPS Service is  */
               /* discovered.                                           */
               UUID.UUID_Type = guUUID_16;
               HPS_ASSIGN_HPS_SERVICE_UUID_16(&(UUID.UUID.UUID_16));

               /* Start the service discovery prochps.                  */
               ret_val = GATT_Start_Service_Discovery(BluetoothStackID, ConnectionID, (sizeof(UUID)/sizeof(GATT_UUID_t)), &UUID, GATT_Service_Discovery_Event_Callback, sdHPS);
               if(!ret_val)
               {
                  /* Display succhps mhpsage.                           */
                  printf("GATT_Service_Discovery_Start success.\r\n");
         ...
         printf("\r\nMust be connected to an HPS Server.\r\n");
   }
   else
      printf("\r\nOnly an HPS client can discover HPS.\r\n");

   return(ret_val);
}

   /* The following command is responsible for reading an HPS           */
   /* Characteristic.  This command may be called by the HPS Client or  */
   /* HPS Server.                                                       */
static int ReadCharacteristicCommand(ParameterList_t *TempParam)
{
   int                          ret_val = 0;
   HPS_Attribute_Handle_Type_t  AttributeHandleType;
   Boolean_t                    ReadLong = FALSE;

   /* Make sure the parameter's are semi-valid.                         */
   if((TempParam) && (TempParam->NumberofParameters >= 1))
   {
      /* Store the parameter.                                           */
      AttributeHandleType = (HPS_Attribute_Handle_Type_t)TempParam->Params[0].intParam;

      if(TempParam->NumberofParameters == 2)
      {
         ReadLong = (Boolean_t)TempParam->Params[1].intParam;
      }

      /* Simply call the internal function to issue the read request.   */
      if((ret_val = ReadHPSCharacteristic(AttributeHandleType, ReadLong)) != 0)
         DisplayFunctionError("ReadHPSCharacteristic", ret_val);
   }
   else
  ...
  return ret_val;
   ...
   /* The following command is responsible for writing an HPS           */
   /* Characteristic.  This command may be called by the HPS Client     */
   /* ONLY.                                                             */
static int WriteCharacteristicCommand(ParameterList_t *TempParam)
{
   int                 ret_val = 0;
   HPS_Request_Data_t  RequestData;

   /* Make sure the parameter's are semi-valid.                         */
   if((TempParam) && (TempParam->NumberofParameters >= 1))
   {
      /* Store the parameter.                                           */
      RequestData.AttributeHandleType = (HPS_Attribute_Handle_Type_t)TempParam->Params[0].intParam;

      /* Determine the optional parameter based on the Attribute Handle */
      /* Type.                                                          */
      switch(RequestData.AttributeHandleType)
      {
         case ahtURI:
         ...
         case ahtHTTP_Entity_Body:
            ...
            if(TempParam->NumberofParameters >= 2)
            {
               ...
               BTPS_MemCopy(RequestData.Buffer, TempParam->Params[1].strParam, BTPS_StringLength(TempParam->Params[1].strParam));

               /* Set the length.                                       */
               RequestData.BufferLength = BTPS_StringLength(TempParam->Params[1].strParam);

               /* Always set the buffer offset to zero in case this is a*/
               /* GATT Prepare Write request.                           */
               RequestData.BufferOffset = 0;
            }
            else
            {
               /* We will simply set the buffer length to zero.  This   */
               /* will tell the HPS Server that we do not wish to use   */
               /* the HTTP Header or HTTP Entity Body for the HTTP      */
               /* request.  This MAY NOT be used for the URI however, we*/
               /* are not going to check this.                          */
               RequestData.BufferLength = 0;
            }

            /* Simply call the internal function to issue the write     */
            /* request.                                                 */
            if((ret_val = WriteHPSCharacteristic(&RequestData)) != 0)
               DisplayFunctionError("WriteHPSCharacteristic", ret_val);
            break;
         case ahtHTTP_Control_Point:
            if(TempParam->NumberofParameters == 2)
            {
               /* Store the parameter.                                  */
               RequestData.RequestType = (HPS_HTTP_Control_Point_Request_Type_t )TempParam->Params[1].intParam;
         ...
         case ahtHTTP_Status_Code_CCCD:
            ...
            {
               RequestData.ClientConfiguration = 0;
            }
            else
            {
               if(TempParam->Params[1].intParam == 1)
               {
                  RequestData.ClientConfiguration = (Word_t)HPS_CLIENT_CHARACTERISTIC_CONFIGURATION_INDICATE_ENABLE;
               }
               else
               {
                  RequestData.ClientConfiguration = (Word_t)HPS_CLIENT_CHARACTERISTIC_CONFIGURATION_NOTIFY_ENABLE;
               }
            }
  ...
  return ret_val;
   ...
   /* the following command is responsible for setting the HTTP Status  */
   /* Code.                                                             */
static int SetHTTPStatusCodeCommand(ParameterList_t *TempParam)
{
   int     ret_val = 0;
   ...
   {
      /* Set the parameters.                                            */
      Status_Code = (Word_t)TempParam->Params[0].intParam;
      Data_Status = (Byte_t)TempParam->Params[1].intParam;

      /* Simply call the internal function to set the HTTP Status Code. */
      if((ret_val = SetHTTPStatusCode(Status_Code, Data_Status)) != 0)
         DisplayFunctionError("SetHTTPStatusCode", ret_val);

      /* Simply return success to the caller.                           */
      ret_val = 0;
   }
   else
      DisplaySetHTTPStatusCodeCommandUsage();

   return(ret_val);
}

   /* The following command is responsible for notifying the HTTP Status*/
   /* Code to an HPS Client.                                            */
static int NotifyHTTPStatusCodeCommand(ParameterList_t *TempParam)
{
   /* Simply call and return the internal function.                     */
   return(NotifyHTTPStatusCode());
}

   /* The following command is responsible for toggling the HTTP        */
   /* Connection.                                                       */
static int ToggleHTTPConnectionCommand(ParameterList_t *TempParam)
{
   /* We MUST be the HPS Server to establish a connection to an HTTP    */
   /* Server.                                                           */
   if(HPSInstanceID)
   {
      /* Simply toggle the connected state.                             */
      HTTPServerConnected = (HTTPServerConnected) ? FALSE : TRUE;

      if(HTTPServerConnected)
      {
         printf("\r\nHTTP Server: connection established.\r\n");
      }
      else
         printf("\r\nHTTP Server: No connection available.\r\n");
   }
   else
      printf("\r\nOnly the HPS Server can use this function.\r\n");
   ...
   /* ReadHPSCharacteristic() function.                                 */
static void DisplayReadCharacteristicCommandUsage(void)
{
   /* Display the usage.                                                */
   printf("\r\nUsage: Read [Attribute Handle Type (UINT8)] [Opt. params]\r\n");
   printf("\r\nWhere Attribute Handle Type is:\r\n");
   printf("   0 = URI (Server ONLY).\r\n");
   printf("   1 = HTTP Headers [Read Long (0 = False, 1 = True)].\r\n");
   printf("   2 = HTTP Entity Body [Read Long (0 = False, 1 = True)].\r\n");
   printf("   5 = HTTP Status Code CCCD (Client ONLY).\r\n");
   printf("   6 = HTTPS Security (Client ONLY).\r\n");
   printf("\r\nNOTE: To use Read Long, the GATT Read request MUST be used first, followed by GATT Read Long for subsequent requests.\r\n");
}

   /* The following function displays the usage for the                 */
   /* WriteHPSCharacteristic() function.                                */
static void DisplayWriteCharacteristicCommandUsage(void)
{
   /* Display the usage.                                                */
   printf("\r\nUsage: Write [Attribute Handle Type (UINT8)] [Opt. params]\r\n");
   printf("\r\nWhere Attribute Handle Type is:\r\n");
   printf("   0 = URI [String (UTF8)].\r\n");
   printf("   1 = HTTP Headers [String (UTF8)].\r\n");
   printf("   2 = HTTP Entity Body [String (UTF8)].\r\n");
   printf("   3 = HTTP Control Point [Command (UINT8)].\r\n");
   printf("Where Command is:\r\n");
   printf("      1  = HTTP Get request.\r\n");
   printf("      2  = HTTP Head request.\r\n");
   printf("      3  = HTTP Post request.\r\n");
   printf("      4  = HTTP Put request.\r\n");
   printf("      5  = HTTP Delete request.\r\n");
   printf("      6  = HTTPS Get request.\r\n");
   printf("      7  = HTTPS Head request.\r\n");
   printf("      8  = HTTPS Post request.\r\n");
   printf("      9  = HTTPS Put request.\r\n");
   printf("      10 = HTTPS Delete request.\r\n");
   printf("      11 = HTTP Cancel request.\r\n");
   printf("   5 = HTTP Status Code CCCD [Configuration (0 = disable, 1 = indicate, 2 = notify)].\r\n");
}

   /* The following function displays the usage for the                 */
   /* SetHTTPStatusCodeCommand() function.                              */
static void DisplaySetHTTPStatusCodeCommandUsage(void)
{
   /* Display the usage.                                                */
   printf("\r\nUsage: SetStatusCode [Status Code (UINT16)] [Data Status(UINT8)]\r\n");
   printf("\r\nWhere Status Code is:\r\n");
   printf("      200 = Success.\r\n");
   printf("\r\nWhere Data Status (Bitmask) is:\r\n");
   printf("      0x01 = Headers Received.\r\n");
   printf("      0x02 = Headers Truncated.\r\n");
   printf("      0x04 = Body Received.\r\n");
   printf("      0x08 = Body Truncated.\r\n");
}
...
static void BTPSAPI GAP_LE_Event_Callback(unsigned int BluetoothStackID, GAP_LE_Event_Data_t *GAP_LE_Event_Data, unsigned long CallbackParameter)
{
   ...
   /* The following is a HPS Server Event Callback.  This function will */
   /* be called whenever an HPS Server Profile Event occurs that is     */
   /* associated with the specified Bluetooth Stack ID.  This function  */
   /* passes to the caller the Bluetooth Stack ID, the HPS Event Data   */
   /* that occurred and the HPS Event Callback Parameter that was       */
   /* specified when this Callback was The caller is free to installed. */
   /* use the contents of the HPS Event Data ONLY in the context If the */
   ...
   /* possible (this argument holds anyway because another HPS Event    */
   /* will not be processed while this function call is outstanding).   */
   /* ** NOTE ** This function MUST NOT Block and wait for events that  */
   /*            can only be satisfied by Receiving HPS Event Packets.  */
   /*            A Deadlock WILL occur because NO HPS Event Callbacks   */
...
static void BTPSAPI HPS_EventCallback(unsigned int BluetoothStackID, HPS_Event_Data_t *HPS_Event_Data, unsigned long CallbackParameter)
{
   ...
   /* Verify that all parameters to this callback are Semi-Valid.       */
   if((BluetoothStackID) && (HPS_Event_Data))
   {
      /* Switch through the event type.                                 */
      switch(HPS_Event_Data->Event_Data_Type)
      {
         case etHPS_Server_Read_Characteristic_Request:
            printf("\r\netHPS_Server_Read_Characteristic_Request with size %u.\r\n", HPS_Event_Data->Event_Data_Size);
            if(HPS_Event_Data->Event_Data.HPS_Server_Read_Characteristic_Request_Data)
            {
               /* Store event information.                              */
               InstanceID     = HPS_Event_Data->Event_Data.HPS_Server_Read_Characteristic_Request_Data->InstanceID;
               ConnectionID   = HPS_Event_Data->Event_Data.HPS_Server_Read_Characteristic_Request_Data->ConnectionID;
               ConnectionType = HPS_Event_Data->Event_Data.HPS_Server_Read_Characteristic_Request_Data->ConnectionType;
               TransactionID  = HPS_Event_Data->Event_Data.HPS_Server_Read_Characteristic_Request_Data->TransactionID;
               RemoteDevice   = HPS_Event_Data->Event_Data.HPS_Server_Read_Characteristic_Request_Data->RemoteDevice;
               Type           = HPS_Event_Data->Event_Data.HPS_Server_Read_Characteristic_Request_Data->Type;
               Offset         = HPS_Event_Data->Event_Data.HPS_Server_Read_Characteristic_Request_Data->Offset;
               ...
               /* Display the HPS Characteristic Type.         */
               DisplayHPSCharacteristicType(Type);
               ...
               {
                  /* Determine what HPS Characteristic was updated so we*/
                  /* can query and display it.                          */
                  switch(Type)
                  {
                     case hctURI:
                        /* Set the information for the response.        */
                        /* * NOTE * The BufferLength will contains the  */
                        /*          remaining length based on the       */
                        /*          specified offset.  The Buffer will  */
                        /*          point to the next offset that needs */
                        /*          to be read.                         */
                        BufferLength = (DeviceInfo->HPSServerInfo.URI_Data.Buffer_Length - Offset);
                        Buffer       = (DeviceInfo->HPSServerInfo.URI_Data.Buffer + Offset);
                        break;
                     case hctHTTP_Headers:
                        /* Set the information for the response.        */
                        /* * NOTE * The BufferLength will contains the  */
                        ...
                        /*          point to the next offset that needs */
                        /*          to be read.                         */
                        BufferLength = (DeviceInfo->HPSServerInfo.HTTP_Headers_Data.Buffer_Length - Offset);
                        Buffer       = (DeviceInfo->HPSServerInfo.HTTP_Headers_Data.Buffer + Offset);
                        break;
                     case hctHTTP_Entity_Body:
                        /* Set the information for the response.        */
                        /* * NOTE * The BufferLength will contains the  */
                        ...
                        /*          point to the next offset that needs */
                        /*          to be read.                         */
                        BufferLength = (DeviceInfo->HPSServerInfo.HTTP_Entity_Body_Data.Buffer_Length - Offset);
                        Buffer       = (DeviceInfo->HPSServerInfo.HTTP_Entity_Body_Data.Buffer + Offset);
                        break;
                     default:
                        /* Should never occur.                          */
                        break;
                  ...
                  /* Send the HPS Characteristic if the type is valid.  */
                  if((Type >= hctURI) && (Type <= hctHTTP_Entity_Body))
                  {
                     /* Send the response.                              */
                     /* * NOTE * The BufferLength may be zero.  The     */
                     /*          Buffer parameter may be excluded       */
                     /*          (NULL), however it will simply be      */
                     /*          ignored.                               */
                     if((Result = HPS_Read_Characteristic_Request_Response(BluetoothStackID,InstanceID,TransactionID, HPS_ERROR_CODE_SUCCESS, Type, BufferLength, Buffer)) != 0)
                        DisplayFunctionError("HPS_Read_Characteristic_Request_Response", Result);
                  }
                  else
                  {
                     /* Send the error response since this is an invalid*/
                     /* request.                                        */
                     if((Result = HPS_Read_Characteristic_Request_Response(BluetoothStackID,InstanceID,TransactionID, HPS_ERROR_CODE_UNLIKELY_ERROR, Type, 0, NULL)) != 0)
                        DisplayFunctionError("HPS_Read_Characteristic_Request_Response", Result);
                  }
               }
               else
            ...
            break;
         case etHPS_Server_Write_Characteristic_Request:
            printf("\r\netHPS_Server_Write_Characteristic_Request with size %u.\r\n", HPS_Event_Data->Event_Data_Size);
            if(HPS_Event_Data->Event_Data.HPS_Server_Write_Characteristic_Request_Data)
            {
               /* Store event information.                              */
               InstanceID     = HPS_Event_Data->Event_Data.HPS_Server_Write_Characteristic_Request_Data->InstanceID;
               ConnectionID   = HPS_Event_Data->Event_Data.HPS_Server_Write_Characteristic_Request_Data->ConnectionID;
               ConnectionType = HPS_Event_Data->Event_Data.HPS_Server_Write_Characteristic_Request_Data->ConnectionType;
               TransactionID  = HPS_Event_Data->Event_Data.HPS_Server_Write_Characteristic_Request_Data->TransactionID;
               RemoteDevice   = HPS_Event_Data->Event_Data.HPS_Server_Write_Characteristic_Request_Data->RemoteDevice;
               Type           = HPS_Event_Data->Event_Data.HPS_Server_Write_Characteristic_Request_Data->Type;
               BufferLength   = HPS_Event_Data->Event_Data.HPS_Server_Write_Characteristic_Request_Data->BufferLength;
               Buffer         = HPS_Event_Data->Event_Data.HPS_Server_Write_Characteristic_Request_Data->Buffer;
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, RemoteDevice)) != NULL)
               {
                  ...
                  {
                     case hctURI:
                        /* Update the HPS Characteristic.               */
                        /* * NOTE * The service will make sure the      */
                        /*          length is valid before this event is*/
                        /*          dispatched.                         */
                        DeviceInfo->HPSServerInfo.URI_Data.Buffer_Length = BufferLength;
                        BTPS_MemCopy(DeviceInfo->HPSServerInfo.URI_Data.Buffer, Buffer, BufferLength);

                        /* Add the NULL terminator to the string so we  */
                        /* don't display past the end of the string.    */
                        DeviceInfo->HPSServerInfo.URI_Data.Buffer[BufferLength] = '\0';

                        /* Display the new value.                       */
                        printf("   URI:                 \"%s\"\r\n", DeviceInfo->HPSServerInfo.URI_Data.Buffer);

                        /* Flag that the HPS Client has written the URI */
                        /* and it is ready for an HTTP Request.         */
                        DeviceInfo->Flags |= DEVICE_INFO_FLAGS_HPS_URI_READY;
                        break;
                     case hctHTTP_Headers:
                        /* Update the HPS Characteristic.               */
                        /* * NOTE * The service will make sure the      */
                        ...
                        /*          dispatched.                         */
                        DeviceInfo->HPSServerInfo.HTTP_Headers_Data.Buffer_Length = BufferLength;
                        BTPS_MemCopy(DeviceInfo->HPSServerInfo.HTTP_Headers_Data.Buffer, Buffer, BufferLength);

                        /* Add the NULL terminator to the string so we  */
                        /* don't display past the end of the string.    */
                        DeviceInfo->HPSServerInfo.HTTP_Headers_Data.Buffer[BufferLength] = '\0';

                        /* Display the new value.                       */
                        printf("   HTTP Headers:        \"%s\"\r\n", DeviceInfo->HPSServerInfo.HTTP_Headers_Data.Buffer);

                        /* Flag that the HPS Client has written the HTTP*/
                        /* Headers and it is ready for an HTTP Request. */
                        DeviceInfo->Flags |= DEVICE_INFO_FLAGS_HPS_HTTP_HEADERS_READY;
                        break;
                     case hctHTTP_Entity_Body:
                        /* Update the HPS Characteristic.               */
                        /* * NOTE * The service will make sure the      */
                        ...
                        /*          dispatched.                         */
                        DeviceInfo->HPSServerInfo.HTTP_Entity_Body_Data.Buffer_Length = BufferLength;
                        BTPS_MemCopy(DeviceInfo->HPSServerInfo.HTTP_Entity_Body_Data.Buffer, Buffer, BufferLength);

                        /* Add the NULL terminator to the string so we  */
                        /* don't display past the end of the string.    */
                        DeviceInfo->HPSServerInfo.HTTP_Entity_Body_Data.Buffer[BufferLength] = '\0';

                        /* Display the new value.                       */
                        printf("   HTTP Entity Body:    \"%s\"\r\n", DeviceInfo->HPSServerInfo.HTTP_Entity_Body_Data.Buffer);

                        /* Flag that the HPS Client has written the HTTP*/
                        /* Entity Body and it is ready for an HTTP      */
                        /* Request.                                     */
                        DeviceInfo->Flags |= DEVICE_INFO_FLAGS_HPS_HTTP_ENTITY_BODY_READY;
                        break;
                     default:
                        /* Should never occur.                          */
                        break;
                  ...
                  /* Make sure the HPS Charactersitic type is valid.    */
                  if((Type >= hctURI) && (Type <= hctHTTP_Entity_Body))
                  {
                     /* Send the response.                              */
                     if((Result = HPS_Write_Characteristic_Request_Response(BluetoothStackID,InstanceID,TransactionID, HPS_ERROR_CODE_SUCCESS, Type)) != 0)
                        DisplayFunctionError("HPS_Write_Characteristic_Request_Response", Result);
                  }
                  else
                  {
                     /* Send the error response since this is an invalid*/
                     /* request.                                        */
                     if((Result = HPS_Write_Characteristic_Request_Response(BluetoothStackID,InstanceID,TransactionID, HPS_ERROR_CODE_UNLIKELY_ERROR, Type)) != 0)
                        DisplayFunctionError("HPS_Write_Characteristic_Request_Response", Result);
                  }
               }
               else
            ...
            break;
         case etHPS_Server_Prepare_Characteristic_Request:
            printf("\r\netHPS_Server_Prepare_Characteristic_Request with size %u.\r\n", HPS_Event_Data->Event_Data_Size);
            if(HPS_Event_Data->Event_Data.HPS_Server_Prepare_Characteristic_Request_Data)
            {
               /* Store event information.                              */
               InstanceID     = HPS_Event_Data->Event_Data.HPS_Server_Prepare_Characteristic_Request_Data->InstanceID;
               ConnectionID   = HPS_Event_Data->Event_Data.HPS_Server_Prepare_Characteristic_Request_Data->ConnectionID;
               ConnectionType = HPS_Event_Data->Event_Data.HPS_Server_Prepare_Characteristic_Request_Data->ConnectionType;
               TransactionID  = HPS_Event_Data->Event_Data.HPS_Server_Prepare_Characteristic_Request_Data->TransactionID;
               RemoteDevice   = HPS_Event_Data->Event_Data.HPS_Server_Prepare_Characteristic_Request_Data->RemoteDevice;
               Type           = HPS_Event_Data->Event_Data.HPS_Server_Prepare_Characteristic_Request_Data->Type;
               ...
               /* * NOTE * We will ALWAYS accept the prepare write      */
               /*          request.  However, if we wanted to require   */
               /*          additional security for the URI, HTTP        */
               /*          Headers, or HTTP Entity Body.  We could      */
               /*          reject the request with the response here.   */
               /*          For example if the HPS Client is not         */
               /*          authorized.                                  */
               ...
               {
                  /* Send the response.                                 */
                  if((Result = HPS_Prepare_Characteristic_Request_Response(BluetoothStackID,InstanceID,TransactionID, HPS_ERROR_CODE_SUCCESS, Type)) != 0)
                     DisplayFunctionError("HPS_Prepare_Characteristic_Request_Response", Result);
               }
               else
               {
                  /* Send the error response since this is an invalid   */
                  /* request.                                           */
                  if((Result = HPS_Prepare_Characteristic_Request_Response(BluetoothStackID,InstanceID,TransactionID, HPS_ERROR_CODE_UNLIKELY_ERROR, Type)) != 0)
                     DisplayFunctionError("HPS_Prepare_Characteristic_Request_Response", Result);
               }
            }
            break;
         case etHPS_Server_Write_HTTP_Control_Point_Request:
            printf("\r\netHPS_Server_Write_HTTP_Control_Point_Request with size %u.\r\n", HPS_Event_Data->Event_Data_Size);
            if(HPS_Event_Data->Event_Data.HPS_Server_Write_HTTP_Control_Point_Request_Data)
            {
               /* Store event information.                              */
               InstanceID     = HPS_Event_Data->Event_Data.HPS_Server_Write_HTTP_Control_Point_Request_Data->InstanceID;
               ConnectionID   = HPS_Event_Data->Event_Data.HPS_Server_Write_HTTP_Control_Point_Request_Data->ConnectionID;
               ConnectionType = HPS_Event_Data->Event_Data.HPS_Server_Write_HTTP_Control_Point_Request_Data->ConnectionType;
               TransactionID  = HPS_Event_Data->Event_Data.HPS_Server_Write_HTTP_Control_Point_Request_Data->TransactionID;
               RemoteDevice   = HPS_Event_Data->Event_Data.HPS_Server_Write_HTTP_Control_Point_Request_Data->RemoteDevice;
               RequestType    = HPS_Event_Data->Event_Data.HPS_Server_Write_HTTP_Control_Point_Request_Data->RequestType;
               ...
               /* Display the HTTP Control Point request type.          */
               DisplayHTTPControlPointRequestType(RequestType);

               /* Get the device information.                           */
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, RemoteDevice)) != NULL)
               {
                  /* Make sure the HPS Client has configured the Status */
                  /* Code CCCD for notifications.  This is required for */
                  /* HTTP requests since the notification let's the     */
                  /* client know that the HTTP response has been        */
                  /* received and it may read the HTTP Headers and HTTP */
                  /* Entity body if the HTTP response was successful.   */
                  if(DeviceInfo->HPSServerInfo.HPS_HTTP_Status_Code_Configuration == HPS_CLIENT_CHARACTERISTIC_CONFIGURATION_NOTIFY_ENABLE)
                  {
                     /* Check that an HTTP request is not currently     */
                     /* outstanding.                                    */
                     /* * NOTE * This should really be a check for all  */
                     /*          HPS Clients, but since this sample only*/
                     /*          supports one HPS Client we will check  */
                     /*          it in the HPS Client's device          */
                     /*          information stored on the HPS Server.  */
                     if(!(DeviceInfo->Flags & DEVICE_INFO_FLAGS_HPS_HTTP_REQUEST_OUTSTANDING))
                     {
                        /* Make sure the request is valid.              */
                        if((RequestType >= httpGetRequest) && (RequestType <= httpCancelRequest))
                        {
                           /* Make sure an HTTP Server connection is    */
                           /* available.                                */
                           if(HTTPServerConnected)
                           {
                              /* Make sure this isn't a request to      */
                              /* cancel an HTTP request that isn't in   */
                              /* progres.                               */
                              if(RequestType != httpCancelRequest)
                              {
                                 /* Make sure the HPS Client has written*/
                                 /* the URI, HTTP Headers, and HTTP     */
                                 /* Entity Body.                        */
                                 /* * NOTE * It may have written a zero */
                                 /*          length string, however the */
                                 /*          HPS Client MUST have       */
                                 /*          written a value.           */
                                 if((DeviceInfo->Flags & DEVICE_INFO_FLAGS_HPS_HTTP_REQUEST_READY) == (DEVICE_INFO_FLAGS_HPS_HTTP_REQUEST_READY))
                                 {
                                    /* Display the HTTP request         */
                                    /* information.                     */
                                    DisplayHTTPRequestInformation(&(DeviceInfo->HPSServerInfo));

                                    /* Simply flag that an HTTP Request */
                                    /* is in progress.                  */
                                    /* * NOTE * This will allow the HPS */
                                    /*          Server to use the       */
                                    /*          SetHTTPStatusCode()     */
                                    /*          function to basically   */
                                    /*          signal that the HTTP    */
                                    /*          request has been        */
                                    /*          completed.              */
                                    DeviceInfo->Flags |= DEVICE_INFO_FLAGS_HPS_HTTP_REQUEST_OUTSTANDING;

                                    /* We will simply simulate the HTTP */
                                    /* request and let the HPS Server   */
                                    /* issue commands to simulate that  */
                                    /* the response has been received   */
                                    /* and setting of the HTTP Status   */
                                    /* Code to be notified.             */

                                    /* Send the HTTP Control Point      */
                                    /* response.                        */
                                    if((Result = HPS_HTTP_Control_Point_Request_Response(BluetoothStackID, InstanceID, TransactionID, HPS_ERROR_CODE_SUCCESS)) != 0)
                                       DisplayFunctionError("HPS_HTTP_Control_Point_Request_Response", Result);

                                    /* Let the application know that the*/
                                    /* HTTP request has been sent.      */
                                    printf("\r\nHTTP request sent.\r\n");
                                    printf("\r\nUse the command: 'SetStatusCode' to indicate that the HTTP response has been received.\r\n");

                                    /* If this is a request where we    */
                                    /* need to check the HTTP Server    */
                                    /* Certificate for authenticity.    */
                                    if((RequestType >= httpSecureGetRequest) && (RequestType <= httpSecureDeleteRequest))
                                    {
                                       /* Simply flag that the HTTP     */
                                       /* Server needs to verify the    */
                                       /* HTTP Server certificate before*/
                                 ...
                                 {
                                    printf("\r\nThe HPS Client MUST write the URI, HTTP Headers, and HTTP Entity Body before an HTTP request can be sent.\r\n");

                                    /* Send the response to let the HPS */
                                    /* Client know that a invalid       */
                                    /* request was made since the HPS   */
                                    /* Client has not prepared the URI, */
                                    /* HTTP Headers, and HTTP Entity    */
                                    /* Body.                            */
                                    if((Result = HPS_HTTP_Control_Point_Request_Response(BluetoothStackID, InstanceID, TransactionID, HPS_ERROR_CODE_INVALID_REQUEST)) != 0)
                                       DisplayFunctionError("HPS_HTTP_Control_Point_Request_Response", Result);
                                 }
                              }
                              else
                              {
                                 printf("\r\nNo HTTP request is in progress to cancel.\r\n");

                                 /* Send the response to let the HPS    */
                                 /* Client know that a invalid request  */
                                 /* was made since there is no HTTP     */
                                 /* request in progress to cancel.      */
                                 if((Result = HPS_HTTP_Control_Point_Request_Response(BluetoothStackID, InstanceID, TransactionID, HPS_ERROR_CODE_INVALID_REQUEST)) != 0)
                                    DisplayFunctionError("HPS_HTTP_Control_Point_Request_Response", Result);
                              }
                           }
                           ...
                           {
                              /* Send the response to let the HPS Client*/
                              /* know that the HTTP Server connection is*/
                              /* not available.                         */
                              if((Result = HPS_HTTP_Control_Point_Request_Response(BluetoothStackID, InstanceID, TransactionID, HPS_ERROR_CODE_NETWORK_NOT_AVAILABLE)) != 0)
                                 DisplayFunctionError("HPS_HTTP_Control_Point_Request_Response", Result);
                           }
                        }
                        else
                        {
                           /* Send the response to let the HPS Client   */
                           /* know that this is an invalid request.     */
                           if((Result = HPS_HTTP_Control_Point_Request_Response(BluetoothStackID, InstanceID, TransactionID, HPS_ERROR_CODE_INVALID_REQUEST)) != 0)
                              DisplayFunctionError("HPS_HTTP_Control_Point_Request_Response", Result);
                        }
                     }
                     ...
                     {
                        /* If this is a request to cancel an outstanding*/
                        /* standing HTTP Request.                       */
                        if(RequestType == httpCancelRequest)
                        {
                           /* Simply clear the flag to mark that the    */
                           /* HTTP request has been cancelled.          */
                           DeviceInfo->Flags &= ~DEVICE_INFO_FLAGS_HPS_HTTP_REQUEST_OUTSTANDING;
                        ...
                        {
                           /* Send the response to let the HPS Client   */
                           /* know that a procedure is already in       */
                           /* progress.                                 */
                           if((Result = HPS_HTTP_Control_Point_Request_Response(BluetoothStackID, InstanceID, TransactionID, HPS_ERROR_CODE_PROCEDURE_ALREADY_IN_PROGRESS)) != 0)
                              DisplayFunctionError("HPS_HTTP_Control_Point_Request_Response", Result);
                        }
                     }
                  }
                  ...
                  {
                     printf("\r\nThe HPS Client MUST configure the HTTP Status Code for notifications.\r\n");

                     /* Send the response to let the HPS Client know    */
                     /* that the Status Code CCCD is not configured for */
                     /* notifications.                                  */
                     if((Result = HPS_HTTP_Control_Point_Request_Response(BluetoothStackID, InstanceID, TransactionID, HPS_ERROR_CODE_CCCD_IMPROPERLY_CONFIGURED)) != 0)
                        DisplayFunctionError("HPS_HTTP_Control_Point_Request_Response", Result);
                  }
               }
               else
               {
                  ...
                  if((Result = HPS_HTTP_Control_Point_Request_Response(BluetoothStackID, InstanceID, TransactionID, ATT_PROTOCOL_ERROR_CODE_UNLIKELY_ERROR)) != 0)
                     DisplayFunctionError("HPS_HTTP_Control_Point_Request_Response", Result);
               }
            }
            break;
         case etHPS_Server_Read_HTTP_Status_Code_CCCD_Request:
            printf("\r\netHPS_Server_Read_HTTP_Status_Code_CCCD_Request with size %u.\r\n", HPS_Event_Data->Event_Data_Size);
            if(HPS_Event_Data->Event_Data.HPS_Server_Read_HTTP_Status_Code_CCCD_Request_Data)
            {
               /* Store event information.                              */
               InstanceID     = HPS_Event_Data->Event_Data.HPS_Server_Read_HTTP_Status_Code_CCCD_Request_Data->InstanceID;
               ConnectionID   = HPS_Event_Data->Event_Data.HPS_Server_Read_HTTP_Status_Code_CCCD_Request_Data->ConnectionID;
               ConnectionType = HPS_Event_Data->Event_Data.HPS_Server_Read_HTTP_Status_Code_CCCD_Request_Data->ConnectionType;
               TransactionID  = HPS_Event_Data->Event_Data.HPS_Server_Read_HTTP_Status_Code_CCCD_Request_Data->TransactionID;
               RemoteDevice   = HPS_Event_Data->Event_Data.HPS_Server_Read_HTTP_Status_Code_CCCD_Request_Data->RemoteDevice;
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, RemoteDevice)) != NULL)
               {
                  ...
                  if((Result = HPS_Read_HTTP_Status_Code_CCCD_Request_Response(BluetoothStackID, InstanceID, TransactionID, HPS_ERROR_CODE_SUCCESS, DeviceInfo->HPSServerInfo.HPS_HTTP_Status_Code_Configuration)) != 0)
                     DisplayFunctionError("HPS_Read_HTTP_Status_Code_CCCD_Request_Response", Result);
               }
               else
               {
                  printf("\r\nUnknown HPS Client.\r\n");
                  ...
                  /* Send the error response.                           */
                  if((Result = HPS_Read_HTTP_Status_Code_CCCD_Request_Response(BluetoothStackID, InstanceID, TransactionID, ATT_PROTOCOL_ERROR_CODE_UNLIKELY_ERROR, 0)) != 0)
                     DisplayFunctionError("HPS_Read_HTTP_Status_Code_CCCD_Request_Response", Result);
               }
            }
            break;
         case etHPS_Server_Write_HTTP_Status_Code_CCCD_Request:
            printf("\r\netHPS_Server_Write_HTTP_Status_Code_CCCD_Request with size %u.\r\n", HPS_Event_Data->Event_Data_Size);
            if(HPS_Event_Data->Event_Data.HPS_Server_Write_HTTP_Status_Code_CCCD_Request_Data)
            {
               /* Store event information.                              */
               InstanceID          = HPS_Event_Data->Event_Data.HPS_Server_Write_HTTP_Status_Code_CCCD_Request_Data->InstanceID;
               ConnectionID        = HPS_Event_Data->Event_Data.HPS_Server_Write_HTTP_Status_Code_CCCD_Request_Data->ConnectionID;
               ConnectionType      = HPS_Event_Data->Event_Data.HPS_Server_Write_HTTP_Status_Code_CCCD_Request_Data->ConnectionType;
               TransactionID       = HPS_Event_Data->Event_Data.HPS_Server_Write_HTTP_Status_Code_CCCD_Request_Data->TransactionID;
               RemoteDevice        = HPS_Event_Data->Event_Data.HPS_Server_Write_HTTP_Status_Code_CCCD_Request_Data->RemoteDevice;
               ClientConfiguration = HPS_Event_Data->Event_Data.HPS_Server_Write_HTTP_Status_Code_CCCD_Request_Data->ClientConfiguration;
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, RemoteDevice)) != NULL)
               {
                  /* Make sure the HPS Client has disabled or enabled   */
                  /* notifications.                                     */
                  if((!ClientConfiguration) || (ClientConfiguration == HPS_CLIENT_CHARACTERISTIC_CONFIGURATION_NOTIFY_ENABLE))
                  {
                     /* Store the new configuration.                    */
                     DeviceInfo->HPSServerInfo.HPS_HTTP_Status_Code_Configuration = ClientConfiguration;

                     /* Send the write response.                        */
                     if((Result = HPS_Write_HTTP_Status_Code_CCCD_Request_Response(BluetoothStackID, InstanceID, TransactionID, HPS_ERROR_CODE_SUCCESS)) != 0)
                        DisplayFunctionError("HPS_Write_HTTP_Status_Code_CCCD_Request_Response", Result);
                  }
                  else
                  {
                     /* Send the error response.                        */
                     if((Result = HPS_Write_HTTP_Status_Code_CCCD_Request_Response(BluetoothStackID, InstanceID, TransactionID, HPS_ERROR_CODE_CCCD_IMPROPERLY_CONFIGURED)) != 0)
                        DisplayFunctionError("HPS_Write_HTTP_Status_Code_CCCD_Request_Response", Result);
                  }
               }
               else
            ...
            break;
         case etHPS_Server_Read_HTTPS_Security_Request:
            printf("\r\netHPS_Server_Read_HTTPS_Security_Request with size %u.\r\n", HPS_Event_Data->Event_Data_Size);
            if(HPS_Event_Data->Event_Data.HPS_Server_Read_HTTPS_Security_Request_Data)
            {
               /* Store event information.                              */
               InstanceID     = HPS_Event_Data->Event_Data.HPS_Server_Read_HTTPS_Security_Request_Data->InstanceID;
               ConnectionID   = HPS_Event_Data->Event_Data.HPS_Server_Read_HTTPS_Security_Request_Data->ConnectionID;
               ConnectionType = HPS_Event_Data->Event_Data.HPS_Server_Read_HTTPS_Security_Request_Data->ConnectionType;
               TransactionID  = HPS_Event_Data->Event_Data.HPS_Server_Read_HTTPS_Security_Request_Data->TransactionID;
               RemoteDevice   = HPS_Event_Data->Event_Data.HPS_Server_Read_HTTPS_Security_Request_Data->RemoteDevice;
               ...
               {
                  /* Send the read response.                            */
                  if((Result = HPS_Read_HTTPS_Security_Request_Response(BluetoothStackID, InstanceID, TransactionID, HPS_ERROR_CODE_SUCCESS, HPSServerInfo.HTTP_Server_Validated)) != 0)
                     DisplayFunctionError("HPS_Read_HTTPS_Security_Request_Response", Result);
               }
               else
               {
                  printf("\r\nUnknown HPS Client.\r\n");
                  ...
                  /* Send the error response.                           */
                  /* * NOTE * The CertificateValid parameter will be    */
                  /*          ignored by the error response.            */
                  if((Result = HPS_Read_HTTPS_Security_Request_Response(BluetoothStackID, InstanceID, TransactionID, ATT_PROTOCOL_ERROR_CODE_UNLIKELY_ERROR, FALSE)) != 0)
                     DisplayFunctionError("HPS_Read_HTTPS_Security_Request_Response", Result);
               }
            }
            break;
   ...
   {
      /* There was an error with one or more of the input parameters.   */
      printf("\r\nHPS Callback Data: Event_Data = NULL.\r\n");
   }
...
static void BTPSAPI GATT_ClientEventCallback_HPS(unsigned int BluetoothStackID, GATT_Client_Event_Data_t *GATT_Client_Event_Data, unsigned long CallbackParameter)
{
   ...
   /* GATT HPS Client Event Callback data types.                        */
   unsigned int               ConnectionID;
   unsigned int               TransactionID;
   GATT_Connection_Type_t     ConnectionType;
   BD_ADDR_t                  RemoteDevice;
   ...
   /* HPS Response data types.                                          */
   Word_t                     ClientConfiguration;
   Byte_t                     HTTPS_Security;
      ...
      switch(GATT_Client_Event_Data->Event_Data_Type)
      {
         case etGATT_Client_Error_Response:
            ...
            if(GATT_Client_Event_Data->Event_Data.GATT_Request_Error_Data)
            {
               ...
               if(GATT_Client_Event_Data->Event_Data.GATT_Request_Error_Data->ErrorType == retErrorResponse)
               {
                  ...
                  {
                     case HPS_ERROR_CODE_CCCD_IMPROPERLY_CONFIGURED:
                        printf("   Error Mesg:       HPS_ERROR_CODE_CCCD_IMPROPERLY_CONFIGURED\r\n");
                        break;
                     case HPS_ERROR_CODE_PROCEDURE_ALREADY_IN_PROGRESS:
                        printf("   Error Mesg:       HPS_ERROR_CODE_PROCEDURE_ALREADY_IN_PROGRESS\r\n");
                        break;
                     case HPS_ERROR_CODE_UNLIKELY_ERROR:
                        printf("   Error Mesg:       HPS_ERROR_CODE_UNLIKELY_ERROR\r\n");
                        break;
                     case HPS_ERROR_CODE_INVALID_REQUEST:
                        printf("   Error Mesg:       HPS_ERROR_CODE_INVALID_REQUEST\r\n");
                        break;
                     case HPS_ERROR_CODE_NETWORK_NOT_AVAILABLE:
                        printf("   Error Mesg:       HPS_ERROR_CODE_NETWORK_NOT_AVAILABLE\r\n");
                        break;
                     default:
                        if(GATT_Client_Event_Data->Event_Data.GATT_Request_Error_Data->ErrorCode < NUMBER_OF_ERROR_CODES)
                        {
         ...
         case etGATT_Client_Read_Response:
            ...
            if(GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data)
            {
               ...
               if((Value) && (ValueLength != 0) && (CallbackParameter != 0))
               {
                  ...
                  if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, RemoteDevice)) != NULL)
                  {
                     /* Switch on the attribute handle type we stored   */
                     /* when the request was sent.                      */
                     switch(DeviceInfo->HPSRequestData.AttributeHandleType)
                     {
                        case ahtHTTP_Headers:
                        case ahtHTTP_Entity_Body:
                           /* Intentional fall-through.                 */

                           /* This MUST be the first read request let's */
                           /* stored the value in the buffer.           */

                           /* Check to make sure we can hold the value  */
                           /* in the buffer.                            */
                           if(ValueLength <= (Word_t)HPS_SA_MAX_BUFFER_SIZE)
                           {
                              /* Simply copy the data since this is the */
                              /* first read request.                    */
                              BTPS_MemCopy(DeviceInfo->HPSRequestData.Buffer, Value, ValueLength);

                              /* Update the offset in case we need to   */
                              /* issue a GATT Read Long request to get  */
                              /* the rest of the string.                */
                              DeviceInfo->HPSRequestData.BufferLength = ValueLength;

                              /* Insert the NULL terminator so we do not*/
                              /* go past the end of the string.         */
                              DeviceInfo->HPSRequestData.Buffer[ValueLength] = (char)'\0';

                              /* Display the string stored in the       */
                              /* buffer.                                */
                              if(DeviceInfo->HPSRequestData.AttributeHandleType == ahtHTTP_Headers)
                              {
                                 printf("\r\nHTTP Headers:\r\n");
                                 printf("   String:  %s\r\n", (char *)DeviceInfo->HPSRequestData.Buffer);
                              }
                              else
                              {
                                 printf("\r\nHTTP Entity Body:\r\n");
                              ...
                              printf("\r\nWarning - ValueLength greater than the buffer can store.\r\n");
                           break;
                        case ahtHTTP_Status_Code_CCCD:
                           ...
                           if(ValueLength >= NON_ALIGNED_WORD_SIZE)
                           {
                              ...
                              /* Display the value.                     */
                              printf("\r\nHTTP Status Code CCCD:\r\n");
                              printf("   Value:  0x%04X\r\n", ClientConfiguration);
                           }
                           else
                              printf("\r\nInvalid ValueLength for the HTTP Status Code CCCD.\r\n");
                           break;
                        case ahtHTTPS_Security:
                           /* Verify the ValueLength.                   */
                           ...
                           {
                              /* Decode the value.                      */
                              HTTPS_Security = READ_UNALIGNED_BYTE_LITTLE_ENDIAN(Value);
                              ...
                              printf("\r\nInvalid ValueLength for the HTTPS Security.\r\n");
                           break;
                        default:
                           printf("\r\nInvalid HPS attribute type for a GATT Read response.\r\n");
                           break;
                     }
                  }
                  else
                  ...
                  if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, RemoteDevice)) != NULL)
                  {
                     ...
                     switch(DeviceInfo->HPSRequestData.AttributeHandleType)
                     {
                        ...
                        case ahtHTTP_Entity_Body:
                           ...
                           /*          Long) request.                   */
                           if((DeviceInfo->HPSRequestData.BufferLength + ValueLength) <= (Word_t)HPS_SA_MAX_BUFFER_SIZE)
                           {
                              /* Simply copy the data at the next offset*/
                              /* in the buffer.  This is the current    */
                              /* length of the buffer.                  */
                              BTPS_MemCopy((DeviceInfo->HPSRequestData.Buffer + DeviceInfo->HPSRequestData.BufferLength), Value, ValueLength);
                              ...
                              /* Insert the NULL terminator so we do not*/
                              /* go past the end of the string.         */
                              DeviceInfo->HPSRequestData.Buffer[DeviceInfo->HPSRequestData.BufferLength] = (char)'\0';
                              ...
                              printf("\r\nWarning - Current buffer size plus ValueLength greater than the buffer can store.\r\n");
                           break;
                        default:
                           printf("\r\nInvalid HPS attribute type for a GATT Read response.\r\n");
                           break;
         ...
         case etGATT_Client_Prepare_Write_Response:
            ...
            if(GATT_Client_Event_Data->Event_Data.GATT_Write_Response_Data)
            {
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, RemoteDevice)) != NULL)
               {
                  /* Switch on the attribute handle type we stored when */
                  ...
                  switch(DeviceInfo->HPSRequestData.AttributeHandleType)
                  {
                     ...
                     case ahtHTTP_Entity_Body:
                        ...
                        /* thus far.                                    */
                        DeviceInfo->HPSRequestData.BufferOffset += ValueLength;

                        /* Determine if we have more data to prepare    */
                        /* from the buffer.                             */
                        if(DeviceInfo->HPSRequestData.BufferOffset >= DeviceInfo->HPSRequestData.BufferLength)
                        {
                           ...
                           if(DeviceInfo->HPSRequestData.BufferOffset == DeviceInfo->HPSRequestData.BufferLength)
                           {
                              ...
                              if((Result = GATT_Execute_Write_Request(BluetoothStackID, ConnectionID, FALSE, GATT_ClientEventCallback_HPS, DeviceInfo->HPSRequestData.AttributeHandle)) > 0)
                              {
                           ...
                           else
                           {
                              ...
                              /*          function, which we could use  */
                              /*          to cancel the request.        */
                              if((Result = GATT_Execute_Write_Request(BluetoothStackID, ConnectionID, TRUE, GATT_ClientEventCallback_HPS, DeviceInfo->HPSRequestData.AttributeHandle)) > 0)
                              {
                                 printf("\r\nGATT Execute Write request sent:\r\n");
                                 printf("   TransactionID:  %d\r\n", Result);
                        ...
                        else
                        {
                           ...
                           /* GATT Prepared Write from the buffer.      */
                           if((Result = GATT_Query_Connection_MTU(BluetoothStackID, ConnectionID, &MTU)) == 0)
                           {
                              /* Determine the remaining length.        */
                              RemainingLength = (DeviceInfo->HPSRequestData.BufferLength - DeviceInfo->HPSRequestData.BufferOffset);

                              /* Determine if we need to send the       */
                              /* remaining length or the maximum length.*/
                              /* * NOTE * The maximum size we can send  */
                              /*          for the GATT Prepare Write is */
                              /*          (MTU-5).                      */
                              AttributeLength = (RemainingLength <= (MTU-5)) ? (RemainingLength) : (MTU-5);

                              /* Store the attribute handle to reduce   */
                              /* parameter length.                      */
                              AttributeHandle = DeviceInfo->HPSRequestData.AttributeHandle;

                              /* * NOTE * We do not need to store the   */
                              /*          attribute handle type since it*/
                              /*          is already set.               */

                              /* Send the next GATT Prepare Write       */
                              /* request.                               */
                              /* * NOTE * We will not save the          */
                              /*          transactionID returned by this*/
                              /*          function, which we could use  */
                              /*          to cancel the request.        */
                              if((Result = GATT_Prepare_Write_Request(BluetoothStackID, ConnectionID, AttributeHandle, AttributeLength, DeviceInfo->HPSRequestData.BufferOffset, (DeviceInfo->HPSRequestData.Buffer + DeviceInfo->HPSRequestData.BufferOffset), GATT_ClientEventCallback_HPS, AttributeHandle)) > 0)
                              {
                                 printf("\r\nGATT Prepare Write Request sent:\r\n");
                                 printf("   TransactionID:     %d\r\n",     Result);
                                 printf("   Attribute Handle:  0x%04X\r\n", AttributeHandle);
               ...
               else
               {
                  ...
                  if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
                  {
                     if(LocalDeviceIsMaster)
                     {
                        ...
                        /* supported.                                   */
                        GATT_Exchange_MTU_Request(BluetoothStackID, ConnectionID, BTPS_CONFIGURATION_GATT_DEFAULT_MAXIMUM_SUPPORTED_MTU_SIZE, GATT_ClientEventCallback_HPS, 0);
                     }
                  }
               }
            }
         ...
         case etGATT_Connection_Server_Notification:
            ...
            if(GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data)
            {
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, RemoteBDADDR)) != NULL)
               {
                  ...
                  if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_HPS_SERVICE_DISCOVERY_COMPLETE)
                  {
                     /* Make sure this is a notification for the Status */
                     /* Code.  We will simply verify the attribute      */
                     /* handle.                                         */
                     if(AttributeHandle == DeviceInfo->HPSClientInfo.HPS_HTTP_Status_Code)
                     {
                        /* Simply call the function to decode the Status*/
                        /* Code.                                        */
                        if((Result = HPS_Decode_HTTP_Status_Code(ValueLength, Value, &StatusCodeData)) == 0)
                        {
                           /* Display the Status Code.                  */
                           printf("\r\nHTTP Status Code:\r\n");
                           printf("   Status Code:  %u\r\n", StatusCodeData.Status_Code);
                           printf("   Data Status:  0x%02X\r\n", StatusCodeData.Data_Status);
                        }
                        else
                           DisplayFunctionError("HPS_Decode_HTTP_Status_Code", Result);
                     }
                     else
                     {
                        printf("\r\nAttribute handle is not for the HPS HTTP Status Code.\r\n");
                     }
                  }
                  else
                     printf("\r\nHPS Service discovery has not been performed.\r\n");
               }
               else
                  printf("\r\nUnknown HPS Server.\r\n");
            }
               ...
               if(GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data)
               {
                  ...
                  switch((Service_Discovery_Type_t)CallbackParameter)
                  {
                     ...
                     case sdGAPS:
                        ...
                        break;
                     case sdHPS:
                        /* Attempt to populate the handles for the HPS  */
                        /* Service.                                     */
                        HPSPopulateHandles(&(DeviceInfo->HPSClientInfo), GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data);
                        break;
                     default:
               ...
               if(GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Complete_Data)
               {
                  ...
                  switch((Service_Discovery_Type_t)CallbackParameter)
                  {
                     ...
                     case sdHPS:
                        /* Flag that service discovery has been         */
                        /* performed on for this connection.            */
                        DeviceInfo->Flags |= DEVICE_INFO_FLAGS_HPS_SERVICE_DISCOVERY_COMPLETE;

                        printf("\r\nHPS Service Discovery Summary\r\n\r\n");
                        printf("   URI                   : %s\r\n", (DeviceInfo->HPSClientInfo.HPS_URI                   ? "Supported" : "Not Supported"));
                        printf("   HTTP Headers          : %s\r\n", (DeviceInfo->HPSClientInfo.HPS_HTTP_Headers          ? "Supported" : "Not Supported"));
                        printf("   HTTP Entity Body      : %s\r\n", (DeviceInfo->HPSClientInfo.HPS_HTTP_Entity_Body      ? "Supported" : "Not Supported"));
                        printf("   HTTP Control Point    : %s\r\n", (DeviceInfo->HPSClientInfo.HPS_HTTP_Control_Point    ? "Supported" : "Not Supported"));
                        printf("   HTTP Status Code      : %s\r\n", (DeviceInfo->HPSClientInfo.HPS_HTTP_Status_Code      ? "Supported" : "Not Supported"));
                        printf("   HTTP Status Code CCCD : %s\r\n", (DeviceInfo->HPSClientInfo.HPS_HTTP_Status_Code_CCCD ? "Supported" : "Not Supported"));
                        printf("   HTTPS Security        : %s\r\n", (DeviceInfo->HPSClientInfo.HPS_HTTPS_Security        ? "Supported" : "Not Supported"));
                        break;
                     default:
                        printf("\r\nWarning - Unknown service discovery type.\r\n");
                        break;
   ...
   if(argc >= NUM_EXPECTED_PARAMETERS_USB)
   {
      ...
      switch(strtol(argv[1], &endptr, 10))
      {
         ...
         case UART_PARAMETER_VALUE:
            ...
            break;
         case HPSP_PARAMETER_VALUE:
            /* The Transport selected was HPSP, check to see if the     */
            /* number of parameters entered on the command line is      */
            /* correct.                                                 */
            if(argc == NUM_EXPECTED_PARAMETERS_UART)
            {
      ...
      else
      {
         ...
         /* invalid.                                                    */
         printf("Parameter Error (Expecting = [USB = 0, UART = 1, HPSP = 2 Flag] [IF !USB [Comm Port or Device File] [Baud Rate]])\r\n");
      }
   }
   else
   {

...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXFMP - Linux Bluetooth Find Me Profile using GATT (LE) application.   */
/*                                                                            */
/*  Author:  Cliff Xu                                                         */
/*                                                                            */
...
#include "LinuxFMP.h"            /* Application Header.                       */
...
#include "SS1BTIAS.h"            /* Main SS1 FMP Service Header.              */
...
#define DEFAULT_DEBUG_LOG_FILE_NAME                "LinuxFMP_ASC.log"
#define DEFAULT_DEBUG_FTS_FILE_NAME                "LinuxFMP_FTS.log"

   /* Determine the Name we will use for this compilation.              */
#define LE_DEMO_DEVICE_NAME                        "LinuxFMP"
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("* Command Options FMP:     RegisterIAS, UnRegisterIAS,           *\r\n");
   printf("*                          DiscoverIAS,                          *\r\n");
   ...
   if((BluetoothStackID) && (!ConnectionID))
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].intParam >= 0) && (TempParam->Params[0].intParam <= 1))
      {
         ...
         else
         {
            ...
            if(IASInstanceID)
            {
               /* Advertise the FMP Server (1 byte type & 2 bytes UUID) */
               ...
               IAS_ASSIGN_IAS_SERVICE_UUID_16(&(Advertisement_Data_Buffer.AdvertisingData.Advertising_Data[5]));
            }
...
static int EnableDebug(ParameterList_t *TempParam)
{
   ...
   /* The following function is responsible for registering a FMP       */
...
static int RegisterIAS(ParameterList_t *TempParam)
{
   ...
   if(!ConnectionID)
   {
      ...
      if(!IASInstanceID)
      {
         /* Register the FMP Service with GATT.                         */
   ...
   /* The following function is responsible for unregistering a FMP     */
   /* Service.  This function will return zero on successful execution  */
   /* and a negative value on errors.                                   */
static int UnregisterIAS(ParameterList_t *TempParam)
{
   ...
   if(IASInstanceID)
   {
      ...
      /* Unregister the FMP Service with GATT.                          */
   ...
   /* The following function is responsible for performing a FMP        */
   /* Service Discovery Operation.  This function will return zero on   */
   /* successful execution and a negative value on errors.              */
static int DiscoverIAS(ParameterList_t *TempParam)
{
   ...
   if(!IASInstanceID)
   {
      ...
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            if(!(DeviceInfo->Flags & DEVICE_INFO_FLAGS_SERVICE_DISCOVERY_OUTSTANDING))
            {
               /* Configure the filter so that only the FMP Service is  */
               /* discovered.                                           */
               UUID[0].UUID_Type = guUUID_16;
      ...
      printf("Cannot discover FMP Services when registered as a service.\r\n");
...
static int SetAlertLevelControlPoint(ParameterList_t *TempParam)
{
   ...
   /* The following function is a utility function that provides a      */
   /* mechanism of populating an FMP Client Information structure with  */
...
static void IASPopulateHandles(DeviceInfo_t *DeviceInfo, GATT_Service_Discovery_Indication_Data_t *ServiceDiscoveryData)
{
   ...
   if((DeviceInfo) && (ServiceDiscoveryData) && (ServiceDiscoveryData->ServiceInformation.UUID.UUID_Type == guUUID_16) && (IAS_COMPARE_IAS_SERVICE_UUID_TO_UUID_16(ServiceDiscoveryData->ServiceInformation.UUID.UUID.UUID_16)))
   {
      ...
      if(CurrentCharacteristic)
      {
         ...
         {
            /* All FMP UUIDs are defined to be 16 bit UUIDs.            */
            if(CurrentCharacteristic->Characteristic_UUID.UUID_Type == guUUID_16)
            {
               /* Determine which characteristic this is.               */
               ...
               {
                  printf("Unknown FMP characteristic detected!\r\n");
               }
            }
         }
      }
...
static void BTPSAPI GAP_LE_Event_Callback(unsigned int BluetoothStackID, GAP_LE_Event_Data_t *GAP_LE_Event_Data, unsigned long CallbackParameter)
{
   ...
   /* The following is a FMP Server Event Callback. This function will  */
   /* be called whenever an FMP Server Profile Event occurs that is     */
   /* associated with the specified Bluetooth Stack ID.  This function  */
   /* passes to the caller the Bluetooth Stack ID, the FMP Event Data   */
   /* that occurred and the FMP Event Callback Parameter that was       */
   /* specified when this Callback was installed. The caller is free to */
   /* use the contents of the FMP Event Data ONLY in the context of this*/
   ...
   /* possible (this argument holds anyway because another FMP Event    */
   /* will not be processed while this function call is outstanding).   */
   /* ** NOTE ** This function MUST NOT Block and wait for events that  */
   /*            can only be satisfied by Receiving FMP Event Packets.  */
   /*            A Deadlock WILL occur because NO FMP Event Callbacks   */
...
static void BTPSAPI IAS_EventCallback(unsigned int BluetoothStackID, IAS_Event_Data_t *IAS_Event_Data, unsigned long CallbackParameter)
{
   ...
   else
   {
      ...
      printf("FMP Callback Data: Event_Data = NULL.\r\n");
   }
         ...
         switch(GATT_Service_Discovery_Event_Data->Event_Data_Type)
         {
            case etGATT_Service_Discovery_Indication:
               ...
               if(GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data)
               {
                  ...
                  /* Check the service discovery type for FMP           */
                  if(((Service_Discovery_Type_t)CallbackParameter) == sdIAS)
                  {
                     /* Attempt to populate the handles for the FMP     */
                     /* Service.                                        */

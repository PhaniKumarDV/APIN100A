...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXANCS - Linux Bluetooth Apple Notification Center Service using GATT  */
/*              (LE) application.                                             */
/*                                                                            */
/*  Author:  Matt Seabold                                                     */
/*                                                                            */
...
/*   --------  -----------    ------------------------------------------------*/
/*   01/17/14  M. Seabold     Initial creation.                               */
/******************************************************************************/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>

#include "LinuxANCS.h"     /* Main Application Prototypes and Constants.      */
...
#include "SS1BTANC.h"      /* Includes for Apple Notification Center Service. */
...
#define DEFAULT_DEBUG_LOG_FILE_NAME                "LinuxANCS_ASC.log"
#define DEFAULT_DEBUG_FTS_FILE_NAME                "LinuxANCS_FTS.log"

   /* Determine the Name we will use for this compilation.              */
#define LE_DEMO_DEVICE_NAME                        "LinuxANCS"
...
typedef struct _tagGAPS_Device_Appearance_Mapping_t
{
   ...
   /* The following structure represents the information to track a     */
   /* remote ANCS service table.                                        */
typedef struct _tagANCS_Client_Info_t
{
   Word_t NotificationSourceHandle;
   Word_t NotificationSourceCCDHandle;
   Word_t ControlPointHandle;
   Word_t DataSourceHandle;
   Word_t DataSourceCCDHandle;
} ANCS_Client_Info_t;
...
static unsigned int        CurrentBufferSize;       /* Variable which notes how much   */
                                                    /* data is currently in the current*/
                                                    /* buffer.                         */

static Byte_t              CurrentBuffer[2048];     /* Variable which is used to hold  */
                                                    /* a buffer of raw GATT data until */
                                                    /* the ANCS module confirms the    */
                                                    /* buffer is completed.            */

static unsigned int        NumAttributesRequested;  /* Variable which tracks the number*/
                                                    /* of attributes requested that we */
                                                    /* should expect in a response.    */

static ANCS_Attribute_Data_Type_t CurrentRequestType; /* Variable which notes the      */
                                                    /* type of the current ongoing     */
                                                    /* request.                        */
...
static void ANCSPopulateHandles(DeviceInfo_t *DeviceInfo, GATT_Service_Discovery_Indication_Data_t *ServiceDiscoveryData);
...
static int DiscoverANCS(ParameterList_t *TempParam);
static int ConfigureANCS(ParameterList_t *TempParam);
static int GetNotificationAttributes(ParameterList_t *TempParam);
static int GetAppAttributes(ParameterList_t *TempParam);
...
static void BTPSAPI GATT_ClientEventCallback_ANCS(unsigned int BluetoothStackID, GATT_Client_Event_Data_t *GATT_Client_Event_Data, unsigned long CallbackParameter);
...
static void UserInterface(void)
{
   ...
   AddCommand("DISCOVERANCS", DiscoverANCS);
   AddCommand("CONFIGUREANCS", ConfigureANCS);
   AddCommand("GETNOTIFICATIONATTRIBUTES", GetNotificationAttributes);
   AddCommand("GETAPPATTRIBUTES", GetAppAttributes);
...
static void GAPSPopulateHandles(GAPS_Client_Info_t *ClientInfo, GATT_Service_Discovery_Indication_Data_t *ServiceInfo)
{
   ...
   /* handling incoming Service Discovery Indication for an ANCS service*/
   /* and extracting all necessary characteristic handles from the      */
   /* indication data.                                                  */
static void ANCSPopulateHandles(DeviceInfo_t *DeviceInfo, GATT_Service_Discovery_Indication_Data_t *ServiceDiscoveryData)
{
   ...
   /* Verify that the input parameters are semi-valid.                  */
   if((ServiceDiscoveryData) && (ServiceDiscoveryData->ServiceInformation.UUID.UUID_Type == guUUID_128) && (ANCS_COMPARE_ANCS_SERVICE_UUID_TO_UUID_128(ServiceDiscoveryData->ServiceInformation.UUID.UUID.UUID_128)))
   {
      /* Loop through all characteristics discovered in the service     */
      /* and populate the correct entry.                                */
      ...
      if(CurrentCharacteristic)
      {
         ...
         {
            /* All ANCS UUIDs are defined to be 128 bit UUIDs.          */
            if(CurrentCharacteristic->Characteristic_UUID.UUID_Type == guUUID_128)
            {
               ClientConfigurationHandle = NULL;

               /* Determine which characteristic this is.               */
               if(!ANCS_COMPARE_NOTIFICATION_SOURCE_UUID_TO_UUID_128(CurrentCharacteristic->Characteristic_UUID.UUID.UUID_128))
               {
                  if(!ANCS_COMPARE_CONTROL_POINT_UUID_TO_UUID_128(CurrentCharacteristic->Characteristic_UUID.UUID.UUID_128))
                  {
                     if(ANCS_COMPARE_DATA_SOURCE_UUID_TO_UUID_128(CurrentCharacteristic->Characteristic_UUID.UUID.UUID_128))
                     {
                        DeviceInfo->ClientInfo.DataSourceHandle = CurrentCharacteristic->Characteristic_Handle;
                        ClientConfigurationHandle   = &(DeviceInfo->ClientInfo.DataSourceCCDHandle);
                     }
                  }
                  else
                  {
                     DeviceInfo->ClientInfo.ControlPointHandle = CurrentCharacteristic->Characteristic_Handle;
                  }
               }
               else
               {
                  DeviceInfo->ClientInfo.NotificationSourceHandle = CurrentCharacteristic->Characteristic_Handle;
                  ClientConfigurationHandle           = &(DeviceInfo->ClientInfo.NotificationSourceCCDHandle);
               }
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("* Command Option ANCS:                                           *\r\n");
   printf("*                          DiscoverANCS (Find ANCS Service),     *\r\n");
   printf("*                          ConfigureANCS (Subscribe to ANCS),    *\r\n");
   printf("*                          GetNotificationAttributes,            *\r\n");
   printf("*                          GetAppAttributes,                     *\r\n");
   printf("*                          PrintAttributeIDs                     *\r\n");
   printf("******************************************************************\r\n");
   ...
   if((BluetoothStackID) && (!ConnectionID))
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].intParam >= 0) && (TempParam->Params[0].intParam <= 1))
      {
         ...
         else
         {
            ...
            /* The iPhones expects Tx power to be in the A.D. Attempt to*/
            /* read it from the chip.                                   */
            if(((ret_val = HCI_LE_Read_Advertising_Channel_Tx_Power(BluetoothStackID, &Status, &Buffer)) != 0) || (Status != HCI_ERROR_CODE_NO_ERROR))
            {
               /* There was an error. We will just hard-code a value    */
               /* here simply for the explicit purpose of working this  */
               /* sample.                                               */
               Buffer = 1;
            }

            Advertisement_Data_Buffer.AdvertisingData.Advertising_Data[3] = 2;
            Advertisement_Data_Buffer.AdvertisingData.Advertising_Data[4] = HCI_LE_ADVERTISING_REPORT_DATA_TYPE_TX_POWER_LEVEL;
            Advertisement_Data_Buffer.AdvertisingData.Advertising_Data[5] = Buffer;

            /* The iPhone looks for the ANCS UUID in the Service        */
            /* Solicitation field of the A.D.                           */
            Length = (UUID_128_SIZE + NON_ALIGNED_BYTE_SIZE);
            ASSIGN_HOST_BYTE_TO_LITTLE_ENDIAN_UNALIGNED_BYTE(&(Advertisement_Data_Buffer.AdvertisingData.Advertising_Data[6]), Length);
            ASSIGN_HOST_BYTE_TO_LITTLE_ENDIAN_UNALIGNED_BYTE(&(Advertisement_Data_Buffer.AdvertisingData.Advertising_Data[6+NON_ALIGNED_BYTE_SIZE]), HCI_LE_ADVERTISING_REPORT_DATA_TYPE_SERVICE_SOLICITATION_128_BIT_SERVICE_UUID);
            ANCS_ASSIGN_ANCS_SERVICE_UUID_128(&(Advertisement_Data_Buffer.AdvertisingData.Advertising_Data[6 + NON_ALIGNED_WORD_SIZE]));
            ...
            ret_val = GAP_LE_Set_Advertising_Data(BluetoothStackID, 8 + UUID_128_SIZE, &(Advertisement_Data_Buffer.AdvertisingData));
            if(!ret_val)
            {
...
static int EnableDebug(ParameterList_t *TempParam)
{
   ...
   /* The following function is responsible for initiating an LE service*/
   /* discovery with the remote device to search for an ANCS service.   */
static int DiscoverANCS(ParameterList_t *TempParam)
{
   DeviceInfo_t *DeviceInfo;
   GATT_UUID_t   UUID[1];
   ...
   if(ConnectionID)
   {
      ...
      if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
      {
         ...
         if(!(DeviceInfo->Flags & DEVICE_INFO_FLAGS_SERVICE_DISCOVERY_OUTSTANDING))
         {
            /* Configure the filter so that only the ANCS Service is    */
            /* discovered.                                              */
            UUID[0].UUID_Type = guUUID_128;
            ANCS_ASSIGN_ANCS_SERVICE_UUID_128(&(UUID[0].UUID.UUID_128));

            /* Start the service discovery process.                     */
            ret_val = GATT_Start_Service_Discovery(BluetoothStackID, ConnectionID, (sizeof(UUID)/sizeof(GATT_UUID_t)), UUID, GATT_Service_Discovery_Event_Callback, sdANCS);
   ...
   /* The following function is responsible for subscribing to the      */
   /* notification characteristics of a remote ANCS service.            */
static int ConfigureANCS(ParameterList_t *TempParam)
{
   DeviceInfo_t *DeviceInfo;
   ...
   if(TempParam && (TempParam->NumberofParameters >= 1))
   {
      ...
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            {
               ret_val = 0;
               /* Attempt to subscribe to notifications from the ANCS   */
               /* service.                                              */
               Buffer = TempParam->Params[0].intParam ? GATT_CLIENT_CONFIGURATION_CHARACTERISTIC_NOTIFY_ENABLE : 0;

               ret_val = GATT_Write_Request(BluetoothStackID, ConnectionID, DeviceInfo->ClientInfo.NotificationSourceCCDHandle, sizeof(Buffer), &Buffer, GATT_ClientEventCallback_ANCS, 0);

               if(ret_val > 0)
                  ret_val = GATT_Write_Request(BluetoothStackID, ConnectionID, DeviceInfo->ClientInfo.DataSourceCCDHandle, sizeof(Buffer), &Buffer, GATT_ClientEventCallback_ANCS, 0);
      ...
      printf("Usage: ConfigureRemoteANS [Notify (0 = disable, 1 = enable)]\r\n");

   return(ret_val);
}

   /* The following function is responsible for formatting and sending a*/
   /* request for notification attributes from a remote ANCS service.   */
static int GetNotificationAttributes(ParameterList_t *TempParam)
{
   int                                         Index;
   int                                         Index1;
   ...
   ANCS_Notification_Attribute_ID_t            TempAttributeID;
   ANCS_Notification_Attribute_Request_Data_t *AttributeRequestData;
   DeviceInfo_t                               *DeviceInfo;
   ...
   if((TempParam) && (TempParam->NumberofParameters >= 2))
   {
      ...
      if((ConnectionID) && (!COMPARE_NULL_BD_ADDR(ConnectionBD_ADDR)))
      {
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            /* Note the Notification UID.                               */
            NotificationUID = (DWord_t)(TempParam->Params[0].intParam);
            ...
            for(Index = 1; Index < TempParam->NumberofParameters; Index++)
            {
               /* Get the next Notification Attribute ID.               */
               TempAttributeID = (ANCS_Notification_Attribute_ID_t)(TempParam->Params[Index].intParam);

               /* If a Max Length parameter is required, make sure it   */
               /* was included in the parameters.                       */
               if((TempAttributeID == idTitle) || (TempAttributeID == idSubtitle) || (TempAttributeID == idMessage))
               {
                  Index++;
            ...
            if(!ret_val)
            {
               ...
               /* length.                                               */
               if((AttributeRequestData = (ANCS_Notification_Attribute_Request_Data_t *)BTPS_AllocateMemory(sizeof(ANCS_Notification_Attribute_Request_Data_t) * NumberOfAttributes)) != NULL)
               {
                  /* Create an entry in AttributeRequestData for each   */
                  /* attribute being requested.                         */
                  for(Index = 0, Index1 = 1; Index < NumberOfAttributes; Index++)
                  {
                     /* Copy over the Notification Attribute ID.        */
                     TempAttributeID                                     = (ANCS_Notification_Attribute_ID_t)(TempParam->Params[Index1].intParam);
                     AttributeRequestData[Index].NotificationAttributeID = TempAttributeID;

                     /* Check to see if a Max Length parameter is       */
                     /* required.                                       */
                     if((TempAttributeID == idTitle) || (TempAttributeID == idSubtitle) || (TempAttributeID == idMessage))
                     {
                        /* Advance the index of the Parameter List to   */
                        /* get the Max Length parameter for the         */
                        /* attribute.                                   */
                        Index1++;
            ...
            if(!ret_val)
            {
               /* Attempt to encode the request into a raw data buffer  */
               /* to send over GATT.                                    */
               if((ret_val = ANCS_Encode_Notification_Attribute_Request(NotificationUID, NumberOfAttributes, AttributeRequestData, &RawBuffer, &TotalBufferLength)) == 0)
               {
                  /* Attempt to send the request over GATT.             */
                  if((ret_val = GATT_Write_Request(BluetoothStackID, ConnectionID, DeviceInfo->ClientInfo.ControlPointHandle, TotalBufferLength, RawBuffer, GATT_ClientEventCallback_ANCS, 0)) > 0)
                  {
                     printf("GATT_Write_Request() Success. Transaction ID: %d\r\n", ret_val);

                     /* Note the current request.                       */
                     CurrentRequestType     = dtNotification;
                     NumAttributesRequested = NumberOfAttributes;
                  ...
                  {
                     printf("GATT_Write_Request() Error: %d\r\n", ret_val);
                     ret_val = FUNCTION_ERROR;
                  }

                  /* Free the raw data buffer that was allocated.       */
                  ANCS_Free_Notification_Attribute_Request(RawBuffer);
               }
               else
               {
                  /* Error sending the command, inform user and flag an */
                  /* error.                                             */
                  printf("ANCS_Encode_Notification_Attribute_Request() Failure: %d\r\n", ret_val);
      ...
      {
         printf("There is no currently connected ANCS service.\r\n");
   ...
   {
      /* One or more of the necessary parameters is/are invalid.        */
      printf("Usage: GetNotificationAttributes [NotificationUID] [Attribute Request Data].\r\n");
   ...
   /* The following function is responsible for formatting and sending  */
   /* an App Attribute request to a remote ANCS service.                */
static int GetAppAttributes(ParameterList_t *TempParam)
{
   int                                Index;
   int                                Index1;
   ...
   unsigned int                       NumberOfAttributes;
   unsigned int                       TotalBufferLength;
   ANCS_App_Attribute_ID_t            TempAttributeID;
   ANCS_App_Attribute_Request_Data_t *AttributeRequestData;
   DeviceInfo_t                      *DeviceInfo;
      ...
      if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters >= 2) && (TempParam->Params[0].strParam))
         {
            ...
            for(Index = 1; Index < TempParam->NumberofParameters; Index++)
            {
               ...
               TempAttributeID = (ANCS_App_Attribute_ID_t)(TempParam->Params[Index].intParam);
            ...
            if(!ret_val)
            {
               /* Allocate enough memory for each Attribute ID and its  */
               /* length.                                               */
               if((AttributeRequestData = (ANCS_App_Attribute_Request_Data_t *)BTPS_AllocateMemory(sizeof(ANCS_App_Attribute_Request_Data_t) * NumberOfAttributes)) != NULL)
               {
                  /* Create an entry in AttributeRequestData for each   */
                  /* attribute being requested.                         */
                  for(Index = 0, Index1 = 1; Index < NumberOfAttributes; Index++)
                  {
                     ...
                     TempAttributeID                                     = (ANCS_App_Attribute_ID_t)(TempParam->Params[Index1].intParam);
            ...
            if(!ret_val)
            {
               /* Attempt to encode the request into a raw data buffer  */
               /* to send over GATT.                                    */
               if((ret_val = ANCS_Encode_App_Attribute_Request(TempParam->Params[0].strParam, NumberOfAttributes, AttributeRequestData, &RawBuffer, &TotalBufferLength)) == 0)
               {
                  ...
                  if((ret_val = GATT_Write_Request(BluetoothStackID, ConnectionID, DeviceInfo->ClientInfo.ControlPointHandle, TotalBufferLength, RawBuffer, GATT_ClientEventCallback_ANCS, 0)) > 0)
                  {
                     ...
                     /* Note the current request.                       */
                     CurrentRequestType     = dtApplication;
                     NumAttributesRequested = NumberOfAttributes;
                  ...
                  /* Free the allocated raw buffer.                     */
                  ANCS_Free_App_Attribute_Request(RawBuffer);
               }
               else
               {
                  /* Error sending the command, inform user and flag an */
                  /* error.                                             */
                  printf("ANCS_Encode_App_Attribute_Request() Failure: %d\r\n", ret_val);
         ...
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: GetAppAttributes [AppIdentifier] [Attribute Request Data].\r\n");
            printf("Where: Attribute Request Data = \r\n");
            printf("                A list of App Attribute IDs, each followed by a max length parameter (if required)");
            printf("                (Note: Use PrintAttributeIDs 0 to obtain a list of valid App Attribute IDs.)\r\n");
         }
      }
      else
      {
   ...
   if((BluetoothStackID) && (GATT_Client_Event_Data))
   {
      ...
      switch(GATT_Client_Event_Data->Event_Data_Type)
      {
         ...
         case etGATT_Client_Read_Response:
            ...
            {
               /* ANCS does use any Readable characteristics.           */
            }
            else
...
static void BTPSAPI GATT_Connection_Event_Callback(unsigned int BluetoothStackID, GATT_Connection_Event_Data_t *GATT_Connection_Event_Data, unsigned long CallbackParameter)
{
   ...
   BoardStr_t                         BoardStr;
   unsigned int                       Index;
   ANCS_Parsed_Attribute_Data_t       ParsedAttributeData;
   ANCS_Notification_Received_Data_t  NotificationReceivedData;
   ...
   if((BluetoothStackID) && (GATT_Connection_Event_Data))
   {
      ...
      switch(GATT_Connection_Event_Data->Event_Data_Type)
      {
            ...
            if(GATT_Connection_Event_Data->Event_Data.GATT_Device_Connection_Data)
            {
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, GATT_Connection_Event_Data->Event_Data.GATT_Device_Connection_Data->RemoteDevice)) != NULL)
               {
                  if(LocalDeviceIsMaster)
                  {
                     ...
                     /* supported.                                      */
                     GATT_Exchange_MTU_Request(BluetoothStackID, ConnectionID, BTPS_CONFIGURATION_GATT_DEFAULT_MAXIMUM_SUPPORTED_MTU_SIZE, GATT_ClientEventCallback_ANCS, 0);
                  }
               }
            }
            else
         ...
         case etGATT_Connection_Server_Notification:
            ...
            {
               /* Check which attribute we are receiving this           */
               /* notification from.                                    */
               ...
               {
                  if(GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data->AttributeHandle == DeviceInfo->ClientInfo.NotificationSourceHandle)
                  {
                     /* Attempt to decode this as a new ANCS            */
                     /* notification.                                   */
                     if(!(Result = ANCS_Decode_Notification_Received_Data(GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data->AttributeValueLength, GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data->AttributeValue, &NotificationReceivedData)))
                     {
                        printf("\r\nNew Notification:\r\n");
                        printf("    EventID: %u\r\n", NotificationReceivedData.EventID);
                        printf("    EventFlags: %08X\r\n", NotificationReceivedData.EventFlags);
                        printf("    CategoryID: %u\r\n", NotificationReceivedData.CategoryID);
                        printf("    Category Count: %u\r\n", NotificationReceivedData.CategoryCount);
                        printf("    UID: %u\r\n", NotificationReceivedData.NotificationUID);
                     }
                     else
                        printf("ANCS_Decode_Notification_Received_Data() Failed: %d\r\n", Result);
                  }
                  else if(GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data->AttributeHandle == DeviceInfo->ClientInfo.DataSourceHandle)
                  {
                     /* Copy in the attribute response data to the      */
                     /* current buffer.                                 */
                     BTPS_MemCopy(&CurrentBuffer[CurrentBufferSize], GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data->AttributeValue, GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data->AttributeValueLength);
                     CurrentBufferSize += GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data->AttributeValueLength;

                     /* Check if we have received the entire response.  */
                     if(ANCS_Is_Buffer_Complete(CurrentBufferSize, CurrentBuffer, NumAttributesRequested))
                     {
                        /* Response is complete, attempt to parse it.   */
                        if(!(Result = ANCS_Decode_Attribute_Data(NumAttributesRequested, CurrentBufferSize, CurrentBuffer, &ParsedAttributeData)))
                        {
                           /* Response parsed, print out the data.      */
                           printf("\r\nAttribute Response:\r\n");
                           printf("    Request Type: %s\r\n", (ParsedAttributeData.Type == dtNotification)?"Notification Attributes":"App Attributes");
                           if(ParsedAttributeData.Type == dtNotification)
                              printf("    NUID: %u\r\n", ParsedAttributeData.RequestIdentifier.NotificationUID);
                           else
                              printf("    AppIdentifier: %s\r\n", ParsedAttributeData.RequestIdentifier.AppIdentifier);

                           printf("    Attribute List:\r\n");
                           for(Index=0;Index<ParsedAttributeData.NumberOfAttributes;Index++)
                           {
                              printf("\r\n");
                              printf("       ID: %u\r\n", ParsedAttributeData.AttributeData[Index].AttributeID.NotificationAttributeID);
                              printf("       Data: %s\r\n", ParsedAttributeData.AttributeData[Index].AttributeData);
                           }

                           /* Free the allocated parsed data.           */
                           ANCS_Free_Parsed_Attribute_Data(&ParsedAttributeData);
                        }
                        else
                           printf("ANCS_Decode_Attribute_Data() Error: %d\r\n", Result);

                        /* Flag that the buffer is now empty.           */
                        CurrentBufferSize = 0;
                     }
                     else
                        printf("Attribute Response is not complete. Waiting for more data.\r\n");
                  }
               }
               else
         ...
         switch(GATT_Service_Discovery_Event_Data->Event_Data_Type)
         {
            case etGATT_Service_Discovery_Indication:
               ...
               if(GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data)
               {
                  ...
                  /* Service.                                           */
                  ANCSPopulateHandles(DeviceInfo, GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data);
               }
               break;

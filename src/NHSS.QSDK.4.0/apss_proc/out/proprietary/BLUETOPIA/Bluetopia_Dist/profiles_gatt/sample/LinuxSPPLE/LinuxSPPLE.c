...
typedef struct _tagParam_Config_t
{
   ...
   GAP_Discoverability_Mode_t         DiscoverabilityMode;
   Boolean_t                          AdvertiseBREDRSupport;
} Param_Config_t;
...
static void DisplayLEParams(int Context)
{
   ...
   switch(Context)
   {
      ...
      case CONFIG_ADVERTISING_PARAMETERS:
         ...
         printf("   Advertise BR/EDR Support: %s\n",ParameterConfiguration.AdvertiseBREDRSupport?"True":"False");
         break;
...
static void DisplayLEParamsUsage(int Context)
{
   ...
   else
   {
      if(Context == CONFIG_ADVERTISING_PARAMETERS)
      {
         ...
         printf("             [0 = None]\n");
         printf("             [1 = WhiteList]\n");
         printf("         Index 6. Advertise BR/EDR Support.\n");
         printf("             [0 = False]\n");
         printf("             [1 = True]\n");
      }
      else
   ...
   if(!BluetoothStackID)
   {
      ...
      if(HCI_DriverInformation)
      {
         ...
         if(Result > 0)
         {
            ...
            ParameterConfiguration.AdvertiseBREDRSupport                    = FALSE;
            ...
            /* Generate a static random address (only set once per power*/
            /* cycle).                                                  */
            GAP_LE_Generate_Static_Address(BluetoothStackID, &BD_ADDR);

            /* Now assign it to the chip.                               */
            Result = GAP_LE_Set_Random_Address(BluetoothStackID, BD_ADDR);
            ...
            if(!Result)
            {
               ...
               printf("Unable to set random address: %d\n", Result);
...
static int SetLEParams(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
            ...
            case CONFIG_ADVERTISING_PARAMETERS:
               switch(index)
               {
                  ...
                  case 4:
                     ...
                     break;
                  case 5:
                     ParameterConfiguration.AdvertiseBREDRSupport = (Boolean_t)TempParam->Params[2].intParam;
                     break;
               }
               break;
            case CONFIG_CONNECTABILITY_PARAMETERS:
      ...
      if((ret_val == INVALID_PARAMETERS_ERROR) && (context > 0))
      {
   ...
   if(BluetoothStackID)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].intParam >= 0) && (TempParam->Params[0].intParam <= 1))
      {
         ...
         else
         {
            ...
            if(!ParameterConfiguration.AdvertiseBREDRSupport)

...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXHRP - Linux Bluetooth Heart Rate Monitor Profile using GATT (LE)     */
/*             application.                                                   */
/*                                                                            */
/*  Author:  Ryan Byrne                                                       */
/*                                                                            */
...
#include "LinuxHRP.h"            /* Application Header.                       */
...
#define DEFAULT_DEBUG_LOG_FILE_NAME                "LinuxHRP_ASC.log"
#define DEFAULT_DEBUG_FTS_FILE_NAME                "LinuxHRP_FTS.log"

   /* Determine the Name we will use for this compilation.              */
#define LE_DEMO_DEVICE_NAME                        "LinuxHRP"
            ...
            if(CurrentCharacteristic->Characteristic_UUID.UUID_Type == guUUID_16)
            {
               ...
               if(!HRS_COMPARE_HRS_HEART_RATE_MEASUREMENT_UUID_TO_UUID_16(CurrentCharacteristic->Characteristic_UUID.UUID.UUID_16))
               {
                  if(!HRS_COMPARE_HRS_BODY_SENSOR_LOCATION_UUID_TO_UUID_16(CurrentCharacteristic->Characteristic_UUID.UUID.UUID_16))
                  {
                     ...
                     else
                     {
                        ...
                        if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_WRITE))
                           printf("Warning - Mandatory write property of Heart Rate Control Point characteristic not supported!\r\n");
                  ...
                  else
                  {
                     ...
                     if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_READ))
                        printf("Warning - Mandatory read property of Body Sensor Location characteristic not supported!\r\n");
               ...
               else
               {
                  ...
                  if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_NOTIFY))
                     printf("Warning - Mandatory notify property of Heart Rate Measurement characteristic not supported!\r\n");
               }
      ...
      if(!Result)
      {
         ...
         if(PrintData)
         {
            ...
            if(Heart_Rate_Measurement.Flags & HRS_HEART_RATE_MEASUREMENT_FLAGS_ENERGY_EXPENDED_PRESENT)
               printf("      Energy Expended:   %u\r\n", Heart_Rate_Measurement.Energy_Expended);

            printf("      Sensor Contact:    ");
                     ...
                     if(ret_val->Flags & HRS_HEART_RATE_MEASUREMENT_FLAGS_RR_INTERVAL_PRESENT)
                     {
                        ...
                        if(ret_val->Number_Of_RR_Intervals != 0)
                        {
                           ...
                           for(Index = 0; Index < (ret_val->Number_Of_RR_Intervals - 1); Index++)
                              printf("%d, ", ret_val->RR_Intervals[Index]);
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("* Command Options HRP:     RegisterHRS, UnregisterHRS,           *\r\n");
   printf("*                          DiscoverHRS, ConfigureRemoteHRS,      *\r\n");
   printf("*                          NotifyHeartRate,                      *\r\n");
   printf("*                          GetBodySensorLocation,                *\r\n");
   printf("*                          SetBodySensorLocation,                *\r\n");
   printf("*                          ResetEnergyExpended                   *\r\n");
   printf("******************************************************************\r\n");
...
static int RegisterHRS(ParameterList_t *TempParam)
{
   ...
   if(!ConnectionID)
   {
      ...
      if(!HRSInstanceID)
      {
         ...
         ret_val = HRS_Initialize_Service(BluetoothStackID, HRS_HEART_RATE_CONTROL_POINT_RESET_ENERGY_EXPENDED_SUPPORTED, HRS_EventCallback, 0, &HRSInstanceID);
         if((ret_val > 0) && (HRSInstanceID > 0))
         {
            /* Display success message.                                 */
            printf("Successfully registered HRP Service.\r\n");
      ...
      {
         printf("HRS Service already registered.\r\n");
...
static int UnregisterHRS(ParameterList_t *TempParam)
{
   ...
   if(HRSInstanceID)
   {
     ...
     ret_val = HRS_Cleanup_Service(BluetoothStackID, HRSInstanceID);
      ...
      {
         /* Display success message.                                    */
         printf("Successfully unregistered HRS Service.\r\n");
      ...
      printf("HRS Service not registered.\r\n");
...
static int DiscoverHRS(ParameterList_t *TempParam)
{
   ...
   if(!HRSInstanceID)
   {
      ...
      printf("Cannot discover HRS Services when registered as a service.\r\n");
            ...
            if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
            {
               ...
               if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_SERVICE_DISCOVERY_COMPLETE)
               {
                  ...
                  if(DeviceInfo->ClientInfo.Heart_Rate_Measurement_Client_Configuration)
                  {
                     ...
                     printf("   Error - Heart Rate Measurment CC not found on this device.\r\n");
      ...
      if(HRSInstanceID)
      {
         if((ConnectionID) && (!COMPARE_NULL_BD_ADDR(ConnectionBD_ADDR)))
         {
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
               {
                  if(DeviceInfo->ServerInfo.Heart_Rate_Measurement_Client_Configuration & GATT_CLIENT_CONFIGURATION_CHARACTERISTIC_NOTIFY_ENABLE)
                  {
                     ...
                     if(HeartRatePtr)
                     {
                        ...
                        BTPS_MemInitialize(HeartRatePtr, 0, sizeof(HRS_Heart_Rate_Measurement_Data_t));
                        ...
                        if((ret_val = HRS_Notify_Heart_Rate_Measurement(BluetoothStackID, HRSInstanceID, ConnectionID, HeartRatePtr))== 0)
                        {
                           if(!(HeartRatePtr->Flags & HRS_HEART_RATE_MEASUREMENT_FLAGS_RR_INTERVAL_PRESENT))
                              printf("HRS_Notify_Heart_Rate_Measurement success.\r\n");
                           else
                              printf("HRS_Notify_Heart_Rate_Measurement success, notified %u intervals.\r\n", (unsigned int)HeartRatePtr->Number_Of_RR_Intervals);
                        }
                        else
                        ...
                        printf("Failed to allocate memory for the Heart Rate Measurement.\r\n");
                  }
                  else
                  {
                     printf("Client has not registered for Heart Rate measurement notifications.\r\n");
            ...
            else
            {
               ...
               /* user.                                                 */
               printf("Usage: NotifyHeartRate [HR (BPM)] ");
               printf("[HR Format (0 = Byte, 1 = Word)] ");
               printf("[Sensor Contact Status (0 = Not Supported, 1 = Supported/Not Detected, 2 = Supported/Detected] ");
               printf("[Energy Expended (0 = Don't send, 1 = Send)] ");
               printf("[RR Intervals (0 = None, X = Number of Intervals)].\r\n");
      ...
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            if(DeviceInfo->ClientInfo.Body_Sensor_Location != 0)
            {
               ...
               printf("Body Sensor Location not supported on remote service.\r\n");
         }
         else
            printf("No Device Info.\r\n");
      }
      else
         printf("Either a HRP server must be registered or a HRP client must be connected.\r\n");
   }
...
static int SetBodySensorLocation(ParameterList_t *TempParam)
{
   ...
   if(HRSInstanceID)
   {
      if((TempParam) && (TempParam->NumberofParameters > 0))
      {
         if(HRS_BODY_SENSOR_LOCATION_VALID_TEMPERATURE_TYPE((Byte_t)TempParam->Params[0].intParam))
         {
            ...
            {
               printf("Body Sensor Location successfully set to %s.\r\n", SensorLocationTable[TempParam->Params[0].intParam]);

               ret_val = 0;
            }
            else
               printf("Error - HRS_Set_Body_Sensor_Location Failed.\r\n");
         }
         else
         {
            printf("Invalid body sensor location. Valid locations are:\r\n");

            for(Index = 0; Index < NUM_SUPPORTED_SENSOR_LOCATIONS; Index++)
               printf("%d - %s\r\n", Index, SensorLocationTable[Index]);
         }
      }
      else
      {
   ...
   {
      if(ConnectionID)
         printf("Cannot write to body sensor location as a client.\r\n");
      else
         printf("HRP server not registered\r\n");
   }
...
static int ResetEnergyExpended(ParameterList_t *TempParam)
{
   ...
   if(!HRSInstanceID)
   {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            if(DeviceInfo->ClientInfo.Heart_Rate_Control_Point != 0)
            {
               ...
               if((ret_val = HRS_Format_Heart_Rate_Control_Command(ccResetEnergyExpended, HRS_HEART_RATE_CONTROL_POINT_VALUE_LENGTH, CommandBuffer)) == 0)
               {
                  ...
                  {
                     printf("Reset Energy Expended request sent, Transaction ID = %u.\r\n", ret_val);
            ...
            {
               printf("Energy Expended not supported on remote service.\r\n");
      ...
      printf("Cannot reset Energy Expended as a service.\r\n");
...
static void BTPSAPI HRS_EventCallback(unsigned int BluetoothStackID, HRS_Event_Data_t *HRS_Event_Data, unsigned long CallbackParameter)
{
   ...
   if((BluetoothStackID) && (HRS_Event_Data))
   {
      switch(HRS_Event_Data->Event_Data_Type)
      {
         case etHRS_Server_Read_Client_Configuration_Request:
            printf("etHRS_Server_Read_Client_Configuration_Request with size %u.\r\n", HRS_Event_Data->Event_Data_Size);
            ...
            if(HRS_Event_Data->Event_Data.HRS_Read_Client_Configuration_Data)
            {
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, HRS_Event_Data->Event_Data.HRS_Read_Client_Configuration_Data->RemoteDevice)) != NULL)
               {
                  ...
                  if(HRS_Event_Data->Event_Data.HRS_Read_Client_Configuration_Data->InstanceID == HRSInstanceID)
                  {
                     ...
                     {
                        case ctHeartRateMeasurement:
                           printf("   Config Type:      ctHeartRateMeasurement.\r\n");
         ...
         case etHRS_Server_Client_Configuration_Update:
            printf("etHRS_Server_Client_Configuration_Update with size %u.\r\n", HRS_Event_Data->Event_Data_Size);
         ...
         case etHRS_Server_Heart_Rate_Control_Point_Command:
            printf("etHRS_Server_Heart_Rate_Control_Point_Command with size %u.\r\n", HRS_Event_Data->Event_Data_Size);
            ...
            if(HRS_Event_Data->Event_Data.HRS_Heart_Rate_Control_Command_Data)
            {
               ...
               switch(HRS_Event_Data->Event_Data.HRS_Heart_Rate_Control_Command_Data->Command)
               {
                  case ccResetEnergyExpended:
                     ...
                     printf("Command:          ccResetEnergyExpended.\r\n");
                     break;
                  default:
   ...
   else
   {
      ...
      printf("HRS Callback Data: Event_Data = NULL.\r\n");
   }
            ...
            if(GATT_Client_Event_Data->Event_Data.GATT_Request_Error_Data)
            {
               ...
               if(GATT_Client_Event_Data->Event_Data.GATT_Request_Error_Data->ErrorType == retErrorResponse)
               {
                  ...
                  else
                  {
                     ...
                     {
                        printf("Error Mesg:      Control Point Not Supported.\r\n");
                     }
                     else
            ...
            if(GATT_Connection_Event_Data->Event_Data.GATT_Device_Connection_Data)
            {
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, GATT_Connection_Event_Data->Event_Data.GATT_Device_Connection_Data->RemoteDevice)) != NULL)
               {
                  if(LocalDeviceIsMaster)
                  {
                     ...
                     /* supported.                                      */
                     GATT_Exchange_MTU_Request(BluetoothStackID, ConnectionID, BTPS_CONFIGURATION_GATT_DEFAULT_MAXIMUM_SUPPORTED_MTU_SIZE, GATT_ClientEventCallback_HRP, 0);
                  }
               }
            }
            else
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data->RemoteDevice)) != NULL)
               {
                  if(DeviceInfo->ClientInfo.Heart_Rate_Measurement == GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data->AttributeHandle)
                  {
                     ...
                     if((HeartRateMeasurement = DecodeDisplayHeartRate(GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data->AttributeValueLength, GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data->AttributeValue, TRUE)) != NULL)
                     {
                        ...
                        DisplayFunctionError("DecodeDisplayHeartRate", (int)HeartRateMeasurement);
                  }
                  else

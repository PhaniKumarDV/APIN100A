...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXANP - Linux Bluetooth Alert Notification Profile using GATT (LE)     */
/*             application.                                                   */
/*                                                                            */
/*  Author:  Cliff Xu                                                         */
/*                                                                            */
...
#include "LinuxANP.h"            /* Application Header.                       */
...
#define MAX_SUPPORTED_COMMANDS                     (47)  /* Denotes the       */
                                                         /* maximum number of */
                                                         /* User Commands that*/
                                                         /* are supported by  */
                                                         /* this application. */
...
#define DEFAULT_DEBUG_LOG_FILE_NAME                "LinuxANP_ASC.log"
#define DEFAULT_DEBUG_FTS_FILE_NAME                "LinuxANP_FTS.log"

   /* Determine the Name we will use for this compilation.              */
#define LE_DEMO_DEVICE_NAME                        "LinxANP"
                     ...
                     if(!ANS_COMPARE_ANS_SUPPORTED_UNREAD_ALERT_CATEGORY_UUID_TO_UUID_16(CurrentCharacteristic->Characteristic_UUID.UUID.UUID_16))
                     {
                        if(!ANS_COMPARE_ANS_UNREAD_ALERT_STATUS_UUID_TO_UUID_16(CurrentCharacteristic->Characteristic_UUID.UUID.UUID_16))
                        {
                           ...
                           else
                           {
                              ...
                              if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_WRITE))
                                 printf("Warning - Mandatory write property of Ringer Control Point characteristic not supported!\r\n");
                        ...
                        else
                        {
                           ...
                           if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_NOTIFY))
                              printf("Warning - Mandatory notify property of Ringer Control Point characteristic not supported!\r\n");
                        }
                     }
                     else
                     {
                        ...
                        if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_READ))
                           printf("Warning - Mandatory read property of Ringer Control Point characteristic not supported!\r\n");
...
static void DisplayUsageAlertCategoryNumericalValue(Boolean_t DisplayAllCategories)
{
   ...
   for(Index = 0; Index < NUM_ALERT_CATEGORIES; Index++)
      printf("     %u    %s\r\n", Index, AlertCategories[Index]);
...
static void DisplayUsageAlertCategoryBitMask(void)
{
   ...
   printf("   Where setting a bit BitMask enables the category.\r\n");
   printf("\r\n   Bit  Category\r\n   ---  --------\r\n");
...
static void DisplaySupportedCategories(Word_t SupportedCategories, char *Prefix, char *CategoryType)
{
   ...
   if(Prefix && CategoryType)
   {
      ...
      for(Index = 0; Index < BTPS_StringLength(CategoryType); Index++)
         printf("-");
      printf("\r\n");

      for(CatagoryIndex = 0; CatagoryIndex < NUM_ALERT_CATEGORIES; CatagoryIndex++)
      {
         printf("%s   %s: ", Prefix, AlertCategories[CatagoryIndex]);

         for(Index = 0; Index < (BTPS_StringLength(AlertCategories[8]) - BTPS_StringLength(AlertCategories[CatagoryIndex])); Index++)
            printf(" ");

         printf("%s Supported\r\n", ((SupportedCategories & (0x1 << CatagoryIndex)) ? "" : "Not "));
      }
...
static void DisplayNewAlertStatus(ANS_Category_Identification_t Category, Byte_t NumAlerts, char *AlertString, char *Prefix)
{
   ...
   if(Prefix)
   {
      ...
      printf("%s   Number of Alerts: %u\r\n", Prefix, NumAlerts);

      /* Print the optional Alert String.                               */
      if(AlertString)
         printf("%s   Alert String:     '%s'\r\n\r\n", Prefix, AlertString);
      else
         printf("\r\n");
   }
}
   ...
   if(ConnectionID)
   {
      ...
      if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
      {
         ...
         if(DeviceInfo->ClientInfo.Control_Point != 0)
         {
            ...
            printf("Error - Control Point not supported on remote service!\r\n");
      }
      else
   ...
   if(BitMask && Prefix)
   {
      ...
      if(BluetoothStackID)
      {
         ...
         if((ConnectionID) && (!COMPARE_NULL_BD_ADDR(ConnectionBD_ADDR)))
         {
            ...
            /* be sent.                                                 */
            i = ANS_ALERT_CATEGORY_ID_SIMPLE_ALERT;
            while((BitMask) && (i <= ANS_ALERT_CATEGORY_ID_INSTANT_MESSAGE))
            {
               if(BitMask & 0x1)
               {
                  ...
                  printf("%sSending New Alert Notification for '%s'... ", Prefix, AlertCategories[i]);
         ...
         if((ConnectionID) && (!COMPARE_NULL_BD_ADDR(ConnectionBD_ADDR)))
         {
            ...
            while((BitMask) && (i <= ANS_ALERT_CATEGORY_ID_INSTANT_MESSAGE))
            {
               if(BitMask & 0x1)
               {
                  ...
                  /* Go ahead and send the Alert Status.                */
                  printf("%sSending Unread Alert Notification for '%s'... ", Prefix, AlertCategories[i]);
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("* Command Options ANP:     RegisterANS, UnregisterANS,           *\r\n");
   printf("*                          DiscoverANS, ConfigureRemoteANS,      *\r\n");
   printf("*                          NotifyNewAlerts, NotifyUnreadAlerts,  *\r\n");
   printf("*                          GetNewAlertStatus, SetNewAlertStatus, *\r\n");
   printf("*                          GetUnreadAlertStatus,                 *\r\n");
   printf("*                          SetUnreadAlertStatus,                 *\r\n");
   printf("*                          GetSupportedNewAlertCategories,       *\r\n");
   printf("*                          SetSupportedNewAlertCategories,       *\r\n");
   printf("*                          GetSupportedUnreadAlertCategories,    *\r\n");
   printf("*                          SetSupportedUnreadAlertCategories,    *\r\n");
   printf("*                          EnableNewAlertNotifications,          *\r\n");
   printf("*                          DisableNewAlertNotifications,         *\r\n");
   printf("*                          EnableUnreadAlertNotifications,       *\r\n");
   printf("*                          DisableUnreadAlertNotifications,      *\r\n");
   printf("*                          NotifyImmediatelyNewAlert,            *\r\n");
   printf("*                          NotifyImmediatelyUnreadAlert          *\r\n");
   printf("******************************************************************\r\n");
   ...
   if((BluetoothStackID) && (!ConnectionID))
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].intParam >= 0) && (TempParam->Params[0].intParam <= 1))
      {
         ...
         else
         {
            ...
            if(ANSInstanceID)
            {
               /* Advertise the Alert Notification Server (1 byte type  */
               /* and 2 bytes UUID.                                     */
...
static int RegisterANS(ParameterList_t *TempParam)
{
   ...
   if(!ConnectionID)
   {
      ...
      if(!ANSInstanceID)
      {
         ...
         {
            /* Display success message.                                 */
            printf("Successfully registered ANP Service.\r\n");
      ...
      {
         printf("ANP Service already registered.\r\n");
...
static int UnregisterANS(ParameterList_t *TempParam)
{
   ...
   if(ANSInstanceID)
   {
      ...
      {
         /* Display success message.                                    */
         printf("Successfully unregistered ANP Service.\r\n");
      ...
      printf("ANP Service not registered.\r\n");
...
static int DiscoverANS(ParameterList_t *TempParam)
{
   ...
   {
      printf("Cannot discover ANP Services when registered as a service.\r\n");
...
static int ConfigureRemoteANS(ParameterList_t *TempParam)
{
   ...
   if(TempParam && (TempParam->NumberofParameters >= 2))
   {
      ...
      if(!ANSInstanceID)
      {
            ...
            if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
            {
               ...
               if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_SERVICE_DISCOVERY_COMPLETE)
               {
                  ...
                  if(DeviceInfo->ClientInfo.New_Alert_Client_Configuration)
                  {
                     ...
                     printf("   Error - New Alert CC not found on this device.\r\n");
                  ...
                  if(ret_val > 0)
                  {
                     ...
                     {
                        printf("   Error - Unread Alert Status CC not found on this device.\r\n");
         ...
         printf("Cannot configure remote ANP Services when registered as a service.\r\n");
   }
   else
   {
      printf("Usage: ConfigureRemoteANS [New Alert Notify (0 = disable, 1 = enable)]\r\n");
      printf("                          [Unread Alert Status Notify (0 = disable, 1 = enable)]\r\n");
   }
   ...
   if((TempParam) && (TempParam->NumberofParameters > 0) && ((TempParam->Params[0].intParam & (~SUPPORTED_CATEGORIES_MASK)) == 0))
   {
         ...
         if((ConnectionID) && (!COMPARE_NULL_BD_ADDR(ConnectionBD_ADDR)))
         {
            if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
            {
               ...
               if(DeviceInfo->ServerInfo.New_Alert_Client_Configuration & GATT_CLIENT_CONFIGURATION_CHARACTERISTIC_NOTIFY_ENABLE)
               {
                  ...
                  DesiredNotifications = (Word_t)TempParam->Params[0].intParam;
                  ret_val              = 0;
                  i                    = 0;
                  ...
                  while(DesiredNotifications)
                  {
                     if(DesiredNotifications & 0x1)
                     {
                        if(!(EnabledNotifications & 0x1))
                        {
                           ...
                           printf("Error - Client has not enabled '%s' for New Alert notifications.\r\n", AlertCategories[i]);
                        }
                     }
                  ...
                  if(ret_val == 0)
                  {
                     SendNewAlertNotification((Word_t)TempParam->Params[0].intParam, "");
                  }
               }
               else
               {
                  printf("Client has not registered for New Alert notifications.\r\n");
      ...
      else
      {
         if(ConnectionID)
            ...
            printf("Error - ANP server not registered\r\n");
      }
   }
   else
   {
         ...
         if((ConnectionID) && (!COMPARE_NULL_BD_ADDR(ConnectionBD_ADDR)))
         {
            if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
            {
               ...
               if(DeviceInfo->ServerInfo.Unread_Alert_Status_Client_Configuration & GATT_CLIENT_CONFIGURATION_CHARACTERISTIC_NOTIFY_ENABLE)
               {
                  ...
                  while(DesiredNotifications)
                  {
                     if(DesiredNotifications & 0x1)
                     {
                        if(!(EnabledNotifications & 0x1))
                        {
                           ...
                           printf("Error - Client has not enabled '%s' for Unread Alert notifications.\r\n", AlertCategories[i]);
                        }
                     }
                  ...
                  if(ret_val == 0)
                  {
                     ret_val = SendUnreadAlertNotification((Word_t)TempParam->Params[0].intParam, "");
                  }
               }
               else
               {
                  printf("Client has not registered for Unread Alert notifications.\r\n");
      ...
      if((ret_val = ANS_Query_Supported_Categories(BluetoothStackID, ANSInstanceID, scNewAlert, &SupportedCategories)) == 0)
      {
         if(SupportedCategories & (0x1 << TempParam->Params[0].intParam))
         {
            ...
            if(BluetoothStackID)
            {
               ...
               {
                  if(ConnectionID)
                     printf("Cannot get new alert statuses as a client.\r\n");
                  else
                     printf("ANP server not registered\r\n");
               }
            }
            else
            {
         ...
         if(SupportedCategories & (0x1 << TempParam->Params[0].intParam))
         {
            ...
            if(BluetoothStackID)
            {
               ...
               if(ANSInstanceID)
               {
                  ...
                  NewAlertStatuses[TempParam->Params[0].intParam].NumberOfAlerts = (Byte_t)TempParam->Params[1].intParam;
                  ...
                  printf("Set New Alert Status success.\r\n");
               ...
               {
                  if(ConnectionID)
                     printf("Cannot set new alert statuses as a client.\r\n");
                  else
                     printf("ANP server not registered\r\n");
               }
            }
      ...
      if((ret_val = ANS_Query_Supported_Categories(BluetoothStackID, ANSInstanceID, scUnreadAlertStatus, &SupportedCategories)) == 0)
      {
         if(SupportedCategories & (0x1 << TempParam->Params[0].intParam))
         {
            ...
            if(BluetoothStackID)
            {
               ...
               {
                  if(ConnectionID)
                     printf("Cannot get unread alert statuses as a client.\r\n");
                  else
                     printf("ANP server not registered\r\n");
               }
            }
      ...
      if((ret_val = ANS_Query_Supported_Categories(BluetoothStackID, ANSInstanceID, scUnreadAlertStatus, &SupportedCategories)) == 0)
      {
         if(SupportedCategories & (0x1 << TempParam->Params[0].intParam))
         {
            ...
            if(BluetoothStackID)
            {
               ...
               {
                  /* Set the number of alerts.                          */
                  UnreadAlertStatuses[TempParam->Params[0].intParam] = (Byte_t)TempParam->Params[1].intParam;

                  printf("Set Unread Alert Status success.\r\n");
               ...
               {
                  if(ConnectionID)
                     printf("Cannot set unread alert statuses as a client.\r\n");
                  else
                     printf("ANP server not registered\r\n");
               }
            }
      ...
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            if(DeviceInfo->ClientInfo.Supported_New_Alert_Category != 0)
            {
               ...
               {
                  printf("Get Supported New Alert Category Request sent, Transaction ID = %u", ret_val);
               ...
               printf("Error - Supported New Alert Category not supported on remote service.\r\n");
         }
         else
            printf("No Device Info.\r\n");
      }
      else
         printf("Either a ANP server must be registered or a ANP client must be connected.\r\n");
   }
...
static int SetSupportedNewAlertCategories(ParameterList_t *TempParam)
{
   ...
   if((TempParam) && (TempParam->NumberofParameters > 0) && ((TempParam->Params[0].intParam == 0) || ((TempParam->Params[0].intParam & (~SUPPORTED_CATEGORIES_MASK)) == 0)))
   {
      ...
      {
         /* Set the supported categories                                */
         SupportedCategories = (Word_t)TempParam->Params[0].intParam;
         ...
         if((ret_val = ANS_Set_Supported_Categories(BluetoothStackID, ANSInstanceID, scNewAlert, SupportedCategories)) == 0)
         {
            ...
            while(DeviceInfo)
            {
               if(DeviceInfo->ServerInfo.Enabled_New_Alert_Categories & (~SupportedCategories))
               {
                  ...
                  printf("Warning - Disabling previously supported notification(s) for device %s.\r\n", BoardStr);
            ...
            printf("Supported New Alert Category successfully set.\r\n");
         }
         else
      ...
      {
         if(ConnectionID)
            printf("Cannot write to Supported New Alert Category as a client.\r\n");
         else
            printf("ANP server not registered\r\n");
      }
   }
      ...
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            if(DeviceInfo->ClientInfo.Supported_Unread_Alert_Category != 0)
            {
               ...
               {
                  printf("Get Supported Unread Alert Category Request sent, Transaction ID = %u", ret_val);
               ...
               printf("Error - Supported Unread Alert Category not supported on remote service.\r\n");
         }
         else
            printf("No Device Info.\r\n");
      }
...
static int SetSupportedUnreadAlertCategories(ParameterList_t *TempParam)
{
   ...
   if((TempParam) && (TempParam->NumberofParameters > 0) && ((TempParam->Params[0].intParam == 0) || ((TempParam->Params[0].intParam & (~SUPPORTED_CATEGORIES_MASK)) == 0)))
   {
      ...
      if(ANSInstanceID)
      {
         ...
         if((ret_val = ANS_Set_Supported_Categories(BluetoothStackID, ANSInstanceID, scUnreadAlertStatus, SupportedCategories)) == 0)
         {
            ...
            while(DeviceInfo)
            {
               ...
               {
                  BD_ADDRToStr(DeviceInfo->ConnectionBD_ADDR, BoardStr);
                  printf("Warning - Disabling previously supported notifications for device %s.\r\n", BoardStr);
            ...
            printf("Supported Unread Alert Category successfully set.\r\n");
         }
         else
            DisplayFunctionError("ANS_Set_Supported_Categories", ret_val);
      }
      ...
      {
         if(ConnectionID)
            printf("Cannot write to Supported Unread Alert Category as a client.\r\n");
         else
            printf("ANP server not registered\r\n");
      }
   }
   ...
   if(!ANSInstanceID)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters > 0) && (((TempParam->Params[0].intParam >= ciSimpleAlert) && (TempParam->Params[0].intParam <= (ciInstantMessage + 1)))))
      {
         ...
         if((ret_val = WriteControlPointCommand(pcEnable_New_Alert_Notifications, ((ANS_Category_Identification_t)TempParam->Params[0].intParam))) == 0)
            printf("Enable New Alert Notifications command request sent.\r\n");
      }
      else
      {
      ...
      printf("Cannot Enable New Alert Notifications as a service.\r\n");
   ...
   if(!ANSInstanceID)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters > 0) && (((TempParam->Params[0].intParam >= ciSimpleAlert) && (TempParam->Params[0].intParam <= (ciInstantMessage + 1)))))
      {
         ...
         if((ret_val = WriteControlPointCommand(pcDisable_New_Alert_Notifications, ((ANS_Category_Identification_t)TempParam->Params[0].intParam))) == 0)
            printf("Disable New Alert Notifications command request sent.\r\n");
      }
      else
      {
      ...
      printf("Cannot Disable New Alert Notifications as a service.\r\n");
   ...
   if(!ANSInstanceID)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters > 0) && (((TempParam->Params[0].intParam >= ciSimpleAlert) && (TempParam->Params[0].intParam <= (ciInstantMessage + 1)))))
      {
         ...
         if((ret_val = WriteControlPointCommand(pcEnable_Unread_Category_Notifications, ((ANS_Category_Identification_t)TempParam->Params[0].intParam))) == 0)
            printf("Enable Unread Category Notifications command request sent.\r\n");
      }
      else
      {
      ...
      printf("Cannot Enable Unread Category Notifications as a service.\r\n");
   ...
   if(!ANSInstanceID)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters > 0) && (((TempParam->Params[0].intParam >= ciSimpleAlert) && (TempParam->Params[0].intParam <= (ciInstantMessage + 1)))))
      {
         ...
         if((ret_val = WriteControlPointCommand(pcDisable_Unread_Category_Notifications, ((ANS_Category_Identification_t)TempParam->Params[0].intParam))) == 0)
            printf("Disable Unread Category Notifications command request sent.\r\n");
      }
      else
      {
      ...
      printf("Cannot Disable Unread Alert Notifications as a service.\r\n");
   ...
   if(!ANSInstanceID)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters > 0) && (((TempParam->Params[0].intParam >= ciSimpleAlert) && (TempParam->Params[0].intParam <= (ciInstantMessage + 1)))))
      {
         ...
         if((ret_val = WriteControlPointCommand(pcNotify_New_Alert_Immediately, ((ANS_Category_Identification_t)TempParam->Params[0].intParam))) == 0)
            printf("Notify New Alert Immediately command request sent.\r\n");
      }
      else
      {
      ...
      printf("Cannot Notify New Alert Immediately as a service.\r\n");
   ...
   if(!ANSInstanceID)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters > 0) && (((TempParam->Params[0].intParam >= ciSimpleAlert) && (TempParam->Params[0].intParam <= (ciInstantMessage + 1)))))
      {
         ...
         if((ret_val = WriteControlPointCommand(pcNotify_Unread_Category_Immediately, ((ANS_Category_Identification_t)TempParam->Params[0].intParam))) == 0)
            printf("Notify Unread Category Immediately command request sent.\r\n");
      }
      else
      {
      ...
      printf("Cannot Notify Unread Category Immediately as a service.\r\n");
...
static void BTPSAPI ANS_EventCallback(unsigned int BluetoothStackID, ANS_Event_Data_t *ANS_Event_Data, unsigned long CallbackParameter)
{
   ...
   if((BluetoothStackID) && (ANS_Event_Data))
   {
      switch(ANS_Event_Data->Event_Data_Type)
      {
         case etANS_Server_Read_Client_Configuration_Request:
            printf("etANS_Server_Read_Client_Configuration_Request with size %u.\r\n", ANS_Event_Data->Event_Data_Size);
                  ...
                  if(ANS_Event_Data->Event_Data.ANS_Read_Client_Configuration_Data->InstanceID == ANSInstanceID)
                  {
                     ...
                     switch(ANS_Event_Data->Event_Data.ANS_Read_Client_Configuration_Data->ClientConfigurationType)
                     {
                        ...
                        case ctUnreadAlertStatus:
                           printf("   Config Type:      ctUnreadAlertStatus.\r\n");
         ...
         case etANS_Server_Client_Configuration_Update:
            printf("etANS_Server_Client_Configuration_Update with size %u.\r\n", ANS_Event_Data->Event_Data_Size);
         ...
         case etANS_Server_Control_Point_Command_Indication:
            printf("etANS_Server_Control_Point_Command_Indication with size %u.\r\n", ANS_Event_Data->Event_Data_Size);
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
               {
                  ...
                  switch(ANS_Event_Data->Event_Data.ANS_Control_Point_Command_Data->Command)
                  {
                     case pcEnable_New_Alert_Notifications:
                        printf("   Command:          pcEnable_New_Alert_Notifications.\r\n\r\n");
                        ...
                        if((Result = ANS_Query_Supported_Categories(BluetoothStackID, ANSInstanceID, scNewAlert, &SupportedCategories)) == 0)
                        {
                           ...
                           if(ANS_Event_Data->Event_Data.ANS_Control_Point_Command_Data->Category == ciAllCategories)
                           {
                              ...
                              if(SupportedCategories)
                              {
                                 ...
                                 /* client.                             */
                                 DeviceInfo->ServerInfo.Enabled_New_Alert_Categories = SUPPORTED_CATEGORIES_MASK;

                                 printf("Enabling all New Alert categories.\r\n");
                              }
                              else
                                 printf("Server does not support ANY New Alert categories!\r\n");
                           }
                           else
                           {
                              ...
                              if(SupportedCategories & (0x1 << ANS_Event_Data->Event_Data.ANS_Control_Point_Command_Data->Category))
                              {
                                 ...
                                 printf("Enabling '%s' New Alert category.\r\n", AlertCategories[ANS_Event_Data->Event_Data.ANS_Control_Point_Command_Data->Category]);
                              }
                              else
                                 printf("Server does not support desired New Alert category.\r\n");
                           }
                        }
                        else
                     ...
                     case pcEnable_Unread_Category_Notifications:
                        printf("   Command:          pcEnable_Unread_Category_Notifications.\r\n\r\n");
                        ...
                        if((Result = ANS_Query_Supported_Categories(BluetoothStackID, ANSInstanceID, scUnreadAlertStatus, &SupportedCategories)) == 0)
                        {
                           ...
                           if(ANS_Event_Data->Event_Data.ANS_Control_Point_Command_Data->Category == ciAllCategories)
                           {
                              ...
                              if(SupportedCategories)
                              {
                                 /* Enable all categories for this      */
                                 /* client.                             */
                                 DeviceInfo->ServerInfo.Enabled_Unread_Alert_Categories = SUPPORTED_CATEGORIES_MASK;

                                 printf("Enabling all Unread Alert categories.\r\n");
                              }
                              else
                                 printf("Server does not support ANY Unread Alert categories!\r\n");
                           }
                           else
                           {
                              /* Verify that the server supports the    */
                              ...
                              if(SupportedCategories & (0x1 << ANS_Event_Data->Event_Data.ANS_Control_Point_Command_Data->Category))
                              {
                                 ...
                                 printf("Enabling '%s' Unread Alert category.\r\n", AlertCategories[ANS_Event_Data->Event_Data.ANS_Control_Point_Command_Data->Category]);
                              }
                              else
                                 printf("Server does not support desired Unread Alert category.\r\n");
                           }
                        }
                        else
                           DisplayFunctionError("ANS_Query_Supported_Categories", Result);
                     ...
                     case pcDisable_New_Alert_Notifications:
                        printf("   Command:          pcDisable_New_Alert_Notifications.\r\n\r\n");
                        ...
                        if(ANS_Event_Data->Event_Data.ANS_Control_Point_Command_Data->Category == ciAllCategories)
                        {
                           ...
                           printf("Disabling all New Alert categories.\r\n");
                        }
                        else
                        {
                           ...
                           printf("Disabling '%s' New Alert category.\r\n", AlertCategories[ANS_Event_Data->Event_Data.ANS_Control_Point_Command_Data->Category]);
                        }
                        break;

                     case pcDisable_Unread_Category_Notifications:
                        printf("   Command:          pcDisable_Unread_Category_Notifications.\r\n\r\n");
                        ...
                        if(ANS_Event_Data->Event_Data.ANS_Control_Point_Command_Data->Category == ciAllCategories)
                        {
                           ...
                           printf("Disabling all Unread Alert categories.\r\n");
                        }
                        else
                        {
                           /* Disable the selected category for this    */
                     ...
                     case pcNotify_New_Alert_Immediately:
                        printf("   Command:          pcNotify_New_Alert_Immediately.\r\n\r\n");
                           ...
                           if((Result = ANS_Query_Supported_Categories(BluetoothStackID, ANSInstanceID, scNewAlert, &SupportedCategories)) == 0)
                           {
                              ...
                              if(ANS_Event_Data->Event_Data.ANS_Control_Point_Command_Data->Category == ciAllCategories)
                              {
                                 ...
                                 if(SupportedCategories)
                                 {
                                    ...
                                    if(DeviceInfo->ServerInfo.Enabled_New_Alert_Categories & SupportedCategories)
                                    {
                                       ...
                                       printf("Client has not enabled any supported New Alert categories!\r\n");
                                 }
                                 else
                                    printf("Server does not support ANY New Alert categories!\r\n");
                              }
                              else
                              {
                                 ...
                                 if(SupportedCategories & (0x1 << ANS_Event_Data->Event_Data.ANS_Control_Point_Command_Data->Category))
                                 {
                                    ...
                                    /* requesting.                      */
                                    if(DeviceInfo->ServerInfo.Enabled_New_Alert_Categories & (0x1 << ANS_Event_Data->Event_Data.ANS_Control_Point_Command_Data->Category))
                                    {
                                       ...
                                       printf("Client has not enabled the requested New Alert category!\r\n");
                                 }
                                 else
                                    printf("Server does not support desired New Alert category.\r\n");
                              }
                     ...
                     case pcNotify_Unread_Category_Immediately:
                        printf("   Command:          pcNotify_Unread_Category_Immediately.\r\n\r\n");
                           ...
                           if((Result = ANS_Query_Supported_Categories(BluetoothStackID, ANSInstanceID, scUnreadAlertStatus, &SupportedCategories)) == 0)
                           {
                              ...
                              if(ANS_Event_Data->Event_Data.ANS_Control_Point_Command_Data->Category == ciAllCategories)
                              {
                                 ...
                                 if(SupportedCategories)
                                 {
                                    ...
                                    if(DeviceInfo->ServerInfo.Enabled_Unread_Alert_Categories & SupportedCategories)
                                    {
                                       ...
                                       printf("Client has not enabled any supported Unread Alert categories!\r\n");
                                 }
                                 else
                                    printf("Server does not support ANY Unread Alert categories!\r\n");
                              }
                              else
                              {
                                 ...
                                 if(SupportedCategories & (0x1 << ANS_Event_Data->Event_Data.ANS_Control_Point_Command_Data->Category))
                                 {
                                    ...
                                    /* enabled the category it is       */
                                    /* requesting.                      */
                                    if(DeviceInfo->ServerInfo.Enabled_Unread_Alert_Categories & (0x1 << ANS_Event_Data->Event_Data.ANS_Control_Point_Command_Data->Category))
                                    {
                                       /* Enable the selected category  */
                                       /* for this client.              */
                                       ...
                                       printf("Client has not enabled the requested Unread Alert category!\r\n");
                                 }
                                 else
                                    printf("Server does not support desired Unread Alert category.\r\n");
                              }
   ...
   else
   {
      ...
      printf("ANS Callback Data: Event_Data = NULL.\r\n");
   }
            ...
            if(GATT_Client_Event_Data->Event_Data.GATT_Request_Error_Data)
            {
               ...
               if(GATT_Client_Event_Data->Event_Data.GATT_Request_Error_Data->ErrorType == retErrorResponse)
               {
                  ...
                  else
                  {
                     ...
                     {
                        printf("Error Mesg:      Not Supported.\r\n");
                     }
                     else
            ...
            if(GATT_Connection_Event_Data->Event_Data.GATT_Device_Connection_Data)
            {
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, GATT_Connection_Event_Data->Event_Data.GATT_Device_Connection_Data->RemoteDevice)) != NULL)
               {
                  if(LocalDeviceIsMaster)
                  {
                     ...
                     /* supported.                                      */
                     GATT_Exchange_MTU_Request(BluetoothStackID, ConnectionID, BTPS_CONFIGURATION_GATT_DEFAULT_MAXIMUM_SUPPORTED_MTU_SIZE, GATT_ClientEventCallback_ANP, 0);
                  }
               }
            }
            else
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data->RemoteDevice)) != NULL)
               {
                  if(DeviceInfo->ClientInfo.New_Alert == GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data->AttributeHandle)
                  {
                     ...
                     if((NewAlertData = ANS_Decode_New_Alert_Notification(GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data->AttributeValueLength, GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data->AttributeValue)) != NULL)
                     {
                        ...
                        DisplayFunctionError("ANS_Decode_New_Alert_Notification", (int)NewAlertData);
                  }
                  else
                  {
...
static void BTPSAPI GATT_Service_Discovery_Event_Callback(unsigned int BluetoothStackID, GATT_Service_Discovery_Event_Data_t *GATT_Service_Discovery_Event_Data, unsigned long CallbackParameter)
{
   ...
   GATT_Characteristic_Information_t            *CharacteristicInfo;
   GATT_Characteristic_Descriptor_Information_t *CharDescriptorInfo;
      ...
      if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
      {
         switch(GATT_Service_Discovery_Event_Data->Event_Data_Type)
         {
            case etGATT_Service_Discovery_Indication:
               ...
               {
                  /* Display the Service Handle Range and UUID of found */
                  /* service.                                           */
                  printf("\r\nService Handle Range: 0x%04X - 0x%04X\r\n", GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data->ServiceInformation.Service_Handle, GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data->ServiceInformation.End_Group_Handle);
                  printf("Service UUID:         0x");
                  DisplayUUID(&(GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data->ServiceInformation.UUID));

                  CharacteristicInfo = &(GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data->CharacteristicInformationList[0]);

                  /* Display all the Characteristics and Descriptors    */
                  /* for the found service.                             */
                  for(Index = 0; Index < GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data->NumberOfCharacteristics; Index++, CharacteristicInfo++)
                  {
                     printf("%d. Characteristic Handle: 0x%04X\r\n", Index+1, CharacteristicInfo->Characteristic_Handle);
                     printf("Properties:            0x%02X\r\n", CharacteristicInfo->Characteristic_Properties);
                     printf("UUID:                  0x");
                     DisplayUUID(&(CharacteristicInfo->Characteristic_UUID));

                     CharDescriptorInfo = &(CharacteristicInfo->DescriptorList[0]);

                     for(Index2 = 0; Index2 < CharacteristicInfo->NumberOfDescriptors; Index2++, CharDescriptorInfo++)
                     {
                        printf("\r\n");
                        printf("%d. Descriptor Handle: 0x%04X\r\n", Index2+1, CharDescriptorInfo->Characteristic_Descriptor_Handle);
                        printf("UUID:              0x");
                        DisplayUUID(&(CharDescriptorInfo->Characteristic_Descriptor_UUID));
                        if((Index2+1) == CharacteristicInfo->NumberOfDescriptors)
                           printf("\r\n");
                     }
                  }

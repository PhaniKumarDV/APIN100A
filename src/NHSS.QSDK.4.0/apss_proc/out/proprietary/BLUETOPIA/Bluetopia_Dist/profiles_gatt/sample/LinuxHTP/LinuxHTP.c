...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXHTP - Linux Bluetooth Health Thermometer Profile using GATT (LE)     */
/*             application.                                                   */
/*                                                                            */
/*  Author:  Ryan Byrne                                                       */
/*                                                                            */
...
#include "LinuxHTP.h"            /* Application Header.                       */
...
#define DEFAULT_DEBUG_LOG_FILE_NAME                "LinuxHTP_ASC.log"
#define DEFAULT_DEBUG_FTS_FILE_NAME                "LinuxHTP_FTS.log"

   /* Determine the Name we will use for this compilation.              */
#define LE_DEMO_DEVICE_NAME                        "LinuxHTP"
...
#define DEVICE_INFO_FLAGS_MI_INDICATE_SUPPORTED             0x10
#define DEVICE_INFO_FLAGS_MI_WRITE_SUPPORTED                0x20
         ...
         for(Index1 = 0; Index1 < ServiceDiscoveryData->NumberOfCharacteristics; Index1++, CurrentCharacteristic++)
         {
            ...
            if(CurrentCharacteristic->Characteristic_UUID.UUID_Type == guUUID_16)
            {
                  ...
                  if(!HTS_COMPARE_HTS_TEMPERATURE_TYPE_UUID_TO_UUID_16(CurrentCharacteristic->Characteristic_UUID.UUID.UUID_16))
                  {
                     ...
                     else
                     {
                        ...
                        if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_NOTIFY))
                           printf("Warning - Mandatory notify property of Intermediate Temperature characteristic not supported!\r\n");
                     }
                  }
                  else
                  {
                     ...
                     /* Verify that read is supported.                  */
                     if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_READ))
                        printf("Warning - Mandatory read property of Temperature Type characteristic not supported!\r\n");
               ...
               else
               {
                  ...
                  if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_INDICATE))
                     printf("Warning - Mandatory indicate property of Temperature Measurement characteristic not supported!\r\n");
               }
...
static int FormatTimeStamp(char *Time, HTS_Time_Stamp_Data_t *TimeStamp)
{
   ...
   if(BTPS_StringLength(Time) == 14)
   {
      ...
      for(Index = 0; ((Index < 14) && (Time[Index] >= '0') && (Time[Index] <= '9')); Index++);
      ...
      if(Index == 14)
      {
         ...
         tmpStr[4] = '\0';
         Time += 4;
         TimeStamp->Year = (Word_t)StringToUnsignedInteger(tmpStr);

         BTPS_MemCopy(tmpStr, Time, 2);
         tmpStr[2] = '\0';
         Time += 2;
         TimeStamp->Month = (Byte_t)StringToUnsignedInteger(tmpStr);

         BTPS_MemCopy(tmpStr, Time, 2);
         tmpStr[2] = '\0';
         Time += 2;
         TimeStamp->Day = (Byte_t)StringToUnsignedInteger(tmpStr);

         BTPS_MemCopy(tmpStr, Time, 2);
         tmpStr[2] = '\0';
         Time += 2;
         TimeStamp->Hours = (Byte_t)StringToUnsignedInteger(tmpStr);

         BTPS_MemCopy(tmpStr, Time, 2);
         tmpStr[2] = '\0';
         Time += 2;
         TimeStamp->Minutes = (Byte_t)StringToUnsignedInteger(tmpStr);

         BTPS_MemCopy(tmpStr, Time, 2);
         tmpStr[2] = '\0';
         TimeStamp->Seconds = (Byte_t)StringToUnsignedInteger(tmpStr);
...
static int FormatTemperatureMeasurement(ParameterList_t *Param, HTS_Temperature_Measurement_Data_t *TemperatureMeasurement)
{
   ...
   if((Param) && (Param->NumberofParameters >= 5))
   {
      ...
      TemperatureMeasurement->Temperature.Value2   = (Byte_t)(Param->Params[1].intParam >> (8 * 2));
      ...
      if(ret_val == 0)
      {
         ...
         if(Param->Params[4].intParam)
         {
            ...
            if(HTS_TEMPERATURE_TYPE_VALID_TEMPERATURE_TYPE(Param->Params[4].intParam))
            {
               ...
               TemperatureMeasurement->Temperature_Type  = (Byte_t)Param->Params[4].intParam;
            }
            else
            {
               printf("Error - Invalid Temperature Type: %d\r\n", (int)Param->Params[4].intParam);
      ...
      {
         printf("Error - Invalid Time Stamp Format: %s\r\n", Param->Params[3].strParam);
...
static int DecodeDisplayTemperatureMeasurement(Word_t BufferLength, Byte_t *Buffer, HTS_Temperature_Measurement_Data_t *TemperatureMeasurement, Boolean_t PrintData)
{
   ...
   if(PrintData)
   {
      ...
      if((TemperatureMeasurement->Temperature.Exponent == 0x00) && (TemperatureMeasurement->Temperature.Value2 == 0x7F) && (TemperatureMeasurement->Temperature.Value1 == 0xFF) && (TemperatureMeasurement->Temperature.Value0 == 0xFF))
         printf("      Temperature Value: NaN\r\n");
      else
         printf("      Temperature Value: (%d) x (10**%d)\r\n", (TemperatureMeasurement->Temperature.Value0 | (TemperatureMeasurement->Temperature.Value1 << 8) | (TemperatureMeasurement->Temperature.Value2 << (8 * 2))), TemperatureMeasurement->Temperature.Exponent);
...
static void DisplayNotifyIndicateTemperatureUsage(const char *Command)
{
   ...
   printf("[Units (0 = Celcius, 1 = Fahrenheit)] ");
   printf("[Time Stamp (0 = Don't Send, yyyymmddhhmmss = Send Time] ");
   printf("[Temperature Type (0 = Don't send, 1-9 = Send Type)]\r\n");
}
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("* Command Options HTP:     RegisterHTS, UnregisterHTS,           *\r\n");
   printf("*                          DiscoverHTS, ConfigureRemoteHTS,      *\r\n");
   printf("*                          IndicateTemperature,                  *\r\n");
   printf("*                          IndicateMeasurementInterval,          *\r\n");
   printf("*                          NotifyIntermediateTemperature,        *\r\n");
   printf("*                          GetTemperatureType,                   *\r\n");
   printf("*                          SetTemperatureType,                   *\r\n");
   printf("*                          GetMeasurementInterval,               *\r\n");
   printf("*                          SetMeasurementInterval,               *\r\n");
   printf("*                          GetValidRange, SetValidRange          *\r\n");
   printf("******************************************************************\r\n");
...
static int RegisterHTS(ParameterList_t *TempParam)
{
   ...
   if(!ConnectionID)
   {
      ...
      if(!HTSInstanceID)
      {
         ...
         ret_val = HTS_Initialize_Service(BluetoothStackID, HTS_EventCallback, 0, &HTSInstanceID);
         if((ret_val > 0) && (HTSInstanceID > 0))
         {
            /* Display success message.                                 */
            printf("Successfully registered HTP Service.\r\n");
      ...
      {
         printf("HTP Service already registered.\r\n");
...
static int UnregisterHTS(ParameterList_t *TempParam)
{
   ...
   if(HTSInstanceID)
   {
      ...
      {
         /* Display success message.                                    */
         printf("Successfully unregistered HTP Service.\r\n");
      ...
      printf("HTP Service not registered.\r\n");
...
static int DiscoverHTS(ParameterList_t *TempParam)
{
   ...
   if(!HTSInstanceID)
   {
      ...
      printf("Cannot discover HTP Services when registered as a service.\r\n");
...
static int ConfigureRemoteHTS(ParameterList_t *TempParam)
{
   ...
   if(TempParam && (TempParam->NumberofParameters > 2))
   {
      ...
      if(!HTSInstanceID)
      {
            ...
            if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
            {
               ...
               if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_SERVICE_DISCOVERY_COMPLETE)
               {
                  ...
                  if(DeviceInfo->ClientInfo.Temperature_Measurement_Client_Configuration)
                  {
                     ...
                     printf("   Error - Temperature Measurment CC not found on this device.\r\n");
                  ...
                  if(ret_val > 0)
                  {
                     ...
                     if(DeviceInfo->ClientInfo.Intermediate_Temperature)
                     {
                        ...
                        {
                           printf("   Error - Intermediate Temperature CC not found on this device.\r\n");

                           ret_val = 0;
                        }
                     }
                     else
                        printf("   Intermediate Temperature not supported on this device.\r\n");
                     ...
                     if(ret_val > 0)
                     {
                        ...
                        if(DeviceInfo->ClientInfo.Measurement_Interval)
                        {
                           ...
                           {
                              printf("   Error - Measurement Interval CC not found on this device.\r\n");

                              ret_val = 0;
                           }
                        }
                        else
                           printf("   Measurement Interval not supported on this device.\r\n");
                     }
                  }
         ...
         printf("Cannot configure remote HTP Services when registered as a service.\r\n");
   }
   else
   {
      printf("Usage: ConfigureRemoteHTS [Temperature Measurment Indicate (0 = disable, 1 = enable)]\r\n");
      printf("                          [Measurement Interval Indicate (0 = disable, 1 = enable)]\r\n");
      printf("                          [Intermediate Temperature Notify (0 = disable, 1 = enable)]\r\n");
   }
   ...
   if(BluetoothStackID)
   {
            ...
            if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
            {
               if(DeviceInfo->ServerInfo.Temperature_Measurement_Client_Configuration & GATT_CLIENT_CONFIGURATION_CHARACTERISTIC_INDICATE_ENABLE)
               {
                  ...
                  if((ret_val = FormatTemperatureMeasurement(TempParam, &TemperatureMeasurement)) == 0)
                  {
                     ...
                     {
                        printf("Temperature Indication success.\r\n");
                     }
                     else
               ...
               {
                  printf("Client has not registered for Temperature Measurement indications.\r\n");
      ...
      else
      {
         if(ConnectionID)
            ...
            printf("Error - HTP server not registered\r\n");
      }
   }
   else
   {
         ...
         if((ConnectionID) && (!COMPARE_NULL_BD_ADDR(ConnectionBD_ADDR)))
         {
            if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
            {
               if(DeviceInfo->ServerInfo.Measurement_Interval_Client_Configuration & GATT_CLIENT_CONFIGURATION_CHARACTERISTIC_INDICATE_ENABLE)
               {
                  ...
                  {
                     printf("Measurement Interval Indication success.\r\n");
                  }
                  else
               ...
               {
                  printf("Client has not registered for Measurement Interval indications.\r\n");
            ...
            if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
            {
               if(DeviceInfo->ServerInfo.Intermediate_Temperature_Client_Configuration & GATT_CLIENT_CONFIGURATION_CHARACTERISTIC_NOTIFY_ENABLE)
               {
                  ...
                  if((ret_val = FormatTemperatureMeasurement(TempParam, &TemperatureMeasurement)) == 0)
                  {
                     ...
                     {
                        printf("Intermediate Temperature Notification success.\r\n");
                     }
                     else
               ...
               {
                  printf("Client has not registered for Intermediate Temperature notifications.\r\n");
...
static int GetTemperatureType(ParameterList_t *TempParam)
{
   ...
   if(HTSInstanceID)
   {
      if((ret_val = HTS_Query_Temperature_Type(BluetoothStackID, HTSInstanceID, &tempType) == 0) && HTS_TEMPERATURE_TYPE_VALID_TEMPERATURE_TYPE((Byte_t)tempType))
      {
      ...
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            if(DeviceInfo->ClientInfo.Temperature_Type != 0)
            {
               ...
               {
                  printf("Get Temperature Type Request sent, Transaction ID = %u", ret_val);
               ...
               printf("Temperature Type not supported on remote service.\r\n");
         }
         else
            printf("No Device Info.\r\n");
      }
      else
         printf("Either a HTP server must be registered or a HTP client must be connected.\r\n");
   }
...
static int SetTemperatureType(ParameterList_t *TempParam)
{
   ...
   if((TempParam) && (TempParam->NumberofParameters > 0))
   {
      if(HTSInstanceID)
      {
         ...
         {
            printf("Invalid temperature type. Valid types are:\r\n");

            for(Index = 0; Index < NUM_SUPPORTED_TEMPERATURE_TYPES; Index++)
               printf("   %d - %s\r\n", (Index + 1), TemperatureTypeTable[Index]);
         }
      }
      else
      {
         if(ConnectionID)
            printf("Cannot write to temperature type as a client.\r\n");
         else
            printf("HTP server not registered\r\n");
      }
   }
   else
   {
      ...
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            if(DeviceInfo->ClientInfo.Measurement_Interval != 0)
            {
               ...
               {
                  printf("Get Measurement Interval Request sent, Transaction ID = %u", ret_val);
               ...
               printf("Measurement Interval not supported on remote service.\r\n");
         }
         else
            printf("No Device Info.\r\n");
      }
   ...
   if((TempParam) && (TempParam->NumberofParameters > 0))
   {
      if(HTSInstanceID)
      {
         ...
         if(HTS_Query_Valid_Range(BluetoothStackID, HTSInstanceID, &ValidRange) || ((((Word_t)TempParam->Params[0].intParam) >= ValidRange.Lower_Bounds) && (((Word_t)TempParam->Params[0].intParam) <= ValidRange.Upper_Bounds)))
         {
            ...
            {
               printf("Measurement Interval successfully set to %u.\r\n", (unsigned int)TempParam->Params[0].intParam);
            }
            else
         ...
         {
            printf("Error - Cannot set measurement interval outside of valid range [%u, %u]\r\n", ValidRange.Lower_Bounds, ValidRange.Upper_Bounds);
            ...
            if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
            {
               if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_MI_WRITE_SUPPORTED)
               {
                  ...
                  if(DeviceInfo->ClientInfo.Measurement_Interval != 0)
                  {
                     ...
                     {
                        printf("Set Measurement Interval Request sent, Transaction ID = %u", ret_val);
                     ...
                     printf("Error - Measurement interval attribute handle is invalid!\r\n");
               }
               else
               {
                  printf("Writing measurement interval not supported on remote service.\r\n");
      ...
      DisplayUsage("SetMeasurementInterval [Interval (0 = no periodic measurement)]");
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_MI_WRITE_SUPPORTED)
            {
               ...
               if(DeviceInfo->ClientInfo.Measurement_Interval_Valid_Range_Descriptor != 0)
               {
                  ...
                  {
                     printf("Get Valid Range Request sent, Transaction ID = %u", ret_val);
                  ...
                  printf("Error - Valid range attribute handle is invalid!\r\n");
            }
            else
            {
               printf("Reading valid range not supported on remote service.\r\n");
...
static int SetValidRange(ParameterList_t *TempParam)
{
   ...
   if((TempParam) && (TempParam->NumberofParameters > 1))
   {
      if(HTSInstanceID)
      {
         ValidRange.Lower_Bounds = (Word_t)TempParam->Params[0].intParam;
         ValidRange.Upper_Bounds = (Word_t)TempParam->Params[1].intParam;

         if((ret_val = HTS_Set_Valid_Range(BluetoothStackID, HTSInstanceID, &ValidRange)) == 0)
         {
            printf("Valid range successfully set to [%u, %u].\r\n", ValidRange.Lower_Bounds, ValidRange.Upper_Bounds);
            ...
            if((HTS_Query_Measurement_Interval(BluetoothStackID, HTSInstanceID, &Interval) == 0) && ((Interval < ValidRange.Lower_Bounds) || (Interval > ValidRange.Upper_Bounds)))
            {
               ...
               printf("Measurement Interval was adjusted to fit inside Valid Range\r\n");
            }
         }
         else
      ...
      {
         if(ConnectionID)
            printf("Error - Writing valid range is not allowed as a client.\r\n");
         else
            printf("Error - HTP server not registered\r\n");
      }
   }
   ...
   {
      DisplayUsage("SetValidRange [LowerBound] [UpperBound]");
...
static void BTPSAPI HTS_EventCallback(unsigned int BluetoothStackID, HTS_Event_Data_t *HTS_Event_Data, unsigned long CallbackParameter)
{
   ...
   if((BluetoothStackID) && (HTS_Event_Data))
   {
      switch(HTS_Event_Data->Event_Data_Type)
      {
         case etHTS_Server_Read_Client_Configuration_Request:
            printf("etHTS_Server_Read_Client_Configuration_Request with size %u.\r\n", HTS_Event_Data->Event_Data_Size);
                  ...
                  if(HTS_Event_Data->Event_Data.HTS_Read_Client_Configuration_Data->InstanceID == HTSInstanceID)
                  {
                     ...
                     switch(HTS_Event_Data->Event_Data.HTS_Read_Client_Configuration_Data->ClientConfigurationType)
                     {
                        case ctTemperatureMeasurement:
                           printf("   Config Type:      ctTemperatureMeasurement.\r\n");
                        ...
                        case ctIntermediateTemperature:
                           printf("   Config Type:      ctIntermediateTemperature.\r\n");
                        ...
                        case ctMeasurementInterval:
                           printf("Config Type:      ctMeasurementInterval.\r\n");
         ...
         case etHTS_Server_Client_Configuration_Update:
            printf("etHTS_Server_Client_Configuration_Update with size %u.\r\n", HTS_Event_Data->Event_Data_Size);
         ...
         case etHTS_Measurement_Interval_Update:
            printf("etHTS_Measurement_Interval_Update with size %u.\r\n", HTS_Event_Data->Event_Data_Size);
         ...
         case etHTS_Confirmation_Response:
            printf("etHTS_Confirmation_Response with size %u.\r\n", HTS_Event_Data->Event_Data_Size);
            ...
            if(HTS_Event_Data->Event_Data.HTS_Confirmation_Data)
            {
               ...
               {
                  case ctTemperatureMeasurement:
                     printf("   Characteristic Type: ctTemperatureMeasurement.\r\n");
                     break;
                  case ctIntermediateTemperature:
                     printf("   Characteristic Type: ctIntermediateTemperature.\r\n");
                     break;
                  case ctMeasurementInterval:
                     printf("   Characteristic Type: ctMeasurementInterval.\r\n");
                     break;
                  default:
                     printf("   Characteristic Type: Unknown.\r\n");
                     break;
               }
   ...
   else
   {
      ...
      printf("HTS Callback Data: Event_Data = NULL.\r\n");
   }
            ...
            if(GATT_Client_Event_Data->Event_Data.GATT_Request_Error_Data)
            {
               ...
               if(GATT_Client_Event_Data->Event_Data.GATT_Request_Error_Data->ErrorType == retErrorResponse)
               {
                  ...
                  else
                  {
                     ...
                     {
                        printf("Error Mesg:      Out of Range.\r\n");
                     }
                     else
            ...
            if(GATT_Connection_Event_Data->Event_Data.GATT_Device_Connection_Data)
            {
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, GATT_Connection_Event_Data->Event_Data.GATT_Device_Connection_Data->RemoteDevice)) != NULL)
               {
                  if(LocalDeviceIsMaster)
                  {
                     ...
                     /* supported.                                      */
                     GATT_Exchange_MTU_Request(BluetoothStackID, ConnectionID, BTPS_CONFIGURATION_GATT_DEFAULT_MAXIMUM_SUPPORTED_MTU_SIZE, GATT_ClientEventCallback_HTP, 0);
                  }
               }
            }
            else
            ...
            if(GATT_Connection_Event_Data->Event_Data.GATT_Server_Indication_Data)
            {
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, GATT_Connection_Event_Data->Event_Data.GATT_Server_Indication_Data->RemoteDevice)) != NULL)
               {
                  ...
                  if(DeviceInfo->ClientInfo.Temperature_Measurement == GATT_Connection_Event_Data->Event_Data.GATT_Server_Indication_Data->AttributeHandle)
                  {
                     ...
                     if(!(Result = DecodeDisplayTemperatureMeasurement(GATT_Connection_Event_Data->Event_Data.GATT_Server_Indication_Data->AttributeValueLength, GATT_Connection_Event_Data->Event_Data.GATT_Server_Indication_Data->AttributeValue, &TemperatureMeasurement, TRUE)))
                     {
                  ...
                  else
                  {
                     if(DeviceInfo->ClientInfo.Measurement_Interval == GATT_Connection_Event_Data->Event_Data.GATT_Server_Indication_Data->AttributeHandle)
                     {
                        ...
                        printf("Error - Unknown Indication Atrribute Handle.\r\n");
                  }
               }
               else
            ...
            if(GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data)
            {
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data->RemoteDevice)) != NULL)
               {
                  if(DeviceInfo->ClientInfo.Intermediate_Temperature == GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data->AttributeHandle)
                  {
                     ...
                     if(!(Result = DecodeDisplayTemperatureMeasurement(GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data->AttributeValueLength, GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data->AttributeValue, &TemperatureMeasurement, TRUE)))
                     {
                        DisplayFunctionError("DecodeDisplayTemperatureMeasurement", Result);
                     }
                  }

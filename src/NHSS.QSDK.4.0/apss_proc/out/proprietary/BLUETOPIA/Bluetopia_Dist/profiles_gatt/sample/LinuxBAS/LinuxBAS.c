...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXBAS - Linux Bluetooth Battery Service using GATT (LE) application.   */
/*                                                                            */
/*  Author:  Tim Cook                                                         */
/*                                                                            */
...
#include "LinuxBAS.h"            /* Application Header.                       */
...
#define DEFAULT_DEBUG_LOG_FILE_NAME                "LinuxBAS_ASC.log"
#define DEFAULT_DEBUG_FTS_FILE_NAME                "LinuxBAS_FTS.log"

   /* Determine the Name we will use for this compilation.              */
#define LE_DEMO_DEVICE_NAME                        "LinuxBAS"
...
static int RegisterBAS(ParameterList_t *TempParam)
{
   ...
   if(!ConnectionID)
   {
      ...
      {
         /* Register the BAP Service with GATT.                         */
         ...
         {
            /* Display success message.                                 */
            printf("Successfully registered BAP Service, InstanceId = %u.\r\n", ret_val);

            /* Save the ServiceID of the registered service.            */
            BASInstanceID[Index] = (unsigned int)ret_val;

            /* Initialize internal BAP variables                        */
...
static int UnregisterBAS(ParameterList_t *TempParam)
{
   ...
   if(Index >= 0)
   {
      ...
      /* Unregister the BAP Service with GATT.                          */
      ret_val = BAS_Cleanup_Service(BluetoothStackID, BASInstanceID[Index]);
      if(ret_val == 0)
      {
         /* Display success message.                                    */
         printf("Successfully unregistered BAP Service InstanceID %u.\r\n",BASInstanceID[Index]);
      ...
      printf("BAP Service not registered.\r\n");
...
static int DiscoverBAS(ParameterList_t *TempParam)
{
   ...
   if(!BASInstanceID[0])
   {
      ...
      printf("Cannot discover BAP Services when registered as a service.\r\n");
...
static int ConfigureRemoteBAS(ParameterList_t *TempParam)
{
   ...
   if(ret_val != INVALID_PARAMETERS_ERROR)
   {
      ...
      if(!BASInstanceID[0])
      {
         ...
         printf("Cannot configure remote BAP Services when registered as a service.\r\n");
   }
...
static int GetBatteryLevel(ParameterList_t *TempParam)
{
   ...
   if(ret_val != INVALID_PARAMETERS_ERROR)
   {
      /* First check for a registered BAP Server                        */
      if(BASInstanceID[0])
      {
      ...
      if((TempParam) && (TempParam->NumberofParameters > 0))
      {
         ...
         else
         {
            if(ConnectionID)
               ...
               printf("BAP server not registered\r\n");
         }
      }
      else
      {
         ...
         switch(GATT_Service_Discovery_Event_Data->Event_Data_Type)
         {
            case etGATT_Service_Discovery_Indication:
               ...
               if(GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data)
               {
                  ...
                  {
                     /* Attempt to populate the handles for the BAP     */
                     /* Service.                                        */

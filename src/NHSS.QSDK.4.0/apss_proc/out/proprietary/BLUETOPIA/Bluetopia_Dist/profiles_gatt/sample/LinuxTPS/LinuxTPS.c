...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXTPS - Linux Bluetooth Transmit Power Service using GATT (LE)         */
/*             application.                                                   */
/*                                                                            */
/*  Author:  Cliff Xu                                                         */
/*                                                                            */
...
#include "LinuxTPS.h"            /* Application Header.                       */
...
#define DEFAULT_DEBUG_LOG_FILE_NAME                "LinuxTPS_ASC.log"
#define DEFAULT_DEBUG_FTS_FILE_NAME                "LinuxTPS_FTS.log"
   /* Determine the Name we will use for this compilation.              */
#define LE_DEMO_DEVICE_NAME                        "LinuxTPS"
...
typedef struct _tagDeviceInfo_t
{
   ...
   TPS_Client_Information_t  ClientInfo;
      ...
      if(CurrentCharacteristic)
      {
         for(Index = 0; Index < ServiceDiscoveryData->NumberOfCharacteristics; Index++, CurrentCharacteristic++)
         {
            ...
            if(CurrentCharacteristic->Characteristic_UUID.UUID_Type == guUUID_16)
            {
               ...
               {
                  DeviceInfo->ClientInfo.Tx_Power_Level = CurrentCharacteristic->Characteristic_Handle;
   ...
   if((BluetoothStackID) && (!ConnectionID))
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].intParam >= 0) && (TempParam->Params[0].intParam <= 1))
      {
         ...
         else
         {
            ...
            if(TPSInstanceID)
            {
               /* Advertise the TPS Server (1 byte type & 2 bytes UUID).*/
               ...
               TPS_ASSIGN_TPS_SERVICE_UUID_16(&(Advertisement_Data_Buffer.AdvertisingData.Advertising_Data[5]));
            }
   ...
   else
   {
      ...
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            /* Attribute Handle.                                        */
            if(DeviceInfo->ClientInfo.Tx_Power_Level != 0)
            {
               /* Finally, submit a read request to the server.         */
               if((ret_val = GATT_Read_Value_Request(BluetoothStackID, ConnectionID, DeviceInfo->ClientInfo.Tx_Power_Level, GATT_ClientEventCallback_TPS, DeviceInfo->ClientInfo.Tx_Power_Level)) > 0)
               {
         ...
         case etGATT_Client_Read_Response:
            if(GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data)
            {
               ...
               if(ValueLength != 0)
               {
                  ...
                  {
                     if(CallbackParameter == DeviceInfo->ClientInfo.Tx_Power_Level)
                     {
                        printf("\r\nTx Power Level: %i.\r\n", (SByte_t)GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data->AttributeValue[0]);
                        if(TPS_TX_POWER_LEVEL_VALID((SByte_t)GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data->AttributeValue[0]))
                        {
                           printf("   Tx Power Level is valid.\r\n");
                        }
                        else
                        {
                           printf("   Tx Power Level is not valid.\r\n");
                        }
                     }
                  }
            ...
            if(GATT_Connection_Event_Data->Event_Data.GATT_Device_Connection_Data)
            {
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, GATT_Connection_Event_Data->Event_Data.GATT_Device_Connection_Data->RemoteDevice)) != NULL)
               {
                  if(LocalDeviceIsMaster)
                  {
                     ...
                     /* supported.                                      */
                     GATT_Exchange_MTU_Request(BluetoothStackID, ConnectionID, BTPS_CONFIGURATION_GATT_DEFAULT_MAXIMUM_SUPPORTED_MTU_SIZE, GATT_ClientEventCallback_TPS, 0);
                  }
               }
            }
            else
            ...
            case etGATT_Service_Discovery_Complete:
               ...
               if(GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Complete_Data)
               {
                  ...
                  if(((Service_Discovery_Type_t)CallbackParameter) == sdTPS)
                  {
                     ...
                     printf("Tx_Power_Level: %s\r\n", (DeviceInfo->ClientInfo.Tx_Power_Level                            ? "Supported" : "Not Supported"));
                  }
               }
               break;
         }

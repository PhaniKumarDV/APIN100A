...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXBMS - Linux Bluetooth Bond Management Service using GATT (LE/BREDR)  */
/*             application.                                                   */
/*                                                                            */
/*  Author:  Ryan McCord                                                      */
/*                                                                            */
...
/*   --------  -----------    ------------------------------------------------*/
/*   07/13/15  R. McCord      Initial creation.                               */
/******************************************************************************/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>

#include "LinuxBMS.h"            /* Application Header.                       */
...
#define DEFAULT_DEBUG_LOG_FILE_NAME                "LinuxBMS_ASC.log"
#define DEFAULT_DEBUG_FTS_FILE_NAME                "LinuxBMS_FTS.log"

   /* Determine the Name we will use for this compilation.              */
#define LE_DEMO_DEVICE_NAME                        "LinuxBMS"
...
typedef struct _tagDeviceInfo_t
{
   ...
   BMS_Client_Information_t  ClientInfo;
...
#define DEVICE_INFO_FLAGS_LINK_KEY_VALID                    0x02
#define DEVICE_INFO_FLAGS_LINK_ENCRYPTED                    0x04
#define DEVICE_INFO_FLAGS_BR_EDR_CONNECTED                  0x08
#define DEVICE_INFO_FLAGS_SERVICE_DISCOVERY_OUTSTANDING     0x10
#define DEVICE_INFO_FLAGS_SERVICE_DISCOVERY_COMPLETE        0x20
...
static int SetGATTMTU(ParameterList_t *TempParam);
   ...
   /* Bond Management Service Function Commands                         */
static int RegisterBMS(ParameterList_t *TempParam);
static int UnregisterBMS(ParameterList_t *TempParam);
static int DiscoverBMS(ParameterList_t *TempParam);

static int SetBMSFeature(ParameterList_t *TempParam);
static int QueryBMSFeature(ParameterList_t *TempParam);
...
static void UserInterface(void)
{
   ...
   AddCommand("REGISTERBMS", RegisterBMS);
   AddCommand("UNREGISTERBMS", UnregisterBMS);
   AddCommand("DISCOVERBMS", DiscoverBMS);
   AddCommand("SETFEATURE", SetBMSFeature);
   AddCommand("GETFEATURE", QueryBMSFeature);
   AddCommand("SENDCOMMAND", BMSCommand);
...
static int CloseStack(void)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(BMSInstanceID)
      {
         BMS_Cleanup_Service(BluetoothStackID, BMSInstanceID);
      }
     ...
     if(CurrentCharacteristic)
     {
        for(Index1 = 0; Index1 < ServiceDiscoveryData->NumberOfCharacteristics; Index1++, CurrentCharacteristic++)
        {
           if(BMS_COMPARE_BM_FEATURE_UUID_TO_UUID_16(CurrentCharacteristic->Characteristic_UUID.UUID.UUID_16))
           {
              ...
              if(CurrentCharacteristic->Characteristic_UUID.UUID_Type == guUUID_16)
              {
                 DeviceInfo->ClientInfo.BM_Feature = CurrentCharacteristic->Characteristic_Handle;
           ...
           else
           {
              if(BMS_COMPARE_BM_CONTROL_POINT_UUID_TO_UUID_16(CurrentCharacteristic->Characteristic_UUID.UUID.UUID_16))
              {
                 ...
                 if(CurrentCharacteristic->Characteristic_UUID.UUID_Type == guUUID_16)
                 {
                    DeviceInfo->ClientInfo.BM_Control_Point = CurrentCharacteristic->Characteristic_Handle;
...
Boolean_t IsBMSFeatureHandle(Word_t Handle, DeviceInfo_t *DeviceInfo)
{
   ...
   if(Handle == DeviceInfo->ClientInfo.BM_Feature)
      ret_val = TRUE;
...
Boolean_t IsBMSControlPointHandle(Word_t Handle, DeviceInfo_t *DeviceInfo)
{
   ...
   if(Handle == DeviceInfo->ClientInfo.BM_Control_Point)
      ret_val = TRUE;
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("* Command Options BMS:                                           *\r\n");
   printf("*                Server:   RegisterBMS,                          *\r\n");
   printf("*                          UnregisterBMS,                        *\r\n");
   printf("*                          SetFeature,                           *\r\n");
   printf("*                          GetFeature,                           *\r\n");
   printf("*                Client:   DiscoverBMS,                          *\r\n");
   printf("*                          GetFeature,                           *\r\n");
   printf("*                          SendCommand,                          *\r\n");
   printf("******************************************************************\r\n");
   ...
   if((BluetoothStackID) && (!ConnectionID))
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].intParam >= 0) && (TempParam->Params[0].intParam <= 1))
      {
         ...
         else
         {
            ...
            /* Configure the flags field bmsed on the Discoverability   */
            /* Mode.                                                    */
            ...
            if(BMSInstanceID)
            {
               ...
               BMS_ASSIGN_BMS_SERVICE_UUID_16(&(Advertisement_Data_Buffer.AdvertisingData.Advertising_Data[5]));
            }
...
static int RegisterBMS(ParameterList_t *TempParam)
{
   ...
   if((TempParam) && (TempParam->NumberofParameters == 1) && (TempParam->Params[0].intParam >= 1) && (TempParam->Params[0].intParam <= 3))
   {
      ...
      if(!ConnectionID)
      {
         ...
         if(!BMSInstanceID)
         {
            /* Register the BMS Service with GATT.                      */
            if(TempParam->Params[0].intParam == 1)
            {
               Flags    = BMS_FLAGS_SUPPORT_LE;
               Features = DEFAULT_BM_LE_FEATURES_BIT_MASK;
            }
            else
            {
               if(TempParam->Params[0].intParam == 2)
               {
                  Flags    = BMS_FLAGS_SUPPORT_BR_EDR;
                  Features = DEFAULT_BM_BR_EDR_FEATURES_BIT_MASK;
               }
               else
               {
                  Flags    = BMS_FLAGS_SUPPORT_DUAL_MODE;
                  Features = (DEFAULT_BM_LE_FEATURES_BIT_MASK|DEFAULT_BM_BR_EDR_FEATURES_BIT_MASK|DEFAULT_BM_DUAL_MODE_FEATURES_BIT_MASK);
               }
            }

            /* Initialize the service.                                  */
            ret_val = BMS_Initialize_Service(BluetoothStackID, Flags, BMS_EventCallback, 0, &BMSInstanceID);
            if((ret_val > 0) && (BMSInstanceID > 0))
            {
               /* Display success message.                              */
               printf("Successfully registered BMS Service, BMSInstanceID = %u.\r\n", ret_val);

               /* Save the ServiceID of the registered service.         */
               BMSInstanceID = (unsigned int)ret_val;

               /* Set the default features.                             */
               if(!(ret_val = BMS_Set_BM_Features(BluetoothStackID, BMSInstanceID, Features)))
               {
                  printf("Successfully set the default features.\r\n");
               ...
               {
                  DisplayFunctionError("BMS_Set_BM_Features", ret_val);

                  /* Clean up the service since we failed to set the    */
                  /* default features.                                  */
                  BMS_Cleanup_Service(BluetoothStackID, BMSInstanceID);
               }
            }
            ...
            {
               DisplayFunctionError("BMS_Initialize_Service", ret_val);
            }
         }
         else
         {
            printf("BMS is already registered.\r\n");
         }
      }
      else
      {
   ...
   {
      printf("Usage: RegisterBMS [Register GATT for (LE = 1, BR/EDR = 2, Dual Mode = 3).\r\n");
      ret_val = INVALID_PARAMETERS_ERROR;
   }
...
static int UnregisterBMS(ParameterList_t *TempParam)
{
   ...
   if(BMSInstanceID)
   {
      ...
      /* Unregister the BMS Service with GATT.                          */
      ret_val = BMS_Cleanup_Service(BluetoothStackID, BMSInstanceID);
      if(ret_val == 0)
      {
         /* Display success message.                                    */
         printf("Successfully unregistered BMS Service InstanceID %u.\r\n",BMSInstanceID);

         /* Clear the InstanceID.                                       */
         BMSInstanceID = 0;
      }
      else
         DisplayFunctionError("BMS_Cleanup_Service", ret_val);
   }
   else
      printf("BMS Service not registered.\r\n");
...
static int DiscoverBMS(ParameterList_t *TempParam)
{
   ...
   /* features of a BMS service. This function will return zero on      */
   /* successful execution and a negative value on errors.              */
static int SetBMSFeature(ParameterList_t *TempParam)
{
   int     ret_val = FUNCTION_ERROR;
   ...
   /* Verify that the features are semi-valid.                          */
   if((TempParam) && (TempParam->NumberofParameters == 1))
   {
      /* Assign the supported features.                                 */
      SupportedFeatures = (DWord_t)(TempParam->Params[0].intParam);

      /* Verify that this server instance has been initialized.         */
      if(BMSInstanceID)
      {
         /* Set the Bond Management Service Supported Features.         */
         if((ret_val = BMS_Set_BM_Features(BluetoothStackID, BMSInstanceID, SupportedFeatures)) == 0)
         {
            printf("\r\nSet BMS Supported Features: 0x%06X.\r\n", SupportedFeatures);

            ret_val = 0;
         }
         else
            DisplayFunctionError("\r\nBMS_Set_BM_Features", ret_val);
      }
      else
         printf("\r\nBMS server not registered.\r\n");
   }
   else
      printf("Usage: SetFeature [Supported Features (UINT32)]\r\n");
...
static int QueryBMSFeature(ParameterList_t *TempParam)
{
   ...
   /* First check for a registered BMS Server.                          */
   if(BMSInstanceID)
   {
     /* Query the Bond Management Service Supported Features.           */
     if((ret_val = BMS_Query_BM_Features(BluetoothStackID, BMSInstanceID, &SupportedFeatures)) == 0)
     {
        printf("\r\nQuery BMS Supported Features: 0x%06X.\r\n", SupportedFeatures);
   ...
   else
   {
      ...
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            /* Verify that the client has received a valid Bond         */
            /* Management Feature Attribute Handle.                     */
            if(DeviceInfo->ClientInfo.BM_Feature != 0)
            {
               /* Finally, submit a read request to the server          */
               if((ret_val = GATT_Read_Value_Request(BluetoothStackID, ConnectionID, DeviceInfo->ClientInfo.BM_Feature, GATT_ClientEventCallback_BMS, DeviceInfo->ClientInfo.BM_Feature)) > 0)
               {
                  printf("\r\nGet Supported Features Request sent, Transaction ID = %u\r\n", ret_val);

                  ret_val = 0;
               }
               else
                  DisplayFunctionError("\r\nGATT_Read_Value_Request", ret_val);
            }
            else
               printf("\r\nBError - BMS Feature Attribute Handle Is Invalid!\r\n.\r\n");
         }
         else
            printf("\r\nNo Device Info.\r\n");
      }
      else
         printf("\r\nEither a BMS server must be registered or a BMS client must be connected.\r\n");
   }
...
static int BMSCommand(ParameterList_t *TempParam)
{
   ...
   if(!BMSInstanceID)
   {
               ...
               if(ret_val == 0)
               {
                  ...
                  if(ConnectionID)
                  {
                     ...
                     if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
                     {
                        ...
                        if((DeviceInfo->ClientInfo.BM_Feature) && (DeviceInfo->ClientInfo.BM_Control_Point))
                        {
                           ...
                           if((Word_t)BMSCommandBufferLength <= (MTU - 3))
                           {
                              ...
                              /* server                                 */
                              if((ret_val = GATT_Write_Request(BluetoothStackID, ConnectionID, DeviceInfo->ClientInfo.BM_Control_Point, BMSCommandBufferLength, BMSCommandBuffer, GATT_ClientEventCallback_BMS, DeviceInfo->ClientInfo.BM_Control_Point)) > 0)
                              {
                           ...
                           else
                           {
                              ...
                              /* to the server                          */
                              if((ret_val = GATT_Prepare_Write_Request(BluetoothStackID, ConnectionID, DeviceInfo->ClientInfo.BM_Control_Point, (MTU - 5), 0, BMSCommandBuffer, GATT_ClientEventCallback_BMS, DeviceInfo->ClientInfo.BM_Control_Point)) > 0)
                              {
                                 printf("Sent %d.\r\n", (MTU - 5));
      ...
      {
         printf("Usage: [Command] [Use Authorization Code (0=FALSE, 1=TRUE)]\r\n");
                  ...
                  case latLongTermKeyRequest:
                     ...
                     if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, Authentication_Event_Data->BD_ADDR)) != NULL)
                     {
                        ...
                        else
                        {
                           ...
                           /* Since we failed to find the requested key we */
                           /* should respond with a negative response.     */
                           GAP_LE_Authentication_Response_Information.GAP_LE_Authentication_Type = larLongTermKey;
      ...
      switch(GATT_Connection_Event_Data->Event_Data_Type)
      {
               ...
               else
               {
                  ...
                  if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, GATT_Connection_Event_Data->Event_Data.GATT_Device_Connection_Data->RemoteDevice)) != NULL)
                  {
                     if(LocalDeviceIsMaster)
                     {
                        ...
                        /* supported.                                   */
                        GATT_Exchange_MTU_Request(BluetoothStackID, ConnectionID, BTPS_CONFIGURATION_GATT_DEFAULT_MAXIMUM_SUPPORTED_MTU_SIZE, GATT_ClientEventCallback_BMS, 0);
                     }
                  }
               }
            }
         ...
         case etGATT_Connection_Device_Connection_MTU_Update:
            if(GATT_Connection_Event_Data->Event_Data.GATT_Device_Connection_MTU_Update_Data)
            {
               ...
               printf("Error - Null Connection Update Data.");
            break;
         default:
            break;
      }
         ...
         switch(GATT_Service_Discovery_Event_Data->Event_Data_Type)
         {
            case etGATT_Service_Discovery_Indication:
               ...
               if(GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data)
               {
                  ...
                  {
                     /* Attempt to populate the handles for the BMS     */
                     /* Service.                                        */
            ...
            case etGATT_Service_Discovery_Complete:
               ...
               if(GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Complete_Data)
               {
                  ...
                  if(((Service_Discovery_Type_t)CallbackParameter) == sdBMS)
                  {
                     ...
                     printf("   Bond Management Control Point:   %s\r\n", (DeviceInfo->ClientInfo.BM_Control_Point ? "Supported" : "Not Supported"));
                     printf("   Bond Management Feature:         %s\r\n", (DeviceInfo->ClientInfo.BM_Feature ? "Supported" : "Not Supported"));
                  }
               }
               break;
         }

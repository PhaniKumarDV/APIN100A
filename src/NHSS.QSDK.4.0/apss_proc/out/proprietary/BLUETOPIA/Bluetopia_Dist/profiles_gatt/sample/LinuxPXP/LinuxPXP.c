...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXPXP - Linux Bluetooth Proximity Profile using GATT (LE) application. */
/*                                                                            */
/*  Author:  Cliff Xu                                                         */
/*                                                                            */
...
#include "LinuxPXP.h"            /* Application Header.                       */
...
#define DEFAULT_DEBUG_LOG_FILE_NAME                "LinuxPXP_ASC.log"
#define DEFAULT_DEBUG_FTS_FILE_NAME                "LinuxPXP_FTS.log"

   /* Determine the Name we will use for this compilation.              */
#define LE_DEMO_DEVICE_NAME                        "LinuxPXP"
...
#define DEVICE_INFO_FLAGS_LLS_SERVICE_DISCOVERY_COMPLETE    0x08
#define DEVICE_INFO_FLAGS_IAS_SERVICE_DISCOVERY_COMPLETE    0x10
#define DEVICE_INFO_FLAGS_TPS_SERVICE_DISCOVERY_COMPLETE    0x20
...
static int SetAlertLevelControlPoint(ParameterList_t *TempParam);
...
static void UserInterface(void)
{
   ...
   AddCommand("SETALERTLEVELCONTROLPOINT",SetAlertLevelControlPoint);
            ...
            if(CurrentCharacteristic->Characteristic_UUID.UUID_Type == guUUID_16)
            {
               ...
               if(LLS_COMPARE_LLS_ALERT_LEVEL_UUID_TO_UUID_16(CurrentCharacteristic->Characteristic_UUID.UUID.UUID_16))
               {
                  ...
                  if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_READ))
                     printf("Warning - Mandatory read property of LLS Alert Level characteristic not supported!\r\n");

                  /* Verify that Write is supported.                    */
                  if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_WRITE))
                     printf("Warning - Mandatory write property of LLS Alert Level characteristic not supported!\r\n");
               }
               else
               {
                  printf("Unknown LLS characteristic detected!\r\n");
               }
            }
         }
      }
            ...
            if(CurrentCharacteristic->Characteristic_UUID.UUID_Type == guUUID_16)
            {
               ...
               if(IAS_COMPARE_ALERT_LEVEL_UUID_TO_UUID_16(CurrentCharacteristic->Characteristic_UUID.UUID.UUID_16))
               {
                  ...
                  if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_WRITE_WITHOUT_RESPONSE))
                     printf("Warning - Mandatory write without response property of Alert Level characteristic not supported!\r\n");
               }
               else
               {
                  printf("Unknown IAS characteristic detected!\r\n");
               }
            }
         }
      }
         ...
         for(Index = 0; Index < ServiceDiscoveryData->NumberOfCharacteristics; Index++, CurrentCharacteristic++)
         {
            ...
            if(CurrentCharacteristic->Characteristic_UUID.UUID_Type == guUUID_16)
            {
               ...
               if(TPS_COMPARE_TPS_TX_POWER_LEVEL_UUID_TO_UUID_16(CurrentCharacteristic->Characteristic_UUID.UUID.UUID_16))
               {
                  ...
                  /* Verify that read is supported.                     */
                  if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_READ))
                     printf("Warning - Mandatory read property of Tx Power Level characteristic not supported!\r\n");

                  continue;
               }
               else
               {
                  printf("Unknown TPS characteristic detected!\r\n");
               }
            }
         }
      }
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("* Command Options LLS:     RegisterLLS, UnRegisterLLS,           *\r\n");
   printf("*                          DiscoverLLS,                          *\r\n");
   printf("*                          GetAlertLevel,                        *\r\n");
   printf("*                          SetAlertLevel,                        *\r\n");
   printf("* Command Options IAS:     RegisterIAS, UnRegisterIAS,           *\r\n");
   printf("*                          DiscoverIAS,                          *\r\n");
   printf("*                          SetAlertLevelControlPoint,            *\r\n");
   printf("* Command Options TPS:     RegisterTPS, UnRegisterTPS,           *\r\n");
   printf("*                          DiscoverTPS,                          *\r\n");
   printf("*                          SetTxPowerLevel,                      *\r\n");
   printf("*                          GetTxPowerLevel,                      *\r\n");
   printf("******************************************************************\r\n");
   ...
   if((BluetoothStackID) && (!ConnectionID))
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].intParam >= 0) && (TempParam->Params[0].intParam <= 1))
      {
         ...
         else
         {
            ...
            if(LLSInstanceID)
            {
               /* Advertise the LLS Server (1 byte type & 2 bytes UUID).*/
...
static int RegisterLLS(ParameterList_t *TempParam)
{
   ...
   if(!ConnectionID)
   {
      ...
      if(!LLSInstanceID)
      {
         ...
         {
            /* Display success message.                                 */
            printf("Successfully registered LLS Service.\r\n");
      ...
      {
         printf("LLS Service already registered.\r\n");
...
static int UnregisterLLS(ParameterList_t *TempParam)
{
   ...
   if(LLSInstanceID)
   {
      ...
      {
         /* Display success message.                                    */
         printf("Successfully unregistered LLS Service.\r\n");
      ...
      printf("LLS Service not registered.\r\n");
...
static int DiscoverLLS(ParameterList_t *TempParam)
{
   ...
   if(!LLSInstanceID)
   {
      ...
      printf("Cannot discover LLS Services when registered as a service.\r\n");
...
static int GetAlertLevel(ParameterList_t *TempParam)
{
   ...
   if(LLSInstanceID)
   {
      ...
      if(ret_val == 0)
      {
         printf("LLS Alert Level: %u", AlertLevel);
      }
      else
      ...
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            if(DeviceInfo->LLS_ClientInfo.Alert_Level != 0)
            {
               ...
               {
                  printf("Get Alert Level request sent, Transaction ID = %u", ret_val);
               ...
               printf("Error - LLS Alert Level attribute handle is invalid!\r\n");
         }
         else
            printf("No Device Info.\r\n");
      }
      else
         printf("Either a LLS server must be registered or a LLS client must be connected.\r\n");
   }
...
static int SetAlertLevel(ParameterList_t *TempParam)
{
   ...
   if((TempParam) && (TempParam->NumberofParameters > 0))
   {
      ...
      if(LLSInstanceID)
      {
         ...
         {
            printf("Alert Level successfully set to %u. \r\n", AlertLevel);
         }
         else
         ...
         if(ConnectionID)
         {
            ...
            if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
            {
               ...
               if(DeviceInfo->LLS_ClientInfo.Alert_Level != 0)
               {
                  ...
                  {
                     printf("Set Alert Level Request sent, Transaction ID = %u", ret_val);
                  ...
                  printf("Error - Alert Level attribute handle is invalid!\r\n");
...
static int RegisterIAS(ParameterList_t *TempParam)
{
   ...
   if(!ConnectionID)
   {
      ...
      if(!IASInstanceID)
      {
         ...
         {
            /* Display success message.                                 */
            printf("Successfully registered IAS Service.\r\n");
      ...
      {
         printf("IAS Service already registered.\r\n");
...
static int UnregisterIAS(ParameterList_t *TempParam)
{
   ...
   if(IASInstanceID)
   {
      ...
      {
         /* Display success message.                                    */
         printf("Successfully unregistered IAS Service.\r\n");
      ...
      printf("IAS Service not registered.\r\n");
...
static int DiscoverIAS(ParameterList_t *TempParam)
{
   ...
   if(!IASInstanceID)
   {
      ...
      printf("Cannot discover IAS Services when registered as a service.\r\n");
         ...
         {  /* Verify that there is a valid connection                  */
            if(ConnectionID)
            {
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
               {
                  ...
                  {
                     /*Format the Command                               */
                     if((ret_val = IAS_Format_Control_Point_Command(Input, IAS_ALERT_LEVEL_CONTROL_POINT_VALUE_LENGTH, Buffer)) == 0)
                     {
                     ...
                     printf("Error - Alert Level Control Point not supported on remote service!\r\n");
               }
               else
            ...
            printf("Cannot Set Alert Level Control Point when registered as a service.\r\n");
      }
      else
      {
         printf("Invalid Input\r\n");
         DisplayUsage(("SetAlertLevelControlPoint [Value]"));
         printf("   Where Value = \r\n");
         printf("      0 : cpNoAlert\r\n");
         printf("      1 : cpMildAlert\r\n");
         printf("      2 : cpHighAlert\r\n");
      }
   }
   else
   {
...
static int RegisterTPS(ParameterList_t *TempParam)
{
   ...
   if(!ConnectionID)
   {
      ...
      if(!TPSInstanceID)
      {
         ...
         {
            /* Display success message.                                 */
            printf("Successfully registered TPS Service.\r\n");
      ...
      {
         printf("TPS Service already registered.\r\n");
...
static int UnregisterTPS(ParameterList_t *TempParam)
{
   ...
   if(TPSInstanceID)
   {
      ...
      {
         /* Display success message.                                    */
         printf("Successfully unregistered TPS Service.\r\n");
      ...
      printf("TPS Service not registered.\r\n");
...
static int DiscoverTPS(ParameterList_t *TempParam)
{
   ...
   if(!TPSInstanceID)
   {
      ...
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            if(!(DeviceInfo->Flags & DEVICE_INFO_FLAGS_SERVICE_DISCOVERY_OUTSTANDING))
            {
               ...
               {
                  /* Display success message.                           */
                  printf("GATT_Start_Service_DiscoveryStart success.\r\n");
      ...
      printf("Cannot discover TPS Services when registered as a service.\r\n");
...
static int GetTxPowerLevel(ParameterList_t *TempParam)
{
   ...
   if(TPSInstanceID)
   {
      ...
      if(ret_val == 0)
      {
         printf("Tx Power Level: %u", TxPowerLevel);
      }
      else
      ...
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            if(DeviceInfo->TPS_ClientInfo.Tx_Power_Level != 0)
            {
               ...
               {
                  printf("Get Tx Power Level request sent, Transaction ID = %u", ret_val);
               ...
               printf("Error - Tx Power Level attribute handle is invalid!\r\n");
         }
         else
            printf("No Device Info.\r\n");
      }
      else
         printf("Either a TPS server must be registered or a TPS client must be connected.\r\n");
   }
...
static int SetTxPowerLevel(ParameterList_t *TempParam)
{
   ...
   /* Use SupportedFeatures here to avoid compiler warning              */
   ...
   if((TempParam) && (TempParam->NumberofParameters >= 1))
   {
      ...
      if(TPSInstanceID)
      {
         ...
         if(ret_val == 0)
         {
            printf("Tx Power Level successfully set.\r\n");
         }
         else
            DisplayFunctionError("TPS_Set_Tx_Power_Level", ret_val);
      }
      else
      {
         if(ConnectionID)
            printf("Cannot write Tx Power Level as a client.\r\n");
         else
            printf("TPS server not registered.\r\n");
      }
   }
   else
   {
      DisplayUsage("SetTxPowerLevel [TxPowerLevel]");
   }
...
static void BTPSAPI LLS_EventCallback(unsigned int BluetoothStackID, LLS_Event_Data_t *LLS_Event_Data, unsigned long CallbackParameter)
{
   ...
   if((BluetoothStackID) && (LLS_Event_Data))
   {
      switch(LLS_Event_Data->Event_Data_Type)
      {
         case etLLS_Alert_Level_Update:
            printf("etLLS_Alert_Level_Update with size %u.\r\n", LLS_Event_Data->Event_Data_Size);
            ...
            if(LLS_Event_Data->Event_Data.LLS_Alert_Level_Update_Data)
            {
               ...
               printf("Alert Level:      %u.\r\n", LLS_Event_Data->Event_Data.LLS_Alert_Level_Update_Data->AlertLevel);

               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, LLS_Event_Data->Event_Data.LLS_Alert_Level_Update_Data->RemoteDevice)) != NULL)
               {
   ...
   else
   {
      ...
      printf("\r\n");
      printf("LLS Callback Data: Event_Data = NULL.\r\n");
   }
   DisplayPrompt();
}
...
static void BTPSAPI IAS_EventCallback(unsigned int BluetoothStackID, IAS_Event_Data_t *IAS_Event_Data, unsigned long CallbackParameter)
{
   ...
   if((BluetoothStackID) && (IAS_Event_Data))
   {
      switch(IAS_Event_Data->Event_Data_Type)
      {
         case etIAS_Server_Alert_Level_Control_Point_Command:
            printf("etIAS_Server_Alert_Level_Control_Point_Command with size %u.\r\n", IAS_Event_Data->Event_Data_Size);
            ...
            if(IAS_Event_Data->Event_Data.IAS_Alert_Level_Control_Point_Command_Data)
            {
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, IAS_Event_Data->Event_Data.IAS_Alert_Level_Control_Point_Command_Data->RemoteDevice)) != NULL)
               {
                  ...
                  if(IAS_Event_Data->Event_Data.IAS_Alert_Level_Control_Point_Command_Data->InstanceID == IASInstanceID)
                  {
                     ...
                     {
                        case cpNoAlert:
                           printf("   Command Type:      cpNoAlert.\r\n");
                           break;

                        case cpMildAlert:
                           printf("   Command Type:      cpMildAlert.\r\n");
                           break;

                        case cpHighAlert:
                           printf("   Command Type:      cpHighAlert.\r\n");
                           break;
   ...
   else
   {
      ...
      printf("IAS Callback Data: Event_Data = NULL.\r\n");
   }
      ...
      switch(GATT_Client_Event_Data->Event_Data_Type)
      {
         case etGATT_Client_Error_Response:
            if(GATT_Client_Event_Data->Event_Data.GATT_Request_Error_Data)
            {
               ...
               {
                  printf("   Error Mesg:    RACP Improperly Configured\r\n");
               }
               else
               {
            ...
            if(GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data)
            {
               ...
               if(ValueLength != 0)
               {
                  if(((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data->RemoteDevice)) != NULL) && (CallbackParameter != 0))
                  {
                     ...
                     {
                        printf("\r\n   Tx Power Level: %u.\r\n", (Word_t)GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data->AttributeValue[0]);
                     }
                  }
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, GATT_Connection_Event_Data->Event_Data.GATT_Device_Connection_Data->RemoteDevice)) != NULL)
               {
                  if(LocalDeviceIsMaster)
                  {
                     ...
                     if(!GATT_Query_Maximum_Supported_MTU(BluetoothStackID, &MTU))
                        GATT_Exchange_MTU_Request(BluetoothStackID, ConnectionID, MTU, GATT_ClientEventCallback_LLS, 0);
                  }
               }
            }
            else

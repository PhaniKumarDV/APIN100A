...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXHOGH - Linux Bluetooth HID over GATT (HOG) Host using GATT (LE)      */
/*             application.                                                   */
/*                                                                            */
...
/*   --------  -----------    ------------------------------------------------*/
/*   03/02/15  M. Rougeux     Initial creation.                               */
/******************************************************************************/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>

#include "LinuxHOGH.h"           /* Application Header.                       */
...
#include "UHID.h"                /* UHID Programming Interface.               */
...
#define MAX_SUPPORTED_HID_INSTANCES                (10)   /* Denotes the       */
                                                         /* maximum number of */
                                                         /* HID Service       */
                                                         /* Instances that are*/
                                                         /* supported by this */
...
#define HIDS_MAXIMUM_NUMBER_REPORTS                (10)  /* Denotes the       */
                                                         /* maximum number of */
                                                         /* supported HIDS    */
                                                         /* reports.          */

#define HIDS_MAXIMUM_NUMBER_EXTERNAL_REPORT_REFERENCES (10)  /* Denotes the   */
                                                         /* maximum number of */
                                                         /* supported HIDS    */
                                                         /* external report   */
                                                         /* references.       */
...
#define DEFAULT_DEBUG_LOG_FILE_NAME                "LinuxHOGH_ASC.log"
#define DEFAULT_DEBUG_FTS_FILE_NAME                "LinuxHOGH_FTS.log"

   /* Determine the Name we will use for this compilation.              */
#define LE_DEMO_DEVICE_NAME                        "LinuxHOGH"

   /* The following type definition represents the container type which */
   /* holds the mapping between Bluetooth devices (based on the         */
   /* BD_ADDR) and the Link Key (BD_ADDR <-> Link Key Mapping).         */
...
#define ToInt(_x)                                  (((_x) > 0x39)?(((_x) & ~0x20)-0x37):((_x)-0x30))
   ...
   /* The following type definition represents the structure which      */
   /* holds all information about the parameter, in particular the      */
   /* parameter as a string and the parameter as an unsigned int.       */
typedef struct _tagParameter_t
{
   ...
   /* The following type definition represents the structure which      */
   /* holds a list of parameters that are to be associated with a       */
   /* command The NumberofParameters variable holds the value of the    */
...
typedef struct _tagParameterList_t
{
   ...
   /* The following type definition represents the structure which      */
   /* holds the command and parameters to be executed.                  */
...
typedef int (*CommandFunction_t)(ParameterList_t *TempParam);
   ...
   /* The following type definition represents the structure which      */
   /* holds information used in the interpretation and execution of     */
   /* Commands.                                                         */
...
typedef struct _tagGAPS_Device_Appearance_Mapping_t
{
   ...
   /* The following structure represents the attribute handles of DIS   */
   /* Service that will hold DIS attribute handles after DIS Service    */
   /* Discovery.                                                        */
...
typedef struct _tagHIDS_Report_t
{
   ...
   Word_t                        Handle;
   Word_t                        ReportReferenceHandle;
   Word_t                        CCCDHandle;
} HIDS_Report_t;

typedef struct _tagHIDS_External_Report_t
{
   Word_t      ExternalReportReferenceHandle;
   GATT_UUID_t ExternalReportReference;
...
typedef struct _tagHIDS_Client_Info_t
{
   ...
   HIDS_HID_Information_Data_t    HIDSInformation;
   ...
   Word_t                         ProtocolModeHandle;
   Word_t                         ReportMapHandle;
   Word_t                         BootKeyboardInputReport;
   Word_t                         BootKeyboardOutputReport;
   Word_t                         BootMouseInputReport;
   Word_t                         HIDSInformationHandle;
   Word_t                         ControlPoint;
   Word_t                         BootKeyboardInputReport_CCCD;
   Word_t                         BootMouseInputReport_CCCD;
...
typedef struct _tagDeviceInfo_t
{
   ...
   BAS_Client_Information_t  BASClientInfo[MAX_SUPPORTED_BATTERY_INSTANCES];
...
#define DEVICE_INFO_FLAGS_HID_SERVICE_DISCOVERY_COMPLETE       0x08
#define DEVICE_INFO_FLAGS_DIS_SERVICE_DISCOVERY_COMPLETE       0x10
#define DEVICE_INFO_FLAGS_BAS_SERVICE_DISCOVERY_COMPLETE       0x20
#define DEVICE_INFO_FLAGS_DIS_SERVICE_FOUND                    0x40
...
static Boolean_t           UHIDInitialized;         /* Specifies if UHID has been      */
                                                    /* initialize or not.              */
#endif
...
static UUID_128_t          NULL_UUID_128;           /* Holds a NULL UUID 128 for       */
                                                    /* comparison.                     */
...
static int SetGATTMTU(ParameterList_t *TempParam);
   ...
   /* HID over GATT Profile Function Commands                           */
static int DiscoverHIDS(ParameterList_t *TempParam);
static int ConfigureHIDS(ParameterList_t *TempParam);
static int ReadHIDSConfiguration(ParameterList_t *TempParam);
static int GetReport(ParameterList_t *TempParam);
static int SetReport(ParameterList_t *TempParam);
static int SetSuspend(ParameterList_t *TempParam);
static int SetProtocolMode(ParameterList_t *TempParam);

   /* HID over GATT Profile Helper Functions                            */
static void HIDSPopulateHandles(HIDS_Client_Info_t *ClientInfo, GATT_Service_Discovery_Indication_Data_t *ServiceDiscoveryData);
static void ReadHIDSInfo(HIDS_Client_Info_t *ClientInfo);
static int  HIDSGetReport(HIDS_Client_Info_t *ClientInfo, Byte_t ReportType, Byte_t ReportID);
static int  HIDSSetReport(HIDS_Client_Info_t *ClientInfo, Byte_t ReportType, Byte_t ReportID, Byte_t *Value, unsigned int ValueLength, Boolean_t ResponseExpected);

   /* Device Information Service Function Commands                      */
static int DiscoverDIS(ParameterList_t *TempParam);

   /* Device Information Service Helper Functions                       */
static void DISPopulateHandles(DIS_Client_Info_t *ClientInfo, GATT_Service_Discovery_Indication_Data_t *ServiceInfo);
static int  ReadDISInfo(DIS_Client_Info_t *ClientInfo);

   /* Battery Service Function Commands                                 */
static int DiscoverBAS(ParameterList_t *TempParam);
static int ConfigureBAS(ParameterList_t *TempParam);
static int ReadBASConfiguration(ParameterList_t *TempParam);
...
static int GetBatteryLevelPresentationFormat(ParameterList_t *TempParam);
   ...
   /* Battery Service Helper Functions                                  */
static void BASPopulateHandles(BAS_Client_Information_t *ClientInfo, GATT_Service_Discovery_Indication_Data_t *ServiceInfo);
...
static void BTPSAPI GATT_ClientEventCallback_HIDS(unsigned int BluetoothStackID, GATT_Client_Event_Data_t *GATT_Client_Event_Data, unsigned long CallbackParameter);
...
static void BTPSAPI GATT_Service_Discovery_Event_Callback(unsigned int BluetoothStackID, GATT_Service_Discovery_Event_Data_t *GATT_Service_Discovery_Event_Data, unsigned long CallbackParameter);
   ...
   /* UHID Event Callback Function Prototype.                           */
static void UHID_Event_Callback(BD_ADDR_t DeviceAddr, UHID_Event_Data_t *EventData, void *CallbackParameter);
...
static void FreeDeviceInfoEntryMemory(DeviceInfo_t *EntryToFree)
{
   ...
   for(InstanceID = 0; InstanceID < MAX_SUPPORTED_HID_INSTANCES; InstanceID++)
   {
      ...
      if(EntryToFree->HIDSClientInfo[InstanceID].ReportMap)
         BTPS_FreeMemory(EntryToFree->HIDSClientInfo[InstanceID].ReportMap);
   }
   ...
   while(DeviceInfo)
   {
      ...
      for(InstanceID = 0; InstanceID < MAX_SUPPORTED_HID_INSTANCES; InstanceID++)
      {
         ...
         if(DeviceInfo->HIDSClientInfo[InstanceID].ReportMap)
            BTPS_FreeMemory(DeviceInfo->HIDSClientInfo[InstanceID].ReportMap);
      }
...
static void UserInterface(void)
{
   ...
   AddCommand("DISCOVERHIDS", DiscoverHIDS);
   AddCommand("CONFIGUREHIDS", ConfigureHIDS);
   AddCommand("READHIDSCONFIGURATION", ReadHIDSConfiguration);
   AddCommand("GETREPORT", GetReport);
   AddCommand("SETREPORT", SetReport);
   AddCommand("SETSUSPEND", SetSuspend);
   AddCommand("SETPROTOCOLMODE", SetProtocolMode);
   AddCommand("DISCOVERDIS", DiscoverDIS);
   AddCommand("DISCOVERBAS", DiscoverBAS);
   AddCommand("CONFIGUREBAS", ConfigureBAS);
   AddCommand("READBASCONFIGURATION", ReadBASConfiguration);
   ...
   /* handle leading and tailing white space, however it does not       */
   /* handle signed or comma delimited values. This function takes as   */
   /* its input the string which is to be converted. The function       */
   /* returns zero if an error occurs otherwise it returns the value    */
   /* parsed from the string passed as the input parameter.             */
static unsigned int StringToUnsignedInteger(char *StringInteger)
{
   int          IsHex;
   unsigned int Index;
   ...
   if((StringInteger) && (BTPS_StringLength(StringInteger)))
   {
      ...
      if(!IsHex)
      {
         ...
         while(1)
         {
            ...
            if((StringInteger[Index] >= '0') && (StringInteger[Index] <= '9'))
            {
               ...
               {
                  /* The next value is invalid so break out of the      */
                  /* loop.                                              */
                  break;
               }
            }
      ...
      else
      {
         ...
         while(1)
         {
            /* First check to make sure that this is a valid            */
            /* Hexadecimal digit.                                       */
...
static char *StringParser(char *String)
{
   ...
   /* is the structure used to pass the parsed command string out of    */
   /* the function. The second parameter of this function is the string */
...
static int CommandParser(UserCommand_t *TempCommand, char *Input)
{
   ...
   /* This information includes the command name and multiple           */
   /* parameters which maybe be passed to the function to be executed.  */
   /* Successful execution of this function is denoted by a return      */
   /* value of zero. A negative return value implies that command was   */
   /* not found and is invalid.                                         */
static int CommandInterpreter(UserCommand_t *TempCommand)
{
   int               Index;
   int               ret_val;
...
static void ClearCommands(void)
{
   ...
   /* function is a pointer to the string in which the converted        */
   /* BD_ADDR is to be stored.                                          */
static void BD_ADDRToStr(BD_ADDR_t Board_Address, BoardStr_t BoardStr)
{
...
static int CloseStack(void)
{
   ...
   /* mode the Device will respond to Inquiry Scans from other          */
   /* Bluetooth Devices. This function requires that a valid Bluetooth  */
   /* Stack ID exists before running. This function returns zero on     */
   /* successful execution and a negative value if an error occurred.   */
static int SetDisc(void)
{
   int ret_val = 0;
...
static Boolean_t AppearanceToString(Word_t Appearance, char **String)
{
   ...
   /* The following function is used to map an Index into the           */
   /* Appearance Mapping table to it's Appearance Value. This function  */
...
static void GAPSPopulateHandles(GAPS_Client_Info_t *ClientInfo, GATT_Service_Discovery_Indication_Data_t *ServiceInfo)
{
   ...
   /* The following function is a utility function that provides a      */
   /* mechanism of populating a HDIS Client Information structure with  */
   /* the information discovered from a GDIS Discovery operation.       */
static void HIDSPopulateHandles(HIDS_Client_Info_t *ClientInfo, GATT_Service_Discovery_Indication_Data_t *ServiceDiscoveryData)
{
   Word_t                                       *ClientConfigurationHandle;
   Word_t                                       *ReportReferenceHandle = NULL;
   unsigned int                                  Index1;
   unsigned int                                  Index2;
   unsigned int                                  Index3;
   GATT_Characteristic_Information_t            *CurrentCharacteristic;
   ...
   /* Verify that the input parameters are semi-valid.                  */
   if((ClientInfo) && (ServiceDiscoveryData) && (ServiceDiscoveryData->ServiceInformation.UUID.UUID_Type == guUUID_16) && (HIDS_COMPARE_HIDS_SERVICE_UUID_TO_UUID_16(ServiceDiscoveryData->ServiceInformation.UUID.UUID.UUID_16)))
   {
      /* Loop through all characteristics discovered in the service and */
      /* populate the correct entry.                                    */
      ...
      if(CurrentCharacteristic)
      {
         for(Index1 = 0; Index1 < ServiceDiscoveryData->NumberOfCharacteristics; Index1++, CurrentCharacteristic++)
         {
            ...
            if(CurrentCharacteristic->Characteristic_UUID.UUID_Type == guUUID_16)
            {
               ...
               /* Determine which characteristic this is.               */
               if(!HIDS_COMPARE_HIDS_PROTOCOL_MODE_UUID_TO_UUID_16(CurrentCharacteristic->Characteristic_UUID.UUID.UUID_16))
               {
                  if(!HIDS_COMPARE_HIDS_REPORT_MAP_UUID_TO_UUID_16(CurrentCharacteristic->Characteristic_UUID.UUID.UUID_16))
                  {
                     if(!HIDS_COMPARE_HIDS_REPORT_UUID_TO_UUID_16(CurrentCharacteristic->Characteristic_UUID.UUID.UUID_16))
                     {
                        if(!HIDS_COMPARE_HIDS_BOOT_KEYBOARD_INPUT_REPORT_UUID_TO_UUID_16(CurrentCharacteristic->Characteristic_UUID.UUID.UUID_16))
                        {
                           if(!HIDS_COMPARE_HIDS_BOOT_KEYBOARD_OUTPUT_REPORT_UUID_TO_UUID_16(CurrentCharacteristic->Characteristic_UUID.UUID.UUID_16))
                           {
                              if(!HIDS_COMPARE_HIDS_BOOT_MOUSE_INPUT_REPORT_UUID_TO_UUID_16(CurrentCharacteristic->Characteristic_UUID.UUID.UUID_16))
                              {
                                 if(!HIDS_COMPARE_HIDS_HID_INFORMATION_UUID_TO_UUID_16(CurrentCharacteristic->Characteristic_UUID.UUID.UUID_16))
                                 {
                                    if(!HIDS_COMPARE_HIDS_HID_CONTROL_POINT_UUID_TO_UUID_16(CurrentCharacteristic->Characteristic_UUID.UUID.UUID_16))
                                    {
                                       continue;
                                    }
                                    else
                                    {
                                       ...
                                       if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_WRITE_WITHOUT_RESPONSE))
                                          printf("Warning - Mandatory write without response property of HIDS Control Point characteristic not supported!\r\n");
                                 ...
                                 else
                                 {
                                    ...
                                    if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_READ))
                                       printf("Warning - Mandatory read property of HIDS Information characteristic not supported!\r\n");
                              ...
                              else
                              {
                                 ...
                                 /* Verify that read is supported.      */
                                 if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_READ))
                                    printf("Warning - Mandatory read property of HIDS Boot Mouse Input Report characteristic not supported!\r\n");

                                 /* Verify that notify is supported.    */
                                 if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_NOTIFY))
                                    printf("Warning - Mandatory notify property of HIDS Boot Mouse Input Report characteristic not supported!\r\n");
                              }
                           }
                           else
                           {
                              ...
                              /* Verify that read is supported.         */
                              if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_READ))
                                 printf("Warning - Mandatory read property of HIDS Boot Keyboard Output Report characteristic not supported!\r\n");

                              /* Verify that write is supported.        */
                              if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_WRITE))
                                 printf("Warning - Mandatory write property of HIDS Boot Keyboard Output Report characteristic not supported!\r\n");

                              /* Verify that write without response is  */
                              /* supported.                             */
                              if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_WRITE_WITHOUT_RESPONSE))
                                 printf("Warning - Mandatory write without response property of HIDS Boot Keyboard Output Report characteristic not supported!\r\n");
                        ...
                        else
                        {
                           ...
                           /* Verify that read is supported.            */
                           if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_READ))
                              printf("Warning - Mandatory read property of HIDS Boot Keyboard Input Report characteristic not supported!\r\n");

                           /* Verify that notify is supported.          */
                           if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_NOTIFY))
                              printf("Warning - Mandatory notify property of HIDS Boot Keyboard Input Report characteristic not supported!\r\n");
                        }
                     }
                     else
                     {
                  ...
                  else
                  {
                     ...
                     /* Verify that read is supported.                  */
                     if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_READ))
                        printf("Warning - Mandatory read property of HIDS Report Map characteristic not supported!\r\n");
                  }
               }
               else
               {
                  ...
                  /* Verify that read is supported.                     */
                  if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_READ))
                     printf("Warning - Mandatory read property of HIDS Protocol Mode characteristic not supported!\r\n");

                  /* Verify that write without response is supported.   */
                  if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_WRITE_WITHOUT_RESPONSE))
                     printf("Warning - Mandatory write without response property of HIDS Protocol Mode characteristic not supported!\r\n");
               ...
               if((CurrentDescriptor))
               {
                  for(Index2 = 0; Index2 < CurrentCharacteristic->NumberOfDescriptors; Index2++, CurrentDescriptor++)
                  {
                     if(CurrentDescriptor->Characteristic_Descriptor_UUID.UUID_Type == guUUID_16)
                     {
                        ...
                        /* Descriptor.                                  */
                        if((ClientConfigurationHandle) && (GATT_COMPARE_CLIENT_CHARACTERISTIC_CONFIGURATION_ATTRIBUTE_TYPE_TO_BLUETOOTH_UUID_16(CurrentDescriptor->Characteristic_Descriptor_UUID.UUID.UUID_16)))
                        {
                        ...
                        else
                        {
                           ...
                           if((ReportReferenceHandle) && (HIDS_COMPARE_REPORT_REFERENCE_DESCRIPTOR_UUID_TO_UUID_16(CurrentDescriptor->Characteristic_Descriptor_UUID.UUID.UUID_16)))
                           {
                           ...
                           else
                           {
                              ...
                              /* Descriptor(s).                         */
                              if(HIDS_COMPARE_EXTERNAL_REPORT_REFERENCE_DESCRIPTOR_UUID_TO_UUID_16(CurrentDescriptor->Characteristic_Descriptor_UUID.UUID.UUID_16))
                              {
                                 ...
                                 for(Index3=0; Index3 < HIDS_MAXIMUM_NUMBER_EXTERNAL_REPORT_REFERENCES; Index3++)
                                 {
                                    ...
                                    /* and its value found.             */
                                    if(!(ClientInfo->ExternalReportReferences[Index3].ExternalReportReferenceHandle) || ((ClientInfo->ExternalReportReferences[Index3].ExternalReportReferenceHandle) && !(COMPARE_UUID_128(NULL_UUID_128, ClientInfo->ExternalReportReferences[Index3].ExternalReportReference.UUID.UUID_128))))
                                    {
...
static void DISPopulateHandles(DIS_Client_Info_t *ClientInfo, GATT_Service_Discovery_Indication_Data_t *ServiceInfo)
{
                                 ...
                                 else
                                 {
                                    ...
                                    else
                                    {
                                       if(DIS_COMPARE_DIS_PNP_ID_UUID_TO_UUID_16(CurrentCharacteristic->Characteristic_UUID.UUID.UUID_16))
                                          ClientInfo->PNPID = CurrentCharacteristic->Characteristic_Handle;
                                    }
                                 }
                              }
                           }
   ...
   /* the information discovered from a GDIS Discovery operation.       */
static void BASPopulateHandles(BAS_Client_Information_t *ClientInfo, GATT_Service_Discovery_Indication_Data_t *ServiceDiscoveryData)
{
   unsigned int                                  Index1;
   unsigned int                                  Index2;
   GATT_Characteristic_Information_t            *CurrentCharacteristic;
   ...
   /* Verify that the input parameters are semi-valid.                  */
   if((ClientInfo) && (ServiceDiscoveryData) && (ServiceDiscoveryData->ServiceInformation.UUID.UUID_Type == guUUID_16) && (BAS_COMPARE_BAS_SERVICE_UUID_TO_UUID_16(ServiceDiscoveryData->ServiceInformation.UUID.UUID.UUID_16)))
   {
      /* Loop through all characteristics discovered in the service and */
      /* populate the correct entry.                                    */
      CurrentCharacteristic = ServiceDiscoveryData->CharacteristicInformationList;
      if(CurrentCharacteristic)
      {
         for(Index1 = 0; Index1 < ServiceDiscoveryData->NumberOfCharacteristics; Index1++, CurrentCharacteristic++)
         {
            if(BAS_COMPARE_BATTERY_LEVEL_UUID_TO_UUID_16(CurrentCharacteristic->Characteristic_UUID.UUID.UUID_16))
            {
               ...
               if(CurrentCharacteristic->Characteristic_UUID.UUID_Type == guUUID_16)
               {
                  ClientInfo->Battery_Level = CurrentCharacteristic->Characteristic_Handle;
                  ...
                  for(Index2 = 0; Index2 < CurrentCharacteristic->NumberOfDescriptors; Index2++)
                  {
                     if(CurrentDescriptor->Characteristic_Descriptor_UUID.UUID_Type == guUUID_16)
                     {
                        ...
                        {
                           ClientInfo->Battery_Level_Client_Configuration = CurrentCharacteristic->DescriptorList[Index2].Characteristic_Descriptor_Handle;
                        }
                        else
                        {
                           ...
                           {
                              ClientInfo->Battery_Level_Presentation_Format = CurrentCharacteristic->DescriptorList[Index2].Characteristic_Descriptor_Handle;
                           }
                        }
                     }
                  }
...
Boolean_t IsBatteryLevelHandle(Word_t Handle, DeviceInfo_t *DeviceInfo)
{
   ...
   /* Battery Level Presentation Format characteristic of any of the    */
   ...
   if((BluetoothStackID) && (!COMPARE_NULL_BD_ADDR(BD_ADDR)))
   {
      if(COMPARE_NULL_BD_ADDR(ConnectionBD_ADDR))
      {
         ...
         if(Result >= 0)
         {
            ...
            ConnectionParameters.Connection_Interval_Min    = 8;
            ConnectionParameters.Connection_Interval_Max    = 20;
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("* Command Options HIDS:    DiscoverHIDS, ConfigureHIDS,          *\r\n");
   printf("*                          ReadHIDSConfiguration,                *\r\n");
   printf("*                          GetReport, SetReport,                 *\r\n");
   printf("*                          SetSuspend, SetProtocolMode           *\r\n");
   printf("* Command Options DIS:     DiscoverDIS                           *\r\n");
   printf("* Command Options BAS:     DiscoverBAS, ConfigureBAS,            *\r\n");
   printf("*                          ReadBASConfiguration,                 *\r\n");
...
static int PairLE(ParameterList_t *TempParam)
{
   ...
   /* The following function is responsible for performing a GAP        */
   /* Service Service Discovery Operation. This function will return    */
...
static int ReadRemoteName(ParameterList_t *TempParam)
{
   ...
   /* The following function is responsible for reading the Local       */
   /* Device Appearance value. This function will return zero on        */
...
static int EnableDebug(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].intParam >= dtLogFile) && (TempParam->Params[0].intParam <= dtFTS))
      {
         ...
         {
            /* Enable, check to see if debugging is not already         */
            /* enabled.                                                 */
            if(!DebugID)
            {
   ...
   /* The following function is responsible for performing a HID        */
...
static int DiscoverHIDS(ParameterList_t *TempParam)
{
   ...
   if(ConnectionID)
   {
      ...
      if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
      {
         ...
         if(!(DeviceInfo->Flags & DEVICE_INFO_FLAGS_SERVICE_DISCOVERY_OUTSTANDING))
         {
            ...
            {
               /* Configure the filter so that only the HID Service is  */
               /* discovered.                                           */
               UUID[0].UUID_Type = guUUID_16;
               HIDS_ASSIGN_HIDS_SERVICE_UUID_16(&(UUID[0].UUID.UUID_16));

               /* Reset the HIDS instance index.                        */
               HIDSInstanceIDIndex = 0;

               /* Start the service discovery process.                  */
               ret_val = GATT_Start_Service_Discovery(BluetoothStackID, ConnectionID, (sizeof(UUID)/sizeof(GATT_UUID_t)), UUID, GATT_Service_Discovery_Event_Callback, sdHIDS);
               if(!ret_val)
               {
                  /* Display success message.                           */
                  printf("GATT_Service_Discovery_Start success.\r\n");
                  ...
                  DisplayFunctionError("HIDS_Service_Discovery_Start", ret_val);
            }
            else
               printf("Must be paired with this device to use HIDS.\r\n");
         }
         else
            printf("Service Discovery Operation Outstanding for Device.\r\n");
      }
   ...
   /* remote Device Information Service. This function will return zero */
   /* on successful execution and a negative value on errors.           */
static int ReadDISInfo(DIS_Client_Info_t *ClientInfo)
{
   int ret_val;

   printf("\r\nAttempting to read DIS information...\r\n");

   /* Read the DIS Manufacturer Name if it exists.                      */
   if(ClientInfo->ManufacturerName)
   {
      ret_val = GATT_Read_Value_Request(BluetoothStackID, ConnectionID, ClientInfo->ManufacturerName, GATT_ClientEventCallback_DIS, ClientInfo->ManufacturerName);

      if(ret_val < 0)
      {
         /* Reading the DIS Manufacturer Name failed.                   */
         DisplayFunctionError("GATT_Read_Value_Request", ret_val);
         ret_val = 0;
      }
   }

   /* Read the DIS Model Number if it exists.                           */
   if(ClientInfo->ModelNumber)
   {
      ret_val = GATT_Read_Value_Request(BluetoothStackID, ConnectionID, ClientInfo->ModelNumber, GATT_ClientEventCallback_DIS, ClientInfo->ModelNumber);

      if(ret_val < 0)
      {
         /* Reading the DIS Model Number failed.                        */
         DisplayFunctionError("GATT_Read_Value_Request", ret_val);
         ret_val = 0;
      }
   }

   /* Read the DIS Serial Number if it exists.                          */
   if(ClientInfo->SerialNumber)
   {
      ret_val = GATT_Read_Value_Request(BluetoothStackID, ConnectionID, ClientInfo->SerialNumber, GATT_ClientEventCallback_DIS, ClientInfo->SerialNumber);

      if(ret_val < 0)
      {
         /* Reading the DIS Serial Number failed.                       */
         DisplayFunctionError("GATT_Read_Value_Request", ret_val);
         ret_val = 0;
      }
   }

   /* Read the DIS Hardware Revision if it exists.                      */
   if(ClientInfo->HardwareRevision)
   {
      ret_val = GATT_Read_Value_Request(BluetoothStackID, ConnectionID, ClientInfo->HardwareRevision, GATT_ClientEventCallback_DIS, ClientInfo->HardwareRevision);

      if(ret_val < 0)
      {
         /* Reading the DIS Hardware Revision failed.                   */
         DisplayFunctionError("GATT_Read_Value_Request", ret_val);
         ret_val = 0;
      }
   }

   /* Read the DIS Firmware Revision if it exists.                      */
   if(ClientInfo->FirmwareRevision)
   {
      ret_val = GATT_Read_Value_Request(BluetoothStackID, ConnectionID, ClientInfo->FirmwareRevision, GATT_ClientEventCallback_DIS, ClientInfo->FirmwareRevision);

      if(ret_val < 0)
      {
         /* Reading the DIS Firmware Revision failed.                   */
         DisplayFunctionError("GATT_Read_Value_Request", ret_val);
         ret_val = 0;
      }
   }

   /* Read the DIS Software Revision if it exists.                      */
   if(ClientInfo->SoftwareRevision)
   {
      ret_val = GATT_Read_Value_Request(BluetoothStackID, ConnectionID, ClientInfo->SoftwareRevision, GATT_ClientEventCallback_DIS, ClientInfo->SoftwareRevision);

      if(ret_val < 0)
      {
         /* Reading the DIS Software Revision failed.                   */
         DisplayFunctionError("GATT_Read_Value_Request", ret_val);
         ret_val = 0;
      }
   }

   /* Read the DIS System ID if it exists.                              */
   if(ClientInfo->SystemID)
   {
      ret_val = GATT_Read_Value_Request(BluetoothStackID, ConnectionID, ClientInfo->SystemID, GATT_ClientEventCallback_DIS, ClientInfo->SystemID);

      if(ret_val < 0)
      {
         /* Reading the DIS System ID failed.                           */
         DisplayFunctionError("GATT_Read_Value_Request", ret_val);
         ret_val = 0;
      }
   }

   /* Read the DIS IEEE Certification Data List if it exists.           */
   if(ClientInfo->IEEECertificationDataList)
   {
      ret_val = GATT_Read_Value_Request(BluetoothStackID, ConnectionID, ClientInfo->IEEECertificationDataList, GATT_ClientEventCallback_DIS, ClientInfo->IEEECertificationDataList);

      if(ret_val < 0)
      {
         /* Reading the DIS IEEE Certification Data List failed.        */
         DisplayFunctionError("GATT_Read_Value_Request", ret_val);
         ret_val = 0;
      }
   }

   /* Read the DIS PnP ID if it exists.                                 */
   if(ClientInfo->PNPID)
   {
      ret_val = GATT_Read_Value_Request(BluetoothStackID, ConnectionID, ClientInfo->PNPID, GATT_ClientEventCallback_DIS, ClientInfo->PNPID);

      if(ret_val < 0)
      {
         /* Reading the DIS PnP ID failed.                              */
         DisplayFunctionError("GATT_Read_Value_Request", ret_val);
         ret_val = 0;
      }
   }
   ...
   {
      ret_val = 0;
      printf("Read DIS Information complete.\r\n");
   }
...
static void ReadHIDSInfo(HIDS_Client_Info_t *ClientInfo)
{
   ...
   {
      ret_val = GATT_Read_Value_Request(BluetoothStackID, ConnectionID, ClientInfo->ProtocolModeHandle, GATT_ClientEventCallback_HIDS, ClientInfo->ProtocolModeHandle);
   ...
   {
      ret_val = GATT_Read_Value_Request(BluetoothStackID, ConnectionID, ClientInfo->HIDSInformationHandle, GATT_ClientEventCallback_HIDS, ClientInfo->HIDSInformationHandle);
      ...
      printf("Error - HID Information handle not found on this device.\r\n");
   ...
   for(Index=0; Index < HIDS_MAXIMUM_NUMBER_REPORTS; Index++)
   {
      ...
      {
         ret_val = GATT_Read_Value_Request(BluetoothStackID, ConnectionID, ClientInfo->Reports[Index].ReportReferenceHandle, GATT_ClientEventCallback_HIDS, ClientInfo->Reports[Index].ReportReferenceHandle);
   ...
   for(Index=0; Index < HIDS_MAXIMUM_NUMBER_EXTERNAL_REPORT_REFERENCES; Index++)
   {
      ...
      {
         ret_val = GATT_Read_Value_Request(BluetoothStackID, ConnectionID, ClientInfo->ExternalReportReferences[Index].ExternalReportReferenceHandle, GATT_ClientEventCallback_HIDS, ClientInfo->ExternalReportReferences[Index].ExternalReportReferenceHandle);
   ...
   {
      ret_val = GATT_Read_Value_Request(BluetoothStackID, ConnectionID, ClientInfo->ReportMapHandle, GATT_ClientEventCallback_HIDS, ClientInfo->ReportMapHandle);
      ...
      printf("Error - Report Map handle not found on this device.\r\n");
}
   ...
   if(ClientInfo)
   {
      ...
      switch(ReportType)
      {
         case hrtHIDSInputReport:
            ...
            for(Index = 0; Index < HIDS_MAXIMUM_NUMBER_REPORTS; Index++)
            {
               if((ClientInfo->Reports[Index].ReportReferenceData.ReportID == ReportID) && (ClientInfo->Reports[Index].ReportReferenceData.ReportType == HIDS_REPORT_REFERENCE_REPORT_TYPE_INPUT_REPORT))
               {
         ...
         case hrtHIDSOutputReport:
            ...
            for(Index = 0; Index < HIDS_MAXIMUM_NUMBER_REPORTS; Index++)
            {
               if((ClientInfo->Reports[Index].ReportReferenceData.ReportID == ReportID) && (ClientInfo->Reports[Index].ReportReferenceData.ReportType == HIDS_REPORT_REFERENCE_REPORT_TYPE_OUTPUT_REPORT))
               {
                  Handle = ClientInfo->Reports[Index].Handle;
                  break;
               }
         ...
         case hrtHIDSFeatureReport:
            ...
            for(Index = 0; Index < HIDS_MAXIMUM_NUMBER_REPORTS; Index++)
            {
               if((ClientInfo->Reports[Index].ReportReferenceData.ReportID == ReportID) && (ClientInfo->Reports[Index].ReportReferenceData.ReportType == HIDS_REPORT_REFERENCE_REPORT_TYPE_FEATURE_REPORT))
               {
                  Handle = ClientInfo->Reports[Index].Handle;
                  break;
               }
      ...
      if(Handle)
      {
         ...
         ret_val = GATT_Read_Value_Request(BluetoothStackID, ConnectionID, Handle, GATT_ClientEventCallback_HIDS, Handle);
...
static int HIDSSetReport(HIDS_Client_Info_t *ClientInfo, Byte_t ReportType, Byte_t ReportID, Byte_t *Value, unsigned int ValueLength, Boolean_t ResponseExpected)
{
   ...
   if((ClientInfo) && (Value))
   {
      ...
      if(Handle)
      {
         ...
         if(ResponseExpected)
            ret_val = GATT_Write_Request(BluetoothStackID, ConnectionID, Handle, ValueLength, Value, GATT_ClientEventCallback_HIDS, Handle);
         else
            ret_val = GATT_Write_Without_Response_Request(BluetoothStackID, ConnectionID, Handle, ValueLength, Value);
      }
      else
         ret_val = BTPS_ERROR_INVALID_PARAMETER;
   }
...
static int ConfigureHIDS(ParameterList_t *TempParam)
{
   ...
   /* Verify that the input parameters are semi-valid.                  */
   if((TempParam) && (TempParam->NumberofParameters > 1) && (TempParam->Params[0].intParam >= 1) && (TempParam->Params[0].intParam <= MAX_SUPPORTED_HID_INSTANCES))
   {
      /* Verify that there is a connection that is established.         */
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            /* this device                                              */
            if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_HID_SERVICE_DISCOVERY_COMPLETE)
            {
               /* Make sure encryption is enabled.                      */
               if((GAP_LE_Query_Encryption_Mode(BluetoothStackID, ConnectionBD_ADDR, &GAPEncryptionMode) != 0) || (GAPEncryptionMode == emEnabled))
               {
                  ...
                  if(DeviceInfo->HIDSClientInfo[InstanceID].BootKeyboardInputReport_CCCD)
                  {
                     ...
                     /* characteristics.                                */
                     ret_val = EnableDisableNotificationsIndications(DeviceInfo->HIDSClientInfo[InstanceID].BootKeyboardInputReport_CCCD, (TempParam->Params[1].intParam ? GATT_CLIENT_CONFIGURATION_CHARACTERISTIC_NOTIFY_ENABLE : 0), GATT_ClientEventCallback_HIDS);
                  ...
                  if(DeviceInfo->HIDSClientInfo[InstanceID].BootMouseInputReport_CCCD)
                  {
                     /* Enable Notifications on the proper              */
                     /* characteristics.                                */
                     ret_val = EnableDisableNotificationsIndications(DeviceInfo->HIDSClientInfo[InstanceID].BootMouseInputReport_CCCD, (TempParam->Params[1].intParam ? GATT_CLIENT_CONFIGURATION_CHARACTERISTIC_NOTIFY_ENABLE : 0), GATT_ClientEventCallback_HIDS);
                  ...
                  for(Index=0; Index < HIDS_MAXIMUM_NUMBER_REPORTS; Index++)
                  {
                     if(DeviceInfo->HIDSClientInfo[InstanceID].Reports[Index].CCCDHandle)
                     {
                        /* Enable Notifications on the proper           */
                        /* characteristics.                             */
                        ret_val = EnableDisableNotificationsIndications(DeviceInfo->HIDSClientInfo[InstanceID].Reports[Index].CCCDHandle, (TempParam->Params[1].intParam ? GATT_CLIENT_CONFIGURATION_CHARACTERISTIC_NOTIFY_ENABLE : 0), GATT_ClientEventCallback_HIDS);
                  ...
                  if(!UHIDInitialized)
                  {
                     ...
                     if(UHID_Create_Device(DeviceInfo->ConnectionBD_ADDR, &(DeviceInfo->HIDSClientInfo[InstanceID].HIDSInformation), DeviceInfo->HIDSClientInfo[InstanceID].ReportMapLength, DeviceInfo->HIDSClientInfo[InstanceID].ReportMap, UHID_Event_Callback, NULL))
                     {
                        printf("\r\nUHID_Create_Device success.\r\n");
                        UHIDInitialized = TRUE;
                     }
                  }
#endif
               ...
               printf("HIDS service discovery has not been performed on this device.\r\n");
         }
         else
            printf("No Device Info.\r\n");
      }
      ...
      DisplayUsage("ConfigureHIDS [InstanceID] [Report Notify (0 = disable, 1 = enable)]");
...
static int ReadHIDSConfiguration(ParameterList_t *TempParam)
{
   ...
   /* Verify that the input parameters are semi-valid.                  */
   if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].intParam >= 1) && (TempParam->Params[0].intParam <= MAX_SUPPORTED_HID_INSTANCES))
   {
      /* Verify that there is a connection that is established.         */
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_HID_SERVICE_DISCOVERY_COMPLETE)
            {
               ...
               if((GAP_LE_Query_Encryption_Mode(BluetoothStackID, ConnectionBD_ADDR, &GAPEncryptionMode) != 0) || (GAPEncryptionMode == emEnabled))
               {
                  ...
                  printf("Attempting to read CCCDs...\r\n");
                  ...
                  {
                     /* Read the CCD.                                   */
                     ret_val = GATT_Read_Value_Request(BluetoothStackID, ConnectionID, DeviceInfo->HIDSClientInfo[InstanceID].BootKeyboardInputReport_CCCD, GATT_ClientEventCallback_HIDS, DeviceInfo->HIDSClientInfo[InstanceID].BootKeyboardInputReport_CCCD);

                     /* Check for CC Read success                       */
                     if(ret_val > 0)
                     {
                        printf("CCCD Read Success.\r\n");
                        ret_val = 0;
                     }
                     else
                     {
                  ...
                  {
                     /* Read the CCD.                                   */
                     ret_val = GATT_Read_Value_Request(BluetoothStackID, ConnectionID, DeviceInfo->HIDSClientInfo[InstanceID].BootMouseInputReport_CCCD, GATT_ClientEventCallback_HIDS, DeviceInfo->HIDSClientInfo[InstanceID].BootMouseInputReport_CCCD);
                  ...
                  for(Index=0; Index < HIDS_MAXIMUM_NUMBER_REPORTS; Index++)
                  {
                     ...
                     {
                        /* Read the CCD.                                */
                        ret_val = GATT_Read_Value_Request(BluetoothStackID, ConnectionID, DeviceInfo->HIDSClientInfo[InstanceID].Reports[Index].CCCDHandle, GATT_ClientEventCallback_HIDS, DeviceInfo->HIDSClientInfo[InstanceID].Reports[Index].CCCDHandle);
      ...
      DisplayUsage("ReadHIDSConfiguration [InstanceID]");
...
static int GetReport(ParameterList_t *TempParam)
{
   ...
   /* Verify that the input parameters are semi-valid.                  */
   if((TempParam) && (TempParam->NumberofParameters >= 3) && (TempParam->Params[0].intParam >= 1) && (TempParam->Params[0].intParam <= MAX_SUPPORTED_HID_INSTANCES))
   {
      /* Verify that there is a connection that is established.         */
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_HID_SERVICE_DISCOVERY_COMPLETE)
            {
               ...
               /* Get the HIDS report.                                  */
               if((ret_val = HIDSGetReport(&DeviceInfo->HIDSClientInfo[InstanceID], TempParam->Params[1].intParam, TempParam->Params[2].intParam)) > 0)
               {
                  printf("HIDSGetReport() Success: Transaction ID = %d.\r\n", ret_val);
                  ret_val = 0;
               }
               else
               {
                  DisplayFunctionError("HIDSGetReport", ret_val);
                  ret_val = FUNCTION_ERROR;
               }
   ...
   {
      DisplayUsage("GetReport [InstanceID] [ReportType] [ReportID (0 = None]");
      printf("Where: ReportType = \r\n");
      printf("         Input Report                = %u\r\n", hrtHIDSInputReport);
      printf("         Output Report               = %u\r\n", hrtHIDSOutputReport);
      printf("         Feature Report              = %u\r\n", hrtHIDSFeatureReport);
      printf("         Boot Keyboard Input Report  = %u\r\n", hrtHIDSBootKeyboardInputReport);
      printf("         Boot Keyboard Output Report = %u\r\n", hrtHIDSBootKeyboardOuputReport);
      printf("         Boot Mouse Input Report     = %u\r\n", hrtHIDSBootMouseInputReport);
   }
...
static int SetReport(ParameterList_t *TempParam)
{
   ...
   /* Verify that the input parameters are semi-valid.                  */
   if((TempParam) && (TempParam->NumberofParameters >= 5) && (TempParam->Params[0].intParam >= 1) && (TempParam->Params[0].intParam <= MAX_SUPPORTED_HID_INSTANCES))
   {
      /* Verify that there is a connection that is established.         */
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_HID_SERVICE_DISCOVERY_COMPLETE)
            {
               ...
               ReportLength = ExtractHexString(TempParam->Params[4].strParam, &Report);
               if((ReportLength) && (Report))
               {
                  ResponseExpected = (Boolean_t)TempParam->Params[3].intParam;

                  /* Set the HIDS report.                               */
                  if((ret_val = HIDSSetReport(&DeviceInfo->HIDSClientInfo[InstanceID], TempParam->Params[1].intParam, TempParam->Params[2].intParam, Report, ReportLength, ResponseExpected)) > 0)
                  {
                     printf("HIDSSetReport() Success: %d.\r\n", ret_val);
                     ret_val = 0;
                  }
                  else
                  {
                     DisplayFunctionError("HIDSSetReport", ret_val);
                     ret_val = FUNCTION_ERROR;
                  }
   ...
   {
      DisplayUsage("SetReport [InstanceID] [ReportType] [ReportID (0 = None] [Response Expected] [Value (Hex String)]");
      printf("Where: ReportType = \r\n");
      printf("         Input Report                = %u\r\n", hrtHIDSInputReport);
      printf("         Output Report               = %u\r\n", hrtHIDSOutputReport);
      printf("         Feature Report              = %u\r\n", hrtHIDSFeatureReport);
...
static int SetSuspend(ParameterList_t *TempParam)
{
   ...
   if(TempParam && (TempParam->NumberofParameters > 1) && (TempParam->Params[0].intParam >= 1) && (TempParam->Params[0].intParam <= MAX_SUPPORTED_HID_INSTANCES))
   {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_HID_SERVICE_DISCOVERY_COMPLETE)
            {
               ...
               if(DeviceInfo->HIDSClientInfo[InstanceID].ControlPoint)
               {
                  ...
                  Command = (TempParam->Params[1].intParam ? pcSuspend : pcExitSuspend);

                  if(!HIDS_Format_Control_Point_Command(Command, sizeof(Buffer), Buffer))
                  {
                     /* Perform the Write Without Response.             */
                     ret_val = GATT_Write_Without_Response_Request(BluetoothStackID, ConnectionID, DeviceInfo->HIDSClientInfo[InstanceID].ControlPoint, sizeof(Buffer), Buffer);

                     /* Check for GATT Write success                    */
                     if(ret_val > 0)
                     {
                        printf("Set Suspend Mode Success: %s.\r\n", (TempParam->Params[1].intParam ? "Suspend" : "Exit Suspend"));
                        ret_val = 0;
                     }
                     else
                     {
                     ...
                     DisplayFunctionError("HIDS_Format_Control_Point_Command", ret_val);
               }
               else
                  printf("Control point not supported.\r\n");
            }
            else
               printf("Service discovery has not been performed on this device.\r\n");
         }
      ...
      DisplayUsage("SetSuspend [InstanceID] [(0 = Exit Suspend, 1 = Suspend)]");
...
static int SetProtocolMode(ParameterList_t *TempParam)
{
   ...
   if(TempParam && (TempParam->NumberofParameters > 1) && (TempParam->Params[0].intParam >= 1) && (TempParam->Params[0].intParam <= MAX_SUPPORTED_HID_INSTANCES))
   {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_HID_SERVICE_DISCOVERY_COMPLETE)
            {
               ...
               if(DeviceInfo->HIDSClientInfo[InstanceID].ProtocolModeHandle)
               {
                  ...
                  ProtocolMode = (TempParam->Params[1].intParam ? pmReport : pmBoot);

                  if(!HIDS_Format_Protocol_Mode(ProtocolMode, sizeof(Buffer), Buffer))
                  {
                     /* Perform the Write Without Response.             */
                     ret_val = GATT_Write_Without_Response_Request(BluetoothStackID, ConnectionID, DeviceInfo->HIDSClientInfo[InstanceID].ProtocolModeHandle, sizeof(Buffer), Buffer);

                     /* Check for GATT Write success                    */
                     if(ret_val > 0)
                     {
                        printf("Set Protocol Mode Success: %s Mode.\r\n", (TempParam->Params[1].intParam ? "Report" : "Boot"));
                        ret_val = 0;
                     }
                     else
                     {
                     ...
                     DisplayFunctionError("HIDS_Format_Protocol_Mode", ret_val);
               }
               else
                  printf("Suspend mode not supported.\r\n");
            }
            else
               printf("Service discovery has not been performed on this device.\r\n");
         }
      ...
      DisplayUsage("SetProtocolMode [InstanceID] [(0 = Boot, 1 = Report)]");

   return(ret_val);
}
   /* The following function is responsible for performing a DIS        */
   /* Service Discovery Operation. This function will return zero on    */
   /* successful execution and a negative value on errors.              */
static int DiscoverDIS(ParameterList_t *TempParam)
{
      ...
      if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
      {
         ...
         if(!(DeviceInfo->Flags & DEVICE_INFO_FLAGS_SERVICE_DISCOVERY_OUTSTANDING))
         {
            ...
            if(!ret_val)
            {
               ...
               DisplayFunctionError("DIS_Service_Discovery_Start", ret_val);
         }
         else
            printf("Service Discovery Operation Outstanding for Device.\r\n");
      }
...
static int DiscoverBAS(ParameterList_t *TempParam)
{
   ...
   if(ConnectionID)
   {
      ...
      if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
      {
         ...
         if(!(DeviceInfo->Flags & DEVICE_INFO_FLAGS_SERVICE_DISCOVERY_OUTSTANDING))
         {
            /* Configure the filter so that only the BAP Service is     */
            /* discovered.                                              */
            UUID[0].UUID_Type = guUUID_16;
   ...
   /* The following function is responsible for configure a BAP Service */
...
static int ConfigureBAS(ParameterList_t *TempParam)
{
   ...
   {
      DisplayUsage("ConfigureBAS [Battery Level Notification (0 = disable, 1 = enable)] [InstanceID]");
   ...
   {
      DisplayUsage("ConfigureBAS [Battery Level Notification (0 = disable, 1 = enable)]");
   ...
   if(ret_val != INVALID_PARAMETERS_ERROR)
   {
      ...
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            /* Determine if service discovery has been performed on     */
            /* this device                                              */
            if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_BAS_SERVICE_DISCOVERY_COMPLETE)
            {
               ret_val = 0;
               ...
               {
                  ret_val = EnableDisableNotificationsIndications(DeviceInfo->BASClientInfo[InstanceID].Battery_Level_Client_Configuration, (TempParam->Params[0].intParam ? GATT_CLIENT_CONFIGURATION_CHARACTERISTIC_NOTIFY_ENABLE : 0), GATT_ClientEventCallback_BAP);
               }
               else
               ...
               printf("BAS service discovery has not been performed on this device.\r\n");
         }
         else
            printf("No Device Info.\r\n");
      }
   ...
   /* The following function is responsible for reading a BAP Service   */
   /* configuration on a remote device. This function will return zero  */
   /* on successful execution and a negative value on errors.           */
static int ReadBASConfiguration(ParameterList_t *TempParam)
{
   ...
   {
      DisplayUsage("ReadBASConfiguration [InstanceID]");
      ...
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_BAS_SERVICE_DISCOVERY_COMPLETE)
            {
               ...
               if(DeviceInfo->BASClientInfo[InstanceID].Battery_Level_Client_Configuration)
               {
                  ret_val = GATT_Read_Value_Request(BluetoothStackID, ConnectionID, DeviceInfo->BASClientInfo[InstanceID].Battery_Level_Client_Configuration, GATT_ClientEventCallback_HIDS, DeviceInfo->BASClientInfo[InstanceID].Battery_Level_Client_Configuration);
               }
               else
                  printf("   Error - Battery Level Client Configuration not found on this device.\r\n");
               ...
               {
                  /* CC Read failed, check to see if it was from a call */
                  /* to SetNotification                                 */
                  if(ret_val < 0)
                  {
                     DisplayFunctionError("GATT_Read_Value_Request", ret_val);
...
static int GetBatteryLevel(ParameterList_t *TempParam)
{
      ...
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            if(DeviceInfo->BASClientInfo[InstanceID].Battery_Level != 0)
            {
               ...
               if((ret_val = GATT_Read_Value_Request(BluetoothStackID, ConnectionID, DeviceInfo->BASClientInfo[InstanceID].Battery_Level, GATT_ClientEventCallback_BAP, DeviceInfo->BASClientInfo[InstanceID].Battery_Level)) > 0)
               {
                  printf("GetBatteryLevel Request sent, Transaction ID = %u\r\n", ret_val);
         ...
         printf("A BAP client must be connected.\r\n");
   }
   ...
   /* client with an open connection to a remote server. If executed as */
   /* a client, a GATT read request will be generated, and the results  */
...
static int GetBatteryLevelPresentationFormat(ParameterList_t *TempParam)
{
   ...
   if(ret_val != INVALID_PARAMETERS_ERROR)
   {
      ...
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            {
               /* Finally, submit a read request to the server          */
               if((ret_val = GATT_Read_Value_Request(BluetoothStackID, ConnectionID, DeviceInfo->BASClientInfo[InstanceID].Battery_Level_Presentation_Format, GATT_ClientEventCallback_BAP, DeviceInfo->BASClientInfo[InstanceID].Battery_Level_Presentation_Format)) > 0)
               {
                  printf("Battery_Level_Presentation_Format Request sent, Transaction ID = %u\r\n", ret_val);
         ...
         printf("Either a BAP server must be registered or a BAP client must be connected.\r\n");
   }
   ...
   /* been registered for the specified GAP LE Action that is           */
   /* associated with the Bluetooth Stack. This function passes to the  */
   /* caller the GAP LE Event Data of the specified Event and the GAP   */
   /* LE Event Callback Parameter that was specified when this Callback */
   /* was installed. The caller is free to use the contents of the GAP  */
   /* LE Event Data ONLY in the context of this callback. If the caller */
   ...
   if((BluetoothStackID) && (GAP_LE_Event_Data))
   {
      switch(GAP_LE_Event_Data->Event_Data_Type)
      {
         ...
         case etLE_Disconnection_Complete:
            ...
            /* Cleanup UHID if it was initialized.                      */
            if(UHIDInitialized)
            {
               if(UHID_Cleanup())
                  printf("UHID_Cleanup success.\r\n");
               else
                  printf("UHID_Cleanup failed.\r\n");
            ...
            if((Authentication_Event_Data = GAP_LE_Event_Data->Event_Data.GAP_LE_Authentication_Event_Data) != NULL)
            {
               ...
               switch(Authentication_Event_Data->GAP_LE_Authentication_Event_Type)
               {
                  ...
                  case latSecurityRequest:
                     ...
                     /* * NOTE * This is only sent from Slave to        */
                     /*          Master. Thus we must be the Master in  */
                     /*          this connection.                       */
            ...
            if(GATT_Client_Event_Data->Event_Data.GATT_Request_Error_Data)
            {
               ...
               if(((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, GATT_Client_Event_Data->Event_Data.GATT_Request_Error_Data->RemoteDevice)) != NULL) && (CallbackParameter != 0))
               {
                  ...
                  if(ClientInfo)
                  {
                     ...
                     if((CallbackParameter != ClientInfo->ReportMapHandle) || ((CallbackParameter == ClientInfo->ReportMapHandle) && (GATT_Client_Event_Data->Event_Data.GATT_Request_Error_Data->ErrorCode != ATT_PROTOCOL_ERROR_CODE_INVALID_OFFSET)))
                     {
                        printf("\r\nError Response.\r\n");
                     ...
                     if(CallbackParameter == ClientInfo->ReportMapHandle)
                     {
                        ...
                        if((ClientInfo->ReportMap) && (ClientInfo->ReportMapLength))
                        {
                           ...
                           for(Index = 0; Index < (ClientInfo->ReportMapLength-1); Index++)
                              printf("0x%02x, ", ClientInfo->ReportMap[Index]);
                        ...
                        if((InstanceIndex < (MAX_SUPPORTED_HID_INSTANCES - 1)) && (DeviceInfo->HIDSClientInfo[InstanceIndex+1].ReportMapHandle))
                        {
                           ...
                           ReadHIDSInfo(&DeviceInfo->HIDSClientInfo[InstanceIndex+1]);
                        }
                        else
                        {
                           /* Flag that no service discovery operation  */
                           /* is outstanding for this device.           */
                           DeviceInfo->Flags &= ~DEVICE_INFO_FLAGS_SERVICE_DISCOVERY_OUTSTANDING;

                           /* Flag that reading HIDS info has been      */
                           /* performed on for this connection.         */
                           DeviceInfo->Flags |= DEVICE_INFO_FLAGS_HID_SERVICE_DISCOVERY_COMPLETE;
                        }
                     }
                     else
                     {
                        ...
                        if(CallbackParameter == ClientInfo->BootKeyboardInputReport_CCCD)
                        {
                           ...
                           /* Show the prompt if there are no more      */
                           /* callbacks.                                */
                           if(ClientInfo->BootMouseInputReport_CCCD || ClientInfo->Reports[0].CCCDHandle)
                              ShowPrompt = FALSE;
                        }
                        else
                        {
                           ...
                           if(CallbackParameter == ClientInfo->BootMouseInputReport_CCCD)
                           {
                              ...
                              /* Show the prompt if there are no more   */
                              /* callbacks.                             */
                              if(ClientInfo->Reports[0].CCCDHandle)
                                 ShowPrompt = FALSE;
                           }
                           else
                           {
                              ...
                              for(Index=0; Index < HIDS_MAXIMUM_NUMBER_EXTERNAL_REPORT_REFERENCES; Index++)
                              {
                                 ...
                                 {
                                    printf("Read HIDS External Report Reference Data failure.\r\n");
                                    ShowPrompt = FALSE;
                              ...
                              for(Index=0; Index < HIDS_MAXIMUM_NUMBER_REPORTS; Index++)
                              {
                                 ...
                                 if(CallbackParameter == ClientInfo->Reports[Index].CCCDHandle)
                                 {
                                    ...
                                    /* Show the prompt if there are no  */
                                    /* more callbacks.                  */
                                    ShowPrompt = TRUE;

                                    /* Loop through the rest of the     */
                                    /* reports and see if we are        */
                                    /* expecting another callback.      */
                                    for(Index2=Index+1; Index2 < HIDS_MAXIMUM_NUMBER_REPORTS; Index2++)
                                    {
                                       if(ClientInfo->Reports[Index2].CCCDHandle)
                                          ShowPrompt = FALSE;
                                    }
                                    break;
                                 }
                  ...
                  if(ClientInfo)
                  {
                     ...
                     if(CallbackParameter == ClientInfo->ReportMapHandle)
                     {
                        ...
                        if(AttributeValueLength != 0)
                        {
                           ...
                           if(ClientInfo->ReportMap)
                           {
                              ...
                              if((ReportMap = BTPS_AllocateMemory(AttributeValueLength + ClientInfo->ReportMapLength)) != NULL)
                              {
                                 /* Copy the old data into the buffer.  */
                                 BTPS_MemCopy(ReportMap, ClientInfo->ReportMap, ClientInfo->ReportMapLength);

                                 /* Append the new data into the        */
                                 /* buffer.                             */
                                 BTPS_MemCopy((ReportMap + ClientInfo->ReportMapLength), AttributeValue, AttributeValueLength);

                                 /* Free the old data and increment the */
                                 /* report map length.                  */
                                 BTPS_FreeMemory(ClientInfo->ReportMap);
                                 ClientInfo->ReportMapLength += AttributeValueLength;

                                 /* Allocate the static buffer to the   */
                                 /* new size to hold the data we just   */
                                 /* copied.                             */
                                 if((ClientInfo->ReportMap = BTPS_AllocateMemory(ClientInfo->ReportMapLength)) != NULL)
                                 {
                                    /* Copy the old and new data into   */
                                    /* the static buffer.               */
                                    BTPS_MemCopy(ClientInfo->ReportMap, ReportMap, ClientInfo->ReportMapLength);

                                    /* Try to read the next blob of     */
                                    /* report map data.                 */
                                    Result = GATT_Read_Long_Value_Request(BluetoothStackID, ConnectionID, ClientInfo->ReportMapHandle, ClientInfo->ReportMapLength, GATT_ClientEventCallback_HIDS, ClientInfo->ReportMapHandle);
                                 }
                                 else
                                 {
                                    printf("Read report map failure: Insufficient Buffer Space.\r\n");
                                 }

                                 /* Free the local buffer.              */
                                 BTPS_FreeMemory(ReportMap);
                              }
                              else
                                 printf("Read report map failure: Insufficient Buffer Space.\r\n");
                           }
                        ...
                        else
                        {
                           ...
                           if(ClientInfo->ReportMapLength)
                           {
                              ...
                              {
                                 /* Flag that no service discovery      */
                                 /* operation is outstanding for this   */
                                 /* device.                             */
                                 DeviceInfo->Flags &= ~DEVICE_INFO_FLAGS_SERVICE_DISCOVERY_OUTSTANDING;

                                 /* Flag that reading HIDS info has     */
                                 /* been performed on for this          */
                                 /* connection.                         */
                                 DeviceInfo->Flags |= DEVICE_INFO_FLAGS_HID_SERVICE_DISCOVERY_COMPLETE;
                              }
                           }
                     ...
                     if((DeviceInfo == NULL) || (CallbackParameter == 0))
                     {
                        ...
                        for(Index = 0; Index < (AttributeValueLength - 1); Index++)
                           printf("0x%02x, ", AttributeValue[Index]);
                  ...
                  if(((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data->RemoteDevice)) != NULL) && (CallbackParameter != 0))
                  {
                     ...
                     if(ClientInfo)
                     {
                        ...
                        if(CallbackParameter == ClientInfo->HIDSInformationHandle)
                        {
                           ...
                           if(AttributeValueLength == HIDS_HID_INFORMATION_SIZE)
                           {
                              /* Decode the HID Information value.      */
                              if((Result = HIDS_Decode_HID_Information(AttributeValueLength, AttributeValue, &ClientInfo->HIDSInformation)) == 0)
                              {
                                 /* Print out the information.          */
                                 printf("\r\nHIDS Information:\r\n");
                                 printf("   Version:              %d\r\n", ClientInfo->HIDSInformation.Version);
                                 printf("   Country Code:         %d\r\n", ClientInfo->HIDSInformation.CountryCode);
                                 printf("   Remote Wake:          %s\r\n", ((ClientInfo->HIDSInformation.Flags & HIDS_HID_INFORMATION_FLAGS_REMOTE_WAKE) ? "Supported" : "Not Supported"));
                                 printf("   Normally Connectable: %s\r\n", ((ClientInfo->HIDSInformation.Flags & HIDS_HID_INFORMATION_FLAGS_REMOTE_WAKE) ? "Yes" : "No"));

                                 /* Show the prompt if there are no     */
                                 /* reports being read after this.      */
                                 if(ClientInfo->Reports[0].Handle)
                                    ShowPrompt = FALSE;
                                 else
                                    ShowPrompt = TRUE;
                              }
                        ...
                        else
                        {
                           ...
                           if(CallbackParameter == ClientInfo->ReportMapHandle)
                           {
                              ...
                              if((ClientInfo->ReportMap = BTPS_AllocateMemory(AttributeValueLength)) != NULL)
                              {
                                 ClientInfo->ReportMapLength = AttributeValueLength;
                                 BTPS_MemCopy(ClientInfo->ReportMap, AttributeValue, ClientInfo->ReportMapLength);
                           ...
                           else
                           {
                              ...
                              {
                                 if(AttributeValueLength == HIDS_PROTOCOL_MODE_VALUE_LENGTH)
                                 {
                              ...
                              else
                              {
                                 ...
                                 for(Index=0; Index < HIDS_MAXIMUM_NUMBER_REPORTS; Index++)
                                 {
                                    if(CallbackParameter == ClientInfo->Reports[Index].ReportReferenceHandle)
                                    {
                                       ...
                                       if((Result = HIDS_Decode_Report_Reference(AttributeValueLength, AttributeValue, &ClientInfo->Reports[Index].ReportReferenceData)) == 0)
                                       {
                                          ...
                                          switch(ClientInfo->Reports[Index].ReportReferenceData.ReportType)
                                          {
                                             case HIDS_REPORT_REFERENCE_REPORT_TYPE_INPUT_REPORT:
                                                printf("   Report Type:          Input Report\r\n");
                                                break;
                                             case HIDS_REPORT_REFERENCE_REPORT_TYPE_OUTPUT_REPORT:
                                                printf("   Report Type:          Output Report\r\n");
                                                break;
                                             case HIDS_REPORT_REFERENCE_REPORT_TYPE_FEATURE_REPORT:
                                                printf("   Report Type:          Feature Report\r\n");
                                                break;
                                             default:
                                                printf("   Report Type:          Unknown.\r\n");
                                                break;
                                          }
                                 ...
                                 for(Index=0; Index < HIDS_MAXIMUM_NUMBER_EXTERNAL_REPORT_REFERENCES; Index++)
                                 {
                                    if(CallbackParameter == ClientInfo->ExternalReportReferences[Index].ExternalReportReferenceHandle)
                                    {
                                       ...
                                       /* reference.                    */
                                       if((Result = HIDS_Decode_External_Report_Reference(AttributeValueLength, AttributeValue, &ClientInfo->ExternalReportReferences[Index].ExternalReportReference)) == 0)
                                       {
                                          ...
                                          {
                                             case guUUID_16:
                                                printf("   UUID Type:            UUID 16\r\n");
                                                break;
                                             case guUUID_128:
                                                printf("   UUID Type:            UUID 128\r\n");
                                                break;
                                             default:
                                                break;
                                          }
                        ...
                        else
                        {
                           ...
                           for(Index=0; Index < HIDS_MAXIMUM_NUMBER_REPORTS; Index++)
                           {
                              if(CallbackParameter == ClientInfo->Reports[Index].CCCDHandle)
                              {
                                 ...
                                 /* Loop through the rest of the        */
                                 /* reports and see if we are expecting */
                                 /* another callback.                   */
                                 for(Index2=Index+1; Index2 < HIDS_MAXIMUM_NUMBER_REPORTS; Index2++)
                                 {
                                    if(ClientInfo->Reports[Index2].CCCDHandle)
...
static void BTPSAPI GATT_Connection_Event_Callback(unsigned int BluetoothStackID, GATT_Connection_Event_Data_t *GATT_Connection_Event_Data, unsigned long CallbackParameter)
{
      ...
      switch(GATT_Connection_Event_Data->Event_Data_Type)
      {
            ...
            if(GATT_Connection_Event_Data->Event_Data.GATT_Device_Connection_Data)
            {
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, GATT_Connection_Event_Data->Event_Data.GATT_Device_Connection_Data->RemoteDevice)) != NULL)
               {
                  if(LocalDeviceIsMaster)
                  {
                     ...
                     /* supported.                                      */
                     GATT_Exchange_MTU_Request(BluetoothStackID, ConnectionID, BTPS_CONFIGURATION_GATT_DEFAULT_MAXIMUM_SUPPORTED_MTU_SIZE, GATT_ClientEventCallback_HIDS, 0);
                  }
               }
            }
            else
         ...
         case etGATT_Connection_Server_Notification:
            if(GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data)
            {
               ...
               printf("   Report Data:      ");
               for(Index1=0; Index1 < GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data->AttributeValueLength; Index1++)
               {
                  printf("0x%02x ", GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data->AttributeValue[Index1]);
               }
               printf("\r\n");
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data->RemoteDevice)) != NULL)
               {
                  ...
                  for(Index1=0; Index1 < MAX_SUPPORTED_HID_INSTANCES; Index1++)
                  {
                     ...
                     for(Index2=0; Index2 < HIDS_MAXIMUM_NUMBER_REPORTS; Index2++)
                     {
                        ...
                        {
                           if(DeviceInfo->HIDSClientInfo[Index1].Reports[Index2].ReportReferenceData.ReportType == HIDS_REPORT_REFERENCE_REPORT_TYPE_INPUT_REPORT)
                           {
                              /* Initialize the UHID data.              */
                              CommandData.EventType                      = UHID_INPUT;

                              /* Get the Report ID.                     */
                              ReportID = DeviceInfo->HIDSClientInfo[Index1].Reports[Index2].ReportReferenceData.ReportID;

                              /* If the ReportID exists (non-zero),     */
                              /* prepend it to the report data.         */
                              if(ReportID)
                              {
                                 /* Get the report length and set the 	*/
                                 /* data size.                          */
                                 ReportLength = GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data->AttributeValueLength + NON_ALIGNED_BYTE_SIZE;
                                 CommandData.EventData.InputReportData.Size = (ReportLength>UHID_DATA_MAX)?UHID_DATA_MAX:ReportLength;

                                 /* Copy the ReportID and Report Data   */
                                 /* into the Command Data buffer.       */
                                 BTPS_MemCopy(CommandData.EventData.InputReportData.Data, &ReportID, NON_ALIGNED_BYTE_SIZE);
                                 BTPS_MemCopy(&CommandData.EventData.InputReportData.Data[NON_ALIGNED_BYTE_SIZE], GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data->AttributeValue, CommandData.EventData.InputReportData.Size);
                              }
                              else
                              {
                                 /* Get the report length and set the 	*/
                                 /* data size.                          */
                                 ReportLength = GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data->AttributeValueLength;
                                 CommandData.EventData.InputReportData.Size = (ReportLength>UHID_DATA_MAX)?UHID_DATA_MAX:ReportLength;

                                 /* Copy the Report Data into the       */
                                 /* Command Data buffer.                */
                                 BTPS_MemCopy(&CommandData.EventData.InputReportData.Data, GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data->AttributeValue, CommandData.EventData.InputReportData.Size);
                              }

                              /* Write the device if initialized.       */
                              if(UHIDInitialized)
                              {
                                 if(UHID_Write_Device(&CommandData))
                                    printf("UHID_Write_Device() success.\r\n");
                                 else
                                    printf("UHID_Write_Device() failed.\r\n");
                              }
                           }
                        }
                     }
   ...
   /* This function will be called whenever a GATT Service is           */
   /* discovered or a previously started service discovery process is   */
   /* completed. This function passes to the caller the GATT Discovery  */
   /* Event Data that occurred and the GATT Client Event Callback       */
   /* Parameter that was specified when this Callback was installed.    */
   /* The caller is free to use the contents of the GATT Discovery      */
   ...
   /* Thread Context of a Thread that the User does NOT own. Therefore, */
   /* processing in this function should be as efficient as possible    */
   /* (this argument holds anyway because another GATT Discovery Event  */
         ...
         switch(GATT_Service_Discovery_Event_Data->Event_Data_Type)
         {
            case etGATT_Service_Discovery_Indication:
               ...
               if(GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data)
               {
                  ...
                  /* Check the service discovery type for HIDS          */
                  if(((Service_Discovery_Type_t)CallbackParameter) == sdHIDS)
                  {
                     /* Attempt to populate the handles for the HID     */
                     /* Service.                                        */
                     DeviceInfo->HIDSClientInfo[HIDSInstanceIDIndex].ServiceHandleRange.Starting_Handle = GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data->ServiceInformation.Service_Handle;
                     DeviceInfo->HIDSClientInfo[HIDSInstanceIDIndex].ServiceHandleRange.Ending_Handle   = GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data->ServiceInformation.End_Group_Handle;
                  ...
                  if(((Service_Discovery_Type_t)CallbackParameter) == sdDIS)
                  {
                     /* Attempt to populate the handles for the HID     */
                     /* Service.                                        */
                     DeviceInfo->Flags |= DEVICE_INFO_FLAGS_DIS_SERVICE_FOUND;
                  ...
                  if(((Service_Discovery_Type_t)CallbackParameter) == sdBAS)
                  {
                     ...
                     /* Service.                                        */
                     BASInstanceIDIndex = 0;
                     BASPopulateHandles(&DeviceInfo->BASClientInfo[BASInstanceIDIndex], GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data);
                  }
               }
               break;
               ...
               if(GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Complete_Data)
               {
                  ...
                  switch((Service_Discovery_Type_t)CallbackParameter)
                  {
                     case sdHIDS:
                        ...
                        /* found                                        */
                        for(InstanceID=1;InstanceID<=MAX_SUPPORTED_HID_INSTANCES;++InstanceID)
                        {
                           if(DeviceInfo->HIDSClientInfo[InstanceID-1].ReportMapHandle)
                           {
                              printf("\r\nHIDS Service Discovery Summary Instance ID %u\r\n", InstanceID);

                              for(Index=0; Index < HIDS_MAXIMUM_NUMBER_REPORTS; Index++)
                              {
                                 if(!(DeviceInfo->HIDSClientInfo[InstanceID-1].Reports[Index].Handle))
                                 {
                                    printf("   Number of Supported Reports:   %d\r\n", Index);
                                    break;
                                 }
                              }
                              printf("   Protocol Mode:                 %s\r\n", (DeviceInfo->HIDSClientInfo[InstanceID-1].ProtocolModeHandle       ? "Supported" : "Not Supported"));
                              printf("   Boot Keyboard Input Report:    %s\r\n", (DeviceInfo->HIDSClientInfo[InstanceID-1].BootKeyboardInputReport  ? "Supported" : "Not Supported"));
                              printf("   Boot Keyboard Output Report:   %s\r\n", (DeviceInfo->HIDSClientInfo[InstanceID-1].BootKeyboardOutputReport ? "Supported" : "Not Supported"));
                              printf("   Boot Mouse Input Report:       %s\r\n", (DeviceInfo->HIDSClientInfo[InstanceID-1].BootMouseInputReport     ? "Supported" : "Not Supported"));
                              printf("   Control Point:                 %s\r\n", (DeviceInfo->HIDSClientInfo[InstanceID-1].ControlPoint             ? "Supported" : "Not Supported"));

                              ShowPrompt = FALSE;
                           }
                        }

                        /* Read the information for the first service.  */
                        if(DeviceInfo->HIDSClientInfo[0].ReportMapHandle)
                        {
                           printf("\r\nAttempting to read HIDS information (Instance ID 1)\r\n");
                           ReadHIDSInfo(&DeviceInfo->HIDSClientInfo[0]);
                        }
                     ...
                     case sdDIS:
                        ...
                        if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_DIS_SERVICE_FOUND)
                        {
                           /* Print a summary of what descriptor were   */
                           /* found                                     */
                           ...
                           printf("     PnP ID                        : %s\r\n", (DeviceInfo->DISClientInfo.PNPID                       ? "Supported" : "Not Supported"));

                           if(ReadDISInfo(&DeviceInfo->DISClientInfo))
                              ShowPrompt = FALSE;
                           else
                              ShowPrompt = TRUE;
                        }
                     ...
                     case sdBAS:
                        /* Flag that service discovery has been         */
                        /* performed on for this connection.            */
                        DeviceInfo->Flags |= DEVICE_INFO_FLAGS_BAS_SERVICE_DISCOVERY_COMPLETE;

                        /* Print a summary of what descriptor were      */
                        /* found                                        */
                        for(InstanceID=1;InstanceID<=MAX_SUPPORTED_BATTERY_INSTANCES;++InstanceID)
                        {
                           if(DeviceInfo->BASClientInfo[InstanceID-1].Battery_Level)
                           {
                              printf("\r\nBAP Service Discovery Summary Instance ID %u\r\n", InstanceID);
                              printf("   Battery Level:                                %s\r\n", (DeviceInfo->BASClientInfo[InstanceID-1].Battery_Level ?                      "Supported" : "Not Supported"));
                              printf("   Battery Level CC:                             %s\r\n", (DeviceInfo->BASClientInfo[InstanceID-1].Battery_Level_Client_Configuration ? "Supported" : "Not Supported"));
                              printf("   Battery Level Presentation Format Descriptor: %s\r\n", (DeviceInfo->BASClientInfo[InstanceID-1].Battery_Level_Presentation_Format ?  "Supported" : "Not Supported"));
                           }
                        }
                        break;
                     default:
...
static void UHID_Event_Callback(BD_ADDR_t DeviceAddr, UHID_Event_Data_t *EventData, void *CallbackParameter)
{
   ...
   if((!COMPARE_NULL_BD_ADDR(DeviceAddr)) && (EventData))
   {
      switch(EventData->EventType)
      {
         case UHID_OUTPUT:
            /* Write the report to the requested device.                */
            printf("UHID Output Callback.\r\n");
            break;
         case UHID_FEATURE:
            /* Attempt to perform a Get Report for the requested        */
            /* Feature Report.                                          */
            printf("UHID Feature Callback.\r\n");
            break;
         default:
            break;
      }
...
{
   DeviceInfo_t      *DeviceInfo;
   DIS_Client_Info_t *ClientInfo;
   DIS_PNP_ID_Data_t  PNP_ID_Data;
   BoardStr_t         BoardStr;
   Word_t             Index;
   int                Result;
   Boolean_t          PrintData  = FALSE;
   Boolean_t          ShowPrompt = TRUE;
      ...
      switch(GATT_Client_Event_Data->Event_Data_Type)
      {
         case etGATT_Client_Error_Response:
            if(GATT_Client_Event_Data->Event_Data.GATT_Request_Error_Data)
            {
               ...
               if(((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, GATT_Client_Event_Data->Event_Data.GATT_Request_Error_Data->RemoteDevice)) != NULL) && (CallbackParameter != 0))
               {
                  /* Get the DIS Client Info.                           */
                  ClientInfo = &DeviceInfo->DISClientInfo;

                  /* Handle the PnP ID callback.                        */
                  if(CallbackParameter == ClientInfo->PNPID)
                  {
                     printf("\r\nRead DIS PnP ID failure.\r\n");

                     /* Show the prompt, this is the last value read.   */
                     ShowPrompt = TRUE;
                  }
                  else
                  {
                     /* Handle the Manufacturer Name callback.          */
                     if(CallbackParameter == ClientInfo->ManufacturerName)
                     {
                        printf("\r\nRead DIS Manufacturer Name failure.\r\n   ");

                        /* Show the prompt if this is the last value    */
                        /* read.                                        */
                        if(!(ClientInfo->ModelNumber || ClientInfo->SerialNumber || ClientInfo->HardwareRevision || ClientInfo->FirmwareRevision || ClientInfo->SoftwareRevision || ClientInfo->SystemID || ClientInfo->IEEECertificationDataList || ClientInfo->PNPID))
                           ShowPrompt = TRUE;
                     }
                     else
                     {
                        /* Handle the Model Number callback.            */
                        if(CallbackParameter == ClientInfo->ModelNumber)
                        {
                           printf("\r\nRead DIS Model Number failure.\r\n   ");

                           /* Show the prompt if this is the last value */
                           /* read.                                     */
                           if(!(ClientInfo->SerialNumber || ClientInfo->HardwareRevision || ClientInfo->FirmwareRevision || ClientInfo->SoftwareRevision || ClientInfo->SystemID || ClientInfo->IEEECertificationDataList || ClientInfo->PNPID))
                              ShowPrompt = TRUE;
                        }
                        else
                        {
                           /* Handle the Serial Number callback.        */
                           if(CallbackParameter == ClientInfo->SerialNumber)
                           {
                              printf("\r\nRead DIS Serial Number failure.\r\n   ");

                              /* Show the prompt if this is the last    */
                              /* value read.                            */
                              if(!(ClientInfo->HardwareRevision || ClientInfo->FirmwareRevision || ClientInfo->SoftwareRevision || ClientInfo->SystemID || ClientInfo->IEEECertificationDataList || ClientInfo->PNPID))
                                 ShowPrompt = TRUE;
                           }
                           else
                           {
                              /* Handle the Hardware Revision callback. */
                              if(CallbackParameter == ClientInfo->HardwareRevision)
                              {
                                 printf("\r\nRead DIS Hardware Revision failure.\r\n   ");

                                 /* Show the prompt if this is the last */
                                 /* value read.                         */
                                 if(!(ClientInfo->FirmwareRevision || ClientInfo->SoftwareRevision || ClientInfo->SystemID || ClientInfo->IEEECertificationDataList || ClientInfo->PNPID))
                                    ShowPrompt = TRUE;
                              }
                              else
                              {
                                 /* Handle the Firmware Revision        */
                                 /* callback.                           */
                                 if(CallbackParameter == ClientInfo->FirmwareRevision)
                                 {
                                    printf("\r\nRead DIS Firmware Revision failure.\r\n   ");

                                    /* Show the prompt if this is the   */
                                    /* last value read.                 */
                                    if(!(ClientInfo->SoftwareRevision || ClientInfo->SystemID || ClientInfo->IEEECertificationDataList || ClientInfo->PNPID))
                                       ShowPrompt = TRUE;
                                 }
                                 else
                                 {
                                    /* Handle the Software Revision     */
                                    /* callback.                        */
                                    if(CallbackParameter == ClientInfo->SoftwareRevision)
                                    {
                                       printf("\r\nRead DIS Software Revision failure.\r\n   ");

                                       /* Show the prompt if this is    */
                                       /* the last value read.          */
                                       if(!(ClientInfo->SystemID || ClientInfo->IEEECertificationDataList || ClientInfo->PNPID))
                                          ShowPrompt = TRUE;
                                    }
                                    else
                                    {
                                       /* Handle the System ID          */
                                       /* callback.                     */
                                       if(CallbackParameter == ClientInfo->SystemID)
                                       {
                                          printf("\r\nRead DIS System ID failure.\r\n   ");

                                          /* Show the prompt if this is */
                                          /* the last value read.       */
                                          if(!(ClientInfo->IEEECertificationDataList || ClientInfo->PNPID))
                                             ShowPrompt = TRUE;
                                       }
                                       else
                                       {
                                          ...
                                          /* callback.                  */
                                          if(CallbackParameter == ClientInfo->IEEECertificationDataList)
                                          {
                                             printf("\r\nRead DIS IEEE Certification Data List failure.\r\n   ");

                                             /* Show the prompt if this */
                                             /* is the last value read. */
                                             if(!(ClientInfo->PNPID))
                                                ShowPrompt = TRUE;
                                          }
                                       }
         ...
         case etGATT_Client_Read_Response:
            if(GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data)
            {
               ...
               if(((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data->RemoteDevice)) != NULL) && (CallbackParameter != 0))
               {
                  ...
                  /* Handle the PnP ID callback.                        */
                  if(CallbackParameter == DeviceInfo->DISClientInfo.PNPID)
                  {
                     /* Check the value length.                         */
                     if(GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data->AttributeValueLength == DIS_PNP_ID_VALUE_DATA_SIZE)
                     {
                        /* Decode the data.                             */
                        if((Result = DIS_Decode_PNP_ID(GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data->AttributeValueLength, GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data->AttributeValue, &PNP_ID_Data)) == 0)
                        {
                           /* Print the data.                           */
                           printf("\r\nDIS PnP Data:\r\n");
                           switch(PNP_ID_Data.VendorID_Source)
                           {
                              case DIS_PNP_ID_VENDOR_SOURCE_BLUETOOTH_SIG:
                                 printf("   Vendor ID Source:     Bluetooth SIG\r\n");
                                 break;
                              case DIS_PNP_ID_VENDOR_SOURCE_USB_IMPLEMENTORS_FORUM:
                                 printf("   Vendor ID Source:     USB Implementor's Forum\r\n");
                                 break;
                              default:
                                 printf("   Vendor ID Source:     Unknown\r\n");
                                 break;
                           }
                           printf("   Vendor ID:            %d\r\n", PNP_ID_Data.VendorID);
                           printf("   Product ID:           %d\r\n", PNP_ID_Data.ProductID);
                           printf("   Product Version:      %d\r\n", PNP_ID_Data.ProductVersion);

                           for(Index = 0; Index < MAX_SUPPORTED_HID_INSTANCES; Index++)
                           {
                              DeviceInfo->HIDSClientInfo[Index].PNP_ID_Data = PNP_ID_Data;
                           }
                        }
                        else
                           DisplayFunctionError("DIS_Decode_PNP_ID", Result);
                     }
                     else
                        printf("\r\nError - Invalid length (%u) for DIS PnP ID response.\r\n", GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data->AttributeValueLength);
                  ...
                  {
                     /* Handle the Manufacturer Name callback.          */
                     if(CallbackParameter == DeviceInfo->DISClientInfo.ManufacturerName)
                     {
                     ...
                     {
                        /* Handle the Model Number callback.            */
                        if(CallbackParameter == DeviceInfo->DISClientInfo.ModelNumber)
                        {
                           printf("\r\nDIS Model Number:\r\n   ");
                           PrintData = TRUE;
                        ...
                        {
                           /* Handle the Serial Number callback.        */
                           if(CallbackParameter == DeviceInfo->DISClientInfo.SerialNumber)
                           {
                              printf("\r\nDIS Serial Number:\r\n   ");
                              PrintData = TRUE;
                           ...
                           {
                              /* Handle the Hardware Revision callback. */
                              if(CallbackParameter == DeviceInfo->DISClientInfo.HardwareRevision)
                              {
                              ...
                              else
                              {
                                 /* Handle the Firmware Revision        */
                                 /* callback.                           */
                                 if(CallbackParameter == DeviceInfo->DISClientInfo.FirmwareRevision)
                                 {
                                    printf("\r\nDIS Firmware Revision:\r\n   ");
                                    PrintData = TRUE;
                                 ...
                                 else
                                 {
                                    /* Handle the Software Revision     */
                                    /* callback.                        */
                                    if(CallbackParameter == DeviceInfo->DISClientInfo.SoftwareRevision)
                                    {
                                       printf("\r\nDIS Software Revision:\r\n   ");
                                       PrintData = TRUE;
                                    ...
                                    else
                                    {
                                       /* Handle the System ID          */
                                       /* callback.                     */
                                       if(CallbackParameter == DeviceInfo->DISClientInfo.SystemID)
                                       {
                                          printf("\r\nDIS System ID:\r\n");

                                          /* Check the value length.    */
                                          if(GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data->AttributeValueLength == DIS_SYSTEM_ID_VALUE_DATA_SIZE)
                                          {
                                             /* Print the manufacturer  */
                                             /* identifier data.        */
                                             printf("   Manufacturer Identifier:                0x");
                                             for(Index = 0; Index < sizeof(((DIS_System_ID_Value_t *)0)->Manufacturer_Identifier); Index++)
                                             {
                                                printf("%02x", GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data->AttributeValue[STRUCTURE_OFFSET(DIS_System_ID_Value_t, Manufacturer_Identifier) + Index]);
                                             }

                                             /* Print the               */
                                             /* organizationally unique */
                                             /* identifier data.        */
                                             printf("\r\n   Organizationally Unique Identifier:     0x");

                                             for(Index = 0; Index < sizeof(((DIS_System_ID_Value_t *)0)->Organizationally_Unique_Identifier); Index++)
                                                printf("%02x", GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data->AttributeValue[STRUCTURE_OFFSET(DIS_System_ID_Value_t, Organizationally_Unique_Identifier) + Index]);
                                       ...
                                       else
                                       {
                                          ...
                                          /* Certification Data List    */
                                          /* callback.                  */
                                          if(CallbackParameter == DeviceInfo->DISClientInfo.IEEECertificationDataList)
                                          {
                                             printf("\r\nDIS IEEE Certification Data List:\r\n");
                                             printf("   Length:     %d", GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data->AttributeValueLength);
                                             ...
                                             if(GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data->AttributeValueLength)
                                             {
                                                ...
                                                /* Print the data.      */
                                                for(Index = 0; Index < GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data->AttributeValueLength; Index++)
                                                   printf("%02x", GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data->AttributeValue[Index]);
                                             }
                  ...
                  /* Print the callback data as a string of characters. */
                  if(PrintData)
                  {
                     /* Print the string if it exists, otherwise        */
                     /* indicate the string is null.                    */
                     if(GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data->AttributeValueLength > 0)
                     {
                        /* Loop through the byte array..                */
                        for(Index = 0; Index < GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data->AttributeValueLength; Index++)
                           printf("%c", GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data->AttributeValue[Index]);
                     }
                     else
                        printf("NULL");
               ...
               /* If the data has not been decoded and displayed, then  */
         ...
         case etGATT_Client_Error_Response:
            if(GATT_Client_Event_Data->Event_Data.GATT_Request_Error_Data)
            {
               ...
               if(((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, GATT_Client_Event_Data->Event_Data.GATT_Request_Error_Data->RemoteDevice)) != NULL) && (CallbackParameter != 0))
               {
                  ...
                  {
                     printf("\r\nWrite Battery Level CC failure.\r\n");
                  }
               }
            }
            else

...
/*   03/18/15  T. Cook        Changes for BT 4.2                              */
/******************************************************************************/
#ifndef __GAPAPIH__
#define __GAPAPIH__
...
#define GAP_LEGACY_IO_CAPABILITIES_SIZE                           (BTPS_STRUCTURE_OFFSET(GAP_IO_Capabilities_t, OOB_256_Data_Present))
   ...
   /* * NOTE * The atRemoteLEPairingRequest is dispatched when the      */
   /*          remote device attempts to start LE Pairing using the     */
   /*          BR/EDR Transport.  A version 4.2 or stack (both locally  */
   /*          and on remote device) is required for this event to be   */
   /*          received.                                                */
typedef enum
{
   atLinkKeyRequest,
...
#define GAP_AUTHENTICATION_EVENT_LINK_KEY_INFO_SIZE               (sizeof(GAP_Authentication_Event_Link_Key_Info_t))
   ...
   /*                                    valid.                         */
   /*                                                                   */
   /*    atRemoteLEPairingRequest      - No Further Data.               */
...
typedef void (BTPSAPI *GAP_Event_Callback_t)(unsigned int BluetoothStackID, GAP_Event_Data_t *GAP_Event_Data, unsigned long CallbackParameter);
   ...
   /* calling the GAP_LE_Perform_Scan() function.                       */
...
#define MAXIMUM_CONNECTION_EVENT_LENGTH                               40959
   ...
   /* The following values represent the minimum, maximum and default   */
   /* Resolvable Private Address Timeout that may be passed to the      */
   /* GAP_LE_Set_Resolvable_Private_Address_Timeout() function.         */
   /* * NOTE * These are specified in seconds.                          */
#define MINIMUM_RESOLVABLE_PRIVATE_ADDRESS_TIMEOUT                        1
#define MAXIMUM_RESOLVABLE_PRIVATE_ADDRESS_TIMEOUT                   0xA1B8
#define DEFAULT_RESOLVABLE_PRIVATE_ADDRESS_TIMEOUT                   0x0384
...
typedef enum
{
   ...
   /* The following enumerated type represents the type of filter policy*/
   /* that may be used for functions that accept a filter policy        */
   /* parameter.                                                        */
   /* * NOTE * The fpNoWhiteListDirectedRPA and fpWhiteListDirectedRPA  */
   /*          types may ONLY be used if the local Bluetooth Controller */
   /*          is a 4.2 controller.  These types also may ONLY be used  */
   /*          with the GAP_LE_Perform_Scan() API and may NOT be used   */
   /*          with any other structures or APIs.                       */
typedef enum
{
   fpNoFilter,
   fpWhiteList,
   fpNoWhiteListDirectedRPA,
   fpWhiteListDirectedRPA
...
typedef enum
{
   ...
   /* The following enumerated type represents the supported Pairability*/
   /* Modes that a Bluetooth device can be set to.  These types are used*/
   /* with the GAP_LE_Set_Pairability_Mode() and the                    */
   /* GAP_LE_Query_Pairability_Mode() functions.                        */
   /* * NOTE * If the lpmPairableMode_EnableExtendedEvents is enabled   */
   /*          then the application must be prepared to handle the      */
   /*          extended events listed below:                            */
   /*             latKeypressNotification                               */
   /*             latExtendedPairingRequest                             */
   /*             latExtendedConfirmationRequest                        */
   /*             latExtendedOutOfBandInformation                       */
typedef enum
{
   lpmNonPairableMode,
   lpmPairableMode,
   lpmPairableMode_EnableExtendedEvents
...
typedef enum
{
   ...
   /* Types.                                                            */
   /* * NOTE * The latPublicIdentity and latRandomIdentity types are    */
   /*          only valid to be used or returned from a v4.2 or greater */
   /*          Bluetooth controller.                                    */
   /* * NOTE * Only the latPublic and latRandom types may be used to API*/
   /*          calls where the local Bluetooth device is a 4.0 or 4.1   */
   /*          version chipset.                                         */
   /* * NOTE * The latPublicIdentity and latRandomIdentity are both     */
   /*          valid to be returned in LE Events dispatched from a 4.2  */
   /*          or greater Bluetooth controller.  These types may also be*/
   /*          used in API calls that take this enumerated type for the */
   /*          Peer's Address Type (NOT for the Local Address Type) if  */
   /*          the local Bluetooth controller is a version 4.2 or       */
   /*          greater controller.                                      */
   /* * NOTE * The latResolvableFallbackPublic and                      */
   /*          latResolvableFallbackPrivate are ONLY valid to be used   */
   /*          for the Local Address Type parameter to the              */
   /*          GAP_LE_Advertising_Enable(), GAP_LE_Create_Connection()  */
   /*          and GAP_LE_Perform_Scan() APIs.                          */
typedef enum
{
   latPublic,
   latRandom,
   latPublicIdentity,
   latRandomIdentity,
   latResolvableFallbackPublic,
   latResolvableFallbackPrivate
   ...
   /* lcmDirectConnectable and represent the Direct Address Type and the*/
   /* Direct Address to attempt to connect to.                          */
   /* * NOTE * The following enumerated types may be used with the      */
   /*          Own_Address_Type member:                                 */
   /*                                                                   */
   /*             latPublic                                             */
   /*             latRandom                                             */
   /*             latResolvableFallbackPublic                           */
   /*             latResolvableFallbackPrivate                          */
   /*                                                                   */
   /*          The last two types (latResolvableFallbackXXX) indicate   */
   /*          that the controller should generate a Resolvable Private */
   /*          Address based on the local IRK from the controller's     */
   /*          resolving list.  If no local IRK is found for the        */
   /*          specified device then the controller will fallback to    */
   /*          using either the Public Address                          */
   /*          (latResolvableFallbackPublic) or using the random address*/
   /*          that has been set via the GAP_LE_Set_Random_Address()    */
   /*          API function (latResolvableFallbackPrivate).             */
   /* * NOTE * If either of the latResolvableFallbackPublic or          */
   /*          latResolvableFallbackPrivate types are specified with    */
   /*          the Own_Address_Type member then the Direct_Address_Type */
   /*          and Direct_Address members MUST specified and must       */
   /*          correspond to the Peer's Address Type and Address that   */
   /*          can be used by the controller to search the resolving    */
   /*          list.                                                    */
   /* * NOTE * The following enumerated types may be used with the      */
   ...
   /*          the Direct_Address member is an identity address (a      */
   ...
   /*          LE Pairing process.                                      */
   /* * NOTE * If the local Bluetooth Controller is a version 4.0 or a  */
   /*          version 4.1 compliant controller then for both the       */
   /*          Own_Address_Type and Direct_Address_Type member's the    */
   /*          only valid enumerated types are the latPublic and        */
   /*          latRandom enumerated types.                              */
...
#define GAP_LE_CONNECTABILITY_PARAMETERS_SIZE                     (sizeof(GAP_LE_Connectability_Parameters_t))
   ...
   /* * NOTE * The following types may NOT be used with the             */
   /*          Scan_Request_Filter and Connect_Request_Filter members of*/
...
#define GAP_LE_CURRENT_CONNECTION_PARAMETERS_SIZE                 (sizeof(GAP_LE_Current_Connection_Parameters_t))

   /* The following structure is a container structure for a pair of Low*/
   /* Energy device addresses.                                          */
typedef struct _tagGAP_LE_Address_Pair_t
{
   GAP_LE_Address_Type_t Local_Address_Type;
   GAP_LE_Address_Type_t Remote_Address_Type;
   BD_ADDR_t             Local_Address;
   BD_ADDR_t             Remote_Address;
} GAP_LE_Address_Pair_t;

#define GAP_LE_ADDRESS_PAIR_SIZE                                  (sizeof(GAP_LE_Address_Pair_t))
   ...
   /*                                                                   */
   /*          The latPublicIdentity and latRandomIdentity indicate that*/
   /*          the Address member is an identity address (a             */
   /*          semi-permanent address that can be used to refer to the  */
   /*          device) that was received from the remote device (along  */
   /*          with the remote device's IRK) during a successful        */
   /*          LE Pairing process.                                      */
...
#define GAP_LE_WHITE_LIST_ENTRY_SIZE                              (sizeof(GAP_LE_White_List_Entry_t))
   ...
   /* The following structure represents the data structure that is the */
   /* structure that represents the data for an individual Resolving    */
   /* List entry that is used with the Resolving List Management        */
   ...
   /*             latRandomIdentity                                     */
   /*                                                                   */
   /*          Note that for this structure latPublic means the same    */
   /*          thing as latPublicIdentity (and latRandom and            */
   /*          latRandomIdentity are equivalent). Both types are allowed*/
   /*          with this structure since the specification is not       */
   /*          consistent when it comes to address types and some legacy*/
   /*          structures will return latPublic/latRandom for the peer's*/
   /*          identity address (see the GAP_LE_Identity_Information_t  */
   /*          structure). So we allow this to be flexible.             */
   /* * NOTE * The Peer_Identity_Address member must be the address     */
   /*          received (along with the Peer_Identity_Address_Type and  */
   /*          the Peer_IRK) from the remote device during a successful */
   /*          LE Pairing Process.                                      */
   /* * NOTE * The APIs that use this structure are only valid to be    */
   /*          called if the local controller is a 4.2 controller.      */
typedef struct _tagGAP_LE_Resolving_List_Entry_t
{
   GAP_LE_Address_Type_t Peer_Identity_Address_Type;
   BD_ADDR_t             Peer_Identity_Address;
   Encryption_Key_t      Peer_IRK;
   Encryption_Key_t      Local_IRK;
} GAP_LE_Resolving_List_Entry_t;

#define GAP_LE_RESOLVING_LIST_ENTRY_SIZE                          (sizeof(GAP_LE_Resolving_List_Entry_t))
...
#define GAP_LE_KEY_DISTRIBUTION_SIZE                              (sizeof(GAP_LE_Key_Distribution_t))
   ...
   /* The following data type represents the structure of the extended  */
   /* Initiator and Responder Key Distribution fields in the            */
   /* GAP_LE_Extended_Pairing_Capabilities_t information.               */
   /* * NOTE * The Link_Key member is a field that indicates the device */
   /*          would like to derive the BR/EDR from the LE LTK.  This is*/
   /*          only valid if the local device supports LE Secure        */
   /*          Connections.                                             */
   /* * NOTE * The Link_Key member is only valid with Dual Mode stacks  */
   /*          and will be ignored in Single Mode configurations.       */
typedef struct _tagGAP_LE_Extended_Key_Distribution_t
{
   Boolean_t Encryption_Key;
   Boolean_t Identification_Key;
   Boolean_t Signing_Key;
   Boolean_t Link_Key;
} GAP_LE_Extended_Key_Distribution_t;

#define GAP_LE_EXTENDED_KEY_DISTRIBUTION_SIZE                     (sizeof(GAP_LE_Extended_Key_Distribution_t))

   /* The following type declaration represents the information that is */
   /* requested in a latPairingRequest of the etLE_Authentication event.*/
   /* * NOTE * The Receiving_Keys structure represents the keys that the*/
   /*          LOCAL device is requesting from the REMOTE device.  The  */
   /*          Sending_Keys structure represents the keys that the LOCAL*/
   /*          device is requesting to send to the REMOTE device.       */
...
#define GAP_LE_PAIRING_CAPABILITIES_SIZE                          (sizeof(GAP_LE_Pairing_Capabilities_t))
   ...
   /* The following type declaration represents the information that is */
   /* requested in a latExtendedPairingRequest of the                   */
   /* etLE_Authentication event when Secure Connections support has been*/
   /* enabled.                                                          */
   /* * NOTE * The Receiving_Keys structure represents the keys that the*/
   /*          LOCAL device is requesting from the REMOTE device.  The  */
   /*          Sending_Keys structure represents the keys that the LOCAL*/
   /*          device is requesting to send to the REMOTE device.       */
   /* * NOTE * The Secure Connections pairability mode must be enabled  */
   /*          by setting the pairability mode to                       */
   /*          lpmPairableMode_SecureConnections via the                */
   /*          GAP_LE_Set_Pairability_Mode() API prior to requesting    */
   /*          Secure Connections pairing via this API.                 */
   /* * NOTE * Secure Connections pairing to multiple devices           */
   /*          SIMULTANEOUSLY is NOT ALLOWED by this module.            */
typedef struct _tagGAP_LE_Extended_Pairing_Capabilities_t
{
   unsigned long                      Flags;
   GAP_LE_IO_Capability_t             IO_Capability;
   GAP_LE_Bonding_Type_t              Bonding_Type;
   Byte_t                             Maximum_Encryption_Key_Size;
   GAP_LE_Extended_Key_Distribution_t Receiving_Keys;
   GAP_LE_Extended_Key_Distribution_t Sending_Keys;
} GAP_LE_Extended_Pairing_Capabilities_t;

#define GAP_LE_EXTENDED_PAIRING_CAPABILITIES_SIZE                          (sizeof(GAP_LE_Extended_Pairing_Capabilities_t))

#define GAP_LE_EXTENDED_PAIRING_CAPABILITIES_FLAGS_KEYPRESS_NOTIFICATIONS  0x00000001
#define GAP_LE_EXTENDED_PAIRING_CAPABILITIES_FLAGS_SECURE_CONNECTIONS      0x00000002
#define GAP_LE_EXTENDED_PAIRING_CAPABILITIES_FLAGS_MITM_REQUESTED          0x00000004
#define GAP_LE_EXTENDED_PAIRING_CAPABILITIES_FLAGS_OOB_DATA_PRESENT        0x00000008
...
#define GAP_LE_OOB_DATA_SIZE                                      (sizeof(GAP_LE_OOB_Data_t))
   ...
   /* The following structure represents the structure of data that is  */
   /* used to specify the Secure Connections Out Of Band Information    */
   /* when pairing (Version 4.2).                                       */
typedef struct _tagGAP_LE_Secure_Connections_OOB_Data_t
{
   unsigned long                     Flags;
   Secure_Connections_Confirmation_t Confirmation;
   Secure_Connections_Randomizer_t   Randomizer;
} GAP_LE_Secure_Connections_OOB_Data_t;

#define GAP_LE_SECURE_CONNECTIONS_OOB_DATA_SIZE                   (sizeof(GAP_LE_Secure_Connections_OOB_Data_t))

   /* The following represent the valid flags that may be set in the    */
   /* Flags member of the GAP_LE_Secure_Connections_OOB_Data_t          */
   /* structure.                                                        */
   /* * NOTE * The                                                      */
   /*                                                                   */
   /*         GAP_LE_SECURE_CONNECTIONS_OOB_DATA_FLAGS_OOB_NOT_RECEIVED */
   /*                                                                   */
   /*          flag indicates that OOB data from the remote device was  */
   /*          NOT received during the OOB transfer.                    */
#define GAP_LE_SECURE_CONNECTIONS_OOB_DATA_FLAGS_OOB_NOT_RECEIVED 0x00000001
...
#define GAP_LE_ENCRYPTION_INFORMATION_DATA_SIZE                   (sizeof(GAP_LE_Encryption_Information_t))
   ...
   /* * NOTE * The following enumerated types may be returned with the  */
   /*          Address_Type member:                                     */
   /*                                                                   */
   /*             latPublic                                             */
   /*             latRandom                                             */
   ...
   /*          Note that for this structure latPublic means the same    */
   /*          thing as latPublicIdentity (and latRandom and            */
   /*          latRandomIdentity are equivalent). For legacy reasons we */
   /*          always return either latPublic/latRandom here even though*/
   /*          the address returned in this structure is an Identity    */
   /*          Address.                                                 */
...
typedef enum
{
   ...
   /*          are valid).                                              */
   /* * NOTE * The Authentication_Data_Length must be set correctly     */
   /*          according to the union member that is valid.             */
   /*          Specifically for larPairingCapabilities and              */
   /*          larOutOfBandData the Authentication_Data_Length field is */
   /*          used to differentiate between whether the                */
   /*          Pairing_Capabilities, Extended_Pairing_Capabilities and  */
   /*          Out_Of_Band_Data, Secure_Connections_OOB_Data structures */
...
typedef struct _tagGAP_LE_Authentication_Response_Information_t
{
   ...
   union
   {
      ...
      GAP_LE_Extended_Pairing_Capabilities_t Extended_Pairing_Capabilities;
      GAP_LE_OOB_Data_t                      Out_Of_Band_Data;
      GAP_LE_Secure_Connections_OOB_Data_t   Secure_Connections_OOB_Data;
      DWord_t                                Passkey;
      Byte_t                                 Error_Code;
...
typedef enum
{
   ...
   etLE_Direct_Advertising_Report,
   etLE_Data_Length_Change
} GAP_LE_Event_Type_t;
...
#define GAP_LE_ADVERTISING_DATA_SIZE                              (sizeof(GAP_LE_Advertising_Data_t))
   ...
   /*             latRandomIdentity                                     */
   /*                                                                   */
   /*          The latPublicIdentity and latRandomIdentity types will   */
   /*          only be returned if the address resolution in the        */
   /*          controller has been enabled via the                      */
   /*          GAP_LE_Set_Address_Resolution_Enable() function.  These  */
   /*          two identity types, when returned, indicate that the     */
   /*          controller has resolved an address received in an        */
   /*          advertising packet to the specified identity address     */
   /*          (that must have been added to the resolving list via the */
   /*          GAP_LE_Add_Device_To_Resolving_List() API function).     */
...
#define GAP_LE_ADVERTISING_REPORT_EVENT_DATA_SIZE                 (sizeof(GAP_LE_Advertising_Report_Event_Data_t))
   ...
   /*          (that must have been added to the resolving list via the */
   /*          GAP_LE_Add_Device_To_Resolving_List() API function).     */
   /* * NOTE * The Direct_Address_Type and Direct_BD_ADDR correspond to */
   /*          the address of the device these advertisements are       */
   /*          directed towards.  NOTE that Direct_Address_Type will    */
   /*          ALWAYS be set to:                                        */
   /*                                                                   */
   /*             latRandom                                             */
   /*                                                                   */
   /*          (as this event is only set to the host by the controller */
   /*          when the directed address is a Resolvable Private        */
   /*          Address).                                                */
   /* * NOTE * This event will only be sent if the local Bluetooth      */
   /*          controller is a 4.2 or greater compliant controller.     */
typedef struct _tagGAP_LE_Direct_Advertising_Report_Data_t
{
   GAP_LE_Address_Type_t Address_Type;
   BD_ADDR_t             BD_ADDR;
   GAP_LE_Address_Type_t Direct_Address_Type;
   BD_ADDR_t             Direct_BD_ADDR;
   SByte_t               RSSI;
} GAP_LE_Direct_Advertising_Report_Data_t;

#define GAP_LE_DIRECT_ADVERTISING_REPORT_DATA_SIZE                (sizeof(GAP_LE_Direct_Advertising_Report_Data_t))

   /* The following type declaration defines the structure of the data  */
   /* returned in a GAP etLE_Direct_Advertising_Report event.  The      */
   /* structure is used to return Advertising Reports returned from     */
   /* multiple devices.                                                 */
typedef struct _tagGAP_LE_Direct_Advertising_Report_Event_Data_t
{
   unsigned int                              Number_Device_Entries;
   GAP_LE_Direct_Advertising_Report_Data_t  *Direct_Advertising_Data;
} GAP_LE_Direct_Advertising_Report_Event_Data_t;

#define GAP_LE_DIRECT_ADVERTISING_REPORT_EVENT_DATA_SIZE          (sizeof(GAP_LE_Direct_Advertising_Report_Event_Data_t))
...
#define GAP_LE_SECURITY_REQUEST_DATA_SIZE                         (sizeof(GAP_LE_Security_Request_t))
   ...
   /* * NOTE * The crtNone type means that "Just Works" pairing method  */
   /*          is being used.                                           */
   /* * NOTE * The crtDisplayYesNo and crtOOB_SecureConnections types   */
   /*          will only been seen when Secure Connections has been     */
   /*          enabled and when Secure Connections pairing with a remote*/
   /*          device is taking place (in a                             */
   /*          latExtendedConfirmationRequest event).                   */
typedef enum
{
   crtNone,
   crtPasskey,
...
typedef struct _tagGAP_LE_Confirmation_Request_t
{
   ...
   /* The following represents the data returned in a                   */
   /* latExtendedConfirmationRequest etLE_Authentication event.         */
   /* * NOTE * The DisplayPasskey member is only valid when RequestType */
   /*          is crtDisplay OR crtDisplayYesNo, and this member should */
   /*          be display to the user to allow the remote device to     */
   /*          enter it (crtDisplay) or to compare with the             */
   /*          corresponding value on the remote device                 */
   /*          (crtDisplayYesNo) .                                      */
   /* * NOTE * The DisplayPasskey member is between 000,000 and 999,999 */
   /*          and all digits MUST BE DISPLAYED, including leading      */
   /*          ZEROS.                                                   */
   /* * NOTE * If the                                                   */
   /*          GAP_LE_EXTENDED_CONFIRMATION_REQUEST_FLAGS_SECURE_CONNECT*/
   /*          flag is set in the Flags member this indicates that      */
   /*          secure connections pairing (Version 4.2) is taking place.*/
   /* * NOTE * If the                                                   */
   /*          GAP_LE_EXTENDED_CONFIRMATION_REQUEST_FLAGS_KEYPRESS_...  */
   /*          ...NOTIFICATIONS_REQUESTED flag is set in the Flags      */
   /*          member this indicates that both sides have indicated     */
   /*          support for keypress notifications and the local device  */
   /*          shall send keypress notification while the passkey is    */
   /*          being entered.                                           */
   /* * NOTE * All Requests types must be responded to with the correct */
   /*          type via the GAP_LE_Authentication_Response() API.  THIS */
   /*          INCLUDES crtDisplay which DID NOT NEED to be responded to*/
   /*          with the LEGACY latConfirmationRequest event.            */
typedef struct _tagGAP_LE_Extended_Confirmation_Request_t
{
   GAP_LE_Confirmation_Request_Type_t Request_Type;
   DWord_t                            Display_Passkey;
   Byte_t                             Negotiated_Encryption_Key_Size;
   unsigned long                      Flags;
} GAP_LE_Extended_Confirmation_Request_t;

#define GAP_LE_EXTENDED_CONFIRMATION_REQUEST_FLAGS_SECURE_CONNECTIONS               0x00000001
#define GAP_LE_EXTENDED_CONFIRMATION_REQUEST_FLAGS_KEYPRESS_NOTIFICATIONS_REQUESTED 0x00000002
#define GAP_LE_EXTENDED_CONFIRMATION_REQUEST_FLAGS_JUST_WORKS_PAIRING               0x00000004

   /* The following represents the data returned in a                   */
   /* latExtendedOutOfBandInformation etLE_Authentication event.  This  */
   /* event provides the Out of Band Information that should be provided*/
   /* to the remote device in an Out of Band Mechanism.                 */
   /* * NOTE * This information represents the information that shall be*/
   /*          transmitted over the OOB interface to the remote device. */
typedef struct _tagGAP_LE_Extended_Out_Of_Band_Information_t
{
   GAP_LE_Address_Type_t             AddressType;
   BD_ADDR_t                         BD_ADDR;
   Secure_Connections_Confirmation_t ConfirmationValue;
   Secure_Connections_Randomizer_t   RandomNumber;
} GAP_LE_Extended_Out_Of_Band_Information_t;
...
#define GAP_LE_PAIRING_STATUS_CLASSIC_PAIRING_IN_PROGRESS                0x10
#define GAP_LE_PAIRING_STATUS_CROSS_TRANSPORT_KEY_GENERATION_NOT_ALLOWED 0x11
...
#define GAP_LE_SECURITY_ESTABLISHMENT_STATUS_CODE_TIMEOUT                  0x05
   ...
   /*                                        ...Complete is valid.      */
   /*  latKeypressNotification          - Keypress_Notification is      */
   /*                                     valid.                        */
   /*  latExtendedPairingRequest        - Extended_Pairing_Request is   */
   /*                                     valid.                        */
   /*  latExtendedConfirmationRequest   - Extended_Confirmation_Request */
   /*                                     is valid.                     */
   /*  latExtendedOutOfBandInformation  - Extended_Out_Of_Band_...      */
...
typedef struct _tagGAP_LE_Authentication_Event_Data_t
{
   ...
   union
   {
      ...
      GAP_LE_Extended_Pairing_Capabilities_t    Extended_Pairing_Request;
      GAP_LE_Security_Request_t                 Security_Request;
      GAP_LE_Confirmation_Request_t             Confirmation_Request;
      GAP_LE_Extended_Confirmation_Request_t    Extended_Confirmation_Request;
      GAP_LE_Extended_Out_Of_Band_Information_t Extended_Out_Of_Band_Information;
      GAP_LE_Pairing_Status_t                   Pairing_Status;
      GAP_LE_Keypress_t                         Keypress_Notification;
...
#define GAP_LE_AUTHENTICATED_PAYLOAD_TIMEOUT_EXPIRED_EVENT_DATA_SIZE (sizeof(GAP_LE_Authenticated_Payload_Timeout_Expired_Event_Data_t))
   ...
   /* The following type declaration specifies the information that is  */
   /* returned in a LE Data Length Change event that is sent by the     */
   /* controller whenever the maximum payload length or maximum         */
   /* transmission time of a data channel packet changes (in either Tx  */
   /* or Rx direction).                                                 */
typedef struct _tagGAP_LE_Data_Length_Change_Event_Data_t
{
   BD_ADDR_t BD_ADDR;
   Word_t    MaxTxOctets;
   Word_t    MaxTxTime;
   Word_t    MaxRxOctets;
   Word_t    MaxRxTime;
} GAP_LE_Data_Length_Change_Event_Data_t;

#define GAP_LE_DATA_LENGTH_CHANGE_EVENT_DATA_SIZE                 (sizeof(GAP_LE_Data_Length_Change_Event_Data_t))
...
typedef struct _tagGAP_LE_Event_Data_t
{
   ...
   union
   {
      ...
      GAP_LE_Direct_Advertising_Report_Event_Data_t             *GAP_LE_Direct_Advertising_Report_Event_Data;
      ...
      GAP_LE_Data_Length_Change_Event_Data_t                    *GAP_LE_Data_Length_Change_Event_Data;
   } Event_Data;
} GAP_LE_Event_Data_t;
...
BTPSAPI_DECLARATION int BTPSAPI GAP_Parse_Extended_Inquiry_Response_Data(Extended_Inquiry_Response_Data_t *Extended_Inquiry_Response_Data, GAP_Extended_Inquiry_Response_Data_t *GAP_Extended_Inquiry_Response_Data);
   ...
   /* The following function provides a mechanism of initiating a LE    */
   /* (Security Manager Protocol) Pairing Process on a BR/EDR ACL Link. */
   /* This function DOES NOT initiate a LE Pairing process on a LE ACL  */
   /* Link.                                                             */
   /* * NOTE * This function may only be used if BR/EDR Secure          */
   /*          Connections (4.1) is locally supported and BR/EDR Secure */
   /*          Connections pairing HAS OCCURRED SUCCESSFULLY with the   */
   /*          specified remote device.                                 */
   /* * NOTE * This function may only be used if the local Bluetooth    */
   /*          Controller is a 4.2 or greater controller.               */
   /* * NOTE * A BR/EDR ACL link MUST be active for this function to be */
   /*          called (i.e.  one will not be initiated).                */
   /* * NOTE * If this function returns sucess (ZERO) then the events   */
   /*          will come from the LE API via the registered LE Remote   */
   /*          Authentication API.                                      */
BTPSAPI_DECLARATION int BTPSAPI GAP_Initiate_LE_Pairing(unsigned int BluetoothStackID, BD_ADDR_t BD_ADDR, GAP_LE_Extended_Pairing_Capabilities_t *Extended_Capabilities, Byte_t LinkKeyType, Link_Key_t *LinkKey);

#ifdef INCLUDE_BLUETOOTH_API_PROTOTYPES
   typedef int (BTPSAPI *PFN_GAP_Initiate_LE_Pairing_t)(unsigned int BluetoothStackID, BD_ADDR_t BD_ADDR, GAP_LE_Extended_Pairing_Capabilities_t *Extended_Capabilities, Byte_t LinkKeyType, Link_Key_t *LinkKey);
#endif

   /* The following function provides a mechanism of responding to a    */
   /* remotely initiated LE (Security Manager Protocol) Pairing Process */
   /* on a BR/EDR ACL Link.  This function DOES NOT respond to remotely */
   /* initiated LE Pairing process on a LE ACL Link.                    */
   /* * NOTE * This function may only be used if BR/EDR Secure          */
   /*          Connections (4.1) is locally supported and BR/EDR Secure */
   /*          Connections pairing HAS OCCURRED SUCCESSFULLY with the   */
   /*          specified remote device.                                 */
   /* * NOTE * This function may only be used if the local Bluetooth    */
   /*          Controller is a 4.2 or greater controller.               */
   /* * NOTE * The Extended_Capabilities parameter is optional.  If it  */
   /*          is valid then the request will be accepted.  If the      */
   /*          Extended_Capabilities parameter is NOT valid (i.e.  NULL)*/
   /*          then the request will be rejected.                       */
   /* * NOTE * If this function returns sucess (ZERO) then the events   */
   /*          will come from the LE API via the registered LE Remote   */
   /*          Authentication API.                                      */
BTPSAPI_DECLARATION int BTPSAPI GAP_Respond_LE_Pairing_Request(unsigned int BluetoothStackID, BD_ADDR_t BD_ADDR, GAP_LE_Extended_Pairing_Capabilities_t *Extended_Capabilities, Byte_t LinkKeyType, Link_Key_t *LinkKey);

#ifdef INCLUDE_BLUETOOTH_API_PROTOTYPES
   typedef int (BTPSAPI *PFN_GAP_Respond_LE_Pairing_Request_t)(unsigned int BluetoothStackID, BD_ADDR_t BD_ADDR, GAP_LE_Extended_Pairing_Capabilities_t *Extended_Capabilities, Byte_t LinkKeyType, Link_Key_t *LinkKey);
#endif
   ...
   /* connection to a remote device the GAP_LE_Advertising_Enable()     */
   ...
   /* * NOTE * This function is asynchronous in nature and the caller is*/
   ...
   /*          device) that was received from the remote device (along  */
   /*          with the remote device's IRK) during a successful        */
   /*          LE Pairing process.  Note these two types are only valid */
   ...
   /*          (latResolvableFallbackXXX) then the RemoteAddressType and*/
   /*          RemoteAddress parameters must be specified and valid as  */
   /*          they will be used to search the Resolving List to find   */
   /*          the local IRK to use when creating the connection.  Note */
   /*          these two types are only valid for this API if the local */
   /*          controller is a version 4.2 or greater Bluetooth         */
   /*          controller.                                              */
   /* * NOTE * If the local Bluetooth controller is a version 4.0 or 4.1*/
   /*          compliant controller then ONLY the latPublic/latRandom   */
...
BTPSAPI_DECLARATION int BTPSAPI GAP_LE_Read_Remote_Features(unsigned int BluetoothStackID, BD_ADDR_t BD_ADDR);
   ...
   /*                                                                   */
   /*          Note if the final two enumerated types are specified     */
   /*          (latResolvableFallbackXXX) then the controller will      */
   ...
   /*          address (latResolvableFallbackPublic) or the random      */
   /*          address set via GAP_LE_Set_Random_Address()              */
   /*          (latResolvableFallbackRandom).  Note these two types are */
   /*          only valid for this API if the local controller is a     */
   /*          version 4.2 or greater Bluetooth controller.             */
   /* * NOTE * If the local Bluetooth controller is a version 4.0 or 4.1*/
   /*          compliant controller then ONLY the latPublic/latRandom   */
   /*          types may be specified for the LocalAddressType          */
   /*          parameter.                                               */
   /* * NOTE * The following enumerated types may be used with the      */
   ...
   /*             fpWhiteListDirectedRPA                                */
   /*                                                                   */
   /*          Note that for the fpNoWhiteListDirectedRPA and           */
   /*          fpWhiteListDirectedRPA types they may only be used if    */
   /*          the local controller is a 4.2 or greater compliant       */
   /*          controller.                                              */
   /* * NOTE * If either of the fpNoWhiteListDirectedRPA or             */
   /*          fpWhiteListDirectedRPA types is specified for the        */
   /*          FilterPolicy the application can expect to receive the   */
   /*          etLE_Direct_Advertising_Report event when a directed     */
   /*          advertisement that is directed towards a resolvable      */
   /*          private address is received by the controller.           */
   /* * NOTE * If the fpNoWhiteListDirectedRPA type is used for the     */
   /*          FilterPolicy parameter then the device can expect to     */
   /*          receive advertising reports based on the following       */
   /*          criteria:                                                */
   /*                                                                   */
   /*             - all un-directed advertising packets                 */
   ...
   /*          receive etLE_Direct_Advertising_Report events when       */
   ...
   /* * NOTE * If the fpWhiteListDirectedRPA type is used for the       */
   /*          FilterPolicy parameter then the device can expect to     */
   /*          receive advertising reports based on the following       */
   /*          criteria:                                                */
   /*                                                                   */
   /*             - all un-directed advertising packets from devices    */
...
BTPSAPI_DECLARATION int BTPSAPI GAP_LE_Set_Random_Address(unsigned int BluetoothStackID, BD_ADDR_t RandomAddress);
   ...
   /* zero on success, or a negative return error code if there was an  */
   /* error.                                                            */
   /* * NOTE * This function will return BTPS_ERROR_WHITE_LIST_IN_USE if*/
   /*          a scanning, advertising or a connection establishment    */
...
BTPSAPI_DECLARATION int BTPSAPI GAP_LE_Read_White_List_Size(unsigned int BluetoothStackID, unsigned int *WhiteListSize);
   ...
   /* pairability mode.  This function will return zero on success, or a*/
   /* negative return error code if there was an error.                 */
...
BTPSAPI_DECLARATION int BTPSAPI GAP_LE_Pair_Remote_Device(unsigned int BluetoothStackID, BD_ADDR_t BD_ADDR, GAP_LE_Pairing_Capabilities_t *Capabilities, GAP_LE_Event_Callback_t GAP_LE_Event_Callback, unsigned long CallbackParameter);
   ...
   /* The following function is provided to allow a means of pairing to */
   /* a remote device with extended pairing capabilities.  The first    */
   /* parameter is the Bluetooth stack ID of Bluetooth device.  The next*/
   /* is the device address of the LE connection to initate pairing,    */
   /* followed by the extended pairing capabilities of the local device.*/
   /* The final parameters specify the GAP LE event callback through    */
   /* which all further GAP LE authentication events will be sent for   */
   /* the pairing operation specified by BD_ADDR.  This function will   */
   /* return zero on success, or a negative return error code if there  */
   /* was an error.                                                     */
   /* * NOTE * This function can only be issued by a master because the */
   /*          slave device is not allowed to perform security          */
   ...
   /*          remote device must already exist before calling this     */
   /*          function.                                                */
BTPSAPI_DECLARATION int BTPSAPI GAP_LE_Extended_Pair_Remote_Device(unsigned int BluetoothStackID, BD_ADDR_t BD_ADDR, GAP_LE_Extended_Pairing_Capabilities_t *Extended_Capabilities, GAP_LE_Event_Callback_t GAP_LE_Event_Callback, unsigned long CallbackParameter);

#ifdef INCLUDE_BLUETOOTH_API_PROTOTYPES
   typedef int (BTPSAPI *PFN_GAP_LE_Extended_Pair_Remote_Device_t)(unsigned int BluetoothStackID, BD_ADDR_t BD_ADDR, GAP_LE_Extended_Pairing_Capabilities_t *Extended_Capabilities, GAP_LE_Event_Callback_t GAP_LE_Event_Callback, unsigned long CallbackParameter);
#endif
...
BTPSAPI_DECLARATION int BTPSAPI GAP_LE_Request_Security(unsigned int BluetoothStackID, BD_ADDR_t BD_ADDR, GAP_LE_Bonding_Type_t Bonding_Type, Boolean_t MITM, GAP_LE_Event_Callback_t GAP_LE_Event_Callback, unsigned long CallbackParameter);
   ...
   /* The following function is provided to allow a means of requesting */
   /* that the remote device begin a security process with extended     */
   /* parameters (either pairing or re-establishing previously          */
   /* exstablished security).  This function accepts the Bluetooth stack*/
   /* ID followed by the address of the remote device, followed by the  */
   /* security information required to be passed to the remote device.  */
   /* This function returns zero if successful or a negative return     */
   /* error code if there was an error.                                 */
   /* * NOTE * This function can only be issued by a slave (because the */
   /*          master is responsible for performing the security        */
   ...
   /*          remote device must already exist before calling this     */
   /*          function.                                                */
BTPSAPI_DECLARATION int BTPSAPI GAP_LE_Extended_Request_Security(unsigned int BluetoothStackID, BD_ADDR_t BD_ADDR, GAP_LE_Extended_Pairing_Capabilities_t *ExtendedCapabilities, GAP_LE_Event_Callback_t GAP_LE_Event_Callback, unsigned long CallbackParameter);

#ifdef INCLUDE_BLUETOOTH_API_PROTOTYPES
   typedef int (BTPSAPI *PFN_GAP_LE_Extended_Request_Security_t)(unsigned int BluetoothStackID, BD_ADDR_t BD_ADDR, GAP_LE_Extended_Pairing_Capabilities_t *ExtendedCapabilities, GAP_LE_Event_Callback_t GAP_LE_Event_Callback, unsigned long CallbackParameter);
#endif
...
BTPSAPI_DECLARATION int BTPSAPI GAP_LE_Set_Fixed_Passkey(unsigned int BluetoothStackID, DWord_t *Fixed_Display_Passkey);
   ...
   /* The following function is a utility function which is provided to */
   /* allow the application to force this module to update the local    */
   /* P-256 Public/Private Key Pair that is used for LE Secure          */
   /* Connections pairing (Version 4.2).  The function accepts as it's  */
   /* only parameter the Bluetooth Stack ID of the local Bluetooth      */
   /* Device.  This function returns zero if successful or a negative   */
   /* return error code if there was an error.                          */
   /* * NOTE * This function may not be called when pairing is in       */
   /*          progress.                                                */
BTPSAPI_DECLARATION int BTPSAPI GAP_LE_Update_Local_P256_Public_Key(unsigned int BluetoothStackID);

#ifdef INCLUDE_BLUETOOTH_API_PROTOTYPES
   typedef int (BTPSAPI *PFN_GAP_LE_Update_Local_P256_Public_Key_t)(unsigned int BluetoothStackID);
#endif
...
BTPSAPI_DECLARATION int BTPSAPI GAP_LE_Query_Connection_Handle(unsigned int BluetoothStackID, BD_ADDR_t BD_ADDR, Word_t *Connection_Handle);
   ...
   /* The following function is provided to allow a means to query the  */
   /* Bluetooth Board Addresses of the local and remote Bluetooth Low   */
   /* Energy Devices which were used when establishing the connection   */
   /* associated with a Bluetooth LE Connection Handle.  This function  */
   /* accepts as it's input parameters the Bluetooth Protocol Stack ID  */
   /* of the Bluetooth Protocol Stack that the connection exists, the   */
   /* Connection Handle for the connection, and a pointer to a variable */
   /* that will receive the Bluetooth Board Addresses and Types of      */
   /* the Local and Remote Bluetooth Low Energy Devices with which      */
   /* the specified Connection Handle is associated.  This function     */
   /* will return zero on success, or a negative return error code if   */
   /* there was an error.  If this function returns success, then the   */
   /* Addresses variable will contain the Bluetooth Board Addresses and */
   /* Types for the respective devices associated with the specified    */
   /* connection.                                                       */
   /* * NOTE * If this function returns with an error, negative value,  */
   /*          the value returned in the Addresses variable should be   */
   /*          considered invalid.                                      */
   /* * NOTE * The following enumerated types may be returned in the    */
   /*          GAP_LE_Address_Pair_t Local_Address_Type member:         */
   /*                                                                   */
   /*             latPublic                                             */
   /*             latRandom                                             */
   /*                                                                   */
   /* * NOTE * The following enumerated types may be returned in the    */
   /*          GAP_LE_Address_Pair_t Remote_Address_Type member:        */
   /*                                                                   */
   /*             latPublic                                             */
   /*             latRandom                                             */
   /*             latPublicIdentity                                     */
   /*             latRandomIdentity                                     */
BTPSAPI_DECLARATION int BTPSAPI GAP_LE_Query_Connection_Addresses(unsigned int BluetoothStackID, Word_t Connection_Handle, GAP_LE_Address_Pair_t *Addresses);

#ifdef INCLUDE_BLUETOOTH_API_PROTOTYPES
   typedef int (BTPSAPI *PFN_GAP_LE_Query_Connection_Addresses_t)(unsigned int BluetoothStackID, Word_t Connection_Handle, GAP_LE_Address_Pair_t *Addresses);
#endif
...
BTPSAPI_DECLARATION int BTPSAPI GAP_LE_Query_Authenticated_Payload_Timeout(unsigned int BluetoothStackID, BD_ADDR_t BD_ADDR, Word_t *AuthenticatedPayloadTimeout);
   ...
   /* The following function is provided to allow a means to set either */
   /* the resolvable private address (RPA) timeout for RPA addresses.   */
   /* This function accepts as it's input parameters the Bluetooth      */
   /* Protocol Stack ID of the Bluetooth Protocol Stack that the        */
   /* connection exists, and the resolvable private address timeout     */
   /* (specified in seconds).  This function will return zero on        */
   /* success, or a negative return error code if there was an error.   */
   /* * NOTE * This function can only be called if the local Bluetooth  */
   /*          device is a 4.2 or greater chipset.                      */
   /* * NOTE * When the controller is generating RPA addresses for use  */
   /*          in over the air transactions the timeout defines the     */
   /*          amount of time an RPA will be used until the controller  */
   /*          autonomously generates a new one.                        */
BTPSAPI_DECLARATION int BTPSAPI GAP_LE_Set_Resolvable_Private_Address_Timeout(unsigned int BluetoothStackID, unsigned int RPA_Timeout);

#ifdef INCLUDE_BLUETOOTH_API_PROTOTYPES
   typedef int (BTPSAPI *PFN_GAP_LE_Set_Resolvable_Private_Address_Timeout_t)(unsigned int BluetoothStackID, unsigned int RPA_Timeout);
#endif

   /* The following function is provided to allow a means to set either */
   /* enable or disable the address resolution feature of the controller*/
   /* (v4.2 or greater).  This function accepts as it's input parameters*/
   /* the Bluetooth Protocol Stack ID of the Bluetooth Protocol Stack   */
   /* that the connection exists, and a boolean to indicate whether the */
   /* address resolution feature is to be enabled (TRUE) or disabled    */
   /* (FALSE).  This function will return zero on success, or a negative*/
   /* return error code if there was an error.                          */
   /* * NOTE * This function can only be called if the local Bluetooth  */
   /*          device is a 4.2 or greater chipset.                      */
BTPSAPI_DECLARATION int BTPSAPI GAP_LE_Set_Address_Resolution_Enable(unsigned int BluetoothStackID, Boolean_t EnableAddressResolution);

#ifdef INCLUDE_BLUETOOTH_API_PROTOTYPES
   typedef int (BTPSAPI *PFN_GAP_LE_Set_Address_Resolution_Enable_t)(unsigned int BluetoothStackID, Boolean_t EnableAddressResolution);
#endif

   /* The following function is provided to allow a means to manage the */
   /* resolving list which is stored in the Bluetooth device which is   */
   /* specified by the first parameter.  This function is responsible   */
   /* for adding one (or more) devices to the resolving list.  The first*/
   /* parameter is the Bluetooth stack ID of the Bluetooth stack that is*/
   /* controlling the local device.  The second parameters specifies the*/
   /* number of devices to add to the resolving list (one or more).  The*/
   /* next parameter is a pointer to an array of resolving list members */
   /* (of at least DeviceCount entries) that specify the device(s) to   */
   /* add to the resolving list.  The final parameter is a pointer to a */
   /* buffer that will contain the number of devices in the             */
   /* ResolvingListEntries array that were successfully added to the    */
   /* Bluetooth Controller's resolving List (on return from this        */
   /* function).  This function will return zero on success, or a       */
   /* negative return error code if there was an error.                 */
BTPSAPI_DECLARATION int BTPSAPI GAP_LE_Add_Device_To_Resolving_List(unsigned int BluetoothStackID, unsigned int DeviceCount, GAP_LE_Resolving_List_Entry_t *ResolvingListEntries, unsigned int *AddedDeviceCount);

#ifdef INCLUDE_BLUETOOTH_API_PROTOTYPES
   typedef int (BTPSAPI *PFN_GAP_LE_Add_Device_To_Resolving_List_t)(unsigned int BluetoothStackID, unsigned int DeviceCount, GAP_LE_Resolving_List_Entry_t *ResolvingListEntries, unsigned int *AddedDeviceCount);
#endif

   /* The following function is provided to allow a means to manage the */
   /* resolving list which is stored in the Bluetooth device which is   */
   /* specified by the first parameter.  This function is responsible   */
   /* for removing devices to the resolving list.  The first parameter  */
   /* is the Bluetooth stack ID of the Bluetooth stack that is          */
   /* controlling the local device.  The second parameters specifies the*/
   /* number of devices to remove from the resolving list.  The final   */
   /* parameters points to an array of resolving list members (of at    */
   /* least DeviceCount entries) that specify the device(s) to remove   */
   /* from the resolving list and a pointer to return the number of     */
   /* devices in ResolvingListEntries that were succesfully removed from*/
   /* the Bluetooth Controller's resolving List (upon return from this  */
   /* function).  This function will return zero on success, or a       */
   /* negative return error code if there was an error.                 */
   /* * NOTE * Specifying a DeviceCount of zero instructs the module to */
   /*          delete ALL members from the resolving list (i.e.  clear  */
   /*          the resolving list).                                     */
   /* * NOTE * The Peer_IRK and Local_IRK members of the                */
   /*          GAP_LE_Resolving_List_Entry_t structure are not used by  */
   /*          this function.                                           */
BTPSAPI_DECLARATION int BTPSAPI GAP_LE_Remove_Device_From_Resolving_List(unsigned int BluetoothStackID, unsigned int DeviceCount, GAP_LE_Resolving_List_Entry_t *ResolvingListEntries, unsigned int *RemovedDeviceCount);

#ifdef INCLUDE_BLUETOOTH_API_PROTOTYPES
   typedef int (BTPSAPI *PFN_GAP_LE_Remove_Device_From_Resolving_List_t)(unsigned int BluetoothStackID, unsigned int DeviceCount, GAP_LE_Resolving_List_Entry_t *ResolvingListEntries, unsigned int *RemovedDeviceCount);
#endif

   /* The following function is provided to allow a means of reading the*/
   /* resolving list size for the Bluetooth device specified by the     */
   /* Bluetooth stack ID parameter.  The next parameter is a pointer to */
   /* an integer where the resolving List size will be returned if this */
   /* function returns success.  This function will return zero on      */
   /* success, or a negative return error code if there was an error.   */
   /* * NOTE * The returned resolving list size is returned in terms of */
   /*          the number of device entries it may hold (not individual */
   /*          bytes).                                                  */
BTPSAPI_DECLARATION int BTPSAPI GAP_LE_Read_Resolving_List_Size(unsigned int BluetoothStackID, unsigned int *ResolvingListSize);

#ifdef INCLUDE_BLUETOOTH_API_PROTOTYPES
   typedef int (BTPSAPI *PFN_GAP_LE_Read_Resolving_List_Size_t)(unsigned int BluetoothStackID, unsigned int *ResolvingListSize);
#endif

   /* The following function is provided to allow a means to suggest    */
   /* maximum Tx Packet size and time for the specified connection to   */
   /* the specified device.  This function accepts as it's input        */
   /* parameters the Bluetooth Protocol Stack ID of the Bluetooth       */
   /* Protocol Stack that the connection exists, the Bluetooth Board    */
   /* Address of the Remote LE Bluetooth Device which is connected, and */
   /* the Suggested Tx Packet Size and Time.  This function will return */
   /* zero on success, or a negative return error code if there was an  */
   /* error.                                                            */
   /* * NOTE * If this function returns BTPS_ERROR_DEVICE_NOT_CONNECTED */
   /*          a connection to the specified Bluetooth Board Address    */
   ...
   /* * NOTE * This function can only be called if the local Bluetooth  */
   /*          device is a 4.2 or greater chipset.                      */
   /* * NOTE * This function is only used to make suggestions to the    */
   /*          controller for the specified connection.  The controller */
   /*          is allowed to make it's own decision on the maximum      */
   /*          packet size/time based on other factors.                 */
   /* * NOTE * The etLE_Data_Length_Change event is dispatched to the   */
   /*          application anytime the data lengths used for a          */
   /*          connection have changed (this can happen anytime even if */
   /*          this API is not used).                                   */
BTPSAPI_DECLARATION int BTPSAPI GAP_LE_Set_Data_Length(unsigned int BluetoothStackID, BD_ADDR_t BD_ADDR, Word_t SuggestedTxPacketSize, Word_t SuggestedTxPacketTime);

#ifdef INCLUDE_BLUETOOTH_API_PROTOTYPES
   typedef int (BTPSAPI *PFN_GAP_LE_Set_Data_Length_t)(unsigned int BluetoothStackID, BD_ADDR_t BD_ADDR, Word_t SuggestedTxPacketSize, Word_t SuggestedTxPacketTime);
#endif

   /* The following function is provided to allow a means to suggest    */
   /* maximum Tx Packet size and time for new LE connection.  This      */
   /* function accepts as it's input parameters the Bluetooth Protocol  */
   /* Stack ID of the Bluetooth Protocol Stack that the connection      */
   /* exists, and the Suggested Tx Packet Size and Time.  This function */
   /* will return zero on success, or a negative return error code if   */
   /* there was an error.                                               */
   /* * NOTE * This function can only be called if the local Bluetooth  */
   /*          device is a 4.2 or greater chipset.                      */
   /* * NOTE * This function is only used to make suggestions to the    */
   /*          controller for new connections.  The controller is       */
   /*          allowed to make it's own decision on the maximum packet  */
   /*          size/time based on other factors.                        */
   /* * NOTE * The etLE_Data_Length_Change event is dispatched to the   */
   /*          application anytime the data lengths used for a          */
   /*          connection have changed (this can happen anytime even if */
   /*          this API is not used).                                   */
BTPSAPI_DECLARATION int BTPSAPI GAP_LE_Set_Default_Data_Length(unsigned int BluetoothStackID, Word_t SuggestedTxPacketSize, Word_t SuggestedTxPacketTime);

#ifdef INCLUDE_BLUETOOTH_API_PROTOTYPES
   typedef int (BTPSAPI *PFN_GAP_LE_Set_Default_Data_Length_t)(unsigned int BluetoothStackID, Word_t SuggestedTxPacketSize, Word_t SuggestedTxPacketTime);
#endif

   /* The following function is provided to allow a means to query the  */
   /* suggested max Tx Packet Size and Time for new LE connections from */
   /* the controller.  This function accepts as it's input parameters   */
   /* the Bluetooth Protocol Stack ID of the Bluetooth Protocol Stack   */
   /* that the connection exists, and the Suggested Tx Packet Size and  */
   /* Time.  This function will return zero on success, or a negative   */
   /* return error code if there was an error.                          */
   /* * NOTE * This function can only be called if the local Bluetooth  */
   /*          device is a 4.2 or greater chipset.                      */
BTPSAPI_DECLARATION int BTPSAPI GAP_LE_Query_Default_Data_Length(unsigned int BluetoothStackID, Word_t *SuggestedTxPacketSize, Word_t *SuggestedTxPacketTime);

#ifdef INCLUDE_BLUETOOTH_API_PROTOTYPES
   typedef int (BTPSAPI *PFN_GAP_LE_Query_Default_Data_Length_t)(unsigned int BluetoothStackID, Word_t *SuggestedTxPacketSize, Word_t *SuggestedTxPacketTime);
#endif

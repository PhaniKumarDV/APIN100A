                               ...
                               struct device_attribute *attr, char *buf)
{
    ...
    if(hw_caps &
        1 << IEEE80211_MODE_11AC_VHT20)
        strcat(buf, "11AC_VHT20 ");
    if(hw_caps &
        1 << IEEE80211_MODE_11AC_VHT40PLUS)
        strcat(buf, "11AC_VHT40PLUS ");
    if(hw_caps &
        1 << IEEE80211_MODE_11AC_VHT40MINUS)
        strcat(buf, "11AC_VHT40MINUS ");
    if(hw_caps &
        1 << IEEE80211_MODE_11AC_VHT40)
        strcat(buf, "11AC_VHT40 ");
    if(hw_caps &
        1 << IEEE80211_MODE_11AC_VHT80)
        strcat(buf, "11AC_VHT80 ");
    if(hw_caps &
        1 << IEEE80211_MODE_11AC_VHT160)
        strcat(buf, "11AC_VHT160 ");
    if(hw_caps &
        1 << IEEE80211_MODE_11AC_VHT80_80)
        strcat(buf, "11AC_VHT80_80 ");
...
static ssize_t wifi_ol_ciphercaps_show(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    if ( ciphercaps & 1<<IEEE80211_CIPHER_WEP ) {
        strncat(buf, "wep40,", sizeof("wep40,"));
        strncat(buf, "wep104,", sizeof("wep104,"));
        if (  scn->is_ar900b )
            strncat(buf, "wep128,", sizeof("wep128,"));
    }
    if ( ciphercaps & 1<<IEEE80211_CIPHER_TKIP )
        strncat(buf, "tkip,", sizeof("tkip,"));
    if ( ciphercaps & 1<<IEEE80211_CIPHER_AES_OCB )
        strncat(buf, "aes-ocb,", sizeof("aes-ocb,"));
    if ( ciphercaps & 1<<IEEE80211_CIPHER_AES_CCM )
        strncat(buf, "aes-ccmp-128,", sizeof("aes-ccmp-128,"));
    if ( ciphercaps & 1<<IEEE80211_CIPHER_AES_CCM_256 )
        strncat(buf, "aes-ccmp-256,", sizeof("aes-ccmp-256,"));
    if ( ciphercaps & 1<<IEEE80211_CIPHER_AES_GCM )
        strncat(buf, "aes-gcmp-128,", sizeof("aes-gcmp-128,"));
    if ( ciphercaps & 1<<IEEE80211_CIPHER_AES_GCM_256 )
        strncat(buf, "aes-gcmp-256,", sizeof("aes-gcmp-256,"));
    if ( ciphercaps & 1<<IEEE80211_CIPHER_CKIP )
        strncat(buf, "ckip,", sizeof("ckip,"));
    if ( ciphercaps & 1<<IEEE80211_CIPHER_WAPI )
        strncat(buf, "wapi,", sizeof("wapi,"));
    if ( ciphercaps & 1<<IEEE80211_CIPHER_AES_CMAC )
        strncat(buf, "aes-cmac-128,", sizeof("aes-cmac-128,"));
    if ( ciphercaps & 1<<IEEE80211_CIPHER_AES_CMAC_256  )
        strncat(buf, "aes-cmac-256,", sizeof("aes-cmac-256,"));
    if ( ciphercaps & 1<<IEEE80211_CIPHER_AES_GMAC )
        strncat(buf, "aes-gmac-128,", sizeof("aes-gmac-128,"));
    if ( ciphercaps & 1<<IEEE80211_CIPHER_AES_GMAC_256 )
        strncat(buf, "aes-gmac-256,", sizeof("aes-gmac-256,"));
    if ( ciphercaps & 1<<IEEE80211_CIPHER_NONE )
        strncat(buf, "none", sizeof("none"));
    return strlen(buf);
}
...
ath_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
    ...
    case SIOCGATHPHYERR:
        if (!capable(CAP_NET_ADMIN)) {
            ...
            error = ol_ath_ucfg_phyerr(scn, (struct ath_diag *)ifr->ifr_data);
        }
        break;
#endif
    case SIOCETHTOOL:
...
ath_getstats(struct net_device *dev)
{
    ...
    stats->tx_packets = pdev->stats.pub.tx.delivered.pkts;
    stats->tx_bytes = pdev->stats.pub.tx.delivered.bytes;
    stats->tx_errors = pdev->stats.pub.tx.dropped.host_reject.pkts
    ...
    stats->rx_packets = pdev->stats.pub.rx.delivered.pkts
                        + pdev->stats.pub.rx.forwarded.pkts;
    stats->rx_bytes = pdev->stats.pub.rx.delivered.bytes
...
#define LTEU_SUPPORT2    0x4
    ...
    if ( EOK != ath_lowi_if_netlink_init()) {
        printk("LOWI driver intferace socket init failed __investigate__ \n");
    }
#endif
...
#ifndef AMIGO_BOARD_REV_PATH
#define AMIGO_BOARD_REV_PATH      "/tmp/sysinfo/amigo_board_rev"
#endif

#ifndef AMIGO_BOARD_REV_APH50C_2G
#define AMIGO_BOARD_REV_APH50C_2G      "boardData_1_0_IPQ4019_Y9803_wifi0_APH50C.bin"
#endif

#ifndef AMIGO_BOARD_REV_APH50C_5G
#define AMIGO_BOARD_REV_APH50C_5G      "boardData_1_0_IPQ4019_Y9803_wifi1_APH50C.bin"
#endif

char absolute_filename2[MAX_ABSOLUTE_FILE_SIZE] ;
                    ...
                    u_int32_t address, bool compressed)
{
    ...
    switch (file)
    {
		...
		/* QDART condition only */
                if(testmode == 1) {
                    ...
                    strncat(absolute_filename, pathname, qdf_str_len(pathname));
                    strncat(absolute_filename, filename, qdf_str_len(filename));
                    ...
                    qdf_mem_set(buf, MAX_FILENAME_BUFF_SIZE, 0);
                    qdf_mem_set(absolute_filename,MAX_ABSOLUTE_FILE_SIZE, 0);
                    qdf_mem_set(absolute_filename2,MAX_ABSOLUTE_FILE_SIZE, 0);
                    qdf_mem_set(amigo_board_id,16, 0);
                    ...
                    board_rev_filename = AMIGO_BOARD_REV_PATH;
                    fbid = filp_open(board_rev_filename, O_RDONLY, 0);
                    if(IS_ERR(fbid))
                    {
                        printk("%s[%d], Error: Open %s failed.\n", __func__, __LINE__, board_rev_filename);
                        strncat(absolute_filename,buf,qdf_str_len(buf));
                    }
                    else
                    {
                        //printk("Open amigo_board_rev file ok\n");
                        fs = get_fs();
                        set_fs(get_ds());
                        if(fbid->f_op->read(fbid, amigo_board_id, 16, &fbid->f_pos) <= 0 )
                        {
                           printk(KERN_ERR "%s[%d], Error: Reading %s failed.\n", __func__, __LINE__, board_rev_filename);
                           set_fs(fs);
                           filp_close(fbid,NULL);
                           strncat(absolute_filename,buf,qdf_str_len(buf));
                        }
                        else
                        {
                           //printk("amigo_board_rev %s\n", amigo_board_id);
                           set_fs(fs);
                           filp_close(fbid,NULL);

                           if(!qdf_mem_cmp(scn->sc_osdev->netdev->name, "wifi1", 5))
                              is_5G=true;
                           if(!strncmp("APH50C", amigo_board_id, sizeof("APH50C") - 1))
                           {
                              printk(KERN_ERR "amigo_board_rev APH50C\n");
                              if(!is_5G)
                                 strncat(absolute_filename,AMIGO_BOARD_REV_APH50C_2G,qdf_str_len(AMIGO_BOARD_REV_APH50C_2G));
                              else
                                 strncat(absolute_filename,AMIGO_BOARD_REV_APH50C_5G,qdf_str_len(AMIGO_BOARD_REV_APH50C_5G));
                           }
                           else
                              match=false;
                           ...
                           if(match)
                           {
                             /* Check file is exist */
                             fbid = NULL;
                             strncat(absolute_filename2,"/lib/firmware/",sizeof("/lib/firmware/"));
                             strncat(absolute_filename2,absolute_filename,qdf_str_len(absolute_filename));
                             fbid = filp_open(absolute_filename2, O_RDONLY, 0);
                             if(IS_ERR(fbid))
                             {
                                printk("%s[%d], Error: Open %s failed.\n", __func__, __LINE__, absolute_filename2);
                                qdf_mem_set(absolute_filename,MAX_ABSOLUTE_FILE_SIZE, 0);
                                strncat(absolute_filename,filename,qdf_str_len(filename));
                                strncat(absolute_filename,buf,qdf_str_len(buf));
                             }
    ...
    if ((file == ATH_BOARD_DATA_FILE) || (file == ATH_FLASH_FILE))
    {
        ...
        {
            printk("BMIReadMemory for Board extended Data download failed \n");
#if FW_CODE_SIGN
            release_secure_firmware(fw_entry);
#else
            release_firmware(fw_entry);

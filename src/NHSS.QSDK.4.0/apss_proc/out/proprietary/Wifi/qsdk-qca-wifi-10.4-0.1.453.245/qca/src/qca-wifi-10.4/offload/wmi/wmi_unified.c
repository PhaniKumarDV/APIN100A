...
wmi_unified_cmd_send(wmi_unified_t wmi_handle, wmi_buf_t buf, int len, WMI_CMD_ID cmd_id)
{
    ...
    if (!cookie) {
        printk("%s : Cookie alloc Failed for cmd_id : %0x, WMI handle = %p\n",__func__,cmd_id,wmi_handle);
        wbuf_free(buf);
        return -ENOMEM;
    }
...
#endif
{
    ...
    printk(KERN_INFO"%s:  WMI UNIFIED SERVICE READY event \n", __func__);
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,5,0)
    status = create_queue_work(wmi_handle->scn_handle, evt_buf);
    if(status) {
        printk("%s: Failed to queue work to handle coherent memory alloc \n", __func__);
    }
#else
...
static int wmi_circ_init(struct wmi_unified *h)
{
  ...
  if (!is_pow_of_2(wmi_ring_size)) {
    printk(KERN_ERR "ring size should be power of 2..\n");
    return 0;
  }
  ...
  if(!wmi_circ->wmi) {
    printk(KERN_ERR "no memory for ring buffer..\n");
    return 0;
  }
...
void wmi_add_to_circ(wmi_unified_t h, char *cmd, int len, WMI_CMD_ID id)
{
  ...
  if (!wmi_circ->wmi) {
    printk(KERN_ERR "wmi ring buffer not initialized!\n");
    return;
  }
                                   ...
                                   size_t count, loff_t *ppos)
{
  ...
  sscanf(buf, "%d", &k);
  if(k != 0) {
    printk(KERN_ERR "Wrong input, echo 0 to clear the wmi buffer\n");
    return -EINVAL;
  }

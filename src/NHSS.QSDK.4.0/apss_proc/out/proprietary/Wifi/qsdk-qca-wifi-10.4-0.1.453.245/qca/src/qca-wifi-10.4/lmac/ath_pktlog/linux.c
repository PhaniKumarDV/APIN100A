                ...
                ppos)
{
    ...
    if (pl_dev && ((pl_dev->pl_info->log_state) & ATH_PKTLOG_TX_CAPTURE_ENABLE)) {
         ...
         * pktlog flags.
         */
        printk("Cannot enable tx capture : Check the flag value 0x%x\n", (pl_dev->pl_info->log_state));
        return -1;
    }
    ...
    if (write) {
        ...
        if (ret == 0) {
            ...
            if (pl_dev && (((pl_dev->pl_info->log_state) & ATH_PKTLOG_TX_CAPTURE_ENABLE) == 0)) {
                printk("pl_dev:%p; pl_funcs:%p; pktlog_enable:%p; scn:%p\n",
                                        pl_dev, pl_dev->pl_funcs,
                                        pl_dev->pl_funcs->pktlog_enable,
                                        pl_dev->scn);
                return pl_dev->pl_funcs->pktlog_enable(scn, enable);
            } else {
                 printk("Cannot enable pktlog as Tx Capture has been enabled\n");
            }
        } else {
...
pktlog_read(struct file *file, char *buf, size_t nbytes, loff_t *ppos)
{
    ...
    if (!pktlog_hdr_size) {
        ...
        printk("%s: WARNING: pl_info->pktlog_hdr_size = %u, Using pktlog_hdr_size as: %u\n",

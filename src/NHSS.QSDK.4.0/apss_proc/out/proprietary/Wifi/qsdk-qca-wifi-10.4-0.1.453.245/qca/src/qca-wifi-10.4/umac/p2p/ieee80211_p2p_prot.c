                                     ...
                                     void *event_arg, ieee80211_p2p_prot_event_handler ev_handler)
{
    ...
    if (ev_handler == NULL) {
        ...
        ieee80211com_note(dev_handle, "%s: Error: No event handler callback parameter.\n",__func__);
        return NULL;
    }
    ...
    if (prot_handle == NULL) {
        ieee80211com_note(dev_handle, "%s p2p protocol device allocation failed\n",__func__);
        return NULL;
    }
    ...
    do {
        ...
        if (p2pdev_handle == NULL) {
            ieee80211com_note(dev_handle, "%s p2p device creation failed\n",__func__);
            status = -EIO;
            break;
        }

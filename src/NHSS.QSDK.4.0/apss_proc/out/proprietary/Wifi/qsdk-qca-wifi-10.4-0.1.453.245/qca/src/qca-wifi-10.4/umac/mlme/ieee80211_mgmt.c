...
#define BEACON_INFO_ELEMENT_OFFSET    (offsetof(struct ieee80211_beacon_frame, info_elements))
    ...
    u_int8_t                         bchan, sig_vht_op_ch_freq_seg2;
    ...
    if (bchan != 0) {
        ...
        if (scan_entry_parameters->chan != NULL) {
            ...
            if (scan_entry_parameters->ie_list.vhtop != NULL) {
                ...
                if (IS_REVSIG_VHT160(ap_vhtop)) {
                    sig_vht_op_ch_freq_seg2 = 0;
                }
                else {
        ...
        case IEEE80211_ACTION_HT_SMPOWERSAVE:
            ...
            if (iasmpowersave->as_control & IEEE80211_A_HT_SMPOWERSAVE_ENABLED) {
                if (iasmpowersave->as_control & IEEE80211_A_HT_SMPOWERSAVE_MODE) {
                    if ((vap->iv_dynamic_mimo_ps) && ((ni->ni_htcap & IEEE80211_HTCAP_C_SM_MASK) !=
                    ...
                    if ((vap->iv_smps) && ((ni->ni_htcap & IEEE80211_HTCAP_C_SM_MASK) !=
            ...
            IEEE80211_NOTE_MAC(vap, IEEE80211_MSG_ACTION, ni->ni_macaddr,
                   "%s: HT SM pwrsave action:   ni_htcap=0x%x, \
                   ni_updaterates=0x%x, vap->iv_dynamic_mimo_ps=0x%x, \
                   vap->iv_smps=0x%x, iasmpowersave->as_control=0x%x  ", __func__, ni->ni_htcap, \
                   ni->ni_updaterates, vap->iv_dynamic_mimo_ps, vap->iv_smps, iasmpowersave->as_control);

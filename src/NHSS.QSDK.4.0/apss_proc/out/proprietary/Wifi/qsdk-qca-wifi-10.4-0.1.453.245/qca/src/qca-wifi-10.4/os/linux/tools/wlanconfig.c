...
usage(void)
{
	...
	fprintf(stderr, "usage: wlanconfig athX ald rem-addr <mac_addr>\n");
...
static void
showatftable(const char *ifname)
{
...
static void
set_addsta_pval(const char *ifname, char *macaddr, char *val)
{
        int s, cnt = 0;
        uint8_t *buf;
        struct iwreq iwr;
...
print_chaninfo(const struct ieee80211_channel *c, const struct ieee80211_channel *c_160)
{
    ...
    if(IEEE80211_IS_CHAN_11AC_VHT160(c_160)) {
        ...
        sprintf(buf1, "%3u", c_160->ic_vhtop_ch_freq_seg2);
        strlcat(buf, buf1, sizeof(buf));
    }
			...
			char *addr1, char *addr2)
{
    ...
    struct iwreq iwr;
    struct ieee80211_wlanconfig *config;
    char wds_ni_macaddr[17] = {0}, wds_macaddr[17] = {0}, data[4096];
...
static int handle_vendorie(const char *ifname, IEEE80211_WLANCONFIG_CMDTYPE cmdtype, int len, char *in_oui, char *in_cap_info, char *in_ftype_map)
{
    ...
    if (cmdtype == IEEE80211_WLANCONFIG_VENDOR_IE_ADD || cmdtype == IEEE80211_WLANCONFIG_VENDOR_IE_UPDATE) {
        ...
        strncpy(oui, in_oui, VENDORIE_OUI_LEN *2 +1);
        ...
        strncpy(cap_info, in_cap_info, (len - VENDORIE_OUI_LEN) *2 + 1);
        ...
        strncpy(ftype_map, in_ftype_map, 2 + 1);

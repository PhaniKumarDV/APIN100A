...
ol_txrx_peer_unref_delete(ol_txrx_peer_handle peer)
{
    ...
    if (qdf_atomic_dec_and_test(&peer->ref_cnt)) {
        ...
        if (found) {
             ...
             printk ("WARN peer %p not found in vdev (%p)->peer_list:%p\n", peer, vdev,
                        &peer->vdev->peer_list);
        }
    ...
    struct ol_txrx_stats_req *req)
{
    ...
    qdf_print("Tx pkts completed\t:  %u\n", stats->tx.delivered.pkts);
    qdf_print("Tx bytes completed\t:  %u\n", stats->tx.delivered.bytes);
    qdf_print("Tx pkts from stack\t:  %u\n", stats->tx.from_stack.pkts);

    qdf_print("\n");
    qdf_print("++++++++++ HOST RX STATISTICS +++++++++++\n");
    qdf_print("Rx pkts completed\t:  %u\n", stats->rx.delivered.pkts);
    qdf_print("Rx bytes completed\t:  %u\n", stats->rx.delivered.bytes);

    qdf_print("++++++++++ HOST FLOW CONTROL STATISTICS +++++++++++\n");
    qdf_print("Receive from stack count: %u\n", stats->tx.from_stack.pkts);

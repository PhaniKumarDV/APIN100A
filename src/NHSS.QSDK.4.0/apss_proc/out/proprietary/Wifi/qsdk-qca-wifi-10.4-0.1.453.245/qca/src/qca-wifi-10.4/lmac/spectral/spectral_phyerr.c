...
int dump_search_fft_report(SPECTRAL_PHYERR_TLV* ptlv, int tlvlen, bool is_160_format)
{
    ...
    printk("Header A = 0x%x Header B = 0x%x\n", phdr->hdr_a, phdr->hdr_b);
        ...
        bool is_160_format)
{
    ...
    if (datalen < sizeof(SPECTRAL_PHYERR_TLV)) {
        printk("DRIVER: Total PHY error data length %u too short to contain "
               "any TLVs\n", datalen);
        return -1;
    }
    ...
    while (bytes_processed < datalen) {
        if (bytes_remaining < sizeof(SPECTRAL_PHYERR_TLV)) {
            printk("DRIVER: Remaining PHY error data length %u too short to "
                   "contain a TLV\n", bytes_remaining);
            return -1;
        }
        ...
        if (curr_tlv_complete_size > bytes_remaining) {
            printk("DRIVER : Current indicated complete TLV size %u greater than"

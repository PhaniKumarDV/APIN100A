...
#  $Header: //source/qcom/qct/core/pkg/bootloaders/rel/1.2/boot_images/core/storage/tools/jsdcc/partition_load_pt/ptool.py#7 $
#  $DateTime: 2012/03/21 12:46:22 $ 
#  $Author: coresvc $
...
def CreateGPTPartitionTable(PhysicalPartitionNumber):
	...
	if HashInstructions['PERFORMANCE_BOUNDARY_IN_KB']>0 and HashInstructions['ALIGN_PARTITIONS_TO_PERFORMANCE_BOUNDARY'] is False:
        ...
        if HashInstructions['ALIGN_PARTITIONS_TO_PERFORMANCE_BOUNDARY'] is True:
            ...
            SectorsTillNextBoundary = ReturnNumSectorsTillBoundary(FirstLBA,HashInstructions['PERFORMANCE_BOUNDARY_IN_KB'])
            if SectorsTillNextBoundary>0:
                ...
                ##print "\tHashInstructions['PERFORMANCE_BOUNDARY_IN_KB']=",HashInstructions['PERFORMANCE_BOUNDARY_IN_KB']
        ...
        if HashInstructions['PERFORMANCE_BOUNDARY_IN_KB']>0:
            AlignedRemainder = FirstLBA % HashInstructions['PERFORMANCE_BOUNDARY_IN_KB'];
            if AlignedRemainder==0:
                print "\t"+"-"*78
                print "\tPERF: This partition is ** ALIGNED ** to a %i KB boundary at sector %i (boundary %i)" % (HashInstructions['PERFORMANCE_BOUNDARY_IN_KB'],FirstLBA,FirstLBA/(ConvertKBtoSectors(HashInstructions['PERFORMANCE_BOUNDARY_IN_KB'])))
                print "\t"+"-"*78

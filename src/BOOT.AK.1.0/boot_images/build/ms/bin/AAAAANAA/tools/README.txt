...
[1] Image creation:
	To create all the images using the configuration given in config/ directory
  use the command: "python genimg.py --nor_tool=nor_tool --mbn_gen=nand_mbn_generator.py
  ...
  --bootconfig_gen=bootconfig_gen.py"
  provided the nor_tool, nand_mbn_generator.py, cdt_mod.py, smem_tool.py, cb-cdt.mbn,
  bootconfig_gen.py and the config directory are present in the current directory. 
  If the listed tools are moved then give the corresponding path (e.g --nor_tool=/tmp/nor_tool)
  in the arguments.

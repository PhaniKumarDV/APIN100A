...
disable_qcawifi() {
	...
	#add WIFI Led, Vince, {
	if [ "$1" = "wifi0" ]; then
	echo 0 > /sys/class/leds/wlan24/brightness
	elif [ "$1" = "wifi1" ]; then
	echo 0 > /sys/class/leds/wlan5/brightness
	fi
	#add WIFI Led, Vince, }
...
enable_qcawifi() {
	...
	for vif in $vifs; do
		...
		#add WIFI Led, Vince, {
	        if [ "$ifname" = "ath0" ]; then
	        echo 1 > /sys/class/leds/wlan24/brightness
	        elif [ "$ifname" = "ath1" ]; then
	        echo 1 > /sys/class/leds/wlan5/brightness
	        fi
		#add WIFI Led, Vince, {

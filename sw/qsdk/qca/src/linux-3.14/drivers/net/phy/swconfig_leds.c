...
swconfig_led_work_func(struct work_struct *work)
{
	...
	for (i = 0; i < SWCONFIG_LED_NUM_PORTS; i++) {
		...
		//Add traffic indication for LAN ports, Vince {
		else {
			struct net_device *dev;
			struct rtnl_link_stats64 temp;
			dev = dev_get_by_name(&init_net, "eth1");
			const struct rtnl_link_stats64 *stats = dev_get_stats(dev, &temp);
			if (i==4)// port mask=0x10
			{
			    sw_trig->port_traffic[i] = stats->tx_bytes +stats->rx_bytes;
			    //printk("[AMIGO]swconfig_led_work_func (%x)(%lx)(%d)(%lx)(%lx)\n",port_mask,sw_trig->port_traffic[i],i,stats->tx_bytes, stats->rx_bytes);
			}

			dev = dev_get_by_name(&init_net, "eth0");
			stats = dev_get_stats(dev, &temp);
			if (i==5)// port mask=0x20
			{
			    sw_trig->port_traffic[i] = stats->tx_bytes +stats->rx_bytes;
			    //printk("[AMIGO]swconfig_led_work_func(%x)(%lx)(%d)(%lx)(%lx)\n",port_mask,sw_trig->port_traffic[i],i,stats->tx_bytes, stats->rx_bytes);
			}

...
int fdt_chosen(void *fdt, int force)
{
	int   nodeoffset;
	int   err;
	char  *str_env;		/* used to set string properties */
	const char *path;
	 ...
	 * is true.
	 */
	str_env = getenv("bootargs");
	strcpy(str, str_env);
	if (str != NULL) {
		{
			char  *str_serial=0;
			char * serial_cmdline[128];
			char * str_country;
			char * country_code[32];
			char * str_board_rev;
			char * board_rev[40];
			str_serial = getenv("amigoserial");
			if(str_serial)
			{
				sprintf(serial_cmdline," amigoserial=%s",str_serial);
				strncat(str,serial_cmdline,strlen(serial_cmdline));
			}
			str_country = getenv("country_code");
			if(str_country)
			{
				sprintf(country_code," country_code=%s",str_country);
				strncat(str,country_code,strlen(country_code));
			}
			str_board_rev = getenv("amigo_board_rev");
			if(str_board_rev)
			{
				sprintf(board_rev," amigo_board_rev=%s",str_board_rev);
				strncat(str,board_rev,strlen(board_rev));
			}
		}
